...
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct Block {
    pub block_data: [u8; MAX_BLOCK_SIZE as usize], // bytes(MaxBlockSize): fixed buffer, used length beside it (SPEC §4.7)
    pub block_data_length: i32,
}
...
impl Default for Block {
    fn default() -> Self {
        Block {
            block_data: [0; MAX_BLOCK_SIZE as usize],
            block_data_length: 0,
        }
    }
}
...
pub fn write_block(stream: &mut WriteStream<'_>, value: &Block) -> Result {
    ...
    }
    {
        ...
        let mut bytes_value = value.block_data;
        stream.serialize_bytes(&mut bytes_value[..value.block_data_length as usize])?;
    }
    Ok(())
}

pub fn read_block(stream: &mut ReadStream<'_>, value: &mut Block) -> Result {
    stream.serialize_int(&mut value.block_data_length, 0, MAX_BLOCK_SIZE as i32)?; // the length guards the slice (§6.3)
    stream.serialize_bytes(&mut value.block_data[..value.block_data_length as usize])?;
    Ok(())
}
...
pub fn write_timescale(stream: &mut WriteStream<'_>, value: &Timescale) -> Result {
    {
        let mut float_value = value.timescale;
...
pub fn read_timescale(stream: &mut ReadStream<'_>, value: &mut Timescale) -> Result {
    stream.serialize_f64(&mut value.timescale)?;
