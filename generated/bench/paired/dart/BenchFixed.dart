...
/// plan compiled from a stranger's layout hold the SAME bounds. A COUNTED
/// ARRAY WALKS ITS LIVE COUNT, never the bound: slack is never clamped.
...
void benchMixedFixedWriteBody(
  ...
  for (var i = 0; i < value.entitiesCount; i++) {
  ...
  for (var i = 0; i < value.statsCount; i++) {
  ...
  bytes.setRange(at + 1122, at + 1122 + 4, value.loadout);
  ...
  bytes.setRange(
    at + 1130,
    at + 1130 + value.playerNameLength,
    value.playerName,
  );
  bytes.fillRange(at + 1130 + value.playerNameLength, at + 1145, 0);
  assert(value.payloadLength >= 0);
  assert(value.payloadLength <= 16);
  view.setInt32(at + 1145, value.payloadLength, Endian.little);
  bytes.setRange(at + 1149, at + 1149 + value.payloadLength, value.payload);
  bytes.fillRange(at + 1149 + value.payloadLength, at + 1165, 0);
