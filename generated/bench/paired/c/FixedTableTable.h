...
static SCHEMA_UNUSED const uint64_t fixed_table_fixed_hash = 0x32f1c4a302a224ebull; /* fnv1a64 over the block's bytes */
...
static SCHEMA_UNUSED const uint8_t fixed_table_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t fixed_table_fixed_vocab_bytes = (int64_t) sizeof( fixed_table_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( fixed_table_fixed_vocab ) + count * ( 8 + 1236 );
}
...
static SCHEMA_UNUSED int64_t fixed_table_fixed_save( const FixedTable * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( fixed_table_fixed_vocab ) );
    memcpy( buffer + 5, fixed_table_fixed_vocab, sizeof( fixed_table_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( fixed_table_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != fixed_table_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, fixed_table_fixed_vocab, (int32_t) sizeof( fixed_table_fixed_vocab ), fixed_table_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
