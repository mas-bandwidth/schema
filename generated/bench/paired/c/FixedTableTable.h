...
/* FixedTable's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by the
...
static SCHEMA_UNUSED SCHEMA_BENCH_TABLE_INLINE void schema_bench_fixed_table_fixed_scatter_( const uint8_t * SCHEMA_TABLE_RESTRICT image, FixedTable * value, TableReport * report )
{
    ...
    { /* stats count */
        int32_t v = (int32_t) table_fixed_get32( image + 464 );
    ...
    { /* player_name */
        int32_t v = (int32_t) table_fixed_get32( image + 1126 );
    ...
    { /* payload */
        int32_t v = (int32_t) table_fixed_get32( image + 1145 );
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        for ( k = 0; k < count; ++k )
        {
            ...
            memcpy( image, at + 8, (size_t) 1236 );
            schema_bench_fixed_table_fixed_scatter_( image, values + k, report );
            schema_bench_fixed_table_fixed_clamp_( values + k, report ); /* the bounds the scatter does not hold (§3.4) */
            at += 8 + 1236;
        }
        return count;
    }
