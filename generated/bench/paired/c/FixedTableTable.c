...
static const TableFieldInfo schema_bench_fixed_table_fields_[] = {
    { "value", "value", "BenchMixed", 0x7ce4fd9430e80ceaull, 13, 0, 0, 0, 0, (uint32_t) offsetof( FixedTable, value ), (uint32_t) sizeof( ( (FixedTable *) 0 )->value ), 0xffffffffu, 0xffffffffu, &schema_bench_bench_mixed_info_, 0, 0.0, 0.0, NULL, 0, -1, NULL, 0, NULL, NULL, -1, NULL, "", TableDocNone, 0, NULL, 0, NULL },
};

const TableTypeInfo schema_bench_fixed_table_info_ = { "FixedTable", (uint32_t) sizeof( FixedTable ), 1, schema_bench_fixed_table_fields_, schema_bench_fixed_table_reset_raw_, TableDocNone, 0, NULL };

int schema_bench_fixed_table_from_json_( FixedTable * value, const char * text, int64_t bytes, TableReport * report )
{
    return table_json_read( value, &schema_bench_fixed_table_info_, text, bytes, report );
}

int64_t schema_bench_fixed_table_to_json_( const FixedTable * value, char * buffer, int64_t capacity )
{
    return table_json_write( value, &schema_bench_fixed_table_info_, buffer, capacity );
}
