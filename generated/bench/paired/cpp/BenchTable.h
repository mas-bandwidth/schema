...
BENCH_TABLE_INLINE bool MixedEntitySaveBody( TableWriter & w, TableIds & ids, const MixedEntity & value )
{
    ...
    {
        w.header( ids.ref( 0x23fcfd6678e36712ull ), 7 ); // entity_id
        w.put16( uint16_t( value.entity_id ) );
    }
    if ( value.pos_x != 0 )
    {
        w.header( ids.ref( 0xcb4b37357667310eull ), 4 ); // pos_x
        w.put32( uint32_t( value.pos_x ) );
    }
    if ( value.pos_y != 0 )
    {
        w.header( ids.ref( 0xcb4b3835766732c1ull ), 4 ); // pos_y
        w.put32( uint32_t( value.pos_y ) );
    }
    if ( value.pos_z != 0 )
    {
        w.header( ids.ref( 0xcb4b353576672da8ull ), 4 ); // pos_z
        w.put32( uint32_t( value.pos_z ) );
    }
    if ( value.yaw != 0 )
    {
        w.header( ids.ref( 0xb54d8e19798e16e8ull ), 7 ); // yaw
        w.put16( uint16_t( value.yaw ) );
    }
    if ( value.pitch != 0 )
    {
        w.header( ids.ref( 0x53a9f665a90cc1b1ull ), 7 ); // pitch
        w.put16( uint16_t( value.pitch ) );
    }
    if ( value.vel_x != 0 )
    {
        w.header( ids.ref( 0x6cede6b6eb60ee67ull ), 4 ); // vel_x
        w.put32( uint32_t( value.vel_x ) );
    }
    if ( value.vel_y != 0 )
    {
        w.header( ids.ref( 0x6cede5b6eb60ecb4ull ), 4 ); // vel_y
        w.put32( uint32_t( value.vel_y ) );
    }
    if ( value.vel_z != 0 )
    {
        w.header( ids.ref( 0x6cede8b6eb60f1cdull ), 4 ); // vel_z
    ...
    if ( value.damage != 0 )
    {
        w.header( ids.ref( 0x7f6308be8ab37fc0ull ), 9 ); // damage
        w.put64( uint64_t( value.damage ) );
    }
    if ( value.moving != false )
    {
        w.header( ids.ref( 0x11a44fc1d1243da7ull ), 1 ); // moving
        w.put8( value.moving ? 1 : 0 );
    }
    if ( value.firing != false )
    {
        w.header( ids.ref( 0x7674cfd19b9031caull ), 1 ); // firing
...
BENCH_TABLE_INLINE bool MixedStatSaveBody( TableWriter & w, TableIds & ids, const MixedStat & value )
{
    if ( value.stat_id != 0 )
    {
        w.header( ids.ref( 0x80ab75f0866dbf65ull ), 6 ); // stat_id
        w.put8( uint8_t( value.stat_id ) );
    }
    if ( value.delta != 0 )
    {
        w.header( ids.ref( 0x52076675ec13a0c1ull ), 4 ); // delta
...
BENCH_TABLE_INLINE bool MixedHitEventSaveBody( TableWriter & w, TableIds & ids, const MixedHitEvent & value )
{
    ...
    {
        w.header( ids.ref( 0xb7bc9ac015a25050ull ), 7 ); // target_id
        w.put16( uint16_t( value.target_id ) );
    }
    if ( value.damage != 0 )
    {
        w.header( ids.ref( 0x7f6308be8ab37fc0ull ), 4 ); // damage
        w.put32( uint32_t( value.damage ) );
    }
    if ( value.hit_kind != 0 )
    {
        w.header( ids.ref( 0x01fbc365b059b925ull ), 4 ); // hit_kind
        w.put32( uint32_t( value.hit_kind ) );
    }
    if ( value.crit != false )
    {
        w.header( ids.ref( 0x126167908c9aa52dull ), 1 ); // crit
...
BENCH_TABLE_INLINE bool MixedChatEventSaveBody( TableWriter & w, TableIds & ids, const MixedChatEvent & value )
{
    if ( value.channel != 0 )
    {
        w.header( ids.ref( 0xa5013e9ad5caeda4ull ), 4 ); // channel
        w.put32( uint32_t( value.channel ) );
    }
    if ( value.speaker != 0 )
    {
        w.header( ids.ref( 0xfbf1ac4d96ebd022ull ), 7 ); // speaker
...
BENCH_TABLE_INLINE bool MixedPickupEventSaveBody( TableWriter & w, TableIds & ids, const MixedPickupEvent & value )
{
    if ( value.item_id != 0 )
    {
        w.header( ids.ref( 0x9e7fd06d864fbd56ull ), 7 ); // item_id
        w.put16( uint16_t( value.item_id ) );
    }
    if ( value.amount != 0 )
    {
        w.header( ids.ref( 0x8113fe7ea2b16969ull ), 4 ); // amount
...
inline int64_t BenchMixedMeasureBody( TableIds & ids, const BenchMixed & value )
{
    ...
    if ( value.nonce != 0 ) { bytes += TableLebBytes( ids.ref( 0x73a94c71d60dc0d8ull ) ) + 1 + 8; } // nonce
    ...
    if ( value.server_time != 0 ) { bytes += TableLebBytes( ids.ref( 0x3c460475f9be69c6ull ) ) + 1 + 4; } // server_time
    ...
    if ( value.wide_key != 0 ) { bytes += TableLebBytes( ids.ref( 0xa3348580461faf16ull ) ) + 1 + 16; } // wide_key
    if ( value.flux != 0 ) { bytes += TableLebBytes( ids.ref( 0xd61bdd7908af2642ull ) ) + 1 + 16; } // flux
    if ( value.ping != 0 ) { bytes += TableLebBytes( ids.ref( 0xbf30e00dc53307a9ull ) ) + 1 + 2; } // ping
    if ( value.crc_hint != 0 ) { bytes += TableLebBytes( ids.ref( 0x560d6527ccd8515full ) ) + 1 + 4; } // crc_hint
    if ( value.has_extra != true ) { bytes += TableLebBytes( ids.ref( 0xc08292176cfd8672ull ) ) + 1 + 1; } // has_extra
    if ( value.has_extra )
    {
...
BENCH_TABLE_INLINE bool BenchMixedSaveBody( TableWriter & w, TableIds & ids, const BenchMixed & value )
{
    if ( value.sequence != 0 )
    {
        w.header( ids.ref( 0xaa38aca481f528a8ull ), 7 ); // sequence
        w.put16( uint16_t( value.sequence ) );
    }
    if ( value.ack_sequence != 0 )
    {
        w.header( ids.ref( 0x0dbe005c56697c3eull ), 4 ); // ack_sequence
        w.put32( uint32_t( value.ack_sequence ) );
    }
    if ( value.ack_bits != 0 )
    {
        w.header( ids.ref( 0x9bae0da8b829ee03ull ), 8 ); // ack_bits
        w.put32( uint32_t( value.ack_bits ) );
    }
    if ( value.session_id != 0 )
    {
        w.header( ids.ref( 0xb7d7b5650a590b05ull ), 9 ); // session_id
        w.put64( uint64_t( value.session_id ) );
    }
    if ( value.client_id != 0 )
    {
        w.header( ids.ref( 0x6d7b98e2d095967eull ), 8 ); // client_id
        w.put32( uint32_t( value.client_id ) );
    }
    if ( value.nonce != 0 )
    {
        w.header( ids.ref( 0x73a94c71d60dc0d8ull ), 9 ); // nonce
        w.put64( uint64_t( value.nonce ) );
    }
    if ( value.world_time != 0 )
    {
        w.header( ids.ref( 0x3eee6b51be54fc85ull ), 5 ); // world_time
        w.put64( uint64_t( value.world_time ) );
    }
    if ( value.frame_tick != 0 )
    {
        w.header( ids.ref( 0x7bbc035f7b6d0112ull ), 9 ); // frame_tick
        w.put64( uint64_t( value.frame_tick ) );
    }
    if ( value.server_time != 0 )
    {
        w.header( ids.ref( 0x3c460475f9be69c6ull ), 22 ); // server_time
    ...
    {
        w.header( ids.ref( 0x2e35dc5321aa5790ull ), 15 ); // game_event
        switch ( value.game_event.type )
        {
            case MixedEventType::Hit:
            {
    ...
    {
        w.header( ids.ref( 0x13a62f542cf8e86eull ), 12 ); // player_name
    ...
    if ( value.aim_x != 0.0f )
    {
        w.header( ids.ref( 0xdbaf7be5e24296c9ull ), 10 ); // aim_x
        w.put32( table_float_to_bits( value.aim_x ) );
    }
    if ( value.aim_y != 0.0f )
    {
        w.header( ids.ref( 0xdbaf7ae5e2429516ull ), 10 ); // aim_y
        w.put32( table_float_to_bits( value.aim_y ) );
    }
    if ( value.aim_z != 0.0f )
    {
        w.header( ids.ref( 0xdbaf79e5e2429363ull ), 10 ); // aim_z
        w.put32( table_float_to_bits( value.aim_z ) );
    }
    if ( value.recoil != 0.0f )
    {
        w.header( ids.ref( 0x9cef31fff7e2e457ull ), 10 ); // recoil
        w.put32( table_float_to_bits( value.recoil ) );
    }
    if ( value.drift != 0.0 )
    {
        w.header( ids.ref( 0x5ab3f9c9341f6c04ull ), 11 ); // drift
        w.put64( table_double_to_bits( value.drift ) );
    }
    if ( value.wide_key != 0 )
    {
        w.header( ids.ref( 0xa3348580461faf16ull ), 19 ); // wide_key
    ...
    if ( value.ping != 0 )
    {
        w.header( ids.ref( 0xbf30e00dc53307a9ull ), 26 ); // ping
        w.put16( uint16_t( value.ping ) );
    }
    if ( value.crc_hint != 0 )
    {
        w.header( ids.ref( 0x560d6527ccd8515full ), 8 ); // crc_hint
        w.put32( uint32_t( value.crc_hint ) );
    }
    if ( value.has_extra != true )
    {
        w.header( ids.ref( 0xc08292176cfd8672ull ), 1 ); // has_extra
    ...
    if ( value.has_extra )
    {
        if ( value.extra != 0 )
        {
            w.header( ids.ref( 0xfd29ee12a979cb69ull ), 4 ); // extra
    ...
    if ( !value.has_extra )
    {
        ...
        {
            w.header( ids.ref( 0x78101ac0aa8cbcfeull ), 4 ); // idle_ticks
