...
// The guard is not vestigial. Several benchtable Table.cpp files may be
...
#ifndef BENCHTABLE_SCHEMA_TABLE_JSON
#define BENCHTABLE_SCHEMA_TABLE_JSON
...
#endif // BENCHTABLE_SCHEMA_TABLE_JSON

namespace benchtable {

bool TableEntityFromJson( TableEntity & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, TableEntityTableType(), text, bytes, report );
}

int64_t TableEntityToJsonMeasure( const TableEntity & value )
{
    return TableJsonWrite( &value, TableEntityTableType(), NULL, 0 );
}

int64_t TableEntityToJson( const TableEntity & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, TableEntityTableType(), buffer, capacity );
}

bool TableStatFromJson( TableStat & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, TableStatTableType(), text, bytes, report );
}

int64_t TableStatToJsonMeasure( const TableStat & value )
{
    return TableJsonWrite( &value, TableStatTableType(), NULL, 0 );
}

int64_t TableStatToJson( const TableStat & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, TableStatTableType(), buffer, capacity );
}

bool TableMixedFromJson( TableMixedBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    TableMixed * root = builder.GetRoot();
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, TableMixedTableType(), text, bytes, report );
}

int64_t TableMixedToJsonMeasure( const TableMixed * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TableMixedTableType(), NULL, 0, allocator );
}

int64_t TableMixedToJson( const TableMixed * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TableMixedTableType(), buffer, capacity, allocator );
}

bool TableHitEventFromJson( TableHitEvent & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, TableHitEventTableType(), text, bytes, report );
}

int64_t TableHitEventToJsonMeasure( const TableHitEvent & value )
{
    return TableJsonWrite( &value, TableHitEventTableType(), NULL, 0 );
}

int64_t TableHitEventToJson( const TableHitEvent & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, TableHitEventTableType(), buffer, capacity );
}

bool TableChatEventFromJson( TableChatEvent & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, TableChatEventTableType(), text, bytes, report );
}

int64_t TableChatEventToJsonMeasure( const TableChatEvent & value )
{
    return TableJsonWrite( &value, TableChatEventTableType(), NULL, 0 );
}

int64_t TableChatEventToJson( const TableChatEvent & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, TableChatEventTableType(), buffer, capacity );
}

bool TablePickupEventFromJson( TablePickupEvent & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, TablePickupEventTableType(), text, bytes, report );
}

int64_t TablePickupEventToJsonMeasure( const TablePickupEvent & value )
{
    return TableJsonWrite( &value, TablePickupEventTableType(), NULL, 0 );
}

int64_t TablePickupEventToJson( const TablePickupEvent & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, TablePickupEventTableType(), buffer, capacity );
}
