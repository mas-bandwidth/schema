...
BENCHTABLE_TABLE_INLINE bool TableMixedSaveBody( TableWriter & w, TableIds & ids, const TableMixed & value )
{
    ...
    {
        w.header( ids.ref( 0x6a5a70d91aa115fdull ), 7 ); // protocol_magic
    ...
    {
        w.header( ids.ref( 0x3c460475f9be69c6ull ), 10 ); // server_time
    ...
    if ( value.wide_key != 0 )
    {
        w.header( ids.ref( 0xa3348580461faf16ull ), 9 ); // wide_key
        w.put64( uint64_t( value.wide_key ) );
    }
    if ( value.flux != 0 )
    {
        w.header( ids.ref( 0xd61bdd7908af2642ull ), 5 ); // flux
        w.put64( uint64_t( value.flux ) );
    }
    if ( value.ping != 0.0f )
    {
        w.header( ids.ref( 0xbf30e00dc53307a9ull ), 10 ); // ping
