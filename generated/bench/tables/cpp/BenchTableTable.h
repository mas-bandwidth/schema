...
constexpr uint64_t TableEntityFixedHash = 0x46b300904afc1aa9ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 30 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t TableEntityFixedVocab[] = {
...
constexpr int64_t TableEntityFixedVocabBytes = (int64_t) sizeof( TableEntityFixedVocab );
...
{
    return 1 + 4 + TableEntityFixedVocabBytes + count * TableEntityFixedRecordBytes;
}
...
inline int64_t TableEntityFixedSave( const TableEntity * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) TableEntityFixedVocabBytes );
    memcpy( buffer + 5, TableEntityFixedVocab, (size_t) TableEntityFixedVocabBytes );
    uint8_t * at = buffer + 5 + TableEntityFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != TableEntityFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, TableEntityFixedVocab, (int32_t) TableEntityFixedVocabBytes, TableEntityFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t TableStatFixedHash = 0x7a16907ec6b21eceull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 3 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t TableStatFixedVocab[] = {
...
constexpr int64_t TableStatFixedVocabBytes = (int64_t) sizeof( TableStatFixedVocab );
...
{
    return 1 + 4 + TableStatFixedVocabBytes + count * TableStatFixedRecordBytes;
}
...
inline int64_t TableStatFixedSave( const TableStat * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) TableStatFixedVocabBytes );
    memcpy( buffer + 5, TableStatFixedVocab, (size_t) TableStatFixedVocabBytes );
    uint8_t * at = buffer + 5 + TableStatFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != TableStatFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, TableStatFixedVocab, (int32_t) TableStatFixedVocabBytes, TableStatFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
