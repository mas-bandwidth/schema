...
{
    return 1 + 4 + TableEntityFixedLayoutBytes + count * TableEntityFixedRecordBytes;
}
...
inline int64_t TableEntityFixedSave( const TableEntity * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) TableEntityFixedLayoutBytes );
    memcpy( buffer + 5, TableEntityFixedLayout, (size_t) TableEntityFixedLayoutBytes );
    uint8_t * at = buffer + 5 + TableEntityFixedLayoutBytes;
...
{
    return 1 + 4 + TableStatFixedLayoutBytes + count * TableStatFixedRecordBytes;
}
...
inline int64_t TableStatFixedSave( const TableStat * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) TableStatFixedLayoutBytes );
    memcpy( buffer + 5, TableStatFixedLayout, (size_t) TableStatFixedLayoutBytes );
    uint8_t * at = buffer + 5 + TableStatFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
