...
{
    return TableJsonRead( &value, ArmorPlateTableType(), text, bytes, report );
}

int64_t ArmorPlateToJsonMeasure( const ArmorPlate & value )
{
    return TableJsonWrite( &value, ArmorPlateTableType(), NULL, 0 );
}

int64_t ArmorPlateToJson( const ArmorPlate & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, ArmorPlateTableType(), buffer, capacity );
}

bool ArmorConfigFromJson( ArmorConfig & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, ArmorConfigTableType(), text, bytes, report );
}

int64_t ArmorConfigToJsonMeasure( const ArmorConfig & value )
{
    return TableJsonWrite( &value, ArmorConfigTableType(), NULL, 0 );
}
