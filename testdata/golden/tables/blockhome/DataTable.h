...
inline bool ArmorPlateLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, ArmorPlate & value );
inline int64_t ArmorConfigMeasureMessageBody( int64_t at, const ArmorConfig & value );
inline bool ArmorConfigSaveMessageBody( TableBitWriter & w, const ArmorConfig & value );
inline bool ArmorConfigLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, ArmorConfig & value );
inline int64_t FiringGroupMeasureMessageBody( int64_t at, const FiringGroup & value );
inline bool FiringGroupSaveMessageBody( TableBitWriter & w, const FiringGroup & value );
inline bool FiringGroupLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, FiringGroup & value );
...
inline int64_t ArmorPlateMeasureBody( TableIds & ids, const ArmorPlate & value )
{
    ...
    if ( value.thickness != 0.0 ) { bytes += TableLebBytes( ids.ref_at( 24, 0xdbae65ca25b4f315ull ) ) + 1 + 8; } // thickness
    if ( value.material != 0 ) { bytes += TableLebBytes( ids.ref_at( 0, 0x026f7568983161e0ull ) ) + 1 + 4; } // material
    if ( value.layer != 0 ) { bytes += TableLebBytes( ids.ref_at( 16, 0x84e39fec29768c56ull ) ) + 1 + 1; } // layer
    return bytes;
}
            ...
            case 0xdbae65ca25b4f315ull: // thickness
            {
                if ( kind != 11 )
                {
                    if ( TableKindWidens( kind, 11 ) )
                    {
                        ...
                        value.thickness = TableWidenF32( r.get32() );
                        r.report->widened++;
                        break;
                    }
...
inline bool ArmorPlateSaveMessageBody( TableBitWriter & w, const ArmorPlate & value )
{
    if ( value.thickness != 0.0 )
    {
        ...
        w.put( table_double_to_bits( value.thickness ), 64 );
    }
    if ( value.material != 0 )
    {
        w.put( 6, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.material ), 32 );
    }
    if ( value.layer != 0 )
    {
        w.put( 7, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.layer ), 8 );
    }
    ...
    for ( ;; )
    {
        ...
        switch ( entry.id )
        {
            case 0xdbae65ca25b4f315ull: // thickness
            {
                ...
                { uint64_t raw = 0; if ( !r.get( raw, 64 ) ) { report->malformed = true; return false; } value.thickness = table_bits_to_double( raw ); }
                break;
            }
            case 0x026f7568983161e0ull: // material
            {
...
inline int64_t ArmorPlateMeasureMessages( const ArmorPlate * values, int64_t count, TableReport * report )
{
    ...
    {
        const int64_t body = ArmorPlateMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
...
inline int64_t ArmorPlateSaveMessages( const ArmorPlate * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !ArmorPlateSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == ArmorPlateMeasureMessages( values, count, report )
}
...
inline bool ArmorPlateLoadMessages( ArmorPlate * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    ...
    *count = 0;
    ...
    {
        if ( !ArmorPlateLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
...
inline int64_t ArmorConfigMeasureBody( TableIds & ids, const ArmorConfig & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        const uint64_t ref_front = ids.ref_at( 11, 0x538b8c566e9e4b38ull );
    ...
    }
    {
        ...
        const uint64_t ref_rear = ids.ref_at( 9, 0x4ce65d1fbfdde703ull );
    ...
    if ( value.rating != 0.0f ) { bytes += TableLebBytes( ids.ref_at( 10, 0x4e79ecbefe5ff4d0ull ) ) + 1 + 4; } // rating
    if ( value.tier != 0 ) { bytes += TableLebBytes( ids.ref_at( 3, 0x1e6f84ef2eb65989ull ) ) + 1 + 1; } // tier
    return bytes;
}
...
inline int64_t ArmorConfigMeasureMessageBody( int64_t at, const ArmorConfig & value )
{
    int64_t bits = 0;
    {
        const int64_t body_front = ArmorPlateMeasureMessageBody( at + bits, value.front );
        if ( body_front < 0 ) { return -1; }
        if ( body_front > kTableMessageRefBitsHere ) // an all-default nested table elides
        {
            bits += kTableMessageRefBitsHere;
            bits += ArmorPlateMeasureMessageBody( at + bits, value.front );
        }
    }
    {
        const int64_t body_rear = ArmorPlateMeasureMessageBody( at + bits, value.rear );
        if ( body_rear < 0 ) { return -1; }
        if ( body_rear > kTableMessageRefBitsHere ) // an all-default nested table elides
        {
            bits += kTableMessageRefBitsHere;
            bits += ArmorPlateMeasureMessageBody( at + bits, value.rear );
        }
    }
    if ( value.rating != 0.0f )
    {
...
{
    {
        const int64_t body_front = ArmorPlateMeasureMessageBody( 0, value.front );
        if ( body_front < 0 ) { return false; }
        if ( body_front > kTableMessageRefBitsHere ) // an all-default nested table elides
        {
            w.put( 1, kTableMessageRefBitsHere );
            if ( !ArmorPlateSaveMessageBody( w, value.front ) ) { return false; }
        }
    }
    {
        const int64_t body_rear = ArmorPlateMeasureMessageBody( 0, value.rear );
        if ( body_rear < 0 ) { return false; }
        if ( body_rear > kTableMessageRefBitsHere ) // an all-default nested table elides
        {
            w.put( 2, kTableMessageRefBitsHere );
            if ( !ArmorPlateSaveMessageBody( w, value.rear ) ) { return false; }
        }
    }
    if ( value.rating != 0.0f )
    {
        w.put( 3, kTableMessageRefBitsHere );
        w.put( (uint64_t) table_float_to_bits( value.rating ), 32 );
    }
    if ( value.tier != 0 )
    {
        w.put( 4, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.tier ), 8 );
    }
    w.put( 0, kTableMessageRefBitsHere ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
    ...
    for ( ;; )
    {
        ...
        switch ( entry.id )
        {
            case 0x538b8c566e9e4b38ull: // front
            {
                ...
                if ( !ArmorPlateLoadMessageBody( r, vocabulary, report, index_bits, value.front ) ) { return false; }
                break;
            }
            case 0x4ce65d1fbfdde703ull: // rear
            {
                ...
                if ( !ArmorPlateLoadMessageBody( r, vocabulary, report, index_bits, value.rear ) ) { return false; }
                break;
            }
            case 0x4e79ecbefe5ff4d0ull: // rating
            {
...
// refusal is learned at MEASURE time, before a buffer is allocated, and a
// caller with more bodies calls again.
inline int64_t ArmorConfigMeasureMessages( const ArmorConfig * values, int64_t count, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    int64_t bits = 8; // the body count, a ranged integer over [1, 256]
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = ArmorConfigMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8; // the form byte, then the stream padded to a byte
...
inline int64_t ArmorConfigSaveMessages( const ArmorConfig * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    TableMessageBatch batch;
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !ArmorConfigSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == ArmorConfigMeasureMessages( values, count, report )
}
...
// BODIES 1 TO k - 1: count says k - 1, one malformed counts, and the storage
// after it is not a body.
inline bool ArmorConfigLoadMessages( ArmorConfig * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    if ( values == NULL || count == NULL ) { to->malformed = true; return false; }
    ...
    *count = 0;
    ...
    for ( int64_t i = 0; i < bodies; i++ )
    {
        if ( !ArmorConfigLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.barrel != 0 ) { bytes += TableLebBytes( ids.ref_at( 17, 0x8ece059ad360b651ull ) ) + 1 + 4; } // barrel
    if ( value.cooldown != 0.0f ) { bytes += TableLebBytes( ids.ref_at( 25, 0xdc2cbe6953343d48ull ) ) + 1 + 4; } // cooldown
    return bytes;
}
...
inline bool FiringGroupSaveMessageBody( TableBitWriter & w, const FiringGroup & value )
{
    if ( value.barrel != 0 )
    {
        ...
        w.put( (uint64_t) ( value.barrel ), 32 );
    }
    if ( value.cooldown != 0.0f )
    {
...
// refusal is learned at MEASURE time, before a buffer is allocated, and a
// caller with more bodies calls again.
inline int64_t FiringGroupMeasureMessages( const FiringGroup * values, int64_t count, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    int64_t bits = 8; // the body count, a ranged integer over [1, 256]
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = FiringGroupMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8; // the form byte, then the stream padded to a byte
...
inline int64_t FiringGroupSaveMessages( const FiringGroup * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    TableMessageBatch batch;
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !FiringGroupSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == FiringGroupMeasureMessages( values, count, report )
}
...
// BODIES 1 TO k - 1: count says k - 1, one malformed counts, and the storage
// after it is not a body.
inline bool FiringGroupLoadMessages( FiringGroup * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    if ( values == NULL || count == NULL ) { to->malformed = true; return false; }
    ...
    *count = 0;
    ...
    for ( int64_t i = 0; i < bodies; i++ )
    {
        if ( !FiringGroupLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
inline int64_t GunnerSettingsMeasureBody( TableIds & ids, const GunnerSettings & value )
{
    ...
    {
        const uint64_t ref_firing_groups = ids.ref_at( 4, 0x260b8ca6b0c3db7full );
    ...
    {
        const uint64_t ref_missile_groups = ids.ref_at( 7, 0x3c99105aa087f6bcull );
    ...
    if ( value.reload_seconds != 0.0f ) { bytes += TableLebBytes( ids.ref_at( 19, 0xad9b64728ea52486ull ) ) + 1 + 4; } // reload_seconds
    if ( value.gunner_id != 0 ) { bytes += TableLebBytes( ids.ref_at( 22, 0xbd9be53180256e02ull ) ) + 1 + 4; } // gunner_id
    return bytes;
}
...
inline int64_t GunnerSettingsMeasureMessageBody( int64_t at, const GunnerSettings & value )
{
    ...
    if ( value.firing_groups_count > 0 )
    {
        ...
        {
            const int64_t elem = FiringGroupMeasureMessageBody( at + bits, value.firing_groups[i] );
            if ( elem < 0 ) { return -1; }
            bits += elem;
        }
    }
    ...
    if ( value.missile_groups_count > 0 )
    {
        ...
        {
            const int64_t elem = FiringGroupMeasureMessageBody( at + bits, value.missile_groups[i] );
            if ( elem < 0 ) { return -1; }
            bits += elem;
        }
    }
...
inline bool GunnerSettingsSaveMessageBody( TableBitWriter & w, const GunnerSettings & value )
{
    ...
    if ( value.firing_groups_count > 0 )
    {
        ...
        w.put( (uint64_t) ( value.firing_groups_count ) - 0, 6 );
        for ( int32_t i = 0; i < value.firing_groups_count; i++ )
        {
            if ( !FiringGroupSaveMessageBody( w, value.firing_groups[i] ) ) { return false; }
        }
    }
    if ( value.missile_groups_count < 0 || value.missile_groups_count > 4 ) { return false; } // storage invariant
    if ( value.missile_groups_count > 0 )
    {
        ...
        w.put( (uint64_t) ( value.missile_groups_count ) - 0, 3 );
        for ( int32_t i = 0; i < value.missile_groups_count; i++ )
        {
            if ( !FiringGroupSaveMessageBody( w, value.missile_groups[i] ) ) { return false; }
        }
    }
    if ( value.reload_seconds != 0.0f )
    {
        w.put( 12, kTableMessageRefBitsHere );
        w.put( (uint64_t) table_float_to_bits( value.reload_seconds ), 32 );
    }
    if ( value.gunner_id != 0 )
    {
        w.put( 13, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.gunner_id ), 32 );
    }
    w.put( 0, kTableMessageRefBitsHere ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
        ...
        switch ( entry.id )
        {
            case 0x260b8ca6b0c3db7full: // firing_groups
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < walk; i++ )
                    {
                        ...
                        FiringGroup scratch;
                        FiringGroupReset( scratch );
                        if ( !FiringGroupLoadMessageBody( r, vocabulary, report, index_bits, ( in_bounds ? value.firing_groups[i] : scratch ) ) ) { return false; }
                    }
                    value.firing_groups_count = kept;
                }
                break;
            }
            case 0x3c99105aa087f6bcull: // missile_groups
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < walk; i++ )
                    {
                        ...
                        FiringGroup scratch;
                        FiringGroupReset( scratch );
                        if ( !FiringGroupLoadMessageBody( r, vocabulary, report, index_bits, ( in_bounds ? value.missile_groups[i] : scratch ) ) ) { return false; }
                    }
                    value.missile_groups_count = kept;
                }
                break;
            }
            case 0xad9b64728ea52486ull: // reload_seconds
            {
                ...
                }
                {
                    ...
                    value.reload_seconds = decoded_f;
                }
                break;
            }
            case 0xbd9be53180256e02ull: // gunner_id
...
template <typename... Args>
inline void ArmorPlateSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorPlate is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void ArmorPlateLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorPlate is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void ArmorPlateSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorPlate is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void ArmorConfigLoadRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorConfig is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}

template <typename... Args>
inline void ArmorConfigMeasureRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorConfig is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void ArmorConfigSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorConfig is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void ArmorConfigLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorConfig is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void ArmorConfigSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "ArmorConfig is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void FiringGroupLoadRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "FiringGroup is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}

template <typename... Args>
inline void FiringGroupMeasureRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "FiringGroup is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void FiringGroupSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "FiringGroup is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void FiringGroupLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "FiringGroup is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void FiringGroupSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "FiringGroup is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
