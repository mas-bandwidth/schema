...
{
    return 1 + 4 + PartRowFixedLayoutBytes + count * PartRowFixedRecordBytes;
}
...
inline int64_t PartRowFixedSave( const PartRow * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) PartRowFixedLayoutBytes );
    memcpy( buffer + 5, PartRowFixedLayout, (size_t) PartRowFixedLayoutBytes );
    uint8_t * at = buffer + 5 + PartRowFixedLayoutBytes;
...
{
    return 1 + 4 + PartFrameFixedLayoutBytes + count * PartFrameFixedRecordBytes;
}
...
inline int64_t PartFrameFixedSave( const PartFrame * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) PartFrameFixedLayoutBytes );
    memcpy( buffer + 5, PartFrameFixedLayout, (size_t) PartFrameFixedLayoutBytes );
    uint8_t * at = buffer + 5 + PartFrameFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
