...
inline int64_t PartRowMeasureBody( TableIds & ids, const PartRow & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        const uint64_t ref_armor = ids.ref( 0xd19988b67e699194ull, 17 );
    ...
    }
    {
        ...
        const uint64_t ref_gunner = ids.ref( 0x40dbb648c0cd44aaull, 18 );
    ...
    if ( value.part_id != 0 ) { bytes += TableLebBytes( ids.ref( 0x04d6206b33415104ull, 19 ) ) + 1 + 4; } // part_id
    if ( value.slot != 0 ) { bytes += TableLebBytes( ids.ref( 0x6a771618f6fe31d1ull, 20 ) ) + 1 + 1; } // slot
    return bytes;
}
...
BLOCKHOME_TABLE_INLINE bool PartRowSaveBody( TableWriter & w, TableIds & ids, const PartRow & value )
{
    ...
    if ( value.part_id != 0 )
    {
        w.putleb( ids.ref( 0x04d6206b33415104ull, 19 ) ); w.put8( 8 ); // part_id
        w.put32( uint32_t( value.part_id ) );
    }
    if ( value.slot != 0 )
    {
        w.putleb( ids.ref( 0x6a771618f6fe31d1ull, 20 ) ); w.put8( 6 ); // slot
...
inline int64_t PartRowMeasureMessage( const PartRow & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = PartRowMeasureBody( ids, value );
...
inline int64_t PartRowSaveMessage( const PartRow & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !PartRowSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == PartRowMeasureMessage( value )
}
...
inline bool PartRowLoadMessage( PartRow & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    PartRowReset( value );
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.version != 0 ) { bytes += TableLebBytes( ids.ref( 0xbb62c62c9808ea37ull, 15 ) ) + 1 + 8; } // version
    if ( value.parts_count < 0 || value.parts_count > 32 ) { return -1; } // storage invariant
    if ( value.parts_count > 0 )
    {
        const uint64_t ref_parts = ids.ref( 0x0c519da7a1f958c5ull, 16 );
...
BLOCKHOME_TABLE_INLINE bool PartFrameSaveBody( TableWriter & w, TableIds & ids, const PartFrame & value )
{
    if ( value.version != 0 )
    {
        w.putleb( ids.ref( 0xbb62c62c9808ea37ull, 15 ) ); w.put8( 9 ); // version
...
// the ids live. Every reference is a compile-time SLOT, so this walk does
// no lookup and a save costs what a save costs.
inline int64_t PartFrameMeasureMessage( const PartFrame & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = PartFrameMeasureBody( ids, value );
...
inline int64_t PartFrameSaveMessage( const PartFrame & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !PartFrameSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == PartFrameMeasureMessage( value )
}
...
// form on its own and does not guess a table, because a guessed table
// decodes a body under the wrong names in silence.
inline bool PartFrameLoadMessage( PartFrame & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    PartFrameReset( value );
