...
constexpr uint64_t StampFixedHash = 0x820f5d6ffaf5e7eeull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry StampFixedPlan[] = {
    { 0u, 12u, 8u, 0u, kTableFixedNoGuard, kTableFixedText, 0, 2, 0, 0 }, // label
...
// Stamp's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
...
inline void StampFixedScatter( const uint8_t * TABLE_RESTRICT image, Stamp & value, TableReport * report )
{
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == StampFixedHash );
    ...
    if ( identity )
    {
        ...
        uint8_t image[StampFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) StampFixedBodyBytes );
            StampFixedScatter( image, values[k], report );
            at += StampFixedRecordBytes;
        }
        return n;
    }
