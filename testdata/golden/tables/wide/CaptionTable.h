...
// package wide — protocol id 0x4b9b89c718af5d5e (packets only: tables version by field id, not by protocol id)
...
#ifndef WIDE_SCHEMA_TABLE_PRIMITIVES
#define WIDE_SCHEMA_TABLE_PRIMITIVES
...
#if defined( _MSC_VER )
#define WIDE_TABLE_INLINE __forceinline
#elif defined( __GNUC__ ) || defined( __clang__ )
#define WIDE_TABLE_INLINE inline __attribute__(( always_inline ))
#else
#define WIDE_TABLE_INLINE inline
#endif
...
struct TableWriter
{
    ...
    WIDE_TABLE_INLINE void raw( const void * data, int64_t bytes )
    {
        if ( offset + bytes > capacity ) { overflow = true; return; }
        memcpy( buffer + offset, data, (size_t) bytes );
        offset += bytes;
    }
    WIDE_TABLE_INLINE void put8( uint8_t v )   { raw( &v, 1 ); }
    WIDE_TABLE_INLINE void put16( uint16_t v ) { uint8_t b[2] = { uint8_t( v ), uint8_t( v >> 8 ) }; raw( b, 2 ); }
    WIDE_TABLE_INLINE void put32( uint32_t v ) { uint8_t b[4] = { uint8_t( v ), uint8_t( v >> 8 ), uint8_t( v >> 16 ), uint8_t( v >> 24 ) }; raw( b, 4 ); }
    ...
    WIDE_TABLE_INLINE void put64( uint64_t v )
    {
    ...
    WIDE_TABLE_INLINE void putleb( uint64_t v )
    {
        uint8_t b[10];
        int64_t n = 0;
...
struct TableIds
{
    static const int32_t kCapacity = 16;
    ...
    WIDE_TABLE_INLINE uint64_t ref( uint64_t id )
    {
...
struct TableReader
{
    ...
    WIDE_TABLE_INLINE bool has( int64_t bytes ) const { return offset + bytes <= size; }
    ...
    WIDE_TABLE_INLINE bool room( uint64_t bytes ) const { return bytes <= (uint64_t) ( size - offset ); }
    WIDE_TABLE_INLINE uint8_t get8()   { return buffer[offset++]; }
    WIDE_TABLE_INLINE uint16_t get16() { uint16_t v = uint16_t( buffer[offset] ) | uint16_t( buffer[offset+1] ) << 8; offset += 2; return v; }
    WIDE_TABLE_INLINE uint32_t get32() { uint32_t v = uint32_t( buffer[offset] ) | uint32_t( buffer[offset+1] ) << 8 | uint32_t( buffer[offset+2] ) << 16 | uint32_t( buffer[offset+3] ) << 24; offset += 4; return v; }
    WIDE_TABLE_INLINE uint64_t get64() { uint64_t lo = get32(); uint64_t hi = get32(); return lo | ( hi << 32 ); }
    WIDE_TABLE_INLINE void get128( uint64_t & lo, uint64_t & hi ) { lo = get64(); hi = get64(); }
...
static const int64_t kTableMessageEntriesHere = 16;
...
// THE UNIT'S ANNOUNCEMENT, byte for byte: 16 entries and 197 bytes. It is an
...
static const uint8_t kTableAnnounce[ kTableAnnounceBytes ] = {
    ...
    ,
...
#endif // WIDE_SCHEMA_TABLE_PRIMITIVES
...
#ifndef WIDE_SCHEMA_TABLE_COOK
#define WIDE_SCHEMA_TABLE_COOK
...
#endif // WIDE_SCHEMA_TABLE_COOK

namespace wide {

// BodyType: union Body's tag — None = 0, then each variant in declared order (SPEC §4.8)
...
// arm's storage is established when the arm is selected — by BodyLoadBody
// before it decodes, or by assigning it: value.wide = a wstring(4).
...
// table Caption — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct Caption {
    char16_t title[7 + 1] = {}; // wstring(7): max length in UTF-16 code units, used length beside it
    int32_t title_length = 0;
    Line line;
    Line lines[3]; // used count beside it; count in [0, 3]
...
inline void CaptionReset( Caption & value );
inline void StampReset( Stamp & value );
inline void LineReset( Line & value );

inline void CaptionReset( Caption & value )
{
    memset( value.title, 0, sizeof( value.title ) );
    value.title_length = 0;
    LineReset( value.line );
    LineReset( value.lines[0] );
    for ( int32_t i = 1; i < 3; i++ ) { value.lines[i] = value.lines[0]; }
    value.lines_count = 0;
    memset( value.body.wide.value, 0, sizeof( value.body.wide.value ) );
    value.body.wide.value_length = 0;
    memset( value.body.narrow.value, 0, sizeof( value.body.narrow.value ) );
    value.body.narrow.value_length = 0;
    value.body.tally = 0;
    value.body.type = BodyType::None;
}
...
inline bool CaptionLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, Caption & value );
...
inline int64_t LineMeasureMessageBody( int64_t at, const Line & value );
inline bool LineSaveMessageBody( TableBitWriter & w, const Line & value );
inline bool LineLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, Line & value );

// ---- codecs: measure/save/load per closure member ----

inline int64_t CaptionMeasureBody( TableIds & ids, const Caption & value );
WIDE_TABLE_INLINE bool CaptionSaveBody( TableWriter & w, TableIds & ids, const Caption & value );
WIDE_TABLE_INLINE bool CaptionLoadBody( TableReader & r, Caption & value );
inline int64_t StampMeasureBody( TableIds & ids, const Stamp & value );
WIDE_TABLE_INLINE bool StampSaveBody( TableWriter & w, TableIds & ids, const Stamp & value );
WIDE_TABLE_INLINE bool StampLoadBody( TableReader & r, Stamp & value );
inline int64_t LineMeasureBody( TableIds & ids, const Line & value );
WIDE_TABLE_INLINE bool LineSaveBody( TableWriter & w, TableIds & ids, const Line & value );
WIDE_TABLE_INLINE bool LineLoadBody( TableReader & r, Line & value );
...
inline int64_t CaptionMeasureBody( TableIds & ids, const Caption & value )
{
    ...
    if ( value.title_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 13, 0xda31296c0c1b6029ull ) ) + 1 + TableLebBytes( (uint64_t) ( (int64_t) value.title_length * 2 ) ) + ( (int64_t) value.title_length * 2 ); } // title
    {
        ...
        else { ids.truncate( mark_line ); } // an all-default nested table elides, and costs no entry
    }
    ...
    if ( value.lines_count > 0 )
    {
        ...
        bytes += TableLebBytes( ref_lines ) + 1 + TableLebBytes( (uint64_t) ( body_lines ) ) + ( body_lines ); // lines
    }
...
inline int64_t CaptionMeasure( const Caption & value )
{
    TableIds ids;
    const int64_t body = CaptionMeasureBody( ids, value );
...
inline int64_t CaptionSave( const Caption & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    w.put8( kTableWireForm ); // the FORM BYTE is the whole header (§3)
    if ( !CaptionSaveBody( w, ids, value ) || ids.overflow ) { return -1; }
    TableIdsWrite( w, ids ); // the ID TABLE is the last thing in the file
    if ( w.overflow ) { return -1; }
    return w.offset; // == CaptionMeasure( value )
}

WIDE_TABLE_INLINE bool CaptionLoadBody( TableReader & r, Caption & value )
{
    CaptionReset( value ); // prefill declared defaults in place, then overlay
    for ( ;; )
    {
        ...
        switch ( field_id )
        {
            case 0xda31296c0c1b6029ull: // title
            {
                ...
                if ( ( len & 1 ) != 0 ) { r.report->malformed = true; value.title[0] = 0; value.title_length = 0; r.offset += (int64_t) len; break; }
                ...
                // parent reads on past L
                if ( !TableUtf16Valid( r.buffer + r.offset, units ) ) { r.report->malformed = true; value.title[0] = 0; value.title_length = 0; r.offset += (int64_t) len; break; }
                int64_t keep = units;
                if ( keep > 7 ) { keep = TableUtf16Clamp( r.buffer + r.offset, units, 7 ); r.report->clamped++; } // never splitting a pair (§3)
                for ( int64_t i = 0; i < keep; i++ ) { value.title[i] = (char16_t) TableUtf16Unit( r.buffer + r.offset, i ); }
                value.title[keep] = 0;
                value.title_length = (int32_t) keep;
                ...
                if ( !r.getleb( body_len ) || !r.room( body_len ) ) { r.report->malformed = true; return false; }
                {
                    ...
                    switch ( arm_id ) // the arm's NAME hash (docs/SPEC-TABLES.md §5)
                    {
                        case 0xa633f1f655715ccaull: // wide
                        {
                            ...
                            if ( ( sub.size & 1 ) != 0 ) { value.body.type = BodyType::None; r.report->malformed = true; break; } // an odd L is not a wstring at any length (§3)
                            memset( (void *) &value.body.wide, 0, sizeof( value.body.wide ) ); // selection establishes the arm (§2.6)
                            {
                                const int64_t arm_units = sub.size / 2;
                                int64_t arm_keep = arm_units;
                                // ILL-FORMED TEXT at an arm (§3): the union reads None, one malformed counts
                                if ( !TableUtf16Valid( sub.buffer + sub.offset, arm_units ) ) { value.body.type = BodyType::None; r.report->malformed = true; break; }
                                if ( arm_keep > 4 ) { arm_keep = TableUtf16Clamp( sub.buffer + sub.offset, arm_units, 4 ); r.report->clamped++; } // never splitting a pair
                                for ( int64_t i = 0; i < arm_keep; i++ ) { value.body.wide.value[i] = (char16_t) TableUtf16Unit( sub.buffer + sub.offset, i ); }
                                value.body.wide.value[arm_keep] = 0;
                                value.body.wide.value_length = (int32_t) arm_keep;
                                sub.offset += sub.size;
                            }
                            break;
                        }
                        case 0x96569b06f223c29aull: // narrow
                        {
                            ...
                            memset( (void *) &value.body.narrow, 0, sizeof( value.body.narrow ) ); // selection establishes the arm (§2.6)
                            {
                                uint32_t arm_len = uint32_t( sub.size );
                                uint32_t arm_keep = arm_len;
                                // ILL-FORMED TEXT at an arm (§3): the union reads None, one malformed counts
                                if ( !TableUtf8Valid( sub.buffer + sub.offset, sub.size ) ) { value.body.type = BodyType::None; r.report->malformed = true; break; }
                                if ( arm_keep > 4 ) { arm_keep = (uint32_t) TableUtf8Clamp( sub.buffer + sub.offset, sub.size, 4 ); r.report->clamped++; } // at a code point boundary
                                memcpy( value.body.narrow.value, sub.buffer + sub.offset, arm_keep );
                                value.body.narrow.value[arm_keep] = 0;
                                value.body.narrow.value_length = (int32_t) arm_keep;
                                sub.offset += arm_len;
                            }
                            break;
                        }
...
inline TableOpenVerdict CaptionLoadVerdict( Caption & value, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    TableIdTable table;
    ...
    if ( !CaptionLoadBody( r, value ) ) { return TableOpenBodyStopped; }
    return TableOpenOk;
}
...
inline bool CaptionLoad( Caption & value, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    return CaptionLoadVerdict( value, buffer, bytes, report ) == TableOpenOk;
}
...
inline int64_t CaptionMeasureMessageBody( int64_t at, const Caption & value )
{
    ...
    if ( value.title_length > 0 )
    {
        ...
        bits += 3 + (int64_t) value.title_length * 16;
    }
    {
        const int64_t body_line = LineMeasureMessageBody( at + bits, value.line );
        if ( body_line < 0 ) { return -1; }
        if ( body_line > kTableMessageRefBitsHere ) // an all-default nested table elides
        {
            bits += kTableMessageRefBitsHere;
            bits += LineMeasureMessageBody( at + bits, value.line );
        }
    }
    if ( value.lines_count < 0 || value.lines_count > 3 ) { return -1; } // storage invariant
    if ( value.lines_count > 0 )
    {
        bits += kTableMessageRefBitsHere;
        bits += 2;
        for ( int32_t i = 0; i < value.lines_count; i++ )
        {
            const int64_t elem = LineMeasureMessageBody( at + bits, value.lines[i] );
            if ( elem < 0 ) { return -1; }
            bits += elem;
        }
    }
    if ( value.body.type != BodyType::None )
    {
        ...
        switch ( value.body.type )
        {
            case BodyType::Wide:
            {
                bits += kTableMessageRefBitsHere;
                if ( value.body.wide.value_length < 0 || value.body.wide.value_length > 4 ) { return -1; } // storage invariant
                bits += 3 + (int64_t) value.body.wide.value_length * 16;
                break;
            }
            case BodyType::Narrow:
            {
...
inline bool CaptionSaveMessageBody( TableBitWriter & w, const Caption & value )
{
    ...
    if ( value.title_length > 0 )
    {
        ...
        w.put( (uint64_t) value.title_length, 3 );
        for ( int32_t i = 0; i < value.title_length; i++ ) { w.put( (uint64_t) (uint16_t) value.title[i], 16 ); } // sixteen bits a unit, no align
    }
    {
        const int64_t body_line = LineMeasureMessageBody( 0, value.line );
        if ( body_line < 0 ) { return false; }
        if ( body_line > kTableMessageRefBitsHere ) // an all-default nested table elides
        {
            w.put( 2, kTableMessageRefBitsHere );
            if ( !LineSaveMessageBody( w, value.line ) ) { return false; }
        }
    }
    if ( value.lines_count < 0 || value.lines_count > 3 ) { return false; } // storage invariant
    if ( value.lines_count > 0 )
    {
        ...
        w.put( (uint64_t) ( value.lines_count ) - 0, 2 );
        for ( int32_t i = 0; i < value.lines_count; i++ )
        {
            if ( !LineSaveMessageBody( w, value.lines[i] ) ) { return false; }
        }
    }
    if ( value.body.type != BodyType::None )
    {
        ...
        switch ( value.body.type )
        {
            case BodyType::Wide:
            {
                ...
                if ( value.body.wide.value_length < 0 || value.body.wide.value_length > 4 ) { return false; } // storage invariant
                w.put( (uint64_t) value.body.wide.value_length, 3 );
                for ( int32_t i_2 = 0; i_2 < value.body.wide.value_length; i_2++ ) { w.put( (uint64_t) (uint16_t) value.body.wide.value[i_2], 16 ); } // sixteen bits a unit, no align
                break;
            }
            case BodyType::Narrow:
            {
                ...
                if ( value.body.narrow.value_length < 0 || value.body.narrow.value_length > 4 ) { return false; } // storage invariant
                w.put( (uint64_t) value.body.narrow.value_length, 3 );
        ...
        switch ( entry.id )
        {
            case 0xda31296c0c1b6029ull: // title
            {
                ...
                }
                {
                    ...
                    int32_t kept = 0;
                    if ( n > (uint64_t) 7 ) { kept = 7; report->clamped++; } else { kept = (int32_t) n; }
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        ...
                        if ( (int32_t) i < kept ) { value.title[i] = (char16_t) u; }
                    }
                    if ( high || ill ) { report->malformed = true; return false; }
                    if ( kept > 0 && (uint64_t) kept < n )
                    {
                        const char16_t last = value.title[kept - 1];
                        if ( last >= 0xD800 && last <= 0xDBFF ) { kept--; }
                    }
                    value.title[kept] = 0;
                    value.title_length = kept;
                }
                break;
            }
            case 0xbf4ba5ad694f5907ull: // line
            {
                ...
                if ( !LineLoadMessageBody( r, vocabulary, report, index_bits, value.line ) ) { return false; }
                break;
            }
            case 0x5ce3f9a9f1d5001cull: // lines
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < walk; i++ )
                    {
                        ...
                        Line scratch;
                        LineReset( scratch );
                        if ( !LineLoadMessageBody( r, vocabulary, report, index_bits, ( in_bounds ? value.lines[i] : scratch ) ) ) { return false; }
                    }
                    value.lines_count = kept;
                }
                break;
            }
            case 0xcd4de79bc6c93295ull: // body
            {
                ...
                }
                {
                    ...
                    if ( arm_ref == 0 ) { value.body.type = BodyType::None; }
                    ...
                    else
                    {
                        switch ( arm.id )
                        {
                            case 0xa633f1f655715ccaull: // wide
                            {
                                if ( arm.kind != 33 || arm.elem_kind != 0 )
                                {
                                    value.body.type = BodyType::None; report->kind_mismatch++;
                                ...
                                {
                                    uint64_t n_2 = 0;
                                    if ( !r.get( n_2, TableBitsRequired( 0, arm.max ) ) || !r.has( (int64_t) n_2 * 16 ) ) { report->malformed = true; return false; }
                                    int32_t kept_2 = 0;
                                    if ( n_2 > (uint64_t) 4 ) { kept_2 = 4; report->clamped++; } else { kept_2 = (int32_t) n_2; }
                                    bool high_2 = false; // a high surrogate awaiting its low half
                                    bool ill_2 = false;
                                    for ( uint64_t i_2 = 0; i_2 < n_2; i_2++ )
                                    {
                                        uint64_t unit_2 = 0;
                                        if ( !r.get( unit_2, 16 ) ) { report->malformed = true; return false; }
                                        const uint16_t u_2 = (uint16_t) unit_2;
                                        if ( u_2 == 0 ) { ill_2 = true; }
                                        if ( high_2 ) { if ( u_2 < 0xDC00 || u_2 > 0xDFFF ) { ill_2 = true; } high_2 = false; }
                                        else if ( u_2 >= 0xD800 && u_2 <= 0xDBFF ) { high_2 = true; }
                                        else if ( u_2 >= 0xDC00 && u_2 <= 0xDFFF ) { ill_2 = true; }
                                        if ( (int32_t) i_2 < kept_2 ) { value.body.wide.value[i_2] = (char16_t) u_2; }
                                    }
                                    if ( high_2 || ill_2 ) { report->malformed = true; return false; }
                                    if ( kept_2 > 0 && (uint64_t) kept_2 < n_2 )
                                    {
                                        const char16_t last_2 = value.body.wide.value[kept_2 - 1];
                                        if ( last_2 >= 0xD800 && last_2 <= 0xDBFF ) { kept_2--; }
                                    }
                                    value.body.wide.value[kept_2] = 0;
                                    value.body.wide.value_length = kept_2;
                                }
                                break;
                            }
                            case 0x96569b06f223c29aull: // narrow
                            ...
                            default:
                                value.body.type = BodyType::None; report->unknown++;
                                if ( !TableMessageSkip( r, vocabulary, index_bits, arm ) ) { report->malformed = true; return false; }
                                break;
                        }
                    }
...
inline int64_t CaptionMeasureMessages( const Caption * values, int64_t count, TableReport * report )
{
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = CaptionMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
...
inline int64_t CaptionSaveMessages( const Caption * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !CaptionSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == CaptionMeasureMessages( values, count, report )
}
...
inline bool CaptionLoadMessages( Caption * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    ...
    *count = 0;
    ...
    {
        if ( !CaptionLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
...
{
    TableIds ids;
    const int64_t body = StampMeasureBody( ids, value );
    if ( body < 0 || ids.overflow ) { return -1; }
    return 1 + body + TableIdsBytes( ids );
}
...
inline int64_t StampSave( const Stamp & value, uint8_t * buffer, int64_t capacity )
{
    ...
    TableIds ids;
    w.put8( kTableWireForm ); // the FORM BYTE is the whole header (§3)
    if ( !StampSaveBody( w, ids, value ) || ids.overflow ) { return -1; }
    TableIdsWrite( w, ids ); // the ID TABLE is the last thing in the file
    if ( w.overflow ) { return -1; }
    ...
    for ( ;; )
    {
        ...
        switch ( field_id )
        {
            case 0x39f7fcec8fcb623dull: // label
            {
                ...
                // AN ODD L IS FRAMING DAMAGE on the body that carries it (§3): the
                // value is L / 2 code units, so an odd L is not a wstring at any length.
                if ( ( len & 1 ) != 0 ) { r.report->malformed = true; value.label[0] = 0; value.label_length = 0; r.offset += (int64_t) len; break; }
                const int64_t units = (int64_t) ( len / 2 );
                // ILL-FORMED TEXT IS DAMAGE (§3, §4): an unpaired surrogate or a zero
                // code unit reads the declared default, one malformed counts, and the
                // parent reads on past L
                if ( !TableUtf16Valid( r.buffer + r.offset, units ) ) { r.report->malformed = true; value.label[0] = 0; value.label_length = 0; r.offset += (int64_t) len; break; }
                int64_t keep = units;
                if ( keep > 4 ) { keep = TableUtf16Clamp( r.buffer + r.offset, units, 4 ); r.report->clamped++; } // never splitting a pair (§3)
                for ( int64_t i = 0; i < keep; i++ ) { value.label[i] = (char16_t) TableUtf16Unit( r.buffer + r.offset, i ); }
                value.label[keep] = 0;
...
// A refusal moves none of the report's five counters, because nothing was
// decoded and there is nothing to count (docs/SPEC-TABLES.md §3).
inline TableOpenVerdict StampLoadVerdict( Stamp & value, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    TableIdTable table;
    ...
    TableReader r( buffer + 1, body_bytes, to, &table );
    r.nested = false; // the ROOT body, the one that may carry a node table
    if ( !StampLoadBody( r, value ) ) { return TableOpenBodyStopped; }
    return TableOpenOk;
}
...
{
    return StampLoadVerdict( value, buffer, bytes, report ) == TableOpenOk;
}
...
inline int64_t StampMeasureMessageBody( int64_t at, const Stamp & value )
{
    ...
    {
        bits += kTableMessageRefBitsHere;
        bits += 3 + (int64_t) value.label_length * 16;
    }
    if ( value.seq != 0 )
    {
        bits += kTableMessageRefBitsHere;
...
inline bool StampSaveMessageBody( TableBitWriter & w, const Stamp & value )
{
    ...
    if ( value.label_length > 0 )
    {
        ...
        w.put( (uint64_t) value.label_length, 3 );
        for ( int32_t i = 0; i < value.label_length; i++ ) { w.put( (uint64_t) (uint16_t) value.label[i], 16 ); } // sixteen bits a unit, no align
    }
    if ( value.seq != 0 )
    {
        w.put( 7, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.seq ), 32 );
    }
    w.put( 0, kTableMessageRefBitsHere ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0x39f7fcec8fcb623dull: // label
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        ...
                        if ( (int32_t) i < kept ) { value.label[i] = (char16_t) u; }
                    }
                    if ( high || ill ) { report->malformed = true; return false; }
                    if ( kept > 0 && (uint64_t) kept < n )
                    {
                        const char16_t last = value.label[kept - 1];
                        if ( last >= 0xD800 && last <= 0xDBFF ) { kept--; }
                    }
                    value.label[kept] = 0;
...
// refusal is learned at MEASURE time, before a buffer is allocated, and a
// caller with more bodies calls again.
inline int64_t StampMeasureMessages( const Stamp * values, int64_t count, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    int64_t bits = 8; // the body count, a ranged integer over [1, 256]
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = StampMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8; // the form byte, then the stream padded to a byte
...
inline int64_t StampSaveMessages( const Stamp * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    TableMessageBatch batch;
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !StampSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == StampMeasureMessages( values, count, report )
}
...
// BODIES 1 TO k - 1: count says k - 1, one malformed counts, and the storage
// after it is not a body.
inline bool StampLoadMessages( Stamp * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    if ( values == NULL || count == NULL ) { to->malformed = true; return false; }
    ...
    *count = 0;
    ...
    for ( int64_t i = 0; i < bodies; i++ )
    {
        if ( !StampLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
inline int64_t LineMeasure( const Line & value )
{
    TableIds ids;
    const int64_t body = LineMeasureBody( ids, value );
    if ( body < 0 || ids.overflow ) { return -1; }
    return 1 + body + TableIdsBytes( ids );
}
...
inline int64_t LineSave( const Line & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    w.put8( kTableWireForm ); // the FORM BYTE is the whole header (§3)
    if ( !LineSaveBody( w, ids, value ) || ids.overflow ) { return -1; }
    TableIdsWrite( w, ids ); // the ID TABLE is the last thing in the file
    if ( w.overflow ) { return -1; }
    return w.offset; // == LineMeasure( value )
}

WIDE_TABLE_INLINE bool LineLoadBody( TableReader & r, Line & value )
{
    LineReset( value ); // prefill declared defaults in place, then overlay
    for ( ;; )
    {
        uint64_t field_ref = 0;
        if ( !r.getleb( field_ref ) ) { r.report->malformed = true; return false; }
        ...
        switch ( field_id )
        {
            case 0xfa04f4ef1995407eull: // text
            {
                ...
                // AN ODD L IS FRAMING DAMAGE on the body that carries it (§3): the
                // value is L / 2 code units, so an odd L is not a wstring at any length.
                if ( ( len & 1 ) != 0 ) { r.report->malformed = true; value.text[0] = 0; value.text_length = 0; r.offset += (int64_t) len; break; }
                const int64_t units = (int64_t) ( len / 2 );
                // ILL-FORMED TEXT IS DAMAGE (§3, §4): an unpaired surrogate or a zero
                // code unit reads the declared default, one malformed counts, and the
                // parent reads on past L
                if ( !TableUtf16Valid( r.buffer + r.offset, units ) ) { r.report->malformed = true; value.text[0] = 0; value.text_length = 0; r.offset += (int64_t) len; break; }
                int64_t keep = units;
                if ( keep > 4 ) { keep = TableUtf16Clamp( r.buffer + r.offset, units, 4 ); r.report->clamped++; } // never splitting a pair (§3)
                for ( int64_t i = 0; i < keep; i++ ) { value.text[i] = (char16_t) TableUtf16Unit( r.buffer + r.offset, i ); }
                value.text[keep] = 0;
...
// A refusal moves none of the report's five counters, because nothing was
// decoded and there is nothing to count (docs/SPEC-TABLES.md §3).
inline TableOpenVerdict LineLoadVerdict( Line & value, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    TableIdTable table;
    ...
    TableReader r( buffer + 1, body_bytes, to, &table );
    r.nested = false; // the ROOT body, the one that may carry a node table
    if ( !LineLoadBody( r, value ) ) { return TableOpenBodyStopped; }
    return TableOpenOk;
}
...
// False is a wire nothing could be decoded from: a refusal, a table that
// cannot be read whole, or a root body the walk could not finish.
inline bool LineLoad( Line & value, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    return LineLoadVerdict( value, buffer, bytes, report ) == TableOpenOk;
}
...
inline int64_t LineMeasureMessageBody( int64_t at, const Line & value )
{
    ...
    {
        bits += kTableMessageRefBitsHere;
        bits += 3 + (int64_t) value.text_length * 16;
    }
    bits += kTableMessageRefBitsHere; // the ZERO REFERENCE that ends the body
    (void) at;
    return bits;
...
inline bool LineSaveMessageBody( TableBitWriter & w, const Line & value )
{
    ...
    if ( value.text_length > 0 )
    {
        ...
        w.put( (uint64_t) value.text_length, 3 );
        for ( int32_t i = 0; i < value.text_length; i++ ) { w.put( (uint64_t) (uint16_t) value.text[i], 16 ); } // sixteen bits a unit, no align
    }
    w.put( 0, kTableMessageRefBitsHere ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0xfa04f4ef1995407eull: // text
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        ...
                        if ( (int32_t) i < kept ) { value.text[i] = (char16_t) u; }
                    }
                    if ( high || ill ) { report->malformed = true; return false; }
                    if ( kept > 0 && (uint64_t) kept < n )
                    {
                        const char16_t last = value.text[kept - 1];
                        if ( last >= 0xD800 && last <= 0xDBFF ) { kept--; }
                    }
                    value.text[kept] = 0;
...
// refusal is learned at MEASURE time, before a buffer is allocated, and a
// caller with more bodies calls again.
inline int64_t LineMeasureMessages( const Line * values, int64_t count, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    int64_t bits = 8; // the body count, a ranged integer over [1, 256]
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = LineMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8; // the form byte, then the stream padded to a byte
...
inline int64_t LineSaveMessages( const Line * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    TableMessageBatch batch;
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !LineSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == LineMeasureMessages( values, count, report )
}
...
// BODIES 1 TO k - 1: count says k - 1, one malformed counts, and the storage
// after it is not a body.
inline bool LineLoadMessages( Line * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    if ( values == NULL || count == NULL ) { to->malformed = true; return false; }
    ...
    *count = 0;
    ...
    for ( int64_t i = 0; i < bodies; i++ )
    {
        if ( !LineLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
static_assert( sizeof( Stamp ) == 20, "Stamp: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( Stamp, label ) == 0, "Stamp.label: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Stamp, label_length ) == 12, "Stamp.label_length: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Stamp, seq ) == 16, "Stamp.seq: the fixed form's plan lands here" );

// Stamp's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void StampFixedWriteBody( uint8_t * b, const Stamp & value )
{
    (void) b; (void) value;
    schema_assert( value.label_length >= 0 && value.label_length <= 4 ); // the declared length is the bound (§3.4)
    TableFixedPut32( b + 0, (uint32_t) value.label_length );
    memcpy( b + 4, value.label, (size_t) ( 2 * value.label_length ) );
...
inline void StampFixedClampBody( Stamp & value, int32_t & clamped, int32_t & damaged )
{
    ...
    if ( !TableUtf16Valid( (const uint8_t *) value.label, (int64_t) value.label_length ) )
    {
        memset( value.label, 0, sizeof( value.label ) );
        value.label_length = 0;
        damaged++;
...
constexpr uint64_t StampFixedHash = 0x820f5d6ffaf5e7eeull; // fnv1a64 over the layout and the definitions digest (bill §13)
...
constexpr TableFixedDst StampFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Stamp
    { (uint32_t) __builtin_offsetof( Stamp, label_length ), 0, (uint32_t) __builtin_offsetof( Stamp, label ), 0, 2 }, // label
    { (uint32_t) __builtin_offsetof( Stamp, seq ), 0, 0, 0, 0 }, // seq
};
...
constexpr TableFixedEntry StampFixedPlan[] = {
    { 0u, 12u, 8u, 0u, kTableFixedNoGuard, kTableFixedText, 0, 2, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // label
    { 12u, 16u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // seq
};
...
constexpr TableFixedFill StampFixedCover[] = {
    { 0u, 10u },
    { 12u, 8u },
};
constexpr int32_t StampFixedCoverCount = 2;
...
constexpr uint64_t StampFixedLineage[] = {
    0x820f5d6ffaf5e7eeull, // identity
};
constexpr int32_t StampFixedLineageCount = 1;
constexpr int32_t StampFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t StampFixedFloorLive = StampFixedFloor;
inline void StampFixedSetFloorForTest( int32_t index ) { StampFixedFloorLive = index; }
#endif

constexpr uint8_t StampFixedKnown0Layout[] = {
    0x03, 0x00, 0x00, 0x00, 0xa4, 0x93, 0xdb, 0xb7, 0x13, 0x91, 0x3b, 0xae, 0x0d, 0x10, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0x3d, 0x62, 0xcb, 0x8f, 0xec, 0xfc, 0xf7, 0x39, 0x21, 0x0c, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3c, 0x13, 0xe2, 0x5c, 0x19, 0x8a, 0x3b, 0x82, 0x08, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr TableFixedKnownLayout StampFixedKnown[] = {
    { 0x820f5d6ffaf5e7eeull, StampFixedKnown0Layout, 55, 24 }, // identity
};

constexpr const TableFixedKnownRange * StampFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t StampFixedKnownRangeCounts[] = { 0 };
...
inline int64_t StampFixedSave( const Stamp * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) StampFixedBodyBytes ); // the prefill's zeros
        StampFixedWriteBody( at + 8, values[k] );
        at += StampFixedRecordBytes;
    }
    return need;
}
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    for ( int32_t i = 0; i < StampFixedLineageCount; ++i )
    {
        if ( StampFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = StampFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = StampFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = StampFixedKnown[lineage_at];
    ...
    if ( hash != StampFixedHash )
    {
        ...
        else
        {
            ...
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, StampFixedLayout, (int32_t) StampFixedLayoutBytes, StampFixedDst,
                                                   StampFixedCover, StampFixedCoverCount,
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( StampFixedKnownRanges[lineage_at], StampFixedKnownRangeCounts[lineage_at],
...
template <typename... Args>
inline void CaptionSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Caption is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void CaptionLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Caption is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void CaptionSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Caption is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void StampLoadRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Stamp is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}

template <typename... Args>
inline void StampMeasureRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Stamp is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void StampSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Stamp is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void StampLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Stamp is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void StampSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Stamp is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void LineLoadRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Line is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}

template <typename... Args>
inline void LineMeasureRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Line is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void LineSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Line is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void LineLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Line is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void LineSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Line is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
// CaptionOpen: match the header and POINT. On a match the bytes ARE what this
...
//
// Caption IS FIXED-SIZE, so its cook is ONE REGION OF ONE NODE and not a second
...
// parameter on a load.
inline const Caption * CaptionOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Caption *) TableCookOpen( bytes, length, (uint64_t) sizeof( Caption ), (uint64_t) alignof( Caption ), reason );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Stamp * StampOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Stamp *) TableCookOpen( bytes, length, (uint64_t) sizeof( Stamp ), (uint64_t) alignof( Stamp ), reason );
}
...
inline void CaptionCookBody( uint8_t * at, const Caption & value, TableByteOrder order );
inline void StampCookBody( uint8_t * at, const Stamp & value, TableByteOrder order );
inline void LineCookBody( uint8_t * at, const Line & value, TableByteOrder order );

inline void CaptionCookBody( uint8_t * at, const Caption & value, TableByteOrder order )
{
    table_cook_units( at + 0, value.title, value.title_length, 8, order );
    table_cook_put( at + 16, (uint64_t) (uint32_t) value.title_length, 4, order );
    LineCookBody( at + 20, value.line, order );
    // all 3 slots: the storage is allocate-max, and a slot past the count rides as it lies (§7.2)
    for ( int32_t i = 0; i < 3; i++ )
    {
        LineCookBody( at + 36 + i * 16, value.lines[ i ], order );
    }
    table_cook_put( at + 84, (uint64_t) (uint32_t) value.lines_count, 4, order );
    {
        table_cook_put( at + 88, (uint64_t) value.body.type, 1, order ); // the tag; None is the tag alone
        switch ( value.body.type )
        {
            case BodyType::Wide:
            {
                table_cook_units( at + 88 + 4, value.body.wide.value, value.body.wide.value_length, 5, order );
                table_cook_put( at + 88 + 16, (uint64_t) (uint32_t) value.body.wide.value_length, 4, order );
                break;
            }
            case BodyType::Narrow:
            {
                table_cook_bytes( at + 88 + 4, value.body.narrow.value, value.body.narrow.value_length, 5 );
...
inline void StampCookBody( uint8_t * at, const Stamp & value, TableByteOrder order )
{
    table_cook_units( at + 0, value.label, value.label_length, 5, order );
...
inline void LineCookBody( uint8_t * at, const Line & value, TableByteOrder order )
{
    table_cook_units( at + 0, value.text, value.text_length, 5, order );
    table_cook_put( at + 12, (uint64_t) (uint32_t) value.text_length, 4, order );
}

// CaptionCookMeasure: the whole cooked file's bytes — the header, the data part
...
//
// Caption IS FIXED-SIZE, so the answer does not depend on the value: its cook is
...
inline int64_t CaptionCookMeasure( const Caption & value )
{
    (void) value;
    return 192; // 64 header + 112 data + 16 attribution
}

// CaptionCook: write one cooked file for the build this code is compiled into,
...
// the same contract CaptionMeasure/CaptionSave has on the wire (§6.1).
//
...
inline bool CaptionCook( const Caption & value, void * out, uint64_t capacity, TableByteOrder order )
{
    if ( out == NULL ) { return false; }
    const uint64_t need = (uint64_t) CaptionCookMeasure( value );
    ...
    CaptionCookBody( raw + 64, value, order );
    ...
    table_cook_put( raw + 184, 0x832ef3a7593a8b05ull, 8, order );
    return true;
}

// StampCookMeasure: the whole cooked file's bytes — the header, the data part
// and the attribution part (docs/SPEC-TABLES.md §7.1). It answers in int64_t
// because a cook's part lengths are 64 bits: the scale this form exists for is
// a catalog, and a 32-bit answer would reimpose the ceiling §3.1 removed.
//
// Stamp IS FIXED-SIZE, so the answer does not depend on the value: its cook is
// ONE REGION OF ONE NODE (§7) — the record at the region's base, its length
// rounded to the region's alignment, and one directory entry.
inline int64_t StampCookMeasure( const Stamp & value )
{
    (void) value;
    return 104; // 64 header + 24 data + 16 attribution
}

// StampCook: write one cooked file for the build this code is compiled into,
// in the byte order the caller names. The bytes are `schema cook`'s, byte for
// byte, and the tool is the reference (§7.6).
//
// THE CALLER OWNS THE BUFFER AND NOTHING IS ALLOCATED: measure, then write.
// A capacity short of the measure writes nothing and returns false, which is
// the same contract StampMeasure/StampSave has on the wire (§6.1).
//
// EVERY BYTE NO FIELD COVERS IS ZERO (§7.2) — interior padding, the record's
// trailing padding, a string's unused tail, the bytes of a union outside its
// set arm, and the slack the rounded data length leaves. It comes from the one
// memset below rather than from a rule per padding site, and it is what makes
// two cooks of one value ONE artifact (§7).
inline bool StampCook( const Stamp & value, void * out, uint64_t capacity, TableByteOrder order )
{
    if ( out == NULL ) { return false; }
    const uint64_t need = (uint64_t) StampCookMeasure( value );
    if ( capacity < need ) { return false; }
    uint8_t * raw = (uint8_t *) out;
    memset( raw, 0, (size_t) need );
    // the HEADER (§7.1), every word a u64 in the order the file is produced in
    ...
    // the two RESERVED words are zero, and the memset already wrote them
    // the DATA part: the region, which for a fixed root is the record at its base
    StampCookBody( raw + 64, value, order );
    // the ATTRIBUTION part: the node directory (§6.3), written beside the data
    // for `schema cook-check` — one entry, the root at offset zero, and its type
    // id is the fnv1a64 of the table's name (§3.1)
    table_cook_put( raw + 88, 0, 8, order );
    table_cook_put( raw + 96, 0xae3b9113b7db93a4ull, 8, order );
    return true;
}
...
// include at all.
static_assert( __is_trivially_copyable( Caption ), "Caption must stay relocatable" );
static_assert( __is_standard_layout( Caption ), "Caption must stay standard-layout for offsetof" );
...
static_assert( __is_trivially_copyable( Line ), "Line must stay relocatable" );
static_assert( __is_standard_layout( Line ), "Line must stay standard-layout for offsetof" );
...
// rather than assumed.
static_assert( sizeof( Caption ) == 108, "Caption's sizeof moved: the build version was taken over 108, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( Caption ) == 4, "Caption's alignof moved: the build version was taken over 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Caption, title ) == 0, "Caption's field title moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Caption, line ) == 20, "Caption's field line moved: the build version was taken over offset 20 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Caption, lines ) == 36, "Caption's field lines moved: the build version was taken over offset 36 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Caption, body ) == 88, "Caption's field body moved: the build version was taken over offset 88 (docs/SPEC-TABLES.md §20.3)" );
...
static_assert( offsetof( Stamp, label ) == 0, "Stamp's field label moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
...
static_assert( sizeof( Line ) == 16, "Line's sizeof moved: the build version was taken over 16, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( Line ) == 4, "Line's alignof moved: the build version was taken over 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Line, text ) == 0, "Line's field text moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );

// ---- reflection descriptors (tables only, docs/SPEC-TABLES.md) ----

inline const TableTypeInfo * CaptionTableType();
inline const TableTypeInfo * StampTableType();
inline const TableTypeInfo * LineTableType();
...
{
    static const TableFieldInfo fields[] = {
        { "label", "label", "wstring", 0x39f7fcec8fcb623dull, 33, false, true, false, 4, (uint32_t) offsetof( Stamp, label ), (uint32_t) sizeof( Stamp::label ), (uint32_t) offsetof( Stamp, label_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
        { "seq", "seq", "uint32", 0x823b8a195ce2133cull, 8, false, false, false, 0, (uint32_t) offsetof( Stamp, seq ), (uint32_t) sizeof( Stamp::seq ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
    };
    static const TableTypeInfo info = { "Stamp", (uint32_t) sizeof( Stamp ), 2, fields, +[]( void * p ) { StampReset( *(Stamp *) p ); }, TableDocNone, 0, NULL };
    return &info;
}
...
{
    static const TableFieldInfo fields[] = {
        { "text", "text", "wstring", 0xfa04f4ef1995407eull, 33, false, true, false, 4, (uint32_t) offsetof( Line, text ), (uint32_t) sizeof( Line::text ), (uint32_t) offsetof( Line, text_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
    };
    static const TableTypeInfo info = { "Line", (uint32_t) sizeof( Line ), 1, fields, +[]( void * p ) { LineReset( *(Line *) p ); }, TableDocNone, 0, NULL };
    return &info;
}
...
// CaptionTable.cpp; link it to use them.
bool CaptionFromJson( Caption & value, const char * text, int64_t bytes, TableReport * report );
int64_t CaptionToJsonMeasure( const Caption & value );
int64_t CaptionToJson( const Caption & value, char * buffer, int64_t capacity );
...
// walk over this type's descriptors (docs/SPEC-TABLES.md §16). Defined in
// CaptionTable.cpp; link it to use them.
bool LineFromJson( Line & value, const char * text, int64_t bytes, TableReport * report );
int64_t LineToJsonMeasure( const Line & value );
int64_t LineToJson( const Line & value, char * buffer, int64_t capacity );
