...
constexpr int StampFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Stamp's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int StampFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    (void) out; (void) src; (void) dst;
    out[n++] = TableFixedEntry{ src + 0u, dst + (uint32_t) __builtin_offsetof( Stamp, label_length ), 8u, dst + (uint32_t) __builtin_offsetof( Stamp, label ), kTableFixedNoGuard, kTableFixedText, 2, 0 }; // label
    {
        const uint32_t es = src + 12u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Stamp, seq );
...
inline constexpr int64_t StampFixedBodyBytes = 16;
inline constexpr int64_t StampFixedRecordBytes = 8 + StampFixedBodyBytes; // the hash and the body
inline constexpr uint64_t StampFixedHash = 0x820f5d6ffaf5e7eeull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t StampFixedLayout[] = {
...
inline constexpr int64_t StampFixedLayoutBytes = (int64_t) sizeof( StampFixedLayout );
...
inline constexpr TableFixedDst StampFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Stamp
...
inline constexpr auto StampFixedPlan = TableFixedBuildPlan<2>( StampFixedLeaves );
...
inline constexpr int64_t StampFixedMeasure( int64_t count )
{
...
inline int64_t StampFixedSave( const Stamp * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) StampFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = StampFixedPlan.entries;
    int32_t entry_count = StampFixedPlan.count;
    ...
    if ( hash != StampFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, StampFixedLayout, (int32_t) StampFixedLayoutBytes, StampFixedDst, plan, plan_capacity, report );
