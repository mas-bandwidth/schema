...
constexpr int PoseFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int SimStateFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Pose's LEAVES: one entry per run of bytes that lands, at a base the caller
...
constexpr int PoseFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Pose, x );
        out[n++] = TableFixedEntry{ es, ed, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 8u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Pose, y );
        out[n++] = TableFixedEntry{ es, ed, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 16u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Pose, heading );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// Pose's stores. The template — the hash, then zeros — is memcpy'd first,
...
// SimState's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int SimStateFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    ...
    {
        const uint32_t es = src + 0u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, tilt );
        out[n++] = TableFixedEntry{ es, ed, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 1u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, angle );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 5u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, position );
        out[n++] = TableFixedEntry{ es, ed, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 13u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, reach );
        out[n++] = TableFixedEntry{ es, ed, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 29u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, ticks );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 33u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, ratio );
        out[n++] = TableFixedEntry{ es, ed, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 34u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, speed );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 38u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, span );
        out[n++] = TableFixedEntry{ es, ed, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 46u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, mass );
        out[n++] = TableFixedEntry{ es, ed, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 62u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, frames );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 66u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, flux );
        out[n++] = TableFixedEntry{ es, ed, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 82u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, energy );
        out[n++] = TableFixedEntry{ es, ed, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 98u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, entity_id );
        out[n++] = TableFixedEntry{ es, ed, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 114u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, scale );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    static_assert( sizeof( int32_t ) == 4, "SimState.samples: a flat element's storage IS its wire image" );
    out[n++] = TableFixedEntry{ src + 118u, dst + (uint32_t) __builtin_offsetof( SimState, samples ), 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // samples, whole
    out[n++] = TableFixedEntry{ src + 130u, dst + (uint32_t) __builtin_offsetof( SimState, weights_count ), 4u, 0u, kTableFixedNoGuard, kTableFixedCount, 0, 0 }; // weights count
    static_assert( sizeof( uint16_t ) == 2, "SimState.weights: a flat element's storage IS its wire image" );
    out[n++] = TableFixedEntry{ src + 134u, dst + (uint32_t) __builtin_offsetof( SimState, weights ), 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // weights, whole
    static_assert( sizeof( int64_t ) == 8, "SimState.axes: a flat element's storage IS its wire image" );
    out[n++] = TableFixedEntry{ src + 142u, dst + (uint32_t) __builtin_offsetof( SimState, axes.slots ), 24u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // axes, whole
    out[n++] = TableFixedEntry{ src + 166u, dst + (uint32_t) __builtin_offsetof( SimState, seeds_count ), 2u, 0u, kTableFixedNoGuard, kTableFixedCount, 0, 0 }; // seeds count
    static_assert( sizeof( serialize::uint128_t ) == 16, "SimState.seeds: a flat element's storage IS its wire image" );
    out[n++] = TableFixedEntry{ src + 170u, dst + (uint32_t) __builtin_offsetof( SimState, seeds ), 32u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // seeds, whole
    {
        const uint32_t es = src + 202u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, pose );
        n += PoseFixedLeaves( out + n, es, ed );
    }
    out[n++] = TableFixedEntry{ src + 222u, dst + (uint32_t) __builtin_offsetof( SimState, spawn_present ), 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // spawn present
    {
        const uint32_t es = src + 223u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( SimState, spawn );
        n += PoseFixedLeaves( out + n, es, ed );
    }
    return n;
}

// SimState's stores. The template — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
...
inline constexpr int64_t SimStateFixedBodyBytes = 243;
inline constexpr int64_t SimStateFixedRecordBytes = 8 + SimStateFixedBodyBytes; // the hash and the body
inline constexpr uint64_t SimStateFixedHash = 0x1b3ae43f63f8eaddull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t SimStateFixedLayout[] = {
...
inline constexpr int64_t SimStateFixedLayoutBytes = (int64_t) sizeof( SimStateFixedLayout );
...
inline constexpr TableFixedDst SimStateFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // SimState
...
// THE IDENTITY PLAN, coalesced at COMPILE TIME out of 27 leaves.
inline constexpr auto SimStateFixedPlan = TableFixedBuildPlan<27>( SimStateFixedLeaves );
...
inline constexpr int64_t SimStateFixedMeasure( int64_t count )
{
...
inline int64_t SimStateFixedSave( const SimState * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) SimStateFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = SimStateFixedPlan.entries;
    int32_t entry_count = SimStateFixedPlan.count;
    ...
    if ( hash != SimStateFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, SimStateFixedLayout, (int32_t) SimStateFixedLayoutBytes, SimStateFixedDst, plan, plan_capacity, report );
