...
struct TableWriter
{
    ...
    SCALARDEMO_TABLE_INLINE void put64( uint64_t v )
    {
        ...
        switch ( entry.id )
        {
            ...
            case 0xe3b1ca6a3b48dddcull: // samples
            {
                ...
                }
                {
                    ...
                    uint64_t walk = n;
                    if ( surplus_bits >= 0 && walk > (uint64_t) 3 ) { walk = (uint64_t) 3; } // the surplus is arithmetic
...
static_assert( sizeof( Pose ) == 24, "Pose: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( Pose, x ) == 0, "Pose.x: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Pose, y ) == 8, "Pose.y: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Pose, heading ) == 16, "Pose.heading: the fixed form's plan lands here" );
static_assert( sizeof( SimState ) == 304, "SimState: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( SimState, tilt ) == 0, "SimState.tilt: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, angle ) == 4, "SimState.angle: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, position ) == 8, "SimState.position: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, reach ) == 16, "SimState.reach: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, ticks ) == 32, "SimState.ticks: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, ratio ) == 36, "SimState.ratio: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, speed ) == 40, "SimState.speed: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, span ) == 48, "SimState.span: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, mass ) == 64, "SimState.mass: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, frames ) == 80, "SimState.frames: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, flux ) == 96, "SimState.flux: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, energy ) == 112, "SimState.energy: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, entity_id ) == 128, "SimState.entity_id: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, scale ) == 144, "SimState.scale: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, samples ) == 148, "SimState.samples: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, weights ) == 160, "SimState.weights: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, weights_count ) == 168, "SimState.weights_count: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, axes.slots ) == 176, "SimState.axes: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, seeds ) == 208, "SimState.seeds: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, seeds_count ) == 240, "SimState.seeds_count: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, pose ) == 248, "SimState.pose: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, spawn ) == 272, "SimState.spawn: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( SimState, spawn_present ) == 296, "SimState.spawn_present: the fixed form's plan lands here" );

// Pose's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void PoseFixedWriteBody( uint8_t * b, const Pose & value )
{
    (void) b; (void) value;
    TableFixedPut64( b + 0, (uint64_t) value.x );
    TableFixedPut64( b + 8, (uint64_t) value.y );
    TableFixedPut32( b + 16, (uint32_t) value.heading );
}

// SimState's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void SimStateFixedWriteBody( uint8_t * b, const SimState & value )
{
    (void) b; (void) value;
    TableFixedPut8( b + 0, (uint8_t) value.tilt );
    TableFixedPut32( b + 1, (uint32_t) value.angle );
    TableFixedPut64( b + 5, (uint64_t) value.position );
    TableFixedPut128( b + 13, value.reach );
    TableFixedPut32( b + 29, (uint32_t) value.ticks );
    TableFixedPut8( b + 33, (uint8_t) value.ratio );
    TableFixedPut32( b + 34, (uint32_t) value.speed );
    TableFixedPut64( b + 38, (uint64_t) value.span );
    TableFixedPut128( b + 46, value.mass );
    TableFixedPut32( b + 62, (uint32_t) value.frames );
    TableFixedPut128( b + 66, value.flux );
...
constexpr uint8_t SimStateFixedLayout[] = {
...
constexpr int64_t SimStateFixedLayoutBytes = (int64_t) sizeof( SimStateFixedLayout );
...
constexpr TableFixedEntry SimStateFixedPlan[] = {
    ...
    { 34u, 40u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // speed
    { 38u, 48u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // span
    { 46u, 64u, 20u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // mass
    { 66u, 96u, 64u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // flux
    { 130u, 168u, 4u, 0u, kTableFixedNoGuard, kTableFixedCount, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // weights count
    { 134u, 160u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // weights, whole
    { 142u, 176u, 24u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // axes, whole
...
constexpr TableFixedFill SimStateFixedCover[] = {
    { 0u, 1u },
    { 4u, 33u },
    { 40u, 4u },
    { 48u, 8u },
...
constexpr int64_t SimStateFixedMeasure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + SimStateFixedLayoutBytes + count * SimStateFixedRecordBytes;
}

inline int64_t SimStateFixedSave( const SimState * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = SimStateFixedMeasure( count );
    ...
    TableFixedPut64( buffer + kTableFixedHashAt, SimStateFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) SimStateFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, SimStateFixedLayout, (size_t) SimStateFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + SimStateFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
        TableFixedPut64( at, SimStateFixedHash );
        memset( at + 8, 0, (size_t) SimStateFixedBodyBytes ); // the prefill's zeros
        SimStateFixedWriteBody( at + 8, values[k] );
        at += SimStateFixedRecordBytes;
    }
    return need;
}
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    int64_t record_bytes = SimStateFixedRecordBytes;
    const TableFixedFill * fill = NULL;
    int32_t fill_count = 0;
    if ( hash != SimStateFixedHash )
    {
        ...
        else
        {
            ...
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, SimStateFixedLayout, (int32_t) SimStateFixedLayoutBytes, SimStateFixedDst,
...
template <typename... Args>
inline void SimStateSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "SimState is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void SimStateLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "SimState is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void SimStateSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "SimState is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PoseLoadRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Pose is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}

template <typename... Args>
inline void PoseMeasureRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Pose is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PoseSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Pose is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PoseLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Pose is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PoseSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Pose is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
