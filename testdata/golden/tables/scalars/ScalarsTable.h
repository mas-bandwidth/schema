...
// SimState's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
...
inline void SimStateFixedScatter( const uint8_t * TABLE_RESTRICT image, SimState & value, TableReport * report )
{
    uint8_t * TABLE_RESTRICT dst = (uint8_t *) &value;
    int32_t clamped = 0;
    (void) dst; (void) clamped;
    memcpy( dst + 0, image + 0, 1 ); // tilt
    memcpy( dst + 4, image + 1, 33 ); // angle
    memcpy( dst + 40, image + 34, 4 ); // speed
    memcpy( dst + 48, image + 38, 8 ); // span
    memcpy( dst + 64, image + 46, 20 ); // mass
    memcpy( dst + 96, image + 66, 64 ); // flux
    { // weights count
        int32_t v = (int32_t) TableFixedGet32( image + 130 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 4 ) { v = 4; clamped++; }
        memcpy( dst + 168, &v, 4 );
    }
    memcpy( dst + 160, image + 134, 8 ); // weights, whole
    memcpy( dst + 176, image + 142, 24 ); // axes, whole
    { // seeds count
        int32_t v = (int32_t) TableFixedGet32( image + 166 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 2 ) { v = 2; clamped++; }
        memcpy( dst + 240, &v, 4 );
    }
    memcpy( dst + 208, image + 170, 32 ); // seeds, whole
    memcpy( dst + 248, image + 202, 20 ); // x
    memcpy( dst + 296, image + 222, 1 ); // spawn present
    memcpy( dst + 272, image + 223, 20 ); // x
    report->clamped += clamped;
}
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == SimStateFixedHash );
    ...
    if ( identity )
    {
        ...
        uint8_t image[SimStateFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) SimStateFixedBodyBytes );
            SimStateFixedScatter( image, values[k], report );
            SimStateFixedClamp( values[k], report ); // the bounds the scatter does not hold (§3.4)
            at += SimStateFixedRecordBytes;
        }
        return n;
    }
