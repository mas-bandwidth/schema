...
static const TableFieldInfo schema_graphdemo_stamp_fields_[] = {
    { "tag", "tag", "string", 0xbc64, 12, 0, 0, 1, 0, 8, (uint32_t) offsetof( Stamp, tag ), (uint32_t) sizeof( ( (Stamp *) 0 )->tag ), (uint32_t) offsetof( Stamp, tag_length ), 0xffffffffu, NULL, 0, 0.0, 0.0, -1, NULL, 0, NULL, NULL, -1, NULL, "" },
    { "seq", "seq", "int32", 0xc29b, 4, 0, 0, 0, 0, 0, (uint32_t) offsetof( Stamp, seq ), (uint32_t) sizeof( ( (Stamp *) 0 )->seq ), 0xffffffffu, 0xffffffffu, NULL, 1, 0.0, 1000.0, -1, NULL, 0, NULL, NULL, -1, NULL, "" },
};
...
{
    return TableJsonRead( value, &schema_graphdemo_stamp_info_, text, bytes, report );
}

int64_t schema_graphdemo_stamp_to_json_( const Stamp * value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( value, &schema_graphdemo_stamp_info_, buffer, capacity );
}

int schema_graphdemo_colour_from_json_( Colour * value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( value, &schema_graphdemo_colour_info_, text, bytes, report );
}

int64_t schema_graphdemo_colour_to_json_( const Colour * value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( value, &schema_graphdemo_colour_info_, buffer, capacity );
}
