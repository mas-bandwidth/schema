...
static SCHEMA_UNUSED const uint64_t meta_fixed_hash = 0x6a1246b224bde396ull; /* fnv1a64 over the block's bytes */
...
static SCHEMA_UNUSED const uint8_t meta_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t meta_fixed_vocab_bytes = (int64_t) sizeof( meta_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( meta_fixed_vocab ) + count * ( 8 + 16 );
}
...
static SCHEMA_UNUSED int64_t meta_fixed_save( const Meta * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( meta_fixed_vocab ) );
    memcpy( buffer + 5, meta_fixed_vocab, sizeof( meta_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( meta_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != meta_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, meta_fixed_vocab, (int32_t) sizeof( meta_fixed_vocab ), meta_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
...
static SCHEMA_UNUSED const uint64_t settings_fixed_hash = 0x34bfbe0ea79913f1ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 3 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t settings_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t settings_fixed_vocab_bytes = (int64_t) sizeof( settings_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( settings_fixed_vocab ) + count * ( 8 + 24 );
}
...
static SCHEMA_UNUSED int64_t settings_fixed_save( const Settings * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( settings_fixed_vocab ) );
    memcpy( buffer + 5, settings_fixed_vocab, sizeof( settings_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( settings_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != settings_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_vocab_read( block, (int64_t) block_bytes, &parsed ) ) { report->refused = 1; report->reason = SCHEMA_TABLE_BLOCK_MALFORMED; return -1; }
        made = table_fixed_compile( &parsed, settings_fixed_vocab, (int32_t) sizeof( settings_fixed_vocab ), settings_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_BLOCK_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
