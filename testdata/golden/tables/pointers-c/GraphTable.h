...
static SCHEMA_UNUSED int settings_save_body( TableWriter * w, const Settings * value )
{
    ...
    { /* quality */
        if ( !( value->quality == 2 ) )
        {
            table_writer_header_at( w, 24, 0x7a8060916400fe66ull, 4 );
    ...
    { /* label */
        ...
        {
            table_writer_header_at( w, 7, 0x39f7fcec8fcb623dull, 12 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
static SCHEMA_UNUSED int list_node_save_body( TableWriter * w, const ListNode * value )
{
    ...
    { /* value */
        if ( !( value->value == 0 ) )
        {
            table_writer_header_at( w, 25, 0x7ce4fd9430e80ceaull, 4 );
...
static SCHEMA_UNUSED int tree_node_save_body( TableWriter * w, const TreeNode * value )
{
    ...
    { /* right */
        ...
        {
            table_writer_header_at( w, 22, 0x76aaaa535714d805ull, 17 );
...
static SCHEMA_UNUSED int layer_save_body( TableWriter * w, const Layer * value )
{
    ...
    { /* depth */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_header_at( w, 21, 0x75d8e97600b296eaull, 4 );
...
static SCHEMA_UNUSED int scene_save_body( TableWriter * w, const Scene * value )
{
    ...
    { /* tree */
        ...
        {
            table_writer_header_at( w, 15, 0x5b25b8ef511eb395ull, 17 );
    ...
    { /* alias */
        ...
        {
            table_writer_header_at( w, 13, 0x509220bb65a646b7ull, 17 );
    ...
    { /* ground */
        ...
        {
            table_writer_header_at( w, 16, 0x60839e2395be697eull, 13 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
    ...
    { /* layers */
        ...
        {
            table_writer_header_at( w, 11, 0x4554e34a747022dfull, 14 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
    ...
    { /* meta */
        ...
        if ( default_probe.offset > 1 )
        {
            table_writer_header_at( w, 9, 0x4320e9a2e32eac38ull, 13 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
static SCHEMA_UNUSED int depot_save_body( TableWriter * w, const Depot * value )
{
    ...
    { /* spare */
        ...
        {
            table_writer_header_at( w, 10, 0x4339ee8ab21c8380ull, 13 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
static SCHEMA_UNUSED int album_save_body( TableWriter * w, const Album * value )
{
    ...
    { /* tint */
        TableWriter default_probe = table_writer_probe( w ); default_probe.check_default = 1;
        if ( !color_save_body( &default_probe, &value->tint ) ) { return 0; }
        table_writer_rewind(&default_probe);
        if ( default_probe.offset > 1 )
        {
            ...
            {
                TableWriter probe = table_writer_probe( w );
                if ( !color_save_body( &probe, &value->tint ) ) { return 0; }
                table_writer_rewind(&probe); table_writer_leb( w, (uint64_t) probe.offset );
                if ( !color_save_body( w, &value->tint ) ) { return 0; }
            }
        }
    ...
    { /* pin */
        ...
        {
            table_writer_header_at( w, 23, 0x77af761956600b54ull, 17 );
    ...
    for ( ;; )
    {
        ...
        switch ( id )
        {
            ...
            case 0x1e4984ef2e958a4cull: /* tint */
            {
                ...
                if ( sub.offset != sub.size ) { r->report->malformed = 1; color_reset( &value->tint ); }
                break;
            }
...
static SCHEMA_UNUSED int album_save_message_body(TableBitWriter * w,const Album * value)
{
 ...
 probe.bits=0;
 probe.check_default=1;
 if(!color_save_message_body(&probe,&value->tint)) return 0;
 if(probe.bits>kTableMessageRefBitsHere) {
  table_bit_put(w,2,kTableMessageRefBitsHere);
  if(!color_save_message_body(w,&value->tint)) return 0;
 }
 }
 { /* stamp */
 TableBitWriter probe=*w;
...
static SCHEMA_UNUSED int album_load_message_body(TableMessageReader * r,Album * value)
{
 ...
 if(!color_load_message_body(r,&value->tint))return 0;
 break;
 }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != meta_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, meta_fixed_layout, (int32_t) sizeof( meta_fixed_layout ), meta_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != settings_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_parse_layout( layout, (int64_t) layout_bytes, &parsed, &why ) ) { report->refused = 1; report->reason = why; return -1; }
        made = table_fixed_compile( &parsed, settings_fixed_layout, (int32_t) sizeof( settings_fixed_layout ), settings_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_LAYOUT_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
static SCHEMA_UNUSED int album_save_body_retain( TableWriter * w, const Album * value , TableRetainWalk retention)
{
    ...
    retention=table_retain_step(body_keep,1,0);
        TableWriter default_probe = table_retain_probe( w , retention);
        default_probe.check_default = 1;
        if ( !color_save_body_retain( &default_probe, &value->tint , retention) ) { return 0;
        }
        table_retain_rewind(&default_probe, retention);
        if ( default_probe.offset > 1 )
        {
            ...
            {
                int64_t frame_begin = w->offset;
                if ( !color_save_body_retain( w, &value->tint , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                TableWriter probe = table_retain_probe( w , retention);
                if ( !color_save_body_retain( &probe, &value->tint , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !color_save_body_retain( w, &value->tint , retention) ) { return 0;
    ...
    for ( ;; )
    {
        ...
        switch ( id )
        {
            ...
            case 0x1e4984ef2e958a4cull: /* tint */
            {
                ...
                color_load_body_retain( &sub, &value->tint , retention);
                if ( sub.offset != sub.size ) { r->report->malformed = 1;
                color_reset( &value->tint );
                }
                break;
...
static SCHEMA_UNUSED int album_load_message_body_retain(TableMessageReader * r,Album * value, TableRetainWalk retention)
{
 ...
 retention=table_retain_step(body_keep,1,0);
 table_retain_discard(body_keep,1,1);
 if(!color_load_message_body_retain(r,&value->tint, retention))return 0;
 break;
 }
 case UINT64_C(0xee7ba9ad45c64144): {
 if(!(entry->kind==13 && entry->elem_kind==0)){if(entry->elem_kind==0 && table_kind_widens(entry->kind,13)){r->report->widened++;
...
static SCHEMA_UNUSED int schema_graphdemo_album_cook_body_(struct TableNumbering * n,uint8_t * at,const void * storage,int order)
{
    ...
    if(!schema_graphdemo_color_cook_body_(n,at+24,&value->tint,order)) { return 0; }
