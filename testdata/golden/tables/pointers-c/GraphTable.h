...
static SCHEMA_UNUSED const uint64_t meta_fixed_hash = 0x6a1246b224bde396ull; /* fnv1a64 over the layout's bytes */
...
/* Meta's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by the
...
static SCHEMA_UNUSED SCHEMA_GRAPHDEMO_TABLE_INLINE void schema_graphdemo_meta_fixed_scatter_( const uint8_t * SCHEMA_TABLE_RESTRICT image, Meta * value, TableReport * report )
{
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( !identity )
    {
        ...
        made = table_fixed_compile( &parsed, meta_fixed_layout, (int32_t) sizeof( meta_fixed_layout ), meta_fixed_dst,
                                    meta_fixed_cover, meta_fixed_cover_count,
    ...
    if ( identity )
    {
        ...
        for ( k = 0; k < count; ++k )
        {
            ...
            schema_graphdemo_meta_fixed_scatter_( image, values + k, report );
            schema_graphdemo_meta_fixed_clamp_( values + k, report ); /* the bounds the scatter does not hold (§3.4) */
            at += 8 + 16;
        }
        return count;
    }
...
static SCHEMA_UNUSED const uint64_t settings_fixed_hash = 0x34bfbe0ea79913f1ull; /* fnv1a64 over the layout's bytes */
...
/* Settings's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by the
   schema compiler into constant-offset moves. Every number in it is a number
   from that plan, and the SCHEMA_TABLE_STATIC_ASSERTs at the top of this form
   tie each one to this compiler's own offsetof and sizeof. */
static SCHEMA_UNUSED SCHEMA_GRAPHDEMO_TABLE_INLINE void schema_graphdemo_settings_fixed_scatter_( const uint8_t * SCHEMA_TABLE_RESTRICT image, Settings * value, TableReport * report )
{
    uint8_t * SCHEMA_TABLE_RESTRICT dst = (uint8_t *) value;
    int32_t clamped = 0;
    (void) dst; (void) clamped;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( !identity )
    {
        ...
        uint32_t fill_at = 0;
        if ( !table_fixed_parse_layout( layout, (int64_t) layout_bytes, &parsed, &why ) ) { report->refused = 1; report->reason = why; return -1; }
        made = table_fixed_compile( &parsed, settings_fixed_layout, (int32_t) sizeof( settings_fixed_layout ), settings_fixed_dst,
                                    settings_fixed_cover, settings_fixed_cover_count,
                                    plan, plan_capacity, &compiled_guarded, &fill_at, &fill_count, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_LAYOUT_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
    ...
    if ( identity )
    {
        ...
        for ( k = 0; k < count; ++k )
        {
            ...
            schema_graphdemo_settings_fixed_scatter_( image, values + k, report );
            schema_graphdemo_settings_fixed_clamp_( values + k, report ); /* the bounds the scatter does not hold (§3.4) */
            at += 8 + 24;
        }
        return count;
    }
