...
SCHEMA_TABLE_STATIC_ASSERT( fixed_meta_size, sizeof( Meta ) == 20, "Meta: the fixed form's plan is laid out for this size" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_meta_build, offsetof( Meta, build ) == 0, "Meta.build: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_meta_tag, offsetof( Meta, tag ) == 4, "Meta.tag: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_meta_tag_length, offsetof( Meta, tag_length ) == 16, "Meta.tag_length: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_settings_size, sizeof( Settings ) == 28, "Settings: the fixed form's plan is laid out for this size" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_settings_quality, offsetof( Settings, quality ) == 0, "Settings.quality: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_settings_label, offsetof( Settings, label ) == 4, "Settings.label: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_settings_label_length, offsetof( Settings, label_length ) == 24, "Settings.label_length: the fixed form's plan lands here" );

static SCHEMA_UNUSED SCHEMA_GRAPHDEMO_TABLE_INLINE void schema_graphdemo_meta_fixed_write_body_( uint8_t * b, const Meta * value );
static SCHEMA_UNUSED SCHEMA_GRAPHDEMO_TABLE_INLINE void schema_graphdemo_settings_fixed_write_body_( uint8_t * b, const Settings * value );

/* Meta's stores. The template — the hash, then zeros — is memcpy'd first,
...
static SCHEMA_UNUSED const int64_t meta_fixed_body_bytes = 16;
static SCHEMA_UNUSED const int64_t meta_fixed_record_bytes = 8 + 16; /* the hash and the body */
static SCHEMA_UNUSED const uint64_t meta_fixed_hash = 0x13e3bd2aacd08c75ull; /* fnv1a64 over the layout and the definitions digest (bill §13) */
...
static SCHEMA_UNUSED const uint8_t meta_fixed_layout[] = {
    0x03, 0x00, 0x00, 0x00, 0x18, 0x6d, 0x9d, 0xe4, 0xae, 0xea, 0xad, 0x6d, 0x0d, 0x10, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0x03, 0x0b, 0xc7, 0x98, 0xe2, 0x17, 0x25, 0x80, 0x04, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf3, 0xa4, 0x48, 0x44, 0x19, 0xab, 0xd7, 0x56, 0x0c, 0x0c,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
static SCHEMA_UNUSED const int64_t meta_fixed_layout_bytes = (int64_t) sizeof( meta_fixed_layout );
...
static SCHEMA_UNUSED const TableFixedEntry meta_fixed_plan[] = {
    { 0u, 0u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1, SCHEMA_TABLE_FIXED_NO_GUARD, 0, 1 }, /* build */
    { 4u, 16u, 8u, 4u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 0, 1, 0, 0, 1, SCHEMA_TABLE_FIXED_NO_GUARD, 0, 1 }, /* tag */
};
...
static SCHEMA_UNUSED const TableFixedFill meta_fixed_cover[] = {
    { 0u, 13u },
    { 16u, 4u },
};
static SCHEMA_UNUSED const int32_t meta_fixed_cover_count = 2;
...
/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   ...
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout meta_fixed_known[] = {
    { 0x13e3bd2aacd08c75ull, meta_fixed_layout, (int64_t) sizeof( meta_fixed_layout ), 24 },
};
static SCHEMA_UNUSED const int32_t meta_fixed_known_count = 1;
...
static SCHEMA_UNUSED const int32_t meta_fixed_floor = 0;
...
static SCHEMA_UNUSED int64_t meta_fixed_measure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + (int64_t) sizeof( meta_fixed_layout ) + count * ( 8 + 16 );
}

static SCHEMA_UNUSED int64_t meta_fixed_save( const Meta * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = meta_fixed_measure( count );
    uint8_t * at;
    int64_t k;
    ...
    table_fixed_put64( buffer + kTableFixedHashAt, meta_fixed_hash );
    table_fixed_put32( buffer + kTableFixedHeaderBytes, (uint32_t) sizeof( meta_fixed_layout ) );
    memcpy( buffer + kTableFixedHeaderBytes + 4, meta_fixed_layout, sizeof( meta_fixed_layout ) );
    at = buffer + kTableFixedHeaderBytes + 4 + (int64_t) sizeof( meta_fixed_layout );
    for ( k = 0; k < count; ++k )
    {
        table_fixed_put64( at, meta_fixed_hash );
        memset( at + 8, 0, (size_t) 16 ); /* the prefill's zeros */
        schema_graphdemo_meta_fixed_write_body_( at + 8, values + k );
        at += 8 + 16;
    }
    return need;
}
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    pick = table_fixed_select( meta_fixed_known, meta_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < meta_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    ...
    if ( (int64_t) layout_bytes != meta_fixed_known[pick].layout_bytes ||
         memcmp( layout, meta_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    ...
    record_bytes = meta_fixed_known[pick].record_bytes;
    ...
    for ( k = 0; k < count; ++k )
    {
        ...
        table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), (uint32_t) sizeof( Meta ), report );
...
/* THE BODY IS ONE CONSTANT on this form: it is the same size for every
   value the type can hold (docs/SPEC-TABLES.md §3.4). */
static SCHEMA_UNUSED const int64_t settings_fixed_body_bytes = 24;
static SCHEMA_UNUSED const int64_t settings_fixed_record_bytes = 8 + 24; /* the hash and the body */
static SCHEMA_UNUSED const uint64_t settings_fixed_hash = 0xbf8a406d152d5cbdull; /* fnv1a64 over the layout and the definitions digest (bill §13) */

/* THE LAYOUT (form 1 calls this the vocabulary block): 3 entries, a
   PRE-ORDER walk of the closure in the writer's declared order.
   Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t settings_fixed_layout[] = {
    0x03, 0x00, 0x00, 0x00, 0xc8, 0xc2, 0x04, 0xb4, 0xa2, 0x8a, 0x8b, 0x9d, 0x0d, 0x18, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0x66, 0xfe, 0x00, 0x64, 0x91, 0x60, 0x80, 0x7a, 0x04, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3d, 0x62, 0xcb, 0x8f, 0xec, 0xfc, 0xf7, 0x39, 0x0c, 0x14,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
static SCHEMA_UNUSED const int64_t settings_fixed_layout_bytes = (int64_t) sizeof( settings_fixed_layout );
...
static SCHEMA_UNUSED const TableFixedEntry settings_fixed_plan[] = {
    { 0u, 0u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1, SCHEMA_TABLE_FIXED_NO_GUARD, 0, 1 }, /* quality */
    { 4u, 24u, 16u, 4u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 0, 1, 0, 0, 1, SCHEMA_TABLE_FIXED_NO_GUARD, 0, 1 }, /* label */
};
...
/* THE TYPE'S VALUE BYTES: every byte of this build's own storage that holds
   ...
   twice; a plan compiled from a stranger's layout subtracts itself from it
   once, when the plan is compiled, and prefills exactly the rest. */
static SCHEMA_UNUSED const TableFixedFill settings_fixed_cover[] = {
    { 0u, 21u },
    { 24u, 4u },
};
static SCHEMA_UNUSED const int32_t settings_fixed_cover_count = 2;

/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   A FILE IS MATCHED ON IS THE HASH; the layout bytes are what a known hash
   is held to by memcmp, and the record size is taken from here and NEVER
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout settings_fixed_known[] = {
    { 0xbf8a406d152d5cbdull, settings_fixed_layout, (int64_t) sizeof( settings_fixed_layout ), 32 },
};
static SCHEMA_UNUSED const int32_t settings_fixed_known_count = 1;
/* THE FLOOR IS ONE NUMBER AND THE LINEAGE IS ONE ARRAY, so "retired" is an
   index cut and the operator's two answers stay distinct: below the floor is
   layout_unsupported (upgrade the client), outside the lineage is
   layout_newer (ship the reader). A retired entry stays forever (§5.2). */
static SCHEMA_UNUSED const int32_t settings_fixed_floor = 0;

/* A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
   — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
   then the layout behind its u32 length, then the records to the end of it. */
static SCHEMA_UNUSED int64_t settings_fixed_measure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + (int64_t) sizeof( settings_fixed_layout ) + count * ( 8 + 24 );
}

static SCHEMA_UNUSED int64_t settings_fixed_save( const Settings * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = settings_fixed_measure( count );
    uint8_t * at;
    int64_t k;
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    memset( buffer, 0, (size_t) kTableFixedHeaderBytes ); /* the seven reserved bytes, and the rest of the header */
    buffer[0] = kTableFixedForm;
    table_fixed_put64( buffer + kTableFixedHashAt, settings_fixed_hash );
    table_fixed_put32( buffer + kTableFixedHeaderBytes, (uint32_t) sizeof( settings_fixed_layout ) );
    memcpy( buffer + kTableFixedHeaderBytes + 4, settings_fixed_layout, sizeof( settings_fixed_layout ) );
    at = buffer + kTableFixedHeaderBytes + 4 + (int64_t) sizeof( settings_fixed_layout );
    for ( k = 0; k < count; ++k )
    {
        table_fixed_put64( at, settings_fixed_hash );
        memset( at + 8, 0, (size_t) 24 ); /* the prefill's zeros */
        schema_graphdemo_settings_fixed_write_body_( at + 8, values + k );
        at += 8 + 24;
    }
    return need;
}
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    hash = table_fixed_get64( data + kTableFixedHashAt );
    /* 5 and 6. SELECT BY HASH, THEN THE FLOOR. Both report THE FILE'S hash. */
    pick = table_fixed_select( settings_fixed_known, settings_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < settings_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    /* 7. A KNOWN HASH IS READ UNDER THE LOCK'S LAYOUT BYTES. A difference is ONE
       name — a lie about a known version — and §1.1's seven rules do not run at
       read time at all: no stranger's layout is ever walked. */
    if ( (int64_t) layout_bytes != settings_fixed_known[pick].layout_bytes ||
         memcmp( layout, settings_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    { report->refused = 1; report->reason = SCHEMA_TABLE_LAYOUT_MALFORMED; return -1; }
    /* 8. the plan this peer resolves to, and the record size FROM THE LOCK. */
    record_bytes = settings_fixed_known[pick].record_bytes;
    /* 9 and 10. the tail must be whole records, and they must fit the caller. */
    at = layout + layout_bytes;
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    if ( record_bytes <= 8 || rest % record_bytes != 0 ) { report->malformed = 1; return -1; }
    ...
    for ( k = 0; k < count; ++k )
    {
        if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
        table_fixed_fill_run( fill, fill_count, (const uint8_t *) &defaults, (uint8_t *) ( values + k ) );
        table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), (uint32_t) sizeof( Settings ), report );
        /* AND THE BOUNDS THE LOOP DOES NOT HOLD, straight-line over the
           storage it just wrote: the same pass for either plan (§3.4). */
