...
static SCHEMA_UNUSED const TableFixedEntry tally_fixed_plan[] = {
    { 0u, 0u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0 }, /* hits */
};
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    record_bytes = 8 + 4;
    identity = ( hash == tally_fixed_hash );
    if ( !identity )
    {
        ...
        made = table_fixed_compile( &parsed, tally_fixed_layout, (int32_t) sizeof( tally_fixed_layout ), tally_fixed_dst,
                                    tally_fixed_cover, tally_fixed_cover_count,
    ...
    if ( identity )
    {
        ...
        for ( k = 0; k < count; ++k )
        {
            ...
            memcpy( (void *) ( values + k ), at + 8, (size_t) 4 );
            schema_graphdemo_tally_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 4;
        }
        return count;
    }
