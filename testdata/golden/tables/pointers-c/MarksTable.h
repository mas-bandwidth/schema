...
static SCHEMA_UNUSED const uint64_t tally_fixed_hash = 0xe155ccfe28f460d7ull; /* fnv1a64 over the layout and the definitions digest (bill §13) */
...
static SCHEMA_UNUSED const TableFixedEntry tally_fixed_plan[] = {
    { 0u, 0u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1, SCHEMA_TABLE_FIXED_NO_GUARD, 0, 1 }, /* hits */
};
...
static SCHEMA_UNUSED const TableFixedFill tally_fixed_cover[] = {
    { 0u, 4u },
};
static SCHEMA_UNUSED const int32_t tally_fixed_cover_count = 1;
...
/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   ...
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout tally_fixed_known[] = {
    { 0xe155ccfe28f460d7ull, tally_fixed_layout, (int64_t) sizeof( tally_fixed_layout ), 12 },
};
static SCHEMA_UNUSED const int32_t tally_fixed_known_count = 1;
...
static SCHEMA_UNUSED const int32_t tally_fixed_floor = 0;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    pick = table_fixed_select( tally_fixed_known, tally_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < tally_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    ...
    if ( (int64_t) layout_bytes != tally_fixed_known[pick].layout_bytes ||
         memcmp( layout, tally_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    ...
    record_bytes = tally_fixed_known[pick].record_bytes;
    ...
    for ( k = 0; k < count; ++k )
    {
        ...
        table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), (uint32_t) sizeof( Tally ), report );
