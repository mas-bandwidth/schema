...
static SCHEMA_UNUSED int stamp_save_message_body(TableBitWriter * w,const Stamp * value)
{
 ...
 if(value->tag_length != 0) {
  ...
  table_bit_align_write(w); table_bit_putbytes(w,value->tag,value->tag_length);
 }
 }
 { /* seq */
 if(!(value->seq == 0)) {
...
{
 stamp_reset(value);for(;;){const TableMessageEntry * entry;
 ...
 {uint64_t n,kept;if(!table_bit_get(&r->bits,&n,table_bits_required(0,entry->max)))goto malformed;
  ...
  {const uint8_t * text=r->bits.buffer+(r->bits.offset>>3);if(!table_wire_utf8(text,n))goto malformed;kept=table_wire_utf8_clamp(text,n,8);memcpy(value->tag,text,(size_t)kept);value->tag[kept]=0;}
  r->bits.offset+=(int64_t)n*8;
  if(kept<n)r->report->clamped++;value->tag_length=(int32_t)kept; }
 break;}
...
static SCHEMA_UNUSED int stamp_load_messages(Stamp * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 for(i=0;i<bodies;i++){if(!stamp_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
{
 colour_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
...
static SCHEMA_UNUSED int colour_load_messages(Colour * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!colour_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int stamp_save_body_retain( TableWriter * w, const Stamp * value , TableRetainWalk retention)
{
    ...
    retention=table_retain_step(body_keep,0,0);
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_retain_writer_id( w, 0x56d7ab194448a4f3ull , retention); table_writer_put8( w, 12 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
            ...
            else
            {
                ...
                table_retain_rewind(&probe, retention); table_writer_leb( w, (uint64_t) probe.offset );
                if ( !schema_graphdemo_stamp_wire_tag__retain( w, value , retention) ) { return 0; }
            }
        }
    }
    ...
    retention=table_retain_step(body_keep,1,0);
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_retain_writer_id( w, 0x823b8a195ce2133cull , retention); table_writer_put8( w, 4 );
            table_writer_put32( w, (uint32_t) ( value->seq ) );
        }
    }
...
static SCHEMA_UNUSED int stamp_load_body_retain( TableReader * r, Stamp * value , TableRetainWalk retention)
{
    ...
    stamp_reset( value );
    TableRetainWalk body_keep=retention;table_retain_discard(retention,0,0);
    for ( ;; )
    {
        uint64_t ref, id; uint8_t kind;
        if ( !table_reader_leb( r, &ref ) ) { r->report->malformed = 1; return 0; }
        ...
        switch ( id )
        {
            ...
            case UINT64_MAX:
                if(r->nodes==NULL) { r->report->unknown++;r->report->retain_lost += retention.retain != NULL; }
                if(!table_reader_skip(r,kind)) { r->report->malformed=1; return 0; }
                break;
            default:
                r->report->unknown += 1;
...
static SCHEMA_UNUSED int stamp_load_message_body_retain(TableMessageReader * r,Stamp * value, TableRetainWalk retention)
{
 ...
 { uint64_t lo,hi;if(!table_message_number(r,entry,&lo,&hi))goto malformed;
  ...
  value->seq=(int32_t)decoded; }
 break;}
 default:r->report->unknown += 1;if(!table_retain_message_capture(r,entry,body_keep))goto malformed;break;
 } }
 malformed:r->report->malformed=1;return 0;
}
static SCHEMA_UNUSED int colour_save_body_retain( TableWriter * w, const Colour * value , TableRetainWalk retention)
{
    ...
    retention=table_retain_step(body_keep,0,0);
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_retain_writer_id( w, 0xaf63ef4c86020cd5ull , retention); table_writer_put8( w, 6 );
            table_writer_put8( w, (uint8_t) ( value->r ) );
        }
    }
    { /* g */
    retention=table_retain_step(body_keep,1,0);
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_retain_writer_id( w, 0xaf63da4c8601e926ull , retention); table_writer_put8( w, 6 );
            table_writer_put8( w, (uint8_t) ( value->g ) );
        }
    }
    { /* b */
    retention=table_retain_step(body_keep,2,0);
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_retain_writer_id( w, 0xaf63df4c8601f1a5ull , retention); table_writer_put8( w, 6 );
            table_writer_put8( w, (uint8_t) ( value->b ) );
        }
    }
    retention=body_keep;if(!table_retain_tail(w,retention))return 0;
