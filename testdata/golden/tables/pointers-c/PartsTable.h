...
SCHEMA_TABLE_STATIC_ASSERT( fixed_stamp_size, sizeof( Stamp ) == 20, "Stamp: the fixed form's plan is laid out for this size" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_stamp_tag, offsetof( Stamp, tag ) == 0, "Stamp.tag: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_stamp_tag_length, offsetof( Stamp, tag_length ) == 12, "Stamp.tag_length: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_stamp_seq, offsetof( Stamp, seq ) == 16, "Stamp.seq: the fixed form's plan lands here" );
...
static SCHEMA_UNUSED SCHEMA_GRAPHDEMO_TABLE_INLINE void schema_graphdemo_stamp_fixed_clamp_body_( Stamp * value, int32_t * clamped, int32_t * damaged )
{
...
static SCHEMA_UNUSED void schema_graphdemo_stamp_fixed_clamp_( Stamp * value, TableReport * report )
{
    int32_t clamped = 0;
    int32_t damaged = 0;
    schema_graphdemo_stamp_fixed_clamp_body_( value, &clamped, &damaged );
    report->clamped += clamped;
...
static SCHEMA_UNUSED const int64_t stamp_fixed_body_bytes = 16;
static SCHEMA_UNUSED const int64_t stamp_fixed_record_bytes = 8 + 16; /* the hash and the body */
static SCHEMA_UNUSED const uint64_t stamp_fixed_hash = 0x2b467a2ee937ea73ull; /* fnv1a64 over the layout and the definitions digest (bill §13) */
...
static SCHEMA_UNUSED const uint8_t stamp_fixed_layout[] = {
...
static SCHEMA_UNUSED const int64_t stamp_fixed_layout_bytes = (int64_t) sizeof( stamp_fixed_layout );
...
static SCHEMA_UNUSED const TableFixedDst stamp_fixed_dst[] = {
    { 0, 0, 0, 0, 0 }, /* Stamp */
    { 12, 0, 0, 0, 1 }, /* tag */
    { 16, 0, 0, 0, 0 }, /* seq */
};
...
static SCHEMA_UNUSED const TableFixedGuard stamp_fixed_guards[] = {
    { 0u, 0u, 0u, 1u } /* no union in this type: the pool names no condition */
};
static SCHEMA_UNUSED const int32_t stamp_fixed_guard_count = 0;
...
static SCHEMA_UNUSED const TableFixedEntry stamp_fixed_plan[] = {
    { 0u, 12u, 8u, 0u, 0u, 0, kTableFixedText, 1, 0, 0 }, /* tag */
...
static SCHEMA_UNUSED const TableFixedFill stamp_fixed_cover[] = {
    { 0u, 9u },
    { 12u, 8u },
};
static SCHEMA_UNUSED const int32_t stamp_fixed_cover_count = 2;
...
/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   ...
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout stamp_fixed_known[] = {
    { 0x2b467a2ee937ea73ull, stamp_fixed_layout, (int64_t) sizeof( stamp_fixed_layout ), 24 },
};
static SCHEMA_UNUSED const int32_t stamp_fixed_known_count = 1;
...
static SCHEMA_UNUSED const int32_t stamp_fixed_floor = 0;
...
static SCHEMA_UNUSED int64_t stamp_fixed_measure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + (int64_t) sizeof( stamp_fixed_layout ) + count * ( 8 + 16 );
}

static SCHEMA_UNUSED int64_t stamp_fixed_save( const Stamp * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = stamp_fixed_measure( count );
    uint8_t * at;
    int64_t k;
    ...
    table_fixed_put64( buffer + kTableFixedHashAt, stamp_fixed_hash );
    table_fixed_put32( buffer + kTableFixedHeaderBytes, (uint32_t) sizeof( stamp_fixed_layout ) );
    memcpy( buffer + kTableFixedHeaderBytes + 4, stamp_fixed_layout, sizeof( stamp_fixed_layout ) );
    at = buffer + kTableFixedHeaderBytes + 4 + (int64_t) sizeof( stamp_fixed_layout );
    for ( k = 0; k < count; ++k )
    {
        table_fixed_put64( at, stamp_fixed_hash );
        memset( at + 8, 0, (size_t) 16 ); /* the prefill's zeros */
        schema_graphdemo_stamp_fixed_write_body_( at + 8, values + k );
        at += 8 + 16;
    }
    return need;
}
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    const uint8_t * guard_pool = (const uint8_t *) stamp_fixed_guards;
    ...
    pick = table_fixed_select( stamp_fixed_known, stamp_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < stamp_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    ...
    if ( (int64_t) layout_bytes != stamp_fixed_known[pick].layout_bytes ||
         memcmp( layout, stamp_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    ...
    record_bytes = stamp_fixed_known[pick].record_bytes;
