...
static SCHEMA_UNUSED const uint64_t stamp_fixed_hash = 0x2aaaa93920374790ull; /* fnv1a64 over the block's bytes */
...
static SCHEMA_UNUSED const uint8_t stamp_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t stamp_fixed_vocab_bytes = (int64_t) sizeof( stamp_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( stamp_fixed_vocab ) + count * ( 8 + 16 );
}
...
static SCHEMA_UNUSED int64_t stamp_fixed_save( const Stamp * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( stamp_fixed_vocab ) );
    memcpy( buffer + 5, stamp_fixed_vocab, sizeof( stamp_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( stamp_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != stamp_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, stamp_fixed_vocab, (int32_t) sizeof( stamp_fixed_vocab ), stamp_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
