                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        for ( k = 0; k < count; ++k )
        {
            ...
            memcpy( (void *) ( values + k ), at + 8, (size_t) 72 );
            at += 8 + 72;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 81 );
            schema_blockdemo_render_ship_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 81;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 59 );
            schema_blockdemo_render_turret_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 59;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 71 );
            schema_blockdemo_render_missile_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 71;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        for ( k = 0; k < count; ++k )
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 71 );
            schema_blockdemo_render_dynamic_prop_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 71;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 78 );
            schema_blockdemo_render_static_prop_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 78;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 79 );
            schema_blockdemo_render_cosmetic_prop_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 79;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 62 );
            schema_blockdemo_render_laser_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 62;
        }
        return count;
    }
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( identity )
    {
        ...
        {
            if ( table_fixed_get64( at ) != hash ) { report->refused = 1; report->reason = SCHEMA_TABLE_NO_LAYOUT; return -1; }
            memcpy( (void *) ( values + k ), at + 8, (size_t) 74 );
            schema_blockdemo_render_explosion_fixed_clamp_( values + k, report ); /* the bounds the copy does not hold (§3.4) */
            at += 8 + 74;
        }
        return count;
    }
