...
static SCHEMA_UNUSED const uint64_t padded_row_fixed_hash = 0x9683695abec5c762ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 17 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t padded_row_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t padded_row_fixed_vocab_bytes = (int64_t) sizeof( padded_row_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( padded_row_fixed_vocab ) + count * ( 8 + 50 );
}
...
static SCHEMA_UNUSED int64_t padded_row_fixed_save( const PaddedRow * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( padded_row_fixed_vocab ) );
    memcpy( buffer + 5, padded_row_fixed_vocab, sizeof( padded_row_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( padded_row_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != padded_row_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, padded_row_fixed_vocab, (int32_t) sizeof( padded_row_fixed_vocab ), padded_row_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
...
static SCHEMA_UNUSED const uint64_t padded_frame_fixed_hash = 0x7b08b877a4f6e867ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 23 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t padded_frame_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t padded_frame_fixed_vocab_bytes = (int64_t) sizeof( padded_frame_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( padded_frame_fixed_vocab ) + count * ( 8 + 3229 );
}
...
static SCHEMA_UNUSED int64_t padded_frame_fixed_save( const PaddedFrame * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( padded_frame_fixed_vocab ) );
    memcpy( buffer + 5, padded_frame_fixed_vocab, sizeof( padded_frame_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( padded_frame_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != padded_frame_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_vocab_read( block, (int64_t) block_bytes, &parsed ) ) { report->refused = 1; report->reason = SCHEMA_TABLE_BLOCK_MALFORMED; return -1; }
        made = table_fixed_compile( &parsed, padded_frame_fixed_vocab, (int32_t) sizeof( padded_frame_fixed_vocab ), padded_frame_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_BLOCK_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
