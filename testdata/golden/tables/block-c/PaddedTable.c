...
{
    return TableJsonRead( value, &schema_blockdemo_padded_row_info_, text, bytes, report );
}

int64_t schema_blockdemo_padded_row_to_json_( const PaddedRow * value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( value, &schema_blockdemo_padded_row_info_, buffer, capacity );
}

int schema_blockdemo_padded_frame_from_json_( PaddedFrame * value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( value, &schema_blockdemo_padded_frame_info_, text, bytes, report );
}

int64_t schema_blockdemo_padded_frame_to_json_( const PaddedFrame * value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( value, &schema_blockdemo_padded_frame_info_, buffer, capacity );
}
