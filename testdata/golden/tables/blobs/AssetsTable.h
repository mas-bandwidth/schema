...
struct TableWriter
{
    ...
    BLOBDEMO_TABLE_INLINE void patch32( int64_t at, uint32_t v )
    {
...
static const uint64_t BuildVersion = 0x73d756fe041d0e93ull;
...
template <typename Ctx> inline int64_t AssetMeasureBody( const Ctx & ctx, const Asset & value );
template <typename Ctx> inline bool AssetSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Asset & value );
template <typename Ctx> inline bool AssetSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Asset & value );
inline bool AssetLoadBody( TableReader & r, const TableNodeMap & nodes, Asset & value );
template <typename Ctx> inline int64_t CatalogMeasureBody( const Ctx & ctx, const Catalog & value );
template <typename Ctx> inline bool CatalogSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Catalog & value );
template <typename Ctx> inline bool CatalogSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Catalog & value );
...
template <typename Ctx> inline int64_t TableNodeMeasure( const Ctx & ctx, const Asset & value ) { return AssetMeasureBody( ctx, value ); }
template <typename Ctx> inline bool TableNodeSave( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Asset & value ) { return AssetSaveBody( ctx, numbering, w, value ); }
template <typename Ctx> inline int64_t TableNodeMeasure( const Ctx & ctx, const Catalog & value ) { return CatalogMeasureBody( ctx, value ); }
template <typename Ctx> inline bool TableNodeSave( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Catalog & value ) { return CatalogSaveBody( ctx, numbering, w, value ); }

template <typename Ctx>
inline int64_t AssetMeasureBody( const Ctx & ctx, const Asset & value )
{
    ...
    if ( value.kind != 0 ) { bytes += 3 + 4; } // kind
    {
        const TableBlob * blob_data = TableBlobAt( ctx, value.data ); // *bytes
        // A BYTE BUFFER RIDES AS A u32 NODE INDEX too (docs/SPEC-TABLES.md §2.5,
        // §3.1): seven bytes here, the bytes themselves as a record in the
        // node table. Null is elided and a non-null blob always rides, even
        // at length zero — null and empty are two values.
        if ( blob_data != NULL ) { bytes += 3 + 4; }
    }
    {
        ...
        // node table. Null is elided and a non-null blob always rides, even
        // at length zero — null and empty are two values.
        if ( blob_caption != NULL ) { bytes += 3 + 4; }
    }
    {
...
template <typename Ctx>
inline bool AssetSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Asset & value )
{
    ...
    }
    {
        ...
        if ( blob_data != NULL )
        {
            ...
            w.put16( 0x3ad7 ); w.put8( 17 ); // data — a NODE INDEX into the flat node table
            w.put32( index_data );
        }
    }
    {
        ...
        {
            uint32_t index_caption = 0;
            if ( !TableNumberingIndex( numbering, (const void *) blob_caption, index_caption ) ) { return false; }
            w.put16( 0xf376 ); w.put8( 17 ); // caption — a NODE INDEX into the flat node table
            w.put32( index_caption );
        }
    }
    {
        const Asset * pointee_next = AssetAt( ctx, value.next ); // *Asset
...
template <typename Ctx>
inline bool AssetSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Asset & value )
{
    if ( !AssetSaveBodyFields( ctx, numbering, w, value ) ) { return false; }
    ...
    for ( ;; )
    {
        ...
        switch ( field_id )
        {
            ...
            case 0x3ad7: // data
            {
                ...
                // A BYTE BUFFER'S PAYLOAD IS A NUMBER too (docs/SPEC-TABLES.md §2.5, §3.1):
                // the blob is a record in the node table, and the slot resolves through
                // the numbering to it under its reserved type id, never followed.
                if ( !r.has( 4 ) ) { r.report->malformed = true; return false; }
                TableNodeResolve( nodes, value.data, r.get32(), kTableBytesTypeId, r.report ); // *bytes
                break;
            }
            case 0xf376: // caption
            {
                ...
                // the numbering to it under its reserved type id, never followed.
                if ( !r.has( 4 ) ) { r.report->malformed = true; return false; }
                TableNodeResolve( nodes, value.caption, r.get32(), kTableStringTypeId, r.report ); // *string
                break;
            }
            case 0xd15e: // next
            {
                ...
                if ( !r.has( 4 ) ) { r.report->malformed = true; return false; }
                TableNodeResolve( nodes, value.next, r.get32(), 0x8e4e6dccfe5f64fbull, r.report ); // *Asset
                break;
            }
            case 0xffff:
            {
...
template <typename Ctx>
inline int64_t CatalogMeasureBody( const Ctx & ctx, const Catalog & value )
{
    int64_t bytes = 2; // terminator
    if ( value.name_length < 0 || value.name_length > 32 ) { return -1; } // storage invariant
    if ( value.name_length > 0 ) { bytes += 3 + 4 + value.name_length; } // name
    ...
    }
    {
        ...
        // node table. Null is elided and a non-null blob always rides, even
        // at length zero — null and empty are two values.
        if ( blob_alias != NULL ) { bytes += 3 + 4; }
    }
    return bytes;
}

template <typename Ctx>
inline bool CatalogSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Catalog & value )
{
    if ( value.name_length < 0 || value.name_length > 32 ) { return false; } // storage invariant
    if ( value.name_length > 0 )
    {
    ...
    }
    {
        ...
        if ( blob_thumb != NULL )
        {
            uint32_t index_thumb = 0;
            if ( !TableNumberingIndex( numbering, (const void *) blob_thumb, index_thumb ) ) { return false; }
            w.put16( 0x694f ); w.put8( 17 ); // thumb — a NODE INDEX into the flat node table
            w.put32( index_thumb );
        }
    }
    {
...
template <typename Ctx>
inline bool CatalogSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, const Catalog & value )
{
    if ( !CatalogSaveBodyFields( ctx, numbering, w, value ) ) { return false; }
    w.put16( 0 ); // terminator
    return !w.overflow;
}
    ...
    for ( ;; )
    {
        ...
        switch ( field_id )
        {
            ...
            case 0x79aa: // head
            {
                ...
                // there is no traversal here and therefore no traversal bound.
                if ( !r.has( 4 ) ) { r.report->malformed = true; return false; }
                TableNodeResolve( nodes, value.head, r.get32(), 0x8e4e6dccfe5f64fbull, r.report ); // *Asset
                break;
            }
            case 0x694f: // thumb
            {
                ...
                // the numbering to it under its reserved type id, never followed.
                if ( !r.has( 4 ) ) { r.report->malformed = true; return false; }
                TableNodeResolve( nodes, value.thumb, r.get32(), kTableBytesTypeId, r.report ); // *bytes
                break;
            }
            case 0x9da7: // note
            {
                ...
                // the numbering to it under its reserved type id, never followed.
                if ( !r.has( 4 ) ) { r.report->malformed = true; return false; }
                TableNodeResolve( nodes, value.note, r.get32(), kTableStringTypeId, r.report ); // *string
                break;
            }
            case 0xfc71: // alias
            {
                ...
                // the numbering to it under its reserved type id, never followed.
                if ( !r.has( 4 ) ) { r.report->malformed = true; return false; }
                TableNodeResolve( nodes, value.alias, r.get32(), kTableBytesTypeId, r.report ); // *bytes
                break;
            }
            case 0xffff:
            {
...
inline int64_t AssetMeasureWire( const Ctx & ctx, const Asset & root, TableAllocator allocator )
{
    ...
    {
        bytes = AssetMeasureBody( ctx, root );
        if ( bytes >= 0 )
        {
...
inline int64_t AssetSaveWire( const Ctx & ctx, const Asset & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator )
{
    ...
    bool ok = AssetSaveBodyFields( ctx, numbering, w, root ) && TableNodeTableSave( ctx, w, numbering );
    TableNumberingShutdown( numbering );
    if ( !ok ) { return -1; }
    w.put16( 0 );
...
inline int64_t AssetLoadMeasure( const uint8_t * wire, int64_t wire_bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
...
// was wrong.
inline const Asset * AssetLoad( uint8_t * region, int64_t region_bytes, const uint8_t * wire, int64_t wire_bytes, TableReport * report )
{
    TableReport ignored;
...
inline bool AssetLoadBuilder( AssetBuilder & builder, const uint8_t * wire, int64_t wire_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
...
inline int64_t CatalogMeasureWire( const Ctx & ctx, const Catalog & root, TableAllocator allocator )
{
    ...
    {
        bytes = CatalogMeasureBody( ctx, root );
        if ( bytes >= 0 )
        {
            int64_t table = TableNodeTableMeasure( ctx, numbering );
            bytes = table < 0 ? -1 : bytes + table;
...
inline int64_t CatalogSaveWire( const Ctx & ctx, const Catalog & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator )
{
    ...
    // terminator: a reader that gives up inside the table has already
    // decoded the ROOT'S OWN FIELDS (§3.1)
    bool ok = CatalogSaveBodyFields( ctx, numbering, w, root ) && TableNodeTableSave( ctx, w, numbering );
    TableNumberingShutdown( numbering );
    if ( !ok ) { return -1; }
    w.put16( 0 );
    if ( w.overflow ) { return -1; } // the caller's buffer was too small
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t CatalogLoadMeasure( const uint8_t * wire, int64_t wire_bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
    TableNodeScan scan = TableNodeScanBegin( wire, wire_bytes, &ignored );
...
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Catalog * CatalogLoad( uint8_t * region, int64_t region_bytes, const uint8_t * wire, int64_t wire_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
...
// the same one; what differs is where a node lives and therefore what a
// resolved slot holds — an arena offset here, a self-relative delta there.
inline bool CatalogLoadBuilder( CatalogBuilder & builder, const uint8_t * wire, int64_t wire_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
...
inline const TableFieldInfo AssetTableFields[] = {
    { "name", "name", "string", 0x30df, 12, false, false, NULL, NULL, true, false, 32, (uint32_t) offsetof( Asset, name ), (uint32_t) sizeof( Asset::name ), (uint32_t) offsetof( Asset, name_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "kind", "kind", "uint32", 0x3b50, 8, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Asset, kind ), (uint32_t) sizeof( Asset::kind ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "data", "data", "bytes", 0x3ad7, 17, false, true, []( const void * slot ) -> const void * { return (const void *) TableBlobAt( *(const TableRef *) slot ); }, NULL, false, false, 0, (uint32_t) offsetof( Asset, data ), (uint32_t) sizeof( TableRef ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "caption", "caption", "string", 0xf376, 17, false, true, []( const void * slot ) -> const void * { return (const void *) TableBlobAt( *(const TableRef *) slot ); }, NULL, false, false, 0, (uint32_t) offsetof( Asset, caption ), (uint32_t) sizeof( TableRef ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "next", "next", "Asset", 0xd15e, 17, false, true, []( const void * slot ) -> const void * { return (const void *) AssetAt( *(const TableRef *) slot ); }, []( TableWorker & worker, void * slot ) -> void * { return (void *) AssetEmplace( worker, *(TableRef *) slot ); }, false, false, 0, (uint32_t) offsetof( Asset, next ), (uint32_t) sizeof( TableRef ), 0xffffffffu, 0xffffffffu, &AssetTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
};
...
inline const TableFieldInfo CatalogTableFields[] = {
    { "name", "name", "string", 0x30df, 12, false, false, NULL, NULL, true, false, 32, (uint32_t) offsetof( Catalog, name ), (uint32_t) sizeof( Catalog::name ), (uint32_t) offsetof( Catalog, name_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "head", "head", "Asset", 0x79aa, 17, false, true, []( const void * slot ) -> const void * { return (const void *) AssetAt( *(const TableRef *) slot ); }, []( TableWorker & worker, void * slot ) -> void * { return (void *) AssetEmplace( worker, *(TableRef *) slot ); }, false, false, 0, (uint32_t) offsetof( Catalog, head ), (uint32_t) sizeof( TableRef ), 0xffffffffu, 0xffffffffu, &AssetTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "thumb", "thumb", "bytes", 0x694f, 17, false, true, []( const void * slot ) -> const void * { return (const void *) TableBlobAt( *(const TableRef *) slot ); }, NULL, false, false, 0, (uint32_t) offsetof( Catalog, thumb ), (uint32_t) sizeof( TableRef ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "note", "note", "string", 0x9da7, 17, false, true, []( const void * slot ) -> const void * { return (const void *) TableBlobAt( *(const TableRef *) slot ); }, NULL, false, false, 0, (uint32_t) offsetof( Catalog, note ), (uint32_t) sizeof( TableRef ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
    { "alias", "alias", "bytes", 0xfc71, 17, false, true, []( const void * slot ) -> const void * { return (const void *) TableBlobAt( *(const TableRef *) slot ); }, NULL, false, false, 0, (uint32_t) offsetof( Catalog, alias ), (uint32_t) sizeof( TableRef ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "" },
};
