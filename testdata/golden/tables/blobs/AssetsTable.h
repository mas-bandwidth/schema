...
inline int64_t AssetNodeStorage( uint64_t type_id, int64_t length )
{
    switch ( type_id )
    {
...
inline int64_t AssetLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
...
// BUFFER's record commands its header and its bytes (docs/SPEC-TABLES.md §2.5),
// which is the one answer the record's LENGTH decides.
inline int64_t CatalogNodeStorage( uint64_t type_id, int64_t length )
{
    switch ( type_id )
    {
        case 0x8e4e6dccfe5f64fbull: return TableAlignUp64( (int64_t) sizeof( Asset ) ); // Asset
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t CatalogLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
    TableIdTable ids_table;
    int64_t body_bytes = 0;
...
// parameter on a load.
inline const Asset * AssetOpen( const void * bytes, uint64_t length )
{
    return (const Asset *) TableCookOpen( bytes, length, (uint64_t) sizeof( Asset ), (uint64_t) alignof( Asset ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Catalog * CatalogOpen( const void * bytes, uint64_t length )
{
    return (const Catalog *) TableCookOpen( bytes, length, (uint64_t) sizeof( Catalog ), (uint64_t) alignof( Catalog ) );
}
