...
// package blobdemo — the TABLE wire's text form (docs/SPEC-TABLES.md §16).
...
// The guard is not vestigial. Several blobdemo Table.cpp files may be
...
#ifndef BLOBDEMO_SCHEMA_TABLE_JSON
#define BLOBDEMO_SCHEMA_TABLE_JSON
...
#endif // BLOBDEMO_SCHEMA_TABLE_JSON
...
bool AssetFromJson( AssetBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, AssetTableType(), text, bytes, report );
}

int64_t AssetToJsonMeasure( const Asset * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, AssetTableType(), NULL, 0, allocator );
}

int64_t AssetToJson( const Asset * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, AssetTableType(), buffer, capacity, allocator );
}
...
bool CatalogFromJson( CatalogBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, CatalogTableType(), text, bytes, report );
}

int64_t CatalogToJsonMeasure( const Catalog * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, CatalogTableType(), NULL, 0, allocator );
}

int64_t CatalogToJson( const Catalog * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, CatalogTableType(), buffer, capacity, allocator );
}
