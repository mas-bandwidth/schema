...
inline int64_t ArchiveConfigMeasureBody( TableIds & ids, const ArchiveConfig & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        const uint64_t ref_root = ids.ref( 0xa354fd1ff0c467c5ull, 2 );
    ...
    if ( value.count != 1 ) { bytes += TableLebBytes( ids.ref( 0xb1e5e28e4479a274ull, 3 ) ) + 1 + 4; } // count
    return bytes;
}
...
TABLEDEMO_TABLE_INLINE bool ArchiveConfigSaveBody( TableWriter & w, TableIds & ids, const ArchiveConfig & value )
{
    ...
    if ( value.count != 1 )
    {
        w.putleb( ids.ref( 0xb1e5e28e4479a274ull, 3 ) ); w.put8( 4 ); // count
...
inline int64_t ArchiveConfigMeasureMessage( const ArchiveConfig & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = ArchiveConfigMeasureBody( ids, value );
...
inline int64_t ArchiveConfigSaveMessage( const ArchiveConfig & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !ArchiveConfigSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == ArchiveConfigMeasureMessage( value )
}
...
inline bool ArchiveConfigLoadMessage( ArchiveConfig & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    ArchiveConfigReset( value );
