...
template <typename Ctx> inline int64_t PatrolMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Patrol & value );
template <typename Ctx> inline bool PatrolSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Patrol & value );
inline bool PatrolLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Patrol & value );
...
inline void TableReset( Patrol & value ) { PatrolReset( value ); }
...
template <typename Ctx> inline int64_t PatrolMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Patrol & value );
template <typename Ctx> inline bool PatrolSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Patrol & value );
template <typename Ctx> inline bool PatrolSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Patrol & value );
inline bool PatrolLoadBody( TableReader & r, const TableNodeMap & nodes, Patrol & value );

// ---- pointer-graph walkers: number (measure/save), pack (Lock) ----

template <typename Ctx> inline bool PatrolNumber( const Ctx & ctx, TableNumbering & numbering, const Patrol & value );
template <typename Ctx> inline int64_t PatrolPackMeasure( const Ctx & ctx, TablePackMap & seen, const Patrol & value );
template <typename Ctx> inline bool PatrolPack( const Ctx & ctx, TablePackMap & seen, const Patrol & src, Patrol & dst, uint8_t * base, int64_t capacity, int64_t & used );

// ---- the numbering's bridge to each member's codec (docs/SPEC-TABLES.md §3.1) ----

template <typename Ctx> inline int64_t TableNodeMeasure( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Patrol & value ) { return PatrolMeasureBody( ctx, numbering, ids, value ); }
template <typename Ctx> inline bool TableNodeSave( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Patrol & value ) { return PatrolSaveBody( ctx, numbering, w, ids, value ); }
template <typename Ctx> inline int64_t TableNodeMessageMeasure( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Patrol & value ) { return PatrolMeasureMessageBody( ctx, numbering, index_bits, at, value ); }
template <typename Ctx> inline bool TableNodeMessageSave( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Patrol & value ) { return PatrolSaveMessageBody( ctx, numbering, index_bits, w, value ); }
...
template <typename Ctx> inline int64_t PatrolMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Patrol & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool PatrolSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Patrol & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool PatrolSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Patrol & value, TableRetain * retain, const TableRetainPath & path );
inline bool PatrolLoadBodyRetain( TableReader & r, const TableNodeMap & nodes, Patrol & value, TableRetain * retain, const TableRetainPath & path );
inline bool PatrolLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Patrol & value, TableRetain * retain, const TableRetainPath & path );

template <typename Ctx>
inline int64_t PatrolMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Patrol & value )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.active != false ) { bytes += TableLebBytes( ids.ref_at( 58, 0x6580790b036f0c6full ) ) + 1 + 1; } // active
    if ( value.active )
    {
    ...
    {
        if ( value.target_id != 0 ) { bytes += TableLebBytes( ids.ref_at( 104, 0xb7bc9ac015a25050ull ) ) + 1 + 4; } // target_id
    }
    if ( value.active && !value.has_target )
    {
        if ( value.wander != 0.5f ) { bytes += TableLebBytes( ids.ref_at( 105, 0xb8c758d8bd1845d4ull ) ) + 1 + 4; } // wander
    }
    if ( !value.active )
    {
        if ( value.note_length < 0 || value.note_length > 8 ) { return -1; } // storage invariant
        if ( value.note_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 38, 0x3bf8fbbad1587cddull ) ) + 1 + TableLebBytes( (uint64_t) ( value.note_length ) ) + ( value.note_length ); } // note
    }
    return bytes;
}

template <typename Ctx>
inline bool PatrolSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Patrol & value )
{
    (void) ctx; (void) numbering;
    if ( value.active != false )
    {
        w.header( ids.ref_at( 58, 0x6580790b036f0c6full ), 1 ); // active
    ...
    if ( value.active && value.has_target )
    {
        ...
        {
            w.header( ids.ref_at( 104, 0xb7bc9ac015a25050ull ), 4 ); // target_id
    ...
    if ( value.active && !value.has_target )
    {
        ...
        {
            w.header( ids.ref_at( 105, 0xb8c758d8bd1845d4ull ), 10 ); // wander
    ...
    if ( !value.active )
    {
        ...
        {
            w.header( ids.ref_at( 38, 0x3bf8fbbad1587cddull ), 12 ); // note
...
template <typename Ctx>
inline bool PatrolSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Patrol & value )
{
    if ( !PatrolSaveBodyFields( ctx, numbering, w, ids, value ) ) { return false; }
...
template <typename Ctx>
inline int64_t PatrolMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Patrol & value )
{
...
template <typename Ctx>
inline bool PatrolSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Patrol & value )
{
    (void) ctx; (void) numbering; (void) index_bits;
    if ( value.active != false )
    {
...
// PatrolNumber: number everything Patrol POINTS AT, in first-visit order —
...
template <typename Ctx>
inline bool PatrolNumber( const Ctx & ctx, TableNumbering & numbering, const Patrol & value )
{
    (void) ctx; (void) numbering; (void) value; // no pointers below this node
    return true;
}

// PatrolPackMeasure: the packed region bytes of everything Patrol POINTS AT.
...
template <typename Ctx>
inline int64_t PatrolPackMeasure( const Ctx & ctx, TablePackMap & seen, const Patrol & value )
{
    int64_t bytes = 0;
...
// PatrolPack: copy src into dst (already placed), then lay every pointee out
...
template <typename Ctx>
inline bool PatrolPack( const Ctx & ctx, TablePackMap & seen, const Patrol & src, Patrol & dst, uint8_t * base, int64_t capacity, int64_t & used )
{
    memcpy( (void *) &dst, (const void *) &src, sizeof( Patrol ) ); // trivially copyable, by construction
...
struct PatrolBuilder
{
    ...
    PatrolBuilder( TableAllocator allocator = TableDefaultAllocator() )
    {
        TableArenaInit( arena, allocator );
        main.arena = &arena;
        TableSlot<Patrol> slot = main.Alloc<Patrol>();
        root_ref = slot.ref;
    }
    ~PatrolBuilder() { TableArenaShutdown( arena ); table_release( arena.allocator, region ); }
    PatrolBuilder( const PatrolBuilder & ) = delete;
    PatrolBuilder & operator=( const PatrolBuilder & ) = delete;
    ...
    Patrol * GetRoot() { return arena.locked ? NULL : (Patrol *) TableArenaAt( arena, (uint32_t) root_ref.value ); }
    bool Locked() const { return arena.locked; }
    const Patrol * AsConst() const { return (const Patrol *) region; }
...
inline bool PatrolBuilder::Lock()
{
    if ( arena.locked ) { return region != NULL; }
    if ( root_ref.null() ) { return false; }
    TableArenaCtx ctx = { &arena };
    const Patrol & root = *(const Patrol *) TableArenaAt( arena, (uint32_t) root_ref.value );
    ...
    int64_t used = TableAlignUp64( (int64_t) sizeof( Patrol ) );
    Patrol * destination = new ( packed ) Patrol; // lifetime only: the Pack below memcpy's the whole node over it
    ...
    if ( TablePackMapReach( seen, (const void *) &root, 0, root_taken, root_slot ) == NULL ||
         !PatrolPack( ctx, seen, root, *destination, packed, total, used ) || used != total )
    {
...
// ---- Patrol on the wire: the FLAT NODE TABLE (docs/SPEC-TABLES.md §3.1) ----
//
...
// PatrolNodeStorage: the region bytes one record commands, or -1 for a type id
...
// PatrolNodePlace: start one record's node's lifetime in the storage pass one
...
// PatrolNodeBody: PASS TWO's half — decode one record's body into the storage it
// already owns.
...
inline int64_t PatrolNodeMessageStorage( uint64_t type_id, int64_t extent, int64_t length )
{
    (void) length;
    (void) extent;
    (void) type_id;
...
// PatrolNodeMessageExtent: step over one TABLE record's body, tallying the extent
...
inline bool PatrolNodeMessageExtent( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & extent )
{
    extent = 0;
...
// PatrolNodeMessageBody: PASS TWO's half, which decodes one record's body into
...
template <typename Ctx>
inline bool PatrolNumberFrom( const Ctx & ctx, TableNumbering & numbering, const Patrol & root )
{
    bool taken = false;
    int64_t slot = 0;
    if ( TablePackMapReach( numbering.seen, (const void *) &root, (int64_t) kTableNodeIndexRoot, taken, slot ) == NULL ) { return false; }
    return PatrolNumber( ctx, numbering, root );
}

template <typename Ctx>
inline int64_t PatrolMeasureWire( const Ctx & ctx, const Patrol & root, TableAllocator allocator )
{
    ...
    {
        TableIds ids;
        bytes = PatrolMeasureBody( ctx, numbering, ids, root );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t PatrolSaveWire( const Ctx & ctx, const Patrol & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    ...
    bool ok = PatrolSaveBodyFields( ctx, numbering, w, ids, root ) && TableNodeTableSave( ctx, w, ids, numbering );
    TableNumberingShutdown( numbering );
    ...
    return w.offset; // == PatrolMeasure( root )
}

inline int64_t PatrolMeasure( const Patrol * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return PatrolMeasureWire( ctx, *root, allocator );
}

inline int64_t PatrolSave( const Patrol * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return PatrolSaveWire( ctx, *root, buffer, capacity, allocator );
}

inline int64_t PatrolMeasure( const PatrolBuilder & builder )
{
    if ( builder.region != NULL ) { return PatrolMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return PatrolMeasureWire( ctx, *(const Patrol *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline int64_t PatrolSave( const PatrolBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return PatrolSave( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return PatrolSaveWire( ctx, *(const Patrol *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator );
}

// ---- Patrol on the MESSAGE wire: the batch over a region (docs/SPEC-TABLES.md §3.3) ----

// PatrolMessageBodyBits: one root body's bits at bit position `at` of the batch,
...
template <typename Ctx>
inline int64_t PatrolMessageBodyBits( const Ctx & ctx, const Patrol & root, int64_t at, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bits = -1;
    if ( PatrolNumberFrom( ctx, numbering, root ) )
    {
        ...
        if ( table >= 0 )
        {
            const int64_t body = PatrolMeasureMessageBody( ctx, numbering, index_bits, at + table, root );
...
template <typename Ctx>
inline bool PatrolMessageBodySave( const Ctx & ctx, const Patrol & root, TableBitWriter & w, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    bool ok = false;
    ...
    {
        const int64_t index_bits = TableBitsRequired( 0, numbering.count + 1 );
        ok = TableMessageNodeTableSave( ctx, numbering, index_bits, w ) && PatrolSaveMessageBody( ctx, numbering, index_bits, w, root );
    }
    TableNumberingShutdown( numbering );
...
inline int64_t PatrolMeasureMessages( const Patrol * const * roots, int64_t count, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        ...
        const int64_t body = PatrolMessageBodyBits( ctx, *roots[i], bits, allocator );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8;
...
inline int64_t PatrolSaveMessages( const Patrol * const * roots, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( roots == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( roots[i] == NULL || !PatrolMessageBodySave( ctx, *roots[i], batch.w, allocator ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == PatrolMeasureMessages( roots, count, report, allocator )
}

// PatrolMessageRecordScan: one node record's type id and the extent its maps
...
inline bool PatrolMessageRecordScan( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, uint64_t & type_id, int64_t & extent, int64_t & length )
{
    uint64_t type_ref = 0;
    ...
    return PatrolNodeMessageExtent( type_id, r, vocabulary, index_bits, extent );
}

// PatrolMessageBodyStorage: one body's node count and data bytes from the FRAMING
...
// and no further (§3.3).
inline bool PatrolMessageBodyStorage( TableBitReader & r, const TableVocabulary & vocabulary, int64_t & records, int64_t & data, bool & complete )
{
    complete = true;
    records = 0;
    data = 0;
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !PatrolMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { return false; }
        const int64_t storage = PatrolNodeMessageStorage( type_id, extent, length );
    ...
    data += TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Patrol ) ) + root_extent );
    return true;
}

// PatrolLoadMeasure's MESSAGE overload: the exact region bytes ONE BATCH needs,
...
inline int64_t PatrolLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
    ...
    for ( int64_t b = 0; b < bodies; b++ )
    {
        ...
        bool complete = true;
        if ( !PatrolMessageBodyStorage( br.r, vocabulary, records, body_data, complete ) ) { return -1; }
        data += body_data;
...
// PatrolLoadMessageBodyInto: one body of a batch into the region at `used`. Its
...
inline bool PatrolLoadMessageBodyInto( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Patrol * & root_out )
{
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        uint64_t type_id = 0;
        int64_t extent = 0, length = 0;
        if ( !PatrolMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { out->malformed = true; return false; }
        const int64_t storage = PatrolNodeMessageStorage( type_id, extent, length );
        ...
        PatrolNodePlace( type_id, region + used, length );
        used += storage;
    }
    const int64_t fields_start = r.offset;
    int64_t root_extent = 0;
    const int64_t root_bytes = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Patrol ) ) + root_extent );
    if ( used + root_bytes > region_bytes ) { out->malformed = true; return false; }
    directory[0].offset = (uint64_t) used;
    directory[0].type_id = 0x1c182e1c2529027bull;
    Patrol * root = new ( region + used ) Patrol; // lifetime only: LoadMessageBody's first act is PatrolReset
    PatrolReset( *root );
    root_out = root;
    used += root_bytes;
    nodes.good = true;
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !PatrolNodeMessageBody( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset ) ) { return false; }
    }
    ...
    return PatrolLoadMessageBody( r, vocabulary, out, nodes, index_bits, *root );
}

// PatrolLoadMessages: decode a BATCH into the caller's exact-sized region and
...
inline bool PatrolLoadMessages( const Patrol ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !PatrolLoadMessageBodyInto( br.r, vocabulary, out, region, region_bytes, used, roots[b] ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
...
// PatrolLoadMeasure: the exact region bytes a wire buffer will need, and it is
...
inline int64_t PatrolLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    ...
    int64_t data = TableAlignUp64( (int64_t) sizeof( Patrol ) );
    int64_t records = 0;
    uint64_t type_id = 0;
    ...
    {
        records++;
        int64_t storage = PatrolNodeStorage( type_id, length, reason );
...
// PatrolLoad: decode the tolerant wire into the caller's exact-sized region and
...
// was wrong.
inline const Patrol * PatrolLoad( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    ...
    const uint8_t * const wire = wire_file + 1;
    const int64_t wire_bytes = body_bytes;
    if ( region == NULL || region_bytes < (int64_t) sizeof( Patrol ) ) { out->malformed = true; return NULL; }
    ...
    directory[0].type_id = 0x1c182e1c2529027bull;
    Patrol * root = new ( region ) Patrol; // lifetime only: LoadBody's first act is PatrolReset
    PatrolReset( *root );
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            if ( directory[k + 1].offset != kTableNodeAbsent )
            {
                ...
                PatrolNodeBody( type_id, sub, nodes, region + directory[k + 1].offset );
            }
            k++;
        }
    }
    ...
    PatrolLoadBody( r, nodes, *root );
    return root;
}

// PatrolLoadBuilder: the TOOL's path — the same tolerant decode into a fresh
...
inline bool PatrolLoadBuilder( PatrolBuilder & builder, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    TableIdTable ids_table;
    ...
    nodes.arena = true; // a resolved slot holds the node's ARENA OFFSET here
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            uint32_t at = PatrolNodeAlloc( type_id, builder.main, length );
            if ( at == 0 )
            {
                unknown_records++;
                directory[k + 1].offset = kTableNodeAbsent;
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                PatrolNodeBody( type_id, sub, nodes, TableArenaAt( builder.arena, (uint32_t) directory[k + 1].offset ) );
            }
            k++;
        }
    }
    TableReader r( wire, wire_bytes, out, &ids_table );
    r.nested = false; // the ROOT body, the one that carries the node table
    bool ok = PatrolLoadBody( r, nodes, *root );
    table_release( allocator, directory );
    return ok;
}

template <typename Ctx>
inline int64_t PatrolMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Patrol & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.active != false ) { bytes += TableLebBytes( ids.ref_at( 58, 0x6580790b036f0c6full ) ) + 1 + 1; } // active
...
template <typename Ctx>
inline bool PatrolSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Patrol & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    if ( value.active != false )
    {
...
template <typename Ctx>
inline bool PatrolSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Patrol & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !PatrolSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
...
// PatrolNodeBodyRetain: PASS TWO's half — decode one record's body into the storage it
// already owns.
...
inline const Patrol * PatrolLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                PatrolNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
    ...
    TableReader r( wire, wire_bytes, out, &ids_table );
    r.nested = false; // the ROOT body, the one that carries the node table
    PatrolLoadBodyRetain( r, nodes, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
    return root;
}

// PatrolNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool PatrolLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Patrol * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !PatrolNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    return PatrolLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// PatrolLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool PatrolLoadRetainMessages( const Patrol ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !PatrolLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// PatrolMeasureRetain and PatrolSaveRetain: the pair, with the retained tail in
...
template <typename Ctx>
inline int64_t PatrolMeasureWireRetain( const Ctx & ctx, const Patrol & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( PatrolNumberFrom( ctx, numbering, root ) )
    {
        ...
        bytes = PatrolMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t PatrolSaveWireRetain( const Ctx & ctx, const Patrol & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    ...
    bool ok = PatrolSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
...
inline int64_t PatrolMeasureRetain( const Patrol * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return PatrolMeasureWireRetain( ctx, *root, retain, allocator );
}
...
inline int64_t PatrolSaveRetain( const Patrol * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return PatrolSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// PatrolSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
...
//
// A REFERENCE INSIDE THE REGION IS DEREFERENCED THROUGH PatrolAt: the slot holds
...
// parameter on a load.
inline const Patrol * PatrolOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Patrol *) TableCookOpen( bytes, length, (uint64_t) sizeof( Patrol ), (uint64_t) alignof( Patrol ), reason );
}
...
template <typename Ctx> inline bool PatrolCookBody( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Patrol & value, TableByteOrder order );
...
// PatrolCookLayout: the tool's own Layout (docs/SPEC-TABLES.md §7.2) over one
...
inline bool PatrolCookLayout( const TableNumbering & numbering, TableCookRegion & region )
{
    region.numbering = &numbering;
    region.count = numbering.count + 1;
    int64_t offset = 36; // the root, at zero
    int64_t align = 8;
...
// PatrolCookMeasureFrom: the whole cooked file's bytes for one graph — the header,
...
template <typename Ctx>
inline int64_t PatrolCookMeasureFrom( const Ctx & ctx, const Patrol & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    TableCookRegion region;
    int64_t bytes = -1;
    if ( PatrolNumberFrom( ctx, numbering, root ) && PatrolCookLayout( numbering, region ) )
    {
...
// PatrolCookFrom: write one cooked file of a pointered graph, in the byte order
...
template <typename Ctx>
inline bool PatrolCookFrom( const Ctx & ctx, const Patrol & root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator )
{
    if ( out == NULL ) { return false; }
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    TableCookRegion region;
    bool ok = PatrolNumberFrom( ctx, numbering, root );
    if ( ok )
    {
        region.offsets = (int64_t *) table_allocate( allocator, ( numbering.count + 1 ) * (int64_t) sizeof( int64_t ) );
        ok = region.offsets != NULL && PatrolCookLayout( numbering, region );
    }
    if ( ok )
    {
        const int64_t data_offset = ( kTableCookHeaderBytes + region.align - 1 ) & ~( region.align - 1 );
        ...
        if ( ok )
        {
            ...
            ok = PatrolCookBody( ctx, region, region.base, root, order );
            ...
            table_cook_put( entry + 8, 0x1c182e1c2529027bull, 8, order ); // the root: fnv1a64( "Patrol" )
            for ( int64_t k = 0; k < numbering.count; k++ )
            {
...
// PatrolCookMeasure / PatrolCook over a REGION root — a locked builder's AsConst, a
// region PatrolLoad produced, or an opened cook — with the pair the numbering
// allocates through as an optional last argument, as the wire's own entries
// take it (§13.9).
inline int64_t PatrolCookMeasure( const Patrol * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return PatrolCookMeasureFrom( ctx, *root, allocator );
}

inline bool PatrolCook( const Patrol * root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return false; }
    TableRegionCtx ctx;
    return PatrolCookFrom( ctx, *root, out, capacity, order, allocator );
}

// and over a BUILDER, locked or not: the builder's own pair, and the arena
// encoding while it is still mutable (§6.3).
inline int64_t PatrolCookMeasure( const PatrolBuilder & builder )
{
    if ( builder.region != NULL ) { return PatrolCookMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return PatrolCookMeasureFrom( ctx, *(const Patrol *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline bool PatrolCook( const PatrolBuilder & builder, void * out, uint64_t capacity, TableByteOrder order )
{
    if ( builder.region != NULL ) { return PatrolCook( builder.AsConst(), out, capacity, order, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return false; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return PatrolCookFrom( ctx, *(const Patrol *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), out, capacity, order, builder.arena.allocator );
}
...
extern const TableTypeInfo PatrolTableInfo;

inline const TableFieldInfo PatrolTableFields[] = {
    { "active", "active", "bool", 0x6580790b036f0c6full, 1, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Patrol, active ), (uint32_t) sizeof( Patrol::active ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
    { "speed", "speed", "float32", 0x2281498aa0200e40ull, 10, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Patrol, speed ), (uint32_t) sizeof( Patrol::speed ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "active", TableDocNone, 0, NULL },
    { "has_target", "has_target", "bool", 0x247f0e7f55aacfbdull, 1, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Patrol, has_target ), (uint32_t) sizeof( Patrol::has_target ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "active", TableDocNone, 0, NULL },
    { "target_id", "target_id", "int32", 0xb7bc9ac015a25050ull, 4, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Patrol, target_id ), (uint32_t) sizeof( Patrol::target_id ), 0xffffffffu, 0xffffffffu, NULL, true, 0.0, 1000.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "active && has_target", TableDocNone, 0, NULL },
    { "wander", "wander", "float32", 0xb8c758d8bd1845d4ull, 10, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Patrol, wander ), (uint32_t) sizeof( Patrol::wander ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "active && !has_target", TableDocNone, 0, NULL },
    { "note", "note", "string", 0x3bf8fbbad1587cddull, 12, false, false, NULL, NULL, true, false, 8, (uint32_t) offsetof( Patrol, note ), (uint32_t) sizeof( Patrol::note ), (uint32_t) offsetof( Patrol, note_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "!active", TableDocNone, 0, NULL },
};
inline const TableTypeInfo PatrolTableInfo = { "Patrol", (uint32_t) sizeof( Patrol ), 6, PatrolTableFields, +[]( void * p ) { PatrolReset( *(Patrol *) p ); }, true, TableDocNone, 0, NULL };
inline const TableTypeInfo * PatrolTableType() { return &PatrolTableInfo; }

// ---- the text form (docs/SPEC-TABLES.md §16) ----

// Patrol in and out of a JSON text (docs/SPEC-TABLES.md §16.7): read into a
// builder, written from a region's const root. A node named more than once
// carries `&node` in the text. Defined in GuardedTable.cpp; link it to use them.
bool PatrolFromJson( PatrolBuilder & builder, const char * text, int64_t bytes, TableReport * report );
int64_t PatrolToJsonMeasure( const Patrol * root, TableAllocator allocator = TableDefaultAllocator() );
int64_t PatrolToJson( const Patrol * root, char * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() );
