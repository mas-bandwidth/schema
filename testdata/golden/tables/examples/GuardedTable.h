...
inline int64_t PatrolMeasureBody( TableIds & ids, const Patrol & value )
{
    ...
    if ( value.active != false ) { bytes += TableLebBytes( ids.ref( 0x6580790b036f0c6full, 33 ) ) + 1 + 1; } // active
    if ( value.active )
    {
        if ( value.speed != 1.0f ) { bytes += TableLebBytes( ids.ref( 0x2281498aa0200e40ull, 34 ) ) + 1 + 4; } // speed
    }
    if ( value.active )
    {
        if ( value.has_target != false ) { bytes += TableLebBytes( ids.ref( 0x247f0e7f55aacfbdull, 35 ) ) + 1 + 1; } // has_target
    }
    if ( value.active && value.has_target )
    {
        if ( value.target_id != 0 ) { bytes += TableLebBytes( ids.ref( 0xb7bc9ac015a25050ull, 36 ) ) + 1 + 4; } // target_id
    }
    if ( value.active && !value.has_target )
    {
        if ( value.wander != 0.5f ) { bytes += TableLebBytes( ids.ref( 0xb8c758d8bd1845d4ull, 37 ) ) + 1 + 4; } // wander
    }
    if ( !value.active )
    {
        if ( value.note_length < 0 || value.note_length > 8 ) { return -1; } // storage invariant
        if ( value.note_length > 0 ) { bytes += TableLebBytes( ids.ref( 0x3bf8fbbad1587cddull, 38 ) ) + 1 + TableLebBytes( (uint64_t) ( value.note_length ) ) + ( value.note_length ); } // note
    }
    return bytes;
}
...
TABLEDEMO_TABLE_INLINE bool PatrolSaveBody( TableWriter & w, TableIds & ids, const Patrol & value )
{
    ...
    {
        w.putleb( ids.ref( 0x6580790b036f0c6full, 33 ) ); w.put8( 1 ); // active
    ...
    if ( value.active )
    {
        if ( value.speed != 1.0f )
        {
            w.putleb( ids.ref( 0x2281498aa0200e40ull, 34 ) ); w.put8( 10 ); // speed
    ...
    if ( value.active )
    {
        ...
        {
            w.putleb( ids.ref( 0x247f0e7f55aacfbdull, 35 ) ); w.put8( 1 ); // has_target
    ...
    if ( value.active && value.has_target )
    {
        ...
        {
            w.putleb( ids.ref( 0xb7bc9ac015a25050ull, 36 ) ); w.put8( 4 ); // target_id
    ...
    if ( value.active && !value.has_target )
    {
        ...
        {
            w.putleb( ids.ref( 0xb8c758d8bd1845d4ull, 37 ) ); w.put8( 10 ); // wander
    ...
    if ( !value.active )
    {
        ...
        {
            w.putleb( ids.ref( 0x3bf8fbbad1587cddull, 38 ) ); w.put8( 12 ); // note
...
inline int64_t PatrolMeasureMessage( const Patrol & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = PatrolMeasureBody( ids, value );
...
inline int64_t PatrolSaveMessage( const Patrol & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !PatrolSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == PatrolMeasureMessage( value )
}
...
inline bool PatrolLoadMessage( Patrol & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    PatrolReset( value );
