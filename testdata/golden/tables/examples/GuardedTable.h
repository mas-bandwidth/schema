...
TABLEDEMO_TABLE_INLINE bool PatrolSaveBody( TableWriter & w, TableIds & ids, const Patrol & value )
{
    ...
    {
        w.header( ids.ref( 0x6580790b036f0c6full ), 1 ); // active
    ...
    if ( value.active )
    {
        if ( value.speed != 1.0f )
        {
            w.header( ids.ref( 0x2281498aa0200e40ull ), 10 ); // speed
    ...
    if ( value.active )
    {
        ...
        {
            w.header( ids.ref( 0x247f0e7f55aacfbdull ), 1 ); // has_target
    ...
    if ( value.active && value.has_target )
    {
        ...
        {
            w.header( ids.ref( 0xb7bc9ac015a25050ull ), 4 ); // target_id
    ...
    if ( value.active && !value.has_target )
    {
        ...
        {
            w.header( ids.ref( 0xb8c758d8bd1845d4ull ), 10 ); // wander
    ...
    if ( !value.active )
    {
        ...
        {
            w.header( ids.ref( 0x3bf8fbbad1587cddull ), 12 ); // note
...
template <typename... Args>
inline void PatrolSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Patrol is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PatrolLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Patrol is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PatrolSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Patrol is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
