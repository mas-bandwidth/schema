...
// GunnerSettings's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
...
inline void GunnerSettingsFixedScatter( const uint8_t * TABLE_RESTRICT image, GunnerSettings & value, TableReport * report )
{
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 5 );
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == GunnerSettingsFixedHash );
    ...
    if ( identity )
    {
        ...
        uint8_t image[GunnerSettingsFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) GunnerSettingsFixedBodyBytes );
            GunnerSettingsFixedScatter( image, values[k], report );
            at += GunnerSettingsFixedRecordBytes;
        }
        return n;
    }
...
// ShipEntry's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
// compiler into constant-offset moves. Every number in it is a number from
// that plan, and the static_asserts at the top of this form tie each one to
// this compiler's own offsetof and sizeof.
inline void ShipEntryFixedScatter( const uint8_t * TABLE_RESTRICT image, ShipEntry & value, TableReport * report )
{
    uint8_t * TABLE_RESTRICT dst = (uint8_t *) &value;
    int32_t clamped = 0;
    (void) dst; (void) clamped;
    ...
    { // hardpoints count
        int32_t v = (int32_t) TableFixedGet32( image + 44 );
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 70 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 24 ) { v = 24; clamped++; }
        memcpy( dst + 100, &v, 4 );
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == ShipEntryFixedHash );
    // THE PREFILL'S RANGES. EMPTY ON THE IDENTITY PLAN, and not by a flag:
    // the rule is the type's value bytes minus what the plan lands, and the
    // identity plan lands all of them.
    const TableFixedFill * fill = NULL;
    ...
    if ( identity )
    {
        ...
        // of the plan above it — and no per-entry dispatch, no runtime length
        // and no plan array competing with the record for cache.
        uint8_t image[ShipEntryFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) ShipEntryFixedBodyBytes );
            ShipEntryFixedScatter( image, values[k], report );
            ShipEntryFixedClamp( values[k], report ); // the bounds the scatter does not hold (§3.4)
            at += ShipEntryFixedRecordBytes;
        }
        return n;
    }
...
// GlobalSettings's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
// compiler into constant-offset moves. Every number in it is a number from
// that plan, and the static_asserts at the top of this form tie each one to
// this compiler's own offsetof and sizeof.
inline void GlobalSettingsFixedScatter( const uint8_t * TABLE_RESTRICT image, GlobalSettings & value, TableReport * report )
{
    uint8_t * TABLE_RESTRICT dst = (uint8_t *) &value;
    int32_t clamped = 0;
    (void) dst; (void) clamped;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == GlobalSettingsFixedHash );
    // THE PREFILL'S RANGES. EMPTY ON THE IDENTITY PLAN, and not by a flag:
    // the rule is the type's value bytes minus what the plan lands, and the
    // identity plan lands all of them.
    const TableFixedFill * fill = NULL;
    ...
    if ( identity )
    {
        ...
        // of the plan above it — and no per-entry dispatch, no runtime length
        // and no plan array competing with the record for cache.
        uint8_t image[GlobalSettingsFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) GlobalSettingsFixedBodyBytes );
            GlobalSettingsFixedScatter( image, values[k], report );
            GlobalSettingsFixedClamp( values[k], report ); // the bounds the scatter does not hold (§3.4)
            at += GlobalSettingsFixedRecordBytes;
        }
        return n;
    }
...
// PackConfig's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
// compiler into constant-offset moves. Every number in it is a number from
// that plan, and the static_asserts at the top of this form tie each one to
// this compiler's own offsetof and sizeof.
inline void PackConfigFixedScatter( const uint8_t * TABLE_RESTRICT image, PackConfig & value, TableReport * report )
{
    uint8_t * TABLE_RESTRICT dst = (uint8_t *) &value;
    int32_t clamped = 0;
    (void) dst; (void) clamped;
    ...
    { // display_name
        int32_t v = (int32_t) TableFixedGet32( image + 73 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 32 ) { v = 32; clamped++; }
        memcpy( dst + 112, &v, 4 );
    ...
    { // hardpoints count
        int32_t v = (int32_t) TableFixedGet32( image + 117 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 4 ) { v = 4; clamped++; }
        memcpy( dst + 140, &v, 4 );
    }
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 143 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 24 ) { v = 24; clamped++; }
        memcpy( dst + 176, &v, 4 );
    ...
    { // display_name
        int32_t v = (int32_t) TableFixedGet32( image + 171 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 32 ) { v = 32; clamped++; }
        memcpy( dst + 220, &v, 4 );
    ...
    { // hardpoints count
        int32_t v = (int32_t) TableFixedGet32( image + 215 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 4 ) { v = 4; clamped++; }
        memcpy( dst + 248, &v, 4 );
    }
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 241 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 24 ) { v = 24; clamped++; }
        memcpy( dst + 284, &v, 4 );
    ...
    { // display_name
        int32_t v = (int32_t) TableFixedGet32( image + 269 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 32 ) { v = 32; clamped++; }
        memcpy( dst + 328, &v, 4 );
    ...
    { // hardpoints count
        int32_t v = (int32_t) TableFixedGet32( image + 313 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 4 ) { v = 4; clamped++; }
        memcpy( dst + 356, &v, 4 );
    }
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 339 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 24 ) { v = 24; clamped++; }
        memcpy( dst + 392, &v, 4 );
    ...
    { // reserves count
        int32_t v = (int32_t) TableFixedGet32( image + 379 );
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 453 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 24 ) { v = 24; clamped++; }
        memcpy( dst + 512, &v, 4 );
    ...
    { // display_name
        int32_t v = (int32_t) TableFixedGet32( image + 481 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 32 ) { v = 32; clamped++; }
        memcpy( dst + 556, &v, 4 );
    ...
    { // hardpoints count
        int32_t v = (int32_t) TableFixedGet32( image + 525 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 4 ) { v = 4; clamped++; }
        memcpy( dst + 584, &v, 4 );
    }
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 551 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 24 ) { v = 24; clamped++; }
        memcpy( dst + 620, &v, 4 );
    ...
    { // display_name
        int32_t v = (int32_t) TableFixedGet32( image + 579 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 32 ) { v = 32; clamped++; }
        memcpy( dst + 664, &v, 4 );
    ...
    { // hardpoints count
        int32_t v = (int32_t) TableFixedGet32( image + 623 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 4 ) { v = 4; clamped++; }
        memcpy( dst + 692, &v, 4 );
    }
    ...
    { // callsign
        int32_t v = (int32_t) TableFixedGet32( image + 649 );
        if ( v < 0 ) { v = 0; clamped++; } else if ( v > 24 ) { v = 24; clamped++; }
        memcpy( dst + 728, &v, 4 );
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == PackConfigFixedHash );
    // THE PREFILL'S RANGES. EMPTY ON THE IDENTITY PLAN, and not by a flag:
    // the rule is the type's value bytes minus what the plan lands, and the
    // identity plan lands all of them.
    const TableFixedFill * fill = NULL;
    ...
    if ( identity )
    {
        ...
        // of the plan above it — and no per-entry dispatch, no runtime length
        // and no plan array competing with the record for cache.
        uint8_t image[PackConfigFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) PackConfigFixedBodyBytes );
            PackConfigFixedScatter( image, values[k], report );
            PackConfigFixedClamp( values[k], report ); // the bounds the scatter does not hold (§3.4)
            at += PackConfigFixedRecordBytes;
        }
        return n;
    }
