...
inline bool TableEnumRef( TableIds & ids, Difficulty value, uint64_t & ref )
{
    switch ( value )
    {
        ...
        case Difficulty::Easy: ref = ids.ref_at( 45, 0x483d9e6c1ccd1f9bull ); return true;
        case Difficulty::Normal: ref = ids.ref_at( 83, 0x9ff3121d30f88d52ull ); return true;
        case Difficulty::Hard: ref = ids.ref_at( 53, 0x58c7b5d87587287cull ); return true;
...
inline bool TableEnumSlot( Difficulty value, uint64_t & slot )
{
    switch ( value )
    {
        ...
        case Difficulty::Easy: slot = 111; return true;
        case Difficulty::Normal: slot = 112; return true;
        case Difficulty::Hard: slot = 113; return true;
        default: return false; // no variant names this value: no wire identity
    }
}
...
inline bool TableEnumRef( TableIds & ids, ShipType value, uint64_t & ref )
{
    switch ( value )
    {
        ...
        case ShipType::Fighter: ref = ids.ref_at( 124, 0xd011f7c3c15285c2ull ); return true;
        case ShipType::Bomber: ref = ids.ref_at( 92, 0xa8216aa1c554cb8aull ); return true;
        case ShipType::Scout: ref = ids.ref_at( 66, 0x7d573c625719c1a5ull ); return true;
        default: return false; // no variant names this value: no wire identity
    }
}
...
inline bool TableEnumSlot( ShipType value, uint64_t & slot )
{
    switch ( value )
    {
        ...
        case ShipType::Fighter: slot = 120; return true;
        case ShipType::Bomber: slot = 121; return true;
        case ShipType::Scout: slot = 122; return true;
        default: return false; // no variant names this value: no wire identity
    }
}
...
inline int64_t GunnerSettingsMeasureBody( TableIds & ids, const GunnerSettings & value )
{
    ...
    if ( value.callsign_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 54, 0x5bc44627b9848818ull ) ) + 1 + TableLebBytes( (uint64_t) ( value.callsign_length ) ) + ( value.callsign_length ); } // callsign
    return bytes;
}
...
TABLEDEMO_TABLE_INLINE bool GunnerSettingsSaveBody( TableWriter & w, TableIds & ids, const GunnerSettings & value )
{
    ...
    {
        w.header( ids.ref_at( 54, 0x5bc44627b9848818ull ), 12 ); // callsign
...
inline int64_t ShipEntryMeasureBody( TableIds & ids, const ShipEntry & value )
{
    ...
    if ( value.display_name_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 28, 0x2d21d7cd66bd5a5dull ) ) + 1 + TableLebBytes( (uint64_t) ( value.display_name_length ) ) + ( value.display_name_length ); } // display_name
    ...
    {
        const uint64_t ref_hardpoints = ids.ref_at( 78, 0x95d0cce09ca82b73ull );
...
TABLEDEMO_TABLE_INLINE bool ShipEntrySaveBody( TableWriter & w, TableIds & ids, const ShipEntry & value )
{
    ...
    {
        w.header( ids.ref_at( 28, 0x2d21d7cd66bd5a5dull ), 12 ); // display_name
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.tick_rate != 60 ) { bytes += TableLebBytes( ids.ref_at( 89, 0xa65f859b617deaf3ull ) ) + 1 + 4; } // tick_rate
    if ( value.difficulty != Difficulty::Normal )
    {
        if ( !TableEnumNamed( value.difficulty ) ) { return -1; } // no variant names this value
        const uint64_t ref_difficulty = ids.ref_at( 42, 0x467f35a74c6e4a70ull );
...
TABLEDEMO_TABLE_INLINE bool GlobalSettingsSaveBody( TableWriter & w, TableIds & ids, const GlobalSettings & value )
{
    ...
    {
        w.header( ids.ref_at( 89, 0xa65f859b617deaf3ull ), 8 ); // tick_rate
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.version != 1 ) { bytes += TableLebBytes( ids.ref_at( 108, 0xbb62c62c9808ea37ull ) ) + 1 + 4; } // version
    {
        const int32_t mark_global = ids.count;
        const uint64_t ref_global = ids.ref_at( 69, 0x7fb43557b54149ceull );
    ...
    }
    {
        ...
        const uint64_t ref_ships = ids.ref_at( 26, 0x294a5c4913e1ad44ull );
    ...
    }
    {
        ...
        const uint64_t ref_thresholds = ids.ref_at( 81, 0x9cda940a344e1571ull );
    ...
    {
        const uint64_t ref_reserves = ids.ref_at( 65, 0x77707fccd201c228ull );
...
TABLEDEMO_TABLE_INLINE bool PackConfigSaveBody( TableWriter & w, TableIds & ids, const PackConfig & value )
{
    if ( value.version != 1 )
    {
        w.header( ids.ref_at( 108, 0xbb62c62c9808ea37ull ), 8 ); // version
...
constexpr TableFixedEntry ShipEntryFixedPlan[] = {
    { 0u, 36u, 32u, 0u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // display_name
...
constexpr TableFixedEntry GlobalSettingsFixedPlan[] = {
    { 0u, 0u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tick_rate
    { 5u, 56u, 48u, 5u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // build_note
    { 57u, 60u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // spawn_delays, whole
};
...
constexpr TableFixedEntry PackConfigFixedPlan[] = {
    { 0u, 0u, 9u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // version
    { 9u, 60u, 48u, 9u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // build_note
    { 61u, 64u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // spawn_delays, whole
    { 73u, 112u, 32u, 76u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // display_name
    ...
    { 383u, 448u, 32u, 412u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // display_name
