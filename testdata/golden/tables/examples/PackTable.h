...
constexpr uint64_t GunnerSettingsFixedHash = 0x096e203e5991edb3ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t GunnerSettingsFixedVocab[] = {
...
constexpr int64_t GunnerSettingsFixedVocabBytes = (int64_t) sizeof( GunnerSettingsFixedVocab );
...
{
    return 1 + 4 + GunnerSettingsFixedVocabBytes + count * GunnerSettingsFixedRecordBytes;
}
...
inline int64_t GunnerSettingsFixedSave( const GunnerSettings * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) GunnerSettingsFixedVocabBytes );
    memcpy( buffer + 5, GunnerSettingsFixedVocab, (size_t) GunnerSettingsFixedVocabBytes );
    uint8_t * at = buffer + 5 + GunnerSettingsFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != GunnerSettingsFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, GunnerSettingsFixedVocab, (int32_t) GunnerSettingsFixedVocabBytes, GunnerSettingsFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t ShipEntryFixedHash = 0x1ce562f6d416e6d1ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 11 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t ShipEntryFixedVocab[] = {
...
constexpr int64_t ShipEntryFixedVocabBytes = (int64_t) sizeof( ShipEntryFixedVocab );
...
{
    return 1 + 4 + ShipEntryFixedVocabBytes + count * ShipEntryFixedRecordBytes;
}
...
inline int64_t ShipEntryFixedSave( const ShipEntry * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) ShipEntryFixedVocabBytes );
    memcpy( buffer + 5, ShipEntryFixedVocab, (size_t) ShipEntryFixedVocabBytes );
    uint8_t * at = buffer + 5 + ShipEntryFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != ShipEntryFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, ShipEntryFixedVocab, (int32_t) ShipEntryFixedVocabBytes, ShipEntryFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t GlobalSettingsFixedHash = 0xc25970ba43db796bull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 9 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t GlobalSettingsFixedVocab[] = {
...
constexpr int64_t GlobalSettingsFixedVocabBytes = (int64_t) sizeof( GlobalSettingsFixedVocab );
...
{
    return 1 + 4 + GlobalSettingsFixedVocabBytes + count * GlobalSettingsFixedRecordBytes;
}
...
inline int64_t GlobalSettingsFixedSave( const GlobalSettings * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) GlobalSettingsFixedVocabBytes );
    memcpy( buffer + 5, GlobalSettingsFixedVocab, (size_t) GlobalSettingsFixedVocabBytes );
    uint8_t * at = buffer + 5 + GlobalSettingsFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != GlobalSettingsFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, GlobalSettingsFixedVocab, (int32_t) GlobalSettingsFixedVocabBytes, GlobalSettingsFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t PackConfigFixedHash = 0xd21606df2ae90463ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 45 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t PackConfigFixedVocab[] = {
...
constexpr int64_t PackConfigFixedVocabBytes = (int64_t) sizeof( PackConfigFixedVocab );
...
{
    return 1 + 4 + PackConfigFixedVocabBytes + count * PackConfigFixedRecordBytes;
}
...
inline int64_t PackConfigFixedSave( const PackConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) PackConfigFixedVocabBytes );
    memcpy( buffer + 5, PackConfigFixedVocab, (size_t) PackConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + PackConfigFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != PackConfigFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, PackConfigFixedVocab, (int32_t) PackConfigFixedVocabBytes, PackConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
