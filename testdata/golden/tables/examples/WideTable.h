...
inline int64_t WideBlobMeasureMessageBody( int64_t at, const WideBlob & value );
inline bool WideBlobSaveMessageBody( TableBitWriter & w, const WideBlob & value );
inline bool WideBlobLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, WideBlob & value );
...
{
    TableIds ids;
    const int64_t body = WideBlobMeasureBody( ids, value );
...
inline int64_t WideBlobSave( const WideBlob & value, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( !WideBlobSaveBody( w, ids, value ) || ids.overflow ) { return -1; }
            ...
            case 0xe3b1ca6a3b48dddcull: // samples
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    else if ( elem_kind != 7 )
                    {
                        ...
                        if ( widened_keep > 70000 ) { widened_keep = 70000; r.report->clamped++; }
                        ...
                        for ( uint64_t widened_i = 0; widened_i < widened_keep; widened_i++ )
                        {
                            ...
                            value.samples[(int32_t) widened_i] = decoded_v;
...
inline TableOpenVerdict WideBlobLoadVerdict( WideBlob & value, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    TableIdTable table;
    ...
    if ( !WideBlobLoadBody( r, value ) ) { return TableOpenBodyStopped; }
    return TableOpenOk;
}
...
{
    return WideBlobLoadVerdict( value, buffer, bytes, report ) == TableOpenOk;
}
...
inline int64_t WideBlobMeasureMessageBody( int64_t at, const WideBlob & value )
{
    ...
    if ( value.samples_count > 0 )
    {
        bits += kTableMessageRefBitsHere;
        bits += 17;
        bits += (int64_t) ( value.samples_count ) * 16;
    }
...
inline bool WideBlobSaveMessageBody( TableBitWriter & w, const WideBlob & value )
{
    ...
    if ( value.label_length > 0 )
    {
        ...
        w.put( (uint64_t) value.label_length, 17 );
    ...
    if ( value.payload_length > 0 )
    {
        w.put( 108, kTableMessageRefBitsHere );
        w.put( (uint64_t) value.payload_length, 17 );
        w.align(); // a string or a bytes ALIGNS before its bytes
    ...
    if ( value.samples_count > 0 )
    {
        w.put( 109, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.samples_count ) - 0, 17 );
        for ( int32_t i = 0; i < value.samples_count; i++ )
        {
            w.put( (uint64_t) ( value.samples[i] ), 16 );
        }
    }
        ...
        switch ( entry.id )
        {
            case 0x39f7fcec8fcb623dull: // label
            {
                ...
                }
                {
                    ...
                    const int32_t kept = (int32_t) TableUtf8Clamp( text, n, 70000 );
...
inline int64_t WideBlobMeasureMessages( const WideBlob * values, int64_t count, TableReport * report )
{
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = WideBlobMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
...
inline int64_t WideBlobSaveMessages( const WideBlob * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !WideBlobSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == WideBlobMeasureMessages( values, count, report )
}
...
inline bool WideBlobLoadMessages( WideBlob * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    ...
    *count = 0;
    ...
    {
        if ( !WideBlobLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
...
static_assert( sizeof( WideBlob ) == 280016, "WideBlob: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( WideBlob, label ) == 0, "WideBlob.label: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( WideBlob, label_length ) == 70004, "WideBlob.label_length: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( WideBlob, payload ) == 70008, "WideBlob.payload: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( WideBlob, payload_length ) == 140008, "WideBlob.payload_length: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( WideBlob, samples ) == 140012, "WideBlob.samples: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( WideBlob, samples_count ) == 280012, "WideBlob.samples_count: the fixed form's plan lands here" );

// WideBlob's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void WideBlobFixedWriteBody( uint8_t * b, const WideBlob & value )
{
    (void) b; (void) value;
    TableFixedPut32( b + 0, (uint32_t) value.label_length );
    memcpy( b + 4, value.label, 70000 );
    TableFixedPut32( b + 70004, (uint32_t) value.payload_length );
    memcpy( b + 70008, value.payload, 70000 );
    TableFixedPut32( b + 140008, (uint32_t) value.samples_count );
    for ( int64_t i = 0; i < 70000; ++i )
    {
        TableFixedPut16( b + 140012 + i * 2 + 0, (uint16_t) value.samples[i] );
    }
}
...
constexpr int64_t WideBlobFixedBodyBytes = 280012;
constexpr int64_t WideBlobFixedRecordBytes = 8 + WideBlobFixedBodyBytes; // the hash and the body
constexpr uint64_t WideBlobFixedHash = 0x8202b907d4ac81c4ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t WideBlobFixedVocab[] = {
...
constexpr int64_t WideBlobFixedVocabBytes = (int64_t) sizeof( WideBlobFixedVocab );
...
constexpr TableFixedDst WideBlobFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // WideBlob
    { (uint32_t) __builtin_offsetof( WideBlob, label_length ), 0, (uint32_t) __builtin_offsetof( WideBlob, label ), 0, 1 }, // label
    { (uint32_t) __builtin_offsetof( WideBlob, payload_length ), 1, (uint32_t) __builtin_offsetof( WideBlob, payload ), 1, 3 }, // payload
    { 0, 0, 0, 0, 0 }, // u8
    { (uint32_t) __builtin_offsetof( WideBlob, samples ), (uint32_t) sizeof( uint16_t ), (uint32_t) __builtin_offsetof( WideBlob, samples_count ), 1, 0 }, // samples
    { 0, 0, 0, 0, 0 }, // element
};
...
// guard at all.
constexpr TableFixedEntry WideBlobFixedPlan[] = {
    { 0u, 70004u, 70000u, 0u, kTableFixedNoGuard, kTableFixedText, 1, 0, 0 }, // label
    { 70004u, 140008u, 70000u, 70008u, kTableFixedNoGuard, kTableFixedText, 3, 0, 0 }, // payload
    { 140008u, 280012u, 70000u, 0u, kTableFixedNoGuard, kTableFixedCount, 0, 0, 0 }, // samples count
    { 140012u, 140012u, 140000u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0 }, // samples, whole
};
constexpr int32_t WideBlobFixedPlanCount = 4;
constexpr int32_t WideBlobFixedPlanGuarded = 4;

// A FILE: the form byte, the block, then the records to the end of it.
constexpr int64_t WideBlobFixedMeasure( int64_t count )
{
    return 1 + 4 + WideBlobFixedVocabBytes + count * WideBlobFixedRecordBytes;
}

inline int64_t WideBlobFixedSave( const WideBlob * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = WideBlobFixedMeasure( count );
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) WideBlobFixedVocabBytes );
    memcpy( buffer + 5, WideBlobFixedVocab, (size_t) WideBlobFixedVocabBytes );
    uint8_t * at = buffer + 5 + WideBlobFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
        TableFixedPut64( at, WideBlobFixedHash );
        memset( at + 8, 0, (size_t) WideBlobFixedBodyBytes ); // the prefill's zeros
        WideBlobFixedWriteBody( at + 8, values[k] );
        at += WideBlobFixedRecordBytes;
    }
    return need;
}
...
inline int64_t WideBlobFixedLoad( WideBlob * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = WideBlobFixedPlan;
    int32_t entry_count = WideBlobFixedPlanCount;
    int32_t entry_guarded = WideBlobFixedPlanGuarded;
    int64_t record_bytes = WideBlobFixedRecordBytes;
    if ( hash != WideBlobFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, WideBlobFixedVocab, (int32_t) WideBlobFixedVocabBytes, WideBlobFixedDst, plan, plan_capacity, &compiled_guarded, report );
    ...
    {
        WideBlobReset( values[k] ); // the declared defaults, one prefill
...
template <typename... Args>
inline void WideBlobSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "WideBlob is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void WideBlobLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "WideBlob is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void WideBlobSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "WideBlob is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
// parameter on a load.
inline const WideBlob * WideBlobOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const WideBlob *) TableCookOpen( bytes, length, (uint64_t) sizeof( WideBlob ), (uint64_t) alignof( WideBlob ), reason );
}
...
inline const TableTypeInfo * WideBlobTableType()
{
    static const TableFieldInfo fields[] = {
        { "label", "label", "string", 0x39f7fcec8fcb623dull, 12, false, true, false, 70000, (uint32_t) offsetof( WideBlob, label ), (uint32_t) sizeof( WideBlob::label ), (uint32_t) offsetof( WideBlob, label_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
        { "payload", "payload", "bytes", 0xcfb8a9d063b5e9e5ull, 6, true, true, false, 70000, (uint32_t) offsetof( WideBlob, payload ), (uint32_t) sizeof( WideBlob::payload[0] ), (uint32_t) offsetof( WideBlob, payload_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
        { "samples", "samples", "uint16", 0xe3b1ca6a3b48dddcull, 7, true, true, false, 70000, (uint32_t) offsetof( WideBlob, samples ), (uint32_t) sizeof( WideBlob::samples[0] ), (uint32_t) offsetof( WideBlob, samples_count ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
    };
    static const TableTypeInfo info = { "WideBlob", (uint32_t) sizeof( WideBlob ), 3, fields, +[]( void * p ) { WideBlobReset( *(WideBlob *) p ); }, TableDocNone, 0, NULL };
    return &info;
}
