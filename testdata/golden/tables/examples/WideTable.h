...
inline int64_t WideBlobMeasureBody( TableIds & ids, const WideBlob & value )
{
    ...
    if ( value.label_length > 0 ) { bytes += TableLebBytes( ids.ref( 0x39f7fcec8fcb623dull, 108 ) ) + 1 + TableLebBytes( (uint64_t) ( value.label_length ) ) + ( value.label_length ); } // label
    ...
    if ( value.payload_length > 0 )
    {
        ...
        bytes += TableLebBytes( ids.ref( 0xcfb8a9d063b5e9e5ull, 109 ) ) + 1 + TableLebBytes( (uint64_t) ( body_payload ) ) + ( body_payload ); // payload
    }
    if ( value.samples_count < 0 || value.samples_count > 70000 ) { return -1; } // storage invariant
    if ( value.samples_count > 0 )
    {
        const uint64_t ref_samples = ids.ref( 0xe3b1ca6a3b48dddcull, 110 );
...
TABLEDEMO_TABLE_INLINE bool WideBlobSaveBody( TableWriter & w, TableIds & ids, const WideBlob & value )
{
    ...
    {
        w.putleb( ids.ref( 0x39f7fcec8fcb623dull, 108 ) ); w.put8( 12 ); // label
    ...
    {
        const int64_t body_payload = 1 + TableLebBytes( (uint64_t) value.payload_length ) + value.payload_length;
        w.putleb( ids.ref( 0xcfb8a9d063b5e9e5ull, 109 ) ); w.put8( 14 ); // payload
...
inline int64_t WideBlobMeasureMessage( const WideBlob & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = WideBlobMeasureBody( ids, value );
...
inline int64_t WideBlobSaveMessage( const WideBlob & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !WideBlobSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == WideBlobMeasureMessage( value )
}
...
inline bool WideBlobLoadMessage( WideBlob & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    WideBlobReset( value );
