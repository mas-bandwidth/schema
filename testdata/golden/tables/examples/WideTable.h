...
inline int64_t WideBlobMeasureBody( TableIds & ids, const WideBlob & value )
{
    ...
    if ( value.label_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 36, 0x39f7fcec8fcb623dull ) ) + 1 + TableLebBytes( (uint64_t) ( value.label_length ) ) + ( value.label_length ); } // label
    ...
    if ( value.payload_length > 0 )
    {
        ...
        bytes += TableLebBytes( ids.ref_at( 123, 0xcfb8a9d063b5e9e5ull ) ) + 1 + TableLebBytes( (uint64_t) ( body_payload ) ) + ( body_payload ); // payload
    }
    if ( value.samples_count < 0 || value.samples_count > 70000 ) { return -1; } // storage invariant
    if ( value.samples_count > 0 )
    {
        const uint64_t ref_samples = ids.ref_at( 137, 0xe3b1ca6a3b48dddcull );
...
TABLEDEMO_TABLE_INLINE bool WideBlobSaveBody( TableWriter & w, TableIds & ids, const WideBlob & value )
{
    ...
    {
        w.header( ids.ref_at( 36, 0x39f7fcec8fcb623dull ), 12 ); // label
    ...
    {
        const int64_t body_payload = 1 + TableLebBytes( (uint64_t) value.payload_length ) + value.payload_length;
        w.header( ids.ref_at( 123, 0xcfb8a9d063b5e9e5ull ), 14 ); // payload
...
inline bool WideBlobSaveMessageBody( TableBitWriter & w, const WideBlob & value )
{
    ...
    if ( value.samples_count > 0 )
    {
        w.put( 110, kTableMessageRefBitsHere );
