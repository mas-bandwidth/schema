...
{
    return 1 + 4 + RangedSignedFixedLayoutBytes + count * RangedSignedFixedRecordBytes;
}
...
inline int64_t RangedSignedFixedSave( const RangedSigned * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) RangedSignedFixedLayoutBytes );
    memcpy( buffer + 5, RangedSignedFixedLayout, (size_t) RangedSignedFixedLayoutBytes );
    uint8_t * at = buffer + 5 + RangedSignedFixedLayoutBytes;
...
{
    return 1 + 4 + RangedUnsignedFixedLayoutBytes + count * RangedUnsignedFixedRecordBytes;
}
...
inline int64_t RangedUnsignedFixedSave( const RangedUnsigned * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) RangedUnsignedFixedLayoutBytes );
    memcpy( buffer + 5, RangedUnsignedFixedLayout, (size_t) RangedUnsignedFixedLayoutBytes );
    uint8_t * at = buffer + 5 + RangedUnsignedFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
...
{
    return 1 + 4 + RangedWidthsFixedLayoutBytes + count * RangedWidthsFixedRecordBytes;
}
...
inline int64_t RangedWidthsFixedSave( const RangedWidths * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) RangedWidthsFixedLayoutBytes );
    memcpy( buffer + 5, RangedWidthsFixedLayout, (size_t) RangedWidthsFixedLayoutBytes );
    uint8_t * at = buffer + 5 + RangedWidthsFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
