...
constexpr uint64_t RangedSignedFixedHash = 0xaaf4b92f603d1a73ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t RangedSignedFixedVocab[] = {
...
constexpr int64_t RangedSignedFixedVocabBytes = (int64_t) sizeof( RangedSignedFixedVocab );
...
{
    return 1 + 4 + RangedSignedFixedVocabBytes + count * RangedSignedFixedRecordBytes;
}
...
inline int64_t RangedSignedFixedSave( const RangedSigned * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) RangedSignedFixedVocabBytes );
    memcpy( buffer + 5, RangedSignedFixedVocab, (size_t) RangedSignedFixedVocabBytes );
    uint8_t * at = buffer + 5 + RangedSignedFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != RangedSignedFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, RangedSignedFixedVocab, (int32_t) RangedSignedFixedVocabBytes, RangedSignedFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t RangedUnsignedFixedHash = 0x9e7760dd0c2ba2f6ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 19 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t RangedUnsignedFixedVocab[] = {
...
constexpr int64_t RangedUnsignedFixedVocabBytes = (int64_t) sizeof( RangedUnsignedFixedVocab );
...
{
    return 1 + 4 + RangedUnsignedFixedVocabBytes + count * RangedUnsignedFixedRecordBytes;
}
...
inline int64_t RangedUnsignedFixedSave( const RangedUnsigned * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) RangedUnsignedFixedVocabBytes );
    memcpy( buffer + 5, RangedUnsignedFixedVocab, (size_t) RangedUnsignedFixedVocabBytes );
    uint8_t * at = buffer + 5 + RangedUnsignedFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != RangedUnsignedFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, RangedUnsignedFixedVocab, (int32_t) RangedUnsignedFixedVocabBytes, RangedUnsignedFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t RangedWidthsFixedHash = 0x0f48d5bc4bc8f930ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 7 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t RangedWidthsFixedVocab[] = {
...
constexpr int64_t RangedWidthsFixedVocabBytes = (int64_t) sizeof( RangedWidthsFixedVocab );
...
{
    return 1 + 4 + RangedWidthsFixedVocabBytes + count * RangedWidthsFixedRecordBytes;
}
...
inline int64_t RangedWidthsFixedSave( const RangedWidths * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) RangedWidthsFixedVocabBytes );
    memcpy( buffer + 5, RangedWidthsFixedVocab, (size_t) RangedWidthsFixedVocabBytes );
    uint8_t * at = buffer + 5 + RangedWidthsFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != RangedWidthsFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, RangedWidthsFixedVocab, (int32_t) RangedWidthsFixedVocabBytes, RangedWidthsFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
