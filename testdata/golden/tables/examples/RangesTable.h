    ...
    for ( ;; )
    {
        ...
        switch ( entry.id )
        {
            case 0x48121511bf702eb5ull: // i8_span
            {
                ...
                {
                    const int64_t width = TableMessageValueBits( 2, entry.packing, entry.value_bits );
                    uint64_t raw = 0;
            ...
            case 0xb655574ea23760b4ull: // i16_span
            {
                ...
                {
                    const int64_t width = TableMessageValueBits( 3, entry.packing, entry.value_bits );
                    uint64_t raw = 0;
                    if ( width < 0 || !r.get( raw, width ) ) { report->malformed = true; return false; }
                    int64_t decoded_wide = (int64_t) raw;
                    if ( entry.packing == 1 ) { decoded_wide = (int64_t) ( raw + (uint64_t) entry.base_lo ); }
            ...
            case 0xc70cde6b85b6197dull: // edges
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        ...
                        int16_t scratch = 0;
                        {
                            const int64_t width_2 = TableMessageValueBits( 3, entry.elem_packing, entry.elem_value_bits );
                            uint64_t raw_2 = 0;
