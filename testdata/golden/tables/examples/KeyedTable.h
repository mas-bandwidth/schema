                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    if ( hash != TeamConfigFixedHash )
    {
        ...
        else
        {
            ...
            const int32_t made = TableFixedCompile( parsed, TeamConfigFixedLayout, (int32_t) TeamConfigFixedLayoutBytes, TeamConfigFixedDst, dest, dest_capacity, &compiled_guarded, report );
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    if ( hash != GunnerConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            const int32_t made = TableFixedCompile( parsed, GunnerConfigFixedLayout, (int32_t) GunnerConfigFixedLayoutBytes, GunnerConfigFixedDst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_malformed : plan_too_large; return -1; }
            record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    if ( hash != TurretConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            const int32_t made = TableFixedCompile( parsed, TurretConfigFixedLayout, (int32_t) TurretConfigFixedLayoutBytes, TurretConfigFixedDst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_malformed : plan_too_large; return -1; }
            record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    if ( hash != HullConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            const int32_t made = TableFixedCompile( parsed, HullConfigFixedLayout, (int32_t) HullConfigFixedLayoutBytes, HullConfigFixedDst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_malformed : plan_too_large; return -1; }
            record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    if ( hash != KeyedConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            const int32_t made = TableFixedCompile( parsed, KeyedConfigFixedLayout, (int32_t) KeyedConfigFixedLayoutBytes, KeyedConfigFixedDst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_malformed : plan_too_large; return -1; }
            record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
