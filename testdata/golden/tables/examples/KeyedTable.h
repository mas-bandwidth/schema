...
constexpr int TeamConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int GunnerConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int TurretConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int HullConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int ScoreBoardFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int KeyedConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// TeamConfig's LEAVES: one entry per run of bytes that lands, at a base the caller
...
constexpr int TeamConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( TeamConfig, spawn_count );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    out[n++] = TableFixedEntry{ src + 4u, dst + (uint32_t) __builtin_offsetof( TeamConfig, banner_length ), 16u, dst + (uint32_t) __builtin_offsetof( TeamConfig, banner ), kTableFixedNoGuard, kTableFixedText, 1, 0 }; // banner
    return n;
}
...
// GunnerConfig's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int GunnerConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    ...
    {
        const uint32_t es = src + 0u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( GunnerConfig, reaction );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 4u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( GunnerConfig, tracking );
...
// TurretConfig's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int TurretConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    ...
    {
        const uint32_t es = src + 0u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( TurretConfig, damage );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 4u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( TurretConfig, cooldown );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    out[n++] = TableFixedEntry{ src + 8u, dst + (uint32_t) __builtin_offsetof( TurretConfig, gunner_present ), 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // gunner present
    {
        const uint32_t es = src + 9u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( TurretConfig, gunner );
        n += GunnerConfigFixedLeaves( out + n, es, ed );
    }
    return n;
}
...
// HullConfig's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int HullConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    ...
    {
        const uint32_t es = src + 0u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( HullConfig, health );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 4u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( HullConfig, mass );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    for ( uint32_t i = 0; i < 3u; ++i )
    {
        const uint32_t es = src + 8u + i * 14u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( HullConfig, turrets.slots ) + i * (uint32_t) sizeof( TurretConfig );
        n += TurretConfigFixedLeaves( out + n, es, ed );
    }
    return n;
}
...
// ScoreBoard's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int ScoreBoardFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    (void) out; (void) src; (void) dst;
    static_assert( sizeof( int32_t ) == 4, "ScoreBoard.per_team: a flat element's storage IS its wire image" );
    out[n++] = TableFixedEntry{ src + 0u, dst + (uint32_t) __builtin_offsetof( ScoreBoard, per_team ), 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // per_team, whole
    return n;
}
...
// KeyedConfig's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int KeyedConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    ...
    for ( uint32_t i = 0; i < 3u; ++i )
    {
        const uint32_t es = src + 0u + i * 24u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( KeyedConfig, teams.slots ) + i * (uint32_t) sizeof( TeamConfig );
        n += TeamConfigFixedLeaves( out + n, es, ed );
    }
    for ( uint32_t i = 0; i < 3u; ++i )
    {
        const uint32_t es = src + 72u + i * 50u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( KeyedConfig, hulls.slots ) + i * (uint32_t) sizeof( HullConfig );
        n += HullConfigFixedLeaves( out + n, es, ed );
    }
    {
        const uint32_t es = src + 222u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( KeyedConfig, scores );
        n += ScoreBoardFixedLeaves( out + n, es, ed );
    }
    return n;
}
...
inline constexpr int64_t TeamConfigFixedBodyBytes = 24;
inline constexpr int64_t TeamConfigFixedRecordBytes = 8 + TeamConfigFixedBodyBytes; // the hash and the body
inline constexpr uint64_t TeamConfigFixedHash = 0x527038c53c0297c3ull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t TeamConfigFixedLayout[] = {
...
inline constexpr int64_t TeamConfigFixedLayoutBytes = (int64_t) sizeof( TeamConfigFixedLayout );
...
inline constexpr TableFixedDst TeamConfigFixedDst[] = {
...
inline int64_t TeamConfigFixedSave( const TeamConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) TeamConfigFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != TeamConfigFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, TeamConfigFixedLayout, (int32_t) TeamConfigFixedLayoutBytes, TeamConfigFixedDst, plan, plan_capacity, report );
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
inline constexpr int64_t GunnerConfigFixedBodyBytes = 5;
inline constexpr int64_t GunnerConfigFixedRecordBytes = 8 + GunnerConfigFixedBodyBytes; // the hash and the body
inline constexpr uint64_t GunnerConfigFixedHash = 0x113a539593602242ull; // fnv1a64 over the layout's bytes

// THE LAYOUT (form 1 called this the vocabulary block): 3 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
inline constexpr uint8_t GunnerConfigFixedLayout[] = {
...
inline constexpr int64_t GunnerConfigFixedLayoutBytes = (int64_t) sizeof( GunnerConfigFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout
// lands values through.
inline constexpr TableFixedDst GunnerConfigFixedDst[] = {
...
inline int64_t GunnerConfigFixedSave( const GunnerConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) GunnerConfigFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != GunnerConfigFixedHash )
    {
        ...
        TableMessageReason why = layout_malformed;
        if ( !TableFixedParseLayout( layout, layout_bytes, parsed, why ) ) { report->refused = true; report->reason = why; return -1; }
        const int32_t made = TableFixedCompile( parsed, GunnerConfigFixedLayout, (int32_t) GunnerConfigFixedLayoutBytes, GunnerConfigFixedDst, plan, plan_capacity, report );
        if ( made < 0 ) { report->refused = true; report->reason = plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
inline constexpr int64_t TurretConfigFixedBodyBytes = 14;
inline constexpr int64_t TurretConfigFixedRecordBytes = 8 + TurretConfigFixedBodyBytes; // the hash and the body
inline constexpr uint64_t TurretConfigFixedHash = 0xd8715b0be3854d91ull; // fnv1a64 over the layout's bytes

// THE LAYOUT (form 1 called this the vocabulary block): 7 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
inline constexpr uint8_t TurretConfigFixedLayout[] = {
...
inline constexpr int64_t TurretConfigFixedLayoutBytes = (int64_t) sizeof( TurretConfigFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout
// lands values through.
inline constexpr TableFixedDst TurretConfigFixedDst[] = {
...
inline int64_t TurretConfigFixedSave( const TurretConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) TurretConfigFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != TurretConfigFixedHash )
    {
        ...
        TableMessageReason why = layout_malformed;
        if ( !TableFixedParseLayout( layout, layout_bytes, parsed, why ) ) { report->refused = true; report->reason = why; return -1; }
        const int32_t made = TableFixedCompile( parsed, TurretConfigFixedLayout, (int32_t) TurretConfigFixedLayoutBytes, TurretConfigFixedDst, plan, plan_capacity, report );
        if ( made < 0 ) { report->refused = true; report->reason = plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
inline constexpr int64_t HullConfigFixedBodyBytes = 50;
inline constexpr int64_t HullConfigFixedRecordBytes = 8 + HullConfigFixedBodyBytes; // the hash and the body
inline constexpr uint64_t HullConfigFixedHash = 0xc65dc3bc15d5a954ull; // fnv1a64 over the layout's bytes

// THE LAYOUT (form 1 called this the vocabulary block): 15 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
inline constexpr uint8_t HullConfigFixedLayout[] = {
...
inline constexpr int64_t HullConfigFixedLayoutBytes = (int64_t) sizeof( HullConfigFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout
// lands values through.
inline constexpr TableFixedDst HullConfigFixedDst[] = {
...
inline int64_t HullConfigFixedSave( const HullConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) HullConfigFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != HullConfigFixedHash )
    {
        ...
        TableMessageReason why = layout_malformed;
        if ( !TableFixedParseLayout( layout, layout_bytes, parsed, why ) ) { report->refused = true; report->reason = why; return -1; }
        const int32_t made = TableFixedCompile( parsed, HullConfigFixedLayout, (int32_t) HullConfigFixedLayoutBytes, HullConfigFixedDst, plan, plan_capacity, report );
        if ( made < 0 ) { report->refused = true; report->reason = plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
inline constexpr int64_t KeyedConfigFixedBodyBytes = 234;
inline constexpr int64_t KeyedConfigFixedRecordBytes = 8 + KeyedConfigFixedBodyBytes; // the hash and the body
inline constexpr uint64_t KeyedConfigFixedHash = 0xac0c8c80f3967de4ull; // fnv1a64 over the layout's bytes

// THE LAYOUT (form 1 called this the vocabulary block): 36 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
inline constexpr uint8_t KeyedConfigFixedLayout[] = {
...
inline constexpr int64_t KeyedConfigFixedLayoutBytes = (int64_t) sizeof( KeyedConfigFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout
// lands values through.
inline constexpr TableFixedDst KeyedConfigFixedDst[] = {
...
inline int64_t KeyedConfigFixedSave( const KeyedConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) KeyedConfigFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != KeyedConfigFixedHash )
    {
        ...
        TableMessageReason why = layout_malformed;
        if ( !TableFixedParseLayout( layout, layout_bytes, parsed, why ) ) { report->refused = true; report->reason = why; return -1; }
        const int32_t made = TableFixedCompile( parsed, KeyedConfigFixedLayout, (int32_t) KeyedConfigFixedLayoutBytes, KeyedConfigFixedDst, plan, plan_capacity, report );
        if ( made < 0 ) { report->refused = true; report->reason = plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
