...
constexpr uint64_t TeamConfigFixedHash = 0x527038c53c0297c3ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t TeamConfigFixedVocab[] = {
...
constexpr int64_t TeamConfigFixedVocabBytes = (int64_t) sizeof( TeamConfigFixedVocab );
...
{
    return 1 + 4 + TeamConfigFixedVocabBytes + count * TeamConfigFixedRecordBytes;
}
...
inline int64_t TeamConfigFixedSave( const TeamConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) TeamConfigFixedVocabBytes );
    memcpy( buffer + 5, TeamConfigFixedVocab, (size_t) TeamConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + TeamConfigFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != TeamConfigFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, TeamConfigFixedVocab, (int32_t) TeamConfigFixedVocabBytes, TeamConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t GunnerConfigFixedHash = 0x113a539593602242ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 3 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t GunnerConfigFixedVocab[] = {
...
constexpr int64_t GunnerConfigFixedVocabBytes = (int64_t) sizeof( GunnerConfigFixedVocab );
...
{
    return 1 + 4 + GunnerConfigFixedVocabBytes + count * GunnerConfigFixedRecordBytes;
}
...
inline int64_t GunnerConfigFixedSave( const GunnerConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) GunnerConfigFixedVocabBytes );
    memcpy( buffer + 5, GunnerConfigFixedVocab, (size_t) GunnerConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + GunnerConfigFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != GunnerConfigFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, GunnerConfigFixedVocab, (int32_t) GunnerConfigFixedVocabBytes, GunnerConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t TurretConfigFixedHash = 0xd8715b0be3854d91ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 7 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t TurretConfigFixedVocab[] = {
...
constexpr int64_t TurretConfigFixedVocabBytes = (int64_t) sizeof( TurretConfigFixedVocab );
...
{
    return 1 + 4 + TurretConfigFixedVocabBytes + count * TurretConfigFixedRecordBytes;
}
...
inline int64_t TurretConfigFixedSave( const TurretConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) TurretConfigFixedVocabBytes );
    memcpy( buffer + 5, TurretConfigFixedVocab, (size_t) TurretConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + TurretConfigFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != TurretConfigFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, TurretConfigFixedVocab, (int32_t) TurretConfigFixedVocabBytes, TurretConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t HullConfigFixedHash = 0xc65dc3bc15d5a954ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 15 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t HullConfigFixedVocab[] = {
...
constexpr int64_t HullConfigFixedVocabBytes = (int64_t) sizeof( HullConfigFixedVocab );
...
{
    return 1 + 4 + HullConfigFixedVocabBytes + count * HullConfigFixedRecordBytes;
}
...
inline int64_t HullConfigFixedSave( const HullConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) HullConfigFixedVocabBytes );
    memcpy( buffer + 5, HullConfigFixedVocab, (size_t) HullConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + HullConfigFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != HullConfigFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, HullConfigFixedVocab, (int32_t) HullConfigFixedVocabBytes, HullConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t KeyedConfigFixedHash = 0xac0c8c80f3967de4ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 36 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t KeyedConfigFixedVocab[] = {
...
constexpr int64_t KeyedConfigFixedVocabBytes = (int64_t) sizeof( KeyedConfigFixedVocab );
...
{
    return 1 + 4 + KeyedConfigFixedVocabBytes + count * KeyedConfigFixedRecordBytes;
}
...
inline int64_t KeyedConfigFixedSave( const KeyedConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) KeyedConfigFixedVocabBytes );
    memcpy( buffer + 5, KeyedConfigFixedVocab, (size_t) KeyedConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + KeyedConfigFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != KeyedConfigFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, KeyedConfigFixedVocab, (int32_t) KeyedConfigFixedVocabBytes, KeyedConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
