...
static_assert( sizeof( TeamConfig ) == 28, "TeamConfig: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( TeamConfig, spawn_count ) == 0, "TeamConfig.spawn_count: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( TeamConfig, banner ) == 4, "TeamConfig.banner: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( TeamConfig, banner_length ) == 24, "TeamConfig.banner_length: the fixed form's plan lands here" );
static_assert( sizeof( GunnerConfig ) == 8, "GunnerConfig: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( GunnerConfig, reaction ) == 0, "GunnerConfig.reaction: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( GunnerConfig, tracking ) == 4, "GunnerConfig.tracking: the fixed form's plan lands here" );
static_assert( sizeof( TurretConfig ) == 20, "TurretConfig: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( TurretConfig, damage ) == 0, "TurretConfig.damage: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( TurretConfig, cooldown ) == 4, "TurretConfig.cooldown: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( TurretConfig, gunner ) == 8, "TurretConfig.gunner: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( TurretConfig, gunner_present ) == 16, "TurretConfig.gunner_present: the fixed form's plan lands here" );
static_assert( sizeof( HullConfig ) == 68, "HullConfig: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( HullConfig, health ) == 0, "HullConfig.health: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( HullConfig, mass ) == 4, "HullConfig.mass: the fixed form's plan lands here" );
...
inline void TeamConfigFixedWriteBody( uint8_t * b, const TeamConfig & value )
{
    (void) b; (void) value;
...
// which is also what zero-fills every byte of declared slack.
inline void GunnerConfigFixedWriteBody( uint8_t * b, const GunnerConfig & value )
{
    (void) b; (void) value;
...
// TurretConfig's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void TurretConfigFixedWriteBody( uint8_t * b, const TurretConfig & value )
{
    (void) b; (void) value;
...
// which is also what zero-fills every byte of declared slack.
inline void HullConfigFixedWriteBody( uint8_t * b, const HullConfig & value )
{
    (void) b; (void) value;
...
// ScoreBoard's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
...
// KeyedConfig's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
...
constexpr int64_t TeamConfigFixedBodyBytes = 24;
constexpr int64_t TeamConfigFixedRecordBytes = 8 + TeamConfigFixedBodyBytes; // the hash and the body
constexpr uint64_t TeamConfigFixedHash = 0x5a61da98d3a823e3ull; // fnv1a64 over the layout and the definitions digest (bill §13)
...
constexpr uint8_t TeamConfigFixedLayout[] = {
...
constexpr int64_t TeamConfigFixedLayoutBytes = (int64_t) sizeof( TeamConfigFixedLayout );
...
constexpr TableFixedEntry TeamConfigFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // spawn_count
    { 4u, 24u, 16u, 4u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // banner
};
...
constexpr TableFixedFill TeamConfigFixedCover[] = {
    { 0u, 21u },
    { 24u, 4u },
};
constexpr int32_t TeamConfigFixedCoverCount = 2;
...
constexpr uint64_t TeamConfigFixedLineage[] = {
    0x5a61da98d3a823e3ull, // identity
};
constexpr int32_t TeamConfigFixedLineageCount = 1;
constexpr int32_t TeamConfigFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t TeamConfigFixedFloorLive = TeamConfigFixedFloor;
inline void TeamConfigFixedSetFloorForTest( int32_t index ) { TeamConfigFixedFloorLive = index; }
#endif

constexpr uint8_t TeamConfigFixedKnown0Layout[] = {
    0x03, 0x00, 0x00, 0x00, 0x3a, 0x11, 0xfb, 0x11, 0x5d, 0x55, 0xf8, 0x1c, 0x0d, 0x18, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0x74, 0xb6, 0x5d, 0xd6, 0xe2, 0x99, 0xec, 0xce, 0x04, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xcf, 0x0c, 0xa0, 0xc7, 0xb1, 0xda, 0xa0, 0xbc, 0x0c, 0x14,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr TableFixedKnownLayout TeamConfigFixedKnown[] = {
    { 0x5a61da98d3a823e3ull, TeamConfigFixedKnown0Layout, 55, 32 }, // identity
};

constexpr const TableFixedKnownRange * TeamConfigFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t TeamConfigFixedKnownRangeCounts[] = { 0 };
...
constexpr int64_t TeamConfigFixedMeasure( int64_t count )
{
...
inline int64_t TeamConfigFixedSave( const TeamConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) TeamConfigFixedBodyBytes ); // the prefill's zeros
        TeamConfigFixedWriteBody( at + 8, values[k] );
        at += TeamConfigFixedRecordBytes;
    }
    return need;
}
...
inline int64_t TeamConfigFixedLoad( TeamConfig * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    for ( int32_t i = 0; i < TeamConfigFixedLineageCount; ++i )
    {
        if ( TeamConfigFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = TeamConfigFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = TeamConfigFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = TeamConfigFixedKnown[lineage_at];
    ...
    const TableFixedEntry * entries = TeamConfigFixedPlan;
    int32_t entry_count = TeamConfigFixedPlanCount;
    int32_t entry_guarded = TeamConfigFixedPlanGuarded;
    ...
    if ( hash != TeamConfigFixedHash )
    {
        ...
        else
        {
            ...
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, TeamConfigFixedLayout, (int32_t) TeamConfigFixedLayoutBytes, TeamConfigFixedDst,
                                                   TeamConfigFixedCover, TeamConfigFixedCoverCount,
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( TeamConfigFixedKnownRanges[lineage_at], TeamConfigFixedKnownRangeCounts[lineage_at],
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
constexpr int64_t GunnerConfigFixedBodyBytes = 5;
constexpr int64_t GunnerConfigFixedRecordBytes = 8 + GunnerConfigFixedBodyBytes; // the hash and the body
constexpr uint64_t GunnerConfigFixedHash = 0x113a539593602242ull; // fnv1a64 over the layout and the definitions digest (bill §13)

// THE LAYOUT (form 1 calls this the vocabulary block): 3 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
constexpr uint8_t GunnerConfigFixedLayout[] = {
...
constexpr int64_t GunnerConfigFixedLayoutBytes = (int64_t) sizeof( GunnerConfigFixedLayout );
...
// twice; a plan compiled from a stranger's layout subtracts itself from it
// once, when the plan is compiled, and prefills exactly the rest.
constexpr TableFixedFill GunnerConfigFixedCover[] = {
    { 0u, 5u },
};
constexpr int32_t GunnerConfigFixedCoverCount = 1;

// THE LINEAGE, oldest first, the reader's own last (bill §6b). LOAD looks
// the file's header hash up here and never parses a stranger's layout.
constexpr uint64_t GunnerConfigFixedLineage[] = {
    0x113a539593602242ull, // identity
};
constexpr int32_t GunnerConfigFixedLineageCount = 1;
constexpr int32_t GunnerConfigFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t GunnerConfigFixedFloorLive = GunnerConfigFixedFloor;
inline void GunnerConfigFixedSetFloorForTest( int32_t index ) { GunnerConfigFixedFloorLive = index; }
#endif

constexpr uint8_t GunnerConfigFixedKnown0Layout[] = {
    0x03, 0x00, 0x00, 0x00, 0x64, 0x1b, 0x41, 0x15, 0x46, 0xe0, 0xcb, 0x5f, 0x0d, 0x05, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0x6a, 0x64, 0x01, 0x22, 0x66, 0xa3, 0x5a, 0xb7, 0x0a, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xbc, 0xb0, 0x02, 0x46, 0x9a, 0x71, 0xbf, 0xa6, 0x01, 0x01,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr TableFixedKnownLayout GunnerConfigFixedKnown[] = {
    { 0x113a539593602242ull, GunnerConfigFixedKnown0Layout, 55, 13 }, // identity
};

constexpr const TableFixedKnownRange * GunnerConfigFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t GunnerConfigFixedKnownRangeCounts[] = { 0 };

// A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
constexpr int64_t GunnerConfigFixedMeasure( int64_t count )
{
...
inline int64_t GunnerConfigFixedSave( const GunnerConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) GunnerConfigFixedBodyBytes ); // the prefill's zeros
        GunnerConfigFixedWriteBody( at + 8, values[k] );
        at += GunnerConfigFixedRecordBytes;
    }
    return need;
}
...
// different bytes is layout_malformed, a hash below the floor is
// layout_unsupported. Identity is a PLAN walked by the same loop.
inline int64_t GunnerConfigFixedLoad( GunnerConfig * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    TableReport local;
    if ( report == NULL ) { report = &local; }
    ...
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    int32_t lineage_at = -1;
    for ( int32_t i = 0; i < GunnerConfigFixedLineageCount; ++i )
    {
        if ( GunnerConfigFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = GunnerConfigFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = GunnerConfigFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = GunnerConfigFixedKnown[lineage_at];
    if ( (int64_t) layout_bytes != known.layout_bytes || memcmp( layout, known.layout, (size_t) layout_bytes ) != 0 )
    {
        report->refused = true; report->reason = layout_malformed; return -1;
    }
    const TableFixedEntry * entries = GunnerConfigFixedPlan;
    int32_t entry_count = GunnerConfigFixedPlanCount;
    int32_t entry_guarded = GunnerConfigFixedPlanGuarded;
    ...
    if ( hash != GunnerConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, GunnerConfigFixedLayout, (int32_t) GunnerConfigFixedLayoutBytes, GunnerConfigFixedDst,
                                                   GunnerConfigFixedCover, GunnerConfigFixedCoverCount,
                                                   dest, dest_capacity, &compiled_guarded, &fill_at, &fill_count, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_record_too_large : plan_too_large; return -1; }
            record_bytes = known.record_bytes;
            fill = ( fill_count > 0 ) ? (const TableFixedFill *) (const void *) ( (const uint8_t *) dest + fill_at ) : NULL;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( GunnerConfigFixedKnownRanges[lineage_at], GunnerConfigFixedKnownRangeCounts[lineage_at],
                                    (uint8_t *) &values[k], report );
        at += record_bytes;
    }
    return n;
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
constexpr int64_t TurretConfigFixedBodyBytes = 14;
constexpr int64_t TurretConfigFixedRecordBytes = 8 + TurretConfigFixedBodyBytes; // the hash and the body
constexpr uint64_t TurretConfigFixedHash = 0xd8715b0be3854d91ull; // fnv1a64 over the layout and the definitions digest (bill §13)

// THE LAYOUT (form 1 calls this the vocabulary block): 7 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
constexpr uint8_t TurretConfigFixedLayout[] = {
...
constexpr int64_t TurretConfigFixedLayoutBytes = (int64_t) sizeof( TurretConfigFixedLayout );
...
constexpr TableFixedEntry TurretConfigFixedPlan[] = {
    { 0u, 0u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 8u, 16u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 9u, 8u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
};
...
// twice; a plan compiled from a stranger's layout subtracts itself from it
// once, when the plan is compiled, and prefills exactly the rest.
constexpr TableFixedFill TurretConfigFixedCover[] = {
    { 0u, 13u },
    { 16u, 1u },
};
constexpr int32_t TurretConfigFixedCoverCount = 2;

// THE LINEAGE, oldest first, the reader's own last (bill §6b). LOAD looks
// the file's header hash up here and never parses a stranger's layout.
constexpr uint64_t TurretConfigFixedLineage[] = {
    0xd8715b0be3854d91ull, // identity
};
constexpr int32_t TurretConfigFixedLineageCount = 1;
constexpr int32_t TurretConfigFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t TurretConfigFixedFloorLive = TurretConfigFixedFloor;
inline void TurretConfigFixedSetFloorForTest( int32_t index ) { TurretConfigFixedFloorLive = index; }
#endif

constexpr uint8_t TurretConfigFixedKnown0Layout[] = {
    0x07, 0x00, 0x00, 0x00, 0x15, 0xad, 0xb2, 0x16, 0x0c, 0xba, 0x9d, 0x46, 0x0d, 0x0e, 0x00, 0x00,
    0x00, 0x03, 0x00, 0x00, 0x00, 0xc0, 0x7f, 0xb3, 0x8a, 0xbe, 0x08, 0x63, 0x7f, 0x0a, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0x3d, 0x34, 0x53, 0x69, 0xbe, 0x2c, 0xdc, 0x0a, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa, 0x44, 0xcd, 0xc0, 0x48, 0xb6, 0xdb, 0x40, 0x23,
...
constexpr TableFixedKnownLayout TurretConfigFixedKnown[] = {
    { 0xd8715b0be3854d91ull, TurretConfigFixedKnown0Layout, 123, 22 }, // identity
};

constexpr const TableFixedKnownRange * TurretConfigFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t TurretConfigFixedKnownRangeCounts[] = { 0 };

// A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
constexpr int64_t TurretConfigFixedMeasure( int64_t count )
{
...
inline int64_t TurretConfigFixedSave( const TurretConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) TurretConfigFixedBodyBytes ); // the prefill's zeros
        TurretConfigFixedWriteBody( at + 8, values[k] );
        at += TurretConfigFixedRecordBytes;
    }
    return need;
}
...
// different bytes is layout_malformed, a hash below the floor is
// layout_unsupported. Identity is a PLAN walked by the same loop.
inline int64_t TurretConfigFixedLoad( TurretConfig * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    TableReport local;
    if ( report == NULL ) { report = &local; }
    ...
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    int32_t lineage_at = -1;
    for ( int32_t i = 0; i < TurretConfigFixedLineageCount; ++i )
    {
        if ( TurretConfigFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = TurretConfigFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = TurretConfigFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = TurretConfigFixedKnown[lineage_at];
    if ( (int64_t) layout_bytes != known.layout_bytes || memcmp( layout, known.layout, (size_t) layout_bytes ) != 0 )
    {
        report->refused = true; report->reason = layout_malformed; return -1;
    }
    const TableFixedEntry * entries = TurretConfigFixedPlan;
    int32_t entry_count = TurretConfigFixedPlanCount;
    int32_t entry_guarded = TurretConfigFixedPlanGuarded;
    ...
    if ( hash != TurretConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, TurretConfigFixedLayout, (int32_t) TurretConfigFixedLayoutBytes, TurretConfigFixedDst,
                                                   TurretConfigFixedCover, TurretConfigFixedCoverCount,
                                                   dest, dest_capacity, &compiled_guarded, &fill_at, &fill_count, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_record_too_large : plan_too_large; return -1; }
            record_bytes = known.record_bytes;
            fill = ( fill_count > 0 ) ? (const TableFixedFill *) (const void *) ( (const uint8_t *) dest + fill_at ) : NULL;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( TurretConfigFixedKnownRanges[lineage_at], TurretConfigFixedKnownRangeCounts[lineage_at],
                                    (uint8_t *) &values[k], report );
        at += record_bytes;
    }
    return n;
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
constexpr int64_t HullConfigFixedBodyBytes = 50;
constexpr int64_t HullConfigFixedRecordBytes = 8 + HullConfigFixedBodyBytes; // the hash and the body
constexpr uint64_t HullConfigFixedHash = 0xc65dc3bc15d5a954ull; // fnv1a64 over the layout and the definitions digest (bill §13)

// THE LAYOUT (form 1 calls this the vocabulary block): 15 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
constexpr uint8_t HullConfigFixedLayout[] = {
...
constexpr int64_t HullConfigFixedLayoutBytes = (int64_t) sizeof( HullConfigFixedLayout );
...
constexpr TableFixedEntry HullConfigFixedPlan[] = {
    { 0u, 0u, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // health
    { 16u, 24u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 17u, 16u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 22u, 28u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 30u, 44u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 31u, 36u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 36u, 48u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 44u, 64u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 45u, 56u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
};
...
// twice; a plan compiled from a stranger's layout subtracts itself from it
// once, when the plan is compiled, and prefills exactly the rest.
constexpr TableFixedFill HullConfigFixedCover[] = {
    { 0u, 21u },
    { 24u, 1u },
    { 28u, 13u },
    { 44u, 1u },
...
constexpr int32_t HullConfigFixedCoverCount = 6;

// THE LINEAGE, oldest first, the reader's own last (bill §6b). LOAD looks
// the file's header hash up here and never parses a stranger's layout.
constexpr uint64_t HullConfigFixedLineage[] = {
    0xc65dc3bc15d5a954ull, // identity
};
constexpr int32_t HullConfigFixedLineageCount = 1;
constexpr int32_t HullConfigFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t HullConfigFixedFloorLive = HullConfigFixedFloor;
inline void HullConfigFixedSetFloorForTest( int32_t index ) { HullConfigFixedFloorLive = index; }
#endif

constexpr uint8_t HullConfigFixedKnown0Layout[] = {
    0x0f, 0x00, 0x00, 0x00, 0x90, 0x78, 0xbd, 0xdf, 0x30, 0xb1, 0x66, 0x30, 0x0d, 0x32, 0x00, 0x00,
    0x00, 0x03, 0x00, 0x00, 0x00, 0xcf, 0xa9, 0x8b, 0x28, 0xb5, 0xd4, 0x69, 0x7f, 0x0a, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb1, 0x0a, 0x7b, 0xce, 0xa2, 0x57, 0x37, 0x1f, 0x0a, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8c, 0x60, 0x83, 0xc2, 0x0b, 0x26, 0xf8, 0x84, 0x10,
...
constexpr TableFixedKnownLayout HullConfigFixedKnown[] = {
    { 0xc65dc3bc15d5a954ull, HullConfigFixedKnown0Layout, 259, 58 }, // identity
};

constexpr const TableFixedKnownRange * HullConfigFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t HullConfigFixedKnownRangeCounts[] = { 0 };

// A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
constexpr int64_t HullConfigFixedMeasure( int64_t count )
{
...
inline int64_t HullConfigFixedSave( const HullConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) HullConfigFixedBodyBytes ); // the prefill's zeros
        HullConfigFixedWriteBody( at + 8, values[k] );
        at += HullConfigFixedRecordBytes;
    }
    return need;
}
...
// different bytes is layout_malformed, a hash below the floor is
// layout_unsupported. Identity is a PLAN walked by the same loop.
inline int64_t HullConfigFixedLoad( HullConfig * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    TableReport local;
    if ( report == NULL ) { report = &local; }
    ...
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    int32_t lineage_at = -1;
    for ( int32_t i = 0; i < HullConfigFixedLineageCount; ++i )
    {
        if ( HullConfigFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = HullConfigFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = HullConfigFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = HullConfigFixedKnown[lineage_at];
    if ( (int64_t) layout_bytes != known.layout_bytes || memcmp( layout, known.layout, (size_t) layout_bytes ) != 0 )
    {
        report->refused = true; report->reason = layout_malformed; return -1;
    }
    const TableFixedEntry * entries = HullConfigFixedPlan;
    int32_t entry_count = HullConfigFixedPlanCount;
    int32_t entry_guarded = HullConfigFixedPlanGuarded;
    ...
    if ( hash != HullConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, HullConfigFixedLayout, (int32_t) HullConfigFixedLayoutBytes, HullConfigFixedDst,
                                                   HullConfigFixedCover, HullConfigFixedCoverCount,
                                                   dest, dest_capacity, &compiled_guarded, &fill_at, &fill_count, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_record_too_large : plan_too_large; return -1; }
            record_bytes = known.record_bytes;
            fill = ( fill_count > 0 ) ? (const TableFixedFill *) (const void *) ( (const uint8_t *) dest + fill_at ) : NULL;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( HullConfigFixedKnownRanges[lineage_at], HullConfigFixedKnownRangeCounts[lineage_at],
                                    (uint8_t *) &values[k], report );
        at += record_bytes;
    }
    return n;
...
constexpr TableFixedEntry KeyedConfigFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // spawn_count
    { 4u, 24u, 16u, 4u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // banner
    { 24u, 28u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // spawn_count
    { 28u, 52u, 16u, 32u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // banner
    { 48u, 56u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // spawn_count
    { 52u, 80u, 16u, 60u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // banner
    { 72u, 84u, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // health
    { 88u, 108u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 89u, 100u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 94u, 112u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 102u, 128u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 103u, 120u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 108u, 132u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 116u, 148u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 117u, 140u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 122u, 152u, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // health
    { 138u, 176u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 139u, 168u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 144u, 180u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 152u, 196u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 153u, 188u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 158u, 200u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 166u, 216u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 167u, 208u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 172u, 220u, 16u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // health
    { 188u, 244u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 189u, 236u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
    { 194u, 248u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // damage
    { 202u, 264u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // gunner present
    { 203u, 256u, 5u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // reaction
...
// twice; a plan compiled from a stranger's layout subtracts itself from it
// once, when the plan is compiled, and prefills exactly the rest.
constexpr TableFixedFill KeyedConfigFixedCover[] = {
    { 0u, 21u },
    { 24u, 25u },
    { 52u, 25u },
    { 80u, 25u },
...
constexpr uint8_t KeyedConfigFixedKnown0Layout[] = {
    0x24, 0x00, 0x00, 0x00, 0xcf, 0xee, 0x4d, 0xe9, 0xe4, 0x3a, 0x63, 0xd6, 0x0d, 0xea, 0x00, 0x00,
    0x00, 0x03, 0x00, 0x00, 0x00, 0x6d, 0xfa, 0xa8, 0xa5, 0x48, 0xb0, 0xae, 0xba, 0x10, 0x48, 0x00,
    0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x0c, 0x37, 0x20, 0x1b, 0xfb, 0xda, 0xb1, 0x24, 0x1e, 0x01,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x7c, 0x1b, 0xac, 0xfe, 0x19, 0xde, 0xf1, 0x9f, 0x20,
...
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
constexpr int64_t KeyedConfigFixedMeasure( int64_t count )
{
...
inline int64_t KeyedConfigFixedSave( const KeyedConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) KeyedConfigFixedBodyBytes ); // the prefill's zeros
        KeyedConfigFixedWriteBody( at + 8, values[k] );
        at += KeyedConfigFixedRecordBytes;
    }
    return need;
}
...
// different bytes is layout_malformed, a hash below the floor is
// layout_unsupported. Identity is a PLAN walked by the same loop.
inline int64_t KeyedConfigFixedLoad( KeyedConfig * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    TableReport local;
    if ( report == NULL ) { report = &local; }
    ...
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    int32_t lineage_at = -1;
    for ( int32_t i = 0; i < KeyedConfigFixedLineageCount; ++i )
    {
        if ( KeyedConfigFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = KeyedConfigFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = KeyedConfigFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = KeyedConfigFixedKnown[lineage_at];
    if ( (int64_t) layout_bytes != known.layout_bytes || memcmp( layout, known.layout, (size_t) layout_bytes ) != 0 )
    {
        report->refused = true; report->reason = layout_malformed; return -1;
    }
    const TableFixedEntry * entries = KeyedConfigFixedPlan;
    int32_t entry_count = KeyedConfigFixedPlanCount;
    int32_t entry_guarded = KeyedConfigFixedPlanGuarded;
    ...
    if ( hash != KeyedConfigFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, KeyedConfigFixedLayout, (int32_t) KeyedConfigFixedLayoutBytes, KeyedConfigFixedDst,
                                                   KeyedConfigFixedCover, KeyedConfigFixedCoverCount,
                                                   dest, dest_capacity, &compiled_guarded, &fill_at, &fill_count, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_record_too_large : plan_too_large; return -1; }
            record_bytes = known.record_bytes;
            fill = ( fill_count > 0 ) ? (const TableFixedFill *) (const void *) ( (const uint8_t *) dest + fill_at ) : NULL;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( KeyedConfigFixedKnownRanges[lineage_at], KeyedConfigFixedKnownRangeCounts[lineage_at],
                                    (uint8_t *) &values[k], report );
