...
{
    return 1 + 4 + WeaponConfigFixedLayoutBytes + count * WeaponConfigFixedRecordBytes;
}
...
inline int64_t WeaponConfigFixedSave( const WeaponConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) WeaponConfigFixedLayoutBytes );
    memcpy( buffer + 5, WeaponConfigFixedLayout, (size_t) WeaponConfigFixedLayoutBytes );
    uint8_t * at = buffer + 5 + WeaponConfigFixedLayoutBytes;
...
{
    return 1 + 4 + LoadoutConfigFixedLayoutBytes + count * LoadoutConfigFixedRecordBytes;
}
...
inline int64_t LoadoutConfigFixedSave( const LoadoutConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) LoadoutConfigFixedLayoutBytes );
    memcpy( buffer + 5, LoadoutConfigFixedLayout, (size_t) LoadoutConfigFixedLayoutBytes );
    uint8_t * at = buffer + 5 + LoadoutConfigFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
