...
constexpr uint64_t WeaponConfigFixedHash = 0x1b545259baa4f450ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t WeaponConfigFixedVocab[] = {
...
constexpr int64_t WeaponConfigFixedVocabBytes = (int64_t) sizeof( WeaponConfigFixedVocab );
...
{
    return 1 + 4 + WeaponConfigFixedVocabBytes + count * WeaponConfigFixedRecordBytes;
}
...
inline int64_t WeaponConfigFixedSave( const WeaponConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) WeaponConfigFixedVocabBytes );
    memcpy( buffer + 5, WeaponConfigFixedVocab, (size_t) WeaponConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + WeaponConfigFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != WeaponConfigFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, WeaponConfigFixedVocab, (int32_t) WeaponConfigFixedVocabBytes, WeaponConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t LoadoutConfigFixedHash = 0xe747768b18247870ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 43 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t LoadoutConfigFixedVocab[] = {
...
constexpr int64_t LoadoutConfigFixedVocabBytes = (int64_t) sizeof( LoadoutConfigFixedVocab );
...
{
    return 1 + 4 + LoadoutConfigFixedVocabBytes + count * LoadoutConfigFixedRecordBytes;
}
...
inline int64_t LoadoutConfigFixedSave( const LoadoutConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) LoadoutConfigFixedVocabBytes );
    memcpy( buffer + 5, LoadoutConfigFixedVocab, (size_t) LoadoutConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + LoadoutConfigFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != LoadoutConfigFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, LoadoutConfigFixedVocab, (int32_t) LoadoutConfigFixedVocabBytes, LoadoutConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
