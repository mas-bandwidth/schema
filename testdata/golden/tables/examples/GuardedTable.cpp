...
// package tabledemo — the TABLE wire's text form (SPEC-TABLES.md §16).
...
{
    return TableJsonRead( &value, PatrolTableType(), text, bytes, report );
}

int64_t PatrolToJsonMeasure( const Patrol & value )
{
    return TableJsonWrite( &value, PatrolTableType(), NULL, 0 );
}

int64_t PatrolToJson( const Patrol & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, PatrolTableType(), buffer, capacity );
}
