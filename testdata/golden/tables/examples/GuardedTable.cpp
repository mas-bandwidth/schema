...
bool PatrolFromJson( PatrolBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    Patrol * root = builder.GetRoot();
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, PatrolTableType(), text, bytes, report );
}

int64_t PatrolToJsonMeasure( const Patrol * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, PatrolTableType(), NULL, 0, allocator );
}

int64_t PatrolToJson( const Patrol * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, PatrolTableType(), buffer, capacity, allocator );
}
