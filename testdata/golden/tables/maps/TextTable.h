...
// A TEXT VALUE'S STORAGE IS A PAIR (§2.8, §7.2): `value` beside
// `value_length`, so the handle is the ENTRY and not one member of it.
// Fill `value` and `value_length`; `key` belongs to the map's own order.
            ...
            case 0x7ce4fd9430e80ceaull: // value
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        uint64_t by = 0;
                        if ( !r.get( by, 8 ) ) { report->malformed = true; return false; }
                        if ( (int32_t) i < kept ) { value.value[i] = (char) by; }
                    }
                    value.value[kept] = 0;
