// Code generated by the schema compiler from Slots.schema. DO NOT EDIT.
...
bool SlotsFromJson( SlotsBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    Slots * root = builder.GetRoot();
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, SlotsTableType(), text, bytes, report );
}

int64_t SlotsToJsonMeasure( const Slots * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SlotsTableType(), NULL, 0, allocator );
}

int64_t SlotsToJson( const Slots * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SlotsTableType(), buffer, capacity, allocator );
}
