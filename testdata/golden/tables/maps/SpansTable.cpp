...
bool SpansFromJson( SpansBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, SpansTableType(), text, bytes, report );
}

int64_t SpansToJsonMeasure( const Spans * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SpansTableType(), NULL, 0, allocator );
}

int64_t SpansToJson( const Spans * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SpansTableType(), buffer, capacity, allocator );
}
