...
bool WideRowFromJson( WideRowBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, WideRowTableType(), text, bytes, report );
}

int64_t WideRowToJsonMeasure( const WideRow * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, WideRowTableType(), NULL, 0, allocator );
}

int64_t WideRowToJson( const WideRow * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, WideRowTableType(), buffer, capacity, allocator );
}
...
bool EdgeRowFromJson( EdgeRowBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, EdgeRowTableType(), text, bytes, report );
}

int64_t EdgeRowToJsonMeasure( const EdgeRow * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, EdgeRowTableType(), NULL, 0, allocator );
}

int64_t EdgeRowToJson( const EdgeRow * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, EdgeRowTableType(), buffer, capacity, allocator );
}
