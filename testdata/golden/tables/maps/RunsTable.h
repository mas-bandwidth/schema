// Code generated by the schema compiler from Runs.schema. DO NOT EDIT.
...
// table RunsSpansEntry — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct RunsSpansEntry {
    uint16_t key = 0;
    Item value[4]; // used count beside it; count in [0, 4]
    int32_t value_count = 0;
};

// table Runs — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct Runs {
    TableMap<RunsSpansEntry> spans; // map[uint16]Item — the sorted entry array, empty until an insert
    int32_t after = 0;
};

// ---- prefill: the declared defaults, in place (docs/SPEC-TABLES.md) ----

inline void RunsSpansEntryReset( RunsSpansEntry & value );
inline void RunsReset( Runs & value );

inline void RunsSpansEntryReset( RunsSpansEntry & value )
{
    value.key = 0;
    ItemReset( value.value[0] );
    for ( int32_t i = 1; i < 4; i++ ) { value.value[i] = value.value[0]; }
    value.value_count = 0;
}

inline void RunsReset( Runs & value )
{
    value.spans.entries.value = 0; // map[uint16]Item: empty
    value.spans.count = 0;
    value.spans.padding = 0;
    value.after = 0;
}

inline int64_t RunsSpansEntryMeasureMessageBody( int64_t at, const RunsSpansEntry & value );
inline bool RunsSpansEntrySaveMessageBody( TableBitWriter & w, const RunsSpansEntry & value );
inline bool RunsSpansEntryLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, RunsSpansEntry & value );
template <typename Ctx> inline int64_t RunsMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Runs & value );
template <typename Ctx> inline bool RunsSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Runs & value );
inline bool RunsLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Runs & value );
inline bool RunsMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at );

// RunsSpansEntryMessageKeyRead: the key of one entry on the message wire, before the
...
struct RunsSpansEntryMessageKeyRead
{
    uint16_t key;
...
inline RunsSpansEntryMessageKeyRead RunsSpansEntryMessageReadKey( TableBitReader r, const TableVocabulary & vocabulary, int64_t index_bits )
{
    RunsSpansEntryMessageKeyRead out = { 0, 0, false, false, false, false, false };
    for ( ;; )
    {
        uint64_t ref = 0;
...
inline void TableReset( RunsSpansEntry & value ) { RunsSpansEntryReset( value ); }
inline void TableReset( Runs & value ) { RunsReset( value ); }
...
MAPDEMO_TABLE_INLINE bool RunsSpansEntrySaveBody( TableWriter & w, TableIds & ids, const RunsSpansEntry & value );
MAPDEMO_TABLE_INLINE bool RunsSpansEntryLoadBody( TableReader & r, RunsSpansEntry & value );
template <typename Ctx> inline int64_t RunsMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Runs & value );
template <typename Ctx> inline bool RunsSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Runs & value );
template <typename Ctx> inline bool RunsSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Runs & value );
...
template <typename Ctx> inline bool RunsNumber( const Ctx & ctx, TableNumbering & numbering, const Runs & value );
template <typename Ctx> inline int64_t RunsPackMeasure( const Ctx & ctx, TablePackMap & seen, const Runs & value );
template <typename Ctx> inline bool RunsPack( const Ctx & ctx, TablePackMap & seen, const Runs & src, Runs & dst, uint8_t * base, int64_t capacity, int64_t & used );

// ---- the numbering's bridge to each member's codec (docs/SPEC-TABLES.md §3.1) ----

template <typename Ctx> inline int64_t TableNodeMeasure( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Runs & value ) { return RunsMeasureBody( ctx, numbering, ids, value ); }
template <typename Ctx> inline bool TableNodeSave( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Runs & value ) { return RunsSaveBody( ctx, numbering, w, ids, value ); }
template <typename Ctx> inline int64_t TableNodeMessageMeasure( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Runs & value ) { return RunsMeasureMessageBody( ctx, numbering, index_bits, at, value ); }
template <typename Ctx> inline bool TableNodeMessageSave( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Runs & value ) { return RunsSaveMessageBody( ctx, numbering, index_bits, w, value ); }

// ---- RunsSpansEntry: the order, the key and the value (docs/SPEC-TABLES.md §2.8) ----
//
// The four overloads the map runtime's templates reach by argument-dependent
// lookup. Nothing outside this file names them.
static_assert( alignof( RunsSpansEntry ) <= kTableAlign, "a map entry's alignment must fit the arena's" );

inline int TableEntryOrder( const RunsSpansEntry & a, const RunsSpansEntry & b )
{
    return TableKeyOrder( (uint64_t) a.key, (uint64_t) b.key ); // integers compare by VALUE, unsigned here
}
inline int TableEntryOrder( const RunsSpansEntry & entry, uint16_t key )
{
    return TableKeyOrder( (uint64_t) entry.key, (uint64_t) key );
}
inline uint16_t TableEntryKey( const RunsSpansEntry & entry ) { return entry.key; }
inline void TableEntrySetKey( RunsSpansEntry & entry, uint16_t key ) { entry.key = key; }
// THIS VALUE'S STORAGE IS A PAIR (§2.8, §4.2, §7.2): `value` beside
// `value_count`, so the handle is the ENTRY and not one member of it.
// Fill both; `key` belongs to the map's own order.
inline const RunsSpansEntry * TableEntryFound( const RunsSpansEntry * entry ) { return entry; }
inline RunsSpansEntry * TableEntryValue( RunsSpansEntry * entry ) { return entry; }
struct RunsSpansEntryEach { uint16_t key; decltype( TableEntryValue( (RunsSpansEntry *) NULL ) ) value; };
inline RunsSpansEntryEach TableEntryEach( RunsSpansEntry * entry ) { return RunsSpansEntryEach{ TableEntryKey( *entry ), TableEntryValue( entry ) }; }
inline void TableResetMapValue( RunsSpansEntry & value )
{
    ItemReset( value.value[0] );
    for ( int32_t i = 1; i < 4; i++ ) { value.value[i] = value.value[0]; }
    value.value_count = 0;
...
// RunsSpansEntryReadKey: the key, before the slot is chosen (docs/SPEC-TABLES.md §2.8).
...
inline RunsSpansEntryKeyRead RunsSpansEntryReadKey( const uint8_t * body, int64_t length, const TableIdTable * ids )
{
    RunsSpansEntryKeyRead out = { 0, false, false, false, false, false };
...
inline int64_t RunsSpansEntryMeasureBody( TableIds & ids, const RunsSpansEntry & value )
{
    ...
    if ( value.value_count < 0 || value.value_count > 4 ) { return -1; } // storage invariant
...
MAPDEMO_TABLE_INLINE bool RunsSpansEntrySaveBody( TableWriter & w, TableIds & ids, const RunsSpansEntry & value )
{
    ...
    if ( value.value_count < 0 || value.value_count > 4 ) { return false; } // storage invariant
    if ( value.value_count > 0 )
    {
        const uint64_t ref_value = ids.ref_at( 36, 0x7ce4fd9430e80ceaull );
        int64_t body_value = 0;
        ...
        w.put8( 13 ); w.putleb( (uint64_t) ( value.value_count ) );
        for ( int32_t elem_i = 0; elem_i < value.value_count; elem_i++ )
        {
            {
...
MAPDEMO_TABLE_INLINE bool RunsSpansEntryLoadBody( TableReader & r, RunsSpansEntry & value )
{
    RunsSpansEntryReset( value ); // prefill declared defaults in place, then overlay
    for ( ;; )
    {
        uint64_t field_ref = 0;
...
inline int64_t RunsSpansEntryMeasureMessageBody( int64_t at, const RunsSpansEntry & value )
{
    ...
    if ( value.value_count > 0 )
    {
        bits += kTableMessageRefBitsHere;
        bits += 3;
        for ( int32_t i = 0; i < value.value_count; i++ )
        {
            const int64_t elem = ItemMeasureMessageBody( at + bits, value.value[i] );
            if ( elem < 0 ) { return -1; }
            bits += elem;
        }
    }
...
inline bool RunsSpansEntrySaveMessageBody( TableBitWriter & w, const RunsSpansEntry & value )
{
    ...
    if ( value.value_count > 0 )
    {
        ...
        w.put( (uint64_t) ( value.value_count ) - 0, 3 );
        for ( int32_t i = 0; i < value.value_count; i++ )
        {
            if ( !ItemSaveMessageBody( w, value.value[i] ) ) { return false; }
        }
    }
            ...
            case 0x7ce4fd9430e80ceaull: // value
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < walk; i++ )
                    {
                        ...
                        Item scratch;
                        ItemReset( scratch );
                        if ( !ItemLoadMessageBody( r, vocabulary, report, index_bits, ( in_bounds ? value.value[i] : scratch ) ) ) { return false; }
                    }
...
template <typename Ctx>
inline int64_t RunsMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Runs & value )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        // spans: a kind 14 array of kind 13 elements, ASCENDING (§2.8)
        TableMapCursor<RunsSpansEntry> order_spans = TableMapOrder( ctx, value.spans );
        if ( !order_spans.ok ) { return -1; } // the sort could not run
        if ( order_spans.count > 0 )
        {
            const uint64_t ref_spans = ids.ref_at( 23, 0x437dfc8ab2566816ull );
            int64_t body_spans = 1 + TableLebBytes( (uint64_t) order_spans.count ); // the element kind byte and the count
            for ( int32_t i = 0; i < order_spans.count; i++ )
            {
                const int64_t elem_spans = RunsSpansEntryMeasureBody( ids, *order_spans[i] );
                if ( elem_spans < 0 ) { TableMapRelease( order_spans ); return -1; }
                body_spans += TableLebBytes( (uint64_t) ( elem_spans ) ) + ( elem_spans ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_spans ) + 1 + TableLebBytes( (uint64_t) ( body_spans ) ) + ( body_spans );
        }
        TableMapRelease( order_spans );
    }
...
template <typename Ctx>
inline bool RunsSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Runs & value )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<RunsSpansEntry> order_spans = TableMapOrder( ctx, value.spans ); // spans
        if ( !order_spans.ok ) { return false; }
        if ( order_spans.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            const uint64_t ref_spans = ids.ref_at( 23, 0x437dfc8ab2566816ull );
            int64_t body_spans = 1 + TableLebBytes( (uint64_t) order_spans.count );
            for ( int32_t i = 0; i < order_spans.count; i++ )
            {
                const int64_t elem_spans = RunsSpansEntryMeasureBody( ids, *order_spans[i] );
                if ( elem_spans < 0 ) { TableMapRelease( order_spans ); return false; }
                body_spans += TableLebBytes( (uint64_t) ( elem_spans ) ) + ( elem_spans );
            }
            w.header( ref_spans, 14 ); w.putleb( (uint64_t) body_spans );
            w.put8( 13 ); w.putleb( (uint64_t) order_spans.count );
            for ( int32_t i = 0; i < order_spans.count; i++ )
            {
                const int64_t elem_len_spans = RunsSpansEntryMeasureBody( ids, *order_spans[i] );
                if ( elem_len_spans < 0 ) { TableMapRelease( order_spans ); return false; }
                w.putleb( (uint64_t) elem_len_spans );
                if ( !RunsSpansEntrySaveBody( w, ids, *order_spans[i] ) ) { TableMapRelease( order_spans ); return false; }
            }
        }
        TableMapRelease( order_spans );
    }
...
template <typename Ctx>
inline bool RunsSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Runs & value )
{
    if ( !RunsSaveBodyFields( ctx, numbering, w, ids, value ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0x437dfc8ab2566816ull: // spans
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        sub.offset += (int64_t) elem_len;
                        RunsSpansEntryKeyRead read = RunsSpansEntryReadKey( elem_body, (int64_t) elem_len, r.ids );
                        ...
                        if ( slot == NULL ) { r.report->malformed = true; break; }
                        {
                            ...
                            RunsSpansEntryLoadBody( elem, *slot );
                        }
...
// its bytes and an align costs what the position says it costs.
template <typename Ctx>
inline int64_t RunsMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Runs & value )
{
    ...
    int64_t bits = 0;
    {
        ...
        if ( order_spans.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_spans.count; i++ )
            {
                const int64_t elem_spans = RunsSpansEntryMeasureMessageBody( at + bits, *order_spans[i] );
                if ( elem_spans < 0 ) { TableMapRelease( order_spans ); return -1; }
                bits += elem_spans; // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
        }
        TableMapRelease( order_spans );
    }
...
// body is self-delimiting: it is written where the VARIABLE form put an L.
template <typename Ctx>
inline bool RunsSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Runs & value )
{
    (void) ctx; (void) numbering; (void) index_bits;
    {
        TableMapCursor<RunsSpansEntry> order_spans = TableMapOrder( ctx, value.spans ); // spans
        if ( !order_spans.ok ) { return false; } // the sort could not run
        if ( order_spans.count > 0 )
        {
            w.put( 39, kTableMessageRefBitsHere );
            w.put( (uint64_t) order_spans.count, 32 ); // the count the data decides
            for ( int32_t i = 0; i < order_spans.count; i++ )
            {
                if ( !RunsSpansEntrySaveMessageBody( w, *order_spans[i] ) ) { TableMapRelease( order_spans ); return false; }
            }
        }
        TableMapRelease( order_spans );
    }
...
// RunsMessageExtent: the extent Runs's maps command on the message wire, from
...
inline bool RunsMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at )
{
    for ( ;; )
    {
        ...
        if ( entry.id == 0x437dfc8ab2566816ull && entry.kind == 14 && entry.elem_kind == 13 ) // spans
        {
            uint64_t n = 0;
            if ( !r.get( n, TableBitsRequired( entry.min, entry.max ) ) ) { return false; }
            n += (uint64_t) entry.min;
            at = ( at + 3 ) & ~(int64_t) 3; // at alignof( RunsSpansEntry )
            at += (int64_t) n * (int64_t) sizeof( RunsSpansEntry ); // the whole array FIRST
            ...
            case 0x437dfc8ab2566816ull: // spans
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( !RunsSpansEntryLoadMessageBody( r, vocabulary, report, index_bits, *slot ) ) { return false; }
...
// RunsWireExtent: the extent Runs's lists and maps command, from the FRAMING alone.
...
inline bool RunsWireExtent( const uint8_t * body, int64_t length, int64_t & at, const TableIdTable * ids, TableRefuseReason & reason )
{
    ...
    for ( ;; )
    {
        ...
        if ( field_id == 0x437dfc8ab2566816ull && field_kind == 14 ) // spans
        {
            ...
            if ( !TableMapWireExtent( map_body, (int64_t) map_len, at, (int64_t) sizeof( RunsSpansEntry ), (int64_t) alignof( RunsSpansEntry ), NULL, ids, reason ) ) { return false; }
            continue;
        }
...
// RunsExtentAt: the node extent Runs's lists and maps take, PRE-ORDER, advancing
// the running offset exactly as RunsExtentPack advances it (§2.8, §2.9).
template <typename Ctx>
inline bool RunsExtentAt( const Ctx & ctx, const Runs & value, int64_t & at )
{
    {
        TableMapCursor<RunsSpansEntry> cursor = TableMapOrder( ctx, value.spans );
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3; // at alignof( RunsSpansEntry )
        at += (int64_t) cursor.count * (int64_t) sizeof( RunsSpansEntry ); // the whole array FIRST
...
template <typename Ctx>
inline int64_t RunsExtent( const Ctx & ctx, const Runs & value )
{
    int64_t at = 0;
    if ( !RunsExtentAt( ctx, value, at ) ) { return -1; }
    return at;
}

// RunsExtentPack: carve Runs's arrays out of the node's extent and copy the
// entries in ASCENDING key order and the elements in INDEX order, PRE-ORDER,
// advancing the same running offset RunsExtentAt advances (§2.8, §2.9).
template <typename Ctx>
inline bool RunsExtentPack( const Ctx & ctx, const Runs & src, Runs & dst, uint8_t * extent, int64_t & at, int64_t capacity )
{
    {
        TableMapCursor<RunsSpansEntry> cursor = TableMapOrder( ctx, src.spans );
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3;
        const int64_t bytes = (int64_t) cursor.count * (int64_t) sizeof( RunsSpansEntry );
        if ( at + bytes > capacity ) { TableMapRelease( cursor ); return false; }
        RunsSpansEntry * placed = (RunsSpansEntry *) ( extent + at );
        at += bytes;
        dst.spans.count = cursor.count;
        dst.spans.padding = 0;
        dst.spans.entries.value = cursor.count > 0 ? (int64_t) ( (uint8_t *) placed - (const uint8_t *) &dst.spans.entries ) : 0;
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            memcpy( (void *) ( placed + i ), (const void *) cursor[i], sizeof( RunsSpansEntry ) ); // trivially copyable, by construction
        }
        TableMapRelease( cursor );
    }
    return true;
...
// ---- Runs.spans: the builder's five and the side index (§2.8) ----
...
inline RunsSpansEntry * RunsSpansInsert( TableWorker & worker, TableMap<RunsSpansEntry> & map, uint16_t key )
{
    RunsSpansEntry * entry = TableMapPlace( worker, map, key );
...
inline RunsSpansEntry * RunsSpansFind( TableArena & arena, TableMap<RunsSpansEntry> & map, uint16_t key )
{
    RunsSpansEntry * found = TableMapScan( arena, map, key );
...
// entry's address is stable" false for exactly one case.
inline bool RunsSpansErase( TableArena & arena, TableMap<RunsSpansEntry> & map, uint16_t key )
{
    return TableMapErase( arena, map, key );
}

// EACH on the builder: INSERTION order, live entries only.
inline TableMapEach<RunsSpansEntry> RunsSpansEach( const TableArena & arena, const TableMap<RunsSpansEntry> & map )
{
...
inline int64_t RunsSpansIndexMeasure( const TableMap<RunsSpansEntry> & map )
{
    return (int64_t) TableMapIndexSlots( map.count ) * (int64_t) sizeof( int32_t );
}

inline TableMapIndex RunsSpansIndex( const TableMap<RunsSpansEntry> & map, void * storage, int64_t bytes )
{
    TableMapIndex index;
    ...
    const RunsSpansEntry * entries = map.Entries();
...
inline const RunsSpansEntry * RunsSpansIndexFind( const TableMapIndex & index, const TableMap<RunsSpansEntry> & map, uint16_t key )
{
...
// RunsNumber: number everything Runs POINTS AT, in first-visit order —
...
template <typename Ctx>
inline bool RunsNumber( const Ctx & ctx, TableNumbering & numbering, const Runs & value )
{
    (void) ctx; (void) numbering; (void) value; // no pointers below this node
    return true;
}

// RunsPackMeasure: the packed region bytes of everything Runs POINTS AT.
...
template <typename Ctx>
inline int64_t RunsPackMeasure( const Ctx & ctx, TablePackMap & seen, const Runs & value )
{
    int64_t bytes = 0;
...
// RunsPack: copy src into dst (already placed), then lay every pointee out
...
template <typename Ctx>
inline bool RunsPackEdges( const Ctx & ctx, TablePackMap & seen, const Runs & src, Runs & dst, uint8_t * base, int64_t capacity, int64_t & used );

template <typename Ctx>
inline bool RunsPack( const Ctx & ctx, TablePackMap & seen, const Runs & src, Runs & dst, uint8_t * base, int64_t capacity, int64_t & used )
{
    memcpy( (void *) &dst, (const void *) &src, sizeof( Runs ) ); // trivially copyable, by construction
    int64_t at = 0;
    uint8_t * extent = (uint8_t *) &dst + TableAlignUp64( (int64_t) sizeof( Runs ) );
    const int64_t room = capacity - ( (int64_t) ( extent - base ) );
    if ( !RunsExtentPack( ctx, src, dst, extent, at, room ) ) { return false; }
    return RunsPackEdges( ctx, seen, src, dst, base, capacity, used );
}
...
// ---- Runs: the variable-length life (docs/SPEC-TABLES.md §2, §6, §9) ----
//
// MUTABLE: RunsBuilder — allocate nodes, wire them together, then Lock.
...
// Runs is never held by value — a file-format-scale structure is a region
...
struct RunsBuilder
{
    ...
    RunsBuilder( TableAllocator allocator = TableDefaultAllocator() )
    {
    ...
    Runs * GetRoot() { return arena.locked ? NULL : (Runs *) TableArenaAt( arena, (uint32_t) root_ref.value ); }
    bool Locked() const { return arena.locked; }
    const Runs * AsConst() const { return (const Runs *) region; }
...
inline bool RunsBuilder::Lock()
{
    if ( arena.locked ) { return region != NULL; }
    if ( root_ref.null() ) { return false; }
    TableArenaCtx ctx = { &arena };
    const Runs & root = *(const Runs *) TableArenaAt( arena, (uint32_t) root_ref.value );
...
// ---- Runs on the wire: the FLAT NODE TABLE (docs/SPEC-TABLES.md §3.1) ----
//
...
// RunsNodeStorage: the region bytes one record commands, or -1 for a type id
...
inline int64_t RunsNodeStorage( uint64_t type_id, int64_t length, TableRefuseReason & reason )
{
...
// RunsNodePlace: start one record's node's lifetime in the storage pass one
...
inline void RunsNodePlace( uint64_t type_id, uint8_t * at, int64_t length )
{
    (void) length;
    (void) type_id;
    (void) at;
...
// RunsNodeRecordBytes: one record's OWN storage, before the extent its maps
// take (docs/SPEC-TABLES.md §2.8) — where a node's extent begins.
inline int64_t RunsNodeRecordBytes( uint64_t type_id )
{
    (void) type_id;
    return 0;
}

// RunsNodeAlloc: the TOOL's path — one record's node in the builder's arena.
// Zero is the arena's null, and it is also what a type id this build cannot
// name answers.
inline uint32_t RunsNodeAlloc( uint64_t type_id, TableWorker & worker, int64_t length )
{
    (void) length;
    (void) type_id;
    (void) worker;
...
// RunsNodeBody: PASS TWO's half — decode one record's body into the storage it
// already owns.
inline void RunsNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        ...
        const int64_t storage = RunsNodeStorage( type_id, r.size, reason );
        const int64_t record = storage > 0 ? RunsNodeRecordBytes( type_id ) : 0;
        carve.at = at + record;
...
// RunsNodeMessageStorage: the region bytes one record commands on the message
...
inline int64_t RunsNodeMessageStorage( uint64_t type_id, int64_t extent, int64_t length )
{
    (void) length;
    (void) extent;
    (void) type_id;
...
// RunsNodeMessageExtent: step over one TABLE record's body, tallying the extent
...
inline bool RunsNodeMessageExtent( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & extent )
{
    extent = 0;
...
// RunsNodeMessageBody: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
inline bool RunsNodeMessageBody( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at )
{
    TableExtentCarve carve;
...
template <typename Ctx>
inline bool RunsNumberFrom( const Ctx & ctx, TableNumbering & numbering, const Runs & root )
{
    bool taken = false;
    int64_t slot = 0;
    if ( TablePackMapReach( numbering.seen, (const void *) &root, (int64_t) kTableNodeIndexRoot, taken, slot ) == NULL ) { return false; }
    return RunsNumber( ctx, numbering, root );
}

template <typename Ctx>
inline int64_t RunsMeasureWire( const Ctx & ctx, const Runs & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    if ( RunsNumberFrom( ctx, numbering, root ) )
    {
        TableIds ids;
        bytes = RunsMeasureBody( ctx, numbering, ids, root );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t RunsSaveWire( const Ctx & ctx, const Runs & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    if ( !RunsNumberFrom( ctx, numbering, root ) ) { TableNumberingShutdown( numbering ); return -1; }
    ...
    bool ok = RunsSaveBodyFields( ctx, numbering, w, ids, root ) && TableNodeTableSave( ctx, w, ids, numbering );
    TableNumberingShutdown( numbering );
    ...
    return w.offset; // == RunsMeasure( root )
}

inline int64_t RunsMeasure( const Runs * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RunsMeasureWire( ctx, *root, allocator );
}

inline int64_t RunsSave( const Runs * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RunsSaveWire( ctx, *root, buffer, capacity, allocator );
}

inline int64_t RunsMeasure( const RunsBuilder & builder )
{
    if ( builder.region != NULL ) { return RunsMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return RunsMeasureWire( ctx, *(const Runs *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline int64_t RunsSave( const RunsBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return RunsSave( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return RunsSaveWire( ctx, *(const Runs *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator );
}

// ---- Runs on the MESSAGE wire: the batch over a region (docs/SPEC-TABLES.md §3.3) ----

// RunsMessageBodyBits: one root body's bits at bit position `at` of the batch,
...
template <typename Ctx>
inline int64_t RunsMessageBodyBits( const Ctx & ctx, const Runs & root, int64_t at, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bits = -1;
    if ( RunsNumberFrom( ctx, numbering, root ) )
    {
        ...
        if ( table >= 0 )
        {
            const int64_t body = RunsMeasureMessageBody( ctx, numbering, index_bits, at + table, root );
...
template <typename Ctx>
inline bool RunsMessageBodySave( const Ctx & ctx, const Runs & root, TableBitWriter & w, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    bool ok = false;
    ...
    {
        const int64_t index_bits = TableBitsRequired( 0, numbering.count + 1 );
        ok = TableMessageNodeTableSave( ctx, numbering, index_bits, w ) && RunsSaveMessageBody( ctx, numbering, index_bits, w, root );
    }
    TableNumberingShutdown( numbering );
...
inline int64_t RunsMeasureMessages( const Runs * const * roots, int64_t count, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        ...
        const int64_t body = RunsMessageBodyBits( ctx, *roots[i], bits, allocator );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8;
...
inline int64_t RunsSaveMessages( const Runs * const * roots, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( roots == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( roots[i] == NULL || !RunsMessageBodySave( ctx, *roots[i], batch.w, allocator ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == RunsMeasureMessages( roots, count, report, allocator )
}

// RunsMessageRecordScan: one node record's type id and the extent its maps
...
inline bool RunsMessageRecordScan( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, uint64_t & type_id, int64_t & extent, int64_t & length )
{
    uint64_t type_ref = 0;
    ...
    return RunsNodeMessageExtent( type_id, r, vocabulary, index_bits, extent );
}

// RunsMessageBodyStorage: one body's node count and data bytes from the FRAMING
...
// and no further (§3.3).
inline bool RunsMessageBodyStorage( TableBitReader & r, const TableVocabulary & vocabulary, int64_t & records, int64_t & data, bool & complete )
{
    complete = true;
    records = 0;
    data = 0;
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !RunsMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { return false; }
    ...
    if ( !RunsMessageExtent( r, vocabulary, index_bits, root_extent ) ) { complete = false; }
    data += TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Runs ) ) + root_extent );
    return true;
}

// RunsLoadMeasure's MESSAGE overload: the exact region bytes ONE BATCH needs,
...
inline int64_t RunsLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
    ...
    for ( int64_t b = 0; b < bodies; b++ )
    {
        ...
        bool complete = true;
        if ( !RunsMessageBodyStorage( br.r, vocabulary, records, body_data, complete ) ) { return -1; }
        data += body_data;
...
// RunsLoadMessageBodyInto: one body of a batch into the region at `used`. Its
...
inline bool RunsLoadMessageBodyInto( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Runs * & root_out )
{
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !RunsNodeMessageBody( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset ) ) { return false; }
    }
    ...
    return RunsLoadMessageBody( r, vocabulary, out, nodes, index_bits, *root );
}

// RunsLoadMessages: decode a BATCH into the caller's exact-sized region and
...
inline bool RunsLoadMessages( const Runs ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !RunsLoadMessageBodyInto( br.r, vocabulary, out, region, region_bytes, used, roots[b] ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
...
// RunsLoadMeasure: the exact region bytes a wire buffer will need, and it is
...
inline int64_t RunsLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    ...
    int64_t root_extent = 0;
    if ( !RunsWireExtent( wire, wire_bytes, root_extent, &ids_table, reason ) ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; }
    int64_t data = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Runs ) ) + root_extent );
    int64_t records = 0;
    uint64_t type_id = 0;
    ...
    {
        records++;
        int64_t storage = RunsNodeStorage( type_id, length, reason );
...
// RunsLoad: decode the tolerant wire into the caller's exact-sized region and
...
// was wrong.
inline const Runs * RunsLoad( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    ...
    const uint8_t * const wire = wire_file + 1;
    const int64_t wire_bytes = body_bytes;
    if ( region == NULL || region_bytes < (int64_t) sizeof( Runs ) ) { out->malformed = true; return NULL; }
    ...
    int64_t root_extent = 0;
    if ( !RunsWireExtent( wire, wire_bytes, root_extent, &ids_table, reason ) ) { out->malformed = true; return NULL; }
    int64_t data = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Runs ) ) + root_extent );
    int64_t records = 0;
    {
    ...
    directory[0].type_id = 0xea7bdd2b70c8c2bbull;
    Runs * root = new ( region ) Runs; // lifetime only: LoadBody's first act is RunsReset
    RunsReset( *root );
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            if ( directory[k + 1].offset != kTableNodeAbsent )
            {
                ...
                RunsNodeBody( type_id, sub, nodes, region + directory[k + 1].offset );
            }
            k++;
        }
    }
    ...
    TableExtentCarve root_carve;
    root_carve.at = region + TableAlignUp64( (int64_t) sizeof( Runs ) );
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    RunsLoadBody( r, nodes, *root );
    return root;
}

// RunsLoadBuilder: the TOOL's path — the same tolerant decode into a fresh
...
inline bool RunsLoadBuilder( RunsBuilder & builder, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    TableIdTable ids_table;
    ...
    nodes.worker = &builder.main; // and a map's entries and a list's elements are the arena's, not a node extent's (§2.8, §2.9)
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            uint32_t at = RunsNodeAlloc( type_id, builder.main, length );
            if ( at == 0 )
            {
                unknown_records++;
                directory[k + 1].offset = kTableNodeAbsent;
    ...
    bool ok = RunsLoadBody( r, nodes, *root );
...
inline int64_t RunsSpansEntryMeasureBodyRetain( TableRetainIds & ids, const RunsSpansEntry & value, TableRetain * retain, const TableRetainPath & path )
{
    ...
    if ( value.value_count > 0 )
    {
        ...
        for ( int32_t elem_i = 0; elem_i < value.value_count; elem_i++ )
        {
            const int64_t elem_bytes = ItemMeasureBodyRetain( ids, value.value[elem_i], retain, TableRetainStepInto( path, 1, (uint32_t) ( elem_i ) ) );
            if ( elem_bytes < 0 ) { return -1; }
            body_value += TableLebBytes( (uint64_t) ( elem_bytes ) ) + ( elem_bytes );
        }
        bytes += TableLebBytes( ref_value ) + 1 + TableLebBytes( (uint64_t) ( body_value ) ) + ( body_value ); // value
...
MAPDEMO_TABLE_INLINE bool RunsSpansEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const RunsSpansEntry & value, TableRetain * retain, const TableRetainPath & path )
{
    ...
    if ( value.value_count > 0 )
    {
        ...
        {
            {
                const int64_t elem_len = ItemMeasureBodyRetain( ids, value.value[elem_i], retain, TableRetainStepInto( path, 1, (uint32_t) ( elem_i ) ) );
                if ( elem_len < 0 ) return false;
                w.putleb( (uint64_t) elem_len );
                if ( !ItemSaveBodyRetain( w, ids, value.value[elem_i], retain, TableRetainStepInto( path, 1, (uint32_t) ( elem_i ) ) ) ) return false;
            }
        }
    }
...
inline int64_t RunsMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Runs & value, TableRetain * retain, const TableRetainPath & path )
{
    ...
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        if ( order_spans.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_spans.count; i++ )
            {
                const int64_t elem_spans = RunsSpansEntryMeasureBodyRetain( ids, *order_spans[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_spans < 0 ) { TableMapRelease( order_spans ); return -1; }
                body_spans += TableLebBytes( (uint64_t) ( elem_spans ) ) + ( elem_spans ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_spans ) + 1 + TableLebBytes( (uint64_t) ( body_spans ) ) + ( body_spans );
...
template <typename Ctx>
inline bool RunsSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Runs & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<RunsSpansEntry> order_spans = TableMapOrder( ctx, value.spans ); // spans
        ...
        if ( order_spans.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            for ( int32_t i = 0; i < order_spans.count; i++ )
            {
                const int64_t elem_len_spans = RunsSpansEntryMeasureBodyRetain( ids, *order_spans[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_len_spans < 0 ) { TableMapRelease( order_spans ); return false; }
                w.putleb( (uint64_t) elem_len_spans );
                if ( !RunsSpansEntrySaveBodyRetain( w, ids, *order_spans[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) ) ) { TableMapRelease( order_spans ); return false; }
            }
        }
        TableMapRelease( order_spans );
    }
...
template <typename Ctx>
inline bool RunsSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Runs & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !RunsSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0x437dfc8ab2566816ull: // spans
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            RunsSpansEntryLoadBodyRetain( elem, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) );
                        }
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
...
inline void RunsNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    // the node's own EXTENT, where its lists' and maps' arrays are carved
    // from, PRE-ORDER as the bodies decode (docs/SPEC-TABLES.md §2.8, §2.9).
    // The tool's path carries a worker instead: there the arrays are the
...
// RunsLoadRetain: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
...
inline const Runs * RunsLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                RunsNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
...
template <typename Ctx>
inline int64_t RunsMeasureWireRetain( const Ctx & ctx, const Runs & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( RunsNumberFrom( ctx, numbering, root ) )
    {
        ...
        bytes = RunsMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t RunsSaveWireRetain( const Ctx & ctx, const Runs & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    ...
    bool ok = RunsSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
...
inline int64_t RunsMeasureRetain( const Runs * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RunsMeasureWireRetain( ctx, *root, retain, allocator );
}
...
inline int64_t RunsSaveRetain( const Runs * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RunsSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// RunsSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
...
//
// A REFERENCE INSIDE THE REGION IS DEREFERENCED THROUGH RunsAt: the slot holds
...
// parameter on a load.
inline const Runs * RunsOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Runs *) TableCookOpen( bytes, length, (uint64_t) sizeof( Runs ), (uint64_t) alignof( Runs ), reason );
}
...
inline void RunsSpansEntryCookBody( uint8_t * at, const RunsSpansEntry & value, TableByteOrder order );
template <typename Ctx> inline bool RunsCookBody( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Runs & value, TableByteOrder order );
...
inline void RunsSpansEntryCookBody( uint8_t * at, const RunsSpansEntry & value, TableByteOrder order )
{
    ...
    {
        ItemCookBody( at + 4 + i * 4, value.value[ i ], order );
    }
    table_cook_put( at + 20, (uint64_t) (uint32_t) value.value_count, 4, order );
}
...
template <typename Ctx> inline bool RunsCookBody( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Runs & value, TableByteOrder order )
{
    ...
    table_cook_put( at + 0, 0, 8, order ); // spans: the array's delta, filled by the extent writer
    table_cook_put( at + 8, 0, 4, order ); // and its count
    table_cook_put( at + 16, (uint64_t) value.after, 4, order );
    return true;
}

template <typename Ctx> inline bool RunsSpansEntryCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const RunsSpansEntry & value, TableByteOrder order );
template <typename Ctx> inline bool RunsCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const Runs & value, TableByteOrder order );

// RunsSpansEntryCookExtent: RunsSpansEntry's arrays into the node's extent, PRE-ORDER, a map's entries
...
// RunsCookExtent: Runs's arrays into the node's extent, PRE-ORDER, a map's entries
// in ASCENDING key order and a list's elements in INDEX order, each through its
// own cook writer (§2.8, §2.9, §7.6).
template <typename Ctx> inline bool RunsCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const Runs & value, TableByteOrder order )
{
    ...
    { // spans
        ...
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            RunsSpansEntryCookBody( array + i * 24, *cursor[i], order );
        }
        TableMapRelease( cursor );
    }
    return true;
...
// RunsSpansEntryCookNode: one node, the record, then the extent its lists and maps take (§2.8, §2.9).
template <typename Ctx> inline bool RunsSpansEntryCookNode( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const RunsSpansEntry & value, TableByteOrder order )
{
    RunsSpansEntryCookBody( at, value, order );
    int64_t extent_at = 0;
    return RunsSpansEntryCookExtent( ctx, region, at + 24, extent_at, at, value, order );
}

// RunsCookNode: one node, the record, then the extent its lists and maps take (§2.8, §2.9).
template <typename Ctx> inline bool RunsCookNode( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Runs & value, TableByteOrder order )
{
    if ( !RunsCookBody( ctx, region, at, value, order ) ) { return false; }
    int64_t extent_at = 0;
    if ( !RunsCookExtent( ctx, region, at + 24, extent_at, at, value, order ) ) { return false; }
    return extent_at == RunsExtent( ctx, value ); // the extent written is the extent measured, or no header is written
}

// RunsCookLayout: the tool's own Layout (docs/SPEC-TABLES.md §7.2) over one
...
template <typename Ctx>
inline bool RunsCookLayout( const Ctx & ctx, const Runs & root, const TableNumbering & numbering, TableCookRegion & region )
{
    region.numbering = &numbering;
    region.count = numbering.count + 1;
    const int64_t root_extent = RunsExtent( ctx, root );
...
// RunsCookMeasureFrom: the whole cooked file's bytes for one graph — the header,
...
template <typename Ctx>
inline int64_t RunsCookMeasureFrom( const Ctx & ctx, const Runs & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    TableCookRegion region;
    int64_t bytes = -1;
    if ( RunsNumberFrom( ctx, numbering, root ) && RunsCookLayout( ctx, root, numbering, region ) )
    {
...
// RunsCookFrom: write one cooked file of a pointered graph, in the byte order
...
template <typename Ctx>
inline bool RunsCookFrom( const Ctx & ctx, const Runs & root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator )
{
    ...
    if ( ok )
    {
        ...
        if ( ok )
        {
            ...
            ok = RunsCookNode( ctx, region, region.base, root, order );
            ...
            table_cook_put( entry + 8, 0xea7bdd2b70c8c2bbull, 8, order ); // the root: fnv1a64( "Runs" )
            for ( int64_t k = 0; k < numbering.count; k++ )
            {
...
// RunsCookMeasure / RunsCook over a REGION root — a locked builder's AsConst, a
// region RunsLoad produced, or an opened cook — with the pair the numbering
// allocates through as an optional last argument, as the wire's own entries
// take it (§13.9).
inline int64_t RunsCookMeasure( const Runs * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RunsCookMeasureFrom( ctx, *root, allocator );
}

inline bool RunsCook( const Runs * root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return false; }
    TableRegionCtx ctx;
    return RunsCookFrom( ctx, *root, out, capacity, order, allocator );
}

// and over a BUILDER, locked or not: the builder's own pair, and the arena
// encoding while it is still mutable (§6.3).
inline int64_t RunsCookMeasure( const RunsBuilder & builder )
{
    if ( builder.region != NULL ) { return RunsCookMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return RunsCookMeasureFrom( ctx, *(const Runs *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline bool RunsCook( const RunsBuilder & builder, void * out, uint64_t capacity, TableByteOrder order )
{
    if ( builder.region != NULL ) { return RunsCook( builder.AsConst(), out, capacity, order, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return false; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return RunsCookFrom( ctx, *(const Runs *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), out, capacity, order, builder.arena.allocator );
}
...
static_assert( __is_trivially_copyable( RunsSpansEntry ), "RunsSpansEntry must stay relocatable" );
static_assert( __is_standard_layout( RunsSpansEntry ), "RunsSpansEntry must stay standard-layout for offsetof" );
static_assert( __is_trivially_copyable( Runs ), "Runs must stay relocatable" );
static_assert( __is_standard_layout( Runs ), "Runs must stay standard-layout for offsetof" );
...
// rather than assumed.
static_assert( sizeof( RunsSpansEntry ) == 24, "RunsSpansEntry's sizeof moved: the build version was taken over 24, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( RunsSpansEntry ) == 4, "RunsSpansEntry's alignof moved: the build version was taken over 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( RunsSpansEntry, key ) == 0, "RunsSpansEntry's field key moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( RunsSpansEntry, value ) == 4, "RunsSpansEntry's field value moved: the build version was taken over offset 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( sizeof( Runs ) == 24, "Runs's sizeof moved: the build version was taken over 24, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( Runs ) == 8, "Runs's alignof moved: the build version was taken over 8 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Runs, spans ) == 0, "Runs's field spans moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Runs, after ) == 16, "Runs's field after moved: the build version was taken over offset 16 (docs/SPEC-TABLES.md §20.3)" );


// ---- reflection descriptors (tables only, docs/SPEC-TABLES.md) ----

inline const TableTypeInfo * RunsSpansEntryTableType();
inline const TableTypeInfo * RunsTableType();
...
extern const TableTypeInfo RunsSpansEntryTableInfo;
extern const TableTypeInfo RunsTableInfo;

inline const TableFieldInfo RunsSpansEntryTableFields[] = {
    { "key", "key", "uint16", 0x3dc94a19365b10ecull, 7, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( RunsSpansEntry, key ), (uint32_t) sizeof( RunsSpansEntry::key ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
    { "value", "value", "Item", 0x7ce4fd9430e80ceaull, 13, true, false, NULL, NULL, true, false, 4, (uint32_t) offsetof( RunsSpansEntry, value ), (uint32_t) sizeof( RunsSpansEntry::value[0] ), (uint32_t) offsetof( RunsSpansEntry, value_count ), 0xffffffffu, &ItemTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
};
inline const TableTypeInfo RunsSpansEntryTableInfo = { "RunsSpansEntry", (uint32_t) sizeof( RunsSpansEntry ), 2, RunsSpansEntryTableFields, +[]( void * p ) { RunsSpansEntryReset( *(RunsSpansEntry *) p ); }, false, TableDocNone, 0, NULL };
inline const TableTypeInfo * RunsSpansEntryTableType() { return &RunsSpansEntryTableInfo; }

inline const TableFieldInfo RunsTableFields[] = {
    { "spans", "spans", "map[uint16]Item", 0x437dfc8ab2566816ull, 13, true, false, NULL, NULL, true, false, 0, (uint32_t) offsetof( Runs, spans ), (uint32_t) sizeof( RunsSpansEntry ), (uint32_t) offsetof( Runs, spans.count ), 0xffffffffu, &RunsSpansEntryTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, []( TableWorker & worker, void * slot, const char *, int32_t, int64_t key_value ) -> void * { return (void *) TableMapPlace( worker, *(TableMap<RunsSpansEntry> *) slot, (uint16_t) key_value ); }, "", TableDocNone, 0, NULL },
    { "after", "after", "int32", 0xbf82010f6f71eae9ull, 4, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Runs, after ), (uint32_t) sizeof( Runs::after ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
};
inline const TableTypeInfo RunsTableInfo = { "Runs", (uint32_t) sizeof( Runs ), 2, RunsTableFields, +[]( void * p ) { RunsReset( *(Runs *) p ); }, true, TableDocNone, 0, NULL };
inline const TableTypeInfo * RunsTableType() { return &RunsTableInfo; }

// ---- the text form (docs/SPEC-TABLES.md §16) ----

// Runs in and out of a JSON text (docs/SPEC-TABLES.md §16.7): read into a
// builder, written from a region's const root. A node named more than once
// carries `&node` in the text. Defined in RunsTable.cpp; link it to use them.
bool RunsFromJson( RunsBuilder & builder, const char * text, int64_t bytes, TableReport * report );
int64_t RunsToJsonMeasure( const Runs * root, TableAllocator allocator = TableDefaultAllocator() );
int64_t RunsToJson( const Runs * root, char * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() );
