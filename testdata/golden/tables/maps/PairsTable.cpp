...
bool PairsFromJson( PairsBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, PairsTableType(), text, bytes, report );
}

int64_t PairsToJsonMeasure( const Pairs * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, PairsTableType(), NULL, 0, allocator );
}

int64_t PairsToJson( const Pairs * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, PairsTableType(), buffer, capacity, allocator );
}
