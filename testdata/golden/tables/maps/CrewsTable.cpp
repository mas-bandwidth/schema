// Code generated by the schema compiler from Crews.schema. DO NOT EDIT.
...
bool CrewsFromJson( CrewsBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    Crews * root = builder.GetRoot();
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, CrewsTableType(), text, bytes, report );
}

int64_t CrewsToJsonMeasure( const Crews * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, CrewsTableType(), NULL, 0, allocator );
}

int64_t CrewsToJson( const Crews * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, CrewsTableType(), buffer, capacity, allocator );
}
