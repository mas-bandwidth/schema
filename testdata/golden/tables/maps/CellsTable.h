...
// table CellsRowsEntry — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct CellsRowsEntry {
    char key[8 + 1] = {}; // string(8): max length, used length beside it
    int32_t key_length = 0;
    int32_t value[3] = {};
};

// table Cells — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct Cells {
    TableMap<CellsRowsEntry> rows; // map[string(8)]int32 — the sorted entry array, empty until an insert
    int32_t after = 0;
};

// ---- prefill: the declared defaults, in place (docs/SPEC-TABLES.md) ----

inline void CellsRowsEntryReset( CellsRowsEntry & value );
inline void CellsReset( Cells & value );
...
inline void CellsReset( Cells & value )
{
    value.rows.entries.value = 0; // map[string(8)]int32: empty
    value.rows.count = 0;
    value.rows.padding = 0;
    value.after = 0;
}

inline int64_t CellsRowsEntryMeasureMessageBody( int64_t at, const CellsRowsEntry & value );
inline bool CellsRowsEntrySaveMessageBody( TableBitWriter & w, const CellsRowsEntry & value );
inline bool CellsRowsEntryLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, CellsRowsEntry & value );
template <typename Ctx> inline int64_t CellsMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Cells & value );
template <typename Ctx> inline bool CellsSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Cells & value );
inline bool CellsLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Cells & value );
inline bool CellsMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at );

// CellsRowsEntryMessageKeyRead: the key of one entry on the message wire, before the
...
struct CellsRowsEntryMessageKeyRead
{
...
inline CellsRowsEntryMessageKeyRead CellsRowsEntryMessageReadKey( TableBitReader r, const TableVocabulary & vocabulary, int64_t index_bits )
{
    CellsRowsEntryMessageKeyRead out = { NULL, 0, 0, false, false, false, false, false };
    for ( ;; )
    {
        uint64_t ref = 0;
...
inline void TableReset( CellsRowsEntry & value ) { CellsRowsEntryReset( value ); }
inline void TableReset( Cells & value ) { CellsReset( value ); }
...
inline int64_t CellsRowsEntryMeasureBody( TableIds & ids, const CellsRowsEntry & value );
MAPDEMO_TABLE_INLINE bool CellsRowsEntrySaveBody( TableWriter & w, TableIds & ids, const CellsRowsEntry & value );
MAPDEMO_TABLE_INLINE bool CellsRowsEntryLoadBody( TableReader & r, CellsRowsEntry & value );
template <typename Ctx> inline int64_t CellsMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Cells & value );
template <typename Ctx> inline bool CellsSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Cells & value );
template <typename Ctx> inline bool CellsSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Cells & value );
...
template <typename Ctx> inline bool CellsNumber( const Ctx & ctx, TableNumbering & numbering, const Cells & value );
template <typename Ctx> inline int64_t CellsPackMeasure( const Ctx & ctx, TablePackMap & seen, const Cells & value );
template <typename Ctx> inline bool CellsPack( const Ctx & ctx, TablePackMap & seen, const Cells & src, Cells & dst, uint8_t * base, int64_t capacity, int64_t & used );

// ---- the numbering's bridge to each member's codec (docs/SPEC-TABLES.md §3.1) ----

template <typename Ctx> inline int64_t TableNodeMeasure( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Cells & value ) { return CellsMeasureBody( ctx, numbering, ids, value ); }
template <typename Ctx> inline bool TableNodeSave( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Cells & value ) { return CellsSaveBody( ctx, numbering, w, ids, value ); }
template <typename Ctx> inline int64_t TableNodeMessageMeasure( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Cells & value ) { return CellsMeasureMessageBody( ctx, numbering, index_bits, at, value ); }
template <typename Ctx> inline bool TableNodeMessageSave( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Cells & value ) { return CellsSaveMessageBody( ctx, numbering, index_bits, w, value ); }

// ---- CellsRowsEntry: the order, the key and the value (docs/SPEC-TABLES.md §2.8) ----
//
// The four overloads the map runtime's templates reach by argument-dependent
// lookup. Nothing outside this file names them.
static_assert( alignof( CellsRowsEntry ) <= kTableAlign, "a map entry's alignment must fit the arena's" );
static const int32_t kCellsRowsEntryKeyBound = 8; // string(8): the key's storage, and the length an insert refuses past

inline int TableEntryOrder( const CellsRowsEntry & a, const CellsRowsEntry & b )
{
...
inline int TableEntryOrder( const CellsRowsEntry & entry, TableMapKeyRef key )
{
    return TableKeyOrder( entry.key, entry.key_length, key.data, key.length );
}
inline int TableEntryOrder( const CellsRowsEntry & entry, const char * key )
{
    return TableEntryOrder( entry, TableMapKeyRef{ key, TableKeyLength( key, kCellsRowsEntryKeyBound ) } );
}
inline const char * TableEntryKey( const CellsRowsEntry & entry ) { return entry.key; } // NUL-terminated, the storage's own bytes
inline void TableEntrySetKey( CellsRowsEntry & entry, TableMapKeyRef key )
{
...
typedef int32_t CellsRowsEntryValue[3];
inline const CellsRowsEntryValue * TableEntryFound( const CellsRowsEntry * entry ) { return entry != NULL ? &entry->value : NULL; }
inline CellsRowsEntryValue * TableEntryValue( CellsRowsEntry * entry ) { return &entry->value; }
struct CellsRowsEntryEach { const char * key; decltype( TableEntryValue( (CellsRowsEntry *) NULL ) ) value; };
inline CellsRowsEntryEach TableEntryEach( CellsRowsEntry * entry ) { return CellsRowsEntryEach{ TableEntryKey( *entry ), TableEntryValue( entry ) }; }
inline void TableResetMapValue( CellsRowsEntry & value )
{
    memset( value.value, 0, sizeof( value.value ) );
}

// CellsRowsEntryReadKey: the key, before the slot is chosen (docs/SPEC-TABLES.md §2.8).
...
inline CellsRowsEntryKeyRead CellsRowsEntryReadKey( const uint8_t * body, int64_t length, const TableIdTable * ids )
{
    CellsRowsEntryKeyRead out = { NULL, 0, false, false, false, false, false };
...
inline int64_t CellsRowsEntryMeasureBodyRetain( TableRetainIds & ids, const CellsRowsEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool CellsRowsEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const CellsRowsEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool CellsRowsEntryLoadBodyRetain( TableReader & r, CellsRowsEntry & value, TableRetain * retain, const TableRetainPath & path );
inline bool CellsRowsEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, CellsRowsEntry & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline int64_t CellsMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Cells & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool CellsSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Cells & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool CellsSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Cells & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool CellsLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Cells & value, TableRetain * retain, const TableRetainPath & path );
...
inline int64_t CellsRowsEntryMeasureBody( TableIds & ids, const CellsRowsEntry & value )
{
    ...
    if ( value.key_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 21, 0x3dc94a19365b10ecull ) ) + 1 + TableLebBytes( (uint64_t) ( value.key_length ) ) + ( value.key_length ); } // key
    {
        ...
        for ( int32_t i = 0; i < 3; i++ ) { if ( value.value[i] != 0 ) { all_default_value = false; break; } }
        if ( !all_default_value )
        {
            const uint64_t ref_value = ids.ref_at( 36, 0x7ce4fd9430e80ceaull );
            int64_t body_value = 0;
            body_value += 1 + TableLebBytes( (uint64_t) ( 3 ) ); // the element kind byte and the count
            body_value += (int64_t) ( 3 ) * 4;
...
MAPDEMO_TABLE_INLINE bool CellsRowsEntrySaveBody( TableWriter & w, TableIds & ids, const CellsRowsEntry & value )
{
    ...
    }
    {
        ...
        if ( !all_default_value )
        {
            ...
            w.put8( 4 ); w.putleb( (uint64_t) ( 3 ) );
            for ( int32_t elem_i = 0; elem_i < 3; elem_i++ )
            {
                w.put32( uint32_t( value.value[elem_i] ) );
            }
        }
    }
...
MAPDEMO_TABLE_INLINE bool CellsRowsEntryLoadBody( TableReader & r, CellsRowsEntry & value )
{
    CellsRowsEntryReset( value ); // prefill declared defaults in place, then overlay
    for ( ;; )
    {
        uint64_t field_ref = 0;
                ...
                if ( body_len >= 2 )
                {
                    ...
                    else if ( elem_kind != 4 )
                    {
                        ...
                        for ( uint64_t widened_i = 0; widened_i < widened_keep; widened_i++ )
                        {
                            ...
                            value.value[(int32_t) widened_i] = decoded_v;
...
inline int64_t CellsRowsEntryMeasureMessageBody( int64_t at, const CellsRowsEntry & value )
{
    ...
    }
    {
        ...
        for ( int32_t i = 0; i < 3; i++ ) { if ( value.value[i] != 0 ) { rides_value = true; break; } }
        if ( rides_value )
        {
            bits += kTableMessageRefBitsHere;
            bits += (int64_t) ( 3 ) * 32;
...
inline bool CellsRowsEntrySaveMessageBody( TableBitWriter & w, const CellsRowsEntry & value )
{
    ...
    }
    {
        ...
        if ( rides_value )
        {
            ...
            {
                w.put( (uint64_t) ( value.value[i] ), 32 );
            }
        }
    }
                    ...
                    {
                        {
                            ...
                            for ( uint64_t i = 0; i < walk; i++ )
                            {
                                ...
                                if ( in_bounds ) { value.value[i] = scratch; }
                            }
...
template <typename Ctx>
inline int64_t CellsMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Cells & value )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        // rows: a kind 14 array of kind 13 elements, ASCENDING (§2.8)
        TableMapCursor<CellsRowsEntry> order_rows = TableMapOrder( ctx, value.rows );
        if ( !order_rows.ok ) { return -1; } // the sort could not run
        if ( order_rows.count > 0 )
        {
            const uint64_t ref_rows = ids.ref_at( 47, 0xa3a7061ff10a8138ull );
            int64_t body_rows = 1 + TableLebBytes( (uint64_t) order_rows.count ); // the element kind byte and the count
            for ( int32_t i = 0; i < order_rows.count; i++ )
            {
                const int64_t elem_rows = CellsRowsEntryMeasureBody( ids, *order_rows[i] );
                if ( elem_rows < 0 ) { TableMapRelease( order_rows ); return -1; }
                body_rows += TableLebBytes( (uint64_t) ( elem_rows ) ) + ( elem_rows ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_rows ) + 1 + TableLebBytes( (uint64_t) ( body_rows ) ) + ( body_rows );
        }
        TableMapRelease( order_rows );
    }
...
template <typename Ctx>
inline bool CellsSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Cells & value )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<CellsRowsEntry> order_rows = TableMapOrder( ctx, value.rows ); // rows
        if ( !order_rows.ok ) { return false; }
        if ( order_rows.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            const uint64_t ref_rows = ids.ref_at( 47, 0xa3a7061ff10a8138ull );
            int64_t body_rows = 1 + TableLebBytes( (uint64_t) order_rows.count );
            for ( int32_t i = 0; i < order_rows.count; i++ )
            {
                const int64_t elem_rows = CellsRowsEntryMeasureBody( ids, *order_rows[i] );
                if ( elem_rows < 0 ) { TableMapRelease( order_rows ); return false; }
                body_rows += TableLebBytes( (uint64_t) ( elem_rows ) ) + ( elem_rows );
            }
            w.putleb( ref_rows ); w.put8( 14 ); w.putleb( (uint64_t) body_rows );
            w.put8( 13 ); w.putleb( (uint64_t) order_rows.count );
            for ( int32_t i = 0; i < order_rows.count; i++ )
            {
                const int64_t elem_len_rows = CellsRowsEntryMeasureBody( ids, *order_rows[i] );
                if ( elem_len_rows < 0 ) { TableMapRelease( order_rows ); return false; }
                w.putleb( (uint64_t) elem_len_rows );
                if ( !CellsRowsEntrySaveBody( w, ids, *order_rows[i] ) ) { TableMapRelease( order_rows ); return false; }
            }
        }
        TableMapRelease( order_rows );
    }
...
template <typename Ctx>
inline bool CellsSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Cells & value )
{
    if ( !CellsSaveBodyFields( ctx, numbering, w, ids, value ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0xa3a7061ff10a8138ull: // rows
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        CellsRowsEntryKeyRead read = CellsRowsEntryReadKey( elem_body, (int64_t) elem_len, r.ids );
                        ...
                        if ( slot == NULL ) { r.report->malformed = true; break; }
                        {
                            ...
                            CellsRowsEntryLoadBody( elem, *slot );
                        }
...
// its bytes and an align costs what the position says it costs.
template <typename Ctx>
inline int64_t CellsMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Cells & value )
{
    ...
    int64_t bits = 0;
    {
        ...
        if ( order_rows.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_rows.count; i++ )
            {
                const int64_t elem_rows = CellsRowsEntryMeasureMessageBody( at + bits, *order_rows[i] );
                if ( elem_rows < 0 ) { TableMapRelease( order_rows ); return -1; }
                bits += elem_rows; // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
        }
        TableMapRelease( order_rows );
    }
...
// body is self-delimiting: it is written where the file form put an L.
template <typename Ctx>
inline bool CellsSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Cells & value )
{
    (void) ctx; (void) numbering; (void) index_bits;
    {
        TableMapCursor<CellsRowsEntry> order_rows = TableMapOrder( ctx, value.rows ); // rows
        if ( !order_rows.ok ) { return false; } // the sort could not run
        if ( order_rows.count > 0 )
        {
            w.put( 17, kTableMessageRefBitsHere );
            w.put( (uint64_t) order_rows.count, 32 ); // the count the data decides
            for ( int32_t i = 0; i < order_rows.count; i++ )
            {
                if ( !CellsRowsEntrySaveMessageBody( w, *order_rows[i] ) ) { TableMapRelease( order_rows ); return false; }
            }
        }
        TableMapRelease( order_rows );
    }
...
// CellsMessageExtent: the extent Cells's maps command on the message wire, from
...
inline bool CellsMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at )
{
    for ( ;; )
    {
        ...
        if ( entry.id == 0xa3a7061ff10a8138ull && entry.kind == 14 && entry.elem_kind == 13 ) // rows
        {
            uint64_t n = 0;
            if ( !r.get( n, TableBitsRequired( entry.min, entry.max ) ) ) { return false; }
            n += (uint64_t) entry.min;
            at = ( at + 3 ) & ~(int64_t) 3; // at alignof( CellsRowsEntry )
            at += (int64_t) n * (int64_t) sizeof( CellsRowsEntry ); // the whole array FIRST
        ...
        switch ( entry.id )
        {
            case 0xa3a7061ff10a8138ull: // rows
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        const CellsRowsEntryMessageKeyRead read = CellsRowsEntryMessageReadKey( r, vocabulary, index_bits );
                        ...
                        if ( !CellsRowsEntryLoadMessageBody( r, vocabulary, report, index_bits, *slot ) ) { return false; }
...
// CellsWireExtent: the extent Cells's lists and maps command, from the FRAMING alone.
...
inline bool CellsWireExtent( const uint8_t * body, int64_t length, int64_t & at, const TableIdTable * ids, TableRefuseReason & reason )
{
    ...
    for ( ;; )
    {
        ...
        if ( field_id == 0xa3a7061ff10a8138ull && field_kind == 14 ) // rows
        {
            ...
            if ( !TableMapWireExtent( map_body, (int64_t) map_len, at, (int64_t) sizeof( CellsRowsEntry ), (int64_t) alignof( CellsRowsEntry ), NULL, ids, reason ) ) { return false; }
            continue;
        }
...
// CellsExtentAt: the node extent Cells's lists and maps take, PRE-ORDER, advancing
// the running offset exactly as CellsExtentPack advances it (§2.8, §2.9).
template <typename Ctx>
inline bool CellsExtentAt( const Ctx & ctx, const Cells & value, int64_t & at )
{
    {
        TableMapCursor<CellsRowsEntry> cursor = TableMapOrder( ctx, value.rows );
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3; // at alignof( CellsRowsEntry )
        at += (int64_t) cursor.count * (int64_t) sizeof( CellsRowsEntry ); // the whole array FIRST
...
template <typename Ctx>
inline int64_t CellsExtent( const Ctx & ctx, const Cells & value )
{
    int64_t at = 0;
    if ( !CellsExtentAt( ctx, value, at ) ) { return -1; }
    return at;
}

// CellsExtentPack: carve Cells's arrays out of the node's extent and copy the
// entries in ASCENDING key order and the elements in INDEX order, PRE-ORDER,
// advancing the same running offset CellsExtentAt advances (§2.8, §2.9).
template <typename Ctx>
inline bool CellsExtentPack( const Ctx & ctx, const Cells & src, Cells & dst, uint8_t * extent, int64_t & at, int64_t capacity )
{
    {
        TableMapCursor<CellsRowsEntry> cursor = TableMapOrder( ctx, src.rows );
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3;
        const int64_t bytes = (int64_t) cursor.count * (int64_t) sizeof( CellsRowsEntry );
        if ( at + bytes > capacity ) { TableMapRelease( cursor ); return false; }
        CellsRowsEntry * placed = (CellsRowsEntry *) ( extent + at );
        at += bytes;
        dst.rows.count = cursor.count;
        dst.rows.padding = 0;
        dst.rows.entries.value = cursor.count > 0 ? (int64_t) ( (uint8_t *) placed - (const uint8_t *) &dst.rows.entries ) : 0;
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            memcpy( (void *) ( placed + i ), (const void *) cursor[i], sizeof( CellsRowsEntry ) ); // trivially copyable, by construction
        }
        TableMapRelease( cursor );
    }
    return true;
...
// ---- Cells.rows: the builder's five and the side index (§2.8) ----
...
inline CellsRowsEntryValue * CellsRowsInsert( TableWorker & worker, TableMap<CellsRowsEntry> & map, const char * key )
{
    if ( key == NULL ) { return NULL; }
    const TableMapKeyRef bytes = { key, TableKeyLength( key, kCellsRowsEntryKeyBound ) };
    if ( bytes.length > kCellsRowsEntryKeyBound ) { return NULL; } // KEYS NEVER CLAMP
    CellsRowsEntry * entry = TableMapPlace( worker, map, bytes );
...
inline CellsRowsEntryValue * CellsRowsFind( TableArena & arena, TableMap<CellsRowsEntry> & map, const char * key )
{
    CellsRowsEntry * found = TableMapScan( arena, map, key );
...
// entry's address is stable" false for exactly one case.
inline bool CellsRowsErase( TableArena & arena, TableMap<CellsRowsEntry> & map, const char * key )
{
    return TableMapErase( arena, map, key );
}

// EACH on the builder: INSERTION order, live entries only.
inline TableMapEach<CellsRowsEntry> CellsRowsEach( const TableArena & arena, const TableMap<CellsRowsEntry> & map )
{
...
inline int64_t CellsRowsIndexMeasure( const TableMap<CellsRowsEntry> & map )
{
    return (int64_t) TableMapIndexSlots( map.count ) * (int64_t) sizeof( int32_t );
}

inline TableMapIndex CellsRowsIndex( const TableMap<CellsRowsEntry> & map, void * storage, int64_t bytes )
{
    TableMapIndex index;
    ...
    const CellsRowsEntry * entries = map.Entries();
...
inline const CellsRowsEntryValue * CellsRowsIndexFind( const TableMapIndex & index, const TableMap<CellsRowsEntry> & map, const char * key )
{
    if ( !index.good ) { return map.Find( key ); } // an index that did not build is not a wrong answer
    const CellsRowsEntry * entries = map.Entries();
    int32_t at = (int32_t) ( TableMapHash( (const void *) key, TableKeyLength( key, kCellsRowsEntryKeyBound ) ) & (uint64_t) ( index.capacity - 1 ) );
...
// CellsNumber: number everything Cells POINTS AT, in first-visit order —
...
template <typename Ctx>
inline bool CellsNumber( const Ctx & ctx, TableNumbering & numbering, const Cells & value )
{
    (void) ctx; (void) numbering; (void) value; // no pointers below this node
    return true;
}

// CellsPackMeasure: the packed region bytes of everything Cells POINTS AT.
...
template <typename Ctx>
inline int64_t CellsPackMeasure( const Ctx & ctx, TablePackMap & seen, const Cells & value )
{
    int64_t bytes = 0;
...
// CellsPack: copy src into dst (already placed), then lay every pointee out
...
template <typename Ctx>
inline bool CellsPackEdges( const Ctx & ctx, TablePackMap & seen, const Cells & src, Cells & dst, uint8_t * base, int64_t capacity, int64_t & used );

template <typename Ctx>
inline bool CellsPack( const Ctx & ctx, TablePackMap & seen, const Cells & src, Cells & dst, uint8_t * base, int64_t capacity, int64_t & used )
{
    memcpy( (void *) &dst, (const void *) &src, sizeof( Cells ) ); // trivially copyable, by construction
    int64_t at = 0;
    uint8_t * extent = (uint8_t *) &dst + TableAlignUp64( (int64_t) sizeof( Cells ) );
    const int64_t room = capacity - ( (int64_t) ( extent - base ) );
    if ( !CellsExtentPack( ctx, src, dst, extent, at, room ) ) { return false; }
    return CellsPackEdges( ctx, seen, src, dst, base, capacity, used );
}
...
// ---- Cells: the variable-length life (docs/SPEC-TABLES.md §2, §6, §9) ----
//
// MUTABLE: CellsBuilder — allocate nodes, wire them together, then Lock.
...
// Cells is never held by value — a file-format-scale structure is a region
...
struct CellsBuilder
{
    ...
    CellsBuilder( TableAllocator allocator = TableDefaultAllocator() )
    {
    ...
    Cells * GetRoot() { return arena.locked ? NULL : (Cells *) TableArenaAt( arena, (uint32_t) root_ref.value ); }
    bool Locked() const { return arena.locked; }
    const Cells * AsConst() const { return (const Cells *) region; }
...
inline bool CellsBuilder::Lock()
{
    if ( arena.locked ) { return region != NULL; }
    if ( root_ref.null() ) { return false; }
    TableArenaCtx ctx = { &arena };
    const Cells & root = *(const Cells *) TableArenaAt( arena, (uint32_t) root_ref.value );
...
// ---- Cells on the wire: the FLAT NODE TABLE (docs/SPEC-TABLES.md §3.1) ----
//
...
// CellsNodeStorage: the region bytes one record commands, or -1 for a type id
...
inline int64_t CellsNodeStorage( uint64_t type_id, int64_t length, TableRefuseReason & reason )
{
...
// CellsNodePlace: start one record's node's lifetime in the storage pass one
...
inline void CellsNodePlace( uint64_t type_id, uint8_t * at, int64_t length )
{
    (void) length;
    (void) at;
    switch ( type_id )
...
// CellsNodeRecordBytes: one record's OWN storage, before the extent its maps
// take (docs/SPEC-TABLES.md §2.8) — where a node's extent begins.
inline int64_t CellsNodeRecordBytes( uint64_t type_id )
{
    switch ( type_id )
    {
        default: break;
...
// CellsNodeAlloc: the TOOL's path — one record's node in the builder's arena.
// Zero is the arena's null, and it is also what a type id this build cannot
// name answers.
inline uint32_t CellsNodeAlloc( uint64_t type_id, TableWorker & worker, int64_t length )
{
    (void) length;
    (void) worker;
    switch ( type_id )
...
// CellsNodeBody: PASS TWO's half — decode one record's body into the storage it
// already owns.
inline void CellsNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        ...
        const int64_t storage = CellsNodeStorage( type_id, r.size, reason );
        const int64_t record = storage > 0 ? CellsNodeRecordBytes( type_id ) : 0;
        carve.at = at + record;
...
// CellsNodeMessageStorage: the region bytes one record commands on the message
...
inline int64_t CellsNodeMessageStorage( uint64_t type_id, int64_t extent, int64_t length )
{
    (void) length;
    (void) extent;
    switch ( type_id )
...
// CellsNodeMessageExtent: step over one TABLE record's body, tallying the extent
...
inline bool CellsNodeMessageExtent( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & extent )
{
    extent = 0;
...
// CellsNodeMessageBody: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
inline bool CellsNodeMessageBody( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at )
{
    TableExtentCarve carve;
    carve.at = at + CellsNodeRecordBytes( type_id );
    carve.left = 0;
    {
        // the extent this record was placed with, re-read from the framing
        TableBitReader walk = r;
        int64_t extent = 0;
        if ( !CellsNodeMessageExtent( type_id, walk, vocabulary, index_bits, extent ) ) { report->malformed = true; return false; }
        carve.left = extent;
    }
...
template <typename Ctx>
inline bool CellsNumberFrom( const Ctx & ctx, TableNumbering & numbering, const Cells & root )
{
    bool taken = false;
    int64_t slot = 0;
    if ( TablePackMapReach( numbering.seen, (const void *) &root, (int64_t) kTableNodeIndexRoot, taken, slot ) == NULL ) { return false; }
    return CellsNumber( ctx, numbering, root );
}

template <typename Ctx>
inline int64_t CellsMeasureWire( const Ctx & ctx, const Cells & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    if ( CellsNumberFrom( ctx, numbering, root ) )
    {
        TableIds ids;
        bytes = CellsMeasureBody( ctx, numbering, ids, root );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t CellsSaveWire( const Ctx & ctx, const Cells & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    if ( !CellsNumberFrom( ctx, numbering, root ) ) { TableNumberingShutdown( numbering ); return -1; }
    ...
    bool ok = CellsSaveBodyFields( ctx, numbering, w, ids, root ) && TableNodeTableSave( ctx, w, ids, numbering );
    TableNumberingShutdown( numbering );
    ...
    return w.offset; // == CellsMeasure( root )
}

inline int64_t CellsMeasure( const Cells * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return CellsMeasureWire( ctx, *root, allocator );
}

inline int64_t CellsSave( const Cells * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return CellsSaveWire( ctx, *root, buffer, capacity, allocator );
}

inline int64_t CellsMeasure( const CellsBuilder & builder )
{
    if ( builder.region != NULL ) { return CellsMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return CellsMeasureWire( ctx, *(const Cells *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline int64_t CellsSave( const CellsBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return CellsSave( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return CellsSaveWire( ctx, *(const Cells *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator );
}

// ---- Cells on the MESSAGE wire: the batch over a region (docs/SPEC-TABLES.md §3.3) ----

// CellsMessageBodyBits: one root body's bits at bit position `at` of the batch,
...
template <typename Ctx>
inline int64_t CellsMessageBodyBits( const Ctx & ctx, const Cells & root, int64_t at, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bits = -1;
    if ( CellsNumberFrom( ctx, numbering, root ) )
    {
        ...
        if ( table >= 0 )
        {
            const int64_t body = CellsMeasureMessageBody( ctx, numbering, index_bits, at + table, root );
...
template <typename Ctx>
inline bool CellsMessageBodySave( const Ctx & ctx, const Cells & root, TableBitWriter & w, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    bool ok = false;
    ...
    {
        const int64_t index_bits = TableBitsRequired( 0, numbering.count + 1 );
        ok = TableMessageNodeTableSave( ctx, numbering, index_bits, w ) && CellsSaveMessageBody( ctx, numbering, index_bits, w, root );
    }
    TableNumberingShutdown( numbering );
...
inline int64_t CellsMeasureMessages( const Cells * const * roots, int64_t count, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        ...
        const int64_t body = CellsMessageBodyBits( ctx, *roots[i], bits, allocator );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8;
...
inline int64_t CellsSaveMessages( const Cells * const * roots, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( roots == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( roots[i] == NULL || !CellsMessageBodySave( ctx, *roots[i], batch.w, allocator ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == CellsMeasureMessages( roots, count, report, allocator )
}

// CellsMessageRecordScan: one node record's type id and the extent its maps
...
inline bool CellsMessageRecordScan( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, uint64_t & type_id, int64_t & extent, int64_t & length )
{
    uint64_t type_ref = 0;
    ...
    return CellsNodeMessageExtent( type_id, r, vocabulary, index_bits, extent );
}

// CellsMessageBodyStorage: one body's node count and data bytes from the FRAMING
...
// and no further (§3.3).
inline bool CellsMessageBodyStorage( TableBitReader & r, const TableVocabulary & vocabulary, int64_t & records, int64_t & data, bool & complete )
{
    complete = true;
    records = 0;
    data = 0;
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !CellsMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { return false; }
        const int64_t storage = CellsNodeMessageStorage( type_id, extent, length );
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
        records++;
    }
    int64_t root_extent = 0;
    if ( !CellsMessageExtent( r, vocabulary, index_bits, root_extent ) ) { complete = false; }
    data += TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Cells ) ) + root_extent );
    return true;
}

// CellsLoadMeasure's MESSAGE overload: the exact region bytes ONE BATCH needs,
...
inline int64_t CellsLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
    ...
    for ( int64_t b = 0; b < bodies; b++ )
    {
        ...
        bool complete = true;
        if ( !CellsMessageBodyStorage( br.r, vocabulary, records, body_data, complete ) ) { return -1; }
        data += body_data;
...
// CellsLoadMessageBodyInto: one body of a batch into the region at `used`. Its
...
inline bool CellsLoadMessageBodyInto( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Cells * & root_out )
{
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        uint64_t type_id = 0;
        int64_t extent = 0, length = 0;
        if ( !CellsMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { out->malformed = true; return false; }
        const int64_t storage = CellsNodeMessageStorage( type_id, extent, length );
        ...
        CellsNodePlace( type_id, region + used, length );
        used += storage;
    }
    ...
    {
        TableBitReader walk = r;
        if ( !CellsMessageExtent( walk, vocabulary, index_bits, root_extent ) ) { out->malformed = true; return false; }
    }
    const int64_t root_bytes = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Cells ) ) + root_extent );
    ...
    Cells * root = new ( region + used ) Cells; // lifetime only: LoadMessageBody's first act is CellsReset
    CellsReset( *root );
    root_out = root;
    TableExtentCarve root_carve;
    root_carve.at = region + used + TableAlignUp64( (int64_t) sizeof( Cells ) );
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !CellsNodeMessageBody( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset ) ) { return false; }
    }
    ...
    return CellsLoadMessageBody( r, vocabulary, out, nodes, index_bits, *root );
}

// CellsLoadMessages: decode a BATCH into the caller's exact-sized region and
...
inline bool CellsLoadMessages( const Cells ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !CellsLoadMessageBodyInto( br.r, vocabulary, out, region, region_bytes, used, roots[b] ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
...
// CellsLoadMeasure: the exact region bytes a wire buffer will need, and it is
...
inline int64_t CellsLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    ...
    int64_t root_extent = 0;
    if ( !CellsWireExtent( wire, wire_bytes, root_extent, &ids_table, reason ) ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; }
    int64_t data = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Cells ) ) + root_extent );
    int64_t records = 0;
    uint64_t type_id = 0;
    ...
    {
        records++;
        int64_t storage = CellsNodeStorage( type_id, length, reason );
...
// CellsLoad: decode the tolerant wire into the caller's exact-sized region and
...
// was wrong.
inline const Cells * CellsLoad( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    ...
    const uint8_t * const wire = wire_file + 1;
    const int64_t wire_bytes = body_bytes;
    if ( region == NULL || region_bytes < (int64_t) sizeof( Cells ) ) { out->malformed = true; return NULL; }
    ...
    int64_t root_extent = 0;
    if ( !CellsWireExtent( wire, wire_bytes, root_extent, &ids_table, reason ) ) { out->malformed = true; return NULL; }
    int64_t data = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Cells ) ) + root_extent );
    int64_t records = 0;
    {
    ...
    directory[0].type_id = 0x1f781dc01a2b5152ull;
    Cells * root = new ( region ) Cells; // lifetime only: LoadBody's first act is CellsReset
    CellsReset( *root );
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            if ( directory[k + 1].offset != kTableNodeAbsent )
            {
                ...
                CellsNodeBody( type_id, sub, nodes, region + directory[k + 1].offset );
            }
            k++;
        }
    }
    ...
    TableExtentCarve root_carve;
    root_carve.at = region + TableAlignUp64( (int64_t) sizeof( Cells ) );
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    CellsLoadBody( r, nodes, *root );
    return root;
}

// CellsLoadBuilder: the TOOL's path — the same tolerant decode into a fresh
...
inline bool CellsLoadBuilder( CellsBuilder & builder, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    TableIdTable ids_table;
    ...
    nodes.worker = &builder.main; // and a map's entries and a list's elements are the arena's, not a node extent's (§2.8, §2.9)
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            uint32_t at = CellsNodeAlloc( type_id, builder.main, length );
            if ( at == 0 )
            {
                unknown_records++;
                directory[k + 1].offset = kTableNodeAbsent;
    ...
    bool ok = CellsLoadBody( r, nodes, *root );
...
template <typename Ctx>
inline int64_t CellsMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Cells & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        if ( order_rows.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_rows.count; i++ )
            {
                const int64_t elem_rows = CellsRowsEntryMeasureBodyRetain( ids, *order_rows[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_rows < 0 ) { TableMapRelease( order_rows ); return -1; }
                body_rows += TableLebBytes( (uint64_t) ( elem_rows ) ) + ( elem_rows ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_rows ) + 1 + TableLebBytes( (uint64_t) ( body_rows ) ) + ( body_rows );
...
template <typename Ctx>
inline bool CellsSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Cells & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<CellsRowsEntry> order_rows = TableMapOrder( ctx, value.rows ); // rows
        ...
        if ( order_rows.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            for ( int32_t i = 0; i < order_rows.count; i++ )
            {
                const int64_t elem_len_rows = CellsRowsEntryMeasureBodyRetain( ids, *order_rows[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_len_rows < 0 ) { TableMapRelease( order_rows ); return false; }
                w.putleb( (uint64_t) elem_len_rows );
                if ( !CellsRowsEntrySaveBodyRetain( w, ids, *order_rows[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) ) ) { TableMapRelease( order_rows ); return false; }
            }
        }
        TableMapRelease( order_rows );
    }
...
template <typename Ctx>
inline bool CellsSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Cells & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !CellsSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0xa3a7061ff10a8138ull: // rows
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            CellsRowsEntryLoadBodyRetain( elem, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) );
                        }
                        last_key = read.key; last_length = read.length; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
            ...
            case 0xa3a7061ff10a8138ull: // rows
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; }
                        if ( !CellsRowsEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) ) ) { return false; }
                        if ( r.offset != read.end ) { report->malformed = true; return false; } // the scan and the decode disagree about where the entry ends
                        last_key = read.key; last_length = read.length; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
...
// CellsNodeBodyRetain: PASS TWO's half — decode one record's body into the storage it
// already owns.
...
inline void CellsNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    // the node's own EXTENT, where its lists' and maps' arrays are carved
    // from, PRE-ORDER as the bodies decode (docs/SPEC-TABLES.md §2.8, §2.9).
    // The tool's path carries a worker instead: there the arrays are the
...
// CellsLoadRetain: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
...
inline const Cells * CellsLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                CellsNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
    ...
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    CellsLoadBodyRetain( r, nodes, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
    return root;
}

// CellsNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool CellsNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + CellsNodeRecordBytes( type_id );
    carve.left = 0;
...
// CellsLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
inline bool CellsLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Cells * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !CellsNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return CellsLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// CellsLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool CellsLoadRetainMessages( const Cells ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !CellsLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// CellsMeasureRetain and CellsSaveRetain: the pair, with the retained tail in
...
template <typename Ctx>
inline int64_t CellsMeasureWireRetain( const Ctx & ctx, const Cells & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( CellsNumberFrom( ctx, numbering, root ) )
    {
        ...
        bytes = CellsMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t CellsSaveWireRetain( const Ctx & ctx, const Cells & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    ...
    bool ok = CellsSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
...
inline int64_t CellsMeasureRetain( const Cells * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return CellsMeasureWireRetain( ctx, *root, retain, allocator );
}
...
inline int64_t CellsSaveRetain( const Cells * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return CellsSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// CellsSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
...
//
// A REFERENCE INSIDE THE REGION IS DEREFERENCED THROUGH CellsAt: the slot holds
...
// parameter on a load.
inline const Cells * CellsOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Cells *) TableCookOpen( bytes, length, (uint64_t) sizeof( Cells ), (uint64_t) alignof( Cells ), reason );
}
...
inline void CellsRowsEntryCookBody( uint8_t * at, const CellsRowsEntry & value, TableByteOrder order );
template <typename Ctx> inline bool CellsCookBody( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Cells & value, TableByteOrder order );
...
inline void CellsRowsEntryCookBody( uint8_t * at, const CellsRowsEntry & value, TableByteOrder order )
{
    ...
    for ( int32_t i = 0; i < 3; i++ )
    {
        table_cook_put( at + 16 + i * 4, (uint64_t) value.value[ i ], 4, order );
    }
}
...
template <typename Ctx> inline bool CellsRowsEntryCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const CellsRowsEntry & value, TableByteOrder order );
template <typename Ctx> inline bool CellsCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const Cells & value, TableByteOrder order );

// CellsRowsEntryCookExtent: CellsRowsEntry's arrays into the node's extent, PRE-ORDER, a map's entries
...
// CellsCookExtent: Cells's arrays into the node's extent, PRE-ORDER, a map's entries
// in ASCENDING key order and a list's elements in INDEX order, each through its
// own cook writer (§2.8, §2.9, §7.6).
template <typename Ctx> inline bool CellsCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const Cells & value, TableByteOrder order )
{
    ...
    { // rows
        ...
        at += (int64_t) cursor.count * 28; // the whole array FIRST
        ...
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            CellsRowsEntryCookBody( array + i * 28, *cursor[i], order );
        }
        TableMapRelease( cursor );
    }
    return true;
...
// CellsRowsEntryCookNode: one node, the record, then the extent its lists and maps take (§2.8, §2.9).
template <typename Ctx> inline bool CellsRowsEntryCookNode( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const CellsRowsEntry & value, TableByteOrder order )
{
    CellsRowsEntryCookBody( at, value, order );
    int64_t extent_at = 0;
    return CellsRowsEntryCookExtent( ctx, region, at + 32, extent_at, at, value, order );
}

// CellsCookNode: one node, the record, then the extent its lists and maps take (§2.8, §2.9).
template <typename Ctx> inline bool CellsCookNode( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Cells & value, TableByteOrder order )
{
    if ( !CellsCookBody( ctx, region, at, value, order ) ) { return false; }
    int64_t extent_at = 0;
    if ( !CellsCookExtent( ctx, region, at + 24, extent_at, at, value, order ) ) { return false; }
    return extent_at == CellsExtent( ctx, value ); // the extent written is the extent measured, or no header is written
}

// CellsCookLayout: the tool's own Layout (docs/SPEC-TABLES.md §7.2) over one
...
template <typename Ctx>
inline bool CellsCookLayout( const Ctx & ctx, const Cells & root, const TableNumbering & numbering, TableCookRegion & region )
{
    region.numbering = &numbering;
    region.count = numbering.count + 1;
    const int64_t root_extent = CellsExtent( ctx, root );
...
// CellsCookMeasureFrom: the whole cooked file's bytes for one graph — the header,
...
template <typename Ctx>
inline int64_t CellsCookMeasureFrom( const Ctx & ctx, const Cells & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    TableCookRegion region;
    int64_t bytes = -1;
    if ( CellsNumberFrom( ctx, numbering, root ) && CellsCookLayout( ctx, root, numbering, region ) )
    {
...
// CellsCookFrom: write one cooked file of a pointered graph, in the byte order
...
template <typename Ctx>
inline bool CellsCookFrom( const Ctx & ctx, const Cells & root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator )
{
    ...
    if ( ok )
    {
        ...
        if ( ok )
        {
            ...
            ok = CellsCookNode( ctx, region, region.base, root, order );
            ...
            table_cook_put( entry + 8, 0x1f781dc01a2b5152ull, 8, order ); // the root: fnv1a64( "Cells" )
            for ( int64_t k = 0; k < numbering.count; k++ )
            {
...
// CellsCookMeasure / CellsCook over a REGION root — a locked builder's AsConst, a
// region CellsLoad produced, or an opened cook — with the pair the numbering
// allocates through as an optional last argument, as the wire's own entries
// take it (§13.9).
inline int64_t CellsCookMeasure( const Cells * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return CellsCookMeasureFrom( ctx, *root, allocator );
}

inline bool CellsCook( const Cells * root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return false; }
    TableRegionCtx ctx;
    return CellsCookFrom( ctx, *root, out, capacity, order, allocator );
}

// and over a BUILDER, locked or not: the builder's own pair, and the arena
// encoding while it is still mutable (§6.3).
inline int64_t CellsCookMeasure( const CellsBuilder & builder )
{
    if ( builder.region != NULL ) { return CellsCookMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return CellsCookMeasureFrom( ctx, *(const Cells *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline bool CellsCook( const CellsBuilder & builder, void * out, uint64_t capacity, TableByteOrder order )
{
    if ( builder.region != NULL ) { return CellsCook( builder.AsConst(), out, capacity, order, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return false; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return CellsCookFrom( ctx, *(const Cells *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), out, capacity, order, builder.arena.allocator );
}
...
static_assert( __is_trivially_copyable( CellsRowsEntry ), "CellsRowsEntry must stay relocatable" );
static_assert( __is_standard_layout( CellsRowsEntry ), "CellsRowsEntry must stay standard-layout for offsetof" );
static_assert( __is_trivially_copyable( Cells ), "Cells must stay relocatable" );
static_assert( __is_standard_layout( Cells ), "Cells must stay standard-layout for offsetof" );
...
// rather than assumed.
static_assert( sizeof( CellsRowsEntry ) == 28, "CellsRowsEntry's sizeof moved: the build version was taken over 28, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( CellsRowsEntry ) == 4, "CellsRowsEntry's alignof moved: the build version was taken over 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( CellsRowsEntry, key ) == 0, "CellsRowsEntry's field key moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( CellsRowsEntry, value ) == 16, "CellsRowsEntry's field value moved: the build version was taken over offset 16 (docs/SPEC-TABLES.md §20.3)" );
static_assert( sizeof( Cells ) == 24, "Cells's sizeof moved: the build version was taken over 24, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( Cells ) == 8, "Cells's alignof moved: the build version was taken over 8 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Cells, rows ) == 0, "Cells's field rows moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Cells, after ) == 16, "Cells's field after moved: the build version was taken over offset 16 (docs/SPEC-TABLES.md §20.3)" );


// ---- reflection descriptors (tables only, docs/SPEC-TABLES.md) ----

inline const TableTypeInfo * CellsRowsEntryTableType();
inline const TableTypeInfo * CellsTableType();
...
extern const TableTypeInfo CellsRowsEntryTableInfo;
extern const TableTypeInfo CellsTableInfo;

inline const TableFieldInfo CellsRowsEntryTableFields[] = {
    { "key", "key", "string", 0x3dc94a19365b10ecull, 12, false, false, NULL, NULL, true, false, 8, (uint32_t) offsetof( CellsRowsEntry, key ), (uint32_t) sizeof( CellsRowsEntry::key ), (uint32_t) offsetof( CellsRowsEntry, key_length ), 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
    { "value", "value", "int32", 0x7ce4fd9430e80ceaull, 4, true, false, NULL, NULL, false, false, 3, (uint32_t) offsetof( CellsRowsEntry, value ), (uint32_t) sizeof( CellsRowsEntry::value[0] ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
};
inline const TableTypeInfo CellsRowsEntryTableInfo = { "CellsRowsEntry", (uint32_t) sizeof( CellsRowsEntry ), 2, CellsRowsEntryTableFields, +[]( void * p ) { CellsRowsEntryReset( *(CellsRowsEntry *) p ); }, false, TableDocNone, 0, NULL };
inline const TableTypeInfo * CellsRowsEntryTableType() { return &CellsRowsEntryTableInfo; }

inline const TableFieldInfo CellsTableFields[] = {
    { "rows", "rows", "map[string(8)]int32", 0xa3a7061ff10a8138ull, 13, true, false, NULL, NULL, true, false, 0, (uint32_t) offsetof( Cells, rows ), (uint32_t) sizeof( CellsRowsEntry ), (uint32_t) offsetof( Cells, rows.count ), 0xffffffffu, &CellsRowsEntryTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, []( TableWorker & worker, void * slot, const char * key, int32_t key_length, int64_t ) -> void * { if ( key == NULL ) { return NULL; } return (void *) TableMapPlace( worker, *(TableMap<CellsRowsEntry> *) slot, TableMapKeyRef{ key, key_length } ); }, "", TableDocNone, 0, NULL },
    { "after", "after", "int32", 0xbf82010f6f71eae9ull, 4, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Cells, after ), (uint32_t) sizeof( Cells::after ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
};
inline const TableTypeInfo CellsTableInfo = { "Cells", (uint32_t) sizeof( Cells ), 2, CellsTableFields, +[]( void * p ) { CellsReset( *(Cells *) p ); }, true, TableDocNone, 0, NULL };
inline const TableTypeInfo * CellsTableType() { return &CellsTableInfo; }

// ---- the text form (docs/SPEC-TABLES.md §16) ----

// Cells in and out of a JSON text (docs/SPEC-TABLES.md §16.7): read into a
// builder, written from a region's const root. A node named more than once
// carries `&node` in the text. Defined in CellsTable.cpp; link it to use them.
bool CellsFromJson( CellsBuilder & builder, const char * text, int64_t bytes, TableReport * report );
int64_t CellsToJsonMeasure( const Cells * root, TableAllocator allocator = TableDefaultAllocator() );
int64_t CellsToJson( const Cells * root, char * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() );
