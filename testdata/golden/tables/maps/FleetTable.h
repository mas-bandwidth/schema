...
constexpr uint64_t ShipConfigFixedHash = 0x9579bc245248ed0full; // fnv1a64 over the block's bytes
...
constexpr uint8_t ShipConfigFixedVocab[] = {
...
constexpr int64_t ShipConfigFixedVocabBytes = (int64_t) sizeof( ShipConfigFixedVocab );
...
{
    return 1 + 4 + ShipConfigFixedVocabBytes + count * ShipConfigFixedRecordBytes;
}
...
inline int64_t ShipConfigFixedSave( const ShipConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) ShipConfigFixedVocabBytes );
    memcpy( buffer + 5, ShipConfigFixedVocab, (size_t) ShipConfigFixedVocabBytes );
    uint8_t * at = buffer + 5 + ShipConfigFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != ShipConfigFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, ShipConfigFixedVocab, (int32_t) ShipConfigFixedVocabBytes, ShipConfigFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t ItemFixedHash = 0xd285587ad8f0ebdcull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 2 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t ItemFixedVocab[] = {
...
constexpr int64_t ItemFixedVocabBytes = (int64_t) sizeof( ItemFixedVocab );
...
{
    return 1 + 4 + ItemFixedVocabBytes + count * ItemFixedRecordBytes;
}
...
inline int64_t ItemFixedSave( const Item * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) ItemFixedVocabBytes );
    memcpy( buffer + 5, ItemFixedVocab, (size_t) ItemFixedVocabBytes );
    uint8_t * at = buffer + 5 + ItemFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != ItemFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, ItemFixedVocab, (int32_t) ItemFixedVocabBytes, ItemFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
