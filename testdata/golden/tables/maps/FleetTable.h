...
constexpr int ShipConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int ItemFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// ShipConfig's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int ShipConfigFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    (void) out; (void) src; (void) dst;
    out[n++] = TableFixedEntry{ src + 0u, dst + (uint32_t) __builtin_offsetof( ShipConfig, name_length ), 64u, dst + (uint32_t) __builtin_offsetof( ShipConfig, name ), kTableFixedNoGuard, kTableFixedText, 1, 0 }; // name
    {
        const uint32_t es = src + 68u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( ShipConfig, health );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// ShipConfig's stores. The template — the hash, then zeros — is memcpy'd first,
...
inline void ShipConfigFixedWriteBody( uint8_t * b, const ShipConfig & value )
{
    ...
    memcpy( b + 4, value.name, 64 );
    TableFixedPut32( b + 68, (uint32_t) value.health );
}

// Item's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int ItemFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Item, count );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// Item's stores. The template — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
...
inline constexpr int64_t ShipConfigFixedBodyBytes = 72;
inline constexpr int64_t ShipConfigFixedRecordBytes = 8 + ShipConfigFixedBodyBytes; // the hash and the body
inline constexpr uint64_t ShipConfigFixedHash = 0x9579bc245248ed0full; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t ShipConfigFixedLayout[] = {
...
inline constexpr int64_t ShipConfigFixedLayoutBytes = (int64_t) sizeof( ShipConfigFixedLayout );
...
inline constexpr TableFixedDst ShipConfigFixedDst[] = {
...
inline constexpr auto ShipConfigFixedPlan = TableFixedBuildPlan<2>( ShipConfigFixedLeaves );
...
inline constexpr int64_t ShipConfigFixedMeasure( int64_t count )
{
...
inline int64_t ShipConfigFixedSave( const ShipConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) ShipConfigFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = ShipConfigFixedPlan.entries;
    int32_t entry_count = ShipConfigFixedPlan.count;
    ...
    if ( hash != ShipConfigFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, ShipConfigFixedLayout, (int32_t) ShipConfigFixedLayoutBytes, ShipConfigFixedDst, plan, plan_capacity, report );
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
inline constexpr int64_t ItemFixedBodyBytes = 4;
inline constexpr int64_t ItemFixedRecordBytes = 8 + ItemFixedBodyBytes; // the hash and the body
inline constexpr uint64_t ItemFixedHash = 0xd285587ad8f0ebdcull; // fnv1a64 over the layout's bytes

// THE LAYOUT (form 1 called this the vocabulary block): 2 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
inline constexpr uint8_t ItemFixedLayout[] = {
...
inline constexpr int64_t ItemFixedLayoutBytes = (int64_t) sizeof( ItemFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout
// lands values through.
inline constexpr TableFixedDst ItemFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Item
    { (uint32_t) __builtin_offsetof( Item, count ), 0, 0, 0, 0 }, // count
};

// THE IDENTITY PLAN, coalesced at COMPILE TIME out of 1 leaves.
inline constexpr auto ItemFixedPlan = TableFixedBuildPlan<1>( ItemFixedLeaves );

// A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
inline constexpr int64_t ItemFixedMeasure( int64_t count )
{
...
inline int64_t ItemFixedSave( const Item * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) ItemFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const uint8_t * at = layout + layout_bytes;
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    const TableFixedEntry * entries = ItemFixedPlan.entries;
    int32_t entry_count = ItemFixedPlan.count;
    ...
    if ( hash != ItemFixedHash )
    {
        ...
        TableMessageReason why = layout_malformed;
        if ( !TableFixedParseLayout( layout, layout_bytes, parsed, why ) ) { report->refused = true; report->reason = why; return -1; }
        const int32_t made = TableFixedCompile( parsed, ItemFixedLayout, (int32_t) ItemFixedLayoutBytes, ItemFixedDst, plan, plan_capacity, report );
        if ( made < 0 ) { report->refused = true; report->reason = plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ItemReset( values[k] ); // the declared defaults, one prefill
        if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
        TableFixedRun( entries, entry_count, at + 8, (uint8_t *) &values[k], report );
        at += record_bytes;
    }
