...
constexpr uint64_t ShipConfigFixedHash = 0x9579bc245248ed0full; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry ShipConfigFixedPlan[] = {
    { 0u, 68u, 64u, 0u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0 }, // name
    { 68u, 72u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // health
};
...
// ShipConfig's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
...
inline void ShipConfigFixedScatter( const uint8_t * TABLE_RESTRICT image, ShipConfig & value, TableReport * report )
{
    ...
    memcpy( dst + 72, image + 68, 4 ); // health
    report->clamped += clamped;
}
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == ShipConfigFixedHash );
    ...
    if ( identity )
    {
        ...
        uint8_t image[ShipConfigFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) ShipConfigFixedBodyBytes );
            ShipConfigFixedScatter( image, values[k], report );
            at += ShipConfigFixedRecordBytes;
        }
        return n;
    }
...
constexpr uint64_t ItemFixedHash = 0xd285587ad8f0ebdcull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry ItemFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // count
};
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == ItemFixedHash );
    // THE PREFILL'S RANGES. EMPTY ON THE IDENTITY PLAN, and not by a flag:
    // the rule is the type's value bytes minus what the plan lands, and the
    // identity plan lands all of them.
    const TableFixedFill * fill = NULL;
    ...
    if ( identity )
    {
        ...
        static_assert( sizeof( Item ) >= (size_t) ItemFixedBodyBytes, "Item: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) ItemFixedBodyBytes );
            at += ItemFixedRecordBytes;
        }
        return n;
    }
