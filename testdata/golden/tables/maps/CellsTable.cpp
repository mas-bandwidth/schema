// Code generated by the schema compiler from Cells.schema. DO NOT EDIT.
...
bool CellsFromJson( CellsBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    Cells * root = builder.GetRoot();
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, CellsTableType(), text, bytes, report );
}

int64_t CellsToJsonMeasure( const Cells * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, CellsTableType(), NULL, 0, allocator );
}

int64_t CellsToJson( const Cells * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, CellsTableType(), buffer, capacity, allocator );
}
