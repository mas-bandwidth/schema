...
// table SlotsSeatsEntry — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct SlotsSeatsEntry {
    int32_t key = 0;
    TableKeyed<int32_t, Slot> value; // [Slot]: one slot per named variant, keyed by the value
};

// table Slots — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct Slots {
    TableMap<SlotsSeatsEntry> seats; // map[int32]int32 — the sorted entry array, empty until an insert
    int32_t after = 0;
};
...
inline void SlotsSeatsEntryReset( SlotsSeatsEntry & value );
inline void SlotsReset( Slots & value );

inline void SlotsSeatsEntryReset( SlotsSeatsEntry & value )
{
    value.key = 0;
    memset( value.value.slots, 0, sizeof( value.value.slots ) );
}

inline void SlotsReset( Slots & value )
{
    value.seats.entries.value = 0; // map[int32]int32: empty
    value.seats.count = 0;
    value.seats.padding = 0;
    value.after = 0;
}

inline int64_t SlotsSeatsEntryMeasureMessageBody( int64_t at, const SlotsSeatsEntry & value );
inline bool SlotsSeatsEntrySaveMessageBody( TableBitWriter & w, const SlotsSeatsEntry & value );
inline bool SlotsSeatsEntryLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, SlotsSeatsEntry & value );
template <typename Ctx> inline int64_t SlotsMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Slots & value );
template <typename Ctx> inline bool SlotsSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Slots & value );
inline bool SlotsLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Slots & value );
inline bool SlotsMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at );

// SlotsSeatsEntryMessageKeyRead: the key of one entry on the message wire, before the
...
struct SlotsSeatsEntryMessageKeyRead
{
    int32_t key;
...
inline SlotsSeatsEntryMessageKeyRead SlotsSeatsEntryMessageReadKey( TableBitReader r, const TableVocabulary & vocabulary, int64_t index_bits )
{
    SlotsSeatsEntryMessageKeyRead out = { 0, 0, false, false, false, false, false };
    for ( ;; )
    {
        uint64_t ref = 0;
...
inline void TableReset( SlotsSeatsEntry & value ) { SlotsSeatsEntryReset( value ); }
inline void TableReset( Slots & value ) { SlotsReset( value ); }
...
MAPDEMO_TABLE_INLINE bool SlotsSeatsEntrySaveBody( TableWriter & w, TableIds & ids, const SlotsSeatsEntry & value );
MAPDEMO_TABLE_INLINE bool SlotsSeatsEntryLoadBody( TableReader & r, SlotsSeatsEntry & value );
template <typename Ctx> inline int64_t SlotsMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Slots & value );
template <typename Ctx> inline bool SlotsSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Slots & value );
template <typename Ctx> inline bool SlotsSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Slots & value );
...
template <typename Ctx> inline bool SlotsNumber( const Ctx & ctx, TableNumbering & numbering, const Slots & value );
template <typename Ctx> inline int64_t SlotsPackMeasure( const Ctx & ctx, TablePackMap & seen, const Slots & value );
template <typename Ctx> inline bool SlotsPack( const Ctx & ctx, TablePackMap & seen, const Slots & src, Slots & dst, uint8_t * base, int64_t capacity, int64_t & used );

// ---- the numbering's bridge to each member's codec (docs/SPEC-TABLES.md §3.1) ----

template <typename Ctx> inline int64_t TableNodeMeasure( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Slots & value ) { return SlotsMeasureBody( ctx, numbering, ids, value ); }
template <typename Ctx> inline bool TableNodeSave( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Slots & value ) { return SlotsSaveBody( ctx, numbering, w, ids, value ); }
template <typename Ctx> inline int64_t TableNodeMessageMeasure( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Slots & value ) { return SlotsMeasureMessageBody( ctx, numbering, index_bits, at, value ); }
template <typename Ctx> inline bool TableNodeMessageSave( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Slots & value ) { return SlotsSaveMessageBody( ctx, numbering, index_bits, w, value ); }

// ---- SlotsSeatsEntry: the order, the key and the value (docs/SPEC-TABLES.md §2.8) ----
//
// The four overloads the map runtime's templates reach by argument-dependent
// lookup. Nothing outside this file names them.
static_assert( alignof( SlotsSeatsEntry ) <= kTableAlign, "a map entry's alignment must fit the arena's" );

inline int TableEntryOrder( const SlotsSeatsEntry & a, const SlotsSeatsEntry & b )
{
    return TableKeyOrder( (int64_t) a.key, (int64_t) b.key ); // integers compare by VALUE, signed here
}
inline int TableEntryOrder( const SlotsSeatsEntry & entry, int32_t key )
{
    return TableKeyOrder( (int64_t) entry.key, (int64_t) key );
}
inline int32_t TableEntryKey( const SlotsSeatsEntry & entry ) { return entry.key; }
inline void TableEntrySetKey( SlotsSeatsEntry & entry, int32_t key ) { entry.key = key; }
inline const TableKeyed<int32_t, Slot> * TableEntryFound( const SlotsSeatsEntry * entry ) { return entry != NULL ? &entry->value : NULL; }
inline TableKeyed<int32_t, Slot> * TableEntryValue( SlotsSeatsEntry * entry ) { return &entry->value; }
struct SlotsSeatsEntryEach { int32_t key; decltype( TableEntryValue( (SlotsSeatsEntry *) NULL ) ) value; };
inline SlotsSeatsEntryEach TableEntryEach( SlotsSeatsEntry * entry ) { return SlotsSeatsEntryEach{ TableEntryKey( *entry ), TableEntryValue( entry ) }; }
inline void TableResetMapValue( SlotsSeatsEntry & value )
{
    memset( value.value.slots, 0, sizeof( value.value.slots ) );
}

// SlotsSeatsEntryReadKey: the key, before the slot is chosen (docs/SPEC-TABLES.md §2.8).
...
inline SlotsSeatsEntryKeyRead SlotsSeatsEntryReadKey( const uint8_t * body, int64_t length, const TableIdTable * ids )
{
    SlotsSeatsEntryKeyRead out = { 0, false, false, false, false, false };
...
MAPDEMO_TABLE_INLINE bool SlotsSeatsEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const SlotsSeatsEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool SlotsSeatsEntryLoadBodyRetain( TableReader & r, SlotsSeatsEntry & value, TableRetain * retain, const TableRetainPath & path );
inline bool SlotsSeatsEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, SlotsSeatsEntry & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline int64_t SlotsMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Slots & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool SlotsSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Slots & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool SlotsSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Slots & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool SlotsLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Slots & value, TableRetain * retain, const TableRetainPath & path );
...
inline int64_t SlotsSeatsEntryMeasureBody( TableIds & ids, const SlotsSeatsEntry & value )
{
    ...
    if ( value.key != 0 ) { bytes += TableLebBytes( ids.ref( 0x3dc94a19365b10ecull ) ) + 1 + 4; } // key
    {
        ...
        {
            if ( value.value.slots[i] == 0 ) { continue; } // a default slot elides
            uint64_t key_ref = 0;
            if ( !TableEnumRef( ids, Slot( i + 1 ), key_ref ) || key_ref == 0 ) { return -1; } // i is the STORAGE index; the key it holds is i + 1
            pairs_value++; body_value += TableLebBytes( key_ref ) + TableLebBytes( 4 ) + 4;
        }
        if ( pairs_value > 0 )
        {
            const int64_t whole_value = 1 + TableLebBytes( (uint64_t) pairs_value ) + body_value;
            bytes += TableLebBytes( ref_value ) + 1 + TableLebBytes( (uint64_t) ( whole_value ) ) + ( whole_value ); // value
        }
        else { ids.truncate( mark_value ); } // an ELIDED field costs nothing in the id table either
    }
    return bytes;
}
...
MAPDEMO_TABLE_INLINE bool SlotsSeatsEntrySaveBody( TableWriter & w, TableIds & ids, const SlotsSeatsEntry & value )
{
    ...
    }
    {
        ...
        if ( pairs_value > 0 )
        {
            ...
            const int64_t whole_value = 1 + TableLebBytes( (uint64_t) pairs_value ) + body_value;
            w.putleb( ref_value ); w.put8( 16 ); w.putleb( (uint64_t) whole_value ); // value (keyed by Slot)
            w.put8( 4 ); w.putleb( (uint64_t) pairs_value );
            ...
            for ( int32_t i = 0; i < 2; i++ )
            {
                ...
                w.putleb( 4 );
                w.put32( uint32_t( value.value.slots[i] ) );
            }
        }
        else { ids.truncate( mark_value ); } // an ELIDED field costs nothing in the id table either
    }
...
MAPDEMO_TABLE_INLINE bool SlotsSeatsEntryLoadBody( TableReader & r, SlotsSeatsEntry & value )
{
    SlotsSeatsEntryReset( value ); // prefill declared defaults in place, then overlay
    for ( ;; )
    {
        uint64_t field_ref = 0;
                    ...
                    if ( elem_kind != 4 )
                    {
                        ...
                        for ( uint64_t i = 0; i < count; i++ )
                        {
                            ...
                            }
                            {
                                ...
                                int32_t decoded_v = (int32_t) widened_v;
                                value.value.slots[int32_t( slot ) - 1] = decoded_v;
                            }
                            sub.offset += (int64_t) elem_len;
                        }
                    }
    ...
    }
    {
        ...
        if ( pairs_value > 0 )
        {
            ...
            for ( int32_t i = 0; i < 2; i++ )
            {
                ...
                w.put( (uint64_t) ( value.value.slots[i] ), 32 );
            }
        }
    }
...
template <typename Ctx>
inline int64_t SlotsMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Slots & value )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        // seats: a kind 14 array of kind 13 elements, ASCENDING (§2.8)
        TableMapCursor<SlotsSeatsEntry> order_seats = TableMapOrder( ctx, value.seats );
        ...
        if ( order_seats.count > 0 )
        {
            ...
            {
                const int64_t elem_seats = SlotsSeatsEntryMeasureBody( ids, *order_seats[i] );
                if ( elem_seats < 0 ) { TableMapRelease( order_seats ); return -1; }
                body_seats += TableLebBytes( (uint64_t) ( elem_seats ) ) + ( elem_seats ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_seats ) + 1 + TableLebBytes( (uint64_t) ( body_seats ) ) + ( body_seats );
        }
...
template <typename Ctx>
inline bool SlotsSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Slots & value )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<SlotsSeatsEntry> order_seats = TableMapOrder( ctx, value.seats ); // seats
        ...
        if ( order_seats.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            {
                const int64_t elem_seats = SlotsSeatsEntryMeasureBody( ids, *order_seats[i] );
                if ( elem_seats < 0 ) { TableMapRelease( order_seats ); return false; }
                body_seats += TableLebBytes( (uint64_t) ( elem_seats ) ) + ( elem_seats );
            }
            w.putleb( ref_seats ); w.put8( 14 ); w.putleb( (uint64_t) body_seats );
            w.put8( 13 ); w.putleb( (uint64_t) order_seats.count );
            for ( int32_t i = 0; i < order_seats.count; i++ )
            {
                const int64_t elem_len_seats = SlotsSeatsEntryMeasureBody( ids, *order_seats[i] );
                if ( elem_len_seats < 0 ) { TableMapRelease( order_seats ); return false; }
                w.putleb( (uint64_t) elem_len_seats );
                if ( !SlotsSeatsEntrySaveBody( w, ids, *order_seats[i] ) ) { TableMapRelease( order_seats ); return false; }
            }
        }
        TableMapRelease( order_seats );
    }
...
template <typename Ctx>
inline bool SlotsSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Slots & value )
{
    if ( !SlotsSaveBodyFields( ctx, numbering, w, ids, value ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
        ...
        switch ( field_id )
        {
            case 0x7f6548303072b061ull: // seats
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    TableMapFill<SlotsSeatsEntry> fill = TableMapFillBegin( nodes, value.seats, count );
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        sub.offset += (int64_t) elem_len;
                        SlotsSeatsEntryKeyRead read = SlotsSeatsEntryReadKey( elem_body, (int64_t) elem_len, r.ids );
                        ...
                        if ( slot == NULL ) { r.report->malformed = true; break; }
                        {
                            ...
                            SlotsSeatsEntryLoadBody( elem, *slot );
                        }
...
// its bytes and an align costs what the position says it costs.
template <typename Ctx>
inline int64_t SlotsMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Slots & value )
{
    ...
    int64_t bits = 0;
    {
        ...
        if ( order_seats.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_seats.count; i++ )
            {
                const int64_t elem_seats = SlotsSeatsEntryMeasureMessageBody( at + bits, *order_seats[i] );
                if ( elem_seats < 0 ) { TableMapRelease( order_seats ); return -1; }
                bits += elem_seats; // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
        }
        TableMapRelease( order_seats );
    }
...
// body is self-delimiting: it is written where the file form put an L.
template <typename Ctx>
inline bool SlotsSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Slots & value )
{
    (void) ctx; (void) numbering; (void) index_bits;
    {
        TableMapCursor<SlotsSeatsEntry> order_seats = TableMapOrder( ctx, value.seats ); // seats
        if ( !order_seats.ok ) { return false; } // the sort could not run
        if ( order_seats.count > 0 )
        {
            w.put( 43, kTableMessageRefBitsHere );
            w.put( (uint64_t) order_seats.count, 32 ); // the count the data decides
            for ( int32_t i = 0; i < order_seats.count; i++ )
            {
                if ( !SlotsSeatsEntrySaveMessageBody( w, *order_seats[i] ) ) { TableMapRelease( order_seats ); return false; }
            }
        }
        TableMapRelease( order_seats );
    }
...
// SlotsMessageExtent: the extent Slots's maps command on the message wire, from
...
inline bool SlotsMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at )
{
    for ( ;; )
    {
        ...
        if ( entry.id == 0x7f6548303072b061ull && entry.kind == 14 && entry.elem_kind == 13 ) // seats
        {
            uint64_t n = 0;
            if ( !r.get( n, TableBitsRequired( entry.min, entry.max ) ) ) { return false; }
            n += (uint64_t) entry.min;
            at = ( at + 3 ) & ~(int64_t) 3; // at alignof( SlotsSeatsEntry )
            at += (int64_t) n * (int64_t) sizeof( SlotsSeatsEntry ); // the whole array FIRST
        ...
        switch ( entry.id )
        {
            case 0x7f6548303072b061ull: // seats
            {
                ...
                }
                {
                    ...
                    TableMapFill<SlotsSeatsEntry> fill = TableMapFillBegin( nodes, value.seats, (uint32_t) count );
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        const SlotsSeatsEntryMessageKeyRead read = SlotsSeatsEntryMessageReadKey( r, vocabulary, index_bits );
                        ...
                        if ( !SlotsSeatsEntryLoadMessageBody( r, vocabulary, report, index_bits, *slot ) ) { return false; }
...
// SlotsWireExtent: the extent Slots's lists and maps command, from the FRAMING alone.
...
inline bool SlotsWireExtent( const uint8_t * body, int64_t length, int64_t & at, const TableIdTable * ids, TableRefuseReason & reason )
{
    ...
    for ( ;; )
    {
        ...
        if ( field_id == 0x7f6548303072b061ull && field_kind == 14 ) // seats
        {
            ...
            if ( !TableMapWireExtent( map_body, (int64_t) map_len, at, (int64_t) sizeof( SlotsSeatsEntry ), (int64_t) alignof( SlotsSeatsEntry ), NULL, ids, reason ) ) { return false; }
            continue;
        }
...
// SlotsExtentAt: the node extent Slots's lists and maps take, PRE-ORDER, advancing
// the running offset exactly as SlotsExtentPack advances it (§2.8, §2.9).
template <typename Ctx>
inline bool SlotsExtentAt( const Ctx & ctx, const Slots & value, int64_t & at )
{
    {
        TableMapCursor<SlotsSeatsEntry> cursor = TableMapOrder( ctx, value.seats );
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3; // at alignof( SlotsSeatsEntry )
        at += (int64_t) cursor.count * (int64_t) sizeof( SlotsSeatsEntry ); // the whole array FIRST
...
template <typename Ctx>
inline int64_t SlotsExtent( const Ctx & ctx, const Slots & value )
{
    int64_t at = 0;
    if ( !SlotsExtentAt( ctx, value, at ) ) { return -1; }
    return at;
}

// SlotsExtentPack: carve Slots's arrays out of the node's extent and copy the
// entries in ASCENDING key order and the elements in INDEX order, PRE-ORDER,
// advancing the same running offset SlotsExtentAt advances (§2.8, §2.9).
template <typename Ctx>
inline bool SlotsExtentPack( const Ctx & ctx, const Slots & src, Slots & dst, uint8_t * extent, int64_t & at, int64_t capacity )
{
    {
        TableMapCursor<SlotsSeatsEntry> cursor = TableMapOrder( ctx, src.seats );
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3;
        const int64_t bytes = (int64_t) cursor.count * (int64_t) sizeof( SlotsSeatsEntry );
        if ( at + bytes > capacity ) { TableMapRelease( cursor ); return false; }
        SlotsSeatsEntry * placed = (SlotsSeatsEntry *) ( extent + at );
        at += bytes;
        dst.seats.count = cursor.count;
        dst.seats.padding = 0;
        dst.seats.entries.value = cursor.count > 0 ? (int64_t) ( (uint8_t *) placed - (const uint8_t *) &dst.seats.entries ) : 0;
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            memcpy( (void *) ( placed + i ), (const void *) cursor[i], sizeof( SlotsSeatsEntry ) ); // trivially copyable, by construction
        }
        TableMapRelease( cursor );
    }
    return true;
...
// ---- Slots.seats: the builder's five and the side index (§2.8) ----
...
inline TableKeyed<int32_t, Slot> * SlotsSeatsInsert( TableWorker & worker, TableMap<SlotsSeatsEntry> & map, int32_t key )
{
    SlotsSeatsEntry * entry = TableMapPlace( worker, map, key );
...
inline TableKeyed<int32_t, Slot> * SlotsSeatsFind( TableArena & arena, TableMap<SlotsSeatsEntry> & map, int32_t key )
{
    SlotsSeatsEntry * found = TableMapScan( arena, map, key );
...
// entry's address is stable" false for exactly one case.
inline bool SlotsSeatsErase( TableArena & arena, TableMap<SlotsSeatsEntry> & map, int32_t key )
{
    return TableMapErase( arena, map, key );
}

// EACH on the builder: INSERTION order, live entries only.
inline TableMapEach<SlotsSeatsEntry> SlotsSeatsEach( const TableArena & arena, const TableMap<SlotsSeatsEntry> & map )
{
...
inline int64_t SlotsSeatsIndexMeasure( const TableMap<SlotsSeatsEntry> & map )
{
    return (int64_t) TableMapIndexSlots( map.count ) * (int64_t) sizeof( int32_t );
}

inline TableMapIndex SlotsSeatsIndex( const TableMap<SlotsSeatsEntry> & map, void * storage, int64_t bytes )
{
    TableMapIndex index;
    ...
    const SlotsSeatsEntry * entries = map.Entries();
...
inline const TableKeyed<int32_t, Slot> * SlotsSeatsIndexFind( const TableMapIndex & index, const TableMap<SlotsSeatsEntry> & map, int32_t key )
{
...
// SlotsNumber: number everything Slots POINTS AT, in first-visit order —
...
template <typename Ctx>
inline bool SlotsNumber( const Ctx & ctx, TableNumbering & numbering, const Slots & value )
{
    (void) ctx; (void) numbering; (void) value; // no pointers below this node
    return true;
}

// SlotsPackMeasure: the packed region bytes of everything Slots POINTS AT.
...
template <typename Ctx>
inline int64_t SlotsPackMeasure( const Ctx & ctx, TablePackMap & seen, const Slots & value )
{
    int64_t bytes = 0;
...
// SlotsPack: copy src into dst (already placed), then lay every pointee out
...
template <typename Ctx>
inline bool SlotsPackEdges( const Ctx & ctx, TablePackMap & seen, const Slots & src, Slots & dst, uint8_t * base, int64_t capacity, int64_t & used );

template <typename Ctx>
inline bool SlotsPack( const Ctx & ctx, TablePackMap & seen, const Slots & src, Slots & dst, uint8_t * base, int64_t capacity, int64_t & used )
{
    memcpy( (void *) &dst, (const void *) &src, sizeof( Slots ) ); // trivially copyable, by construction
    int64_t at = 0;
    uint8_t * extent = (uint8_t *) &dst + TableAlignUp64( (int64_t) sizeof( Slots ) );
    const int64_t room = capacity - ( (int64_t) ( extent - base ) );
    if ( !SlotsExtentPack( ctx, src, dst, extent, at, room ) ) { return false; }
    return SlotsPackEdges( ctx, seen, src, dst, base, capacity, used );
}
...
// ---- Slots: the variable-length life (docs/SPEC-TABLES.md §2, §6, §9) ----
//
// MUTABLE: SlotsBuilder — allocate nodes, wire them together, then Lock.
...
// Slots is never held by value — a file-format-scale structure is a region
...
struct SlotsBuilder
{
    ...
    SlotsBuilder( TableAllocator allocator = TableDefaultAllocator() )
    {
    ...
    Slots * GetRoot() { return arena.locked ? NULL : (Slots *) TableArenaAt( arena, (uint32_t) root_ref.value ); }
    bool Locked() const { return arena.locked; }
    const Slots * AsConst() const { return (const Slots *) region; }
...
inline bool SlotsBuilder::Lock()
{
    if ( arena.locked ) { return region != NULL; }
    if ( root_ref.null() ) { return false; }
    TableArenaCtx ctx = { &arena };
    const Slots & root = *(const Slots *) TableArenaAt( arena, (uint32_t) root_ref.value );
...
// ---- Slots on the wire: the FLAT NODE TABLE (docs/SPEC-TABLES.md §3.1) ----
//
...
// SlotsNodeStorage: the region bytes one record commands, or -1 for a type id
...
inline int64_t SlotsNodeStorage( uint64_t type_id, int64_t length, TableRefuseReason & reason )
{
...
// SlotsNodePlace: start one record's node's lifetime in the storage pass one
...
inline void SlotsNodePlace( uint64_t type_id, uint8_t * at, int64_t length )
{
    (void) length;
    (void) at;
    switch ( type_id )
...
// SlotsNodeRecordBytes: one record's OWN storage, before the extent its maps
// take (docs/SPEC-TABLES.md §2.8) — where a node's extent begins.
inline int64_t SlotsNodeRecordBytes( uint64_t type_id )
{
    switch ( type_id )
    {
        default: break;
...
// SlotsNodeAlloc: the TOOL's path — one record's node in the builder's arena.
// Zero is the arena's null, and it is also what a type id this build cannot
// name answers.
inline uint32_t SlotsNodeAlloc( uint64_t type_id, TableWorker & worker, int64_t length )
{
    (void) length;
    (void) worker;
    switch ( type_id )
...
// SlotsNodeBody: PASS TWO's half — decode one record's body into the storage it
// already owns.
inline void SlotsNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        ...
        const int64_t storage = SlotsNodeStorage( type_id, r.size, reason );
        const int64_t record = storage > 0 ? SlotsNodeRecordBytes( type_id ) : 0;
        carve.at = at + record;
...
// SlotsNodeMessageStorage: the region bytes one record commands on the message
...
inline int64_t SlotsNodeMessageStorage( uint64_t type_id, int64_t extent, int64_t length )
{
    (void) length;
    (void) extent;
    switch ( type_id )
...
// SlotsNodeMessageExtent: step over one TABLE record's body, tallying the extent
...
inline bool SlotsNodeMessageExtent( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & extent )
{
    extent = 0;
...
// SlotsNodeMessageBody: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
inline bool SlotsNodeMessageBody( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at )
{
    TableExtentCarve carve;
    carve.at = at + SlotsNodeRecordBytes( type_id );
    carve.left = 0;
    {
        // the extent this record was placed with, re-read from the framing
        TableBitReader walk = r;
        int64_t extent = 0;
        if ( !SlotsNodeMessageExtent( type_id, walk, vocabulary, index_bits, extent ) ) { report->malformed = true; return false; }
        carve.left = extent;
    }
...
template <typename Ctx>
inline bool SlotsNumberFrom( const Ctx & ctx, TableNumbering & numbering, const Slots & root )
{
    bool taken = false;
    int64_t slot = 0;
    if ( TablePackMapReach( numbering.seen, (const void *) &root, (int64_t) kTableNodeIndexRoot, taken, slot ) == NULL ) { return false; }
    return SlotsNumber( ctx, numbering, root );
}

template <typename Ctx>
inline int64_t SlotsMeasureWire( const Ctx & ctx, const Slots & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    if ( SlotsNumberFrom( ctx, numbering, root ) )
    {
        TableIds ids;
        bytes = SlotsMeasureBody( ctx, numbering, ids, root );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t SlotsSaveWire( const Ctx & ctx, const Slots & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    if ( !SlotsNumberFrom( ctx, numbering, root ) ) { TableNumberingShutdown( numbering ); return -1; }
    ...
    bool ok = SlotsSaveBodyFields( ctx, numbering, w, ids, root ) && TableNodeTableSave( ctx, w, ids, numbering );
    TableNumberingShutdown( numbering );
    ...
    return w.offset; // == SlotsMeasure( root )
}

inline int64_t SlotsMeasure( const Slots * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return SlotsMeasureWire( ctx, *root, allocator );
}

inline int64_t SlotsSave( const Slots * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return SlotsSaveWire( ctx, *root, buffer, capacity, allocator );
}

inline int64_t SlotsMeasure( const SlotsBuilder & builder )
{
    if ( builder.region != NULL ) { return SlotsMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return SlotsMeasureWire( ctx, *(const Slots *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline int64_t SlotsSave( const SlotsBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return SlotsSave( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return SlotsSaveWire( ctx, *(const Slots *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator );
}

// ---- Slots on the MESSAGE wire: the batch over a region (docs/SPEC-TABLES.md §3.3) ----

// SlotsMessageBodyBits: one root body's bits at bit position `at` of the batch,
...
template <typename Ctx>
inline int64_t SlotsMessageBodyBits( const Ctx & ctx, const Slots & root, int64_t at, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bits = -1;
    if ( SlotsNumberFrom( ctx, numbering, root ) )
    {
        ...
        if ( table >= 0 )
        {
            const int64_t body = SlotsMeasureMessageBody( ctx, numbering, index_bits, at + table, root );
...
template <typename Ctx>
inline bool SlotsMessageBodySave( const Ctx & ctx, const Slots & root, TableBitWriter & w, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    bool ok = false;
    ...
    {
        const int64_t index_bits = TableBitsRequired( 0, numbering.count + 1 );
        ok = TableMessageNodeTableSave( ctx, numbering, index_bits, w ) && SlotsSaveMessageBody( ctx, numbering, index_bits, w, root );
    }
    TableNumberingShutdown( numbering );
...
inline int64_t SlotsMeasureMessages( const Slots * const * roots, int64_t count, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        ...
        const int64_t body = SlotsMessageBodyBits( ctx, *roots[i], bits, allocator );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8;
...
inline int64_t SlotsSaveMessages( const Slots * const * roots, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( roots == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( roots[i] == NULL || !SlotsMessageBodySave( ctx, *roots[i], batch.w, allocator ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == SlotsMeasureMessages( roots, count, report, allocator )
}

// SlotsMessageRecordScan: one node record's type id and the extent its maps
...
inline bool SlotsMessageRecordScan( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, uint64_t & type_id, int64_t & extent, int64_t & length )
{
    uint64_t type_ref = 0;
    ...
    return SlotsNodeMessageExtent( type_id, r, vocabulary, index_bits, extent );
}

// SlotsMessageBodyStorage: one body's node count and data bytes from the FRAMING
...
// and no further (§3.3).
inline bool SlotsMessageBodyStorage( TableBitReader & r, const TableVocabulary & vocabulary, int64_t & records, int64_t & data, bool & complete )
{
    complete = true;
    records = 0;
    data = 0;
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !SlotsMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { return false; }
        const int64_t storage = SlotsNodeMessageStorage( type_id, extent, length );
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
        records++;
    }
    int64_t root_extent = 0;
    if ( !SlotsMessageExtent( r, vocabulary, index_bits, root_extent ) ) { complete = false; }
    data += TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Slots ) ) + root_extent );
    return true;
}

// SlotsLoadMeasure's MESSAGE overload: the exact region bytes ONE BATCH needs,
...
inline int64_t SlotsLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
    ...
    for ( int64_t b = 0; b < bodies; b++ )
    {
        ...
        bool complete = true;
        if ( !SlotsMessageBodyStorage( br.r, vocabulary, records, body_data, complete ) ) { return -1; }
        data += body_data;
...
// SlotsLoadMessageBodyInto: one body of a batch into the region at `used`. Its
...
inline bool SlotsLoadMessageBodyInto( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Slots * & root_out )
{
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        uint64_t type_id = 0;
        int64_t extent = 0, length = 0;
        if ( !SlotsMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { out->malformed = true; return false; }
        const int64_t storage = SlotsNodeMessageStorage( type_id, extent, length );
        ...
        SlotsNodePlace( type_id, region + used, length );
        used += storage;
    }
    ...
    {
        TableBitReader walk = r;
        if ( !SlotsMessageExtent( walk, vocabulary, index_bits, root_extent ) ) { out->malformed = true; return false; }
    }
    const int64_t root_bytes = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Slots ) ) + root_extent );
    ...
    Slots * root = new ( region + used ) Slots; // lifetime only: LoadMessageBody's first act is SlotsReset
    SlotsReset( *root );
    root_out = root;
    TableExtentCarve root_carve;
    root_carve.at = region + used + TableAlignUp64( (int64_t) sizeof( Slots ) );
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !SlotsNodeMessageBody( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset ) ) { return false; }
    }
    ...
    return SlotsLoadMessageBody( r, vocabulary, out, nodes, index_bits, *root );
}

// SlotsLoadMessages: decode a BATCH into the caller's exact-sized region and
...
inline bool SlotsLoadMessages( const Slots ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !SlotsLoadMessageBodyInto( br.r, vocabulary, out, region, region_bytes, used, roots[b] ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
...
// SlotsLoadMeasure: the exact region bytes a wire buffer will need, and it is
...
inline int64_t SlotsLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    ...
    int64_t root_extent = 0;
    if ( !SlotsWireExtent( wire, wire_bytes, root_extent, &ids_table, reason ) ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; }
    int64_t data = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Slots ) ) + root_extent );
    int64_t records = 0;
    uint64_t type_id = 0;
    ...
    {
        records++;
        int64_t storage = SlotsNodeStorage( type_id, length, reason );
...
// SlotsLoad: decode the tolerant wire into the caller's exact-sized region and
...
// was wrong.
inline const Slots * SlotsLoad( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    ...
    const uint8_t * const wire = wire_file + 1;
    const int64_t wire_bytes = body_bytes;
    if ( region == NULL || region_bytes < (int64_t) sizeof( Slots ) ) { out->malformed = true; return NULL; }
    ...
    int64_t root_extent = 0;
    if ( !SlotsWireExtent( wire, wire_bytes, root_extent, &ids_table, reason ) ) { out->malformed = true; return NULL; }
    int64_t data = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( Slots ) ) + root_extent );
    int64_t records = 0;
    {
    ...
    directory[0].type_id = 0xf96b15cd3921d4a6ull;
    Slots * root = new ( region ) Slots; // lifetime only: LoadBody's first act is SlotsReset
    SlotsReset( *root );
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            if ( directory[k + 1].offset != kTableNodeAbsent )
            {
                ...
                SlotsNodeBody( type_id, sub, nodes, region + directory[k + 1].offset );
            }
            k++;
        }
    }
    ...
    TableExtentCarve root_carve;
    root_carve.at = region + TableAlignUp64( (int64_t) sizeof( Slots ) );
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    SlotsLoadBody( r, nodes, *root );
    return root;
}

// SlotsLoadBuilder: the TOOL's path — the same tolerant decode into a fresh
...
inline bool SlotsLoadBuilder( SlotsBuilder & builder, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    TableIdTable ids_table;
    ...
    nodes.worker = &builder.main; // and a map's entries and a list's elements are the arena's, not a node extent's (§2.8, §2.9)
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            uint32_t at = SlotsNodeAlloc( type_id, builder.main, length );
            if ( at == 0 )
            {
                unknown_records++;
                directory[k + 1].offset = kTableNodeAbsent;
    ...
    bool ok = SlotsLoadBody( r, nodes, *root );
...
template <typename Ctx>
inline int64_t SlotsMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Slots & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        if ( order_seats.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_seats.count; i++ )
            {
                const int64_t elem_seats = SlotsSeatsEntryMeasureBodyRetain( ids, *order_seats[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_seats < 0 ) { TableMapRelease( order_seats ); return -1; }
                body_seats += TableLebBytes( (uint64_t) ( elem_seats ) ) + ( elem_seats ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_seats ) + 1 + TableLebBytes( (uint64_t) ( body_seats ) ) + ( body_seats );
...
template <typename Ctx>
inline bool SlotsSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Slots & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<SlotsSeatsEntry> order_seats = TableMapOrder( ctx, value.seats ); // seats
        ...
        if ( order_seats.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            for ( int32_t i = 0; i < order_seats.count; i++ )
            {
                const int64_t elem_len_seats = SlotsSeatsEntryMeasureBodyRetain( ids, *order_seats[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_len_seats < 0 ) { TableMapRelease( order_seats ); return false; }
                w.putleb( (uint64_t) elem_len_seats );
                if ( !SlotsSeatsEntrySaveBodyRetain( w, ids, *order_seats[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) ) ) { TableMapRelease( order_seats ); return false; }
            }
        }
        TableMapRelease( order_seats );
    }
...
template <typename Ctx>
inline bool SlotsSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Slots & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !SlotsSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0x7f6548303072b061ull: // seats
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            SlotsSeatsEntryLoadBodyRetain( elem, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) );
                        }
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
            ...
            case 0x7f6548303072b061ull: // seats
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; }
                        if ( !SlotsSeatsEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) ) ) { return false; }
                        if ( r.offset != read.end ) { report->malformed = true; return false; } // the scan and the decode disagree about where the entry ends
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
...
// SlotsNodeBodyRetain: PASS TWO's half — decode one record's body into the storage it
// already owns.
...
inline void SlotsNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    // the node's own EXTENT, where its lists' and maps' arrays are carved
    // from, PRE-ORDER as the bodies decode (docs/SPEC-TABLES.md §2.8, §2.9).
    // The tool's path carries a worker instead: there the arrays are the
...
// SlotsLoadRetain: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
...
inline const Slots * SlotsLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                SlotsNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
    ...
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    SlotsLoadBodyRetain( r, nodes, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
    return root;
}

// SlotsNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool SlotsNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + SlotsNodeRecordBytes( type_id );
    carve.left = 0;
...
// SlotsLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
inline bool SlotsLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Slots * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !SlotsNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return SlotsLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// SlotsLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool SlotsLoadRetainMessages( const Slots ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !SlotsLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// SlotsMeasureRetain and SlotsSaveRetain: the pair, with the retained tail in
...
template <typename Ctx>
inline int64_t SlotsMeasureWireRetain( const Ctx & ctx, const Slots & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( SlotsNumberFrom( ctx, numbering, root ) )
    {
        ...
        bytes = SlotsMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t SlotsSaveWireRetain( const Ctx & ctx, const Slots & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    ...
    bool ok = SlotsSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
...
inline int64_t SlotsMeasureRetain( const Slots * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return SlotsMeasureWireRetain( ctx, *root, retain, allocator );
}
...
inline int64_t SlotsSaveRetain( const Slots * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return SlotsSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// SlotsSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
...
inline int64_t SlotsSaveRetainMessages( Args &&... )
{
...
// SlotsOpen: match the header and POINT. On a match the bytes ARE what this
...
//
// A REFERENCE INSIDE THE REGION IS DEREFERENCED THROUGH SlotsAt: the slot holds
...
// parameter on a load.
inline const Slots * SlotsOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Slots *) TableCookOpen( bytes, length, (uint64_t) sizeof( Slots ), (uint64_t) alignof( Slots ), reason );
}
...
inline void SlotsSeatsEntryCookBody( uint8_t * at, const SlotsSeatsEntry & value, TableByteOrder order );
template <typename Ctx> inline bool SlotsCookBody( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Slots & value, TableByteOrder order );
...
inline void SlotsSeatsEntryCookBody( uint8_t * at, const SlotsSeatsEntry & value, TableByteOrder order )
{
    ...
    for ( int32_t i = 0; i < 2; i++ )
    {
        table_cook_put( at + 4 + i * 4, (uint64_t) value.value.slots[ i ], 4, order );
    }
}
...
template <typename Ctx> inline bool SlotsCookBody( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Slots & value, TableByteOrder order )
{
    ...
    table_cook_put( at + 0, 0, 8, order ); // seats: the array's delta, filled by the extent writer
    table_cook_put( at + 8, 0, 4, order ); // and its count
    table_cook_put( at + 16, (uint64_t) value.after, 4, order );
    return true;
}

template <typename Ctx> inline bool SlotsSeatsEntryCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const SlotsSeatsEntry & value, TableByteOrder order );
template <typename Ctx> inline bool SlotsCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const Slots & value, TableByteOrder order );

// SlotsSeatsEntryCookExtent: SlotsSeatsEntry's arrays into the node's extent, PRE-ORDER, a map's entries
...
// SlotsCookExtent: Slots's arrays into the node's extent, PRE-ORDER, a map's entries
// in ASCENDING key order and a list's elements in INDEX order, each through its
// own cook writer (§2.8, §2.9, §7.6).
template <typename Ctx> inline bool SlotsCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const Slots & value, TableByteOrder order )
{
    ...
    { // seats
        ...
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            SlotsSeatsEntryCookBody( array + i * 12, *cursor[i], order );
        }
        TableMapRelease( cursor );
    }
    return true;
...
// SlotsSeatsEntryCookNode: one node, the record, then the extent its lists and maps take (§2.8, §2.9).
template <typename Ctx> inline bool SlotsSeatsEntryCookNode( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const SlotsSeatsEntry & value, TableByteOrder order )
{
    SlotsSeatsEntryCookBody( at, value, order );
    int64_t extent_at = 0;
    return SlotsSeatsEntryCookExtent( ctx, region, at + 16, extent_at, at, value, order );
}

// SlotsCookNode: one node, the record, then the extent its lists and maps take (§2.8, §2.9).
template <typename Ctx> inline bool SlotsCookNode( const Ctx & ctx, const TableCookRegion & region, uint8_t * at, const Slots & value, TableByteOrder order )
{
    if ( !SlotsCookBody( ctx, region, at, value, order ) ) { return false; }
    int64_t extent_at = 0;
    if ( !SlotsCookExtent( ctx, region, at + 24, extent_at, at, value, order ) ) { return false; }
    return extent_at == SlotsExtent( ctx, value ); // the extent written is the extent measured, or no header is written
}

// SlotsCookLayout: the tool's own Layout (docs/SPEC-TABLES.md §7.2) over one
...
template <typename Ctx>
inline bool SlotsCookLayout( const Ctx & ctx, const Slots & root, const TableNumbering & numbering, TableCookRegion & region )
{
    region.numbering = &numbering;
    region.count = numbering.count + 1;
    const int64_t root_extent = SlotsExtent( ctx, root );
...
// SlotsCookMeasureFrom: the whole cooked file's bytes for one graph — the header,
...
template <typename Ctx>
inline int64_t SlotsCookMeasureFrom( const Ctx & ctx, const Slots & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    TableCookRegion region;
    int64_t bytes = -1;
    if ( SlotsNumberFrom( ctx, numbering, root ) && SlotsCookLayout( ctx, root, numbering, region ) )
    {
...
// SlotsCookFrom: write one cooked file of a pointered graph, in the byte order
...
template <typename Ctx>
inline bool SlotsCookFrom( const Ctx & ctx, const Slots & root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator )
{
    ...
    if ( ok )
    {
        ...
        if ( ok )
        {
            ...
            ok = SlotsCookNode( ctx, region, region.base, root, order );
            ...
            table_cook_put( entry + 8, 0xf96b15cd3921d4a6ull, 8, order ); // the root: fnv1a64( "Slots" )
            for ( int64_t k = 0; k < numbering.count; k++ )
            {
...
// SlotsCookMeasure / SlotsCook over a REGION root — a locked builder's AsConst, a
// region SlotsLoad produced, or an opened cook — with the pair the numbering
// allocates through as an optional last argument, as the wire's own entries
// take it (§13.9).
inline int64_t SlotsCookMeasure( const Slots * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return SlotsCookMeasureFrom( ctx, *root, allocator );
}

inline bool SlotsCook( const Slots * root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return false; }
    TableRegionCtx ctx;
    return SlotsCookFrom( ctx, *root, out, capacity, order, allocator );
}

// and over a BUILDER, locked or not: the builder's own pair, and the arena
// encoding while it is still mutable (§6.3).
inline int64_t SlotsCookMeasure( const SlotsBuilder & builder )
{
    if ( builder.region != NULL ) { return SlotsCookMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return SlotsCookMeasureFrom( ctx, *(const Slots *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline bool SlotsCook( const SlotsBuilder & builder, void * out, uint64_t capacity, TableByteOrder order )
{
    if ( builder.region != NULL ) { return SlotsCook( builder.AsConst(), out, capacity, order, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return false; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return SlotsCookFrom( ctx, *(const Slots *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), out, capacity, order, builder.arena.allocator );
}
...
static_assert( __is_trivially_copyable( SlotsSeatsEntry ), "SlotsSeatsEntry must stay relocatable" );
static_assert( __is_standard_layout( SlotsSeatsEntry ), "SlotsSeatsEntry must stay standard-layout for offsetof" );
static_assert( __is_trivially_copyable( Slots ), "Slots must stay relocatable" );
static_assert( __is_standard_layout( Slots ), "Slots must stay standard-layout for offsetof" );
...
// rather than assumed.
static_assert( sizeof( SlotsSeatsEntry ) == 12, "SlotsSeatsEntry's sizeof moved: the build version was taken over 12, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( SlotsSeatsEntry ) == 4, "SlotsSeatsEntry's alignof moved: the build version was taken over 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( SlotsSeatsEntry, key ) == 0, "SlotsSeatsEntry's field key moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( SlotsSeatsEntry, value ) == 4, "SlotsSeatsEntry's field value moved: the build version was taken over offset 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( sizeof( Slots ) == 24, "Slots's sizeof moved: the build version was taken over 24, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( Slots ) == 8, "Slots's alignof moved: the build version was taken over 8 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Slots, seats ) == 0, "Slots's field seats moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Slots, after ) == 16, "Slots's field after moved: the build version was taken over offset 16 (docs/SPEC-TABLES.md §20.3)" );


// ---- reflection descriptors (tables only, docs/SPEC-TABLES.md) ----

inline const TableTypeInfo * SlotsSeatsEntryTableType();
inline const TableTypeInfo * SlotsTableType();
...
extern const TableTypeInfo SlotsSeatsEntryTableInfo;
extern const TableTypeInfo SlotsTableInfo;

inline const TableFieldInfo SlotsSeatsEntryTableFields[] = {
    { "key", "key", "int32", 0x3dc94a19365b10ecull, 4, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( SlotsSeatsEntry, key ), (uint32_t) sizeof( SlotsSeatsEntry::key ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
    { "value", "value", "int32", 0x7ce4fd9430e80ceaull, 4, true, false, NULL, NULL, false, false, (int32_t) Slot::Max, (uint32_t) offsetof( SlotsSeatsEntry, value ), (uint32_t) sizeof( SlotsSeatsEntry::value.slots[0] ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, "Slot", +[]( uint64_t v ) { return EnumName( Slot( v ) ); }, +[]( uint64_t v ) -> uint64_t { uint64_t id = 0; TableEnumId( Slot( v ), id ); return id; }, NULL, NULL, "", TableDocNone, 0, NULL },
};
inline const TableTypeInfo SlotsSeatsEntryTableInfo = { "SlotsSeatsEntry", (uint32_t) sizeof( SlotsSeatsEntry ), 2, SlotsSeatsEntryTableFields, +[]( void * p ) { SlotsSeatsEntryReset( *(SlotsSeatsEntry *) p ); }, false, TableDocNone, 0, NULL };
inline const TableTypeInfo * SlotsSeatsEntryTableType() { return &SlotsSeatsEntryTableInfo; }

inline const TableFieldInfo SlotsTableFields[] = {
    { "seats", "seats", "map[int32]int32", 0x7f6548303072b061ull, 13, true, false, NULL, NULL, true, false, 0, (uint32_t) offsetof( Slots, seats ), (uint32_t) sizeof( SlotsSeatsEntry ), (uint32_t) offsetof( Slots, seats.count ), 0xffffffffu, &SlotsSeatsEntryTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, []( TableWorker & worker, void * slot, const char *, int32_t, int64_t key_value ) -> void * { return (void *) TableMapPlace( worker, *(TableMap<SlotsSeatsEntry> *) slot, (int32_t) key_value ); }, "", TableDocNone, 0, NULL },
    { "after", "after", "int32", 0xbf82010f6f71eae9ull, 4, false, false, NULL, NULL, false, false, 0, (uint32_t) offsetof( Slots, after ), (uint32_t) sizeof( Slots::after ), 0xffffffffu, 0xffffffffu, NULL, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "", TableDocNone, 0, NULL },
};
inline const TableTypeInfo SlotsTableInfo = { "Slots", (uint32_t) sizeof( Slots ), 2, SlotsTableFields, +[]( void * p ) { SlotsReset( *(Slots *) p ); }, true, TableDocNone, 0, NULL };
inline const TableTypeInfo * SlotsTableType() { return &SlotsTableInfo; }

// ---- the text form (docs/SPEC-TABLES.md §16) ----

// Slots in and out of a JSON text (docs/SPEC-TABLES.md §16.7): read into a
// builder, written from a region's const root. A node named more than once
// carries `&node` in the text. Defined in SlotsTable.cpp; link it to use them.
bool SlotsFromJson( SlotsBuilder & builder, const char * text, int64_t bytes, TableReport * report );
int64_t SlotsToJsonMeasure( const Slots * root, TableAllocator allocator = TableDefaultAllocator() );
int64_t SlotsToJson( const Slots * root, char * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() );
