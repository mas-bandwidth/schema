...
bool TextFromJson( TextBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, TextTableType(), text, bytes, report );
}

int64_t TextToJsonMeasure( const Text * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TextTableType(), NULL, 0, allocator );
}

int64_t TextToJson( const Text * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TextTableType(), buffer, capacity, allocator );
}
