...
bool DepthFromJson( DepthBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, DepthTableType(), text, bytes, report );
}

int64_t DepthToJsonMeasure( const Depth * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DepthTableType(), NULL, 0, allocator );
}

int64_t DepthToJson( const Depth * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DepthTableType(), buffer, capacity, allocator );
}
