...
inline bool TableEnumRef( TableRetainIds & ids, Slot value, uint64_t & ref )
{
    switch ( value )
    {
        case Slot::None: ref = 0; return true;
...
inline bool TableEnumSlot( Slot value, uint64_t & slot )
{
    switch ( value )
    {
        ...
        case Slot::Alpha: slot = 50; return true;
        case Slot::Beta: slot = 51; return true;
        default: return false; // no variant names this value: no wire identity
    }
}
...
MAPDEMO_TABLE_INLINE bool SquadRosterEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const SquadRosterEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool SquadRosterEntryLoadBodyRetain( TableReader & r, SquadRosterEntry & value, TableRetain * retain, const TableRetainPath & path );
inline bool SquadRosterEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, SquadRosterEntry & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool SquadLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Squad & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline int64_t DepthMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Depth & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool DepthSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Depth & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool DepthSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Depth & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool DepthLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Depth & value, TableRetain * retain, const TableRetainPath & path );

inline int64_t SquadRosterEntryMeasureBody( TableIds & ids, const SquadRosterEntry & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.key != 0 ) { bytes += TableLebBytes( ids.ref_at( 21, 0x3dc94a19365b10ecull ) ) + 1 + 1; } // key
    {
...
MAPDEMO_TABLE_INLINE bool SquadRosterEntrySaveBody( TableWriter & w, TableIds & ids, const SquadRosterEntry & value )
{
    if ( value.key != 0 )
    {
        w.header( ids.ref_at( 21, 0x3dc94a19365b10ecull ), 6 ); // key
...
inline int64_t SquadMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Squad & value )
{
    ...
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        {
            const uint64_t ref_roster = ids.ref_at( 11, 0x1c84390d304f4f42ull );
...
inline int64_t DepthMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Depth & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        const uint64_t ref_one = ids.ref_at( 10, 0x1a08aa1921ca5cafull );
    ...
    {
        const uint64_t ref_many = ids.ref_at( 12, 0x1f6459a2cea1fc02ull );
        int64_t body_many = 0;
    ...
    }
    {
        ...
        const uint64_t ref_keyed = ids.ref_at( 31, 0x70551ff29550f15dull );
    ...
    {
        bytes += TableLebBytes( ids.ref_at( 66, 0xe756c0190570ccb5ull ) ) + 1;
        switch ( value.arm.type )
        {
            ...
            case ForceType::Squad:
            {
                ...
                const uint64_t arm_ref = ids.ref_at( 60, 0xd5c2bb95d63e6331ull );
                {
            ...
            {
                int64_t arm_payload = 0;
                const uint64_t arm_ref = ids.ref_at( 73, 0xfd4d194e1652b207ull );
                arm_payload += 4; // int32
                bytes += TableLebBytes( arm_ref ) + 1 + TableLebBytes( (uint64_t) ( arm_payload ) ) + ( arm_payload );
                break;
            }
...
inline bool DepthSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Depth & value )
{
    ...
    {
        w.header( ids.ref_at( 66, 0xe756c0190570ccb5ull ), 15 ); // arm
        switch ( value.arm.type )
        {
            case ForceType::Squad:
            {
            ...
            case 0x1c84390d304f4f42ull: // roster
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; }
                        if ( !SquadRosterEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) ) ) { return false; }
                        if ( r.offset != read.end ) { report->malformed = true; return false; } // the scan and the decode disagree about where the entry ends
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
...
template <typename Ctx>
inline int64_t DepthMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Depth & value, TableRetain * retain, const TableRetainPath & path )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        const int32_t mark_one = ids.count;
        const uint64_t ref_one = ids.ref_at( 10, 0x1a08aa1921ca5cafull );
        const int64_t body_one = SquadMeasureBodyRetain( ctx, numbering, ids, value.one, retain, TableRetainStepInto( path, 0, (uint32_t) ( 0 ) ) );
        if ( body_one < 0 ) { return -1; }
        if ( body_one > 1 ) { bytes += TableLebBytes( ref_one ) + 1 + TableLebBytes( (uint64_t) ( body_one ) ) + ( body_one ); } // one
        else { ids.truncate( mark_one ); } // an all-default nested table elides, and costs no entry
    }
    ...
    if ( value.many_count > 0 )
    {
        ...
        for ( int32_t elem_i = 0; elem_i < value.many_count; elem_i++ )
        {
            const int64_t elem_bytes = SquadMeasureBodyRetain( ctx, numbering, ids, value.many[elem_i], retain, TableRetainStepInto( path, 1, (uint32_t) ( elem_i ) ) );
            if ( elem_bytes < 0 ) { return -1; }
            body_many += TableLebBytes( (uint64_t) ( elem_bytes ) ) + ( elem_bytes );
        }
        bytes += TableLebBytes( ref_many ) + 1 + TableLebBytes( (uint64_t) ( body_many ) ) + ( body_many ); // many
    }
    {
        ...
        for ( int32_t i = 0; i < 2; i++ ) // [Slot]: every stored slot is a named variant's
        {
            ...
            uint64_t key_ref = 0;
            if ( !TableEnumRef( ids, Slot( i + 1 ), key_ref ) || key_ref == 0 ) { return -1; } // i is the STORAGE index; the key it holds is i + 1
            const int64_t elem_bytes = SquadMeasureBodyRetain( ctx, numbering, ids, value.keyed.slots[i], retain, TableRetainStepInto( path, 2, (uint32_t) ( i ) ) );
            if ( elem_bytes < 0 ) { return -1; }
            if ( elem_bytes <= 1 ) { ids.truncate( slot_mark ); continue; } // an all-default slot elides
            pairs_keyed++; body_keyed += TableLebBytes( key_ref ) + TableLebBytes( (uint64_t) ( elem_bytes ) ) + ( elem_bytes );
        }
    ...
    if ( value.arm.type != ForceType::None ) // None elides — the absence of the field is the None
    {
        ...
        switch ( value.arm.type )
        {
            ...
            case ForceType::Squad:
            {
                ...
                const uint64_t arm_ref = ids.ref_at( 60, 0xd5c2bb95d63e6331ull );
                {
                    const int64_t arm_body = SquadMeasureBodyRetain( ctx, numbering, ids, value.arm.squad, retain, TableRetainStepInto( path, 3, (uint32_t) ( 0 ) ) );
                    if ( arm_body < 0 ) { return -1; }
                    arm_payload += arm_body; // the arm's own table body (§3)
                }
                bytes += TableLebBytes( arm_ref ) + 1 + TableLebBytes( (uint64_t) ( arm_payload ) ) + ( arm_payload );
...
template <typename Ctx>
inline bool DepthSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Depth & value, TableRetain * retain, const TableRetainPath & path )
{
    {
        const int32_t mark_one = ids.count;
        const uint64_t ref_one = ids.ref_at( 10, 0x1a08aa1921ca5cafull );
    ...
    if ( value.many_count > 0 )
    {
        ...
        {
            {
                const int64_t elem_len = SquadMeasureBodyRetain( ctx, numbering, ids, value.many[elem_i], retain, TableRetainStepInto( path, 1, (uint32_t) ( elem_i ) ) );
                if ( elem_len < 0 ) return false;
                w.putleb( (uint64_t) elem_len );
                if ( !SquadSaveBodyRetain( ctx, numbering, w, ids, value.many[elem_i], retain, TableRetainStepInto( path, 1, (uint32_t) ( elem_i ) ) ) ) return false;
            }
        }
    }
    {
        ...
        if ( pairs_keyed > 0 )
        {
            ...
            for ( int32_t i = 0; i < 2; i++ )
            {
                ...
                w.putleb( key_ref ); // the slot's VARIANT reference, not its position
                w.putleb( (uint64_t) elem_bytes );
                if ( !SquadSaveBodyRetain( ctx, numbering, w, ids, value.keyed.slots[i], retain, TableRetainStepInto( path, 2, (uint32_t) ( i ) ) ) ) return false;
            }
        }
        else { ids.truncate( mark_keyed ); } // an ELIDED field costs nothing in the id table either
    }
...
template <typename Ctx>
inline bool DepthSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Depth & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !DepthSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
    ...
    for ( ;; )
    {
        ...
        switch ( entry.id )
        {
            case 0x1a08aa1921ca5cafull: // one
            {
                ...
                TableRetainDiscardField( retain, path, 0 );
                if ( !SquadLoadMessageBodyRetain( r, vocabulary, report, nodes, index_bits, value.one, retain, TableRetainStepInto( path, 0, (uint32_t) ( 0 ) ) ) ) { return false; }
                break;
            }
            case 0x1f6459a2cea1fc02ull: // many
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < walk; i++ )
                    {
                        ...
                        Squad scratch;
                        SquadReset( scratch );
                        if ( !SquadLoadMessageBodyRetain( r, vocabulary, report, nodes, index_bits, ( in_bounds ? value.many[i] : scratch ), ( in_bounds ? retain : NULL ), TableRetainStepInto( path, 1, (uint32_t) ( i ) ) ) ) { return false; }
                    }
                    value.many_count = kept;
                }
                break;
            ...
            case 0x70551ff29550f15dull: // keyed
            {
                ...
                }
                {
                    ...
                    for ( uint64_t p = 0; p < n; p++ )
                    {
                        ...
                        Squad scratch;
                        SquadReset( scratch );
                        if ( !SquadLoadMessageBodyRetain( r, vocabulary, report, nodes, index_bits, ( in_bounds ? value.keyed.slots[slot] : scratch ), ( in_bounds ? retain : NULL ), TableRetainStepInto( path, 2, (uint32_t) ( slot ) ) ) ) { return false; }
                    }
                }
                break;
            }
                    ...
                    else
                    {
                        switch ( arm.id )
                        {
                            case 0xd5c2bb95d63e6331ull: // squad
                            {
                                ...
                                value.arm.type = ForceType::Squad;
                                if ( !SquadLoadMessageBodyRetain( r, vocabulary, report, nodes, index_bits, value.arm.squad, retain, TableRetainStepInto( path, 3, (uint32_t) ( 0 ) ) ) ) { return false; }
                                break;
                            }
                            case 0xfd4d194e1652b207ull: // plain
                            {
                            ...
                            default:
                                value.arm.type = ForceType::None; report->unknown++; report->retain_lost++;
                                if ( !TableMessageSkip( r, vocabulary, index_bits, arm ) ) { report->malformed = true; return false; }
                                break;
                        }
                    }
...
// SquadNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool SquadNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + SquadNodeRecordBytes( type_id );
    carve.left = 0;
...
inline bool SquadLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Squad * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !SquadNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return SquadLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// SquadLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool SquadLoadRetainMessages( const Squad ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !SquadLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// index is the region directory's, which Load fills from the wire's framing
// and nothing afterwards renumbers.
inline void DepthNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    // the node's own EXTENT, where its lists' and maps' arrays are carved
    // from, PRE-ORDER as the bodies decode (docs/SPEC-TABLES.md §2.8, §2.9).
    // The tool's path carries a worker instead: there the arrays are the
...
// DepthLoadRetain: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
...
// (docs/SPEC-TABLES.md §6.6). It is Load's own path and nothing else: the
// reader's data is exactly what it would have been with retention off.
inline const Depth * DepthLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                DepthNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
    ...
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    DepthLoadBodyRetain( r, nodes, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
    return root;
}

// DepthNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
// index is this body's own directory position, which the batch's load fills
// from the framing and nothing afterwards renumbers.
inline bool DepthNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + DepthNodeRecordBytes( type_id );
    carve.left = 0;
...
// DepthLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
// each body carries its own node directory inside that one region and a
// record's first step is an index into it (docs/SPEC-TABLES.md §3.3, §6.6).
inline bool DepthLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Depth * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !DepthNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return DepthLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// DepthLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
// SaveRetain from `roots[k]` takes `retains[k]` and is the VARIABLE form's own
// pair unchanged: retention writing FORM 2 refuses by name (§3.3).
inline bool DepthLoadRetainMessages( const Depth ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !DepthLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// DepthMeasureRetain and DepthSaveRetain: the pair, with the retained tail in
// every body it belongs to (docs/SPEC-TABLES.md §6.6). They drop the same
// records under the same walk, so Measure's answer is the size the save
// writes even where a record could not be placed.
template <typename Ctx>
inline int64_t DepthMeasureWireRetain( const Ctx & ctx, const Depth & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( DepthNumberFrom( ctx, numbering, root ) )
    {
        TableRetainIds ids( retain );
        if ( retain != NULL ) { retain->id_used = 0; } // one walk fills the list, and the save's own walk refills it
        bytes = DepthMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
            const int64_t table = TableNodeTableMeasureRetain( ctx, ids, numbering, retain, retain_measure );
            bytes = table < 0 || ids.overflow ? -1 : 1 + bytes + table + TableRetainIdsBytes( ids );
...
template <typename Ctx>
inline int64_t DepthSaveWireRetain( const Ctx & ctx, const Depth & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, TableDefaultAllocator() );
    auto retain_measure = []( const Ctx & c, const TableNumbering & nn, TableRetainIds & ii, uint64_t type_id, const void * node, TableRetain * rt ) -> int64_t
    ...
    // field: a retained field is one of the root's own values, and the tail
    // is pinned before the large and damage-prone part (§6.6, §3.1)
    bool ok = DepthSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
              TableNodeTableSaveRetain( ctx, w, ids, numbering, retain, retain_measure, retain_save );
    TableNumberingShutdown( numbering );
    if ( !ok || ids.overflow ) { return -1; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the root body
...
inline int64_t DepthMeasureRetain( const Depth * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return DepthMeasureWireRetain( ctx, *root, retain, allocator );
}
...
// else. A surface that let a caller retain, save and never find out would
// be a promise it could not check.
inline int64_t DepthSaveRetain( const Depth * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return DepthSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// DepthSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
// (docs/SPEC-TABLES.md §3.3). It is a MISUSE refusal on §6.6's own
// precedent and never a silent drop, and the two answers are named: a
// caller that must carry unknowns across a rewrite writes the VARIABLE form,
// which carries its own table and takes §6.6 unchanged, and a RELAY
// forwards the sending peer's announcement and its batch bytes verbatim.
template <typename... Args>
inline int64_t DepthSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Depth: a form 2 writer names entries through slots of a vocabulary the compiler settled, and a retained id is one this build's closure does not contain, so it has neither a slot nor an announced shape. Retention writing the MESSAGE form is refused by name (docs/SPEC-TABLES.md §3.3). Write the VARIABLE form, which carries its own table and takes §6.6 unchanged, or relay the sender's announcement and batch bytes verbatim." );
    return -1;
}
