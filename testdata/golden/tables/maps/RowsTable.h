...
inline int64_t RowMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Row & value )
{
    ...
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        {
            const uint64_t ref_entries = ids.ref( 0xc5b2a72c0845a253ull, 15 );
...
// the connection's announced table and no trailer to write.
template <typename Ctx>
inline int64_t WideRowMeasureWire( const Ctx & ctx, const WideRow & root, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
...
template <typename Ctx>
inline int64_t WideRowSaveWire( const Ctx & ctx, const WideRow & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t WideRowMeasureMessage( const WideRow * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return WideRowMeasureWire( ctx, *root, allocator, true );
}

inline int64_t WideRowSaveMessage( const WideRow * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return WideRowSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t WideRowMeasureMessage( const WideRowBuilder & builder )
{
    if ( builder.region != NULL ) { return WideRowMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return WideRowMeasureWire( ctx, *(const WideRow *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t WideRowSaveMessage( const WideRowBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return WideRowSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return WideRowSaveWire( ctx, *(const WideRow *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// WideRowLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const WideRow * WideRowLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, and then the connection's own table:
