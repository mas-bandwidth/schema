// Code generated by the schema compiler from Rows.schema. DO NOT EDIT.
...
// table RowEntriesEntry — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct RowEntriesEntry {
...
// member initializers (docs/SPEC-TABLES.md)
struct Row {
    TableMap<RowEntriesEntry> entries; // map[string(8)]Item — the sorted entry array, empty until an insert
    int32_t after = 0;
};

// table WideRowEntriesEntry — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct WideRowEntriesEntry {
    uint32_t key = 0;
    Item value;
};

// table WideRow — TABLE-wire storage: relocatable, bounded, defaults in the
// member initializers (docs/SPEC-TABLES.md)
struct WideRow {
    TableMap<WideRowEntriesEntry> entries; // map[uint32]Item — the sorted entry array, empty until an insert
    int32_t after = 0;
};
...
inline int64_t WideRowEntriesEntryMeasureMessageBody( int64_t at, const WideRowEntriesEntry & value );
inline bool WideRowEntriesEntrySaveMessageBody( TableBitWriter & w, const WideRowEntriesEntry & value );
inline bool WideRowEntriesEntryLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, WideRowEntriesEntry & value );
template <typename Ctx> inline int64_t WideRowMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const WideRow & value );
...
struct RowEntriesEntryMessageKeyRead
{
...
inline RowEntriesEntryMessageKeyRead RowEntriesEntryMessageReadKey( TableBitReader r, const TableVocabulary & vocabulary, int64_t index_bits )
{
    RowEntriesEntryMessageKeyRead out = { NULL, 0, 0, false, false, false, false, false };
    for ( ;; )
    {
        uint64_t ref = 0;
...
// WideRowEntriesEntryMessageKeyRead: the key of one entry on the message wire, before the
// slot is chosen (docs/SPEC-TABLES.md §2.8, §3.3), and the bit the entry's
// body ends at. Field order inside a body is not contractual, so this scans
// the whole body by its announced shapes rather than assuming a position.
struct WideRowEntriesEntryMessageKeyRead
{
    uint32_t key;
    int64_t end;        // the bit after the entry's own zero reference
    bool found;         // the body carried the key's id
...
inline WideRowEntriesEntryMessageKeyRead WideRowEntriesEntryMessageReadKey( TableBitReader r, const TableVocabulary & vocabulary, int64_t index_bits )
{
    WideRowEntriesEntryMessageKeyRead out = { 0, 0, false, false, false, false, false };
    for ( ;; )
    {
        uint64_t ref = 0;
        if ( !r.get( ref, vocabulary.ref_bits ) ) { out.malformed = true; return out; }
...
inline void TableReset( RowEntriesEntry & value ) { RowEntriesEntryReset( value ); }
...
MAPDEMO_TABLE_INLINE bool RowEntriesEntrySaveBody( TableWriter & w, TableIds & ids, const RowEntriesEntry & value );
MAPDEMO_TABLE_INLINE bool RowEntriesEntryLoadBody( TableReader & r, RowEntriesEntry & value );
...
inline int64_t WideRowEntriesEntryMeasureBody( TableIds & ids, const WideRowEntriesEntry & value );
MAPDEMO_TABLE_INLINE bool WideRowEntriesEntrySaveBody( TableWriter & w, TableIds & ids, const WideRowEntriesEntry & value );
MAPDEMO_TABLE_INLINE bool WideRowEntriesEntryLoadBody( TableReader & r, WideRowEntriesEntry & value );
template <typename Ctx> inline int64_t WideRowMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const WideRow & value );
...
template <typename Ctx> inline int64_t TableNodeMeasure( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const WideRow & value ) { return WideRowMeasureBody( ctx, numbering, ids, value ); }
...
static_assert( alignof( RowEntriesEntry ) <= kTableAlign, "a map entry's alignment must fit the arena's" );
static const int32_t kRowEntriesEntryKeyBound = 8; // string(8): the key's storage, and the length an insert refuses past

inline int TableEntryOrder( const RowEntriesEntry & a, const RowEntriesEntry & b )
{
...
inline int TableEntryOrder( const RowEntriesEntry & entry, TableMapKeyRef key )
{
    return TableKeyOrder( entry.key, entry.key_length, key.data, key.length );
}
inline int TableEntryOrder( const RowEntriesEntry & entry, const char * key )
{
    return TableEntryOrder( entry, TableMapKeyRef{ key, TableKeyLength( key, kRowEntriesEntryKeyBound ) } );
}
inline const char * TableEntryKey( const RowEntriesEntry & entry ) { return entry.key; } // NUL-terminated, the storage's own bytes
inline void TableEntrySetKey( RowEntriesEntry & entry, TableMapKeyRef key )
{
...
inline const Item * TableEntryFound( const RowEntriesEntry * entry ) { return entry != NULL ? &entry->value : NULL; }
inline Item * TableEntryValue( RowEntriesEntry * entry ) { return &entry->value; }
struct RowEntriesEntryEach { const char * key; decltype( TableEntryValue( (RowEntriesEntry *) NULL ) ) value; };
inline RowEntriesEntryEach TableEntryEach( RowEntriesEntry * entry ) { return RowEntriesEntryEach{ TableEntryKey( *entry ), TableEntryValue( entry ) }; }
inline void TableResetMapValue( RowEntriesEntry & value )
{
    ItemReset( value.value );
}

// RowEntriesEntryReadKey: the key, before the slot is chosen (docs/SPEC-TABLES.md §2.8).
...
inline RowEntriesEntryKeyRead RowEntriesEntryReadKey( const uint8_t * body, int64_t length, const TableIdTable * ids )
{
    RowEntriesEntryKeyRead out = { NULL, 0, false, false, false, false, false };
...
// ---- WideRowEntriesEntry: the order, the key and the value (docs/SPEC-TABLES.md §2.8) ----
//
// The four overloads the map runtime's templates reach by argument-dependent
// lookup. Nothing outside this file names them.
static_assert( alignof( WideRowEntriesEntry ) <= kTableAlign, "a map entry's alignment must fit the arena's" );

inline int TableEntryOrder( const WideRowEntriesEntry & a, const WideRowEntriesEntry & b )
{
    return TableKeyOrder( (uint64_t) a.key, (uint64_t) b.key ); // integers compare by VALUE, unsigned here
}
inline int TableEntryOrder( const WideRowEntriesEntry & entry, uint32_t key )
{
    return TableKeyOrder( (uint64_t) entry.key, (uint64_t) key );
}
inline uint32_t TableEntryKey( const WideRowEntriesEntry & entry ) { return entry.key; }
inline void TableEntrySetKey( WideRowEntriesEntry & entry, uint32_t key ) { entry.key = key; }
inline const Item * TableEntryFound( const WideRowEntriesEntry * entry ) { return entry != NULL ? &entry->value : NULL; }
inline Item * TableEntryValue( WideRowEntriesEntry * entry ) { return &entry->value; }
struct WideRowEntriesEntryEach { uint32_t key; decltype( TableEntryValue( (WideRowEntriesEntry *) NULL ) ) value; };
inline WideRowEntriesEntryEach TableEntryEach( WideRowEntriesEntry * entry ) { return WideRowEntriesEntryEach{ TableEntryKey( *entry ), TableEntryValue( entry ) }; }
inline void TableResetMapValue( WideRowEntriesEntry & value )
{
    ItemReset( value.value );
}

// WideRowEntriesEntryReadKey: the key, before the slot is chosen (docs/SPEC-TABLES.md §2.8).
// Field order inside a body is not contractual (§3), so this scans rather
// than assuming a position — and this implementation writes the key first,
// so on any wire it wrote the scan ends at the first header.
struct WideRowEntriesEntryKeyRead
{
    uint32_t key;
    bool found;         // the body carried the key's id
    bool kind_bad;      // it carried it under another kind: the MAP's event
...
inline WideRowEntriesEntryKeyRead WideRowEntriesEntryReadKey( const uint8_t * body, int64_t length, const TableIdTable * ids )
{
    WideRowEntriesEntryKeyRead out = { 0, false, false, false, false, false };
    TableReport scratch; // the scan's own framing damage is the MAP's, raised by the caller
    TableReader r( body, length, &scratch, ids );
    for ( ;; )
    {
...
inline int64_t RowEntriesEntryMeasureBodyRetain( TableRetainIds & ids, const RowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool RowEntriesEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const RowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool RowEntriesEntryLoadBodyRetain( TableReader & r, RowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
inline bool RowEntriesEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, RowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool RowLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Row & value, TableRetain * retain, const TableRetainPath & path );
inline int64_t WideRowEntriesEntryMeasureBodyRetain( TableRetainIds & ids, const WideRowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool WideRowEntriesEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const WideRowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool WideRowEntriesEntryLoadBodyRetain( TableReader & r, WideRowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
inline bool WideRowEntriesEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, WideRowEntriesEntry & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline int64_t WideRowMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const WideRow & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool WideRowSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const WideRow & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool WideRowSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const WideRow & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool WideRowLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, WideRow & value, TableRetain * retain, const TableRetainPath & path );
inline int64_t EdgeRowNamesEntryMeasureBodyRetain( TableRetainIds & ids, const EdgeRowNamesEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool EdgeRowNamesEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const EdgeRowNamesEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool EdgeRowNamesEntryLoadBodyRetain( TableReader & r, EdgeRowNamesEntry & value, TableRetain * retain, const TableRetainPath & path );
inline bool EdgeRowNamesEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, EdgeRowNamesEntry & value, TableRetain * retain, const TableRetainPath & path );
inline int64_t EdgeRowIdsEntryMeasureBodyRetain( TableRetainIds & ids, const EdgeRowIdsEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool EdgeRowIdsEntrySaveBodyRetain( TableWriter & w, TableRetainIds & ids, const EdgeRowIdsEntry & value, TableRetain * retain, const TableRetainPath & path );
MAPDEMO_TABLE_INLINE bool EdgeRowIdsEntryLoadBodyRetain( TableReader & r, EdgeRowIdsEntry & value, TableRetain * retain, const TableRetainPath & path );
inline bool EdgeRowIdsEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, EdgeRowIdsEntry & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline int64_t EdgeRowMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const EdgeRow & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool EdgeRowSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const EdgeRow & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool EdgeRowSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const EdgeRow & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool EdgeRowLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, EdgeRow & value, TableRetain * retain, const TableRetainPath & path );
...
MAPDEMO_TABLE_INLINE bool RowEntriesEntryLoadBody( TableReader & r, RowEntriesEntry & value )
{
    RowEntriesEntryReset( value ); // prefill declared defaults in place, then overlay
    for ( ;; )
    {
        uint64_t field_ref = 0;
...
inline int64_t RowMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Row & value )
{
    ...
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        // entries: a kind 14 array of kind 13 elements, ASCENDING (§2.8)
        TableMapCursor<RowEntriesEntry> order_entries = TableMapOrder( ctx, value.entries );
        if ( !order_entries.ok ) { return -1; } // the sort could not run
        if ( order_entries.count > 0 )
        {
            const uint64_t ref_entries = ids.ref_at( 58, 0xc5b2a72c0845a253ull );
            int64_t body_entries = 1 + TableLebBytes( (uint64_t) order_entries.count ); // the element kind byte and the count
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_entries = RowEntriesEntryMeasureBody( ids, *order_entries[i] );
                if ( elem_entries < 0 ) { TableMapRelease( order_entries ); return -1; }
                body_entries += TableLebBytes( (uint64_t) ( elem_entries ) ) + ( elem_entries ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_entries ) + 1 + TableLebBytes( (uint64_t) ( body_entries ) ) + ( body_entries );
        }
        TableMapRelease( order_entries );
    }
...
inline bool RowSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Row & value )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<RowEntriesEntry> order_entries = TableMapOrder( ctx, value.entries ); // entries
        if ( !order_entries.ok ) { return false; }
        if ( order_entries.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            const uint64_t ref_entries = ids.ref_at( 58, 0xc5b2a72c0845a253ull );
            int64_t body_entries = 1 + TableLebBytes( (uint64_t) order_entries.count );
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_entries = RowEntriesEntryMeasureBody( ids, *order_entries[i] );
                if ( elem_entries < 0 ) { TableMapRelease( order_entries ); return false; }
                body_entries += TableLebBytes( (uint64_t) ( elem_entries ) ) + ( elem_entries );
            }
            w.header( ref_entries, 14 ); w.putleb( (uint64_t) body_entries );
            w.put8( 13 ); w.putleb( (uint64_t) order_entries.count );
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_len_entries = RowEntriesEntryMeasureBody( ids, *order_entries[i] );
                if ( elem_len_entries < 0 ) { TableMapRelease( order_entries ); return false; }
                w.putleb( (uint64_t) elem_len_entries );
                if ( !RowEntriesEntrySaveBody( w, ids, *order_entries[i] ) ) { TableMapRelease( order_entries ); return false; }
            }
        }
        TableMapRelease( order_entries );
    }
            ...
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        RowEntriesEntryKeyRead read = RowEntriesEntryReadKey( elem_body, (int64_t) elem_len, r.ids );
                        ...
                        if ( slot == NULL ) { r.report->malformed = true; break; }
                        {
                            ...
                            RowEntriesEntryLoadBody( elem, *slot );
                        }
...
inline int64_t RowMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const Row & value )
{
    ...
    int64_t bits = 0;
    {
        ...
        if ( order_entries.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_entries = RowEntriesEntryMeasureMessageBody( at + bits, *order_entries[i] );
                if ( elem_entries < 0 ) { TableMapRelease( order_entries ); return -1; }
                bits += elem_entries; // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
        }
        TableMapRelease( order_entries );
    }
...
inline bool RowSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const Row & value )
{
    (void) ctx; (void) numbering; (void) index_bits;
    {
        ...
        if ( order_entries.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                if ( !RowEntriesEntrySaveMessageBody( w, *order_entries[i] ) ) { TableMapRelease( order_entries ); return false; }
            }
        }
        TableMapRelease( order_entries );
    }
...
inline bool RowMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at )
{
    for ( ;; )
    {
        ...
        if ( entry.id == 0xc5b2a72c0845a253ull && entry.kind == 14 && entry.elem_kind == 13 ) // entries
        {
            uint64_t n = 0;
            if ( !r.get( n, TableBitsRequired( entry.min, entry.max ) ) ) { return false; }
            n += (uint64_t) entry.min;
            at = ( at + 3 ) & ~(int64_t) 3; // at alignof( RowEntriesEntry )
            at += (int64_t) n * (int64_t) sizeof( RowEntriesEntry ); // the whole array FIRST
        ...
        switch ( entry.id )
        {
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        const RowEntriesEntryMessageKeyRead read = RowEntriesEntryMessageReadKey( r, vocabulary, index_bits );
                        ...
                        if ( !RowEntriesEntryLoadMessageBody( r, vocabulary, report, index_bits, *slot ) ) { return false; }
...
MAPDEMO_TABLE_INLINE bool WideRowEntriesEntrySaveBody( TableWriter & w, TableIds & ids, const WideRowEntriesEntry & value )
{
    if ( value.key != 0 )
    {
        w.header( ids.ref_at( 21, 0x3dc94a19365b10ecull ), 8 ); // key
...
MAPDEMO_TABLE_INLINE bool WideRowEntriesEntryLoadBody( TableReader & r, WideRowEntriesEntry & value )
{
    WideRowEntriesEntryReset( value ); // prefill declared defaults in place, then overlay
    for ( ;; )
    {
        uint64_t field_ref = 0;
        if ( !r.getleb( field_ref ) ) { r.report->malformed = true; return false; }
...
inline bool WideRowEntriesEntryLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, WideRowEntriesEntry & value )
{
    WideRowEntriesEntryReset( value );
    for ( ;; )
    {
        uint64_t ref = 0;
        if ( !r.get( ref, vocabulary.ref_bits ) ) { report->malformed = true; return false; }
...
template <typename Ctx>
inline int64_t WideRowMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const WideRow & value )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        // entries: a kind 14 array of kind 13 elements, ASCENDING (§2.8)
        TableMapCursor<WideRowEntriesEntry> order_entries = TableMapOrder( ctx, value.entries );
        if ( !order_entries.ok ) { return -1; } // the sort could not run
        if ( order_entries.count > 0 )
        {
            const uint64_t ref_entries = ids.ref_at( 58, 0xc5b2a72c0845a253ull );
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_entries = WideRowEntriesEntryMeasureBody( ids, *order_entries[i] );
                if ( elem_entries < 0 ) { TableMapRelease( order_entries ); return -1; }
                body_entries += TableLebBytes( (uint64_t) ( elem_entries ) ) + ( elem_entries ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_entries ) + 1 + TableLebBytes( (uint64_t) ( body_entries ) ) + ( body_entries );
...
template <typename Ctx>
inline bool WideRowSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const WideRow & value )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<WideRowEntriesEntry> order_entries = TableMapOrder( ctx, value.entries ); // entries
        if ( !order_entries.ok ) { return false; }
        if ( order_entries.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            const uint64_t ref_entries = ids.ref_at( 58, 0xc5b2a72c0845a253ull );
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_len_entries = WideRowEntriesEntryMeasureBody( ids, *order_entries[i] );
                if ( elem_len_entries < 0 ) { TableMapRelease( order_entries ); return false; }
                w.putleb( (uint64_t) elem_len_entries );
                if ( !WideRowEntriesEntrySaveBody( w, ids, *order_entries[i] ) ) { TableMapRelease( order_entries ); return false; }
            }
        }
        TableMapRelease( order_entries );
    }
...
template <typename Ctx>
inline bool WideRowSaveBody( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const WideRow & value )
{
    if ( !WideRowSaveBodyFields( ctx, numbering, w, ids, value ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        const uint8_t * elem_body = sub.buffer + sub.offset;
                        sub.offset += (int64_t) elem_len;
                        WideRowEntriesEntryKeyRead read = WideRowEntriesEntryReadKey( elem_body, (int64_t) elem_len, r.ids );
                        // A KEY KIND THE DECLARATION WIDENS: the map counts ONE widened (§2.8, §4)
                        if ( read.widened && !map_widened ) { map_widened = true; r.report->widened++; }
                        // THE KEY KIND IS CHECKED FIRST: a key read under another kind
                        // desynchronizes the rest of the scan, and the honest answer to a
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            WideRowEntriesEntryLoadBody( elem, *slot );
                        }
...
// its bytes and an align costs what the position says it costs.
template <typename Ctx>
inline int64_t WideRowMeasureMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, int64_t at, const WideRow & value )
{
    (void) ctx; (void) numbering; (void) index_bits;
    int64_t bits = 0;
    {
        ...
        if ( order_entries.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_entries = WideRowEntriesEntryMeasureMessageBody( at + bits, *order_entries[i] );
                if ( elem_entries < 0 ) { TableMapRelease( order_entries ); return -1; }
                bits += elem_entries; // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
        }
...
// body is self-delimiting: it is written where the VARIABLE form put an L.
template <typename Ctx>
inline bool WideRowSaveMessageBody( const Ctx & ctx, const TableNumbering & numbering, int64_t index_bits, TableBitWriter & w, const WideRow & value )
{
    (void) ctx; (void) numbering; (void) index_bits;
    {
        TableMapCursor<WideRowEntriesEntry> order_entries = TableMapOrder( ctx, value.entries ); // entries
        ...
        if ( order_entries.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                if ( !WideRowEntriesEntrySaveMessageBody( w, *order_entries[i] ) ) { TableMapRelease( order_entries ); return false; }
            }
        }
        TableMapRelease( order_entries );
    }
...
// WideRowMessageExtent: the extent WideRow's maps command on the message wire, from
// the FRAMING alone (docs/SPEC-TABLES.md §2.8, §3.3, §6.5).
inline bool WideRowMessageExtent( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & at )
{
    for ( ;; )
    {
        ...
        if ( entry.id == 0xc5b2a72c0845a253ull && entry.kind == 14 && entry.elem_kind == 13 ) // entries
        {
            ...
            if ( !r.get( n, TableBitsRequired( entry.min, entry.max ) ) ) { return false; }
            n += (uint64_t) entry.min;
            at = ( at + 3 ) & ~(int64_t) 3; // at alignof( WideRowEntriesEntry )
            at += (int64_t) n * (int64_t) sizeof( WideRowEntriesEntry ); // the whole array FIRST
            for ( uint64_t i = 0; i < n; i++ ) // then, entry by entry in key order
            {
                if ( !TableMessageSkipBody( r, vocabulary, index_bits ) ) { return false; }
            }
        ...
        switch ( entry.id )
        {
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        const WideRowEntriesEntryMessageKeyRead read = WideRowEntriesEntryMessageReadKey( r, vocabulary, index_bits );
                        if ( read.malformed ) { report->malformed = true; return false; }
                        // A KEY KIND THE DECLARATION WIDENS: the map counts ONE widened (§2.8, §4)
                        if ( read.widened && !map_widened ) { map_widened = true; report->widened++; }
                        if ( read.kind_bad )
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.key != 0 ) { bytes += TableLebBytes( ids.ref_at( 21, 0x3dc94a19365b10ecull ) ) + 1 + 8; } // key
    {
        const int32_t mark_value = ids.count;
        const uint64_t ref_value = ids.ref_at( 36, 0x7ce4fd9430e80ceaull );
        const int64_t body_value = ItemMeasureBody( ids, value.value );
...
MAPDEMO_TABLE_INLINE bool EdgeRowIdsEntrySaveBody( TableWriter & w, TableIds & ids, const EdgeRowIdsEntry & value )
{
    if ( value.key != 0 )
    {
        w.header( ids.ref_at( 21, 0x3dc94a19365b10ecull ), 9 ); // key
...
inline int64_t EdgeRowMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const EdgeRow & value )
{
    ...
    }
    {
        ...
        {
            const uint64_t ref_ids = ids.ref_at( 18, 0x2b7dea192bb7be29ull );
...
inline bool EdgeRowSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const EdgeRow & value )
{
    ...
    }
    {
        ...
        if ( order_ids.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            w.header( ref_ids, 14 ); w.putleb( (uint64_t) body_ids );
...
{
    {
        ...
        at = ( at + 3 ) & ~(int64_t) 3; // at alignof( RowEntriesEntry )
        at += (int64_t) cursor.count * (int64_t) sizeof( RowEntriesEntry ); // the whole array FIRST
...
{
    {
        ...
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3;
        const int64_t bytes = (int64_t) cursor.count * (int64_t) sizeof( RowEntriesEntry );
        if ( at + bytes > capacity ) { TableMapRelease( cursor ); return false; }
        RowEntriesEntry * placed = (RowEntriesEntry *) ( extent + at );
        at += bytes;
        dst.entries.count = cursor.count;
        dst.entries.padding = 0;
        dst.entries.entries.value = cursor.count > 0 ? (int64_t) ( (uint8_t *) placed - (const uint8_t *) &dst.entries.entries ) : 0;
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            memcpy( (void *) ( placed + i ), (const void *) cursor[i], sizeof( RowEntriesEntry ) ); // trivially copyable, by construction
        }
        TableMapRelease( cursor );
    }
    return true;
...
{
    {
        ...
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3; // at alignof( WideRowEntriesEntry )
        at += (int64_t) cursor.count * (int64_t) sizeof( WideRowEntriesEntry ); // the whole array FIRST
        TableMapRelease( cursor );
    }
    return true;
}
...
{
    {
        ...
        if ( !cursor.ok ) { return false; }
        at = ( at + 3 ) & ~(int64_t) 3;
        const int64_t bytes = (int64_t) cursor.count * (int64_t) sizeof( WideRowEntriesEntry );
        if ( at + bytes > capacity ) { TableMapRelease( cursor ); return false; }
        WideRowEntriesEntry * placed = (WideRowEntriesEntry *) ( extent + at );
        at += bytes;
        dst.entries.count = cursor.count;
        dst.entries.padding = 0;
        dst.entries.entries.value = cursor.count > 0 ? (int64_t) ( (uint8_t *) placed - (const uint8_t *) &dst.entries.entries ) : 0;
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            memcpy( (void *) ( placed + i ), (const void *) cursor[i], sizeof( WideRowEntriesEntry ) ); // trivially copyable, by construction
        }
        TableMapRelease( cursor );
    }
    return true;
...
inline Item * RowEntriesInsert( TableWorker & worker, TableMap<RowEntriesEntry> & map, const char * key )
{
    if ( key == NULL ) { return NULL; }
    const TableMapKeyRef bytes = { key, TableKeyLength( key, kRowEntriesEntryKeyBound ) };
    if ( bytes.length > kRowEntriesEntryKeyBound ) { return NULL; } // KEYS NEVER CLAMP
    RowEntriesEntry * entry = TableMapPlace( worker, map, bytes );
...
inline Item * RowEntriesFind( TableArena & arena, TableMap<RowEntriesEntry> & map, const char * key )
{
    RowEntriesEntry * found = TableMapScan( arena, map, key );
...
// entry's address is stable" false for exactly one case.
inline bool RowEntriesErase( TableArena & arena, TableMap<RowEntriesEntry> & map, const char * key )
{
    return TableMapErase( arena, map, key );
}

// EACH on the builder: INSERTION order, live entries only.
inline TableMapEach<RowEntriesEntry> RowEntriesEach( const TableArena & arena, const TableMap<RowEntriesEntry> & map )
{
...
inline int64_t RowEntriesIndexMeasure( const TableMap<RowEntriesEntry> & map )
{
    return (int64_t) TableMapIndexSlots( map.count ) * (int64_t) sizeof( int32_t );
}

inline TableMapIndex RowEntriesIndex( const TableMap<RowEntriesEntry> & map, void * storage, int64_t bytes )
{
    TableMapIndex index;
    ...
    const RowEntriesEntry * entries = map.Entries();
...
inline const Item * RowEntriesIndexFind( const TableMapIndex & index, const TableMap<RowEntriesEntry> & map, const char * key )
{
    if ( !index.good ) { return map.Find( key ); } // an index that did not build is not a wrong answer
    const RowEntriesEntry * entries = map.Entries();
    int32_t at = (int32_t) ( TableMapHash( (const void *) key, TableKeyLength( key, kRowEntriesEntryKeyBound ) ) & (uint64_t) ( index.capacity - 1 ) );
...
// ---- WideRow.entries: the builder's five and the side index (§2.8) ----
...
// allocation and the key copy, and this half is the bound and the
// const char * key's length. Nothing here mutates an entry (§2.8).
inline Item * WideRowEntriesInsert( TableWorker & worker, TableMap<WideRowEntriesEntry> & map, uint32_t key )
{
    WideRowEntriesEntry * entry = TableMapPlace( worker, map, key );
    return entry != NULL ? TableEntryValue( entry ) : NULL;
}
...
// INDEX, and that is a rule — the sort happens once, at Lock, Save or
// Cook, and every lookup that matters runs over the sorted region.
inline Item * WideRowEntriesFind( TableArena & arena, TableMap<WideRowEntriesEntry> & map, uint32_t key )
{
    WideRowEntriesEntry * found = TableMapScan( arena, map, key );
    return found != NULL ? TableEntryValue( found ) : NULL;
}
...
// resets and never reused mid-build, because reusing a slot would make "an
// entry's address is stable" false for exactly one case.
inline bool WideRowEntriesErase( TableArena & arena, TableMap<WideRowEntriesEntry> & map, uint32_t key )
{
    return TableMapErase( arena, map, key );
}

// EACH on the builder: INSERTION order, live entries only.
inline TableMapEach<WideRowEntriesEntry> WideRowEntriesEach( const TableArena & arena, const TableMap<WideRowEntriesEntry> & map )
{
    return TableMapEachOf( arena, map );
}
...
// CONTRACT of the lookup — the same value the sorted array's Find returns
// for the same key, and no allocation past the storage the caller handed in.
inline int64_t WideRowEntriesIndexMeasure( const TableMap<WideRowEntriesEntry> & map )
{
    return (int64_t) TableMapIndexSlots( map.count ) * (int64_t) sizeof( int32_t );
}

inline TableMapIndex WideRowEntriesIndex( const TableMap<WideRowEntriesEntry> & map, void * storage, int64_t bytes )
{
    TableMapIndex index;
    const int32_t slots = TableMapIndexSlots( map.count );
    if ( storage == NULL || bytes < (int64_t) slots * (int64_t) sizeof( int32_t ) ) { return index; }
    ...
    index.capacity = slots;
    for ( int32_t i = 0; i < slots; i++ ) { index.slots[i] = 0; }
    const WideRowEntriesEntry * entries = map.Entries();
    for ( int32_t i = 0; i < map.count; i++ ) // ONE PASS over the sorted array
    {
...
inline const Item * WideRowEntriesIndexFind( const TableMapIndex & index, const TableMap<WideRowEntriesEntry> & map, uint32_t key )
{
    if ( !index.good ) { return map.Find( key ); } // an index that did not build is not a wrong answer
    const WideRowEntriesEntry * entries = map.Entries();
...
// WideRowNumber: number everything WideRow POINTS AT, in first-visit order —
// the fields in declaration order, a by-value edge descended in place.
// A reference to an entry whose descent is still OPEN is a data cycle,
// named here rather than recursed away (docs/SPEC-TABLES.md §3.1).
template <typename Ctx>
inline bool WideRowNumber( const Ctx & ctx, TableNumbering & numbering, const WideRow & value )
{
    (void) ctx; (void) numbering; (void) value; // no pointers below this node
    return true;
}

// WideRowPackMeasure: the packed region bytes of everything WideRow POINTS AT.
// ONE VISIT PER NODE: `seen` carries the first-visit numbering (§3.1), so a
// node two references name is measured ONCE and packed once, and a
// reference to a node whose descent is still open is a data cycle, refused.
template <typename Ctx>
inline int64_t WideRowPackMeasure( const Ctx & ctx, TablePackMap & seen, const WideRow & value )
{
    int64_t bytes = 0;
    (void) ctx; (void) seen; (void) value; // no pointers below this node
    return bytes;
...
// WideRowPack: copy src into dst (already placed), then lay every pointee out
// depth-first behind it, in FIELD ORDER, by bump allocation.
//
// ONE NODE, ONE BODY (§6.2): `seen` holds every node already placed and
// where it landed, so a node's FIRST reference lays it out and every later
...
template <typename Ctx>
inline bool WideRowPack( const Ctx & ctx, TablePackMap & seen, const WideRow & src, WideRow & dst, uint8_t * base, int64_t capacity, int64_t & used )
{
    memcpy( (void *) &dst, (const void *) &src, sizeof( WideRow ) ); // trivially copyable, by construction
    int64_t at = 0;
...
// ---- WideRow: the variable-length life (docs/SPEC-TABLES.md §2, §6, §9) ----
//
// MUTABLE: WideRowBuilder — allocate nodes, wire them together, then Lock.
// CONST:   one packed region, root at its base. Lock produces it and Load
//          produces it, so a locked structure and a loaded one are the
//          SAME representation with one view API. There is no unlock:
//          re-editing means loading the const form into a fresh builder.
// WideRow is never held by value — a file-format-scale structure is a region
// and a root pointer, not a struct you copy.

struct WideRowBuilder
{
    TableArena arena;
    TableWorker main;      // the calling thread's allocation front
    TableRef root_ref;
    ...
    // the packed region, the wire walks' numbering, and the tool path's node
    // directory. Name your own and a profiler sees every byte under it.
    WideRowBuilder( TableAllocator allocator = TableDefaultAllocator() )
    {
        TableArenaInit( arena, allocator );
        main.arena = &arena;
    ...
    // example. The checker refuses a table named after any member here,
    // so the remaining spellings cannot collide either.
    WideRow * GetRoot() { return arena.locked ? NULL : (WideRow *) TableArenaAt( arena, (uint32_t) root_ref.value ); }
    bool Locked() const { return arena.locked; }
    const WideRow * AsConst() const { return (const WideRow *) region; }
    const uint8_t * Region() const { return region; }
    int64_t RegionBytes() const { return region_bytes; }
...
inline bool WideRowBuilder::Lock()
{
    if ( arena.locked ) { return region != NULL; }
    if ( root_ref.null() ) { return false; }
    TableArenaCtx ctx = { &arena };
    const WideRow & root = *(const WideRow *) TableArenaAt( arena, (uint32_t) root_ref.value );
    // The ROOT takes the map's first entry: it is packed at offset 0, and its
    // descent is open for the whole walk (docs/SPEC-TABLES.md §3.1).
    TablePackMap seen;
    TablePackMapInit( seen, arena.allocator );
...
// ---- WideRow on the wire: the FLAT NODE TABLE (docs/SPEC-TABLES.md §3.1) ----
//
// A pointered save writes every reachable node ONCE, into a node table under
// the reserved id 0xFFFF, and a pointer field rides as a u32 INDEX into it
// under kind 17. No pointer edge is a nesting level, so a chain's length is
...
// WideRowNodeStorage: the region bytes one record commands, or -1 for a type id
// this build cannot name — which keeps its index and reads null. A BYTE
// BUFFER's record commands its header and its bytes (docs/SPEC-TABLES.md §2.5),
// which is the one answer the record's LENGTH decides, and a blob past the
// size cap answers kTableNodeRefused with its reason (§3.1, §6.5).
...
// depth, summed from the FRAMING: N is framing and not a value, and this
// reads no field. kTableNodeRefused is a wire whose N its L cannot carry.
inline int64_t WideRowNodeStorage( uint64_t type_id, int64_t length, TableRefuseReason & reason )
{
    (void) length; // no byte buffer below this root: every node's storage is its type's
    switch ( type_id )
    {
...
// WideRowNodePlace: start one record's node's lifetime in the storage pass one
// reserved for it, holding exactly the declared defaults — a byte buffer's
// header holds its length, and its bytes come in pass two.
inline void WideRowNodePlace( uint64_t type_id, uint8_t * at, int64_t length )
{
    (void) length;
    (void) at;
    switch ( type_id )
...
// WideRowNodeRecordBytes: one record's OWN storage, before the extent its maps
// take (docs/SPEC-TABLES.md §2.8) — where a node's extent begins.
inline int64_t WideRowNodeRecordBytes( uint64_t type_id )
{
    switch ( type_id )
    {
        default: break;
...
// WideRowNodeAlloc: the TOOL's path — one record's node in the builder's arena.
// Zero is the arena's null, and it is also what a type id this build cannot
// name answers.
inline uint32_t WideRowNodeAlloc( uint64_t type_id, TableWorker & worker, int64_t length )
{
    (void) length;
    (void) worker;
    switch ( type_id )
...
inline void WideRowNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    {
        TableRefuseReason reason = count_over_length; // pass one already refused what this could refuse
        const int64_t storage = WideRowNodeStorage( type_id, r.size, reason );
        const int64_t record = storage > 0 ? WideRowNodeRecordBytes( type_id ) : 0;
        carve.at = at + record;
        carve.left = storage > record ? storage - record : 0;
    }
    nodes.carve = &carve;
...
// WideRowNodeMessageStorage: the region bytes one record commands on the message
// wire, or -1 for a type id this build cannot name. A table's is its own
// storage plus the extent its maps take; a byte buffer's is its header and
// its bytes, which is the one answer the record's LENGTH decides.
inline int64_t WideRowNodeMessageStorage( uint64_t type_id, int64_t extent, int64_t length )
{
    (void) length;
    (void) extent;
    switch ( type_id )
...
// WideRowNodeMessageExtent: step over one TABLE record's body, tallying the extent
// its maps take where its type has any (§2.8). A type this build cannot
// name is stepped over by its announced shapes and takes no extent.
inline bool WideRowNodeMessageExtent( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, int64_t & extent )
{
    extent = 0;
    (void) type_id; // no map below any node this root can name
    return TableMessageSkipBody( r, vocabulary, index_bits );
...
// WideRowNodeMessageBody: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
inline bool WideRowNodeMessageBody( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at )
{
    TableExtentCarve carve;
    carve.at = at + WideRowNodeRecordBytes( type_id );
    carve.left = 0;
    {
        // the extent this record was placed with, re-read from the framing
        TableBitReader walk = r;
        int64_t extent = 0;
        if ( !WideRowNodeMessageExtent( type_id, walk, vocabulary, index_bits, extent ) ) { report->malformed = true; return false; }
        carve.left = extent;
    }
    TableExtentCarve * const outer = nodes.carve;
    nodes.carve = &carve;
...
// reference back at it is the cycle it is (§3.1).
template <typename Ctx>
inline bool WideRowNumberFrom( const Ctx & ctx, TableNumbering & numbering, const WideRow & root )
{
    bool taken = false;
    int64_t slot = 0;
    if ( TablePackMapReach( numbering.seen, (const void *) &root, (int64_t) kTableNodeIndexRoot, taken, slot ) == NULL ) { return false; }
    return WideRowNumber( ctx, numbering, root );
}

template <typename Ctx>
inline int64_t WideRowMeasureWire( const Ctx & ctx, const WideRow & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    if ( WideRowNumberFrom( ctx, numbering, root ) )
    {
        TableIds ids;
        bytes = WideRowMeasureBody( ctx, numbering, ids, root );
        if ( bytes >= 0 )
        {
            const int64_t table = TableNodeTableMeasure( ctx, ids, numbering );
            // the FORM BYTE, the ROOT BODY — its own fields, the node table
...
template <typename Ctx>
inline int64_t WideRowSaveWire( const Ctx & ctx, const WideRow & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    if ( !WideRowNumberFrom( ctx, numbering, root ) ) { TableNumberingShutdown( numbering ); return -1; }
    TableWriter w( buffer, capacity );
    TableIds ids;
    w.put8( kTableWireForm ); // the FORM BYTE is the whole header (§3)
    // the root's own fields, then the node table's field, then the
    // terminator: a reader that gives up inside the table has already
    // decoded the ROOT'S OWN FIELDS (§3.1)
    bool ok = WideRowSaveBodyFields( ctx, numbering, w, ids, root ) && TableNodeTableSave( ctx, w, ids, numbering );
    TableNumberingShutdown( numbering );
    if ( !ok || ids.overflow ) { return -1; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the root body
    TableIdsWrite( w, ids );
    if ( w.overflow ) { return -1; } // the caller's buffer was too small
    return w.offset; // == WideRowMeasure( root )
}

inline int64_t WideRowMeasure( const WideRow * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return WideRowMeasureWire( ctx, *root, allocator );
}

inline int64_t WideRowSave( const WideRow * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return WideRowSaveWire( ctx, *root, buffer, capacity, allocator );
}

inline int64_t WideRowMeasure( const WideRowBuilder & builder )
{
    if ( builder.region != NULL ) { return WideRowMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return WideRowMeasureWire( ctx, *(const WideRow *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline int64_t WideRowSave( const WideRowBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return WideRowSave( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return WideRowSaveWire( ctx, *(const WideRow *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator );
}

// ---- WideRow on the MESSAGE wire: the batch over a region (docs/SPEC-TABLES.md §3.3) ----

// WideRowMessageBodyBits: one root body's bits at bit position `at` of the batch,
// with the numbering derived from the graph, the node table FIRST, then the
// fields, then the zero reference. Measure derives the numbering and save
// derives the same one, and nothing passes between them (§3.1).
template <typename Ctx>
inline int64_t WideRowMessageBodyBits( const Ctx & ctx, const WideRow & root, int64_t at, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bits = -1;
    if ( WideRowNumberFrom( ctx, numbering, root ) )
    {
        ...
        if ( table >= 0 )
        {
            const int64_t body = WideRowMeasureMessageBody( ctx, numbering, index_bits, at + table, root );
            bits = body < 0 ? -1 : table + body;
        }
    }
    TableNumberingShutdown( numbering );
...
template <typename Ctx>
inline bool WideRowMessageBodySave( const Ctx & ctx, const WideRow & root, TableBitWriter & w, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    bool ok = false;
    ...
    {
        const int64_t index_bits = TableBitsRequired( 0, numbering.count + 1 );
        ok = TableMessageNodeTableSave( ctx, numbering, index_bits, w ) && WideRowSaveMessageBody( ctx, numbering, index_bits, w, root );
    }
    TableNumberingShutdown( numbering );
    return ok && !w.overflow;
}
...
// root is a locked region's, `builder.AsConst()`, or a loaded one's. M above
// 256 is a refusal by name, batch_too_large, with nothing written.
inline int64_t WideRowMeasureMessages( const WideRow * const * roots, int64_t count, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( roots == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    TableRegionCtx ctx;
    ...
    {
        if ( roots[i] == NULL ) { return -1; }
        const int64_t body = WideRowMessageBodyBits( ctx, *roots[i], bits, allocator );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8;
...
inline int64_t WideRowSaveMessages( const WideRow * const * roots, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( roots == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    TableMessageBatch batch;
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( roots[i] == NULL || !WideRowMessageBodySave( ctx, *roots[i], batch.w, allocator ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == WideRowMeasureMessages( roots, count, report, allocator )
}

// WideRowMessageRecordScan: one node record's type id and the extent its maps
// take, or a blob's length, the reader left after the record. A type id
// reference of 0, one past E, or one naming anything but a kind-0 entry is
// damage, as §3.1 and §3.3 say.
inline bool WideRowMessageRecordScan( TableBitReader & r, const TableVocabulary & vocabulary, int64_t index_bits, uint64_t & type_id, int64_t & extent, int64_t & length )
{
    uint64_t type_ref = 0;
    if ( !r.get( type_ref, vocabulary.ref_bits ) ) { return false; }
    TableMessageEntry type_entry;
    ...
    return WideRowNodeMessageExtent( type_id, r, vocabulary, index_bits, extent );
}

// WideRowMessageBodyStorage: one body's node count and data bytes from the FRAMING
// alone, the reader left at the next body. The node table is walked record
// by record, a table record's body stepped over by its announced shapes and
// a blob's by its length, then the root's own fields. False is a numbering
// that could not be sized; `complete` false is a ROOT body whose own framing
...
// bodies before it were delivered, so the batch is sized through this body
// and no further (§3.3).
inline bool WideRowMessageBodyStorage( TableBitReader & r, const TableVocabulary & vocabulary, int64_t & records, int64_t & data, bool & complete )
{
    complete = true;
    records = 0;
    data = 0;
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        uint64_t type_id = 0;
        int64_t extent = 0, length = 0;
        if ( !WideRowMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { return false; }
        const int64_t storage = WideRowNodeMessageStorage( type_id, extent, length );
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
        records++;
    }
    int64_t root_extent = 0;
    if ( !WideRowMessageExtent( r, vocabulary, index_bits, root_extent ) ) { complete = false; }
    data += TableAlignUp64( TableAlignUp64( (int64_t) sizeof( WideRow ) ) + root_extent );
    return true;
}

// WideRowLoadMeasure's MESSAGE overload: the exact region bytes ONE BATCH needs,
// which is one measurement, one allocation and one bounds check for however
// many bodies ride (§3.3, §6.5). It is a scan by the announced shapes and
// reads no field value. The answer is the data bytes plus the attribution,
// one node directory a body, and -1 for a wire it cannot size: no vocabulary,
// another form, or framing that gives out.
inline int64_t WideRowLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, int64_t * attribution_bytes = NULL )
{
    TableReport ignored;
    TableMessageBatchReader br;
    const int64_t bodies = TableMessageBatchOpen( br, vocabulary, buffer, bytes, &ignored );
    ...
    for ( int64_t b = 0; b < bodies; b++ )
    {
        int64_t records = 0, body_data = 0;
        bool complete = true;
        if ( !WideRowMessageBodyStorage( br.r, vocabulary, records, body_data, complete ) ) { return -1; }
        data += body_data;
        attribution += ( records + 1 ) * (int64_t) sizeof( TableNodeDirEntry );
        if ( !complete ) { break; } // damage inside this body: the load delivers the ones before it
    }
...
// WideRowLoadMessageBodyInto: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
// owns; the ROOT's own body decodes last, so every index it carries resolves
// against a numbering already known whole.
inline bool WideRowLoadMessageBodyInto( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const WideRow * & root_out )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        uint64_t type_id = 0;
        int64_t extent = 0, length = 0;
        if ( !WideRowMessageRecordScan( r, vocabulary, index_bits, type_id, extent, length ) ) { out->malformed = true; return false; }
        const int64_t storage = WideRowNodeMessageStorage( type_id, extent, length );
        directory[k + 1].type_id = type_id;
        if ( storage <= 0 )
        {
        ...
        if ( used + storage > region_bytes ) { out->malformed = true; return false; }
        directory[k + 1].offset = (uint64_t) used;
        WideRowNodePlace( type_id, region + used, length );
        used += storage;
    }
    const int64_t fields_start = r.offset;
    int64_t root_extent = 0;
    {
        TableBitReader walk = r;
        if ( !WideRowMessageExtent( walk, vocabulary, index_bits, root_extent ) ) { out->malformed = true; return false; }
    }
    const int64_t root_bytes = TableAlignUp64( TableAlignUp64( (int64_t) sizeof( WideRow ) ) + root_extent );
    if ( used + root_bytes > region_bytes ) { out->malformed = true; return false; }
    directory[0].offset = (uint64_t) used;
    directory[0].type_id = 0xb413964e3571a316ull;
    WideRow * root = new ( region + used ) WideRow; // lifetime only: LoadMessageBody's first act is WideRowReset
    WideRowReset( *root );
    root_out = root;
    TableExtentCarve root_carve;
    root_carve.at = region + used + TableAlignUp64( (int64_t) sizeof( WideRow ) );
    root_carve.left = root_extent;
    used += root_bytes;
    nodes.good = true;
    out->unknown += unknown_records;
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !WideRowNodeMessageBody( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return WideRowLoadMessageBody( r, vocabulary, out, nodes, index_bits, *root );
}

// WideRowLoadMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
// no reference, so there is no depth cap and no visited set. NULL roots
// beyond count are not bodies.
inline bool WideRowLoadMessages( const WideRow ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !WideRowLoadMessageBodyInto( br.r, vocabulary, out, region, region_bytes, used, roots[b] ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// WideRowLoadMeasure: the exact region bytes a wire buffer will need, and it is
// ONE SCAN — a record's type id gives its storage size, its length gives the
// next record — reading no field value at all, so the caller owns the
// allocation and can refuse a number it did not expect (§6.5).
//
...
inline int64_t WideRowLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = WideRowNodeStorage( type_id, length, reason );
        if ( storage == kTableNodeRefused ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; } // an N the record's framing cannot carry, or a blob past the cap (§2.8, §2.9, §3.1)
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
    }
    int64_t attribution = ( records + 1 ) * (int64_t) sizeof( TableNodeDirEntry );
...
// WideRowLoad: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const WideRow * WideRowLoad( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    const uint8_t * const wire = wire_file + 1;
    const int64_t wire_bytes = body_bytes;
    if ( region == NULL || region_bytes < (int64_t) sizeof( WideRow ) ) { out->malformed = true; return NULL; }
    if ( ( ( (uintptr_t) region ) & ( kTableAlign - 1 ) ) != 0 ) { out->malformed = true; return NULL; }
    memset( region, 0, (size_t) region_bytes );
    uint64_t type_id = 0;
    const uint8_t * body = NULL;
    ...
    directory[0].offset = 0; // position 0 is the ROOT, at offset 0 (§6.3)
    directory[0].type_id = 0xb413964e3571a316ull;
    WideRow * root = new ( region ) WideRow; // lifetime only: LoadBody's first act is WideRowReset
    WideRowReset( *root );
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                WideRowNodeBody( type_id, sub, nodes, region + directory[k + 1].offset );
            }
            k++;
        }
    }
    ...
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    WideRowLoadBody( r, nodes, *root );
    return root;
}

// WideRowLoadBuilder: the TOOL's path — the same tolerant decode into a fresh
// builder, so loaded data can be edited and locked again. The numbering is
// the same one; what differs is where a node lives and therefore what a
// resolved slot holds — an arena offset here, a self-relative delta there.
inline bool WideRowLoadBuilder( WideRowBuilder & builder, const uint8_t * wire_file, int64_t wire_file_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    TableIdTable ids_table;
    ...
    nodes.worker = &builder.main; // and a map's entries and a list's elements are the arena's, not a node extent's (§2.8, §2.9)
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            uint32_t at = WideRowNodeAlloc( type_id, builder.main, length );
            if ( at == 0 )
            {
                unknown_records++;
                directory[k + 1].offset = kTableNodeAbsent;
    ...
    for ( ;; )
    {
        ...
        switch ( entry.id )
        {
            ...
            case 0x7ce4fd9430e80ceaull: // value
            {
                ...
                TableRetainDiscardField( retain, path, 1 );
                if ( !ItemLoadMessageBodyRetain( r, vocabulary, report, index_bits, value.value, retain, TableRetainStepInto( path, 1, (uint32_t) ( 0 ) ) ) ) { return false; }
                break;
            }
            default:
                report->unknown++;
...
inline int64_t RowMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Row & value, TableRetain * retain, const TableRetainPath & path )
{
    ...
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        if ( order_entries.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_entries = RowEntriesEntryMeasureBodyRetain( ids, *order_entries[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_entries < 0 ) { TableMapRelease( order_entries ); return -1; }
                body_entries += TableLebBytes( (uint64_t) ( elem_entries ) ) + ( elem_entries ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_entries ) + 1 + TableLebBytes( (uint64_t) ( body_entries ) ) + ( body_entries );
...
inline bool RowSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Row & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    {
        ...
        if ( order_entries.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_len_entries = RowEntriesEntryMeasureBodyRetain( ids, *order_entries[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_len_entries < 0 ) { TableMapRelease( order_entries ); return false; }
                w.putleb( (uint64_t) elem_len_entries );
                if ( !RowEntriesEntrySaveBodyRetain( w, ids, *order_entries[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) ) ) { TableMapRelease( order_entries ); return false; }
            }
        }
        TableMapRelease( order_entries );
    }
            ...
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            RowEntriesEntryLoadBodyRetain( elem, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) );
                        }
                        last_key = read.key; last_length = read.length; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
            ...
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; }
                        if ( !RowEntriesEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) ) ) { return false; }
                        if ( r.offset != read.end ) { report->malformed = true; return false; } // the scan and the decode disagree about where the entry ends
                        last_key = read.key; last_length = read.length; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
...
template <typename Ctx>
inline int64_t WideRowMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const WideRow & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        if ( order_entries.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_entries = WideRowEntriesEntryMeasureBodyRetain( ids, *order_entries[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_entries < 0 ) { TableMapRelease( order_entries ); return -1; }
                body_entries += TableLebBytes( (uint64_t) ( elem_entries ) ) + ( elem_entries ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_entries ) + 1 + TableLebBytes( (uint64_t) ( body_entries ) ) + ( body_entries );
...
template <typename Ctx>
inline bool WideRowSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const WideRow & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<WideRowEntriesEntry> order_entries = TableMapOrder( ctx, value.entries ); // entries
        ...
        if ( order_entries.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            for ( int32_t i = 0; i < order_entries.count; i++ )
            {
                const int64_t elem_len_entries = WideRowEntriesEntryMeasureBodyRetain( ids, *order_entries[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_len_entries < 0 ) { TableMapRelease( order_entries ); return false; }
                w.putleb( (uint64_t) elem_len_entries );
                if ( !WideRowEntriesEntrySaveBodyRetain( w, ids, *order_entries[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) ) ) { TableMapRelease( order_entries ); return false; }
            }
        }
        TableMapRelease( order_entries );
    }
...
template <typename Ctx>
inline bool WideRowSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const WideRow & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !WideRowSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            WideRowEntriesEntryLoadBodyRetain( elem, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) );
                        }
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
            ...
            case 0xc5b2a72c0845a253ull: // entries
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; }
                        if ( !WideRowEntriesEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) ) ) { return false; }
                        if ( r.offset != read.end ) { report->malformed = true; return false; } // the scan and the decode disagree about where the entry ends
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
...
template <typename Ctx>
inline int64_t EdgeRowMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const EdgeRow & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        if ( order_names.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_names.count; i++ )
            {
                const int64_t elem_names = EdgeRowNamesEntryMeasureBodyRetain( ids, *order_names[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_names < 0 ) { TableMapRelease( order_names ); return -1; }
                body_names += TableLebBytes( (uint64_t) ( elem_names ) ) + ( elem_names ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_names ) + 1 + TableLebBytes( (uint64_t) ( body_names ) ) + ( body_names );
    ...
    }
    {
        ...
        if ( order_ids.count > 0 )
        {
            ...
            for ( int32_t i = 0; i < order_ids.count; i++ )
            {
                const int64_t elem_ids = EdgeRowIdsEntryMeasureBodyRetain( ids, *order_ids[i], retain, TableRetainStepInto( path, 1, (uint32_t) ( i ) ) );
                if ( elem_ids < 0 ) { TableMapRelease( order_ids ); return -1; }
                body_ids += TableLebBytes( (uint64_t) ( elem_ids ) ) + ( elem_ids ); // BUT THE ENTRY ALWAYS RIDES: identity here is the key
            }
            bytes += TableLebBytes( ref_ids ) + 1 + TableLebBytes( (uint64_t) ( body_ids ) ) + ( body_ids );
...
template <typename Ctx>
inline bool EdgeRowSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const EdgeRow & value, TableRetain * retain, const TableRetainPath & path )
{
    (void) ctx; (void) numbering;
    {
        TableMapCursor<EdgeRowNamesEntry> order_names = TableMapOrder( ctx, value.names ); // names
        ...
        if ( order_names.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            for ( int32_t i = 0; i < order_names.count; i++ )
            {
                const int64_t elem_len_names = EdgeRowNamesEntryMeasureBodyRetain( ids, *order_names[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) );
                if ( elem_len_names < 0 ) { TableMapRelease( order_names ); return false; }
                w.putleb( (uint64_t) elem_len_names );
                if ( !EdgeRowNamesEntrySaveBodyRetain( w, ids, *order_names[i], retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) ) ) { TableMapRelease( order_names ); return false; }
            }
        }
        TableMapRelease( order_names );
    }
    {
        ...
        if ( order_ids.count > 0 ) // an EMPTY map elides, the by-value rule (§3)
        {
            ...
            for ( int32_t i = 0; i < order_ids.count; i++ )
            {
                const int64_t elem_len_ids = EdgeRowIdsEntryMeasureBodyRetain( ids, *order_ids[i], retain, TableRetainStepInto( path, 1, (uint32_t) ( i ) ) );
                if ( elem_len_ids < 0 ) { TableMapRelease( order_ids ); return false; }
                w.putleb( (uint64_t) elem_len_ids );
                if ( !EdgeRowIdsEntrySaveBodyRetain( w, ids, *order_ids[i], retain, TableRetainStepInto( path, 1, (uint32_t) ( i ) ) ) ) { TableMapRelease( order_ids ); return false; }
            }
        }
        TableMapRelease( order_ids );
    }
...
template <typename Ctx>
inline bool EdgeRowSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const EdgeRow & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !EdgeRowSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0xafb728fff268814full: // names
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            EdgeRowNamesEntryLoadBodyRetain( elem, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) );
                        }
                        last_key = read.key; last_length = read.length; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
            ...
            case 0x2b7dea192bb7be29ull: // ids
            {
                ...
                if ( body_len >= 2 )
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        {
                            TableReader elem( elem_body, (int64_t) elem_len, r.report, r.ids );
                            EdgeRowIdsEntryLoadBodyRetain( elem, *slot, retain, TableRetainStepInto( path, 1, (uint32_t) ( fill.map->count - 1 ) ) );
                        }
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
            ...
            case 0xafb728fff268814full: // names
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; }
                        if ( !EdgeRowNamesEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *slot, retain, TableRetainStepInto( path, 0, (uint32_t) ( fill.map->count - 1 ) ) ) ) { return false; }
                        if ( r.offset != read.end ) { report->malformed = true; return false; } // the scan and the decode disagree about where the entry ends
                        last_key = read.key; last_length = read.length; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
            ...
            case 0x2b7dea192bb7be29ull: // ids
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < count; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; }
                        if ( !EdgeRowIdsEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *slot, retain, TableRetainStepInto( path, 1, (uint32_t) ( fill.map->count - 1 ) ) ) ) { return false; }
                        if ( r.offset != read.end ) { report->malformed = true; return false; } // the scan and the decode disagree about where the entry ends
                        last_key = read.key; // the WIRE keys of the entries that LAND
                        landed = true;
                    }
...
// RowNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool RowNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + RowNodeRecordBytes( type_id );
    carve.left = 0;
...
inline bool RowLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Row * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !RowNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return RowLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// RowLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool RowLoadRetainMessages( const Row ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !RowLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// index is the region directory's, which Load fills from the wire's framing
// and nothing afterwards renumbers.
inline void WideRowNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    // the node's own EXTENT, where its lists' and maps' arrays are carved
    // from, PRE-ORDER as the bodies decode (docs/SPEC-TABLES.md §2.8, §2.9).
    // The tool's path carries a worker instead: there the arrays are the
...
// WideRowLoadRetain: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
...
// (docs/SPEC-TABLES.md §6.6). It is Load's own path and nothing else: the
// reader's data is exactly what it would have been with retention off.
inline const WideRow * WideRowLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                WideRowNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
    ...
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    WideRowLoadBodyRetain( r, nodes, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
    return root;
}

// WideRowNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
// index is this body's own directory position, which the batch's load fills
// from the framing and nothing afterwards renumbers.
inline bool WideRowNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + WideRowNodeRecordBytes( type_id );
    carve.left = 0;
...
// WideRowLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
// each body carries its own node directory inside that one region and a
// record's first step is an index into it (docs/SPEC-TABLES.md §3.3, §6.6).
inline bool WideRowLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const WideRow * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !WideRowNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return WideRowLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// WideRowLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
// SaveRetain from `roots[k]` takes `retains[k]` and is the VARIABLE form's own
// pair unchanged: retention writing FORM 2 refuses by name (§3.3).
inline bool WideRowLoadRetainMessages( const WideRow ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !WideRowLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// WideRowMeasureRetain and WideRowSaveRetain: the pair, with the retained tail in
// every body it belongs to (docs/SPEC-TABLES.md §6.6). They drop the same
// records under the same walk, so Measure's answer is the size the save
// writes even where a record could not be placed.
template <typename Ctx>
inline int64_t WideRowMeasureWireRetain( const Ctx & ctx, const WideRow & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( WideRowNumberFrom( ctx, numbering, root ) )
    {
        TableRetainIds ids( retain );
        if ( retain != NULL ) { retain->id_used = 0; } // one walk fills the list, and the save's own walk refills it
        bytes = WideRowMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
            const int64_t table = TableNodeTableMeasureRetain( ctx, ids, numbering, retain, retain_measure );
            bytes = table < 0 || ids.overflow ? -1 : 1 + bytes + table + TableRetainIdsBytes( ids );
...
template <typename Ctx>
inline int64_t WideRowSaveWireRetain( const Ctx & ctx, const WideRow & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, TableDefaultAllocator() );
    auto retain_measure = []( const Ctx & c, const TableNumbering & nn, TableRetainIds & ii, uint64_t type_id, const void * node, TableRetain * rt ) -> int64_t
    ...
    // field: a retained field is one of the root's own values, and the tail
    // is pinned before the large and damage-prone part (§6.6, §3.1)
    bool ok = WideRowSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
              TableNodeTableSaveRetain( ctx, w, ids, numbering, retain, retain_measure, retain_save );
    TableNumberingShutdown( numbering );
    if ( !ok || ids.overflow ) { return -1; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the root body
...
inline int64_t WideRowMeasureRetain( const WideRow * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return WideRowMeasureWireRetain( ctx, *root, retain, allocator );
}
...
// else. A surface that let a caller retain, save and never find out would
// be a promise it could not check.
inline int64_t WideRowSaveRetain( const WideRow * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return WideRowSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// WideRowSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
// (docs/SPEC-TABLES.md §3.3). It is a MISUSE refusal on §6.6's own
// precedent and never a silent drop, and the two answers are named: a
// caller that must carry unknowns across a rewrite writes the VARIABLE form,
// which carries its own table and takes §6.6 unchanged, and a RELAY
// forwards the sending peer's announcement and its batch bytes verbatim.
template <typename... Args>
inline int64_t WideRowSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "WideRow: a form 2 writer names entries through slots of a vocabulary the compiler settled, and a retained id is one this build's closure does not contain, so it has neither a slot nor an announced shape. Retention writing the MESSAGE form is refused by name (docs/SPEC-TABLES.md §3.3). Write the VARIABLE form, which carries its own table and takes §6.6 unchanged, or relay the sender's announcement and batch bytes verbatim." );
    return -1;
}
...
// index is the region directory's, which Load fills from the wire's framing
// and nothing afterwards renumbers.
inline void EdgeRowNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    // the node's own EXTENT, where its lists' and maps' arrays are carved
    // from, PRE-ORDER as the bodies decode (docs/SPEC-TABLES.md §2.8, §2.9).
    // The tool's path carries a worker instead: there the arrays are the
...
// EdgeRowLoadRetain: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
...
// (docs/SPEC-TABLES.md §6.6). It is Load's own path and nothing else: the
// reader's data is exactly what it would have been with retention off.
inline const EdgeRow * EdgeRowLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                EdgeRowNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
    ...
    root_carve.left = root_extent;
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    EdgeRowLoadBodyRetain( r, nodes, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
    return root;
}

// EdgeRowNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
// index is this body's own directory position, which the batch's load fills
// from the framing and nothing afterwards renumbers.
inline bool EdgeRowNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + EdgeRowNodeRecordBytes( type_id );
    carve.left = 0;
...
// EdgeRowLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
// each body carries its own node directory inside that one region and a
// record's first step is an index into it (docs/SPEC-TABLES.md §3.3, §6.6).
inline bool EdgeRowLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const EdgeRow * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !EdgeRowNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return EdgeRowLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// EdgeRowLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
// SaveRetain from `roots[k]` takes `retains[k]` and is the VARIABLE form's own
// pair unchanged: retention writing FORM 2 refuses by name (§3.3).
inline bool EdgeRowLoadRetainMessages( const EdgeRow ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !EdgeRowLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// EdgeRowMeasureRetain and EdgeRowSaveRetain: the pair, with the retained tail in
// every body it belongs to (docs/SPEC-TABLES.md §6.6). They drop the same
// records under the same walk, so Measure's answer is the size the save
// writes even where a record could not be placed.
template <typename Ctx>
inline int64_t EdgeRowMeasureWireRetain( const Ctx & ctx, const EdgeRow & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( EdgeRowNumberFrom( ctx, numbering, root ) )
    {
        TableRetainIds ids( retain );
        if ( retain != NULL ) { retain->id_used = 0; } // one walk fills the list, and the save's own walk refills it
        bytes = EdgeRowMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
            const int64_t table = TableNodeTableMeasureRetain( ctx, ids, numbering, retain, retain_measure );
            bytes = table < 0 || ids.overflow ? -1 : 1 + bytes + table + TableRetainIdsBytes( ids );
...
template <typename Ctx>
inline int64_t EdgeRowSaveWireRetain( const Ctx & ctx, const EdgeRow & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, TableDefaultAllocator() );
    auto retain_measure = []( const Ctx & c, const TableNumbering & nn, TableRetainIds & ii, uint64_t type_id, const void * node, TableRetain * rt ) -> int64_t
    ...
    // field: a retained field is one of the root's own values, and the tail
    // is pinned before the large and damage-prone part (§6.6, §3.1)
    bool ok = EdgeRowSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
              TableNodeTableSaveRetain( ctx, w, ids, numbering, retain, retain_measure, retain_save );
    TableNumberingShutdown( numbering );
    if ( !ok || ids.overflow ) { return -1; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the root body
...
inline int64_t EdgeRowMeasureRetain( const EdgeRow * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return EdgeRowMeasureWireRetain( ctx, *root, retain, allocator );
}
...
// else. A surface that let a caller retain, save and never find out would
// be a promise it could not check.
inline int64_t EdgeRowSaveRetain( const EdgeRow * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return EdgeRowSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// EdgeRowSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
// (docs/SPEC-TABLES.md §3.3). It is a MISUSE refusal on §6.6's own
// precedent and never a silent drop, and the two answers are named: a
// caller that must carry unknowns across a rewrite writes the VARIABLE form,
// which carries its own table and takes §6.6 unchanged, and a RELAY
// forwards the sending peer's announcement and its batch bytes verbatim.
template <typename... Args>
inline int64_t EdgeRowSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "EdgeRow: a form 2 writer names entries through slots of a vocabulary the compiler settled, and a retained id is one this build's closure does not contain, so it has neither a slot nor an announced shape. Retention writing the MESSAGE form is refused by name (docs/SPEC-TABLES.md §3.3). Write the VARIABLE form, which carries its own table and takes §6.6 unchanged, or relay the sender's announcement and batch bytes verbatim." );
    return -1;
}
...
// WideRowOpen: match the header and POINT. On a match the bytes ARE what this
// build wrote, in this build's layout and this build's byte order, so there
// is nothing to validate and nothing to fix up and the root comes back as it
// lies. On ANY refusal it returns NULL and NAMES the refusal in the caller's
// TableRefuseReason, the first failing clause in §7's order (a wrong build
...
// touching nothing at open rather than a separate mechanism.
//
// A REFERENCE INSIDE THE REGION IS DEREFERENCED THROUGH WideRowAt: the slot holds
// the signed self-relative byte delta of §6.3, so a deref is one add and
// needs no base pointer, a whole region relocates by plain memcpy, and a
// delta of zero is null.
//
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const WideRow * WideRowOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
...
inline void RowEntriesEntryCookBody( uint8_t * at, const RowEntriesEntry & value, TableByteOrder order )
{
    ...
    ItemCookBody( at + 16, value.value, order );
}
...
template <typename Ctx> inline bool RowCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const Row & value, TableByteOrder order )
{
    ...
    { // entries
        ...
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            RowEntriesEntryCookBody( array + i * 20, *cursor[i], order );
        }
        TableMapRelease( cursor );
    }
    return true;
...
template <typename Ctx> inline bool WideRowCookExtent( const Ctx & ctx, const TableCookRegion & region, uint8_t * extent, int64_t & at, uint8_t * record, const WideRow & value, TableByteOrder order )
{
    ...
    { // entries
        ...
        for ( int32_t i = 0; i < cursor.count; i++ )
        {
            WideRowEntriesEntryCookBody( array + i * 8, *cursor[i], order );
        }
        TableMapRelease( cursor );
    }
    return true;
...
// WideRowCookLayout: the tool's own Layout (docs/SPEC-TABLES.md §7.2) over one
// numbering — the root at zero, then every node in index order at
// align_up( offset, alignof ) for its OWN type, no slack between them, the
// data length rounded to the greatest alignment among them and never below
// eight. The offsets go into the region's table when it has one, and are only
...
// WideRowCookMeasureFrom: the whole cooked file's bytes for one graph — the header,
// the data part and the attribution part (§7.1). IT DEPENDS ON THE VALUE,
// because the answer is the numbering: the depth-first walk of §3.1 is run
// here and run again by the write, and neither carries the other's (§7.6). A
// data cycle is refused by the walk and answers -1.
template <typename Ctx>
inline int64_t WideRowCookMeasureFrom( const Ctx & ctx, const WideRow & root, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    TableCookRegion region;
    int64_t bytes = -1;
    if ( WideRowNumberFrom( ctx, numbering, root ) && WideRowCookLayout( ctx, root, numbering, region ) )
    {
        const int64_t data_offset = ( kTableCookHeaderBytes + region.align - 1 ) & ~( region.align - 1 );
        bytes = data_offset + region.bytes + region.count * (int64_t) sizeof( TableNodeDirEntry );
    }
...
// WideRowCookFrom: write one cooked file of a pointered graph, in the byte order
// the caller names. The bytes are `schema cook`'s, byte for byte (§7.6).
//
// THE CALLER OWNS THE OUTPUT and nothing is allocated toward it. What is
// allocated is the numbering — the identity map, the entry array and one
...
// mistake them for a cook.
template <typename Ctx>
inline bool WideRowCookFrom( const Ctx & ctx, const WideRow & root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator )
{
    if ( out == NULL ) { return false; }
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    ...
    if ( ok )
    {
        ...
        if ( ok )
        {
            ...
            // the DATA part: the root at the region's base, then every numbered
            // node at the offset the layout gave it, each through its own writer
            ok = WideRowCookNode( ctx, region, region.base, root, order );
            // the ATTRIBUTION part: the node directory (§6.3), one entry per node
            // in index order, for `schema cook-check`
            uint8_t * entry = raw + data_offset + region.bytes;
            table_cook_put( entry, 0, 8, order );
            table_cook_put( entry + 8, 0xb413964e3571a316ull, 8, order ); // the root: fnv1a64( "WideRow" )
            for ( int64_t k = 0; k < numbering.count; k++ )
            {
                entry += sizeof( TableNodeDirEntry );
                table_cook_put( entry, (uint64_t) region.offsets[k + 1], 8, order );
...
// WideRowCookMeasure / WideRowCook over a REGION root — a locked builder's AsConst, a
// region WideRowLoad produced, or an opened cook — with the pair the numbering
// allocates through as an optional last argument, as the wire's own entries
// take it (§13.9).
inline int64_t WideRowCookMeasure( const WideRow * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return WideRowCookMeasureFrom( ctx, *root, allocator );
}

inline bool WideRowCook( const WideRow * root, void * out, uint64_t capacity, TableByteOrder order, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return false; }
    TableRegionCtx ctx;
    return WideRowCookFrom( ctx, *root, out, capacity, order, allocator );
}

// and over a BUILDER, locked or not: the builder's own pair, and the arena
// encoding while it is still mutable (§6.3).
inline int64_t WideRowCookMeasure( const WideRowBuilder & builder )
{
    if ( builder.region != NULL ) { return WideRowCookMeasure( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
    return WideRowCookMeasureFrom( ctx, *(const WideRow *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator );
}

inline bool WideRowCook( const WideRowBuilder & builder, void * out, uint64_t capacity, TableByteOrder order )
{
    if ( builder.region != NULL ) { return WideRowCook( builder.AsConst(), out, capacity, order, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return false; } // the root allocation failed
    TableArenaCtx ctx = { &builder.arena };
...
static_assert( __is_trivially_copyable( RowEntriesEntry ), "RowEntriesEntry must stay relocatable" );
static_assert( __is_standard_layout( RowEntriesEntry ), "RowEntriesEntry must stay standard-layout for offsetof" );
...
static_assert( __is_trivially_copyable( WideRowEntriesEntry ), "WideRowEntriesEntry must stay relocatable" );
...
// rather than assumed.
static_assert( sizeof( RowEntriesEntry ) == 20, "RowEntriesEntry's sizeof moved: the build version was taken over 20, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( RowEntriesEntry ) == 4, "RowEntriesEntry's alignof moved: the build version was taken over 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( RowEntriesEntry, key ) == 0, "RowEntriesEntry's field key moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( RowEntriesEntry, value ) == 16, "RowEntriesEntry's field value moved: the build version was taken over offset 16 (docs/SPEC-TABLES.md §20.3)" );
...
static_assert( offsetof( Row, entries ) == 0, "Row's field entries moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( Row, after ) == 16, "Row's field after moved: the build version was taken over offset 16 (docs/SPEC-TABLES.md §20.3)" );
static_assert( sizeof( WideRowEntriesEntry ) == 8, "WideRowEntriesEntry's sizeof moved: the build version was taken over 8, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
static_assert( alignof( WideRowEntriesEntry ) == 4, "WideRowEntriesEntry's alignof moved: the build version was taken over 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( WideRowEntriesEntry, key ) == 0, "WideRowEntriesEntry's field key moved: the build version was taken over offset 0 (docs/SPEC-TABLES.md §20.3)" );
static_assert( offsetof( WideRowEntriesEntry, value ) == 4, "WideRowEntriesEntry's field value moved: the build version was taken over offset 4 (docs/SPEC-TABLES.md §20.3)" );
static_assert( sizeof( WideRow ) == 24, "WideRow's sizeof moved: the build version was taken over 24, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
...
inline const TableTypeInfo * RowEntriesEntryTableType();
...
extern const TableTypeInfo RowEntriesEntryTableInfo;
...
inline const TableFieldInfo RowTableFields[] = {
    { "entries", "entries", "map[string(8)]Item", 0xc5b2a72c0845a253ull, 13, true, false, NULL, NULL, true, false, 0, (uint32_t) offsetof( Row, entries ), (uint32_t) sizeof( RowEntriesEntry ), (uint32_t) offsetof( Row, entries.count ), 0xffffffffu, &RowEntriesEntryTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, []( TableWorker & worker, void * slot, const char * key, int32_t key_length, int64_t ) -> void * { if ( key == NULL ) { return NULL; } return (void *) TableMapPlace( worker, *(TableMap<RowEntriesEntry> *) slot, TableMapKeyRef{ key, key_length } ); }, "", TableDocNone, 0, NULL },
...
inline const TableFieldInfo WideRowTableFields[] = {
    { "entries", "entries", "map[uint32]Item", 0xc5b2a72c0845a253ull, 13, true, false, NULL, NULL, true, false, 0, (uint32_t) offsetof( WideRow, entries ), (uint32_t) sizeof( WideRowEntriesEntry ), (uint32_t) offsetof( WideRow, entries.count ), 0xffffffffu, &WideRowEntriesEntryTableInfo, false, 0.0, 0.0, 0, NULL, -1, NULL, NULL, NULL, NULL, NULL, NULL, []( TableWorker & worker, void * slot, const char *, int32_t, int64_t key_value ) -> void * { return (void *) TableMapPlace( worker, *(TableMap<WideRowEntriesEntry> *) slot, (uint32_t) key_value ); }, "", TableDocNone, 0, NULL },
...
// WideRow in and out of a JSON text (docs/SPEC-TABLES.md §16.7): read into a
// builder, written from a region's const root. A node named more than once
// carries `&node` in the text. Defined in RowsTable.cpp; link it to use them.
