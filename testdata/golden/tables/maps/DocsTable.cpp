...
bool DocsFromJson( DocsBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, DocsTableType(), text, bytes, report );
}

int64_t DocsToJsonMeasure( const Docs * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DocsTableType(), NULL, 0, allocator );
}

int64_t DocsToJson( const Docs * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DocsTableType(), buffer, capacity, allocator );
}
