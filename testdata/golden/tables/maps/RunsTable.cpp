...
bool RunsFromJson( RunsBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, RunsTableType(), text, bytes, report );
}

int64_t RunsToJsonMeasure( const Runs * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, RunsTableType(), NULL, 0, allocator );
}

int64_t RunsToJson( const Runs * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, RunsTableType(), buffer, capacity, allocator );
}
