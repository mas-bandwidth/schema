// Code generated by the schema compiler from Trails.schema. DO NOT EDIT.
...
bool TrailsFromJson( TrailsBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    Trails * root = builder.GetRoot();
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, TrailsTableType(), text, bytes, report );
}

int64_t TrailsToJsonMeasure( const Trails * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TrailsTableType(), NULL, 0, allocator );
}

int64_t TrailsToJson( const Trails * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TrailsTableType(), buffer, capacity, allocator );
}
