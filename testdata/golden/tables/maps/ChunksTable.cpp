// Code generated by the schema compiler from Chunks.schema. DO NOT EDIT.
...
bool ChunksFromJson( ChunksBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    Chunks * root = builder.GetRoot();
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, ChunksTableType(), text, bytes, report );
}

int64_t ChunksToJsonMeasure( const Chunks * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ChunksTableType(), NULL, 0, allocator );
}

int64_t ChunksToJson( const Chunks * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ChunksTableType(), buffer, capacity, allocator );
}
