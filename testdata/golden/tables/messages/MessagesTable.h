...
constexpr uint64_t UserFixedHash = 0x353c49c5bfd611e4ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t UserFixedVocab[] = {
...
constexpr int64_t UserFixedVocabBytes = (int64_t) sizeof( UserFixedVocab );
...
{
    return 1 + 4 + UserFixedVocabBytes + count * UserFixedRecordBytes;
}
...
inline int64_t UserFixedSave( const User * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) UserFixedVocabBytes );
    memcpy( buffer + 5, UserFixedVocab, (size_t) UserFixedVocabBytes );
    uint8_t * at = buffer + 5 + UserFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != UserFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, UserFixedVocab, (int32_t) UserFixedVocabBytes, UserFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t ScriptFixedHash = 0xd588b514f822228full; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 3 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t ScriptFixedVocab[] = {
...
constexpr int64_t ScriptFixedVocabBytes = (int64_t) sizeof( ScriptFixedVocab );
...
{
    return 1 + 4 + ScriptFixedVocabBytes + count * ScriptFixedRecordBytes;
}
...
inline int64_t ScriptFixedSave( const Script * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) ScriptFixedVocabBytes );
    memcpy( buffer + 5, ScriptFixedVocab, (size_t) ScriptFixedVocabBytes );
    uint8_t * at = buffer + 5 + ScriptFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != ScriptFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, ScriptFixedVocab, (int32_t) ScriptFixedVocabBytes, ScriptFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t SelectionFixedHash = 0x46becbf37e590518ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 7 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t SelectionFixedVocab[] = {
...
constexpr int64_t SelectionFixedVocabBytes = (int64_t) sizeof( SelectionFixedVocab );
...
{
    return 1 + 4 + SelectionFixedVocabBytes + count * SelectionFixedRecordBytes;
}
...
inline int64_t SelectionFixedSave( const Selection * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) SelectionFixedVocabBytes );
    memcpy( buffer + 5, SelectionFixedVocab, (size_t) SelectionFixedVocabBytes );
    uint8_t * at = buffer + 5 + SelectionFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != SelectionFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, SelectionFixedVocab, (int32_t) SelectionFixedVocabBytes, SelectionFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t RemoveTextFixedHash = 0x2229751598fd8bffull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 8 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t RemoveTextFixedVocab[] = {
...
constexpr int64_t RemoveTextFixedVocabBytes = (int64_t) sizeof( RemoveTextFixedVocab );
...
{
    return 1 + 4 + RemoveTextFixedVocabBytes + count * RemoveTextFixedRecordBytes;
}
...
inline int64_t RemoveTextFixedSave( const RemoveText * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) RemoveTextFixedVocabBytes );
    memcpy( buffer + 5, RemoveTextFixedVocab, (size_t) RemoveTextFixedVocabBytes );
    uint8_t * at = buffer + 5 + RemoveTextFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != RemoveTextFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, RemoveTextFixedVocab, (int32_t) RemoveTextFixedVocabBytes, RemoveTextFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t OpenDocumentFixedHash = 0xf1954fe809ff3467ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 8 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t OpenDocumentFixedVocab[] = {
...
constexpr int64_t OpenDocumentFixedVocabBytes = (int64_t) sizeof( OpenDocumentFixedVocab );
...
{
    return 1 + 4 + OpenDocumentFixedVocabBytes + count * OpenDocumentFixedRecordBytes;
}
...
inline int64_t OpenDocumentFixedSave( const OpenDocument * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) OpenDocumentFixedVocabBytes );
    memcpy( buffer + 5, OpenDocumentFixedVocab, (size_t) OpenDocumentFixedVocabBytes );
    uint8_t * at = buffer + 5 + OpenDocumentFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != OpenDocumentFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, OpenDocumentFixedVocab, (int32_t) OpenDocumentFixedVocabBytes, OpenDocumentFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t SaveDocumentFixedHash = 0xcb0776aa6c044299ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 3 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t SaveDocumentFixedVocab[] = {
...
constexpr int64_t SaveDocumentFixedVocabBytes = (int64_t) sizeof( SaveDocumentFixedVocab );
...
{
    return 1 + 4 + SaveDocumentFixedVocabBytes + count * SaveDocumentFixedRecordBytes;
}
...
inline int64_t SaveDocumentFixedSave( const SaveDocument * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) SaveDocumentFixedVocabBytes );
    memcpy( buffer + 5, SaveDocumentFixedVocab, (size_t) SaveDocumentFixedVocabBytes );
    uint8_t * at = buffer + 5 + SaveDocumentFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != SaveDocumentFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, SaveDocumentFixedVocab, (int32_t) SaveDocumentFixedVocabBytes, SaveDocumentFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
