        ...
        switch ( entry.id )
        {
            ...
            case 0xc4bcadba8e631b86ull: // name
            {
                ...
                }
                {
                    ...
                    if ( !r.get( n, TableBitsRequired( 0, entry.max ) ) || !r.align() || !r.has( (int64_t) n * 8 ) ) { report->malformed = true; return false; }
                    int32_t kept = (int32_t) n;
                    if ( kept > 12 ) { kept = 12; report->clamped++; }
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        uint64_t by = 0;
                        if ( !r.get( by, 8 ) ) { report->malformed = true; return false; }
...
// ListNodeNodeMessageBody: PASS TWO's half — decode one record's body into the
...
// TreeNodeNodeMessageBody: PASS TWO's half — decode one record's body into the
// storage it already owns, its map entries carved from its own extent.
...
// LayerNodeMessageBody: PASS TWO's half — decode one record's body into the
// storage it already owns, its map entries carved from its own extent.
...
// SceneNodeMessageBody: PASS TWO's half — decode one record's body into the
// storage it already owns, its map entries carved from its own extent.
...
// DepotNodeMessageBody: PASS TWO's half — decode one record's body into the
// storage it already owns, its map entries carved from its own extent.
