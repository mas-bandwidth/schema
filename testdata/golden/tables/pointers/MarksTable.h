...
struct TableWriter
{
    ...
    GRAPHDEMO_TABLE_INLINE void put64( uint64_t v )
    {
...
GRAPHDEMO_TABLE_INLINE bool TallyLoadBodyRetain( TableReader & r, Tally & value, TableRetain * retain, const TableRetainPath & path );
inline bool TallyLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, Tally & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline int64_t MarkerMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Marker & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool MarkerSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Marker & value, TableRetain * retain, const TableRetainPath & path );
template <typename Ctx> inline bool MarkerSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Marker & value, TableRetain * retain, const TableRetainPath & path );
inline bool MarkerLoadBodyRetain( TableReader & r, const TableNodeMap & nodes, Marker & value, TableRetain * retain, const TableRetainPath & path );
inline bool MarkerLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Marker & value, TableRetain * retain, const TableRetainPath & path );
...
static_assert( sizeof( Tally ) == 4, "Tally: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( Tally, hits ) == 0, "Tally.hits: the fixed form's plan lands here" );

// Tally's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void TallyFixedWriteBody( uint8_t * b, const Tally & value )
{
    (void) b; (void) value;
...
constexpr int64_t TallyFixedBodyBytes = 4;
constexpr int64_t TallyFixedRecordBytes = 8 + TallyFixedBodyBytes; // the hash and the body
constexpr uint64_t TallyFixedHash = 0xe155ccfe28f460d7ull; // fnv1a64 over the layout and the definitions digest (bill §13)
...
constexpr uint8_t TallyFixedLayout[] = {
...
constexpr int64_t TallyFixedLayoutBytes = (int64_t) sizeof( TallyFixedLayout );
...
constexpr TableFixedDst TallyFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Tally
    { (uint32_t) __builtin_offsetof( Tally, hits ), 0, 0, 0, 0 }, // hits
};
...
// test a guard at all.
constexpr TableFixedEntry TallyFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // hits
};
constexpr int32_t TallyFixedPlanCount = 1;
constexpr int32_t TallyFixedPlanGuarded = 1;
...
constexpr TableFixedFill TallyFixedCover[] = {
    { 0u, 4u },
};
constexpr int32_t TallyFixedCoverCount = 1;
...
constexpr uint64_t TallyFixedLineage[] = {
    0xe155ccfe28f460d7ull, // identity
};
constexpr int32_t TallyFixedLineageCount = 1;
constexpr int32_t TallyFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t TallyFixedFloorLive = TallyFixedFloor;
inline void TallyFixedSetFloorForTest( int32_t index ) { TallyFixedFloorLive = index; }
#endif

constexpr uint8_t TallyFixedKnown0Layout[] = {
    0x02, 0x00, 0x00, 0x00, 0x3d, 0xa7, 0x42, 0x42, 0x90, 0x34, 0xff, 0x69, 0x0d, 0x04, 0x00, 0x00,
    0x00, 0x01, 0x00, 0x00, 0x00, 0xbb, 0xf0, 0x0c, 0x9b, 0xcc, 0xfb, 0x2d, 0x73, 0x04, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr TableFixedKnownLayout TallyFixedKnown[] = {
    { 0xe155ccfe28f460d7ull, TallyFixedKnown0Layout, 38, 12 }, // identity
};

constexpr const TableFixedKnownRange * TallyFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t TallyFixedKnownRangeCounts[] = { 0 };
...
constexpr int64_t TallyFixedMeasure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + TallyFixedLayoutBytes + count * TallyFixedRecordBytes;
}

inline int64_t TallyFixedSave( const Tally * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = TallyFixedMeasure( count );
    ...
    TableFixedPut64( buffer + kTableFixedHashAt, TallyFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) TallyFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, TallyFixedLayout, (size_t) TallyFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + TallyFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
        TableFixedPut64( at, TallyFixedHash );
        memset( at + 8, 0, (size_t) TallyFixedBodyBytes ); // the prefill's zeros
        TallyFixedWriteBody( at + 8, values[k] );
        at += TallyFixedRecordBytes;
    }
    return need;
}
...
inline int64_t TallyFixedLoad( Tally * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    for ( int32_t i = 0; i < TallyFixedLineageCount; ++i )
    {
        if ( TallyFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = TallyFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = TallyFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = TallyFixedKnown[lineage_at];
    ...
    const TableFixedEntry * entries = TallyFixedPlan;
    int32_t entry_count = TallyFixedPlanCount;
    int32_t entry_guarded = TallyFixedPlanGuarded;
    int64_t record_bytes = TallyFixedRecordBytes;
    const TableFixedFill * fill = NULL;
    int32_t fill_count = 0;
    if ( hash != TallyFixedHash )
    {
        ...
        else
        {
            ...
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, TallyFixedLayout, (int32_t) TallyFixedLayoutBytes, TallyFixedDst,
                                                   TallyFixedCover, TallyFixedCoverCount,
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( TallyFixedKnownRanges[lineage_at], TallyFixedKnownRangeCounts[lineage_at],
...
inline int64_t MarkerNodeStorage( uint64_t type_id, int64_t length, TableRefuseReason & reason )
{
...
inline int64_t MarkerLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    ...
    {
        records++;
        int64_t storage = MarkerNodeStorage( type_id, length, reason );
...
template <typename Ctx>
inline int64_t MarkerMeasureBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableRetainIds & ids, const Marker & value, TableRetain * retain, const TableRetainPath & path )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.label_length < 0 || value.label_length > 8 ) { return -1; } // storage invariant
    if ( value.label_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 6, 0x39f7fcec8fcb623dull ) ) + 1 + TableLebBytes( (uint64_t) ( value.label_length ) ) + ( value.label_length ); } // label
...
template <typename Ctx>
inline bool MarkerSaveBodyFieldsRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Marker & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( value.label_length < 0 || value.label_length > 8 ) { return false; } // storage invariant
    if ( value.label_length > 0 )
    {
...
template <typename Ctx>
inline bool MarkerSaveBodyRetain( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableRetainIds & ids, const Marker & value, TableRetain * retain, const TableRetainPath & path )
{
    if ( !MarkerSaveBodyFieldsRetain( ctx, numbering, w, ids, value, retain, path ) ) { return false; }
    w.put8( 0 ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
...
inline void MarkerNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    (void) nodes; // every node this root can name is a FIXED table
    switch ( type_id )
    {
...
// MarkerLoadRetain: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
...
inline const Marker * MarkerLoadRetain( uint8_t * region, int64_t region_bytes, const uint8_t * wire_file, int64_t wire_file_bytes, TableRetain * retain, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, then the trailer, and only then a body:
    ...
    if ( nodes.good )
    {
        ...
        while ( TableNodeScanNext( scan, type_id, body, length ) )
        {
            ...
            {
                TableReader sub( body, length, out, &ids_table );
                MarkerNodeBodyRetain( type_id, sub, nodes, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) );
            }
            k++;
        }
    }
    ...
    TableReader r( wire, wire_bytes, out, &ids_table );
    r.nested = false; // the ROOT body, the one that carries the node table
    MarkerLoadBodyRetain( r, nodes, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
    return root;
}

// MarkerNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool MarkerNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    (void) nodes; (void) index_bits; // every node this root can name is a FIXED table
    bool ok = false;
    switch ( type_id )
    {
        case 0x69ff34904242a73dull: ok = TallyLoadMessageBodyRetain( r, vocabulary, report, index_bits, *(Tally *) at, retain, TableRetainPathRoot( (const void *) at, node ) ); break; // Tally
        // a record this dispatch cannot name never reaches here: pass one left it absent
        default: report->malformed = true; break;
    }
    return ok;
...
// MarkerLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
inline bool MarkerLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Marker * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !MarkerNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    return MarkerLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// MarkerLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool MarkerLoadRetainMessages( const Marker ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !MarkerLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// MarkerMeasureRetain and MarkerSaveRetain: the pair, with the retained tail in
...
template <typename Ctx>
inline int64_t MarkerMeasureWireRetain( const Ctx & ctx, const Marker & root, TableRetain * retain, TableAllocator allocator )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
    ...
    if ( MarkerNumberFrom( ctx, numbering, root ) )
    {
        ...
        bytes = MarkerMeasureBodyRetain( ctx, numbering, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) );
        if ( bytes >= 0 )
        {
...
template <typename Ctx>
inline int64_t MarkerSaveWireRetain( const Ctx & ctx, const Marker & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    TableNumbering numbering;
    ...
    bool ok = MarkerSaveBodyFieldsRetain( ctx, numbering, w, ids, root, retain, TableRetainPathRoot( (const void *) &root, 1 ) ) &&
...
inline int64_t MarkerMeasureRetain( const Marker * root, TableRetain * retain, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return MarkerMeasureWireRetain( ctx, *root, retain, allocator );
}
...
inline int64_t MarkerSaveRetain( const Marker * root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( root == NULL || report == NULL ) { return -1; }
    TableRegionCtx ctx;
    return MarkerSaveWireRetain( ctx, *root, retain, buffer, capacity, report );
}

// MarkerSaveRetainMessages: RETENTION WRITING FORM 2 IS REFUSED BY NAME
...
template <typename... Args>
inline void TallySaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Tally is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void TallyLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Tally is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void TallySaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Tally is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
// parameter on a load.
inline const Tally * TallyOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Tally *) TableCookOpen( bytes, length, (uint64_t) sizeof( Tally ), (uint64_t) alignof( Tally ), reason );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Marker * MarkerOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Marker *) TableCookOpen( bytes, length, (uint64_t) sizeof( Marker ), (uint64_t) alignof( Marker ), reason );
}
