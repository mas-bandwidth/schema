...
inline int64_t TallyMeasureBody( TableIds & ids, const Tally & value )
{
    ...
    if ( value.hits != 0 ) { bytes += TableLebBytes( ids.ref( 0x732dfbcc9b0cf0bbull, 29 ) ) + 1 + 4; } // hits
    return bytes;
}
...
GRAPHDEMO_TABLE_INLINE bool TallySaveBody( TableWriter & w, TableIds & ids, const Tally & value )
{
    if ( value.hits != 0 )
    {
        w.putleb( ids.ref( 0x732dfbcc9b0cf0bbull, 29 ) ); w.put8( 4 ); // hits
...
inline int64_t TallyMeasureMessage( const Tally & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = TallyMeasureBody( ids, value );
...
inline int64_t TallySaveMessage( const Tally & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !TallySaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == TallyMeasureMessage( value )
}
...
inline bool TallyLoadMessage( Tally & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    TallyReset( value );
...
inline int64_t MarkerMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Marker & value )
{
    ...
    if ( value.label_length > 0 ) { bytes += TableLebBytes( ids.ref( 0x39f7fcec8fcb623dull, 16 ) ) + 1 + TableLebBytes( (uint64_t) ( value.label_length ) ) + ( value.label_length ); } // label
    {
        ...
        if ( pointee_note != NULL )
        {
            ...
            bytes += TableLebBytes( ids.ref( 0x3bf8fbbad1587cddull, 17 ) ) + 1 + TableLebBytes( index_note );
        }
    }
    return bytes;
}
...
inline bool MarkerSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Marker & value )
{
    ...
    }
    {
        ...
        if ( pointee_note != NULL )
        {
            ...
            w.putleb( ids.ref( 0x3bf8fbbad1587cddull, 17 ) ); w.put8( 17 ); // note — a NODE INDEX into the flat node table
            w.putleb( index_note );
        }
    }
    return !w.overflow;
...
{
    {
        ...
        if ( pointee != NULL )
        {
            ...
            else
            {
                ...
                node.type_slot = 47; // its slot in the unit's vocabulary (§3.3)
...
template <typename Ctx>
inline int64_t MarkerMeasureWire( const Ctx & ctx, const Marker & root, TableAllocator allocator, bool message = false )
{
...
template <typename Ctx>
inline int64_t MarkerSaveWire( const Ctx & ctx, const Marker & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t MarkerMeasureMessage( const Marker * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return MarkerMeasureWire( ctx, *root, allocator, true );
}

inline int64_t MarkerSaveMessage( const Marker * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return MarkerSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t MarkerMeasureMessage( const MarkerBuilder & builder )
{
    if ( builder.region != NULL ) { return MarkerMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return MarkerMeasureWire( ctx, *(const Marker *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t MarkerSaveMessage( const MarkerBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return MarkerSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return MarkerSaveWire( ctx, *(const Marker *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t MarkerLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
...
// MarkerLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Marker * MarkerLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
