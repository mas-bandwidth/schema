...
constexpr uint64_t TallyFixedHash = 0x772fc9106c65a1e0ull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry TallyFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // hits
};
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == TallyFixedHash );
    ...
    if ( identity )
    {
        ...
        static_assert( sizeof( Tally ) >= (size_t) TallyFixedBodyBytes, "Tally: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) TallyFixedBodyBytes );
            TallyFixedClamp( values[k], report ); // the bounds the copy does not hold (§3.4)
            at += TallyFixedRecordBytes;
        }
        return n;
    }
