...
constexpr int TallyFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Tally's LEAVES: one entry per run of bytes that lands, at a base the caller
...
constexpr int TallyFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Tally, hits );
...
inline constexpr int64_t TallyFixedBodyBytes = 4;
inline constexpr int64_t TallyFixedRecordBytes = 8 + TallyFixedBodyBytes; // the hash and the body
inline constexpr uint64_t TallyFixedHash = 0x772fc9106c65a1e0ull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t TallyFixedLayout[] = {
...
inline constexpr int64_t TallyFixedLayoutBytes = (int64_t) sizeof( TallyFixedLayout );
...
inline constexpr TableFixedDst TallyFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Tally
    { (uint32_t) __builtin_offsetof( Tally, hits ), 0, 0, 0, 0 }, // hits
};

// THE IDENTITY PLAN, coalesced at COMPILE TIME out of 1 leaves.
inline constexpr auto TallyFixedPlan = TableFixedBuildPlan<1>( TallyFixedLeaves );
...
inline constexpr int64_t TallyFixedMeasure( int64_t count )
{
...
inline int64_t TallyFixedSave( const Tally * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) TallyFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = TallyFixedPlan.entries;
    int32_t entry_count = TallyFixedPlan.count;
    ...
    if ( hash != TallyFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, TallyFixedLayout, (int32_t) TallyFixedLayoutBytes, TallyFixedDst, plan, plan_capacity, report );
