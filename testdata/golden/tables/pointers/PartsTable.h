...
inline bool StampOpenWalk( const Stamp * node, const uint8_t * base, int64_t bytes, int64_t & watermark, int32_t depth );
inline bool ColourOpenWalk( const Colour * node, const uint8_t * base, int64_t bytes, int64_t & watermark, int32_t depth );
...
// StampOpenWalk: validate the REFERENCE GRAPH and the counts that bound a
// traversal of it — no field value is read, no payload is decoded.
//
// Stamp holds no reference of its own, so it neither reads the watermark nor
...
// ColourOpenWalk: validate the REFERENCE GRAPH and the counts that bound a
// traversal of it — no field value is read, no payload is decoded.
//
// Colour holds no reference of its own, so it neither reads the watermark nor
// advances it: its placement in the region is decided entirely by the pack
// of whatever variable table nests it, and the mark passes through here
// untouched. What this walk owes is the COUNT COMPANIONS — a member nested
// by value carries bounds that any traversal trusts, and an unbounded one
