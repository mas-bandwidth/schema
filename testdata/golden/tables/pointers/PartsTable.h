...
constexpr int StampFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    out[n++] = TableFixedEntry{ src + 0u, dst + (uint32_t) __builtin_offsetof( Stamp, tag_length ), 8u, dst + (uint32_t) __builtin_offsetof( Stamp, tag ), kTableFixedNoGuard, kTableFixedText, 1, 0 }; // tag
    {
...
inline constexpr uint64_t StampFixedHash = 0x2aaaa93920374790ull; // fnv1a64 over the layout's bytes
...
{
    return 1 + 4 + StampFixedLayoutBytes + count * StampFixedRecordBytes;
}
...
inline int64_t StampFixedSave( const Stamp * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) StampFixedLayoutBytes );
    memcpy( buffer + 5, StampFixedLayout, (size_t) StampFixedLayoutBytes );
    uint8_t * at = buffer + 5 + StampFixedLayoutBytes;
