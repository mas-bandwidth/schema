...
inline bool ColourSaveMessageBody( TableBitWriter & w, const Colour & value )
{
    if ( value.r != 0 )
    {
        ...
        w.put( (uint64_t) ( value.r ), 8 );
    }
    if ( value.g != 0 )
    {
        w.put( 8, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.g ), 8 );
    }
    if ( value.b != 0 )
    {
        w.put( 9, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.b ), 8 );
    }
    w.put( 0, kTableMessageRefBitsHere ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
...
static_assert( (uint32_t) __builtin_offsetof( Stamp, tag ) == 0, "Stamp.tag: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Stamp, tag_length ) == 12, "Stamp.tag_length: the fixed form's plan lands here" );
...
constexpr uint64_t StampFixedHash = 0x2b467a2ee937ea73ull; // fnv1a64 over the layout and the definitions digest (bill §13)
...
constexpr TableFixedEntry StampFixedPlan[] = {
    { 0u, 12u, 8u, 0u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // tag
...
constexpr uint64_t StampFixedLineage[] = {
    0x2b467a2ee937ea73ull, // identity
};
...
constexpr TableFixedKnownLayout StampFixedKnown[] = {
    { 0x2b467a2ee937ea73ull, StampFixedKnown0Layout, 55, 24 }, // identity
};
...
inline void StampCookBody( uint8_t * at, const Stamp & value, TableByteOrder order )
{
    ...
    table_cook_put( at + 12, (uint64_t) (uint32_t) value.tag_length, 4, order );
