...
// package graphdemo — the TABLE wire's text form (docs/SPEC-TABLES.md §16).
...
#ifndef GRAPHDEMO_SCHEMA_TABLE_JSON
#define GRAPHDEMO_SCHEMA_TABLE_JSON
...
#endif // GRAPHDEMO_SCHEMA_TABLE_JSON

namespace graphdemo {

bool TallyFromJson( Tally & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, TallyTableType(), text, bytes, report );
}

int64_t TallyToJsonMeasure( const Tally & value )
{
    return TableJsonWrite( &value, TallyTableType(), NULL, 0 );
}

int64_t TallyToJson( const Tally & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, TallyTableType(), buffer, capacity );
}
...
bool MarkerFromJson( MarkerBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, MarkerTableType(), text, bytes, report );
}

int64_t MarkerToJsonMeasure( const Marker * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, MarkerTableType(), NULL, 0, allocator );
}

int64_t MarkerToJson( const Marker * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, MarkerTableType(), buffer, capacity, allocator );
}
