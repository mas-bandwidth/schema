...
{
    return TableJsonRead( &value, MetaTableType(), text, bytes, report );
}

int64_t MetaToJsonMeasure( const Meta & value )
{
    return TableJsonWrite( &value, MetaTableType(), NULL, 0 );
}

int64_t MetaToJson( const Meta & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, MetaTableType(), buffer, capacity );
}

bool SettingsFromJson( Settings & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, SettingsTableType(), text, bytes, report );
}

int64_t SettingsToJsonMeasure( const Settings & value )
{
    return TableJsonWrite( &value, SettingsTableType(), NULL, 0 );
}

int64_t SettingsToJson( const Settings & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, SettingsTableType(), buffer, capacity );
}
...
bool ListNodeFromJson( ListNodeBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, ListNodeTableType(), text, bytes, report );
}

int64_t ListNodeToJsonMeasure( const ListNode * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ListNodeTableType(), NULL, 0, allocator );
}

int64_t ListNodeToJson( const ListNode * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ListNodeTableType(), buffer, capacity, allocator );
}
...
bool TreeNodeFromJson( TreeNodeBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, TreeNodeTableType(), text, bytes, report );
}

int64_t TreeNodeToJsonMeasure( const TreeNode * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TreeNodeTableType(), NULL, 0, allocator );
}

int64_t TreeNodeToJson( const TreeNode * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, TreeNodeTableType(), buffer, capacity, allocator );
}
...
bool LayerFromJson( LayerBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, LayerTableType(), text, bytes, report );
}

int64_t LayerToJsonMeasure( const Layer * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, LayerTableType(), NULL, 0, allocator );
}

int64_t LayerToJson( const Layer * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, LayerTableType(), buffer, capacity, allocator );
}
...
bool SceneFromJson( SceneBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, SceneTableType(), text, bytes, report );
}

int64_t SceneToJsonMeasure( const Scene * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SceneTableType(), NULL, 0, allocator );
}

int64_t SceneToJson( const Scene * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SceneTableType(), buffer, capacity, allocator );
}
...
bool DepotFromJson( DepotBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, DepotTableType(), text, bytes, report );
}

int64_t DepotToJsonMeasure( const Depot * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DepotTableType(), NULL, 0, allocator );
}

int64_t DepotToJson( const Depot * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DepotTableType(), buffer, capacity, allocator );
}
