...
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE void gunner_settings_fixed_identity_clamps( GunnerSettings * value, TableReport * report )
{
    (void) report;
    {
        int32_t n = value->callsign_length;
        if ( n < 0 ) { n = 0; report->clamped++; } else if ( n > 24 ) { n = 24; report->clamped++; }
        value->callsign_length = n;
    }
    value->callsign[value->callsign_length] = 0;
}

/* COUNT AND TEXT CLAMPS, straight-line after the identity copy. A record
   under this build's own hash can still carry a count this build does not
   admit; the plan no longer names those ops, so this is what holds them. */
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE void ship_entry_fixed_identity_clamps( ShipEntry * value, TableReport * report )
{
    (void) report;
    {
        int32_t n = value->display_name_length;
        if ( n < 0 ) { n = 0; report->clamped++; } else if ( n > 32 ) { n = 32; report->clamped++; }
        value->display_name_length = n;
    }
    value->display_name[value->display_name_length] = 0;
    {
        int32_t n = value->hardpoints_count;
        if ( n < 0 ) { n = 0; report->clamped++; } else if ( n > 4 ) { n = 4; report->clamped++; }
        value->hardpoints_count = n;
    }
    gunner_settings_fixed_identity_clamps( &value->gunner, report );
}

/* COUNT AND TEXT CLAMPS, straight-line after the identity copy. A record
   under this build's own hash can still carry a count this build does not
   admit; the plan no longer names those ops, so this is what holds them. */
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE void global_settings_fixed_identity_clamps( GlobalSettings * value, TableReport * report )
{
    (void) report;
    {
        int32_t n = value->build_note_length;
        if ( n < 0 ) { n = 0; report->clamped++; } else if ( n > 48 ) { n = 48; report->clamped++; }
        value->build_note_length = n;
    }
    value->build_note[value->build_note_length] = 0;
}

/* COUNT AND TEXT CLAMPS, straight-line after the identity copy. A record
   under this build's own hash can still carry a count this build does not
   admit; the plan no longer names those ops, so this is what holds them. */
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE void pack_config_fixed_identity_clamps( PackConfig * value, TableReport * report )
{
    (void) report;
    global_settings_fixed_identity_clamps( &value->global, report );
    {
        int64_t i;
        for ( i = 0; i < 3; ++i )
        {
            ship_entry_fixed_identity_clamps( &value->ships[i], report );
        }
    }
    {
        int32_t n = value->reserves_count;
        if ( n < 0 ) { n = 0; report->clamped++; } else if ( n > 3 ) { n = 3; report->clamped++; }
        value->reserves_count = n;
    }
    {
        int64_t i;
        for ( i = 0; i < 3; ++i )
        {
            ship_entry_fixed_identity_clamps( &value->reserves[i], report );
        }
    }
}
...
static SCHEMA_UNUSED const TableFixedEntry gunner_settings_fixed_plan[] = {
    ...
    { 5u, 32u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 9u, 5u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
};
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    record_bytes = 8 + 33;
    identity = ( hash == gunner_settings_fixed_hash );
    if ( !identity )
    {
        ...
        else
        {
            ...
            made = table_fixed_compile( &parsed, gunner_settings_fixed_layout, (int32_t) sizeof( gunner_settings_fixed_layout ), gunner_settings_fixed_dst, dest, dest_capacity, &compiled_guarded, report );
    ...
    for ( k = 0; k < count; ++k )
    {
        ...
        if ( identity )
        {
            ...
            gunner_settings_fixed_identity_clamps( values + k, report ); /* count and text clamps, after the copy */
        }
        else
        {
            gunner_settings_reset( values + k ); /* the declared defaults, one prefill — compiled path only */
            table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), report );
        }
...
static SCHEMA_UNUSED const TableFixedEntry ship_entry_fixed_plan[] = {
    { 0u, 36u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name length */
    { 4u, 0u, 32u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name */
    { 36u, 40u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 44u, 64u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* hardpoints count */
    ...
    { 70u, 100u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 74u, 73u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
};
static SCHEMA_UNUSED const int32_t ship_entry_fixed_plan_count = 9;
static SCHEMA_UNUSED const int32_t ship_entry_fixed_plan_guarded = 9;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    record_bytes = 8 + 98;
    identity = ( hash == ship_entry_fixed_hash );
    if ( !identity )
    {
        /* ANOTHER WRITER: the same loop, over a plan compiled from its layout
           and CACHED BY HASH, so the compile is paid once per peer, not per record.
        ...
        else
        {
            ...
            made = table_fixed_compile( &parsed, ship_entry_fixed_layout, (int32_t) sizeof( ship_entry_fixed_layout ), ship_entry_fixed_dst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_LAYOUT_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
            record_bytes = 8 + (int64_t) table_fixed_entry_at( &parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
    ...
    for ( k = 0; k < count; ++k )
    {
        ...
        {
            table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), report );
            ship_entry_fixed_identity_clamps( values + k, report ); /* count and text clamps, after the copy */
        }
        else
        {
            ship_entry_reset( values + k ); /* the declared defaults, one prefill — compiled path only */
            table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), report );
        }
        /* AND THE BOUNDS THE LOOP DOES NOT HOLD, straight-line over the
           storage it just wrote: the same pass for either plan (§3.4). */
...
static SCHEMA_UNUSED const TableFixedEntry global_settings_fixed_plan[] = {
    ...
    { 5u, 56u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* build_note length */
    { 9u, 5u, 48u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* build_note */
    { 57u, 60u, 12u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* spawn_delays, whole */
};
static SCHEMA_UNUSED const int32_t global_settings_fixed_plan_count = 4;
static SCHEMA_UNUSED const int32_t global_settings_fixed_plan_guarded = 4;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    record_bytes = 8 + 69;
    identity = ( hash == global_settings_fixed_hash );
    if ( !identity )
    {
        /* ANOTHER WRITER: the same loop, over a plan compiled from its layout
           and CACHED BY HASH, so the compile is paid once per peer, not per record.
        ...
        else
        {
            ...
            made = table_fixed_compile( &parsed, global_settings_fixed_layout, (int32_t) sizeof( global_settings_fixed_layout ), global_settings_fixed_dst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_LAYOUT_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
            record_bytes = 8 + (int64_t) table_fixed_entry_at( &parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
    ...
    for ( k = 0; k < count; ++k )
    {
        ...
        {
            table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), report );
            global_settings_fixed_identity_clamps( values + k, report ); /* count and text clamps, after the copy */
        }
        else
        {
            global_settings_reset( values + k ); /* the declared defaults, one prefill — compiled path only */
            table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), report );
        }
        /* AND THE BOUNDS THE LOOP DOES NOT HOLD, straight-line over the
           storage it just wrote: the same pass for either plan (§3.4). */
...
static SCHEMA_UNUSED const TableFixedEntry pack_config_fixed_plan[] = {
    ...
    { 9u, 60u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* build_note length */
    { 13u, 9u, 48u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* build_note */
    { 61u, 64u, 12u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* spawn_delays, whole */
    { 73u, 112u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name length */
    { 77u, 76u, 32u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name */
    { 109u, 116u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 117u, 140u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* hardpoints count */
    ...
    { 143u, 176u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 147u, 149u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
    { 171u, 220u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name length */
    { 175u, 184u, 32u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name */
    { 207u, 224u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 215u, 248u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* hardpoints count */
    ...
    { 241u, 284u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 245u, 257u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
    { 269u, 328u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name length */
    { 273u, 292u, 32u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name */
    { 305u, 332u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 313u, 356u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* hardpoints count */
    ...
    { 339u, 392u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 343u, 365u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
    { 367u, 400u, 12u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* thresholds, whole */
    { 379u, 736u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reserves count */
    { 383u, 448u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name length */
    { 387u, 412u, 32u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name */
    { 419u, 452u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 427u, 476u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* hardpoints count */
    ...
    { 453u, 512u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 457u, 485u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
    { 481u, 556u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name length */
    { 485u, 520u, 32u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name */
    { 517u, 560u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 525u, 584u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* hardpoints count */
    ...
    { 551u, 620u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 555u, 593u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
    { 579u, 664u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name length */
    { 583u, 628u, 32u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* display_name */
    { 615u, 668u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 623u, 692u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* hardpoints count */
    ...
    { 649u, 728u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign length */
    { 653u, 701u, 24u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* callsign */
};
static SCHEMA_UNUSED const int32_t pack_config_fixed_plan_count = 60;
static SCHEMA_UNUSED const int32_t pack_config_fixed_plan_guarded = 60;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    record_bytes = 8 + 677;
    identity = ( hash == pack_config_fixed_hash );
    if ( !identity )
    {
        /* ANOTHER WRITER: the same loop, over a plan compiled from its layout
           and CACHED BY HASH, so the compile is paid once per peer, not per record.
        ...
        else
        {
            ...
            made = table_fixed_compile( &parsed, pack_config_fixed_layout, (int32_t) sizeof( pack_config_fixed_layout ), pack_config_fixed_dst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_LAYOUT_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
            record_bytes = 8 + (int64_t) table_fixed_entry_at( &parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
    ...
    for ( k = 0; k < count; ++k )
    {
        ...
        {
            table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), report );
            pack_config_fixed_identity_clamps( values + k, report ); /* count and text clamps, after the copy */
        }
        else
        {
            pack_config_reset( values + k ); /* the declared defaults, one prefill — compiled path only */
            table_fixed_run( entries, entry_count, entry_guarded, at + 8, (uint8_t *) ( values + k ), report );
        }
        /* AND THE BOUNDS THE LOOP DOES NOT HOLD, straight-line over the
           storage it just wrote: the same pass for either plan (§3.4). */
