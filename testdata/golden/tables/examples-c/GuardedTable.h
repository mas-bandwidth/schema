...
SCHEMA_TABLE_STATIC_ASSERT( schema_tabledemo_patrol_arena_alignment_, SCHEMA_TABLE_ALIGNOF(Patrol) <= kTableAlign, "a table node's alignment must fit the arena's" );
static const TableNodeType schema_tabledemo_patrol_node_type_;
static SCHEMA_UNUSED int schema_tabledemo_patrol_graph_edges_( TableNumbering * numbering, const void * storage );
static SCHEMA_UNUSED int schema_tabledemo_patrol_extent_(TableNumbering * n,const void * storage,uint8_t * target,int64_t * at);
static SCHEMA_UNUSED int schema_tabledemo_patrol_wire_extent_(TableReader r,int64_t * at);
static SCHEMA_UNUSED int64_t schema_tabledemo_patrol_wire_storage_(const TableReader * r);
...
static SCHEMA_UNUSED int patrol_save_body( TableWriter * w, const Patrol * value )
{
    ...
    { /* active */
        if ( !( value->active == 0 ) )
        {
            table_writer_header_at( w, 58, 0x6580790b036f0c6full, 1 );
    ...
    if ( value->active && value->has_target )
    {
        ...
        {
            table_writer_header_at( w, 104, 0xb7bc9ac015a25050ull, 4 );
    ...
    if ( value->active && !value->has_target )
    {
        ...
        {
            table_writer_header_at( w, 105, 0xb8c758d8bd1845d4ull, 10 );
    ...
    if ( !value->active )
    {
        ...
        {
            table_writer_header_at( w, 38, 0x3bf8fbbad1587cddull, 12 );
            if ( w->buffer == NULL )
            {
...
static SCHEMA_UNUSED int patrol_save_body_retain(TableWriter *,const Patrol *, TableRetainWalk retention);
static SCHEMA_UNUSED int patrol_load_body_retain(TableReader *,Patrol *, TableRetainWalk retention);
static SCHEMA_UNUSED int patrol_load_message_body_retain(TableMessageReader *,Patrol *, TableRetainWalk retention);
static SCHEMA_UNUSED int schema_tabledemo_patrol_wire_note__retain( TableWriter * w, const Patrol * value , TableRetainWalk retention)
{
    (void)retention;
...
static SCHEMA_UNUSED int patrol_save_body_retain( TableWriter * w, const Patrol * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    retention=table_retain_step(body_keep,0,0);
        if ( !( value->active == 0 ) )
        {
            table_retain_writer_id( w, 0x6580790b036f0c6full , retention);
            table_writer_put8( w, 1 );
            table_writer_put8( w, value->active ? 1 : 0 );
        }
    }
    ...
    if ( value->active )
    {
        if ( !( value->speed == 1.0f ) )
        {
            table_retain_writer_id( w, 0x2281498aa0200e40ull , retention);
    ...
    if ( value->active )
    {
        if ( !( value->has_target == 0 ) )
        {
            table_retain_writer_id( w, 0x247f0e7f55aacfbdull , retention);
            table_writer_put8( w, 1 );
            table_writer_put8( w, value->has_target ? 1 : 0 );
        }
    }
    ...
    if ( value->active && !value->has_target )
    {
        if ( !( value->wander == 0.5f ) )
        {
            table_retain_writer_id( w, 0xb8c758d8bd1845d4ull , retention);
            table_writer_put8( w, 10 );
            table_writer_put32( w, table_float_to_bits( value->wander ) );
        }
    }
    ...
    if ( !value->active )
    {
        ...
        if ( value->note_length != 0 )
        {
            ...
            {
                int64_t frame_begin = w->offset;
                if ( !schema_tabledemo_patrol_wire_note__retain( w, value , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                ...
                if ( !schema_tabledemo_patrol_wire_note__retain( &probe, value , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !schema_tabledemo_patrol_wire_note__retain( w, value , retention) ) { return 0;
...
static SCHEMA_UNUSED int patrol_load_body_retain( TableReader * r, Patrol * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    for ( ;; )
    {
        ...
        switch ( id )
        {
            ...
            case 0x3bf8fbbad1587cddull: /* note */
            {
                ...
                memcpy( value->note, sub.buffer, (size_t) keep );
                value->note[keep] = 0;
...
static SCHEMA_UNUSED int patrol_load_message_body_retain(TableMessageReader * r,Patrol * value, TableRetainWalk retention)
{
    (void)retention;
...
static SCHEMA_UNUSED int schema_tabledemo_patrol_extent_(TableNumbering * n,const void * storage,uint8_t * target,int64_t * at)
{
    const Patrol * value=(const Patrol *)storage; Patrol * out=(Patrol *)(void *)target;
...
static SCHEMA_UNUSED int64_t schema_tabledemo_patrol_wire_storage_(const TableReader * r)
{
    int64_t at=sizeof(Patrol); if(!schema_tabledemo_patrol_wire_extent_(*r,&at) || at>INT64_MAX-kTableAlign) { return -1; } return table_align_up64(at);
}
static SCHEMA_UNUSED int schema_tabledemo_patrol_node_save_( TableWriter * w, const void * storage )
{ return patrol_save_body(w,(const Patrol *)storage); }
static SCHEMA_UNUSED int schema_tabledemo_patrol_graph_edges_( TableNumbering * numbering, const void * storage )
{
...
static const TableNodeType schema_tabledemo_patrol_node_type_ = { UINT64_C(0x1c182e1c2529027b), sizeof(Patrol), schema_tabledemo_patrol_node_save_, schema_tabledemo_patrol_graph_edges_, 0, schema_tabledemo_patrol_extent_, schema_tabledemo_patrol_wire_storage_, 4, 0, schema_tabledemo_patrol_cook_body_, NULL };

/* ---- Patrol: the variable-length life (docs/SPEC-TABLES.md §2, §6, §9) ----

   MUTABLE: PatrolBuilder — allocate nodes, wire them together, then Lock.
   ...
   Patrol is never held by value — a file-format-scale structure is a region
...
typedef struct PatrolBuilder
{
    TableArena arena;
    TableWorker main;       /* the calling thread's allocation front */
    TableRef root_ref;
    uint8_t * region;       /* the packed const form, produced by patrol_builder_lock */
...
static SCHEMA_UNUSED int patrol_builder_init_with_allocator( PatrolBuilder * builder, TableAllocator allocator )
{
    uint32_t at;
    ...
    /* the ROOT is allocated like any node and is not a pointer target, so
       it takes the arena's untyped bump directly rather than an patrol_emplace
       that only a pointed-at table has */
    at = table_worker_bump( &builder->main, (uint32_t) sizeof( Patrol ) );
    if ( at == kTableAllocFailed ) { return 0; }
    builder->root_ref.value = (int64_t) at;
    patrol_reset( (Patrol *) (void *) table_arena_at( &builder->arena, at ) );
    return 1;
}

static SCHEMA_UNUSED int patrol_builder_init(PatrolBuilder * builder) { return patrol_builder_init_with_allocator(builder,table_default_allocator()); }

static SCHEMA_UNUSED void patrol_builder_shutdown( PatrolBuilder * builder )
{
...
static SCHEMA_UNUSED Patrol * patrol_builder_root( PatrolBuilder * builder )
{
    if ( builder->arena.locked || builder->root_ref.value == 0 ) { return NULL; }
    return (Patrol *) (void *) table_arena_at( &builder->arena, (uint32_t) builder->root_ref.value );
}

static SCHEMA_UNUSED int patrol_builder_lock(PatrolBuilder * builder)
{
    TableCtx ctx; const Patrol * root; uint8_t * packed; int64_t bytes;
    if (builder->arena.locked) { return builder->region!=NULL; }
    if (builder->root_ref.value==0) { return 0; }
    ctx.arena=&builder->arena; root=(const Patrol *)table_arena_at(&builder->arena,(uint32_t)builder->root_ref.value);
    packed=table_graph_pack(&ctx,root,&schema_tabledemo_patrol_node_type_,&bytes);
...
static SCHEMA_UNUSED const TableNodeType * schema_tabledemo_patrol_node_lookup_(uint64_t id)
{
    switch (id) {
        default: return NULL;
    }
}
static SCHEMA_UNUSED int64_t patrol_measure_with_allocator(const TableCtx * ctx, const Patrol * root, TableAllocator allocator)
{
    return table_graph_save(ctx,root,&schema_tabledemo_patrol_node_type_,NULL,INT64_MAX,allocator);
}
static SCHEMA_UNUSED int64_t patrol_measure(const TableCtx * ctx,const Patrol * root)
{ return patrol_measure_with_allocator(ctx,root,ctx!=NULL && ctx->arena!=NULL ? ctx->arena->allocator : table_default_allocator()); }
static SCHEMA_UNUSED int64_t patrol_save_with_allocator(const TableCtx * ctx, const Patrol * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator)
{
    if (buffer==NULL || capacity<0) { return -1; }
    return table_graph_save(ctx,root,&schema_tabledemo_patrol_node_type_,buffer,capacity,allocator);
}
static SCHEMA_UNUSED int64_t patrol_save(const TableCtx * ctx,const Patrol * root, uint8_t * buffer, int64_t capacity)
{ return patrol_save_with_allocator(ctx,root,buffer,capacity,ctx!=NULL && ctx->arena!=NULL ? ctx->arena->allocator : table_default_allocator()); }
static SCHEMA_UNUSED int64_t schema_tabledemo_patrol_load_layout_(const TableReader * reader, int64_t * data_out, int64_t * records_out)
{
    TableNodeScan scan=table_node_scan_begin(reader); TableReader body; uint64_t id;
    int64_t data=schema_tabledemo_patrol_wire_storage_(reader), records=0;
    if(data<0) { return -1; }
    while (table_node_scan_next(&scan,&id,&body)) {
        const TableNodeType * type=schema_tabledemo_patrol_node_lookup_(id);
...
static SCHEMA_UNUSED int64_t patrol_load_measure_ex(const uint8_t * wire, int64_t bytes, int64_t * attribution, TableRefuseReason * reason)
{
    ...
    report.reason=SCHEMA_TABLE_REFUSE_COUNT_OVER_LENGTH; total=schema_tabledemo_patrol_load_layout_(&reader,&data,&records);
...
static SCHEMA_UNUSED int64_t patrol_load_measure(const uint8_t * wire, int64_t bytes)
{ return patrol_load_measure_ex(wire,bytes,NULL,NULL); }
static SCHEMA_UNUSED int schema_tabledemo_patrol_load_graph_(TableReader * reader, Patrol * root, TableNodeMap * nodes, TableNodeDirEntry * directory, TableSink * sink)
{
    ...
    if(nodes->good) {
        ...
        while(table_node_scan_next(&scan,&id,&body)) {
            if(directory[k+1].offset!=UINT64_MAX) {
                ...
                TableRegionSink extent; TableSink carve; const TableNodeType * type=schema_tabledemo_patrol_node_lookup_(id);
    ...
    { int read_ok; TableRegionSink extent; TableSink carve; carve.worker=sink->worker; carve.region=NULL;
      if(!nodes->arena) { extent.base=(uint8_t *)(void *)root; extent.used=sizeof(*root); extent.capacity=schema_tabledemo_patrol_wire_storage_(reader); carve.region=&extent; }
      nodes->sink=&carve;  read_ok=patrol_load_body(reader,root); nodes->sink=NULL; return !nodes->refused && (!nodes->arena || read_ok); }
}
static SCHEMA_UNUSED const Patrol * patrol_load(uint8_t * region, int64_t region_bytes, const uint8_t * wire, int64_t wire_bytes, TableReport * report)
{
    ...
    total=schema_tabledemo_patrol_load_layout_(&reader,&data,&records);
    ...
    directory[0].offset=0; directory[0].type_id=UINT64_C(0x1c182e1c2529027b);
    ...
    place.base=region; place.capacity=data; place.used=schema_tabledemo_patrol_wire_storage_(&reader); sink.region=&place; sink.worker=NULL;
    if(!schema_tabledemo_patrol_load_graph_(&reader,(Patrol *)(void *)region,&nodes,directory,&sink)) { return NULL; }
    return (const Patrol *)(const void *)region;
}
static SCHEMA_UNUSED int patrol_load_builder(PatrolBuilder * builder, const uint8_t * wire, int64_t wire_bytes, TableReport * report)
{
    TableReport ignored; TableReader reader; TableNodeMap nodes; TableNodeDirEntry * directory; TableSink sink; TableNodeScan scan; TableReader body; uint64_t id; int64_t records=0; Patrol * root; int ok;
    memset(&ignored,0,sizeof(ignored)); if(report==NULL) { report=&ignored; }
    ...
    root=patrol_builder_root(builder); if(root==NULL) { report->malformed=1; return 0; }
    ...
    directory[0].offset=(uint64_t)builder->root_ref.value; directory[0].type_id=UINT64_C(0x1c182e1c2529027b);
    ...
    ok=schema_tabledemo_patrol_load_graph_(&reader,root,&nodes,directory,&sink); table_release(builder->arena.allocator,directory); return ok;
}
static SCHEMA_UNUSED int schema_tabledemo_patrol_message_graph_save_(TableBitWriter * w,const Patrol * root,TableAllocator allocator)
{
 TableNumbering numbering;
 uint64_t i;
 int ok=0;
 if(!table_number_build(&numbering,NULL,root,&schema_tabledemo_patrol_node_type_,allocator))goto done;
 w->nodes=&numbering;
 w->index_bits=table_bits_required(0,(int64_t)numbering.count);
 if(numbering.count-1>UINT32_MAX)goto done;
 if(numbering.count>1){
 table_bit_put(w,131,kTableMessageRefBitsHere);
 table_bit_put(w,numbering.count-1,32);
 for(i=1;i<numbering.count;i++){const TableNodeEntry * node=numbering.entries+i;
 switch(node->type->id){
 default:goto done;
 } } }ok=patrol_save_message_body(w,root);
 done:table_number_dispose(&numbering);
 w->nodes=NULL;
 return ok&&!w->overflow;
}
static SCHEMA_UNUSED int64_t patrol_measure_messages_with_allocator(const Patrol * const * roots,int64_t count,TableReport * report,TableAllocator allocator)
{
 ...
 for(i=0;i<count;i++)if(!roots[i]||!schema_tabledemo_patrol_message_graph_save_(&w,roots[i],allocator))return -1;
 table_bit_align_write(&w);
 return w.overflow?-1:w.bits/8;
}
static SCHEMA_UNUSED int64_t patrol_measure_messages(const Patrol * const * roots,int64_t count,TableReport * report)
{ return patrol_measure_messages_with_allocator(roots,count,report,table_default_allocator());
}
static SCHEMA_UNUSED int64_t patrol_save_messages_with_allocator(const Patrol * const * roots,int64_t count,uint8_t * buffer,int64_t capacity,TableReport * report,TableAllocator allocator)
{
...
static SCHEMA_UNUSED int64_t patrol_save_messages(const Patrol * const * roots,int64_t count,uint8_t * buffer,int64_t capacity,TableReport * report)
{ return patrol_save_messages_with_allocator(roots,count,buffer,capacity,report,table_default_allocator());
}
static SCHEMA_UNUSED int schema_tabledemo_patrol_message_record_scan_(TableMessageReader * r,uint64_t * id,int64_t * size,uint64_t * length)
{
 const TableMessageEntry * entry;
 int64_t at=0;
 *size=0;
...
static SCHEMA_UNUSED int schema_tabledemo_patrol_message_storage_(TableMessageReader * r,uint64_t * records,int64_t * data,int64_t * root_bytes,int * complete)
{
 uint64_t i,id,length;
 int64_t size,root=sizeof(Patrol);
 *data=0;
 *complete=1;
 if(!table_message_nodes_open(r,records))return 0;
 for(i=0;i<*records;i++){
 if(!schema_tabledemo_patrol_message_record_scan_(r,&id,&size,&length)||size>INT64_MAX-*data)return 0;
 *data+=size;
 }
 if(!schema_tabledemo_patrol_message_extent_(r,&root))*complete=0;
...
static SCHEMA_UNUSED int64_t patrol_load_measure_messages_ex(const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,int64_t * attribution,TableRefuseReason * reason)
{
 ...
 int complete;
 if(!schema_tabledemo_patrol_message_storage_(&r,&records,&data,&root,&complete)){if(reason)*reason=SCHEMA_TABLE_REFUSE_COUNT_OVER_LENGTH;
 return -1;
 }
...
static SCHEMA_UNUSED int64_t patrol_load_measure_messages(const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes)
{ return patrol_load_measure_messages_ex(vocabulary,buffer,bytes,NULL,NULL);
}
static SCHEMA_UNUSED int schema_tabledemo_patrol_message_load_into_(TableMessageReader * r,uint8_t * region,int64_t capacity,int64_t * used,const Patrol ** out)
{
 uint64_t count,i,id,length;
 int64_t dir_bytes,records_start,fields_start,size,root_size=sizeof(Patrol);
 ...
 *used+=dir_bytes;
 ...
 for(i=0;i<count;i++){if(!schema_tabledemo_patrol_message_record_scan_(r,&id,&size,&length))goto malformed;
 ...
 root=(Patrol *)(void *)(region+*used);
 directory[0].offset=(uint64_t)*used;
 directory[0].type_id=UINT64_C(0x1c182e1c2529027b);
 *used+=root_size;
 patrol_reset(root);
 *out=root;
 nodes.good=1;
 ...
 ok=patrol_load_message_body(r,root);
 r->nodes=NULL;
 if(!ok)return 0;
 *out=root;
 return 1;
...
static SCHEMA_UNUSED int patrol_load_messages(const Patrol ** roots,int64_t * count,uint8_t * region,int64_t region_bytes,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 TableReport ignored={0};
 TableMessageReader r;
 ...
 *count=0;
 ...
 if(!schema_tabledemo_patrol_message_load_into_(&r,region,region_bytes,&used,roots+i))return 0;
 (*count)++;
 }return table_message_close(&r);
}
static SCHEMA_UNUSED int schema_tabledemo_patrol_load_graph_retain_(TableReader * reader, Patrol * root, TableNodeMap * nodes, TableNodeDirEntry * directory, TableSink * sink, TableRetainWalk retention)
{
    (void)retention;
    ...
    carve.region=NULL;
      ...
      extent.capacity=schema_tabledemo_patrol_wire_storage_(reader);
      carve.region=&extent;
      }
      nodes->sink=&carve;
      retention.path=table_retain_root(root,1);
      read_ok=patrol_load_body_retain(reader,root, retention);
      nodes->sink=NULL;
      return !nodes->refused && (!nodes->arena || read_ok);
      }
}
static SCHEMA_UNUSED const Patrol * patrol_load_retain(uint8_t * region,int64_t region_bytes,const uint8_t * wire,int64_t wire_bytes,TableRetain * retain,TableReport * report)
{
 TableReport ignored={0};
 TableReader reader;
 TableNodeMap nodes;
 ...
 place.base=region;
 place.capacity=data;
 place.used=schema_tabledemo_patrol_wire_storage_(&reader);
 sink.region=&place;
 sink.worker=NULL;
 ...
 if(!schema_tabledemo_patrol_load_graph_retain_(&reader,(Patrol *)(void *)region,&nodes,directory,&sink,retention))return NULL;
 return (const Patrol *)(const void *)region;
}
static SCHEMA_UNUSED int schema_tabledemo_patrol_retain_save_node_(TableWriter * w,const void * value,uint64_t id,TableRetainWalk retention)
{
 switch(id){
 case UINT64_C(0x1c182e1c2529027b):return patrol_save_body_retain(w,(const Patrol *)value,retention);
 default:return 0;
 }
}
static SCHEMA_UNUSED int64_t patrol_measure_retain_with_allocator(const Patrol * root,TableRetain * retain,TableAllocator allocator)
{if(!root)return -1;
return table_retain_graph_save(root,&schema_tabledemo_patrol_node_type_,retain,NULL,INT64_MAX,NULL,allocator,schema_tabledemo_patrol_retain_save_node_);
}
static SCHEMA_UNUSED int64_t patrol_measure_retain(const Patrol * root,TableRetain * retain)
{return patrol_measure_retain_with_allocator(root,retain,table_default_allocator());
}
static SCHEMA_UNUSED int64_t patrol_save_retain_with_allocator(const Patrol * root,TableRetain * retain,uint8_t * buffer,int64_t capacity,TableReport * report,TableAllocator allocator)
{if(!root||!buffer||capacity<0||!report)return -1;
return table_retain_graph_save(root,&schema_tabledemo_patrol_node_type_,retain,buffer,capacity,report,allocator,schema_tabledemo_patrol_retain_save_node_);
}
static SCHEMA_UNUSED int64_t patrol_save_retain(const Patrol * root,TableRetain * retain,uint8_t * buffer,int64_t capacity,TableReport * report)
{return patrol_save_retain_with_allocator(root,retain,buffer,capacity,report,table_default_allocator());
}
static SCHEMA_UNUSED int schema_tabledemo_patrol_message_load_into_retain_(TableMessageReader * r,uint8_t * region,int64_t capacity,int64_t * used,const Patrol ** out, TableRetainWalk retention)
{
    (void)retention;
 ...
 *out=root;
 ...
 carve.used=sizeof(*root);
 carve.capacity=root_size;
 ok=patrol_load_message_body_retain(r,root, retention);
 r->nodes=NULL;
 if(!ok)return 0;
 *out=root;
 return 1;
...
static SCHEMA_UNUSED int patrol_load_retain_messages(const Patrol ** roots,int64_t * count,uint8_t * region,int64_t region_bytes,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableRetain * retains,TableReport * report)
{
 TableReport ignored={0};
 TableMessageReader r;
 TableRetainWalk retention;
 ...
 *count=0;
 ...
 if(!schema_tabledemo_patrol_message_load_into_retain_(&r,region,region_bytes,&used,roots+i,retention))return 0;
 (*count)++;
 }return table_message_close(&r);
}
...
/* patrol_open: match the header and POINT. On a match the bytes ARE what this
   ...
   A REFERENCE INSIDE THE REGION IS DEREFERENCED THROUGH patrol_at with a NULL
...
static SCHEMA_UNUSED int64_t patrol_cook_measure_with_allocator(const TableCtx * ctx,const Patrol * value,TableAllocator allocator)
{ return table_graph_cook(ctx,value,&schema_tabledemo_patrol_node_type_,NULL,0,TableByteOrder_Little,allocator,SCHEMA_TABLEDEMO_BUILD_VERSION_VALUE,1); }
static SCHEMA_UNUSED int patrol_cook_with_allocator(const TableCtx * ctx,const Patrol * value,void * output,uint64_t capacity,TableByteOrder order,TableAllocator allocator)
{ return table_graph_cook(ctx,value,&schema_tabledemo_patrol_node_type_,output,capacity,order,allocator,SCHEMA_TABLEDEMO_BUILD_VERSION_VALUE,0)>=0; }
static SCHEMA_UNUSED int64_t patrol_cook_measure(const TableCtx * ctx,const Patrol * value)
{ return patrol_cook_measure_with_allocator(ctx,value,ctx!=NULL && ctx->arena!=NULL ? ctx->arena->allocator : table_default_allocator()); }
static SCHEMA_UNUSED int patrol_cook(const TableCtx * ctx,const Patrol * value,void * output,uint64_t capacity,TableByteOrder order)
{ return patrol_cook_with_allocator(ctx,value,output,capacity,order,ctx!=NULL && ctx->arena!=NULL ? ctx->arena->allocator : table_default_allocator()); }
...
int schema_tabledemo_patrol_from_json_( PatrolBuilder * builder, const char * text, int64_t bytes, TableReport * report );
int64_t schema_tabledemo_patrol_to_json_( const Patrol * value, char * buffer, int64_t capacity, TableAllocator allocator );
static SCHEMA_UNUSED int patrol_from_json( PatrolBuilder * builder, const char * text, int64_t bytes, TableReport * report ) { return schema_tabledemo_patrol_from_json_(builder,text,bytes,report); }
static SCHEMA_UNUSED int64_t patrol_to_json_measure( const Patrol * value ) { return schema_tabledemo_patrol_to_json_(value,NULL,0,table_default_allocator()); }
static SCHEMA_UNUSED int64_t patrol_to_json( const Patrol * value, char * buffer, int64_t capacity ) { return schema_tabledemo_patrol_to_json_(value,buffer,capacity,table_default_allocator()); }

static SCHEMA_UNUSED int64_t patrol_to_json_with_allocator(const Patrol * value,char * buffer,int64_t capacity,TableAllocator allocator) { return schema_tabledemo_patrol_to_json_(value,buffer,capacity,allocator); }
static SCHEMA_UNUSED int64_t patrol_to_json_measure_with_allocator(const Patrol * value,TableAllocator allocator) { return schema_tabledemo_patrol_to_json_(value,NULL,0,allocator); }
