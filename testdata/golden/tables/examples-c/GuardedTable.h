...
/* table Patrol — TABLE-wire storage: relocatable, bounded. C has no member
   initializers, so the declared defaults live in PatrolReset and nowhere
...
static SCHEMA_UNUSED void PatrolReset( Patrol * value );
...
static SCHEMA_UNUSED void schema_tabledemo_patrol_reset_raw_( void * storage ) { PatrolReset( (Patrol *) storage ); }

/* ---- codecs: measure/save/load per closure member ---- */

static SCHEMA_UNUSED int64_t PatrolMeasure( const Patrol * value );
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE int PatrolSaveBody( TableWriter * w, const Patrol * value );
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE int PatrolLoadBody( TableReader * r, Patrol * value );
...
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE int PatrolSaveBody( TableWriter * w, const Patrol * value )
{
    if ( value->active != 0 )
    {
        TableWriterPut16( w, 0x405a ); TableWriterPut8( w, 1 ); /* active */
        TableWriterPut8( w, value->active ? 1 : 0 );
    }
    if ( value->active )
    {
        if ( value->speed != 1.0f )
        {
            TableWriterPut16( w, 0xbc00 ); TableWriterPut8( w, 10 ); /* speed */
    ...
    if ( value->active )
    {
        ...
        {
            TableWriterPut16( w, 0xb0a7 ); TableWriterPut8( w, 1 ); /* has_target */
            TableWriterPut8( w, value->has_target ? 1 : 0 );
        }
    }
    if ( value->active && value->has_target )
    {
        ...
        {
            TableWriterPut16( w, 0xdf6a ); TableWriterPut8( w, 4 ); /* target_id */
            TableWriterPut32( w, (uint32_t) ( value->target_id ) );
        }
    }
    if ( value->active && !value->has_target )
    {
        ...
        {
            TableWriterPut16( w, 0x832f ); TableWriterPut8( w, 10 ); /* wander */
            TableWriterPut32( w, table_float_to_bits( value->wander ) );
        }
    }
    if ( !value->active )
    {
        ...
        {
            TableWriterPut16( w, 0x9da7 ); TableWriterPut8( w, 12 ); /* note */
            TableWriterPut32( w, (uint32_t) value->note_length );
            TableWriterRaw( w, value->note, value->note_length );
        }
    }
...
static SCHEMA_UNUSED int64_t PatrolSave( const Patrol * value, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( ;; )
    {
        ...
        switch ( field_id )
        {
            case 0x405a: /* active */
            {
                ...
                if ( !TableReaderHas( &(*r), 1 ) ) { r->report->malformed = 1; return 0; }
                value->active = TableReaderGet8( &(*r) ) != 0;
                break;
            }
            case 0xbc00: /* speed */
            {
            ...
            case 0xb0a7: /* has_target */
            {
                ...
                if ( !TableReaderHas( &(*r), 1 ) ) { r->report->malformed = 1; return 0; }
                value->has_target = TableReaderGet8( &(*r) ) != 0;
                break;
            }
            ...
            case 0x832f: /* wander */
            {
                ...
                if ( !TableReaderHas( &(*r), 4 ) ) { r->report->malformed = 1; return 0; }
                value->wander = table_bits_to_float( TableReaderGet32( &(*r) ) );
                break;
            }
            case 0x9da7: /* note */
            {
...
static SCHEMA_UNUSED int PatrolLoad( Patrol * value, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReader r;
...
/* PatrolOpen: match the header and POINT. On a match the bytes ARE what this
   ...
   parameter on a load. */
static SCHEMA_UNUSED const Patrol * PatrolOpen( const void * bytes, uint64_t length )
{
    return (const Patrol *) TableCookOpen( bytes, length, (uint64_t) sizeof( Patrol ), (uint64_t) SCHEMA_TABLE_ALIGNOF( Patrol ) );
}
...
static SCHEMA_UNUSED const TableTypeInfo * PatrolTableType( void ) { return &schema_tabledemo_patrol_info_; }
...
static SCHEMA_UNUSED int PatrolFromJson( Patrol * value, const char * text, int64_t bytes, TableReport * report )
{
    return schema_tabledemo_patrol_from_json_( value, text, bytes, report );
}
static SCHEMA_UNUSED int64_t PatrolToJsonMeasure( const Patrol * value ) { return schema_tabledemo_patrol_to_json_( value, NULL, 0 ); }
static SCHEMA_UNUSED int64_t PatrolToJson( const Patrol * value, char * buffer, int64_t capacity )
{
