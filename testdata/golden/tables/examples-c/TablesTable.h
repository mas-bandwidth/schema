...
{
 switch(value->type) {
 case EFFECT_TYPE_NONE: table_bit_put(w,0,kTableMessageRefBitsHere); break;
...
static SCHEMA_UNUSED int schema_tabledemo_effect_wire_message_load_(TableMessageReader * r,Effect * value)
{
 ...
 value->type=EFFECT_TYPE_BUFF;break;}
 ...
 value->type=EFFECT_TYPE_DEBUFF;break;}
...
static SCHEMA_UNUSED int schema_tabledemo_effect_message_extent_(TableMessageReader * r,int64_t * at)
{
 ...
 switch(entry->id){
 default:break;}return table_message_skip(r,entry);
}
...
{
 weapon_config_reset(value);for(;;){const TableMessageEntry * entry;
 ...
 { uint64_t lo,hi;if(!table_message_number(r,entry,&lo,&hi))goto malformed;
  ...
  if(decoded>63ull){decoded=63ull;r->report->clamped++;}
 ...
 if(!(entry->kind==15 && entry->elem_kind==0)){if(entry->elem_kind==0 && table_kind_widens(entry->kind,15)){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
...
static SCHEMA_UNUSED int weapon_config_load_messages(WeaponConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 for(i=0;i<bodies;i++){if(!weapon_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int loadout_config_save_message_body(TableBitWriter * w,const LoadoutConfig * value)
{
 ...
 if(!(value->grade == GRADE_SILVER)) {
  ...
  switch(value->grade) {
   case GRADE_NONE: table_bit_put(w,0,kTableMessageRefBitsHere); break;
   case GRADE_BRONZE: table_bit_put(w,113,kTableMessageRefBitsHere); break;
   case GRADE_SILVER: table_bit_put(w,114,kTableMessageRefBitsHere); break;
   case GRADE_GOLD: table_bit_put(w,115,kTableMessageRefBitsHere); break;
   default: return 0;
  }
 }
 }
 ...
 if(value->grades_count>0) {
  ...
  { int32_t i; for(i=0;i<value->grades_count;i++) {
    switch(value->grades[i]) {
     case GRADE_NONE: table_bit_put(w,0,kTableMessageRefBitsHere); break;
     case GRADE_BRONZE: table_bit_put(w,113,kTableMessageRefBitsHere); break;
     case GRADE_SILVER: table_bit_put(w,114,kTableMessageRefBitsHere); break;
 ...
 { /* podium */
 int rides=0; { int32_t i; for(i=0;i<3;i++) if(!(value->podium[i] == GRADE_NONE)) { rides=1; break; } }
 if(rides) {
  if(w->check_default) {w->bits=kTableMessageRefBitsHere+1;return 1;}
  table_bit_put(w,22,kTableMessageRefBitsHere);
  if(3<0 || 3>3) return 0;
  table_bit_put(w,(uint64_t)3-3,0);
  { int32_t i; for(i=0;i<3;i++) {
    switch(value->podium[i]) {
     case GRADE_NONE: table_bit_put(w,0,kTableMessageRefBitsHere); break;
     case GRADE_BRONZE: table_bit_put(w,113,kTableMessageRefBitsHere); break;
     case GRADE_SILVER: table_bit_put(w,114,kTableMessageRefBitsHere); break;
 ...
 if(value->attachments_count>0) {
  ...
  { int32_t i; for(i=0;i<value->attachments_count;i++) {
...
{
 loadout_config_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
 ...
 { const TableMessageEntry * named; if(!table_message_ref(r,&named,1)) goto malformed;
  if(named==NULL) value->grade=GRADE_NONE; else switch(named->id) {
  case UINT64_C(0xc4927739aac4c591): value->grade=GRADE_BRONZE;break;
  case UINT64_C(0xc3e51adeeaf12580): value->grade=GRADE_SILVER;break;
  case UINT64_C(0xc416c97e0d3218b3): value->grade=GRADE_GOLD;break;
  default:value->grade=GRADE_NONE;r->report->unknown++;break;
  } }
 break;}
 case UINT64_C(0xd90a4e7682f799c5): {
 if(!(entry->kind==14 && entry->elem_kind==30)){if(0){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  ...
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){Grade scratch; Grade * item=i<kept ? &value->grades[i] : &scratch;
  { const TableMessageEntry * named; if(!table_message_ref(r,&named,1)) goto malformed;
   if(named==NULL) (*item)=GRADE_NONE; else switch(named->id) {
   case UINT64_C(0xc4927739aac4c591): (*item)=GRADE_BRONZE;break;
   case UINT64_C(0xc3e51adeeaf12580): (*item)=GRADE_SILVER;break;
   case UINT64_C(0xc416c97e0d3218b3): (*item)=GRADE_GOLD;break;
   default:(*item)=GRADE_NONE;r->report->unknown++;break;
   } }
 ...
 if(!(entry->kind==14 && entry->elem_kind==30)){if(0){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;kept=n>3?3:n;if(kept<n)r->report->clamped++;
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){Grade scratch; Grade * item=i<kept ? &value->podium[i] : &scratch;
  { const TableMessageEntry * named; if(!table_message_ref(r,&named,1)) goto malformed;
   if(named==NULL) (*item)=GRADE_NONE; else switch(named->id) {
   case UINT64_C(0xc4927739aac4c591): (*item)=GRADE_BRONZE;break;
   case UINT64_C(0xc3e51adeeaf12580): (*item)=GRADE_SILVER;break;
 ...
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;kept=n>2?2:n;if(kept<n)r->report->clamped++;
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){WeaponConfig scratch; WeaponConfig * item=i<kept ? &value->backups[i] : &scratch;
 ...
 if(!(entry->kind==14 && entry->elem_kind==13)){if(entry->kind==14 && table_kind_widens(entry->elem_kind,13)){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;kept=n>8?8:n;if(kept<n)r->report->clamped++;
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){Attachment scratch; Attachment * item=i<kept ? &value->attachments[i] : &scratch;
...
static SCHEMA_UNUSED int loadout_config_load_messages(LoadoutConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!loadout_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int profile_config_save_message_body(TableBitWriter * w,const ProfileConfig * value)
{
 ...
 if(value->name_length != 0) {
  ...
  table_bit_align_write(w); table_bit_putbytes(w,value->name,value->name_length);
 }
 }
 { /* icon */
 if(value->icon_length<0 || value->icon_length>16) return 0;
 if(value->icon_length != 0) {
  ...
  table_bit_align_write(w); table_bit_putbytes(w,value->icon,value->icon_length);
 }
 }
 { /* experience */
 if(!(value->experience == 0)) {
 ...
 { /* ratings */
 int rides=0; { int32_t i; for(i=0;i<4;i++) if(!(value->ratings[i] == 0.0f)) { rides=1; break; } }
 if(rides) {
  if(w->check_default) {w->bits=kTableMessageRefBitsHere+1;return 1;}
  table_bit_put(w,48,kTableMessageRefBitsHere);
  if(4<0 || 4>4) return 0;
  table_bit_put(w,(uint64_t)4-4,0);
  { int32_t i; for(i=0;i<4;i++) {
...
{
 profile_config_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
 ...
 {uint64_t n,kept;if(!table_bit_get(&r->bits,&n,table_bits_required(0,entry->max)))goto malformed;
  ...
  {const uint8_t * text=r->bits.buffer+(r->bits.offset>>3);if(!table_wire_utf8(text,n))goto malformed;kept=table_wire_utf8_clamp(text,n,32);memcpy(value->name,text,(size_t)kept);value->name[kept]=0;}
  r->bits.offset+=(int64_t)n*8;
  if(kept<n)r->report->clamped++;value->name_length=(int32_t)kept; }
 break;}
 case UINT64_C(0xdbff4cc56c2092f0): {
 if(!(entry->kind==14 && entry->elem_kind==6)){if(entry->elem_kind==0 && table_kind_widens(entry->kind,14)){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,kept;if(!table_bit_get(&r->bits,&n,table_bits_required(entry->min,entry->max)))goto malformed;
  if(!table_bit_align_read(&r->bits))goto malformed;
  if(n>INT64_MAX/8 || !table_bit_has(&r->bits,(int64_t)n*8))goto malformed;
  kept=n>16 ? 16 : n;
  if(!table_bit_getbytes(&r->bits,value->icon,(int64_t)kept)||!table_message_skip_run(r,n-kept,8))goto malformed;
  if(kept<n)r->report->clamped++;value->icon_length=(int32_t)kept; }
 break;}
 ...
 if(!(entry->kind==14 && entry->elem_kind==10)){if(entry->kind==14 && table_kind_widens(entry->elem_kind,10)){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;kept=n>4?4:n;if(kept<n)r->report->clamped++;
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){float scratch; float * item=i<kept ? &value->ratings[i] : &scratch;
  { float decoded; if(element_entry->kind==10) {float narrow;if(!table_message_float(r,element_entry,&narrow))goto malformed;decoded=narrow;} else {uint64_t raw;if(!table_bit_get(&r->bits,&raw,64))goto malformed;decoded=(float)table_bits_to_double(raw);}
   (*item)=decoded; }
  }if(walk<n && !table_message_skip_run(r,n-walk,element_entry->value_bits))goto malformed;
 }
 break;}
...
static SCHEMA_UNUSED int profile_config_load_messages(ProfileConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!profile_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int root_config_save_message_body(TableBitWriter * w,const RootConfig * value)
{
 ...
 if(value->version_note_length != 0) {
  ...
  table_bit_align_write(w); table_bit_putbytes(w,value->version_note,value->version_note_length);
 }
 }
 { /* weapons */
 if(value->weapons_count<0 || value->weapons_count>8) return 0;
 if(value->weapons_count>0) {
  ...
  { int32_t i; for(i=0;i<value->weapons_count;i++) {
 ...
 if(value->profiles_count>0) {
  ...
  { int32_t i; for(i=0;i<value->profiles_count;i++) {
...
{
 root_config_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
 ...
 {uint64_t n,kept;if(!table_bit_get(&r->bits,&n,table_bits_required(0,entry->max)))goto malformed;
  if(!table_bit_align_read(&r->bits))goto malformed;
  if(n>INT64_MAX/8 || !table_bit_has(&r->bits,(int64_t)n*8))goto malformed;
  {const uint8_t * text=r->bits.buffer+(r->bits.offset>>3);if(!table_wire_utf8(text,n))goto malformed;kept=table_wire_utf8_clamp(text,n,16);memcpy(value->version_note,text,(size_t)kept);value->version_note[kept]=0;}
  r->bits.offset+=(int64_t)n*8;
  if(kept<n)r->report->clamped++;value->version_note_length=(int32_t)kept; }
 break;}
 case UINT64_C(0x41cbd901b87fabb6): {
 if(!(entry->kind==14 && entry->elem_kind==13)){if(entry->kind==14 && table_kind_widens(entry->elem_kind,13)){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;kept=n>8?8:n;if(kept<n)r->report->clamped++;
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){WeaponConfig scratch; WeaponConfig * item=i<kept ? &value->weapons[i] : &scratch;
  if(!weapon_config_load_message_body(r,&(*item)))return 0;
  }if(walk<n && !table_message_skip_run(r,n-walk,element_entry->value_bits))goto malformed;
 ...
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;kept=n>4?4:n;if(kept<n)r->report->clamped++;
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){ProfileConfig scratch; ProfileConfig * item=i<kept ? &value->profiles[i] : &scratch;
...
static SCHEMA_UNUSED int root_config_load_messages(RootConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!root_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
{
 attachment_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
...
static SCHEMA_UNUSED int attachment_load_messages(Attachment * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!attachment_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
{
 buff_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
...
static SCHEMA_UNUSED int buff_load_messages(Buff * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!buff_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
{
 debuff_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
...
static SCHEMA_UNUSED int debuff_load_messages(Debuff * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!debuff_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
