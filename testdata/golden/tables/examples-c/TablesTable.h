...
static SCHEMA_UNUSED const uint64_t weapon_config_fixed_hash = 0x1b545259baa4f450ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 11 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t weapon_config_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t weapon_config_fixed_vocab_bytes = (int64_t) sizeof( weapon_config_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( weapon_config_fixed_vocab ) + count * ( 8 + 22 );
}
...
static SCHEMA_UNUSED int64_t weapon_config_fixed_save( const WeaponConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( weapon_config_fixed_vocab ) );
    memcpy( buffer + 5, weapon_config_fixed_vocab, sizeof( weapon_config_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( weapon_config_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != weapon_config_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, weapon_config_fixed_vocab, (int32_t) sizeof( weapon_config_fixed_vocab ), weapon_config_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
...
static SCHEMA_UNUSED const uint64_t loadout_config_fixed_hash = 0xe747768b18247870ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 43 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t loadout_config_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t loadout_config_fixed_vocab_bytes = (int64_t) sizeof( loadout_config_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( loadout_config_fixed_vocab ) + count * ( 8 + 154 );
}
...
static SCHEMA_UNUSED int64_t loadout_config_fixed_save( const LoadoutConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( loadout_config_fixed_vocab ) );
    memcpy( buffer + 5, loadout_config_fixed_vocab, sizeof( loadout_config_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( loadout_config_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != loadout_config_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_vocab_read( block, (int64_t) block_bytes, &parsed ) ) { report->refused = 1; report->reason = SCHEMA_TABLE_BLOCK_MALFORMED; return -1; }
        made = table_fixed_compile( &parsed, loadout_config_fixed_vocab, (int32_t) sizeof( loadout_config_fixed_vocab ), loadout_config_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_BLOCK_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
