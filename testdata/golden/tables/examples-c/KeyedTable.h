...
static SCHEMA_UNUSED int team_config_save_body( TableWriter * w, const TeamConfig * value )
{
    ...
    { /* spawn_count */
        if ( !( value->spawn_count == 4 ) )
        {
            ...
            table_writer_header_at( w, 121, 0xceec99e2d65db674ull, 4 );
    ...
    { /* banner */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_header_at( w, 109, 0xbca0dab1c7a00ccfull, 12 );
            if ( w->buffer == NULL )
            {
...
static SCHEMA_UNUSED int gunner_config_save_body( TableWriter * w, const GunnerConfig * value )
{
    ...
    if ( w->buffer == NULL )
    {
        ...
        {
            table_writer_id_at( w, 102, 0xb75aa3662201646aull );
            payload_bytes += 5; /* kind and fixed-width payload */
        }
        if ( !( value->tracking == 0 ) )
        {
            table_writer_id_at( w, 91, 0xa6bf719a4602b0bcull );
    ...
    { /* reaction */
        if ( !( value->reaction == 0.2f ) )
        {
            table_writer_header_at( w, 102, 0xb75aa3662201646aull, 10 );
    ...
    { /* tracking */
        if ( !( value->tracking == 0 ) )
        {
            table_writer_header_at( w, 91, 0xa6bf719a4602b0bcull, 1 );
...
static SCHEMA_UNUSED int turret_config_save_body( TableWriter * w, const TurretConfig * value )
{
    ...
    { /* cooldown */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_header_at( w, 134, 0xdc2cbe6953343d48ull, 10 );
    ...
    { /* gunner */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_header_at( w, 39, 0x40dbb648c0cd44aaull, 13 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
static SCHEMA_UNUSED int hull_config_save_body( TableWriter * w, const HullConfig * value )
{
    ...
    { /* health */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_header_at( w, 68, 0x7f69d4b5288ba9cfull, 10 );
    ...
    { /* turrets */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_header_at( w, 72, 0x84f8260bc283608cull, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
  ...
  int32_t i;
   ...
   for(i=0;i<3;i++) {
    ...
    switch(i+1) {
     ...
     break;
     case WEAPON_CANNON: table_bit_put(w,126,kTableMessageRefBitsHere);
     break;
     case WEAPON_MISSILE: table_bit_put(w,127,kTableMessageRefBitsHere);
     break;
     case WEAPON_MINE: table_bit_put(w,128,kTableMessageRefBitsHere);
     break;
     default: return 0;
    }
...
static SCHEMA_UNUSED int keyed_config_save_body( TableWriter * w, const KeyedConfig * value )
{
    ...
    { /* teams */
        ...
        if ( rides )
        {
            table_writer_header_at( w, 107, 0xbaaeb048a5a8fa6dull, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
    ...
    { /* hulls */
        ...
        if ( rides )
        {
            table_writer_header_at( w, 120, 0xce0ac3c25694d8ffull, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
    ...
    { /* season */
        if ( !( value->season == 0 ) )
        {
            table_writer_header_at( w, 25, 0x28dfd5e2759ba3f8ull, 8 );
            table_writer_put32( w, (uint32_t) ( value->season ) );
        }
    }
    table_writer_leb( w, 0 );
    return !w->overflow;
...
static SCHEMA_UNUSED int keyed_config_save_message_body(TableBitWriter * w,const KeyedConfig * value)
{
  ...
  int32_t i;
   ...
   for(i=0;i<3;i++) {
    ...
    switch(i+1) {
     ...
     break;
     case TEAM_RED: table_bit_put(w,123,kTableMessageRefBitsHere);
     break;
     case TEAM_BLUE: table_bit_put(w,124,kTableMessageRefBitsHere);
     break;
     case TEAM_GREEN: table_bit_put(w,125,kTableMessageRefBitsHere);
     break;
     default: return 0;
    }
  ...
  int32_t i;
   ...
   for(i=0;i<3;i++) {
    ...
    switch(i+1) {
     ...
     break;
     case HULL_INTERCEPTOR: table_bit_put(w,117,kTableMessageRefBitsHere);
     break;
     case HULL_GUNSHIP: table_bit_put(w,118,kTableMessageRefBitsHere);
     break;
     case HULL_FREIGHTER: table_bit_put(w,119,kTableMessageRefBitsHere);
     break;
     default: return 0;
    }
 ...
 if(!(value->season == 0)) {
  ...
  table_bit_put(w,(uint64_t)(value->season)-UINT64_C(0),32);
 }
 }
 table_bit_put(w,0,kTableMessageRefBitsHere);
 return !w->overflow;
...
static SCHEMA_UNUSED int keyed_config_load_message_body(TableMessageReader * r,KeyedConfig * value)
{
 ...
 case UINT64_C(0x28dfd5e2759ba3f8): {
...
static SCHEMA_UNUSED int score_board_save_body( TableWriter * w, const ScoreBoard * value )
{
    ...
    { /* per_team */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_header_at( w, 143, 0xf10fad739a0e1660ull, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
SCHEMA_TABLE_STATIC_ASSERT( fixed_keyed_config_size, sizeof( KeyedConfig ) == 304, "KeyedConfig: the fixed form's plan is laid out for this size" );
...
SCHEMA_TABLE_STATIC_ASSERT( fixed_keyed_config_season, offsetof( KeyedConfig, season ) == 300, "KeyedConfig.season: the fixed form's plan lands here" );
...
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE void schema_tabledemo_keyed_config_fixed_write_body_( uint8_t * b, const KeyedConfig * value )
{
    ...
    table_fixed_put32( b + 234, (uint32_t) value->season );
}
...
static SCHEMA_UNUSED const TableFixedEntry team_config_fixed_plan[] = {
    { 0u, 0u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* spawn_count */
    { 4u, 24u, 16u, 4u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 0, 1, 0, 0, 1 }, /* banner */
};
...
static SCHEMA_UNUSED const TableFixedFill team_config_fixed_cover[] = {
    { 0u, 21u },
    { 24u, 4u },
};
static SCHEMA_UNUSED const int32_t team_config_fixed_cover_count = 2;
...
/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   ...
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout team_config_fixed_known[] = {
    { 0x5a61da98d3a823e3ull, team_config_fixed_layout, (int64_t) sizeof( team_config_fixed_layout ), 32 },
};
static SCHEMA_UNUSED const int32_t team_config_fixed_known_count = 1;
...
static SCHEMA_UNUSED const int32_t team_config_fixed_floor = 0;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    pick = table_fixed_select( team_config_fixed_known, team_config_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < team_config_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    ...
    if ( (int64_t) layout_bytes != team_config_fixed_known[pick].layout_bytes ||
         memcmp( layout, team_config_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    ...
    record_bytes = team_config_fixed_known[pick].record_bytes;
...
/* THE TYPE'S VALUE BYTES: every byte of this build's own storage that holds
   ...
   twice; a plan compiled from a stranger's layout subtracts itself from it
   once, when the plan is compiled, and prefills exactly the rest. */
static SCHEMA_UNUSED const TableFixedFill gunner_config_fixed_cover[] = {
    { 0u, 5u },
};
static SCHEMA_UNUSED const int32_t gunner_config_fixed_cover_count = 1;

/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   A FILE IS MATCHED ON IS THE HASH; the layout bytes are what a known hash
   is held to by memcmp, and the record size is taken from here and NEVER
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout gunner_config_fixed_known[] = {
    { 0x113a539593602242ull, gunner_config_fixed_layout, (int64_t) sizeof( gunner_config_fixed_layout ), 13 },
};
static SCHEMA_UNUSED const int32_t gunner_config_fixed_known_count = 1;
/* THE FLOOR IS ONE NUMBER AND THE LINEAGE IS ONE ARRAY, so "retired" is an
   index cut and the operator's two answers stay distinct: below the floor is
   layout_unsupported (upgrade the client), outside the lineage is
   layout_newer (ship the reader). A retired entry stays forever (§5.2). */
static SCHEMA_UNUSED const int32_t gunner_config_fixed_floor = 0;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    hash = table_fixed_get64( data + kTableFixedHashAt );
    /* 5 and 6. SELECT BY HASH, THEN THE FLOOR. Both report THE FILE'S hash. */
    pick = table_fixed_select( gunner_config_fixed_known, gunner_config_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < gunner_config_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    /* 7. A KNOWN HASH IS READ UNDER THE LOCK'S LAYOUT BYTES. A difference is ONE
       name — a lie about a known version — and §1.1's seven rules do not run at
       read time at all: no stranger's layout is ever walked. */
    if ( (int64_t) layout_bytes != gunner_config_fixed_known[pick].layout_bytes ||
         memcmp( layout, gunner_config_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    { report->refused = 1; report->reason = SCHEMA_TABLE_LAYOUT_MALFORMED; return -1; }
    /* 8. the plan this peer resolves to, and the record size FROM THE LOCK. */
    record_bytes = gunner_config_fixed_known[pick].record_bytes;
    /* 9 and 10. the tail must be whole records, and they must fit the caller. */
    at = layout + layout_bytes;
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    if ( record_bytes <= 8 || rest % record_bytes != 0 ) { report->malformed = 1; return -1; }
...
static SCHEMA_UNUSED const TableFixedEntry turret_config_fixed_plan[] = {
    { 0u, 0u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 8u, 16u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 9u, 8u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
};
...
/* THE TYPE'S VALUE BYTES: every byte of this build's own storage that holds
   ...
   twice; a plan compiled from a stranger's layout subtracts itself from it
   once, when the plan is compiled, and prefills exactly the rest. */
static SCHEMA_UNUSED const TableFixedFill turret_config_fixed_cover[] = {
    { 0u, 13u },
    { 16u, 1u },
};
static SCHEMA_UNUSED const int32_t turret_config_fixed_cover_count = 2;

/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   A FILE IS MATCHED ON IS THE HASH; the layout bytes are what a known hash
   is held to by memcmp, and the record size is taken from here and NEVER
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout turret_config_fixed_known[] = {
    { 0xd8715b0be3854d91ull, turret_config_fixed_layout, (int64_t) sizeof( turret_config_fixed_layout ), 22 },
};
static SCHEMA_UNUSED const int32_t turret_config_fixed_known_count = 1;
/* THE FLOOR IS ONE NUMBER AND THE LINEAGE IS ONE ARRAY, so "retired" is an
   index cut and the operator's two answers stay distinct: below the floor is
   layout_unsupported (upgrade the client), outside the lineage is
   layout_newer (ship the reader). A retired entry stays forever (§5.2). */
static SCHEMA_UNUSED const int32_t turret_config_fixed_floor = 0;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    hash = table_fixed_get64( data + kTableFixedHashAt );
    /* 5 and 6. SELECT BY HASH, THEN THE FLOOR. Both report THE FILE'S hash. */
    pick = table_fixed_select( turret_config_fixed_known, turret_config_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < turret_config_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    /* 7. A KNOWN HASH IS READ UNDER THE LOCK'S LAYOUT BYTES. A difference is ONE
       name — a lie about a known version — and §1.1's seven rules do not run at
       read time at all: no stranger's layout is ever walked. */
    if ( (int64_t) layout_bytes != turret_config_fixed_known[pick].layout_bytes ||
         memcmp( layout, turret_config_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    { report->refused = 1; report->reason = SCHEMA_TABLE_LAYOUT_MALFORMED; return -1; }
    /* 8. the plan this peer resolves to, and the record size FROM THE LOCK. */
    record_bytes = turret_config_fixed_known[pick].record_bytes;
    /* 9 and 10. the tail must be whole records, and they must fit the caller. */
    at = layout + layout_bytes;
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    if ( record_bytes <= 8 || rest % record_bytes != 0 ) { report->malformed = 1; return -1; }
...
static SCHEMA_UNUSED const TableFixedEntry hull_config_fixed_plan[] = {
    { 0u, 0u, 16u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 16u, 24u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 17u, 16u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 22u, 28u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 30u, 44u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 31u, 36u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 36u, 48u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 44u, 64u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 45u, 56u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
};
...
/* THE TYPE'S VALUE BYTES: every byte of this build's own storage that holds
   ...
   twice; a plan compiled from a stranger's layout subtracts itself from it
   once, when the plan is compiled, and prefills exactly the rest. */
static SCHEMA_UNUSED const TableFixedFill hull_config_fixed_cover[] = {
    { 0u, 21u },
    { 24u, 1u },
    { 28u, 13u },
    { 44u, 1u },
...
static SCHEMA_UNUSED const int32_t hull_config_fixed_cover_count = 6;

/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   A FILE IS MATCHED ON IS THE HASH; the layout bytes are what a known hash
   is held to by memcmp, and the record size is taken from here and NEVER
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout hull_config_fixed_known[] = {
    { 0xc65dc3bc15d5a954ull, hull_config_fixed_layout, (int64_t) sizeof( hull_config_fixed_layout ), 58 },
};
static SCHEMA_UNUSED const int32_t hull_config_fixed_known_count = 1;
/* THE FLOOR IS ONE NUMBER AND THE LINEAGE IS ONE ARRAY, so "retired" is an
   index cut and the operator's two answers stay distinct: below the floor is
   layout_unsupported (upgrade the client), outside the lineage is
   layout_newer (ship the reader). A retired entry stays forever (§5.2). */
static SCHEMA_UNUSED const int32_t hull_config_fixed_floor = 0;
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    hash = table_fixed_get64( data + kTableFixedHashAt );
    /* 5 and 6. SELECT BY HASH, THEN THE FLOOR. Both report THE FILE'S hash. */
    pick = table_fixed_select( hull_config_fixed_known, hull_config_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < hull_config_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    /* 7. A KNOWN HASH IS READ UNDER THE LOCK'S LAYOUT BYTES. A difference is ONE
       name — a lie about a known version — and §1.1's seven rules do not run at
       read time at all: no stranger's layout is ever walked. */
    if ( (int64_t) layout_bytes != hull_config_fixed_known[pick].layout_bytes ||
         memcmp( layout, hull_config_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    { report->refused = 1; report->reason = SCHEMA_TABLE_LAYOUT_MALFORMED; return -1; }
    /* 8. the plan this peer resolves to, and the record size FROM THE LOCK. */
    record_bytes = hull_config_fixed_known[pick].record_bytes;
    /* 9 and 10. the tail must be whole records, and they must fit the caller. */
    at = layout + layout_bytes;
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    if ( record_bytes <= 8 || rest % record_bytes != 0 ) { report->malformed = 1; return -1; }
...
/* THE BODY IS ONE CONSTANT on this form: it is the same size for every
   value the type can hold (docs/SPEC-TABLES.md §3.4). */
static SCHEMA_UNUSED const int64_t keyed_config_fixed_body_bytes = 238;
static SCHEMA_UNUSED const int64_t keyed_config_fixed_record_bytes = 8 + 238; /* the hash and the body */
static SCHEMA_UNUSED const uint64_t keyed_config_fixed_hash = 0xa357cf9e47e391b6ull; /* fnv1a64 over the layout and the definitions digest (bill §13) */
...
static SCHEMA_UNUSED const TableFixedEntry keyed_config_fixed_plan[] = {
    { 0u, 0u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* spawn_count */
    { 4u, 24u, 16u, 4u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 0, 1, 0, 0, 1 }, /* banner */
    { 24u, 28u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* spawn_count */
    { 28u, 52u, 16u, 32u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 0, 1, 0, 0, 1 }, /* banner */
    { 48u, 56u, 4u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* spawn_count */
    { 52u, 80u, 16u, 60u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 0, 1, 0, 0, 1 }, /* banner */
    { 72u, 84u, 16u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 88u, 108u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 89u, 100u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 94u, 112u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 102u, 128u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 103u, 120u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 108u, 132u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 116u, 148u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 117u, 140u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 122u, 152u, 16u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 138u, 176u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 139u, 168u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 144u, 180u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 152u, 196u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 153u, 188u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 158u, 200u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 166u, 216u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 167u, 208u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 172u, 220u, 16u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* health */
    { 188u, 244u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 189u, 236u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 194u, 248u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 202u, 264u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 203u, 256u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 208u, 268u, 8u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* damage */
    { 216u, 284u, 1u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* gunner present */
    { 217u, 276u, 5u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* reaction */
    { 222u, 288u, 16u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0, 0, 1 }, /* per_team, whole */
};
...
/* THE TYPE'S VALUE BYTES: every byte of this build's own storage that holds
   ...
   twice; a plan compiled from a stranger's layout subtracts itself from it
   once, when the plan is compiled, and prefills exactly the rest. */
static SCHEMA_UNUSED const TableFixedFill keyed_config_fixed_cover[] = {
    { 0u, 21u },
    { 24u, 25u },
    { 52u, 25u },
    { 80u, 25u },
...
/* THE LINEAGE, OLDEST FIRST, THE CURRENT LAYOUT LAST (§5.2). THE ONLY FACT
   ...
   is held to by memcmp, and the record size is taken from here and NEVER
   from the file. */
static SCHEMA_UNUSED const TableFixedKnownLayout keyed_config_fixed_known[] = {
    { 0x39fbd26a1a70582bull, keyed_config_fixed_known_0, (int64_t) sizeof( keyed_config_fixed_known_0 ), 242 },
    { 0xa357cf9e47e391b6ull, keyed_config_fixed_layout, (int64_t) sizeof( keyed_config_fixed_layout ), 246 },
};
static SCHEMA_UNUSED const int32_t keyed_config_fixed_known_count = 2;
/* THE FLOOR IS ONE NUMBER AND THE LINEAGE IS ONE ARRAY, so "retired" is an
   index cut and the operator's two answers stay distinct: below the floor is
   layout_unsupported (upgrade the client), outside the lineage is
   layout_newer (ship the reader). A retired entry stays forever (§5.2). */
...
/* ONE PLAN PER OLDER LINEAGE ENTRY, BUILT FROM THE LOCK'S OWN BYTES and
   ...
   allocates nothing — and the walk runs ONCE PER PROCESS, off the record
   loop: a load selects a plan and never builds one, and a load can no
   longer fail for want of one. C HAS NO PACKAGE INITIALIZER (§5.9 #7 of
   this leg's PR), so "once" is this flag and not a language phase; what
   §5.8 row 3 forbids — compiling from A FILE'S bytes into a cache a load
   can miss — is gone either way: the bytes here are the lock's. */
#define KEYED_CONFIG_FIXED_LINEAGE_STRIDE 320
...
#endif
#endif
static TableFixedEntry keyed_config_fixed_lineage_storage[2][KEYED_CONFIG_FIXED_LINEAGE_STRIDE];
static TableFixedLineagePlan keyed_config_fixed_lineage[2];
static TableFixedOnce keyed_config_fixed_lineage_ready = SCHEMA_TABLE_FIXED_ONCE_UNBUILT;
static SCHEMA_UNUSED void keyed_config_fixed_lineage_build( void )
{
    int32_t i;
    /* THE FLAG IS PUBLISHED LAST, AFTER THE LOOP, and a racing load WAITS for
       it: a second thread that sees the plans unpublished must never read
       them, because an unbuilt TableFixedLineagePlan is all zeros — entries
       NULL, count 0 — and zeros are a plan step 8 accepts, landing a
       prefill-only record where the read owed a refusal. So: READY is an
       acquire load, the build is CLAIMED by one thread, every other thread
       waits on that acquire, and the publish is a release store of READY
       after the last entry is written (§5.9 #7). */
    if ( table_fixed_once_ready( &keyed_config_fixed_lineage_ready ) ) { return; }
    if ( !table_fixed_once_claim( &keyed_config_fixed_lineage_ready ) )
    {
        table_fixed_once_wait( &keyed_config_fixed_lineage_ready ); /* WAIT — never the zeroed plan */
        return;
    }
    for ( i = 0; i < keyed_config_fixed_known_count; ++i )
    {
        TableFixedLayoutView parsed;
        TableReport census;
        TableFixedEntry * dest = keyed_config_fixed_lineage_storage[i];
        int32_t guarded = 0;
        int32_t made;
        int32_t fills = 0;
        uint32_t fill_at = 0;
        int why = SCHEMA_TABLE_LAYOUT_MALFORMED;
        memset( &keyed_config_fixed_lineage[i], 0, sizeof( keyed_config_fixed_lineage[i] ) );
        if ( keyed_config_fixed_known[i].hash == keyed_config_fixed_hash ) { continue; } /* the identity entry: the baked plan answers it */
        memset( &census, 0, sizeof( census ) );
        /* A LINEAGE ENTRY THAT IS NOT A PARSEABLE LAYOUT IS A BUG IN THE LOCK
           (§5.9 #8). C cannot fail a build from a header, so the entry carries
           the name and LOAD refuses by it if a file ever matches its hash. */
        if ( !table_fixed_parse_layout( keyed_config_fixed_known[i].layout, keyed_config_fixed_known[i].layout_bytes, &parsed, &why ) )
        { keyed_config_fixed_lineage[i].reason = SCHEMA_TABLE_LAYOUT_MALFORMED; continue; }
        made = table_fixed_compile( &parsed, keyed_config_fixed_layout, (int32_t) sizeof( keyed_config_fixed_layout ), keyed_config_fixed_dst,
                                    keyed_config_fixed_cover, keyed_config_fixed_cover_count,
                                    dest, KEYED_CONFIG_FIXED_LINEAGE_STRIDE, &guarded, &fill_at, &fills, &census );
        if ( made < 0 )
        {
            /* §5.9 #4: the DECLARED CAPACITY a build holds an entry to is real,
               and an entry whose plan exceeds it records plan_too_large ON THAT
               ENTRY. A layout that is not one is the lock's bug, by its name. */
            keyed_config_fixed_lineage[i].reason = ( made == -2 ) ? SCHEMA_TABLE_LAYOUT_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE;
            continue;
        }
        keyed_config_fixed_lineage[i].entries = dest;
        keyed_config_fixed_lineage[i].count = made;
        keyed_config_fixed_lineage[i].guarded = guarded;
        keyed_config_fixed_lineage[i].fill = ( fills > 0 ) ? (const TableFixedFill *) (const void *) ( (const uint8_t *) dest + fill_at ) : NULL;
        keyed_config_fixed_lineage[i].fill_count = fills;
        /* THE COMPILE CENSUS, ONCE PER PEER and never per record (§5.4). */
        keyed_config_fixed_lineage[i].unknown = census.unknown;
        keyed_config_fixed_lineage[i].kind_mismatch = census.kind_mismatch;
    }
    /* AND THE FLAG LAST: every entry above is written before this store, and
       this store is what a waiting thread's acquire is waiting for. */
    table_fixed_once_publish( &keyed_config_fixed_lineage_ready );
}
...
{
    return kTableFixedHeaderBytes + 4 + (int64_t) sizeof( keyed_config_fixed_layout ) + count * ( 8 + 238 );
}
...
static SCHEMA_UNUSED int64_t keyed_config_fixed_save( const KeyedConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) 238 ); /* the prefill's zeros */
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    hash = table_fixed_get64( data + kTableFixedHashAt );
    /* 5 and 6. SELECT BY HASH, THEN THE FLOOR. Both report THE FILE'S hash. */
    pick = table_fixed_select( keyed_config_fixed_known, keyed_config_fixed_known_count, hash );
    if ( pick < 0 ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_NEWER, hash ); }
    if ( pick < keyed_config_fixed_floor ) { return table_fixed_refuse_hash( report, SCHEMA_TABLE_LAYOUT_UNSUPPORTED, hash ); }
    /* 7. A KNOWN HASH IS READ UNDER THE LOCK'S LAYOUT BYTES. A difference is ONE
       name — a lie about a known version — and §1.1's seven rules do not run at
       read time at all: no stranger's layout is ever walked. */
    if ( (int64_t) layout_bytes != keyed_config_fixed_known[pick].layout_bytes ||
         memcmp( layout, keyed_config_fixed_known[pick].layout, (size_t) layout_bytes ) != 0 )
    { report->refused = 1; report->reason = SCHEMA_TABLE_LAYOUT_MALFORMED; return -1; }
    /* 8. the plan this peer resolves to, and the record size FROM THE LOCK. */
    record_bytes = keyed_config_fixed_known[pick].record_bytes;
    if ( hash != keyed_config_fixed_hash )
    {
        keyed_config_fixed_lineage_build(); /* once per process, from THE LOCK'S bytes */
        if ( keyed_config_fixed_lineage[pick].reason != 0 ) { report->refused = 1; report->reason = keyed_config_fixed_lineage[pick].reason; return -1; }
        if ( keyed_config_fixed_lineage[pick].count > plan_capacity ) { report->refused = 1; report->reason = SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = keyed_config_fixed_lineage[pick].entries;
        entry_count = keyed_config_fixed_lineage[pick].count;
        entry_guarded = keyed_config_fixed_lineage[pick].guarded;
        fill = keyed_config_fixed_lineage[pick].fill;
        fill_count = keyed_config_fixed_lineage[pick].fill_count;
        /* THE CENSUS IS THE PLAN'S OWN NUMBER, landed ONCE after the record loop
           and only on a read that RETURNS: REFUSE moves no counter (§5.9 #6). */
        census_unknown = keyed_config_fixed_lineage[pick].unknown;
        census_kind = keyed_config_fixed_lineage[pick].kind_mismatch;
    }
    /* 9 and 10. the tail must be whole records, and they must fit the caller. */
    at = layout + layout_bytes;
    rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
...
static SCHEMA_UNUSED int team_config_save_body_retain(TableWriter *,const TeamConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int team_config_load_body_retain(TableReader *,TeamConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int gunner_config_save_body_retain(TableWriter *,const GunnerConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int gunner_config_load_body_retain(TableReader *,GunnerConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int turret_config_save_body_retain(TableWriter *,const TurretConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int turret_config_load_body_retain(TableReader *,TurretConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int hull_config_save_body_retain(TableWriter *,const HullConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int hull_config_load_body_retain(TableReader *,HullConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int keyed_config_save_body_retain(TableWriter *,const KeyedConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int keyed_config_load_body_retain(TableReader *,KeyedConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int score_board_save_body_retain(TableWriter *,const ScoreBoard *, TableRetainWalk retention);
static SCHEMA_UNUSED int score_board_load_body_retain(TableReader *,ScoreBoard *, TableRetainWalk retention);
static SCHEMA_UNUSED int team_config_load_message_body_retain(TableMessageReader *,TeamConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int gunner_config_load_message_body_retain(TableMessageReader *,GunnerConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int turret_config_load_message_body_retain(TableMessageReader *,TurretConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int hull_config_load_message_body_retain(TableMessageReader *,HullConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int keyed_config_load_message_body_retain(TableMessageReader *,KeyedConfig *, TableRetainWalk retention);
static SCHEMA_UNUSED int score_board_load_message_body_retain(TableMessageReader *,ScoreBoard *, TableRetainWalk retention);
static SCHEMA_UNUSED int schema_tabledemo_team_config_wire_banner__retain( TableWriter * w, const TeamConfig * value , TableRetainWalk retention)
{
    (void)retention;
...
static SCHEMA_UNUSED int team_config_save_body_retain( TableWriter * w, const TeamConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    retention=table_retain_step(body_keep,0,0);
        if ( !( value->spawn_count == 4 ) )
        {
            ...
            table_retain_writer_id( w, 0xceec99e2d65db674ull , retention);
    ...
    retention=table_retain_step(body_keep,1,0);
        ...
        if ( value->banner_length != 0 )
        {
            ...
            table_retain_writer_id( w, 0xbca0dab1c7a00ccfull , retention);
            table_writer_put8( w, 12 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
                if ( !schema_tabledemo_team_config_wire_banner__retain( w, value , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                ...
                if ( !schema_tabledemo_team_config_wire_banner__retain( &probe, value , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !schema_tabledemo_team_config_wire_banner__retain( w, value , retention) ) { return 0;
...
static SCHEMA_UNUSED int team_config_load_body_retain( TableReader * r, TeamConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    for ( ;; )
    {
        ...
        switch ( id )
        {
            ...
            case 0xbca0dab1c7a00ccfull: /* banner */
            {
                ...
                memcpy( value->banner, sub.buffer, (size_t) keep );
                value->banner[keep] = 0;
...
static SCHEMA_UNUSED int team_config_load_message_body_retain(TableMessageReader * r,TeamConfig * value, TableRetainWalk retention)
{
    (void)retention;
...
static SCHEMA_UNUSED int gunner_config_save_body_retain( TableWriter * w, const GunnerConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    retention=table_retain_step(body_keep,0,0);
        if ( !( value->reaction == 0.2f ) )
        {
            table_retain_writer_id( w, 0xb75aa3662201646aull , retention);
    ...
    retention=table_retain_step(body_keep,1,0);
        if ( !( value->tracking == 0 ) )
        {
            table_retain_writer_id( w, 0xa6bf719a4602b0bcull , retention);
            table_writer_put8( w, 1 );
            table_writer_put8( w, value->tracking ? 1 : 0 );
        }
    }
...
static SCHEMA_UNUSED int gunner_config_load_body_retain( TableReader * r, GunnerConfig * value , TableRetainWalk retention)
{
    (void)retention;
...
static SCHEMA_UNUSED int gunner_config_load_message_body_retain(TableMessageReader * r,GunnerConfig * value, TableRetainWalk retention)
{
    (void)retention;
...
static SCHEMA_UNUSED int turret_config_save_body_retain( TableWriter * w, const TurretConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    retention=table_retain_step(body_keep,1,0);
        if ( !( value->cooldown == 0.5f ) )
        {
            ...
            table_retain_writer_id( w, 0xdc2cbe6953343d48ull , retention);
            table_writer_put8( w, 10 );
            table_writer_put32( w, table_float_to_bits( value->cooldown ) );
        }
    }
    ...
    retention=table_retain_step(body_keep,2,0);
        if ( value->gunner_present )
        {
            ...
            table_retain_writer_id( w, 0x40dbb648c0cd44aaull , retention);
            table_writer_put8( w, 13 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
                if ( !gunner_config_save_body_retain( w, &value->gunner , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                TableWriter probe = table_retain_probe( w , retention);
                if ( !gunner_config_save_body_retain( &probe, &value->gunner , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !gunner_config_save_body_retain( w, &value->gunner , retention) ) { return 0;
...
static SCHEMA_UNUSED int turret_config_load_body_retain( TableReader * r, TurretConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    for ( ;; )
    {
        ...
        switch ( id )
        {
            ...
            case 0x40dbb648c0cd44aaull: /* gunner */
            {
                ...
                gunner_config_load_body_retain( &sub, &value->gunner , retention);
...
static SCHEMA_UNUSED int turret_config_load_message_body_retain(TableMessageReader * r,TurretConfig * value, TableRetainWalk retention)
{
    (void)retention;
 ...
 retention=table_retain_step(body_keep,2,0);
 table_retain_discard(body_keep,1,2);
 if(!gunner_config_load_message_body_retain(r,&value->gunner, retention))return 0;
 value->gunner_present=1;
 break;
 }
 default:r->report->unknown += 1;
...
static SCHEMA_UNUSED int schema_tabledemo_hull_config_wire_turrets__retain( TableWriter * w, const HullConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    for ( i = 0; i < WEAPON_MAX; i++ )
    {
        ...
        if ( !turret_config_save_body_retain( &slot_probe, &value->turrets[i] , retention) ) { return 0;
        }
    ...
    for ( i = 0; i < WEAPON_MAX; i++ )
    {
        ...
        switch ( i + 1 )
        {
            case WEAPON_NONE: table_writer_leb( w, 0 );
            break;
            case WEAPON_CANNON: table_retain_writer_id( w, 0x5854debe3b2e767cull , retention);
            break;
            case WEAPON_MISSILE: table_retain_writer_id( w, 0xb528592e5a4583e3ull , retention);
            break;
            case WEAPON_MINE: table_retain_writer_id( w, 0x04dc16aea8ff5276ull , retention);
            break;
            default: return 0;
        ...
        {
            int64_t frame_begin = w->offset;
            if ( !turret_config_save_body_retain( w, &value->turrets[i] , retention) ) { return 0;
            }
            table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
        }
        else
        {
            TableWriter probe = table_retain_probe( w , retention);
            if ( !turret_config_save_body_retain( &probe, &value->turrets[i] , retention) ) { return 0;
            }
            table_retain_rewind(&probe, retention);
            table_writer_leb( w, (uint64_t) probe.offset );
            if ( !turret_config_save_body_retain( w, &value->turrets[i] , retention) ) { return 0;
...
static SCHEMA_UNUSED int hull_config_save_body_retain( TableWriter * w, const HullConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    retention=table_retain_step(body_keep,1,0);
        if ( !( value->mass == 1.0f ) )
        {
            ...
            table_retain_writer_id( w, 0x1f3757a2ce7b0ab1ull , retention);
            table_writer_put8( w, 10 );
            table_writer_put32( w, table_float_to_bits( value->mass ) );
        }
    }
    ...
    retention=table_retain_step(body_keep,2,0);
        ...
        if ( rides )
        {
            ...
            table_retain_writer_id( w, 0x84f8260bc283608cull , retention);
            table_writer_put8( w, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
                if ( !schema_tabledemo_hull_config_wire_turrets__retain( w, value , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                TableWriter probe = table_retain_probe( w , retention);
                if ( !schema_tabledemo_hull_config_wire_turrets__retain( &probe, value , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !schema_tabledemo_hull_config_wire_turrets__retain( w, value , retention) ) { return 0;
...
static SCHEMA_UNUSED int hull_config_load_body_retain( TableReader * r, HullConfig * value , TableRetainWalk retention)
{
    (void)retention;
            ...
            case 0x84f8260bc283608cull: /* turrets */
            {
                ...
                if ( sub.size >= 2 )
                {
                    ...
                    for ( i = 0; i < count; i++ )
                    {
                        ...
                        switch ( key )
                        {
                            case 0x5854debe3b2e767cull: slot = 0;
                            break;
                            case 0xb528592e5a4583e3ull: slot = 1;
                            break;
                            case 0x04dc16aea8ff5276ull: slot = 2;
                            break;
                            default: r->report->unknown++;
                            r->report->retain_lost += retention.retain != NULL;
                            break;
                        ...
                        turret_config_load_body_retain( &elem, &value->turrets[slot] , retention);
                        if(elem.offset!=elem.size) { r->report->malformed=1;
                        turret_config_reset(&value->turrets[slot]);
                        }
                    }
                }
                break;
...
static SCHEMA_UNUSED int hull_config_load_message_body_retain(TableMessageReader * r,HullConfig * value, TableRetainWalk retention)
{
    (void)retention;
 ...
 (void)element_entry;
  ...
  if(!turret_config_load_message_body_retain(r,&scratch, retention))return 0;
  retention=named_retention;
  }continue;
  }
  retention=table_retain_index(retention,(uint32_t)(slot));
  if(!turret_config_load_message_body_retain(r,&value->turrets[slot], retention))return 0;
 } }
 break;
 }
 default:r->report->unknown += 1;
...
static SCHEMA_UNUSED int schema_tabledemo_keyed_config_wire_teams__retain( TableWriter * w, const KeyedConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    for ( i = 0; i < TEAM_MAX; i++ )
    {
        ...
        TableWriter slot_probe = table_retain_probe( w , retention);
        slot_probe.check_default = 1;
        if ( !team_config_save_body_retain( &slot_probe, &value->teams[i] , retention) ) { return 0;
        }
        table_retain_rewind(&slot_probe, retention);
        if ( slot_probe.offset == 1 ) { continue;
        }
    ...
    for ( i = 0; i < TEAM_MAX; i++ )
    {
        ...
        switch ( i + 1 )
        {
            case TEAM_NONE: table_writer_leb( w, 0 );
            break;
            case TEAM_RED: table_retain_writer_id( w, 0x9ff1de19feac1b7cull , retention);
            break;
            case TEAM_BLUE: table_retain_writer_id( w, 0xecf3d3a7c1693e2dull , retention);
            break;
            case TEAM_GREEN: table_retain_writer_id( w, 0xcf00d78fd5953f1cull , retention);
            break;
            default: return 0;
            /* no declared identity */
        }
        ...
        {
            int64_t frame_begin = w->offset;
            if ( !team_config_save_body_retain( w, &value->teams[i] , retention) ) { return 0;
            }
            table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
        }
        else
        {
            TableWriter probe = table_retain_probe( w , retention);
            if ( !team_config_save_body_retain( &probe, &value->teams[i] , retention) ) { return 0;
            }
            table_retain_rewind(&probe, retention);
            table_writer_leb( w, (uint64_t) probe.offset );
            if ( !team_config_save_body_retain( w, &value->teams[i] , retention) ) { return 0;
...
static SCHEMA_UNUSED int schema_tabledemo_keyed_config_wire_hulls__retain( TableWriter * w, const KeyedConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    for ( i = 0; i < HULL_MAX; i++ )
    {
        ...
        TableWriter slot_probe = table_retain_probe( w , retention);
        slot_probe.check_default = 1;
        if ( !hull_config_save_body_retain( &slot_probe, &value->hulls[i] , retention) ) { return 0;
        }
        table_retain_rewind(&slot_probe, retention);
        if ( slot_probe.offset == 1 ) { continue;
        }
    ...
    for ( i = 0; i < HULL_MAX; i++ )
    {
        ...
        switch ( i + 1 )
        {
            case HULL_NONE: table_writer_leb( w, 0 );
            break;
            case HULL_INTERCEPTOR: table_retain_writer_id( w, 0xae61a6cbe88c2cf4ull , retention);
            break;
            case HULL_GUNSHIP: table_retain_writer_id( w, 0x334d24e1420dbc1full , retention);
            break;
            case HULL_FREIGHTER: table_retain_writer_id( w, 0x6c2321a3d00e23dbull , retention);
            break;
            default: return 0;
            /* no declared identity */
        }
        ...
        {
            int64_t frame_begin = w->offset;
            if ( !hull_config_save_body_retain( w, &value->hulls[i] , retention) ) { return 0;
            }
            table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
        }
        else
        {
            TableWriter probe = table_retain_probe( w , retention);
            if ( !hull_config_save_body_retain( &probe, &value->hulls[i] , retention) ) { return 0;
            }
            table_retain_rewind(&probe, retention);
            table_writer_leb( w, (uint64_t) probe.offset );
            if ( !hull_config_save_body_retain( w, &value->hulls[i] , retention) ) { return 0;
...
static SCHEMA_UNUSED int keyed_config_save_body_retain( TableWriter * w, const KeyedConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    retention=table_retain_step(body_keep,0,0);
        ...
        if ( rides )
        {
            table_retain_writer_id( w, 0xbaaeb048a5a8fa6dull , retention);
            table_writer_put8( w, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
                if ( !schema_tabledemo_keyed_config_wire_teams__retain( w, value , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                TableWriter probe = table_retain_probe( w , retention);
                if ( !schema_tabledemo_keyed_config_wire_teams__retain( &probe, value , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !schema_tabledemo_keyed_config_wire_teams__retain( w, value , retention) ) { return 0;
    ...
    retention=table_retain_step(body_keep,1,0);
        ...
        if ( rides )
        {
            table_retain_writer_id( w, 0xce0ac3c25694d8ffull , retention);
            table_writer_put8( w, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
                if ( !schema_tabledemo_keyed_config_wire_hulls__retain( w, value , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                TableWriter probe = table_retain_probe( w , retention);
                if ( !schema_tabledemo_keyed_config_wire_hulls__retain( &probe, value , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !schema_tabledemo_keyed_config_wire_hulls__retain( w, value , retention) ) { return 0;
    ...
    retention=table_retain_step(body_keep,2,0);
        ...
        if ( !score_board_save_body_retain( &default_probe, &value->scores , retention) ) { return 0;
        }
        table_retain_rewind(&default_probe, retention);
        if ( default_probe.offset > 1 )
        {
            table_retain_writer_id( w, 0x01986b0b27400fb2ull , retention);
            table_writer_put8( w, 13 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
                if ( !score_board_save_body_retain( w, &value->scores , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                TableWriter probe = table_retain_probe( w , retention);
                if ( !score_board_save_body_retain( &probe, &value->scores , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !score_board_save_body_retain( w, &value->scores , retention) ) { return 0;
    ...
    retention=table_retain_step(body_keep,3,0);
        if ( !( value->season == 0 ) )
        {
            table_retain_writer_id( w, 0x28dfd5e2759ba3f8ull , retention);
...
static SCHEMA_UNUSED int keyed_config_load_body_retain( TableReader * r, KeyedConfig * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    for ( ;; )
    {
        ...
        switch ( id )
        {
            case 0xbaaeb048a5a8fa6dull: /* teams */
            {
                ...
                if ( sub.size >= 2 )
                {
                    ...
                    for ( i = 0; i < count; i++ )
                    {
                        ...
                        switch ( key )
                        {
                            case 0x9ff1de19feac1b7cull: slot = 0;
                            break;
                            case 0xecf3d3a7c1693e2dull: slot = 1;
                            break;
                            case 0xcf00d78fd5953f1cull: slot = 2;
                            break;
                            default: r->report->unknown++;
                            r->report->retain_lost += retention.retain != NULL;
                            break;
                        ...
                        retention=table_retain_index(retention,(uint32_t)(slot));
                        team_config_load_body_retain( &elem, &value->teams[slot] , retention);
                        if(elem.offset!=elem.size) { r->report->malformed=1;
                        team_config_reset(&value->teams[slot]);
                        }
                    }
                }
                break;
            ...
            case 0xce0ac3c25694d8ffull: /* hulls */
            {
                ...
                if ( sub.size >= 2 )
                {
                    ...
                    for ( i = 0; i < count; i++ )
                    {
                        ...
                        switch ( key )
                        {
                            case 0xae61a6cbe88c2cf4ull: slot = 0;
                            break;
                            case 0x334d24e1420dbc1full: slot = 1;
                            break;
                            case 0x6c2321a3d00e23dbull: slot = 2;
                            break;
                            default: r->report->unknown++;
                            r->report->retain_lost += retention.retain != NULL;
                            break;
                        ...
                        retention=table_retain_index(retention,(uint32_t)(slot));
                        hull_config_load_body_retain( &elem, &value->hulls[slot] , retention);
                        if(elem.offset!=elem.size) { r->report->malformed=1;
                        hull_config_reset(&value->hulls[slot]);
                        }
                    }
                }
                break;
            ...
            case 0x01986b0b27400fb2ull: /* scores */
            {
                ...
                score_board_load_body_retain( &sub, &value->scores , retention);
                if ( sub.offset != sub.size ) { r->report->malformed = 1;
                score_board_reset( &value->scores );
                }
                break;
...
static SCHEMA_UNUSED int keyed_config_load_message_body_retain(TableMessageReader * r,KeyedConfig * value, TableRetainWalk retention)
{
    (void)retention;
 ...
 (void)element_entry;
  ...
  TableRetainWalk named_retention=retention;
  retention.retain=NULL;
  if(!team_config_load_message_body_retain(r,&scratch, retention))return 0;
  retention=named_retention;
  }continue;
  }
  retention=table_retain_index(retention,(uint32_t)(slot));
  if(!team_config_load_message_body_retain(r,&value->teams[slot], retention))return 0;
 } }
 break;
 }
 case UINT64_C(0xce0ac3c25694d8ff): {
 ...
 (void)element_entry;
  ...
  TableRetainWalk named_retention=retention;
  retention.retain=NULL;
  if(!hull_config_load_message_body_retain(r,&scratch, retention))return 0;
  retention=named_retention;
  }continue;
  }
  retention=table_retain_index(retention,(uint32_t)(slot));
  if(!hull_config_load_message_body_retain(r,&value->hulls[slot], retention))return 0;
 } }
 break;
 }
 case UINT64_C(0x01986b0b27400fb2): {
 ...
 retention=table_retain_step(body_keep,2,0);
 table_retain_discard(body_keep,1,2);
 if(!score_board_load_message_body_retain(r,&value->scores, retention))return 0;
 break;
 }
 case UINT64_C(0x28dfd5e2759ba3f8): {
 if(!(entry->kind==8 && entry->elem_kind==0)){if(entry->elem_kind==0 && table_kind_widens(entry->kind,8)){r->report->widened++;
...
static SCHEMA_UNUSED int schema_tabledemo_score_board_wire_per_team__retain( TableWriter * w, const ScoreBoard * value , TableRetainWalk retention)
{
    (void)retention;
...
static SCHEMA_UNUSED int score_board_save_body_retain( TableWriter * w, const ScoreBoard * value , TableRetainWalk retention)
{
    (void)retention;
    ...
    retention=table_retain_step(body_keep,0,0);
        ...
        if ( rides )
        {
            ...
            table_retain_writer_id( w, 0xf10fad739a0e1660ull , retention);
            table_writer_put8( w, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
                if ( !schema_tabledemo_score_board_wire_per_team__retain( w, value , retention) ) { return 0;
                }
                table_writer_leb( w, (uint64_t)(w->offset - frame_begin) );
            }
            else
            {
                TableWriter probe = table_retain_probe( w , retention);
                if ( !schema_tabledemo_score_board_wire_per_team__retain( &probe, value , retention) ) { return 0;
                }
                table_retain_rewind(&probe, retention);
                table_writer_leb( w, (uint64_t) probe.offset );
                if ( !schema_tabledemo_score_board_wire_per_team__retain( w, value , retention) ) { return 0;
...
static SCHEMA_UNUSED int score_board_load_body_retain( TableReader * r, ScoreBoard * value , TableRetainWalk retention)
{
    (void)retention;
        ...
        switch ( id )
        {
            case 0xf10fad739a0e1660ull: /* per_team */
            {
                ...
                if ( sub.size >= 2 )
                {
                    ...
                    if ( elem_kind != 4 ) { if ( !(table_kind_widens(elem_kind,4)) ) { r->report->kind_mismatch++;
                    break;
                    } r->report->widened++;
                    }
                    for ( i = 0; i < count; i++ )
                    {
                        ...
                        switch ( elem_kind )
                        {
                            case 2:
                            {
                                if ( !table_reader_has( &elem, 1 ) ) { r->report->malformed = 1;
                                goto next_per_team;
                                }
                                int64_t decoded_wide = (int8_t) table_reader_get8( &elem );
                                if ( decoded_wide < 0ll ) { decoded_wide = 0ll;
                                ...
                                if ( decoded_wide > 100000ll ) { decoded_wide = 100000ll;
                                r->report->clamped++;
                                }
                                value->per_team[slot] = decoded_wide;
                                break;
                            ...
                            case 3:
                            {
                                if ( !table_reader_has( &elem, 2 ) ) { r->report->malformed = 1;
                                goto next_per_team;
                                }
                                int64_t decoded_wide = (int16_t) table_reader_get16( &elem );
                                if ( decoded_wide < 0ll ) { decoded_wide = 0ll;
                            ...
                            case 4:
                            {
                                if ( !table_reader_has( &elem, 4 ) ) { r->report->malformed = 1;
                                goto next_per_team;
                                }
                                {
                                    int32_t decoded_v = (int32_t) table_reader_get32( &elem );
                                    ...
                                    else if ( decoded_v > 100000 ) { decoded_v = 100000;
                                    r->report->clamped++;
                                    }
                                    value->per_team[slot] = decoded_v;
                                }
...
static SCHEMA_UNUSED int score_board_load_message_body_retain(TableMessageReader * r,ScoreBoard * value, TableRetainWalk retention)
{
    (void)retention;
...
SCHEMA_TABLE_STATIC_ASSERT( KeyedConfig_sizeof, sizeof( KeyedConfig ) == 304, "KeyedConfig's sizeof moved: the build version was taken over 304, so a cook of it would not be this build's file (docs/SPEC-TABLES.md §20.3)" );
...
SCHEMA_TABLE_STATIC_ASSERT( KeyedConfig_season_offset, offsetof( KeyedConfig, season ) == 300, "KeyedConfig's field season moved: the build version was taken over offset 300 (docs/SPEC-TABLES.md §20.3)" );
