...
static SCHEMA_UNUSED int team_config_save_body( TableWriter * w, const TeamConfig * value )
{
    ...
    { /* spawn_count */
        if ( !( value->spawn_count == 4 ) )
        {
            ...
            table_writer_id( w, 0xceec99e2d65db674ull ); table_writer_put8( w, 4 );
    ...
    { /* banner */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_id( w, 0xbca0dab1c7a00ccfull ); table_writer_put8( w, 12 );
            if ( w->buffer == NULL )
            {
...
static SCHEMA_UNUSED int team_config_save_message_body(TableBitWriter * w,const TeamConfig * value)
{
 ...
 if(value->banner_length != 0) {
  ...
  table_bit_align_write(w); table_bit_putbytes(w,value->banner,value->banner_length);
 }
 }
...
{
 team_config_reset(value);for(;;){const TableMessageEntry * entry;
 ...
 {uint64_t n,kept;if(!table_bit_get(&r->bits,&n,table_bits_required(0,entry->max)))goto malformed;
  ...
  {const uint8_t * text=r->bits.buffer+(r->bits.offset>>3);if(!table_wire_utf8(text,n))goto malformed;kept=table_wire_utf8_clamp(text,n,16);memcpy(value->banner,text,(size_t)kept);value->banner[kept]=0;}
  r->bits.offset+=(int64_t)n*8;
  if(kept<n)r->report->clamped++;value->banner_length=(int32_t)kept; }
 break;}
...
static SCHEMA_UNUSED int team_config_load_messages(TeamConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 for(i=0;i<bodies;i++){if(!team_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
{
 gunner_config_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
...
static SCHEMA_UNUSED int gunner_config_load_messages(GunnerConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!gunner_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int turret_config_save_body( TableWriter * w, const TurretConfig * value )
{
    ...
    { /* cooldown */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_id( w, 0xdc2cbe6953343d48ull ); table_writer_put8( w, 10 );
...
{
 turret_config_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
...
static SCHEMA_UNUSED int turret_config_load_messages(TurretConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!turret_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int hull_config_save_body( TableWriter * w, const HullConfig * value )
{
    ...
    { /* turrets */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_id( w, 0x84f8260bc283608cull ); table_writer_put8( w, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
static SCHEMA_UNUSED int hull_config_save_message_body(TableBitWriter * w,const HullConfig * value)
{
 ...
 { /* turrets */
 int rides=0; { int32_t i; for(i=0;i<3;i++) {
 ...
 if(rides) {
  ...
  { uint32_t count=0; int32_t i;
   ...
   for(i=0;i<3;i++) {
    ...
    if(probe.bits==kTableMessageRefBitsHere) continue;
    switch(i+1) {
     case WEAPON_NONE: table_bit_put(w,0,kTableMessageRefBitsHere); break;
     case WEAPON_CANNON: table_bit_put(w,125,kTableMessageRefBitsHere); break;
     case WEAPON_MISSILE: table_bit_put(w,126,kTableMessageRefBitsHere); break;
     case WEAPON_MINE: table_bit_put(w,127,kTableMessageRefBitsHere); break;
     default: return 0;
    }
...
{
 hull_config_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
 ...
 if(!(entry->kind==16 && entry->elem_kind==13)){if(0){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;for(i=0;i<n;i++){const TableMessageEntry * key;int32_t slot=-1;
  if(!table_message_ref(r,&key,1)||key==NULL)goto malformed;switch(key->id){
...
static SCHEMA_UNUSED int hull_config_load_messages(HullConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!hull_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int keyed_config_save_body( TableWriter * w, const KeyedConfig * value )
{
    ...
    { /* hulls */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_id( w, 0xce0ac3c25694d8ffull ); table_writer_put8( w, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
    ...
    { /* scores */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_id( w, 0x01986b0b27400fb2ull ); table_writer_put8( w, 13 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
  ...
  { uint32_t count=0; int32_t i;
   ...
   for(i=0;i<3;i++) {
    ...
    switch(i+1) {
     ...
     case TEAM_RED: table_bit_put(w,122,kTableMessageRefBitsHere); break;
     case TEAM_BLUE: table_bit_put(w,123,kTableMessageRefBitsHere); break;
     case TEAM_GREEN: table_bit_put(w,124,kTableMessageRefBitsHere); break;
     default: return 0;
    }
 ...
 if(rides) {
  ...
  { uint32_t count=0; int32_t i;
   ...
   for(i=0;i<3;i++) {
    ...
    if(probe.bits==kTableMessageRefBitsHere) continue;
    switch(i+1) {
     case HULL_NONE: table_bit_put(w,0,kTableMessageRefBitsHere); break;
     case HULL_INTERCEPTOR: table_bit_put(w,116,kTableMessageRefBitsHere); break;
     case HULL_GUNSHIP: table_bit_put(w,117,kTableMessageRefBitsHere); break;
     case HULL_FREIGHTER: table_bit_put(w,118,kTableMessageRefBitsHere); break;
     default: return 0;
    }
...
{
 keyed_config_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
...
static SCHEMA_UNUSED int keyed_config_load_messages(KeyedConfig * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!keyed_config_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
...
static SCHEMA_UNUSED int score_board_save_body( TableWriter * w, const ScoreBoard * value )
{
    ...
    { /* per_team */
        ...
        {
            if ( w->check_default ) { w->offset = 2; return 1; }
            table_writer_id( w, 0xf10fad739a0e1660ull ); table_writer_put8( w, 16 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
{
 score_board_reset(value);for(;;){const TableMessageEntry * entry;
 if(!table_message_ref(r,&entry,0))goto malformed;if(entry==NULL)return 1;switch(entry->id){
 case UINT64_C(0xf10fad739a0e1660): {
 if(!(entry->kind==16 && entry->elem_kind==4)){if(0){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;for(i=0;i<n;i++){const TableMessageEntry * key;int32_t slot=-1;
  if(!table_message_ref(r,&key,1)||key==NULL)goto malformed;switch(key->id){
  case UINT64_C(0x9ff1de19feac1b7c):slot=0;break;
...
static SCHEMA_UNUSED int score_board_load_messages(ScoreBoard * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 bodies=table_message_open(&r,vocabulary,buffer,bytes,report);if(bodies<0)return 0;
 if(bodies>capacity){*count=bodies;report->refused=1;report->reason=SCHEMA_TABLE_BATCH_TOO_LARGE;return 0;}
 for(i=0;i<bodies;i++){if(!score_board_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
