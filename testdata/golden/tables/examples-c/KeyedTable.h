...
static SCHEMA_UNUSED const uint64_t team_config_fixed_hash = 0x527038c53c0297c3ull; /* fnv1a64 over the block's bytes */
...
static SCHEMA_UNUSED const uint8_t team_config_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t team_config_fixed_vocab_bytes = (int64_t) sizeof( team_config_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( team_config_fixed_vocab ) + count * ( 8 + 24 );
}
...
static SCHEMA_UNUSED int64_t team_config_fixed_save( const TeamConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( team_config_fixed_vocab ) );
    memcpy( buffer + 5, team_config_fixed_vocab, sizeof( team_config_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( team_config_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != team_config_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, team_config_fixed_vocab, (int32_t) sizeof( team_config_fixed_vocab ), team_config_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
...
static SCHEMA_UNUSED const uint64_t gunner_config_fixed_hash = 0x113a539593602242ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 3 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t gunner_config_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t gunner_config_fixed_vocab_bytes = (int64_t) sizeof( gunner_config_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( gunner_config_fixed_vocab ) + count * ( 8 + 5 );
}
...
static SCHEMA_UNUSED int64_t gunner_config_fixed_save( const GunnerConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( gunner_config_fixed_vocab ) );
    memcpy( buffer + 5, gunner_config_fixed_vocab, sizeof( gunner_config_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( gunner_config_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != gunner_config_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_vocab_read( block, (int64_t) block_bytes, &parsed ) ) { report->refused = 1; report->reason = SCHEMA_TABLE_BLOCK_MALFORMED; return -1; }
        made = table_fixed_compile( &parsed, gunner_config_fixed_vocab, (int32_t) sizeof( gunner_config_fixed_vocab ), gunner_config_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_BLOCK_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
static SCHEMA_UNUSED const uint64_t turret_config_fixed_hash = 0xd8715b0be3854d91ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 7 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t turret_config_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t turret_config_fixed_vocab_bytes = (int64_t) sizeof( turret_config_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( turret_config_fixed_vocab ) + count * ( 8 + 14 );
}
...
static SCHEMA_UNUSED int64_t turret_config_fixed_save( const TurretConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( turret_config_fixed_vocab ) );
    memcpy( buffer + 5, turret_config_fixed_vocab, sizeof( turret_config_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( turret_config_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != turret_config_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_vocab_read( block, (int64_t) block_bytes, &parsed ) ) { report->refused = 1; report->reason = SCHEMA_TABLE_BLOCK_MALFORMED; return -1; }
        made = table_fixed_compile( &parsed, turret_config_fixed_vocab, (int32_t) sizeof( turret_config_fixed_vocab ), turret_config_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_BLOCK_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
static SCHEMA_UNUSED const uint64_t hull_config_fixed_hash = 0xc65dc3bc15d5a954ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 15 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t hull_config_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t hull_config_fixed_vocab_bytes = (int64_t) sizeof( hull_config_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( hull_config_fixed_vocab ) + count * ( 8 + 50 );
}
...
static SCHEMA_UNUSED int64_t hull_config_fixed_save( const HullConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( hull_config_fixed_vocab ) );
    memcpy( buffer + 5, hull_config_fixed_vocab, sizeof( hull_config_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( hull_config_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != hull_config_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_vocab_read( block, (int64_t) block_bytes, &parsed ) ) { report->refused = 1; report->reason = SCHEMA_TABLE_BLOCK_MALFORMED; return -1; }
        made = table_fixed_compile( &parsed, hull_config_fixed_vocab, (int32_t) sizeof( hull_config_fixed_vocab ), hull_config_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_BLOCK_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
static SCHEMA_UNUSED const uint64_t keyed_config_fixed_hash = 0xac0c8c80f3967de4ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 36 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t keyed_config_fixed_vocab[] = {
...
static SCHEMA_UNUSED const int64_t keyed_config_fixed_vocab_bytes = (int64_t) sizeof( keyed_config_fixed_vocab );
...
{
    return 1 + 4 + (int64_t) sizeof( keyed_config_fixed_vocab ) + count * ( 8 + 234 );
}
...
static SCHEMA_UNUSED int64_t keyed_config_fixed_save( const KeyedConfig * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( keyed_config_fixed_vocab ) );
    memcpy( buffer + 5, keyed_config_fixed_vocab, sizeof( keyed_config_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( keyed_config_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
                                 ...
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != keyed_config_fixed_hash )
    {
        ...
        int32_t made;
        if ( !table_fixed_vocab_read( block, (int64_t) block_bytes, &parsed ) ) { report->refused = 1; report->reason = SCHEMA_TABLE_BLOCK_MALFORMED; return -1; }
        made = table_fixed_compile( &parsed, keyed_config_fixed_vocab, (int32_t) sizeof( keyed_config_fixed_vocab ), keyed_config_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = 1; report->reason = ( made == -2 ) ? SCHEMA_TABLE_BLOCK_MALFORMED : SCHEMA_TABLE_PLAN_TOO_LARGE; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
