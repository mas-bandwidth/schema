...
{
    return TableJsonRead( value, &schema_tabledemo_wide_blob_info_, text, bytes, report );
}

int64_t schema_tabledemo_wide_blob_to_json_( const WideBlob * value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( value, &schema_tabledemo_wide_blob_info_, buffer, capacity );
}
