...
{
    return TableJsonRead( value, &schema_tabledemo_archive_config_info_, text, bytes, report );
}

int64_t schema_tabledemo_archive_config_to_json_( const ArchiveConfig * value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( value, &schema_tabledemo_archive_config_info_, buffer, capacity );
}
