...
SCHEMA_TABLE_STATIC_ASSERT( fixed_wide_blob_size, sizeof( WideBlob ) == 280016, "WideBlob: the fixed form's plan is laid out for this size" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_wide_blob_label, offsetof( WideBlob, label ) == 0, "WideBlob.label: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_wide_blob_label_length, offsetof( WideBlob, label_length ) == 70004, "WideBlob.label_length: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_wide_blob_payload, offsetof( WideBlob, payload ) == 70008, "WideBlob.payload: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_wide_blob_payload_length, offsetof( WideBlob, payload_length ) == 140008, "WideBlob.payload_length: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_wide_blob_samples, offsetof( WideBlob, samples ) == 140012, "WideBlob.samples: the fixed form's plan lands here" );
SCHEMA_TABLE_STATIC_ASSERT( fixed_wide_blob_samples_count, offsetof( WideBlob, samples_count ) == 280012, "WideBlob.samples_count: the fixed form's plan lands here" );

static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE void schema_tabledemo_wide_blob_fixed_write_body_( uint8_t * b, const WideBlob * value );

/* WideBlob's stores. The template — the hash, then zeros — is memcpy'd first,
   which is also what zero-fills every byte of declared slack. */
static SCHEMA_UNUSED SCHEMA_TABLEDEMO_TABLE_INLINE void schema_tabledemo_wide_blob_fixed_write_body_( uint8_t * b, const WideBlob * value )
{
    (void) b; (void) value;
    table_fixed_put32( b + 0, (uint32_t) value->label_length );
    memcpy( b + 4, value->label, 70000 );
    table_fixed_put32( b + 70004, (uint32_t) value->payload_length );
    memcpy( b + 70008, value->payload, 70000 );
    table_fixed_put32( b + 140008, (uint32_t) value->samples_count );
    {
        int64_t i;
        for ( i = 0; i < 70000; ++i )
        {
            table_fixed_put16( b + 140012 + i * 2 + 0, (uint16_t) value->samples[i] );
        }
    }
}

/* ---- WideBlob, the fixed form ---- */

/* THE BODY IS ONE CONSTANT on this form: it is the same size for every
   value the type can hold (docs/SPEC-TABLES.md §3.4). */
static SCHEMA_UNUSED const int64_t wide_blob_fixed_body_bytes = 280012;
static SCHEMA_UNUSED const int64_t wide_blob_fixed_record_bytes = 8 + 280012; /* the hash and the body */
static SCHEMA_UNUSED const uint64_t wide_blob_fixed_hash = 0x8202b907d4ac81c4ull; /* fnv1a64 over the block's bytes */

/* THE VOCABULARY BLOCK: 6 entries, a PRE-ORDER walk of the closure in the
   writer's declared order. Every byte is settled by the schema compiler. */
static SCHEMA_UNUSED const uint8_t wide_blob_fixed_vocab[] = {
    0x06, 0x00, 0x00, 0x00, 0x51, 0x64, 0x36, 0xd3, 0x61, 0x69, 0x6a, 0xab, 0x0d, 0xcc, 0x45, 0x04,
    0x00, 0x03, 0x00, 0x00, 0x00, 0x3d, 0x62, 0xcb, 0x8f, 0xec, 0xfc, 0xf7, 0x39, 0x0c, 0x74, 0x11,
    0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe5, 0xe9, 0xb5, 0x63, 0xd0, 0xa9, 0xb8, 0xcf, 0x0e, 0x74,
    0x11, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06,
    0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xdc, 0xdd, 0x48, 0x3b, 0x6a, 0xca, 0xb1, 0xe3,
    0x0e, 0xe4, 0x22, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x07, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
static SCHEMA_UNUSED const int64_t wide_blob_fixed_vocab_bytes = (int64_t) sizeof( wide_blob_fixed_vocab );
...
static SCHEMA_UNUSED const TableFixedDst wide_blob_fixed_dst[] = {
    { 0, 0, 0, 0, 0 }, /* WideBlob */
    { 70004, 0, 0, 0, 1 }, /* label */
    { 140008, 1, 70008, 1, 3 }, /* payload */
    { 0, 0, 0, 0, 0 }, /* u8 */
    { 140012, 2, 280012, 1, 0 }, /* samples */
    { 0, 0, 0, 0, 0 }, /* element */
};
...
/* THE IDENTITY PLAN, coalesced out of 4 leaves by the schema compiler —
   ...
   guard at all. */
static SCHEMA_UNUSED const TableFixedEntry wide_blob_fixed_plan[] = {
    { 0u, 70004u, 70000u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 1, 0, 0 }, /* label */
    { 70004u, 140008u, 70000u, 70008u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedText, 3, 0, 0 }, /* payload */
    { 140008u, 280012u, 70000u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCount, 0, 0, 0 }, /* samples count */
    { 140012u, 140012u, 140000u, 0u, SCHEMA_TABLE_FIXED_NO_GUARD, kTableFixedCopy, 0, 0, 0 }, /* samples, whole */
};
static SCHEMA_UNUSED const int32_t wide_blob_fixed_plan_count = 4;
static SCHEMA_UNUSED const int32_t wide_blob_fixed_plan_guarded = 4;

/* A FILE: the form byte, the block, then the records to the end of it. */
static SCHEMA_UNUSED int64_t wide_blob_fixed_measure( int64_t count )
{
    return 1 + 4 + (int64_t) sizeof( wide_blob_fixed_vocab ) + count * ( 8 + 280012 );
}

static SCHEMA_UNUSED int64_t wide_blob_fixed_save( const WideBlob * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = wide_blob_fixed_measure( count );
    uint8_t * at;
    int64_t k;
    if ( count < 0 || values == NULL || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    table_fixed_put32( buffer + 1, (uint32_t) sizeof( wide_blob_fixed_vocab ) );
    memcpy( buffer + 5, wide_blob_fixed_vocab, sizeof( wide_blob_fixed_vocab ) );
    at = buffer + 5 + (int64_t) sizeof( wide_blob_fixed_vocab );
    for ( k = 0; k < count; ++k )
    {
        table_fixed_put64( at, wide_blob_fixed_hash );
        memset( at + 8, 0, (size_t) 280012 ); /* the prefill's zeros */
        schema_tabledemo_wide_blob_fixed_write_body_( at + 8, values + k );
        at += 8 + 280012;
    }
    return need;
}
...
static SCHEMA_UNUSED int64_t wide_blob_fixed_load( WideBlob * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                                 TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = wide_blob_fixed_plan;
    int32_t entry_count = wide_blob_fixed_plan_count;
    int32_t entry_guarded = wide_blob_fixed_plan_guarded;
    ...
    record_bytes = 8 + 280012;
    if ( hash != wide_blob_fixed_hash )
    {
        ...
        made = table_fixed_compile( &parsed, wide_blob_fixed_vocab, (int32_t) sizeof( wide_blob_fixed_vocab ), wide_blob_fixed_dst, plan, plan_capacity, &compiled_guarded, report );
    ...
    for ( k = 0; k < count; ++k )
    {
        wide_blob_reset( values + k ); /* the declared defaults, one prefill */
