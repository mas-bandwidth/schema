...
static SCHEMA_UNUSED int wide_blob_save_body( TableWriter * w, const WideBlob * value )
{
    ...
    { /* label */
        ...
        {
            table_writer_header_at( w, 36, 0x39f7fcec8fcb623dull, 12 );
            if ( w->buffer == NULL )
            {
    ...
    { /* payload */
        ...
        {
            table_writer_header_at( w, 123, 0xcfb8a9d063b5e9e5ull, 14 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
    ...
    { /* samples */
        ...
        {
            table_writer_header_at( w, 137, 0xe3b1ca6a3b48dddcull, 14 );
            if ( w->buffer == NULL )
            {
                int64_t frame_begin = w->offset;
...
static SCHEMA_UNUSED int wide_blob_save_message_body(TableBitWriter * w,const WideBlob * value)
{
 ...
 if(value->samples_count<0 || value->samples_count>70000) return 0;
 if(value->samples_count>0) {
  table_bit_put(w,110,kTableMessageRefBitsHere);
  if(value->samples_count<0 || value->samples_count>70000) return 0;
