...
static SCHEMA_UNUSED int wide_blob_save_message_body(TableBitWriter * w,const WideBlob * value)
{
 ...
 if(value->payload_length != 0) {
  ...
  table_bit_align_write(w); table_bit_putbytes(w,value->payload,value->payload_length);
 }
 }
 { /* samples */
 if(value->samples_count<0 || value->samples_count>70000) return 0;
 if(value->samples_count>0) {
  ...
  { int32_t i; for(i=0;i<value->samples_count;i++) {
...
{
 wide_blob_reset(value);for(;;){const TableMessageEntry * entry;
 ...
 {uint64_t n,kept;if(!table_bit_get(&r->bits,&n,table_bits_required(0,entry->max)))goto malformed;
  ...
  {const uint8_t * text=r->bits.buffer+(r->bits.offset>>3);if(!table_wire_utf8(text,n))goto malformed;kept=table_wire_utf8_clamp(text,n,70000);memcpy(value->label,text,(size_t)kept);value->label[kept]=0;}
 ...
 {uint64_t n,kept;if(!table_bit_get(&r->bits,&n,table_bits_required(entry->min,entry->max)))goto malformed;
  ...
  if(kept<n)r->report->clamped++;value->payload_length=(int32_t)kept; }
 break;}
 case UINT64_C(0xe3b1ca6a3b48dddc): {
 if(!(entry->kind==14 && entry->elem_kind==7)){if(entry->kind==14 && table_kind_widens(entry->elem_kind,7)){r->report->widened++;}else{r->report->kind_mismatch++;if(!table_message_skip(r,entry))goto malformed;break;}}
 {uint64_t n,kept,walk,i;TableMessageEntry element_shape=table_message_element(entry);const TableMessageEntry * element_entry=&element_shape;(void)element_entry;
  if(!table_message_count(r,entry,&n))goto malformed;kept=n>70000?70000:n;if(kept<n)r->report->clamped++;
  walk=element_entry->value_bits>=0?kept:n;for(i=0;i<walk;i++){uint16_t scratch; uint16_t * item=i<kept ? &value->samples[i] : &scratch;
...
static SCHEMA_UNUSED int wide_blob_load_messages(WideBlob * values,int64_t * count,const TableVocabulary * vocabulary,const uint8_t * buffer,int64_t bytes,TableReport * report)
{
 ...
 for(i=0;i<bodies;i++){if(!wide_blob_load_message_body(&r,values+i))return 0;(*count)++;}return table_message_close(&r);
}
