...
inline int64_t RowNodeStorage( uint64_t type_id, int64_t length )
{
...
inline void RowNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    {
        const int64_t storage = RowNodeStorage( type_id, r.size );
...
// depth, summed from the FRAMING: N is framing and not a value, and this
// reads no field. kTableNodeRefused is a wire whose N its L cannot carry.
inline int64_t ArmyNodeStorage( uint64_t type_id, int64_t length )
{
    (void) length; // no byte buffer below this root: every node's storage is its type's
    switch ( type_id )
    {
...
inline void ArmyNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        const int64_t storage = ArmyNodeStorage( type_id, r.size );
...
inline int64_t ArmyLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = ArmyNodeStorage( type_id, length );
        if ( storage == kTableNodeRefused ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; } // an N the record's framing cannot carry (§2.8, §2.9)
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
    }
    int64_t attribution = ( records + 1 ) * (int64_t) sizeof( TableNodeDirEntry );
...
// depth, summed from the FRAMING: N is framing and not a value, and this
// reads no field. kTableNodeRefused is a wire whose N its L cannot carry.
inline int64_t DeckNodeStorage( uint64_t type_id, int64_t length )
{
    (void) length; // no byte buffer below this root: every node's storage is its type's
    switch ( type_id )
    {
...
inline void DeckNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        const int64_t storage = DeckNodeStorage( type_id, r.size );
...
inline int64_t DeckLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = DeckNodeStorage( type_id, length );
        if ( storage == kTableNodeRefused ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; } // an N the record's framing cannot carry (§2.8, §2.9)
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
    }
    int64_t attribution = ( records + 1 ) * (int64_t) sizeof( TableNodeDirEntry );
...
// parameter on a load.
inline const Sample * SampleOpen( const void * bytes, uint64_t length )
{
    return (const Sample *) TableCookOpen( bytes, length, (uint64_t) sizeof( Sample ), (uint64_t) alignof( Sample ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Row * RowOpen( const void * bytes, uint64_t length )
{
    return (const Row *) TableCookOpen( bytes, length, (uint64_t) sizeof( Row ), (uint64_t) alignof( Row ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Sheet * SheetOpen( const void * bytes, uint64_t length )
{
    return (const Sheet *) TableCookOpen( bytes, length, (uint64_t) sizeof( Sheet ), (uint64_t) alignof( Sheet ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Army * ArmyOpen( const void * bytes, uint64_t length )
{
    return (const Army *) TableCookOpen( bytes, length, (uint64_t) sizeof( Army ), (uint64_t) alignof( Army ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Deck * DeckOpen( const void * bytes, uint64_t length )
{
    return (const Deck *) TableCookOpen( bytes, length, (uint64_t) sizeof( Deck ), (uint64_t) alignof( Deck ) );
}
