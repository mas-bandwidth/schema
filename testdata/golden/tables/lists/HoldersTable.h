...
inline int64_t SampleMeasureBody( TableIds & ids, const Sample & value )
{
    ...
    if ( value.v != 0 ) { bytes += TableLebBytes( ids.ref( 0xaf63eb4c86020609ull, 23 ) ) + 1 + 4; } // v
    return bytes;
}
...
LISTDEMO_TABLE_INLINE bool SampleSaveBody( TableWriter & w, TableIds & ids, const Sample & value )
{
    if ( value.v != 0 )
    {
        w.putleb( ids.ref( 0xaf63eb4c86020609ull, 23 ) ); w.put8( 4 ); // v
...
inline int64_t SampleMeasureMessage( const Sample & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = SampleMeasureBody( ids, value );
...
inline int64_t SampleSaveMessage( const Sample & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !SampleSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == SampleMeasureMessage( value )
}
...
inline bool SampleLoadMessage( Sample & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    SampleReset( value );
...
inline int64_t RowMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Row & value )
{
    ...
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        {
            const uint64_t ref_items = ids.ref( 0x3e7884bf4f412c6full, 6 );
    ...
    if ( value.label != 0 ) { bytes += TableLebBytes( ids.ref( 0x39f7fcec8fcb623dull, 22 ) ) + 1 + 4; } // label
    return bytes;
}
...
inline bool RowSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Row & value )
{
    ...
    if ( value.label != 0 )
    {
        w.putleb( ids.ref( 0x39f7fcec8fcb623dull, 22 ) ); w.put8( 4 ); // label
...
inline int64_t SheetMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Sheet & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        {
            const uint64_t ref_rows = ids.ref( 0xa3a7061ff10a8138ull, 27 );
            int64_t body_rows = 0;
    ...
    }
    {
        ...
        if ( pointee_pinned != NULL )
        {
            ...
            bytes += TableLebBytes( ids.ref( 0x5f82477707ad620full, 28 ) ) + 1 + TableLebBytes( index_pinned );
        }
    }
    return bytes;
}
...
inline bool SheetSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Sheet & value )
{
    ...
    }
    {
        ...
        if ( pointee_pinned != NULL )
        {
            ...
            w.putleb( ids.ref( 0x5f82477707ad620full, 28 ) ); w.put8( 17 ); // pinned — a NODE INDEX into the flat node table
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.count != 0 ) { bytes += TableLebBytes( ids.ref( 0xb1e5e28e4479a274ull, 11 ) ) + 1 + 4; } // count
    return bytes;
}
...
LISTDEMO_TABLE_INLINE bool ItemSaveBody( TableWriter & w, TableIds & ids, const Item & value )
{
    if ( value.count != 0 )
    {
        w.putleb( ids.ref( 0xb1e5e28e4479a274ull, 11 ) ); w.put8( 4 ); // count
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.key != 0 ) { bytes += TableLebBytes( ids.ref( 0x3dc94a19365b10ecull, 31 ) ) + 1 + 1; } // key
    {
        const int32_t mark_value = ids.count;
        const uint64_t ref_value = ids.ref( 0x7ce4fd9430e80ceaull, 32 );
...
LISTDEMO_TABLE_INLINE bool SquadRosterEntrySaveBody( TableWriter & w, TableIds & ids, const SquadRosterEntry & value )
{
    if ( value.key != 0 )
    {
        w.putleb( ids.ref( 0x3dc94a19365b10ecull, 31 ) ); w.put8( 6 ); // key
...
inline int64_t SquadMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Squad & value )
{
    ...
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        {
            const uint64_t ref_roster = ids.ref( 0x1c84390d304f4f42ull, 29 );
    ...
    if ( value.name != 0 ) { bytes += TableLebBytes( ids.ref( 0xc4bcadba8e631b86ull, 30 ) ) + 1 + 4; } // name
    return bytes;
}
...
inline bool SquadSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Squad & value )
{
    ...
    if ( value.name != 0 )
    {
        w.putleb( ids.ref( 0xc4bcadba8e631b86ull, 30 ) ); w.put8( 4 ); // name
...
inline int64_t ArmyMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Army & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        {
            const uint64_t ref_squads = ids.ref( 0x7848019b0c02a926ull, 4 );
...
inline int64_t DeckMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Deck & value )
{
    ...
    {
        const uint64_t ref_hands = ids.ref( 0x81b46a69304ee2c9ull, 9 );
...
template <typename Ctx>
inline int64_t RowMeasureWire( const Ctx & ctx, const Row & root, TableAllocator allocator, bool message = false )
{
...
template <typename Ctx>
inline int64_t RowSaveWire( const Ctx & ctx, const Row & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t RowMeasureMessage( const Row * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RowMeasureWire( ctx, *root, allocator, true );
}

inline int64_t RowSaveMessage( const Row * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RowSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t RowMeasureMessage( const RowBuilder & builder )
{
    if ( builder.region != NULL ) { return RowMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return RowMeasureWire( ctx, *(const Row *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t RowSaveMessage( const RowBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return RowSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return RowSaveWire( ctx, *(const Row *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// RowLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Row * RowLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
...
// the connection's announced table and no trailer to write.
template <typename Ctx>
inline int64_t SheetMeasureWire( const Ctx & ctx, const Sheet & root, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
...
template <typename Ctx>
inline int64_t SheetSaveWire( const Ctx & ctx, const Sheet & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t SheetMeasureMessage( const Sheet * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return SheetMeasureWire( ctx, *root, allocator, true );
}

inline int64_t SheetSaveMessage( const Sheet * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return SheetSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t SheetMeasureMessage( const SheetBuilder & builder )
{
    if ( builder.region != NULL ) { return SheetMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return SheetMeasureWire( ctx, *(const Sheet *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t SheetSaveMessage( const SheetBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return SheetSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return SheetSaveWire( ctx, *(const Sheet *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t SheetLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    if ( message_bytes < 1 || message[0] != kTableWireMessageForm || !vocabulary.announced ) { return -1; }
    const TableIdTable & ids_table = vocabulary.table;
...
// SheetLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Sheet * SheetLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, and then the connection's own table:
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t SquadLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    if ( message_bytes < 1 || message[0] != kTableWireMessageForm || !vocabulary.announced ) { return -1; }
    const TableIdTable & ids_table = vocabulary.table;
...
// the connection's announced table and no trailer to write.
template <typename Ctx>
inline int64_t ArmyMeasureWire( const Ctx & ctx, const Army & root, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
...
template <typename Ctx>
inline int64_t ArmySaveWire( const Ctx & ctx, const Army & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t ArmyMeasureMessage( const Army * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return ArmyMeasureWire( ctx, *root, allocator, true );
}

inline int64_t ArmySaveMessage( const Army * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return ArmySaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t ArmyMeasureMessage( const ArmyBuilder & builder )
{
    if ( builder.region != NULL ) { return ArmyMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return ArmyMeasureWire( ctx, *(const Army *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t ArmySaveMessage( const ArmyBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return ArmySaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return ArmySaveWire( ctx, *(const Army *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t ArmyLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    if ( message_bytes < 1 || message[0] != kTableWireMessageForm || !vocabulary.announced ) { return -1; }
    const TableIdTable & ids_table = vocabulary.table;
...
// ArmyLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Army * ArmyLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, and then the connection's own table:
...
// the connection's announced table and no trailer to write.
template <typename Ctx>
inline int64_t DeckMeasureWire( const Ctx & ctx, const Deck & root, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
...
template <typename Ctx>
inline int64_t DeckSaveWire( const Ctx & ctx, const Deck & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t DeckMeasureMessage( const Deck * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return DeckMeasureWire( ctx, *root, allocator, true );
}

inline int64_t DeckSaveMessage( const Deck * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return DeckSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t DeckMeasureMessage( const DeckBuilder & builder )
{
    if ( builder.region != NULL ) { return DeckMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return DeckMeasureWire( ctx, *(const Deck *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t DeckSaveMessage( const DeckBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return DeckSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return DeckSaveWire( ctx, *(const Deck *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t DeckLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    if ( message_bytes < 1 || message[0] != kTableWireMessageForm || !vocabulary.announced ) { return -1; }
    const TableIdTable & ids_table = vocabulary.table;
...
// DeckLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Deck * DeckLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, and then the connection's own table:
