...
{
    return TableJsonRead( &value, PlacementTableType(), text, bytes, report );
}

int64_t PlacementToJsonMeasure( const Placement & value )
{
    return TableJsonWrite( &value, PlacementTableType(), NULL, 0 );
}

int64_t PlacementToJson( const Placement & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, PlacementTableType(), buffer, capacity );
}

bool LogEntryFromJson( LogEntry & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, LogEntryTableType(), text, bytes, report );
}

int64_t LogEntryToJsonMeasure( const LogEntry & value )
{
    return TableJsonWrite( &value, LogEntryTableType(), NULL, 0 );
}

int64_t LogEntryToJson( const LogEntry & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, LogEntryTableType(), buffer, capacity );
}
...
bool SaveFromJson( SaveBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, SaveTableType(), text, bytes, report );
}

int64_t SaveToJsonMeasure( const Save * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SaveTableType(), NULL, 0, allocator );
}

int64_t SaveToJson( const Save * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SaveTableType(), buffer, capacity, allocator );
}

bool PointFromJson( Point & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, PointTableType(), text, bytes, report );
}

int64_t PointToJsonMeasure( const Point & value )
{
    return TableJsonWrite( &value, PointTableType(), NULL, 0 );
}

int64_t PointToJson( const Point & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, PointTableType(), buffer, capacity );
}
...
bool MixedFromJson( MixedBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, MixedTableType(), text, bytes, report );
}

int64_t MixedToJsonMeasure( const Mixed * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, MixedTableType(), NULL, 0, allocator );
}

int64_t MixedToJson( const Mixed * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, MixedTableType(), buffer, capacity, allocator );
}
