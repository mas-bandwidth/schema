                            ...
                            {
                                {
                                    ...
                                    if ( elem_arm_ref_hits != 0 ) // the zero reference is a None element in its place
                                    {
                                        ...
                                        switch ( elem_arm_id_hits ) // the arm's NAME hash (docs/SPEC-TABLES.md §5)
                                        {
                                            ...
                                            {
                                                if ( elem_arm_kind_hits != 13 ) { ( *slot ).type = HitType::None; r.report->kind_mismatch++; break; }
                                            ...
                                            {
                                                if ( elem_arm_kind_hits != 4 ) { ( *slot ).type = HitType::None; r.report->kind_mismatch++; break; }
...
inline int64_t SaveNodeStorage( uint64_t type_id, int64_t length )
{
...
inline void SaveNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    {
        const int64_t storage = SaveNodeStorage( type_id, r.size );
...
inline int64_t SaveLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = SaveNodeStorage( type_id, length );
...
// depth, summed from the FRAMING: N is framing and not a value, and this
// reads no field. kTableNodeRefused is a wire whose N its L cannot carry.
inline int64_t MixedNodeStorage( uint64_t type_id, int64_t length )
{
    (void) length; // no byte buffer below this root: every node's storage is its type's
    switch ( type_id )
    {
...
inline void MixedNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        const int64_t storage = MixedNodeStorage( type_id, r.size );
...
inline int64_t MixedLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = MixedNodeStorage( type_id, length );
        if ( storage == kTableNodeRefused ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; } // an N the record's framing cannot carry (§2.8, §2.9)
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
    }
    int64_t attribution = ( records + 1 ) * (int64_t) sizeof( TableNodeDirEntry );
...
// parameter on a load.
inline const Placement * PlacementOpen( const void * bytes, uint64_t length )
{
    return (const Placement *) TableCookOpen( bytes, length, (uint64_t) sizeof( Placement ), (uint64_t) alignof( Placement ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const LogEntry * LogEntryOpen( const void * bytes, uint64_t length )
{
    return (const LogEntry *) TableCookOpen( bytes, length, (uint64_t) sizeof( LogEntry ), (uint64_t) alignof( LogEntry ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Save * SaveOpen( const void * bytes, uint64_t length )
{
    return (const Save *) TableCookOpen( bytes, length, (uint64_t) sizeof( Save ), (uint64_t) alignof( Save ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Point * PointOpen( const void * bytes, uint64_t length )
{
    return (const Point *) TableCookOpen( bytes, length, (uint64_t) sizeof( Point ), (uint64_t) alignof( Point ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Mixed * MixedOpen( const void * bytes, uint64_t length )
{
    return (const Mixed *) TableCookOpen( bytes, length, (uint64_t) sizeof( Mixed ), (uint64_t) alignof( Mixed ) );
}
