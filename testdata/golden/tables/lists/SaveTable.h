...
constexpr uint64_t PlacementFixedHash = 0x612211bc49eed17aull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry PlacementFixedPlan[] = {
    { 0u, 0u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // x
};
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == PlacementFixedHash );
    ...
    if ( identity )
    {
        ...
        static_assert( sizeof( Placement ) >= (size_t) PlacementFixedBodyBytes, "Placement: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) PlacementFixedBodyBytes );
            at += PlacementFixedRecordBytes;
        }
        return n;
    }
...
constexpr uint64_t LogEntryFixedHash = 0xea98a82ef5707ea9ull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry LogEntryFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // tick
};
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == LogEntryFixedHash );
    // THE PREFILL'S RANGES. EMPTY ON THE IDENTITY PLAN, and not by a flag:
    // the rule is the type's value bytes minus what the plan lands, and the
    // identity plan lands all of them.
    const TableFixedFill * fill = NULL;
    ...
    if ( identity )
    {
        ...
        // So the read is that memcpy, the scatter is empty and none is emitted,
        // and the prefill's list is empty for the reason above.
        static_assert( sizeof( LogEntry ) >= (size_t) LogEntryFixedBodyBytes, "LogEntry: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) LogEntryFixedBodyBytes );
            at += LogEntryFixedRecordBytes;
        }
        return n;
    }
...
constexpr uint64_t PointFixedHash = 0x1e621e0309d27347ull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry PointFixedPlan[] = {
    { 0u, 0u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // x
};
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == PointFixedHash );
    // THE PREFILL'S RANGES. EMPTY ON THE IDENTITY PLAN, and not by a flag:
    // the rule is the type's value bytes minus what the plan lands, and the
    // identity plan lands all of them.
    const TableFixedFill * fill = NULL;
    ...
    if ( identity )
    {
        ...
        // So the read is that memcpy, the scatter is empty and none is emitted,
        // and the prefill's list is empty for the reason above.
        static_assert( sizeof( Point ) >= (size_t) PointFixedBodyBytes, "Point: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) PointFixedBodyBytes );
            at += PointFixedRecordBytes;
        }
        return n;
    }
