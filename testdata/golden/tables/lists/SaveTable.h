...
constexpr uint64_t PlacementFixedHash = 0x612211bc49eed17aull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 4 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t PlacementFixedVocab[] = {
...
constexpr int64_t PlacementFixedVocabBytes = (int64_t) sizeof( PlacementFixedVocab );
...
{
    return 1 + 4 + PlacementFixedVocabBytes + count * PlacementFixedRecordBytes;
}
...
inline int64_t PlacementFixedSave( const Placement * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) PlacementFixedVocabBytes );
    memcpy( buffer + 5, PlacementFixedVocab, (size_t) PlacementFixedVocabBytes );
    uint8_t * at = buffer + 5 + PlacementFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != PlacementFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, PlacementFixedVocab, (int32_t) PlacementFixedVocabBytes, PlacementFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t LogEntryFixedHash = 0xea98a82ef5707ea9ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 2 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t LogEntryFixedVocab[] = {
...
constexpr int64_t LogEntryFixedVocabBytes = (int64_t) sizeof( LogEntryFixedVocab );
...
{
    return 1 + 4 + LogEntryFixedVocabBytes + count * LogEntryFixedRecordBytes;
}
...
inline int64_t LogEntryFixedSave( const LogEntry * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) LogEntryFixedVocabBytes );
    memcpy( buffer + 5, LogEntryFixedVocab, (size_t) LogEntryFixedVocabBytes );
    uint8_t * at = buffer + 5 + LogEntryFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != LogEntryFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, LogEntryFixedVocab, (int32_t) LogEntryFixedVocabBytes, LogEntryFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
...
constexpr uint64_t PointFixedHash = 0x1e621e0309d27347ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 3 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t PointFixedVocab[] = {
...
constexpr int64_t PointFixedVocabBytes = (int64_t) sizeof( PointFixedVocab );
...
{
    return 1 + 4 + PointFixedVocabBytes + count * PointFixedRecordBytes;
}
...
inline int64_t PointFixedSave( const Point * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) PointFixedVocabBytes );
    memcpy( buffer + 5, PointFixedVocab, (size_t) PointFixedVocabBytes );
    uint8_t * at = buffer + 5 + PointFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != PointFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, PointFixedVocab, (int32_t) PointFixedVocabBytes, PointFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
