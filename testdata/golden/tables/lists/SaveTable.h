...
{
    return 1 + 4 + PlacementFixedLayoutBytes + count * PlacementFixedRecordBytes;
}
...
inline int64_t PlacementFixedSave( const Placement * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) PlacementFixedLayoutBytes );
    memcpy( buffer + 5, PlacementFixedLayout, (size_t) PlacementFixedLayoutBytes );
    uint8_t * at = buffer + 5 + PlacementFixedLayoutBytes;
...
{
    return 1 + 4 + LogEntryFixedLayoutBytes + count * LogEntryFixedRecordBytes;
}
...
inline int64_t LogEntryFixedSave( const LogEntry * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) LogEntryFixedLayoutBytes );
    memcpy( buffer + 5, LogEntryFixedLayout, (size_t) LogEntryFixedLayoutBytes );
    uint8_t * at = buffer + 5 + LogEntryFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
...
{
    return 1 + 4 + PointFixedLayoutBytes + count * PointFixedRecordBytes;
}
...
inline int64_t PointFixedSave( const Point * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) PointFixedLayoutBytes );
    memcpy( buffer + 5, PointFixedLayout, (size_t) PointFixedLayoutBytes );
    uint8_t * at = buffer + 5 + PointFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
