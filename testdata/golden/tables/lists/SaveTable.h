...
inline bool PlacementLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, Placement & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool LogEntryLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, LogEntry & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool SaveLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Save & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool PointLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, Point & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool MixedLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Mixed & value, TableRetain * retain, const TableRetainPath & path );
...
{
    {
        ...
        if ( cursor_placements.count > 0 ) // an EMPTY list elides, the by-value rule (§3)
        {
            ...
            w.header( ref_placements, 14 ); w.putleb( (uint64_t) body_placements ); // placements
    ...
    }
    {
        ...
        if ( cursor_log.count > 0 ) // an EMPTY list elides, the by-value rule (§3)
        {
            ...
            w.header( ref_log, 14 ); w.putleb( (uint64_t) body_log ); // log
    ...
    }
    {
        ...
        if ( cursor_scores.count > 0 ) // an EMPTY list elides, the by-value rule (§3)
        {
            ...
            body_scores += 1 + TableLebBytes( (uint64_t) ( cursor_scores.count ) ); // the element kind byte and the count
            body_scores += (int64_t) ( cursor_scores.count ) * 4;
            w.header( ref_scores, 14 ); w.putleb( (uint64_t) body_scores ); // scores
...
inline bool MixedSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Mixed & value )
{
    ...
    }
    {
        ...
        if ( cursor_perms.count > 0 ) // an EMPTY list elides, the by-value rule (§3)
        {
            ...
            body_perms += 1 + TableLebBytes( (uint64_t) ( cursor_perms.count ) ); // the element kind byte and the count
            body_perms += (int64_t) ( cursor_perms.count ) * 8;
            w.header( ref_perms, 14 ); w.putleb( (uint64_t) body_perms ); // perms
    ...
    }
    {
        ...
        if ( cursor_hits.count > 0 ) // an EMPTY list elides, the by-value rule (§3)
        {
            ...
            w.header( ref_hits, 14 ); w.putleb( (uint64_t) body_hits ); // hits
                ...
                else
                {
                    switch ( cursor_hits[elem_i_hits].type )
                    {
                        case HitType::Point:
                        {
                            ...
                            w.header( arm_ref_hitsu, 13 ); w.putleb( (uint64_t) arm_payload_hitsu ); // point
                        ...
                        case HitType::Damage:
                        {
                            ...
                            int64_t arm_payload_hitsu = 0;
                            arm_payload_hitsu += 4; // int32
                            w.header( arm_ref_hitsu, 4 ); w.putleb( (uint64_t) arm_payload_hitsu ); // damage
    ...
    }
    {
        ...
        if ( cursor_bounds.count > 0 ) // an EMPTY list elides, the by-value rule (§3)
        {
            ...
            body_bounds += 1 + TableLebBytes( (uint64_t) ( cursor_bounds.count ) ); // the element kind byte and the count
            body_bounds += (int64_t) ( cursor_bounds.count ) * 4;
            w.header( ref_bounds, 14 ); w.putleb( (uint64_t) body_bounds ); // bounds
...
static_assert( sizeof( Placement ) == 12, "Placement: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( Placement, x ) == 0, "Placement.x: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Placement, y ) == 4, "Placement.y: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Placement, model ) == 8, "Placement.model: the fixed form's plan lands here" );
static_assert( sizeof( LogEntry ) == 4, "LogEntry: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( LogEntry, tick ) == 0, "LogEntry.tick: the fixed form's plan lands here" );
static_assert( sizeof( Point ) == 8, "Point: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( Point, x ) == 0, "Point.x: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Point, y ) == 4, "Point.y: the fixed form's plan lands here" );

// Placement's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void PlacementFixedWriteBody( uint8_t * b, const Placement & value )
{
    (void) b; (void) value;
    TableFixedPutF32( b + 0, value.x );
    TableFixedPutF32( b + 4, value.y );
    TableFixedPut32( b + 8, (uint32_t) value.model );
}

// LogEntry's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void LogEntryFixedWriteBody( uint8_t * b, const LogEntry & value )
{
    (void) b; (void) value;
    TableFixedPut32( b + 0, (uint32_t) value.tick );
}

// Point's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void PointFixedWriteBody( uint8_t * b, const Point & value )
{
    (void) b; (void) value;
    TableFixedPut32( b + 0, (uint32_t) value.x );
    TableFixedPut32( b + 4, (uint32_t) value.y );
}

// ---- Placement, the fixed form ----
...
constexpr int64_t PlacementFixedBodyBytes = 12;
constexpr int64_t PlacementFixedRecordBytes = 8 + PlacementFixedBodyBytes; // the hash and the body
constexpr uint64_t PlacementFixedHash = 0x612211bc49eed17aull; // fnv1a64 over the layout and the definitions digest (bill §13)
...
constexpr uint8_t PlacementFixedLayout[] = {
    0x04, 0x00, 0x00, 0x00, 0x80, 0xea, 0x3a, 0xb9, 0xf1, 0x21, 0xf7, 0x41, 0x0d, 0x0c, 0x00, 0x00,
    0x00, 0x03, 0x00, 0x00, 0x00, 0x07, 0x17, 0x02, 0x86, 0x4c, 0xf5, 0x63, 0xaf, 0x0a, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0x15, 0x02, 0x86, 0x4c, 0xf4, 0x63, 0xaf, 0x0a, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3a, 0x70, 0x6e, 0x3e, 0x93, 0x43, 0xe5, 0x9d, 0x08,
    0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr int64_t PlacementFixedLayoutBytes = (int64_t) sizeof( PlacementFixedLayout );
...
constexpr TableFixedDst PlacementFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Placement
    { (uint32_t) __builtin_offsetof( Placement, x ), 0, 0, 0, 0 }, // x
    { (uint32_t) __builtin_offsetof( Placement, y ), 0, 0, 0, 0 }, // y
    { (uint32_t) __builtin_offsetof( Placement, model ), 0, 0, 0, 0 }, // model
};
...
// test a guard at all.
constexpr TableFixedEntry PlacementFixedPlan[] = {
    { 0u, 0u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // x
};
constexpr int32_t PlacementFixedPlanCount = 1;
constexpr int32_t PlacementFixedPlanGuarded = 1;
...
constexpr TableFixedFill PlacementFixedCover[] = {
    { 0u, 12u },
};
constexpr int32_t PlacementFixedCoverCount = 1;
...
constexpr uint64_t PlacementFixedLineage[] = {
    0x612211bc49eed17aull, // identity
};
constexpr int32_t PlacementFixedLineageCount = 1;
constexpr int32_t PlacementFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t PlacementFixedFloorLive = PlacementFixedFloor;
inline void PlacementFixedSetFloorForTest( int32_t index ) { PlacementFixedFloorLive = index; }
#endif

constexpr uint8_t PlacementFixedKnown0Layout[] = {
    0x04, 0x00, 0x00, 0x00, 0x80, 0xea, 0x3a, 0xb9, 0xf1, 0x21, 0xf7, 0x41, 0x0d, 0x0c, 0x00, 0x00,
    0x00, 0x03, 0x00, 0x00, 0x00, 0x07, 0x17, 0x02, 0x86, 0x4c, 0xf5, 0x63, 0xaf, 0x0a, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0x15, 0x02, 0x86, 0x4c, 0xf4, 0x63, 0xaf, 0x0a, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3a, 0x70, 0x6e, 0x3e, 0x93, 0x43, 0xe5, 0x9d, 0x08,
...
constexpr TableFixedKnownLayout PlacementFixedKnown[] = {
    { 0x612211bc49eed17aull, PlacementFixedKnown0Layout, 72, 20 }, // identity
};

constexpr const TableFixedKnownRange * PlacementFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t PlacementFixedKnownRangeCounts[] = { 0 };
...
constexpr int64_t PlacementFixedMeasure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + PlacementFixedLayoutBytes + count * PlacementFixedRecordBytes;
}

inline int64_t PlacementFixedSave( const Placement * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = PlacementFixedMeasure( count );
    ...
    TableFixedPut64( buffer + kTableFixedHashAt, PlacementFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) PlacementFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, PlacementFixedLayout, (size_t) PlacementFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + PlacementFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
        TableFixedPut64( at, PlacementFixedHash );
        memset( at + 8, 0, (size_t) PlacementFixedBodyBytes ); // the prefill's zeros
        PlacementFixedWriteBody( at + 8, values[k] );
        at += PlacementFixedRecordBytes;
    }
    return need;
}
...
inline int64_t PlacementFixedLoad( Placement * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    for ( int32_t i = 0; i < PlacementFixedLineageCount; ++i )
    {
        if ( PlacementFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = PlacementFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = PlacementFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = PlacementFixedKnown[lineage_at];
    ...
    const TableFixedEntry * entries = PlacementFixedPlan;
    int32_t entry_count = PlacementFixedPlanCount;
    int32_t entry_guarded = PlacementFixedPlanGuarded;
    int64_t record_bytes = PlacementFixedRecordBytes;
    const TableFixedFill * fill = NULL;
    int32_t fill_count = 0;
    if ( hash != PlacementFixedHash )
    {
        ...
        else
        {
            ...
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, PlacementFixedLayout, (int32_t) PlacementFixedLayoutBytes, PlacementFixedDst,
                                                   PlacementFixedCover, PlacementFixedCoverCount,
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( PlacementFixedKnownRanges[lineage_at], PlacementFixedKnownRangeCounts[lineage_at],
...
// ---- LogEntry, the fixed form ----

// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
constexpr int64_t LogEntryFixedBodyBytes = 4;
constexpr int64_t LogEntryFixedRecordBytes = 8 + LogEntryFixedBodyBytes; // the hash and the body
constexpr uint64_t LogEntryFixedHash = 0xea98a82ef5707ea9ull; // fnv1a64 over the layout and the definitions digest (bill §13)

// THE LAYOUT (form 1 calls this the vocabulary block): 2 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
constexpr uint8_t LogEntryFixedLayout[] = {
    0x02, 0x00, 0x00, 0x00, 0x5f, 0x82, 0x58, 0xac, 0x36, 0x15, 0x78, 0x5e, 0x0d, 0x04, 0x00, 0x00,
    0x00, 0x01, 0x00, 0x00, 0x00, 0x84, 0x76, 0xbc, 0x2e, 0xef, 0x83, 0x76, 0x1e, 0x08, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr int64_t LogEntryFixedLayoutBytes = (int64_t) sizeof( LogEntryFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout lands values through.
constexpr TableFixedDst LogEntryFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // LogEntry
    { (uint32_t) __builtin_offsetof( LogEntry, tick ), 0, 0, 0, 0 }, // tick
};
...
// FIRST, then the arms: the entries that are nearly all of a plan never
// test a guard at all.
constexpr TableFixedEntry LogEntryFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // tick
};
constexpr int32_t LogEntryFixedPlanCount = 1;
constexpr int32_t LogEntryFixedPlanGuarded = 1;
...
// twice; a plan compiled from a stranger's layout subtracts itself from it
// once, when the plan is compiled, and prefills exactly the rest.
constexpr TableFixedFill LogEntryFixedCover[] = {
    { 0u, 4u },
};
constexpr int32_t LogEntryFixedCoverCount = 1;

// THE LINEAGE, oldest first, the reader's own last (bill §6b). LOAD looks
// the file's header hash up here and never parses a stranger's layout.
constexpr uint64_t LogEntryFixedLineage[] = {
    0xea98a82ef5707ea9ull, // identity
};
constexpr int32_t LogEntryFixedLineageCount = 1;
constexpr int32_t LogEntryFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t LogEntryFixedFloorLive = LogEntryFixedFloor;
inline void LogEntryFixedSetFloorForTest( int32_t index ) { LogEntryFixedFloorLive = index; }
#endif

constexpr uint8_t LogEntryFixedKnown0Layout[] = {
    0x02, 0x00, 0x00, 0x00, 0x5f, 0x82, 0x58, 0xac, 0x36, 0x15, 0x78, 0x5e, 0x0d, 0x04, 0x00, 0x00,
    0x00, 0x01, 0x00, 0x00, 0x00, 0x84, 0x76, 0xbc, 0x2e, 0xef, 0x83, 0x76, 0x1e, 0x08, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr TableFixedKnownLayout LogEntryFixedKnown[] = {
    { 0xea98a82ef5707ea9ull, LogEntryFixedKnown0Layout, 38, 12 }, // identity
};

constexpr const TableFixedKnownRange * LogEntryFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t LogEntryFixedKnownRangeCounts[] = { 0 };

// A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
constexpr int64_t LogEntryFixedMeasure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + LogEntryFixedLayoutBytes + count * LogEntryFixedRecordBytes;
}

inline int64_t LogEntryFixedSave( const LogEntry * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = LogEntryFixedMeasure( count );
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    memset( buffer, 0, (size_t) kTableFixedHeaderBytes ); // the seven reserved bytes, and the rest of the header
    buffer[0] = kTableFixedForm;
    TableFixedPut64( buffer + kTableFixedHashAt, LogEntryFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) LogEntryFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, LogEntryFixedLayout, (size_t) LogEntryFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + LogEntryFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
        TableFixedPut64( at, LogEntryFixedHash );
        memset( at + 8, 0, (size_t) LogEntryFixedBodyBytes ); // the prefill's zeros
        LogEntryFixedWriteBody( at + 8, values[k] );
        at += LogEntryFixedRecordBytes;
    }
    return need;
}
...
// different bytes is layout_malformed, a hash below the floor is
// layout_unsupported. Identity is a PLAN walked by the same loop.
inline int64_t LogEntryFixedLoad( LogEntry * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    TableReport local;
    if ( report == NULL ) { report = &local; }
    ...
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    int32_t lineage_at = -1;
    for ( int32_t i = 0; i < LogEntryFixedLineageCount; ++i )
    {
        if ( LogEntryFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = LogEntryFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = LogEntryFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = LogEntryFixedKnown[lineage_at];
    if ( (int64_t) layout_bytes != known.layout_bytes || memcmp( layout, known.layout, (size_t) layout_bytes ) != 0 )
    {
        report->refused = true; report->reason = layout_malformed; return -1;
    }
    const TableFixedEntry * entries = LogEntryFixedPlan;
    int32_t entry_count = LogEntryFixedPlanCount;
    int32_t entry_guarded = LogEntryFixedPlanGuarded;
    int64_t record_bytes = LogEntryFixedRecordBytes;
    const TableFixedFill * fill = NULL;
    int32_t fill_count = 0;
    if ( hash != LogEntryFixedHash )
    {
        // AN OLDER WRITER: compile the plan from the TRUSTED known layout,
        // never from the file's bytes (those were only compared). Cached by hash.
        const TableFixedPlanCacheSlot * hit = NULL;
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, LogEntryFixedLayout, (int32_t) LogEntryFixedLayoutBytes, LogEntryFixedDst,
                                                   LogEntryFixedCover, LogEntryFixedCoverCount,
                                                   dest, dest_capacity, &compiled_guarded, &fill_at, &fill_count, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_record_too_large : plan_too_large; return -1; }
            record_bytes = known.record_bytes;
            fill = ( fill_count > 0 ) ? (const TableFixedFill *) (const void *) ( (const uint8_t *) dest + fill_at ) : NULL;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( LogEntryFixedKnownRanges[lineage_at], LogEntryFixedKnownRangeCounts[lineage_at],
                                    (uint8_t *) &values[k], report );
        at += record_bytes;
    }
    return n;
...
// ---- Point, the fixed form ----

// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
constexpr int64_t PointFixedBodyBytes = 8;
constexpr int64_t PointFixedRecordBytes = 8 + PointFixedBodyBytes; // the hash and the body
constexpr uint64_t PointFixedHash = 0x1e621e0309d27347ull; // fnv1a64 over the layout and the definitions digest (bill §13)

// THE LAYOUT (form 1 calls this the vocabulary block): 3 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
constexpr uint8_t PointFixedLayout[] = {
    0x03, 0x00, 0x00, 0x00, 0x11, 0xed, 0xd9, 0xce, 0x96, 0x92, 0x43, 0x8a, 0x0d, 0x08, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0x07, 0x17, 0x02, 0x86, 0x4c, 0xf5, 0x63, 0xaf, 0x04, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0x15, 0x02, 0x86, 0x4c, 0xf4, 0x63, 0xaf, 0x04, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr int64_t PointFixedLayoutBytes = (int64_t) sizeof( PointFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout lands values through.
constexpr TableFixedDst PointFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Point
    { (uint32_t) __builtin_offsetof( Point, x ), 0, 0, 0, 0 }, // x
    { (uint32_t) __builtin_offsetof( Point, y ), 0, 0, 0, 0 }, // y
};
...
// FIRST, then the arms: the entries that are nearly all of a plan never
// test a guard at all.
constexpr TableFixedEntry PointFixedPlan[] = {
    { 0u, 0u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // x
};
constexpr int32_t PointFixedPlanCount = 1;
constexpr int32_t PointFixedPlanGuarded = 1;
...
// twice; a plan compiled from a stranger's layout subtracts itself from it
// once, when the plan is compiled, and prefills exactly the rest.
constexpr TableFixedFill PointFixedCover[] = {
    { 0u, 8u },
};
constexpr int32_t PointFixedCoverCount = 1;

// THE LINEAGE, oldest first, the reader's own last (bill §6b). LOAD looks
// the file's header hash up here and never parses a stranger's layout.
constexpr uint64_t PointFixedLineage[] = {
    0x1e621e0309d27347ull, // identity
};
constexpr int32_t PointFixedLineageCount = 1;
constexpr int32_t PointFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t PointFixedFloorLive = PointFixedFloor;
inline void PointFixedSetFloorForTest( int32_t index ) { PointFixedFloorLive = index; }
#endif

constexpr uint8_t PointFixedKnown0Layout[] = {
    0x03, 0x00, 0x00, 0x00, 0x11, 0xed, 0xd9, 0xce, 0x96, 0x92, 0x43, 0x8a, 0x0d, 0x08, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0x07, 0x17, 0x02, 0x86, 0x4c, 0xf5, 0x63, 0xaf, 0x04, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x54, 0x15, 0x02, 0x86, 0x4c, 0xf4, 0x63, 0xaf, 0x04, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr TableFixedKnownLayout PointFixedKnown[] = {
    { 0x1e621e0309d27347ull, PointFixedKnown0Layout, 55, 16 }, // identity
};

constexpr const TableFixedKnownRange * PointFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t PointFixedKnownRangeCounts[] = { 0 };

// A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
constexpr int64_t PointFixedMeasure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + PointFixedLayoutBytes + count * PointFixedRecordBytes;
}

inline int64_t PointFixedSave( const Point * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = PointFixedMeasure( count );
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    memset( buffer, 0, (size_t) kTableFixedHeaderBytes ); // the seven reserved bytes, and the rest of the header
    buffer[0] = kTableFixedForm;
    TableFixedPut64( buffer + kTableFixedHashAt, PointFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) PointFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, PointFixedLayout, (size_t) PointFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + PointFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
        TableFixedPut64( at, PointFixedHash );
        memset( at + 8, 0, (size_t) PointFixedBodyBytes ); // the prefill's zeros
        PointFixedWriteBody( at + 8, values[k] );
        at += PointFixedRecordBytes;
    }
    return need;
}
...
// different bytes is layout_malformed, a hash below the floor is
// layout_unsupported. Identity is a PLAN walked by the same loop.
inline int64_t PointFixedLoad( Point * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    TableReport local;
    if ( report == NULL ) { report = &local; }
    ...
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    int32_t lineage_at = -1;
    for ( int32_t i = 0; i < PointFixedLineageCount; ++i )
    {
        if ( PointFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = PointFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = PointFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = PointFixedKnown[lineage_at];
    if ( (int64_t) layout_bytes != known.layout_bytes || memcmp( layout, known.layout, (size_t) layout_bytes ) != 0 )
    {
        report->refused = true; report->reason = layout_malformed; return -1;
    }
    const TableFixedEntry * entries = PointFixedPlan;
    int32_t entry_count = PointFixedPlanCount;
    int32_t entry_guarded = PointFixedPlanGuarded;
    int64_t record_bytes = PointFixedRecordBytes;
    const TableFixedFill * fill = NULL;
    int32_t fill_count = 0;
    if ( hash != PointFixedHash )
    {
        // AN OLDER WRITER: compile the plan from the TRUSTED known layout,
        // never from the file's bytes (those were only compared). Cached by hash.
        const TableFixedPlanCacheSlot * hit = NULL;
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, PointFixedLayout, (int32_t) PointFixedLayoutBytes, PointFixedDst,
                                                   PointFixedCover, PointFixedCoverCount,
                                                   dest, dest_capacity, &compiled_guarded, &fill_at, &fill_count, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_record_too_large : plan_too_large; return -1; }
            record_bytes = known.record_bytes;
            fill = ( fill_count > 0 ) ? (const TableFixedFill *) (const void *) ( (const uint8_t *) dest + fill_at ) : NULL;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( PointFixedKnownRanges[lineage_at], PointFixedKnownRangeCounts[lineage_at],
                                    (uint8_t *) &values[k], report );
        at += record_bytes;
    }
    return n;
            ...
            case 0xd24733aa574d4b09ull: // placements
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        Placement * slot = TableListFillNext( fill );
                        if ( slot == NULL ) { report->malformed = true; return false; } // the arena could not carve
                        if ( !PlacementLoadMessageBodyRetain( r, vocabulary, report, index_bits, ( *slot ), retain, TableRetainStepInto( path, 0, (uint32_t) ( i ) ) ) ) { return false; }
                    }
                    TableListFillEnd( fill );
                }
                break;
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        ...
                        if ( slot == NULL ) { report->malformed = true; return false; } // the arena could not carve
                        {
                            ...
                            else if ( !TableEnumValue( variant_entry_2.id, ( *slot ) ) ) { ( *slot ) = Grade::None; report->unknown++; report->retain_lost++; }
                        }
                    }
                    TableListFillEnd( fill );
                }
                            ...
                            else
                            {
                                switch ( arm_2.id )
                                {
                                    case 0x73feab3544c345b1ull: // point
                                    {
                                        ...
                                        ( *slot ).type = HitType::Point;
                                        if ( !PointLoadMessageBodyRetain( r, vocabulary, report, index_bits, ( *slot ).point, retain, TableRetainStepInto( path, 2, (uint32_t) ( 0 ) ) ) ) { return false; }
                                        break;
                                    }
                                    case 0x7f6308be8ab37fc0ull: // damage
                                    {
                                    ...
                                    default:
                                        ( *slot ).type = HitType::None; report->unknown++; report->retain_lost++;
                                        if ( !TableMessageSkip( r, vocabulary, index_bits, arm_2 ) ) { report->malformed = true; return false; }
                                        break;
                                }
                            }
...
// SaveNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool SaveNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + SaveNodeRecordBytes( type_id );
    carve.left = 0;
    ...
    switch ( type_id )
    {
        case 0x5e781536ac58825full: ok = LogEntryLoadMessageBodyRetain( r, vocabulary, report, index_bits, *(LogEntry *) at, retain, TableRetainPathRoot( (const void *) at, node ) ); break; // LogEntry
        // a record this dispatch cannot name never reaches here: pass one left it absent
        default: report->malformed = true; break;
    }
    nodes.carve = outer;
...
// SaveLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
inline bool SaveLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Save * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !SaveNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return SaveLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// SaveLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool SaveLoadRetainMessages( const Save ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !SaveLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
// MixedNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
// index is this body's own directory position, which the batch's load fills
// from the framing and nothing afterwards renumbers.
inline bool MixedNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + MixedNodeRecordBytes( type_id );
    carve.left = 0;
...
// MixedLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
// each body carries its own node directory inside that one region and a
// record's first step is an index into it (docs/SPEC-TABLES.md §3.3, §6.6).
inline bool MixedLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Mixed * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !MixedNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return MixedLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// MixedLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
// SaveRetain from `roots[k]` takes `retains[k]` and is the VARIABLE form's own
// pair unchanged: retention writing FORM 2 refuses by name (§3.3).
inline bool MixedLoadRetainMessages( const Mixed ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !MixedLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
template <typename... Args>
inline void PlacementLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Placement is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PlacementSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Placement is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void LogEntryLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "LogEntry is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void LogEntrySaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "LogEntry is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PointLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Point is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PointSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Point is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
