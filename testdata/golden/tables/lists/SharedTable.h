...
constexpr int PhotoFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Photo's LEAVES: one entry per run of bytes that lands, at a base the caller
...
constexpr int PhotoFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Photo, width );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    {
        const uint32_t es = src + 4u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Photo, height );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// Photo's stores. The template — the hash, then zeros — is memcpy'd first,
...
inline constexpr int64_t PhotoFixedBodyBytes = 8;
inline constexpr int64_t PhotoFixedRecordBytes = 8 + PhotoFixedBodyBytes; // the hash and the body
inline constexpr uint64_t PhotoFixedHash = 0x80b2a2152ea36570ull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t PhotoFixedLayout[] = {
...
inline constexpr int64_t PhotoFixedLayoutBytes = (int64_t) sizeof( PhotoFixedLayout );
...
inline constexpr TableFixedDst PhotoFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Photo
...
inline constexpr auto PhotoFixedPlan = TableFixedBuildPlan<2>( PhotoFixedLeaves );
...
inline constexpr int64_t PhotoFixedMeasure( int64_t count )
{
...
inline int64_t PhotoFixedSave( const Photo * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) PhotoFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = PhotoFixedPlan.entries;
    int32_t entry_count = PhotoFixedPlan.count;
    ...
    if ( hash != PhotoFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, PhotoFixedLayout, (int32_t) PhotoFixedLayoutBytes, PhotoFixedDst, plan, plan_capacity, report );
