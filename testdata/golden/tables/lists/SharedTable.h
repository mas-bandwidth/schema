...
constexpr uint64_t PhotoFixedHash = 0x80b2a2152ea36570ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t PhotoFixedVocab[] = {
...
constexpr int64_t PhotoFixedVocabBytes = (int64_t) sizeof( PhotoFixedVocab );
...
{
    return 1 + 4 + PhotoFixedVocabBytes + count * PhotoFixedRecordBytes;
}
...
inline int64_t PhotoFixedSave( const Photo * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) PhotoFixedVocabBytes );
    memcpy( buffer + 5, PhotoFixedVocab, (size_t) PhotoFixedVocabBytes );
    uint8_t * at = buffer + 5 + PhotoFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != PhotoFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, PhotoFixedVocab, (int32_t) PhotoFixedVocabBytes, PhotoFixedDst, plan, plan_capacity, &compiled_guarded, report );
