...
{
    return 1 + 4 + PhotoFixedLayoutBytes + count * PhotoFixedRecordBytes;
}
...
inline int64_t PhotoFixedSave( const Photo * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) PhotoFixedLayoutBytes );
    memcpy( buffer + 5, PhotoFixedLayout, (size_t) PhotoFixedLayoutBytes );
    uint8_t * at = buffer + 5 + PhotoFixedLayoutBytes;
