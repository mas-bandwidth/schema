...
LISTDEMO_TABLE_INLINE bool PhotoLoadBodyRetain( TableReader & r, Photo & value, TableRetain * retain, const TableRetainPath & path );
inline bool PhotoLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, Photo & value, TableRetain * retain, const TableRetainPath & path );
...
inline bool AlbumLoadMessageBodyRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, Album & value, TableRetain * retain, const TableRetainPath & path );
...
{
    {
        ...
        if ( cursor_photos.count > 0 ) // an EMPTY list elides, the by-value rule (§3)
        {
            ...
            w.header( ref_photos, 14 ); w.putleb( (uint64_t) body_photos ); // photos
    ...
    }
    {
        ...
        if ( pointee_cover != NULL )
        {
            ...
            w.header( ids.ref_at( 38, 0xaa19a78e404dea20ull ), 17 ); // cover — a NODE INDEX into the flat node table
...
static_assert( sizeof( Photo ) == 8, "Photo: the fixed form's plan is laid out for this size" );
static_assert( (uint32_t) __builtin_offsetof( Photo, width ) == 0, "Photo.width: the fixed form's plan lands here" );
static_assert( (uint32_t) __builtin_offsetof( Photo, height ) == 4, "Photo.height: the fixed form's plan lands here" );

// Photo's stores. The prefill — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
inline void PhotoFixedWriteBody( uint8_t * b, const Photo & value )
{
    (void) b; (void) value;
    TableFixedPut32( b + 0, (uint32_t) value.width );
    TableFixedPut32( b + 4, (uint32_t) value.height );
}

// ---- Photo, the fixed form ----
...
constexpr int64_t PhotoFixedBodyBytes = 8;
constexpr int64_t PhotoFixedRecordBytes = 8 + PhotoFixedBodyBytes; // the hash and the body
constexpr uint64_t PhotoFixedHash = 0x80b2a2152ea36570ull; // fnv1a64 over the layout and the definitions digest (bill §13)
...
constexpr uint8_t PhotoFixedLayout[] = {
    0x03, 0x00, 0x00, 0x00, 0xc3, 0x64, 0x89, 0x50, 0xd2, 0x8d, 0xa7, 0xf1, 0x0d, 0x08, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0xbf, 0xe9, 0xd1, 0x2f, 0x93, 0xcd, 0xda, 0xdb, 0x08, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x72, 0x34, 0x7d, 0xf6, 0x0b, 0x72, 0x17, 0x08, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr int64_t PhotoFixedLayoutBytes = (int64_t) sizeof( PhotoFixedLayout );
...
constexpr TableFixedDst PhotoFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Photo
    { (uint32_t) __builtin_offsetof( Photo, width ), 0, 0, 0, 0 }, // width
    { (uint32_t) __builtin_offsetof( Photo, height ), 0, 0, 0, 0 }, // height
};
...
// test a guard at all.
constexpr TableFixedEntry PhotoFixedPlan[] = {
    { 0u, 0u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1, kTableFixedNoGuard, 0, 1 }, // width
};
constexpr int32_t PhotoFixedPlanCount = 1;
constexpr int32_t PhotoFixedPlanGuarded = 1;
...
constexpr TableFixedFill PhotoFixedCover[] = {
    { 0u, 8u },
};
constexpr int32_t PhotoFixedCoverCount = 1;
...
constexpr uint64_t PhotoFixedLineage[] = {
    0x80b2a2152ea36570ull, // identity
};
constexpr int32_t PhotoFixedLineageCount = 1;
constexpr int32_t PhotoFixedFloor = 0;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
inline int32_t PhotoFixedFloorLive = PhotoFixedFloor;
inline void PhotoFixedSetFloorForTest( int32_t index ) { PhotoFixedFloorLive = index; }
#endif

constexpr uint8_t PhotoFixedKnown0Layout[] = {
    0x03, 0x00, 0x00, 0x00, 0xc3, 0x64, 0x89, 0x50, 0xd2, 0x8d, 0xa7, 0xf1, 0x0d, 0x08, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x00, 0xbf, 0xe9, 0xd1, 0x2f, 0x93, 0xcd, 0xda, 0xdb, 0x08, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x72, 0x34, 0x7d, 0xf6, 0x0b, 0x72, 0x17, 0x08, 0x04,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
constexpr TableFixedKnownLayout PhotoFixedKnown[] = {
    { 0x80b2a2152ea36570ull, PhotoFixedKnown0Layout, 55, 16 }, // identity
};

constexpr const TableFixedKnownRange * PhotoFixedKnownRanges[] = {
    NULL,
};
constexpr int32_t PhotoFixedKnownRangeCounts[] = { 0 };
...
constexpr int64_t PhotoFixedMeasure( int64_t count )
{
    return kTableFixedHeaderBytes + 4 + PhotoFixedLayoutBytes + count * PhotoFixedRecordBytes;
}

inline int64_t PhotoFixedSave( const Photo * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = PhotoFixedMeasure( count );
    ...
    TableFixedPut64( buffer + kTableFixedHashAt, PhotoFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) PhotoFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, PhotoFixedLayout, (size_t) PhotoFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + PhotoFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
        TableFixedPut64( at, PhotoFixedHash );
        memset( at + 8, 0, (size_t) PhotoFixedBodyBytes ); // the prefill's zeros
        PhotoFixedWriteBody( at + 8, values[k] );
        at += PhotoFixedRecordBytes;
    }
    return need;
}
...
inline int64_t PhotoFixedLoad( Photo * values, int64_t capacity, const uint8_t * data, int64_t bytes,
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    for ( int32_t i = 0; i < PhotoFixedLineageCount; ++i )
    {
        if ( PhotoFixedLineage[i] == hash ) { lineage_at = i; break; }
    }
    if ( lineage_at < 0 )
    {
        report->refused = true; report->reason = layout_newer; report->layout_hash = hash; return -1;
    }
    int32_t floor_at = PhotoFixedFloor;
#ifdef SCHEMA_FIXED_FLOOR_TEST_HOOKS
    floor_at = PhotoFixedFloorLive;
#endif
    if ( lineage_at < floor_at )
    {
        report->refused = true; report->reason = layout_unsupported; report->layout_hash = hash; return -1;
    }
    const TableFixedKnownLayout & known = PhotoFixedKnown[lineage_at];
    ...
    const TableFixedEntry * entries = PhotoFixedPlan;
    int32_t entry_count = PhotoFixedPlanCount;
    int32_t entry_guarded = PhotoFixedPlanGuarded;
    int64_t record_bytes = PhotoFixedRecordBytes;
    const TableFixedFill * fill = NULL;
    int32_t fill_count = 0;
    if ( hash != PhotoFixedHash )
    {
        ...
        else
        {
            ...
            uint32_t fill_at = 0;
            const int32_t made = TableFixedCompile( parsed, PhotoFixedLayout, (int32_t) PhotoFixedLayoutBytes, PhotoFixedDst,
                                                   PhotoFixedCover, PhotoFixedCoverCount,
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        ...
        TableFixedClampKnownRanges( PhotoFixedKnownRanges[lineage_at], PhotoFixedKnownRangeCounts[lineage_at],
...
inline void AlbumNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        ...
        const int64_t storage = AlbumNodeStorage( type_id, r.size, reason );
...
inline void AlbumNodeBodyRetain( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at, TableRetain * retain, uint32_t node )
{
    ...
    switch ( type_id )
    {
        case 0xf1a78dd2508964c3ull: PhotoLoadBodyRetain( r, *(Photo *) at, retain, TableRetainPathRoot( (const void *) at, node ) ); break; // Photo
        default: break;
    }
    nodes.carve = NULL; // the cursor is ONE node's, and this node's body is done
}
...
// AlbumNodeMessageBodyRetain: PASS TWO's half, which decodes one record's body into
// storage it already owns, its map entries carved from its own extent.
// EACH NODE BODY IS A PATH ROOT of its own (docs/SPEC-TABLES.md §6.6): the
...
inline bool AlbumNodeMessageBodyRetain( uint64_t type_id, TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, const TableNodeMap & nodes, int64_t index_bits, uint8_t * at, TableRetain * retain, uint32_t node )
{
    TableExtentCarve carve;
    carve.at = at + AlbumNodeRecordBytes( type_id );
    carve.left = 0;
    ...
    switch ( type_id )
    {
        case 0xf1a78dd2508964c3ull: ok = PhotoLoadMessageBodyRetain( r, vocabulary, report, index_bits, *(Photo *) at, retain, TableRetainPathRoot( (const void *) at, node ) ); break; // Photo
        // a record this dispatch cannot name never reaches here: pass one left it absent
        default: report->malformed = true; break;
    }
    nodes.carve = outer;
...
// AlbumLoadMessageBodyIntoRetain: one body of a batch into the region at `used`. Its
// chunk is the node DIRECTORY, then the records in wire order, then the root
// and the extent its maps take, so every offset a pass needs is known when
// the pass reaches it. PASS ONE fills the numbering from the framing and
// places every node; PASS TWO decodes each record's body into the storage it
...
inline bool AlbumLoadMessageBodyIntoRetain( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * out, uint8_t * region, int64_t region_bytes, int64_t & used, const Album * & root_out, TableRetain * retain )
{
    // the node table opens the body, or the body has none
    int64_t count = 0;
    if ( !TableMessageNodeTableOpen( r, vocabulary, count ) ) { out->malformed = true; return false; }
    ...
    for ( int64_t k = 0; k < count; k++ )
    {
        ...
        if ( !AlbumNodeMessageBodyRetain( type_id, r, vocabulary, out, nodes, index_bits, region + directory[k + 1].offset, retain, (uint32_t) ( k + 2 ) ) ) { return false; }
    }
    if ( r.offset != fields_start ) { out->malformed = true; return false; } // the two passes disagree about the table's extent

    // and the ROOT's own body last
    nodes.carve = &root_carve; // the ROOT's extent is its own, like every node's
    return AlbumLoadMessageBodyRetain( r, vocabulary, out, nodes, index_bits, *root, retain, TableRetainPathRoot( (const void *) root, 1 ) );
}

// AlbumLoadRetainMessages: decode a BATCH into the caller's exact-sized region and
// write each body's root into `roots`. `count` is IN and OUT: the storage the
// caller has room for, then what it got. M above the capacity is a refusal
// by name with count holding the wire's M; damage inside body k delivers
// bodies 1 to k - 1 and count says k - 1 (§3.3). LOAD IS A SCAN: it follows
...
inline bool AlbumLoadRetainMessages( const Album ** roots, int64_t * count, uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableRetain * retains, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    if ( roots == NULL || count == NULL ) { out->malformed = true; return false; }
    ...
    *count = 0;
    ...
    {
        roots[b] = NULL;
        if ( !AlbumLoadMessageBodyIntoRetain( br.r, vocabulary, out, region, region_bytes, used, roots[b], retains != NULL ? &retains[b] : NULL ) ) { *count = b; return false; }
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
inline int64_t AlbumMeasureWireRetain( const Ctx & ctx, const Album & root, TableRetain * retain, TableAllocator allocator )
{
    ...
    auto retain_measure = []( const Ctx & c, const TableNumbering & nn, TableRetainIds & ii, uint64_t type_id, const void * node, TableRetain * rt ) -> int64_t
    {
        ...
        switch ( type_id )
        {
            case 0xf1a78dd2508964c3ull: return PhotoMeasureBodyRetain( ii, *(const Photo *) node, rt, at ); // Photo
            default: break;
        }
        return -1;
    };
...
inline int64_t AlbumSaveWireRetain( const Ctx & ctx, const Album & root, TableRetain * retain, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    ...
    auto retain_save = []( const Ctx & c, const TableNumbering & nn, TableWriter & ww, TableRetainIds & ii, uint64_t type_id, const void * node, TableRetain * rt ) -> bool
    {
        ...
        switch ( type_id )
        {
            case 0xf1a78dd2508964c3ull: return PhotoSaveBodyRetain( ww, ii, *(const Photo *) node, rt, at ); // Photo
            default: break;
        }
        return false;
    };
...
template <typename... Args>
inline void PhotoSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Photo is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PhotoLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Photo is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PhotoSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "Photo is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
// parameter on a load.
inline const Photo * PhotoOpen( const void * bytes, uint64_t length, TableRefuseReason * reason = NULL )
{
    return (const Photo *) TableCookOpen( bytes, length, (uint64_t) sizeof( Photo ), (uint64_t) alignof( Photo ), reason );
}
