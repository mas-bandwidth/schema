...
inline int64_t PhotoMeasureBody( TableIds & ids, const Photo & value )
{
    ...
    if ( value.width != 0 ) { bytes += TableLebBytes( ids.ref( 0xdbdacd932fd1e9bfull, 17 ) ) + 1 + 4; } // width
    if ( value.height != 0 ) { bytes += TableLebBytes( ids.ref( 0x17720bf67d347222ull, 18 ) ) + 1 + 4; } // height
    return bytes;
}
...
LISTDEMO_TABLE_INLINE bool PhotoSaveBody( TableWriter & w, TableIds & ids, const Photo & value )
{
    if ( value.width != 0 )
    {
        w.putleb( ids.ref( 0xdbdacd932fd1e9bfull, 17 ) ); w.put8( 8 ); // width
        w.put32( uint32_t( value.width ) );
    }
    if ( value.height != 0 )
    {
        w.putleb( ids.ref( 0x17720bf67d347222ull, 18 ) ); w.put8( 8 ); // height
...
inline int64_t PhotoMeasureMessage( const Photo & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = PhotoMeasureBody( ids, value );
...
inline int64_t PhotoSaveMessage( const Photo & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !PhotoSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == PhotoMeasureMessage( value )
}
...
inline bool PhotoLoadMessage( Photo & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    PhotoReset( value );
...
inline int64_t AlbumMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Album & value )
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    {
        ...
        {
            const uint64_t ref_photos = ids.ref( 0x40b1d94aff3ab130ull, 2 );
    ...
    }
    {
        ...
        if ( pointee_cover != NULL )
        {
            ...
            bytes += TableLebBytes( ids.ref( 0xaa19a78e404dea20ull, 3 ) ) + 1 + TableLebBytes( index_cover );
        }
    }
    return bytes;
}
...
inline bool AlbumSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Album & value )
{
    ...
    }
    {
        ...
        if ( pointee_cover != NULL )
        {
            ...
            w.putleb( ids.ref( 0xaa19a78e404dea20ull, 3 ) ); w.put8( 17 ); // cover — a NODE INDEX into the flat node table
        ...
        {
            {
                ...
                if ( pointee != NULL )
                {
                    ...
                    else
                    {
                        ...
                        node.type_slot = 52; // its slot in the unit's vocabulary (§3.3)
