...
constexpr uint64_t PhotoFixedHash = 0x80b2a2152ea36570ull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry PhotoFixedPlan[] = {
    { 0u, 0u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // width
};
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == PhotoFixedHash );
    ...
    if ( identity )
    {
        ...
        static_assert( sizeof( Photo ) >= (size_t) PhotoFixedBodyBytes, "Photo: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) PhotoFixedBodyBytes );
            at += PhotoFixedRecordBytes;
        }
        return n;
    }
