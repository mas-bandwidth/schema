...
{
    return TableJsonRead( &value, PhotoTableType(), text, bytes, report );
}

int64_t PhotoToJsonMeasure( const Photo & value )
{
    return TableJsonWrite( &value, PhotoTableType(), NULL, 0 );
}

int64_t PhotoToJson( const Photo & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, PhotoTableType(), buffer, capacity );
}
...
bool AlbumFromJson( AlbumBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, AlbumTableType(), text, bytes, report );
}

int64_t AlbumToJsonMeasure( const Album * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, AlbumTableType(), NULL, 0, allocator );
}

int64_t AlbumToJson( const Album * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, AlbumTableType(), buffer, capacity, allocator );
}
