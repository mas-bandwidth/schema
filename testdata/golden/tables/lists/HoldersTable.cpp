...
// package listdemo — the TABLE wire's text form (docs/SPEC-TABLES.md §16).
...
// The guard is not vestigial. Several listdemo Table.cpp files may be
...
#ifndef LISTDEMO_SCHEMA_TABLE_JSON
#define LISTDEMO_SCHEMA_TABLE_JSON
...
#endif // LISTDEMO_SCHEMA_TABLE_JSON

namespace listdemo {

bool SampleFromJson( Sample & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, SampleTableType(), text, bytes, report );
}

int64_t SampleToJsonMeasure( const Sample & value )
{
    return TableJsonWrite( &value, SampleTableType(), NULL, 0 );
}

int64_t SampleToJson( const Sample & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, SampleTableType(), buffer, capacity );
}
...
bool RowFromJson( RowBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, RowTableType(), text, bytes, report );
}

int64_t RowToJsonMeasure( const Row * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, RowTableType(), NULL, 0, allocator );
}

int64_t RowToJson( const Row * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, RowTableType(), buffer, capacity, allocator );
}
...
bool SheetFromJson( SheetBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, SheetTableType(), text, bytes, report );
}

int64_t SheetToJsonMeasure( const Sheet * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SheetTableType(), NULL, 0, allocator );
}

int64_t SheetToJson( const Sheet * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SheetTableType(), buffer, capacity, allocator );
}
...
bool SquadFromJson( SquadBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, SquadTableType(), text, bytes, report );
}

int64_t SquadToJsonMeasure( const Squad * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SquadTableType(), NULL, 0, allocator );
}

int64_t SquadToJson( const Squad * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, SquadTableType(), buffer, capacity, allocator );
}
...
bool ArmyFromJson( ArmyBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, ArmyTableType(), text, bytes, report );
}

int64_t ArmyToJsonMeasure( const Army * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ArmyTableType(), NULL, 0, allocator );
}

int64_t ArmyToJson( const Army * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ArmyTableType(), buffer, capacity, allocator );
}
...
bool DeckFromJson( DeckBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, DeckTableType(), text, bytes, report );
}

int64_t DeckToJsonMeasure( const Deck * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DeckTableType(), NULL, 0, allocator );
}

int64_t DeckToJson( const Deck * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, DeckTableType(), buffer, capacity, allocator );
}
