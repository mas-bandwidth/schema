                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    if ( hash != UnitFixedHash )
    {
        ...
        else
        {
            ...
            const int32_t made = TableFixedCompile( parsed, UnitFixedLayout, (int32_t) UnitFixedLayoutBytes, UnitFixedDst, dest, dest_capacity, &compiled_guarded, report );
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableFixedPlanCache * cache, TableReport * report )
{
    ...
    if ( hash != BoundedFixedHash )
    {
        ...
        else
        {
            ...
            int32_t compiled_guarded = 0;
            const int32_t made = TableFixedCompile( parsed, BoundedFixedLayout, (int32_t) BoundedFixedLayoutBytes, BoundedFixedDst, dest, dest_capacity, &compiled_guarded, report );
            if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_malformed : plan_too_large; return -1; }
            record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
            if ( cache != NULL ) { cache->compiles++; }
            if ( storing )
