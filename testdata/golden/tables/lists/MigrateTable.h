...
constexpr int UnitFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );
constexpr int BoundedFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Unit's LEAVES: one entry per run of bytes that lands, at a base the caller
...
constexpr int UnitFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Unit, v );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// Unit's stores. The template — the hash, then zeros — is memcpy'd first,
...
// Bounded's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int BoundedFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    (void) out; (void) src; (void) dst;
    out[n++] = TableFixedEntry{ src + 0u, dst + (uint32_t) __builtin_offsetof( Bounded, items_count ), 8u, 0u, kTableFixedNoGuard, kTableFixedCount, 0, 0 }; // items count
    static_assert( sizeof( Unit ) == 4, "Bounded.items: a flat element's storage IS its wire image" );
    out[n++] = TableFixedEntry{ src + 4u, dst + (uint32_t) __builtin_offsetof( Bounded, items ), 32u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 }; // items, whole
    {
        const uint32_t es = src + 36u;
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Bounded, tag );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// Bounded's stores. The template — the hash, then zeros — is memcpy'd first,
// which is also what zero-fills every byte of declared slack.
...
inline constexpr int64_t UnitFixedBodyBytes = 4;
inline constexpr int64_t UnitFixedRecordBytes = 8 + UnitFixedBodyBytes; // the hash and the body
inline constexpr uint64_t UnitFixedHash = 0x9e3ff698790d29feull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t UnitFixedLayout[] = {
...
inline constexpr int64_t UnitFixedLayoutBytes = (int64_t) sizeof( UnitFixedLayout );
...
inline constexpr TableFixedDst UnitFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Unit
    { (uint32_t) __builtin_offsetof( Unit, v ), 0, 0, 0, 0 }, // v
};

// THE IDENTITY PLAN, coalesced at COMPILE TIME out of 1 leaves.
inline constexpr auto UnitFixedPlan = TableFixedBuildPlan<1>( UnitFixedLeaves );
...
inline constexpr int64_t UnitFixedMeasure( int64_t count )
{
...
inline int64_t UnitFixedSave( const Unit * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) UnitFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = UnitFixedPlan.entries;
    int32_t entry_count = UnitFixedPlan.count;
    ...
    if ( hash != UnitFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, UnitFixedLayout, (int32_t) UnitFixedLayoutBytes, UnitFixedDst, plan, plan_capacity, report );
...
// MeasureBody IS A CONSTEXPR on this form: the body is the same size for
// every value the type can hold (docs/SPEC-TABLES.md §3.4).
inline constexpr int64_t BoundedFixedBodyBytes = 40;
inline constexpr int64_t BoundedFixedRecordBytes = 8 + BoundedFixedBodyBytes; // the hash and the body
inline constexpr uint64_t BoundedFixedHash = 0x2b285a2d28d41c82ull; // fnv1a64 over the layout's bytes

// THE LAYOUT (form 1 called this the vocabulary block): 5 entries, a
// PRE-ORDER walk of the closure in the writer's declared order.
// Every byte is settled by the compiler.
inline constexpr uint8_t BoundedFixedLayout[] = {
...
inline constexpr int64_t BoundedFixedLayoutBytes = (int64_t) sizeof( BoundedFixedLayout );

// MY SIDE of the layout, one row per entry: the storage facts a layout
// entry cannot carry, which is what a plan compiled from another writer's
// layout
// lands values through.
inline constexpr TableFixedDst BoundedFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Bounded
...
inline constexpr auto BoundedFixedPlan = TableFixedBuildPlan<3>( BoundedFixedLeaves );

// A FILE: THE HEADER (docs/SPEC-TABLES.md §3, one rule for all five forms)
// — form byte, seven reserved zero bytes, the LAYOUT HASH at 8, body at 16 —
// then the layout behind its u32 length, then the records to the end of it.
inline constexpr int64_t BoundedFixedMeasure( int64_t count )
{
...
inline int64_t BoundedFixedSave( const Bounded * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) BoundedFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const uint8_t * at = layout + layout_bytes;
    const int64_t rest = bytes - kTableFixedHeaderBytes - 4 - (int64_t) layout_bytes;
    const TableFixedEntry * entries = BoundedFixedPlan.entries;
    int32_t entry_count = BoundedFixedPlan.count;
    ...
    if ( hash != BoundedFixedHash )
    {
        ...
        TableMessageReason why = layout_malformed;
        if ( !TableFixedParseLayout( layout, layout_bytes, parsed, why ) ) { report->refused = true; report->reason = why; return -1; }
        const int32_t made = TableFixedCompile( parsed, BoundedFixedLayout, (int32_t) BoundedFixedLayoutBytes, BoundedFixedDst, plan, plan_capacity, report );
        if ( made < 0 ) { report->refused = true; report->reason = plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        record_bytes = 8 + (int64_t) TableFixedEntryAt( parsed, 0 ).size;
