...
{
    return 1 + 4 + UnitFixedLayoutBytes + count * UnitFixedRecordBytes;
}
...
inline int64_t UnitFixedSave( const Unit * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) UnitFixedLayoutBytes );
    memcpy( buffer + 5, UnitFixedLayout, (size_t) UnitFixedLayoutBytes );
    uint8_t * at = buffer + 5 + UnitFixedLayoutBytes;
...
{
    return 1 + 4 + BoundedFixedLayoutBytes + count * BoundedFixedRecordBytes;
}
...
inline int64_t BoundedFixedSave( const Bounded * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) BoundedFixedLayoutBytes );
    memcpy( buffer + 5, BoundedFixedLayout, (size_t) BoundedFixedLayoutBytes );
    uint8_t * at = buffer + 5 + BoundedFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
