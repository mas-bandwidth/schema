...
constexpr uint64_t UnitFixedHash = 0x9e3ff698790d29feull; // fnv1a64 over the block's bytes
...
constexpr uint8_t UnitFixedVocab[] = {
...
constexpr int64_t UnitFixedVocabBytes = (int64_t) sizeof( UnitFixedVocab );
...
{
    return 1 + 4 + UnitFixedVocabBytes + count * UnitFixedRecordBytes;
}
...
inline int64_t UnitFixedSave( const Unit * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) UnitFixedVocabBytes );
    memcpy( buffer + 5, UnitFixedVocab, (size_t) UnitFixedVocabBytes );
    uint8_t * at = buffer + 5 + UnitFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != UnitFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, UnitFixedVocab, (int32_t) UnitFixedVocabBytes, UnitFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t BoundedFixedHash = 0x2b285a2d28d41c82ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 5 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t BoundedFixedVocab[] = {
...
constexpr int64_t BoundedFixedVocabBytes = (int64_t) sizeof( BoundedFixedVocab );
...
{
    return 1 + 4 + BoundedFixedVocabBytes + count * BoundedFixedRecordBytes;
}
...
inline int64_t BoundedFixedSave( const Bounded * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) BoundedFixedVocabBytes );
    memcpy( buffer + 5, BoundedFixedVocab, (size_t) BoundedFixedVocabBytes );
    uint8_t * at = buffer + 5 + BoundedFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != BoundedFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, BoundedFixedVocab, (int32_t) BoundedFixedVocabBytes, BoundedFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
