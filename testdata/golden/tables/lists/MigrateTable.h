...
inline int64_t UnboundedNodeStorage( uint64_t type_id, int64_t length )
{
...
inline void UnboundedNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    {
        const int64_t storage = UnboundedNodeStorage( type_id, r.size );
...
inline int64_t UnboundedLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = UnboundedNodeStorage( type_id, length );
...
// parameter on a load.
inline const Unit * UnitOpen( const void * bytes, uint64_t length )
{
    return (const Unit *) TableCookOpen( bytes, length, (uint64_t) sizeof( Unit ), (uint64_t) alignof( Unit ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Bounded * BoundedOpen( const void * bytes, uint64_t length )
{
    return (const Bounded *) TableCookOpen( bytes, length, (uint64_t) sizeof( Bounded ), (uint64_t) alignof( Bounded ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Unbounded * UnboundedOpen( const void * bytes, uint64_t length )
{
    return (const Unbounded *) TableCookOpen( bytes, length, (uint64_t) sizeof( Unbounded ), (uint64_t) alignof( Unbounded ) );
}
