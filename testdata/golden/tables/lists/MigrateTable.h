...
inline int64_t UnitMeasureMessage( const Unit & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = UnitMeasureBody( ids, value );
...
inline int64_t UnitSaveMessage( const Unit & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !UnitSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == UnitMeasureMessage( value )
}
...
inline bool UnitLoadMessage( Unit & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    UnitReset( value );
...
inline int64_t BoundedMeasureBody( TableIds & ids, const Bounded & value )
{
    ...
    if ( value.tag != 0 ) { bytes += TableLebBytes( ids.ref( 0x56d7ab194448a4f3ull, 7 ) ) + 1 + 4; } // tag
    return bytes;
}
...
LISTDEMO_TABLE_INLINE bool BoundedSaveBody( TableWriter & w, TableIds & ids, const Bounded & value )
{
    ...
    if ( value.tag != 0 )
    {
        w.putleb( ids.ref( 0x56d7ab194448a4f3ull, 7 ) ); w.put8( 4 ); // tag
...
// the ids live. Every reference is a compile-time SLOT, so this walk does
// no lookup and a save costs what a save costs.
inline int64_t BoundedMeasureMessage( const Bounded & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = BoundedMeasureBody( ids, value );
...
inline int64_t BoundedSaveMessage( const Bounded & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !BoundedSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == BoundedMeasureMessage( value )
}
...
// form on its own and does not guess a table, because a guessed table
// decodes a body under the wrong names in silence.
inline bool BoundedLoadMessage( Bounded & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    BoundedReset( value );
    ...
    TableReader r( buffer + 1, bytes - 1, to, &vocabulary.table );
    r.nested = false; // the ROOT body, the one that may carry a node table
    return BoundedLoadBody( r, value );
}
...
template <typename Ctx>
inline int64_t UnboundedMeasureWire( const Ctx & ctx, const Unbounded & root, TableAllocator allocator, bool message = false )
{
...
template <typename Ctx>
inline int64_t UnboundedSaveWire( const Ctx & ctx, const Unbounded & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t UnboundedMeasureMessage( const Unbounded * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return UnboundedMeasureWire( ctx, *root, allocator, true );
}

inline int64_t UnboundedSaveMessage( const Unbounded * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return UnboundedSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t UnboundedMeasureMessage( const UnboundedBuilder & builder )
{
    if ( builder.region != NULL ) { return UnboundedMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return UnboundedMeasureWire( ctx, *(const Unbounded *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t UnboundedSaveMessage( const UnboundedBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return UnboundedSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return UnboundedSaveWire( ctx, *(const Unbounded *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t UnboundedLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
...
// UnboundedLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Unbounded * UnboundedLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
