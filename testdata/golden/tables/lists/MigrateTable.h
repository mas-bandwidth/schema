                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != UnitFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, UnitFixedLayout, (int32_t) UnitFixedLayoutBytes, UnitFixedDst, plan, plan_capacity, &compiled_guarded, report );
    ...
    {
        UnitReset( values[k] ); // the declared defaults, one prefill
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != BoundedFixedHash )
    {
        ...
        if ( !TableFixedParseLayout( layout, layout_bytes, parsed, why ) ) { report->refused = true; report->reason = why; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, BoundedFixedLayout, (int32_t) BoundedFixedLayoutBytes, BoundedFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? layout_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
    ...
    for ( int64_t k = 0; k < n; ++k )
    {
        BoundedReset( values[k] ); // the declared defaults, one prefill
        if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
        TableFixedRun( entries, entry_count, entry_guarded, at + 8, (uint8_t *) &values[k], report );
        at += record_bytes;
    }
