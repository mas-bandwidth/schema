...
{
    return TableJsonRead( &value, UnitTableType(), text, bytes, report );
}

int64_t UnitToJsonMeasure( const Unit & value )
{
    return TableJsonWrite( &value, UnitTableType(), NULL, 0 );
}

int64_t UnitToJson( const Unit & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, UnitTableType(), buffer, capacity );
}

bool BoundedFromJson( Bounded & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, BoundedTableType(), text, bytes, report );
}

int64_t BoundedToJsonMeasure( const Bounded & value )
{
    return TableJsonWrite( &value, BoundedTableType(), NULL, 0 );
}

int64_t BoundedToJson( const Bounded & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, BoundedTableType(), buffer, capacity );
}
...
bool UnboundedFromJson( UnboundedBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, UnboundedTableType(), text, bytes, report );
}

int64_t UnboundedToJsonMeasure( const Unbounded * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, UnboundedTableType(), NULL, 0, allocator );
}

int64_t UnboundedToJson( const Unbounded * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, UnboundedTableType(), buffer, capacity, allocator );
}
