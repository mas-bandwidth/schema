...
// package streamdemo — the TABLE wire's text form (docs/SPEC-TABLES.md §16).
...
// The guard is not vestigial. Several streamdemo Table.cpp files may be
...
#ifndef STREAMDEMO_SCHEMA_TABLE_JSON
#define STREAMDEMO_SCHEMA_TABLE_JSON
...
#endif // STREAMDEMO_SCHEMA_TABLE_JSON

namespace streamdemo {

bool HeaderFromJson( Header & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, HeaderTableType(), text, bytes, report );
}

int64_t HeaderToJsonMeasure( const Header & value )
{
    return TableJsonWrite( &value, HeaderTableType(), NULL, 0 );
}

int64_t HeaderToJson( const Header & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, HeaderTableType(), buffer, capacity );
}
...
bool ChunkFromJson( ChunkBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, ChunkTableType(), text, bytes, report );
}

int64_t ChunkToJsonMeasure( const Chunk * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ChunkTableType(), NULL, 0, allocator );
}

int64_t ChunkToJson( const Chunk * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, ChunkTableType(), buffer, capacity, allocator );
}
...
bool FeedFromJson( FeedBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    if ( root == NULL ) { if ( report != NULL ) { report->malformed = true; } return false; } // locked, or the root allocation failed
    return TableJsonReadGraph( builder.main, root, FeedTableType(), text, bytes, report );
}

int64_t FeedToJsonMeasure( const Feed * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, FeedTableType(), NULL, 0, allocator );
}

int64_t FeedToJson( const Feed * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, FeedTableType(), buffer, capacity, allocator );
}
