...
constexpr uint64_t HeaderFixedHash = 0xaaf88cbc7745c547ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t HeaderFixedVocab[] = {
...
constexpr int64_t HeaderFixedVocabBytes = (int64_t) sizeof( HeaderFixedVocab );
...
{
    return 1 + 4 + HeaderFixedVocabBytes + count * HeaderFixedRecordBytes;
}
...
inline int64_t HeaderFixedSave( const Header * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) HeaderFixedVocabBytes );
    memcpy( buffer + 5, HeaderFixedVocab, (size_t) HeaderFixedVocabBytes );
    uint8_t * at = buffer + 5 + HeaderFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != HeaderFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, HeaderFixedVocab, (int32_t) HeaderFixedVocabBytes, HeaderFixedDst, plan, plan_capacity, &compiled_guarded, report );
