...
constexpr uint64_t HeaderFixedHash = 0xaaf88cbc7745c547ull; // fnv1a64 over the layout's bytes
...
// Header's STRAIGHT-LINE SCATTER: the identity plan above, unrolled by this
...
inline void HeaderFixedScatter( const uint8_t * TABLE_RESTRICT image, Header & value, TableReport * report )
{
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == HeaderFixedHash );
    ...
    if ( identity )
    {
        ...
        uint8_t image[HeaderFixedBodyBytes];
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( image, at + 8, (size_t) HeaderFixedBodyBytes );
            HeaderFixedScatter( image, values[k], report );
            at += HeaderFixedRecordBytes;
        }
        return n;
    }
