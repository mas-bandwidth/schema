...
constexpr int HeaderFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Header's LEAVES: one entry per run of bytes that lands, at a base the caller
// gives. TableFixedBuildPlan coalesces the adjacent ones, at compile time.
constexpr int HeaderFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    int n = 0;
    (void) out; (void) src; (void) dst;
    out[n++] = TableFixedEntry{ src + 0u, dst + (uint32_t) __builtin_offsetof( Header, name_length ), 16u, dst + (uint32_t) __builtin_offsetof( Header, name ), kTableFixedNoGuard, kTableFixedText, 1, 0 }; // name
    return n;
}

// Header's stores. The template — the hash, then zeros — is memcpy'd first,
...
inline constexpr int64_t HeaderFixedBodyBytes = 20;
inline constexpr int64_t HeaderFixedRecordBytes = 8 + HeaderFixedBodyBytes; // the hash and the body
inline constexpr uint64_t HeaderFixedHash = 0xaaf88cbc7745c547ull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t HeaderFixedLayout[] = {
...
inline constexpr int64_t HeaderFixedLayoutBytes = (int64_t) sizeof( HeaderFixedLayout );
...
inline constexpr TableFixedDst HeaderFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Header
...
inline constexpr auto HeaderFixedPlan = TableFixedBuildPlan<1>( HeaderFixedLeaves );
...
inline constexpr int64_t HeaderFixedMeasure( int64_t count )
{
...
inline int64_t HeaderFixedSave( const Header * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) HeaderFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = HeaderFixedPlan.entries;
    int32_t entry_count = HeaderFixedPlan.count;
    ...
    if ( hash != HeaderFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, HeaderFixedLayout, (int32_t) HeaderFixedLayoutBytes, HeaderFixedDst, plan, plan_capacity, report );
