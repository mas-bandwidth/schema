                    ...
                    else
                    {
                        switch ( arm.id )
                        {
                            ...
                            case 0xfd4d194e1652b207ull: // plain
                            {
                                ...
                                value.carry.type = CarryType::Plain;
                                {
                                    const int64_t width_2 = TableMessageValueBits( 4, arm.packing, arm.value_bits );
                                    uint64_t raw_2 = 0;
                                    if ( width_2 < 0 || !r.get( raw_2, width_2 ) ) { report->malformed = true; return false; }
                                    int64_t decoded_wide_2 = (int64_t) raw_2;
                    ...
                    {
                        {
                            ...
                            if ( elem_arm_ref != 0 ) // the zero reference is a None element in its place
                            {
                                ...
                                switch ( elem_arm_id ) // the arm's NAME hash (docs/SPEC-TABLES.md §5)
                                {
                                    case 0x24ad84ada20208d5ull: // leaf
                                    {
                                        if ( elem_arm_kind != 13 ) { value.entries[(int32_t) i].type = CarryType::None; r.report->kind_mismatch++; break; }
                                    ...
                                    case 0xfd4d194e1652b207ull: // plain
                                    {
                                        if ( elem_arm_kind != 4 ) { value.entries[(int32_t) i].type = CarryType::None; r.report->kind_mismatch++; break; }
...
inline int64_t LeafNodeStorage( uint64_t type_id, int64_t length )
{
...
inline void LeafNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    {
        const int64_t storage = LeafNodeStorage( type_id, r.size );
...
// LeafNodeMessageBody: PASS TWO's half — decode one record's body into the
...
inline int64_t LeafLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = LeafNodeStorage( type_id, length );
...
// depth, summed from the FRAMING: N is framing and not a value, and this
// reads no field. kTableNodeRefused is a wire whose N its L cannot carry.
inline int64_t HolderNodeStorage( uint64_t type_id, int64_t length )
{
    (void) length; // no byte buffer below this root: every node's storage is its type's
    switch ( type_id )
    {
...
inline void HolderNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        const int64_t storage = HolderNodeStorage( type_id, r.size );
...
// HolderNodeMessageBody: PASS TWO's half — decode one record's body into the
// storage it already owns, its map entries carved from its own extent.
...
inline int64_t HolderLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = HolderNodeStorage( type_id, length );
        if ( storage == kTableNodeRefused ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; } // an N the record's framing cannot carry (§2.8, §2.9)
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
    }
    int64_t attribution = ( records + 1 ) * (int64_t) sizeof( TableNodeDirEntry );
...
// depth, summed from the FRAMING: N is framing and not a value, and this
// reads no field. kTableNodeRefused is a wire whose N its L cannot carry.
inline int64_t HandNodeStorage( uint64_t type_id, int64_t length )
{
    (void) length; // no byte buffer below this root: every node's storage is its type's
    switch ( type_id )
    {
...
inline void HandNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    if ( carve.worker == NULL )
    {
        const int64_t storage = HandNodeStorage( type_id, r.size );
...
// HandNodeMessageBody: PASS TWO's half — decode one record's body into the
// storage it already owns, its map entries carved from its own extent.
...
inline int64_t HandLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = HandNodeStorage( type_id, length );
        if ( storage == kTableNodeRefused ) { if ( reason_out != NULL ) { *reason_out = reason; } return -1; } // an N the record's framing cannot carry (§2.8, §2.9)
        if ( storage > 0 ) { data += storage; } // a type id this build cannot name commands none
    }
    int64_t attribution = ( records + 1 ) * (int64_t) sizeof( TableNodeDirEntry );
...
// ChainNodeMessageBody: PASS TWO's half — decode one record's body into the
// storage it already owns, its map entries carved from its own extent.
...
// parameter on a load.
inline const Leaf * LeafOpen( const void * bytes, uint64_t length )
{
    return (const Leaf *) TableCookOpen( bytes, length, (uint64_t) sizeof( Leaf ), (uint64_t) alignof( Leaf ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Holder * HolderOpen( const void * bytes, uint64_t length )
{
    return (const Holder *) TableCookOpen( bytes, length, (uint64_t) sizeof( Holder ), (uint64_t) alignof( Holder ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Hand * HandOpen( const void * bytes, uint64_t length )
{
