...
{
    return TableJsonRead( &value, NodeTableType(), text, bytes, report );
}

int64_t NodeToJsonMeasure( const Node & value )
{
    return TableJsonWrite( &value, NodeTableType(), NULL, 0 );
}

int64_t NodeToJson( const Node & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, NodeTableType(), buffer, capacity );
}
...
bool RingFromJson( RingBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, RingTableType(), text, bytes, report );
}

int64_t RingToJsonMeasure( const Ring * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, RingTableType(), NULL, 0, allocator );
}

int64_t RingToJson( const Ring * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
