...
{
    return TableJsonRead( &value, OnlyTableType(), text, bytes, report );
}

int64_t OnlyToJsonMeasure( const Only & value )
{
    return TableJsonWrite( &value, OnlyTableType(), NULL, 0 );
}

int64_t OnlyToJson( const Only & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, OnlyTableType(), buffer, capacity );
}
...
bool GateFromJson( GateBuilder & builder, const char * text, int64_t bytes, TableReport * report )
{
    ...
    return TableJsonReadGraph( builder.main, root, GateTableType(), text, bytes, report );
}

int64_t GateToJsonMeasure( const Gate * root, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, GateTableType(), NULL, 0, allocator );
}

int64_t GateToJson( const Gate * root, char * buffer, int64_t capacity, TableAllocator allocator )
{
    return TableJsonWriteGraph( root, GateTableType(), buffer, capacity, allocator );
}
