...
constexpr int NodeFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Node's LEAVES: one entry per run of bytes that lands, at a base the caller
...
constexpr int NodeFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Node, v );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// Node's stores. The template — the hash, then zeros — is memcpy'd first,
...
inline constexpr int64_t NodeFixedBodyBytes = 4;
inline constexpr int64_t NodeFixedRecordBytes = 8 + NodeFixedBodyBytes; // the hash and the body
inline constexpr uint64_t NodeFixedHash = 0x79d3bdf48b9c6647ull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t NodeFixedLayout[] = {
...
inline constexpr int64_t NodeFixedLayoutBytes = (int64_t) sizeof( NodeFixedLayout );
...
inline constexpr TableFixedDst NodeFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Node
...
inline int64_t NodeFixedSave( const Node * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) NodeFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != NodeFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, NodeFixedLayout, (int32_t) NodeFixedLayoutBytes, NodeFixedDst, plan, plan_capacity, report );
