...
inline int64_t NodeMeasureMessage( const Node & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = NodeMeasureBody( ids, value );
...
inline int64_t NodeSaveMessage( const Node & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !NodeSaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == NodeMeasureMessage( value )
}
...
inline bool NodeLoadMessage( Node & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    NodeReset( value );
...
template <typename Ctx>
inline int64_t RingMeasureWire( const Ctx & ctx, const Ring & root, TableAllocator allocator, bool message = false )
{
...
template <typename Ctx>
inline int64_t RingSaveWire( const Ctx & ctx, const Ring & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t RingMeasureMessage( const Ring * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RingMeasureWire( ctx, *root, allocator, true );
}

inline int64_t RingSaveMessage( const Ring * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return RingSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t RingMeasureMessage( const RingBuilder & builder )
{
    if ( builder.region != NULL ) { return RingMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return RingMeasureWire( ctx, *(const Ring *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t RingSaveMessage( const RingBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return RingSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return RingSaveWire( ctx, *(const Ring *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t RingLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
...
// RingLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Ring * RingLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
