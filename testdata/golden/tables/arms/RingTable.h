...
constexpr uint64_t NodeFixedHash = 0x79d3bdf48b9c6647ull; // fnv1a64 over the layout's bytes
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == NodeFixedHash );
    ...
    if ( identity )
    {
        ...
        static_assert( sizeof( Node ) >= (size_t) NodeFixedBodyBytes, "Node: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) NodeFixedBodyBytes );
            at += NodeFixedRecordBytes;
        }
        return n;
    }
