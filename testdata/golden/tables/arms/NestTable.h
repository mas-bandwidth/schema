...
inline int64_t TwigMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Twig & value )
{
    ...
    {
        bytes += TableLebBytes( ids.ref( 0x2d4c068f5f889287ull, 12 ) ) + 1;
...
inline bool TwigSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Twig & value )
{
    ...
    {
        w.putleb( ids.ref( 0x2d4c068f5f889287ull, 12 ) ); w.put8( 15 ); // inner
        switch ( value.inner.type )
        {
            case InnerType::Leaf:
            {
...
inline int64_t NestMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Nest & value )
{
    ...
    {
        bytes += TableLebBytes( ids.ref( 0x30e6ed678f5e1bd4ull, 9 ) ) + 1;
        switch ( value.outer.type )
        {
            ...
            {
                int64_t arm_payload = 0;
                const uint64_t arm_ref = ids.ref( 0x2d4c068f5f889287ull, 12 );
                ...
                else
                {
                    switch ( value.outer.inner.type )
                    {
                        ...
                        case InnerType::Leaf:
                        {
                            ...
                            const uint64_t arm_refa = ids.ref( 0x24ad84ada20208d5ull, 13 );
                            {
                        ...
                        {
                            int64_t arm_payloada = 0;
                            const uint64_t arm_refa = ids.ref( 0xfd4d194e1652b207ull, 14 );
...
inline bool NestSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Nest & value )
{
    ...
    {
        w.putleb( ids.ref( 0x30e6ed678f5e1bd4ull, 9 ) ); w.put8( 15 ); // outer
        switch ( value.outer.type )
        {
            case OuterType::Inner:
            {
...
template <typename Ctx>
inline int64_t TwigMeasureWire( const Ctx & ctx, const Twig & root, TableAllocator allocator, bool message = false )
{
...
template <typename Ctx>
inline int64_t TwigSaveWire( const Ctx & ctx, const Twig & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t TwigMeasureMessage( const Twig * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return TwigMeasureWire( ctx, *root, allocator, true );
}

inline int64_t TwigSaveMessage( const Twig * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return TwigSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t TwigMeasureMessage( const TwigBuilder & builder )
{
    if ( builder.region != NULL ) { return TwigMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return TwigMeasureWire( ctx, *(const Twig *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t TwigSaveMessage( const TwigBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return TwigSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return TwigSaveWire( ctx, *(const Twig *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t TwigLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
...
// TwigLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Twig * TwigLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
...
// the connection's announced table and no trailer to write.
template <typename Ctx>
inline int64_t NestMeasureWire( const Ctx & ctx, const Nest & root, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
    int64_t bytes = -1;
...
template <typename Ctx>
inline int64_t NestSaveWire( const Ctx & ctx, const Nest & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t NestMeasureMessage( const Nest * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return NestMeasureWire( ctx, *root, allocator, true );
}

inline int64_t NestSaveMessage( const Nest * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return NestSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t NestMeasureMessage( const NestBuilder & builder )
{
    if ( builder.region != NULL ) { return NestMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return NestMeasureWire( ctx, *(const Nest *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t NestSaveMessage( const NestBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return NestSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return NestSaveWire( ctx, *(const Nest *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t NestLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
    // a FORM BYTE this build does not carry is refused by name (§3, §6.5); a
    // message with no announced table is the connection's refusal (§3.3)
...
// NestLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Nest * NestLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
    // THE FORM BYTE IS READ FIRST, and then the connection's own table:
