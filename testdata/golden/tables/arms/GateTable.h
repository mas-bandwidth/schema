...
inline int64_t OnlyMeasureBody( TableIds & ids, const Only & value )
{
    ...
    if ( value.w != 0 ) { bytes += TableLebBytes( ids.ref( 0xaf63ea4c86020456ull, 10 ) ) + 1 + 4; } // w
    return bytes;
}
...
ARMDEMO_TABLE_INLINE bool OnlySaveBody( TableWriter & w, TableIds & ids, const Only & value )
{
    if ( value.w != 0 )
    {
        w.putleb( ids.ref( 0xaf63ea4c86020456ull, 10 ) ); w.put8( 4 ); // w
...
inline int64_t GateMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Gate & value )
{
    ...
    {
        bytes += TableLebBytes( ids.ref( 0x891da1f305deb858ull, 2 ) ) + 1;
        switch ( value.reach.type )
        {
            ...
            case ReachType::Only:
            {
                ...
                const uint64_t arm_ref = ids.ref( 0x072ddab46af04ab7ull, 14 );
                {
            ...
            {
                int64_t arm_payload = 0;
                const uint64_t arm_ref = ids.ref( 0xfa04f4ef1995407eull, 15 );
                {
...
inline bool GateSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Gate & value )
{
    ...
    {
        w.putleb( ids.ref( 0x891da1f305deb858ull, 2 ) ); w.put8( 15 ); // reach
        switch ( value.reach.type )
        {
            case ReachType::Only:
            {
        ...
        {
    {
        ...
        if ( pointee != NULL )
        {
            ...
            else
            {
                ...
                node.type_slot = 27; // its slot in the unit's vocabulary (§3.3)
        ...
        {
    {
        ...
        if ( blob != NULL )
        {
            ...
            if ( taken )
            {
                ...
                node.type_slot = 19; // its slot in the unit's vocabulary (§3.3)
