...
inline int64_t OnlyMeasureBody( TableIds & ids, const Only & value )
{
    ...
    if ( value.w != 0 ) { bytes += TableLebBytes( ids.ref( 0xaf63ea4c86020456ull, 11 ) ) + 1 + 4; } // w
    return bytes;
}
...
ARMDEMO_TABLE_INLINE bool OnlySaveBody( TableWriter & w, TableIds & ids, const Only & value )
{
    if ( value.w != 0 )
    {
        w.putleb( ids.ref( 0xaf63ea4c86020456ull, 11 ) ); w.put8( 4 ); // w
...
inline int64_t OnlyMeasureMessage( const Only & value )
{
    TableIds ids;
    ids.vocabulary = true;
    const int64_t body = OnlyMeasureBody( ids, value );
...
inline int64_t OnlySaveMessage( const Only & value, uint8_t * buffer, int64_t capacity )
{
    TableWriter w( buffer, capacity );
    TableIds ids;
    ids.vocabulary = true;
    w.put8( kTableWireMessageForm );
    if ( !OnlySaveBody( w, ids, value ) || w.overflow ) { return -1; }
    return w.offset; // == OnlyMeasureMessage( value )
}
...
inline bool OnlyLoadMessage( Only & value, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    OnlyReset( value );
...
inline int64_t GateMeasureBody( const Ctx & ctx, const TableNumbering & numbering, TableIds & ids, const Gate & value )
{
    ...
    {
        bytes += TableLebBytes( ids.ref( 0x891da1f305deb858ull, 4 ) ) + 1;
        switch ( value.reach.type )
        {
            ...
            case ReachType::Only:
            {
                ...
                const uint64_t arm_ref = ids.ref( 0x072ddab46af04ab7ull, 15 );
                {
            ...
            {
                int64_t arm_payload = 0;
                const uint64_t arm_ref = ids.ref( 0xfa04f4ef1995407eull, 16 );
                {
...
inline bool GateSaveBodyFields( const Ctx & ctx, const TableNumbering & numbering, TableWriter & w, TableIds & ids, const Gate & value )
{
    ...
    {
        w.putleb( ids.ref( 0x891da1f305deb858ull, 4 ) ); w.put8( 15 ); // reach
        switch ( value.reach.type )
        {
            case ReachType::Only:
            {
...
template <typename Ctx>
inline int64_t GateMeasureWire( const Ctx & ctx, const Gate & root, TableAllocator allocator, bool message = false )
{
...
template <typename Ctx>
inline int64_t GateSaveWire( const Ctx & ctx, const Gate & root, uint8_t * buffer, int64_t capacity, TableAllocator allocator, bool message = false )
{
    TableNumbering numbering;
    TableNumberingInit( numbering, allocator );
...
inline int64_t GateMeasureMessage( const Gate * root, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return GateMeasureWire( ctx, *root, allocator, true );
}

inline int64_t GateSaveMessage( const Gate * root, uint8_t * buffer, int64_t capacity, TableAllocator allocator = TableDefaultAllocator() )
{
    if ( root == NULL ) { return -1; }
    TableRegionCtx ctx;
    return GateSaveWire( ctx, *root, buffer, capacity, allocator, true );
}

inline int64_t GateMeasureMessage( const GateBuilder & builder )
{
    if ( builder.region != NULL ) { return GateMeasureMessage( builder.AsConst(), builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return GateMeasureWire( ctx, *(const Gate *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), builder.arena.allocator, true );
}

inline int64_t GateSaveMessage( const GateBuilder & builder, uint8_t * buffer, int64_t capacity )
{
    if ( builder.region != NULL ) { return GateSaveMessage( builder.AsConst(), buffer, capacity, builder.arena.allocator ); }
    if ( builder.root_ref.null() ) { return -1; }
    TableArenaCtx ctx = { &builder.arena };
    return GateSaveWire( ctx, *(const Gate *) TableArenaAt( builder.arena, (uint32_t) builder.root_ref.value ), buffer, capacity, builder.arena.allocator, true );
}
...
// attribution is the wire's numbering made resident (§6.3) and a caller may
// release it once Load returns. The answer is their sum.
inline int64_t GateLoadMeasure( const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    TableReport ignored;
...
// GateLoadMessage: decode the tolerant wire into the caller's exact-sized region and
// return the root. LOAD IS A SCAN, and that is the whole of its bound: it
// follows no reference, so there is no depth cap, no visited set and no
// ordering rule on the indices. Partial results are kept, as everywhere on
// this wire — the report says what happened. NULL means the CALLER's buffer
// was wrong.
inline const Gate * GateLoadMessage( uint8_t * region, int64_t region_bytes, const TableVocabulary & vocabulary, const uint8_t * message, int64_t message_bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * out = report != NULL ? report : &ignored;
