...
{
    return 1 + 4 + OnlyFixedLayoutBytes + count * OnlyFixedRecordBytes;
}
...
inline int64_t OnlyFixedSave( const Only * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) OnlyFixedLayoutBytes );
    memcpy( buffer + 5, OnlyFixedLayout, (size_t) OnlyFixedLayoutBytes );
    uint8_t * at = buffer + 5 + OnlyFixedLayoutBytes;
