...
constexpr uint64_t OnlyFixedHash = 0x2078b2b4cf504f6eull; // fnv1a64 over the layout's bytes
...
constexpr TableFixedEntry OnlyFixedPlan[] = {
    { 0u, 0u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0 }, // w
};
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const bool identity = ( hash == OnlyFixedHash );
    ...
    if ( identity )
    {
        ...
        static_assert( sizeof( Only ) >= (size_t) OnlyFixedBodyBytes, "Only: the body lands inside the storage" );
        for ( int64_t k = 0; k < n; ++k )
        {
            if ( TableFixedGet64( at ) != hash ) { report->refused = true; report->reason = no_layout; return -1; }
            memcpy( (void *) &values[k], at + 8, (size_t) OnlyFixedBodyBytes );
            at += OnlyFixedRecordBytes;
        }
        return n;
    }
