...
constexpr int OnlyFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst );

// Only's LEAVES: one entry per run of bytes that lands, at a base the caller
...
constexpr int OnlyFixedLeaves( TableFixedEntry * out, uint32_t src, uint32_t dst )
{
    ...
    (void) out; (void) src; (void) dst;
    {
        ...
        const uint32_t ed = dst + (uint32_t) __builtin_offsetof( Only, w );
        out[n++] = TableFixedEntry{ es, ed, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0 };
    }
    return n;
}

// Only's stores. The template — the hash, then zeros — is memcpy'd first,
...
inline constexpr int64_t OnlyFixedBodyBytes = 4;
inline constexpr int64_t OnlyFixedRecordBytes = 8 + OnlyFixedBodyBytes; // the hash and the body
inline constexpr uint64_t OnlyFixedHash = 0x2078b2b4cf504f6eull; // fnv1a64 over the layout's bytes
...
inline constexpr uint8_t OnlyFixedLayout[] = {
...
inline constexpr int64_t OnlyFixedLayoutBytes = (int64_t) sizeof( OnlyFixedLayout );
...
inline constexpr TableFixedDst OnlyFixedDst[] = {
    { 0, 0, 0, 0, 0 }, // Only
    { (uint32_t) __builtin_offsetof( Only, w ), 0, 0, 0, 0 }, // w
};

// THE IDENTITY PLAN, coalesced at COMPILE TIME out of 1 leaves.
inline constexpr auto OnlyFixedPlan = TableFixedBuildPlan<1>( OnlyFixedLeaves );
...
inline constexpr int64_t OnlyFixedMeasure( int64_t count )
{
...
inline int64_t OnlyFixedSave( const Only * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    for ( int64_t k = 0; k < count; ++k )
    {
        ...
        memset( at + 8, 0, (size_t) OnlyFixedBodyBytes ); // the template's zeros
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    const TableFixedEntry * entries = OnlyFixedPlan.entries;
    int32_t entry_count = OnlyFixedPlan.count;
    ...
    if ( hash != OnlyFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, OnlyFixedLayout, (int32_t) OnlyFixedLayoutBytes, OnlyFixedDst, plan, plan_capacity, report );
