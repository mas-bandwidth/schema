...
inline int64_t GateNodeStorage( uint64_t type_id, int64_t length )
{
    switch ( type_id )
    {
...
inline void GateNodeBody( uint64_t type_id, TableReader & r, const TableNodeMap & nodes, uint8_t * at )
{
    ...
    {
        const int64_t storage = GateNodeStorage( type_id, r.size );
...
// GateNodeMessageBody: PASS TWO's half — decode one record's body into the
...
inline int64_t GateLoadMeasure( const uint8_t * wire_file, int64_t wire_file_bytes, int64_t * attribution_bytes = NULL, TableRefuseReason * reason_out = NULL )
{
    ...
    {
        records++;
        int64_t storage = GateNodeStorage( type_id, length );
...
// parameter on a load.
inline const Only * OnlyOpen( const void * bytes, uint64_t length )
{
    return (const Only *) TableCookOpen( bytes, length, (uint64_t) sizeof( Only ), (uint64_t) alignof( Only ) );
}
...
// over the ATTRIBUTION part beside the data — a person's decision, never a
// parameter on a load.
inline const Gate * GateOpen( const void * bytes, uint64_t length )
{
    return (const Gate *) TableCookOpen( bytes, length, (uint64_t) sizeof( Gate ), (uint64_t) alignof( Gate ) );
}
