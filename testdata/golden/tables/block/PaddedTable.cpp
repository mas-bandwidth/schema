...
{
    return TableJsonRead( &value, PaddedRowTableType(), text, bytes, report );
}

int64_t PaddedRowToJsonMeasure( const PaddedRow & value )
{
    return TableJsonWrite( &value, PaddedRowTableType(), NULL, 0 );
}

int64_t PaddedRowToJson( const PaddedRow & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, PaddedRowTableType(), buffer, capacity );
}

bool PaddedFrameFromJson( PaddedFrame & value, const char * text, int64_t bytes, TableReport * report )
{
    return TableJsonRead( &value, PaddedFrameTableType(), text, bytes, report );
}

int64_t PaddedFrameToJsonMeasure( const PaddedFrame & value )
{
    return TableJsonWrite( &value, PaddedFrameTableType(), NULL, 0 );
}

int64_t PaddedFrameToJson( const PaddedFrame & value, char * buffer, int64_t capacity )
{
    return TableJsonWrite( &value, PaddedFrameTableType(), buffer, capacity );
}
