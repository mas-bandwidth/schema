...
constexpr uint64_t PaddedRowFixedHash = 0x9683695abec5c762ull; // fnv1a64 over the block's bytes
...
constexpr uint8_t PaddedRowFixedVocab[] = {
...
constexpr int64_t PaddedRowFixedVocabBytes = (int64_t) sizeof( PaddedRowFixedVocab );
...
{
    return 1 + 4 + PaddedRowFixedVocabBytes + count * PaddedRowFixedRecordBytes;
}
...
inline int64_t PaddedRowFixedSave( const PaddedRow * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) PaddedRowFixedVocabBytes );
    memcpy( buffer + 5, PaddedRowFixedVocab, (size_t) PaddedRowFixedVocabBytes );
    uint8_t * at = buffer + 5 + PaddedRowFixedVocabBytes;
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != PaddedRowFixedHash )
    {
        ...
        const int32_t made = TableFixedCompile( parsed, PaddedRowFixedVocab, (int32_t) PaddedRowFixedVocabBytes, PaddedRowFixedDst, plan, plan_capacity, &compiled_guarded, report );
...
constexpr uint64_t PaddedFrameFixedHash = 0x7b08b877a4f6e867ull; // fnv1a64 over the block's bytes

// THE VOCABULARY BLOCK: 23 entries, a PRE-ORDER walk of the closure in the
// writer's declared order. Every byte is settled by the compiler.
constexpr uint8_t PaddedFrameFixedVocab[] = {
...
constexpr int64_t PaddedFrameFixedVocabBytes = (int64_t) sizeof( PaddedFrameFixedVocab );
...
{
    return 1 + 4 + PaddedFrameFixedVocabBytes + count * PaddedFrameFixedRecordBytes;
}
...
inline int64_t PaddedFrameFixedSave( const PaddedFrame * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) PaddedFrameFixedVocabBytes );
    memcpy( buffer + 5, PaddedFrameFixedVocab, (size_t) PaddedFrameFixedVocabBytes );
    uint8_t * at = buffer + 5 + PaddedFrameFixedVocabBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
                            ...
                            TableFixedEntry * plan, int32_t plan_capacity, TableReport * report )
{
    ...
    if ( hash != PaddedFrameFixedHash )
    {
        ...
        if ( !TableFixedVocabRead( block, block_bytes, parsed ) ) { report->refused = true; report->reason = block_malformed; return -1; }
        int32_t compiled_guarded = 0;
        const int32_t made = TableFixedCompile( parsed, PaddedFrameFixedVocab, (int32_t) PaddedFrameFixedVocabBytes, PaddedFrameFixedDst, plan, plan_capacity, &compiled_guarded, report );
        if ( made < 0 ) { report->refused = true; report->reason = ( made == -2 ) ? block_malformed : plan_too_large; return -1; }
        entries = plan;
        entry_count = made;
        entry_guarded = compiled_guarded;
