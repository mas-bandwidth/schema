...
inline int64_t PaddedRowMeasureMessageBody( int64_t at, const PaddedRow & value );
inline bool PaddedRowSaveMessageBody( TableBitWriter & w, const PaddedRow & value );
inline bool PaddedRowLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, PaddedRow & value );
inline int64_t PaddedFrameMeasureMessageBody( int64_t at, const PaddedFrame & value );
inline bool PaddedFrameSaveMessageBody( TableBitWriter & w, const PaddedFrame & value );
inline bool PaddedFrameLoadMessageBody( TableBitReader & r, const TableVocabulary & vocabulary, TableReport * report, int64_t index_bits, PaddedFrame & value );
...
inline int64_t PaddedRowMeasureBody( TableIds & ids, const PaddedRow & value )
{
    ...
    if ( value.tag != 0 ) { bytes += TableLebBytes( ids.ref_at( 25, 0x56d7ab194448a4f3ull ) ) + 1 + 1; } // tag
    if ( value.value != 0.0 ) { bytes += TableLebBytes( ids.ref_at( 36, 0x7ce4fd9430e80ceaull ) ) + 1 + 8; } // value
    if ( value.flag != false ) { bytes += TableLebBytes( ids.ref_at( 73, 0xd5f2d079088c0b17ull ) ) + 1 + 1; } // flag
    if ( value.id != 0 ) { bytes += TableLebBytes( ids.ref_at( 0, 0x08b72e07b55c3ac0ull ) ) + 1 + 4; } // id
    if ( value.label_length < 0 || value.label_length > 15 ) { return -1; } // storage invariant
    if ( value.label_length > 0 ) { bytes += TableLebBytes( ids.ref_at( 16, 0x39f7fcec8fcb623dull ) ) + 1 + TableLebBytes( (uint64_t) ( value.label_length ) ) + ( value.label_length ); } // label
    {
        bool all_default_slots = true;
        for ( int32_t i = 0; i < 4; i++ ) { if ( value.slots[i] != 0 ) { all_default_slots = false; break; } }
        if ( !all_default_slots )
        {
            const uint64_t ref_slots = ids.ref_at( 79, 0xe68c2e6bb1ee5646ull );
    ...
    }
    {
        ...
        const uint64_t ref_teams = ids.ref_at( 58, 0xbaaeb048a5a8fa6dull );
    ...
    {
        bytes += TableLebBytes( ids.ref_at( 35, 0x77976c7416517c63ull ) ) + 1 + 4; // counter
    }
    return bytes;
}
...
BLOCKDEMO_TABLE_INLINE bool PaddedRowSaveBody( TableWriter & w, TableIds & ids, const PaddedRow & value )
{
    if ( value.tag != 0 )
    {
        w.header( ids.ref_at( 25, 0x56d7ab194448a4f3ull ), 6 ); // tag
        w.put8( uint8_t( value.tag ) );
    }
    if ( value.value != 0.0 )
    {
        w.header( ids.ref_at( 36, 0x7ce4fd9430e80ceaull ), 11 ); // value
        w.put64( table_double_to_bits( value.value ) );
    }
    if ( value.flag != false )
    {
        w.header( ids.ref_at( 73, 0xd5f2d079088c0b17ull ), 1 ); // flag
        w.put8( value.flag ? 1 : 0 );
    }
    if ( value.id != 0 )
    {
        w.header( ids.ref_at( 0, 0x08b72e07b55c3ac0ull ), 8 ); // id
    ...
    {
        w.header( ids.ref_at( 16, 0x39f7fcec8fcb623dull ), 12 ); // label
    ...
    {
        w.header( ids.ref_at( 35, 0x77976c7416517c63ull ), 4 ); // counter
...
inline int64_t PaddedRowMeasureMessageBody( int64_t at, const PaddedRow & value )
{
    ...
    {
        bool rides_slots = false;
        for ( int32_t i = 0; i < 4; i++ ) { if ( value.slots[i] != 0 ) { rides_slots = true; break; } }
        if ( rides_slots )
        {
            bits += kTableMessageRefBitsHere;
            bits += (int64_t) ( 4 ) * 16;
...
inline bool PaddedRowSaveMessageBody( TableBitWriter & w, const PaddedRow & value )
{
    if ( value.tag != 0 )
    {
        ...
        w.put( (uint64_t) ( value.tag ), 8 );
    }
    if ( value.value != 0.0 )
    {
        w.put( 6, kTableMessageRefBitsHere );
        w.put( table_double_to_bits( value.value ), 64 );
    }
    if ( value.flag != false )
    {
        w.put( 7, kTableMessageRefBitsHere );
        w.put( value.flag ? 1 : 0, 1 );
    }
    if ( value.id != 0 )
    {
    ...
    }
    {
        ...
        if ( rides_slots )
        {
            ...
            for ( int32_t i = 0; i < 4; i++ )
            {
                w.put( (uint64_t) ( value.slots[i] ), 16 );
            }
        }
    }
    {
        ...
        if ( pairs_teams > 0 )
        {
            ...
            for ( int32_t i = 0; i < 4; i++ )
            {
                ...
                w.put( (uint64_t) ( value.teams.slots[i] ), 8 );
            }
        }
    }
    if ( value.counter_present ) // ?int32
    {
        ...
        w.put( (uint64_t) ( value.counter ), 32 );
    }
    ...
    for ( ;; )
    {
        ...
        switch ( entry.id )
        {
            ...
            case 0x7ce4fd9430e80ceaull: // value
            {
                ...
                { uint64_t raw = 0; if ( !r.get( raw, 64 ) ) { report->malformed = true; return false; } value.value = table_bits_to_double( raw ); }
                break;
            }
            case 0xd5f2d079088c0b17ull: // flag
            {
                ...
                { uint64_t raw = 0; if ( !r.get( raw, 1 ) ) { report->malformed = true; return false; } value.flag = raw != 0; }
                break;
            }
            case 0x08b72e07b55c3ac0ull: // id
            {
            ...
            case 0xbaaeb048a5a8fa6dull: // teams
            {
                ...
                // A RANGE that moved is not one: the shapes differ and the entry
                // carries the SENDER's, so the field decodes and clamps (§4).
                if ( entry.kind != 16 || entry.elem_kind != 6 )
                {
                    report->kind_mismatch++;
                    if ( !TableMessageSkip( r, vocabulary, index_bits, entry ) ) { report->malformed = true; return false; }
                    break;
                }
                {
                    ...
                    for ( uint64_t p = 0; p < n; p++ )
                    {
                        ...
                        const bool in_bounds = slot >= 0 && slot < 4;
                        uint8_t scratch = 0;
                        {
                            const int64_t width_2 = entry.elem_value_bits;
                            uint64_t raw_2 = 0;
                        ...
                        if ( in_bounds ) { value.teams.slots[slot] = scratch; }
                    }
                }
                break;
            }
...
inline int64_t PaddedRowMeasureMessages( const PaddedRow * values, int64_t count, TableReport * report )
{
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = PaddedRowMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
...
inline int64_t PaddedRowSaveMessages( const PaddedRow * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !PaddedRowSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == PaddedRowMeasureMessages( values, count, report )
}
...
inline bool PaddedRowLoadMessages( PaddedRow * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    ...
    *count = 0;
    ...
    {
        if ( !PaddedRowLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
...
{
    int64_t bytes = 1; // the ZERO REFERENCE that ends the body
    if ( value.marker != 0 ) { bytes += TableLebBytes( ids.ref_at( 81, 0xeddcb72b15486e77ull ) ) + 1 + 1; } // marker
    if ( value.stamp != 0 ) { bytes += TableLebBytes( ids.ref_at( 83, 0xee7ba9ad45c64144ull ) ) + 1 + 8; } // stamp
    if ( value.rows_count < 0 || value.rows_count > 64 ) { return -1; } // storage invariant
    if ( value.rows_count > 0 )
    {
        const uint64_t ref_rows = ids.ref_at( 48, 0xa3a7061ff10a8138ull );
        int64_t body_rows = 0;
    ...
    if ( value.blob_length > 0 )
    {
        ...
        bytes += TableLebBytes( ids.ref_at( 64, 0xc573b39bc29148caull ) ) + 1 + TableLebBytes( (uint64_t) ( body_blob ) ) + ( body_blob ); // blob
    }
    return bytes;
}
...
BLOCKDEMO_TABLE_INLINE bool PaddedFrameSaveBody( TableWriter & w, TableIds & ids, const PaddedFrame & value )
{
    if ( value.marker != 0 )
    {
        w.header( ids.ref_at( 81, 0xeddcb72b15486e77ull ), 6 ); // marker
        w.put8( uint8_t( value.marker ) );
    }
    if ( value.stamp != 0 )
    {
        w.header( ids.ref_at( 83, 0xee7ba9ad45c64144ull ), 9 ); // stamp
    ...
    {
        const int64_t body_blob = 1 + TableLebBytes( (uint64_t) value.blob_length ) + value.blob_length;
        w.header( ids.ref_at( 64, 0xc573b39bc29148caull ), 14 ); // blob
    ...
    for ( ;; )
    {
        ...
        switch ( field_id )
        {
            ...
            case 0xa3a7061ff10a8138ull: // rows
            {
                ...
                if ( body_len >= 2 )
                {
                    const int32_t previous_count = value.rows_count;
                    uint8_t elem_kind = r.get8();
                    uint64_t count = 0;
                    const bool counted_ok = r.getleb( count );
                    // A DAMAGED COUNT stops the elements and nothing else: the field
                    ...
                    for ( int32_t tail = value.rows_count; tail < previous_count; tail++ ) {
...
inline int64_t PaddedFrameMeasureMessageBody( int64_t at, const PaddedFrame & value )
{
    ...
    if ( value.rows_count > 0 )
    {
        ...
        {
            const int64_t elem = PaddedRowMeasureMessageBody( at + bits, value.rows[i] );
            if ( elem < 0 ) { return -1; }
            bits += elem;
        }
    }
    ...
    if ( value.blob_length > 0 )
    {
        ...
        bits += 4;
        bits += TableAlignBits( at + bits );
        bits += (int64_t) value.blob_length * 8;
    }
    bits += kTableMessageRefBitsHere; // the ZERO REFERENCE that ends the body
    (void) at;
    return bits;
...
inline bool PaddedFrameSaveMessageBody( TableBitWriter & w, const PaddedFrame & value )
{
    if ( value.marker != 0 )
    {
        ...
        w.put( (uint64_t) ( value.marker ), 8 );
    }
    if ( value.stamp != 0 )
    {
        w.put( 2, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.stamp ), 64 );
    }
    if ( value.rows_count < 0 || value.rows_count > 64 ) { return false; } // storage invariant
    if ( value.rows_count > 0 )
    {
        w.put( 3, kTableMessageRefBitsHere );
        w.put( (uint64_t) ( value.rows_count ) - 0, 7 );
        for ( int32_t i = 0; i < value.rows_count; i++ )
        {
            if ( !PaddedRowSaveMessageBody( w, value.rows[i] ) ) { return false; }
        }
    }
    if ( value.blob_length < 0 || value.blob_length > 12 ) { return false; } // storage invariant
    if ( value.blob_length > 0 )
    {
        ...
        w.put( (uint64_t) value.blob_length, 4 );
        w.align(); // a string or a bytes ALIGNS before its bytes
        w.putbytes( (const uint8_t *) value.blob, value.blob_length );
    }
    w.put( 0, kTableMessageRefBitsHere ); // the ZERO REFERENCE that ends the body
    return !w.overflow;
}
            ...
            case 0xa3a7061ff10a8138ull: // rows
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < walk; i++ )
                    {
                        const bool in_bounds = (int32_t) i < kept;
                        PaddedRow scratch;
                        PaddedRowReset( scratch );
                        if ( !PaddedRowLoadMessageBody( r, vocabulary, report, index_bits, ( in_bounds ? value.rows[i] : scratch ) ) ) { return false; }
                    }
                    value.rows_count = kept;
                }
                break;
            }
            case 0xc573b39bc29148caull: // blob
            {
                ...
                }
                {
                    ...
                    for ( uint64_t i = 0; i < n; i++ )
                    {
                        ...
                        if ( (int32_t) i < kept ) { value.blob[i] = (uint8_t) by; }
                    }
                    value.blob_length = kept;
                }
                break;
            }
            default:
...
// refusal is learned at MEASURE time, before a buffer is allocated, and a
// caller with more bodies calls again.
inline int64_t PaddedFrameMeasureMessages( const PaddedFrame * values, int64_t count, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    int64_t bits = 8; // the body count, a ranged integer over [1, 256]
    for ( int64_t i = 0; i < count; i++ )
    {
        const int64_t body = PaddedFrameMeasureMessageBody( bits, values[i] );
        if ( body < 0 ) { return -1; }
        bits += body;
    }
    return 1 + ( bits + 7 ) / 8; // the form byte, then the stream padded to a byte
...
inline int64_t PaddedFrameSaveMessages( const PaddedFrame * values, int64_t count, uint8_t * buffer, int64_t capacity, TableReport * report )
{
    if ( values == NULL || count < 1 ) { return -1; }
    if ( count > kTableMessageBatchMax ) { TableMessageRefuseBatch( report ); return -1; }
    TableMessageBatch batch;
    ...
    for ( int64_t i = 0; i < count; i++ )
    {
        if ( !PaddedFrameSaveMessageBody( batch.w, values[i] ) ) { return -1; }
        batch.written++;
    }
    return TableMessageBatchEnd( batch ); // == PaddedFrameMeasureMessages( values, count, report )
}
...
// BODIES 1 TO k - 1: count says k - 1, one malformed counts, and the storage
// after it is not a body.
inline bool PaddedFrameLoadMessages( PaddedFrame * values, int64_t * count, const TableVocabulary & vocabulary, const uint8_t * buffer, int64_t bytes, TableReport * report )
{
    TableReport ignored;
    TableReport * to = report != NULL ? report : &ignored;
    if ( values == NULL || count == NULL ) { to->malformed = true; return false; }
    ...
    *count = 0;
    ...
    for ( int64_t i = 0; i < bodies; i++ )
    {
        if ( !PaddedFrameLoadMessageBody( br.r, vocabulary, to, 0, values[i] ) ) { *count = i; return false; } // a fixed root numbers no node: no index width
        br.remaining--;
    }
    *count = bodies;
    return TableMessageBatchClose( br );
...
{
    (void) b; (void) value;
    TableFixedPut8( b + 0, (uint8_t) value.tag );
...
constexpr TableFixedDst PaddedRowFixedDst[] = {
    ...
    { (uint32_t) __builtin_offsetof( PaddedRow, flag ), 0, 0, 0, 0 }, // flag
    { (uint32_t) __builtin_offsetof( PaddedRow, id ), 0, 0, 0, 0 }, // id
    { (uint32_t) __builtin_offsetof( PaddedRow, label_length ), 0, (uint32_t) __builtin_offsetof( PaddedRow, label ), 0, 1 }, // label
    { (uint32_t) __builtin_offsetof( PaddedRow, slots ), (uint32_t) sizeof( uint16_t ), 0, 0, 0 }, // slots
    { 0, 0, 0, 0, 0 }, // element
    { (uint32_t) __builtin_offsetof( PaddedRow, teams ), (uint32_t) sizeof( uint8_t ), 0, 0, 0 }, // teams
    { 0, 0, 0, 0, 0 }, // Team
    { 0, 0, 0, 0, 0 }, // Red
    { 0, 0, 0, 0, 0 }, // Blue
    { 0, 0, 0, 0, 0 }, // Green
...
constexpr TableFixedEntry PaddedRowFixedPlan[] = {
    { 0u, 0u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 1u, 8u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 9u, 16u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 10u, 20u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 14u, 40u, 15u, 24u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 33u, 44u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 45u, 60u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 46u, 56u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
};
constexpr int32_t PaddedRowFixedPlanCount = 8;
constexpr int32_t PaddedRowFixedPlanGuarded = 8;
...
inline int64_t PaddedRowFixedSave( const PaddedRow * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = PaddedRowFixedMeasure( count );
    ...
    TableFixedPut64( buffer + kTableFixedHashAt, PaddedRowFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) PaddedRowFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, PaddedRowFixedLayout, (size_t) PaddedRowFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + PaddedRowFixedLayoutBytes;
...
constexpr TableFixedDst PaddedFrameFixedDst[] = {
    ...
    { (uint32_t) __builtin_offsetof( PaddedFrame, rows ), (uint32_t) sizeof( PaddedRow ), (uint32_t) __builtin_offsetof( PaddedFrame, rows_count ), 1, 0 }, // rows
    { 0, 0, 0, 0, 0 }, // element
    { (uint32_t) __builtin_offsetof( PaddedRow, tag ), 0, 0, 0, 0 }, // tag
    { (uint32_t) __builtin_offsetof( PaddedRow, value ), 0, 0, 0, 0 }, // value
    { (uint32_t) __builtin_offsetof( PaddedRow, flag ), 0, 0, 0, 0 }, // flag
...
constexpr TableFixedEntry PaddedFrameFixedPlan[] = {
    { 0u, 0u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // marker
    { 1u, 8u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // stamp
    { 9u, 4112u, 64u, 0u, kTableFixedNoGuard, kTableFixedCount, 0, 0, 0, 0, 1 }, // rows count
    { 13u, 16u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 14u, 24u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 22u, 32u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 23u, 36u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 27u, 56u, 15u, 40u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 46u, 60u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 58u, 76u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 59u, 72u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 63u, 80u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 64u, 88u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 72u, 96u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 73u, 100u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 77u, 120u, 15u, 104u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 96u, 124u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 108u, 140u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 109u, 136u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 113u, 144u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 114u, 152u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 122u, 160u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 123u, 164u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 127u, 184u, 15u, 168u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 146u, 188u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 158u, 204u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 159u, 200u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 163u, 208u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 164u, 216u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 172u, 224u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 173u, 228u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 177u, 248u, 15u, 232u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 196u, 252u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 208u, 268u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 209u, 264u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 213u, 272u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 214u, 280u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 222u, 288u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 223u, 292u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 227u, 312u, 15u, 296u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 246u, 316u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 258u, 332u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 259u, 328u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 263u, 336u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 264u, 344u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 272u, 352u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 273u, 356u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 277u, 376u, 15u, 360u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 296u, 380u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 308u, 396u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 309u, 392u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 313u, 400u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 314u, 408u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 322u, 416u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 323u, 420u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 327u, 440u, 15u, 424u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 346u, 444u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 358u, 460u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 359u, 456u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 363u, 464u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 364u, 472u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 372u, 480u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 373u, 484u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 377u, 504u, 15u, 488u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 396u, 508u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 408u, 524u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 409u, 520u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 413u, 528u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 414u, 536u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 422u, 544u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 423u, 548u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 427u, 568u, 15u, 552u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 446u, 572u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 458u, 588u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 459u, 584u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 463u, 592u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 464u, 600u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 472u, 608u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 473u, 612u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 477u, 632u, 15u, 616u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 496u, 636u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 508u, 652u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 509u, 648u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 513u, 656u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 514u, 664u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 522u, 672u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 523u, 676u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 527u, 696u, 15u, 680u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 546u, 700u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 558u, 716u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 559u, 712u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 563u, 720u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 564u, 728u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 572u, 736u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 573u, 740u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 577u, 760u, 15u, 744u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 596u, 764u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 608u, 780u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 609u, 776u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 613u, 784u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 614u, 792u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 622u, 800u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 623u, 804u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 627u, 824u, 15u, 808u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 646u, 828u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 658u, 844u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 659u, 840u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 663u, 848u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 664u, 856u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 672u, 864u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 673u, 868u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 677u, 888u, 15u, 872u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 696u, 892u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    {
, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2577u, 3320u, 15u, 3304u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2596u, 3324u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2608u, 3340u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2609u, 3336u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2613u, 3344u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2614u, 3352u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2622u, 3360u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2623u, 3364u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2627u, 3384u, 15u, 3368u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2646u, 3388u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2658u, 3404u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2659u, 3400u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2663u, 3408u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2664u, 3416u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2672u, 3424u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2673u, 3428u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2677u, 3448u, 15u, 3432u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2696u, 3452u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2708u, 3468u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2709u, 3464u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2713u, 3472u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2714u, 3480u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2722u, 3488u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2723u, 3492u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2727u, 3512u, 15u, 3496u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2746u, 3516u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2758u, 3532u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2759u, 3528u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2763u, 3536u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2764u, 3544u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2772u, 3552u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2773u, 3556u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2777u, 3576u, 15u, 3560u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2796u, 3580u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2808u, 3596u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2809u, 3592u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2813u, 3600u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2814u, 3608u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2822u, 3616u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2823u, 3620u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2827u, 3640u, 15u, 3624u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2846u, 3644u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2858u, 3660u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2859u, 3656u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2863u, 3664u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2864u, 3672u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2872u, 3680u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2873u, 3684u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2877u, 3704u, 15u, 3688u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2896u, 3708u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2908u, 3724u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2909u, 3720u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2913u, 3728u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2914u, 3736u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2922u, 3744u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2923u, 3748u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2927u, 3768u, 15u, 3752u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2946u, 3772u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 2958u, 3788u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 2959u, 3784u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 2963u, 3792u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 2964u, 3800u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 2972u, 3808u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 2973u, 3812u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 2977u, 3832u, 15u, 3816u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 2996u, 3836u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 3008u, 3852u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 3009u, 3848u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 3013u, 3856u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 3014u, 3864u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 3022u, 3872u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 3023u, 3876u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 3027u, 3896u, 15u, 3880u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 3046u, 3900u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 3058u, 3916u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 3059u, 3912u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 3063u, 3920u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 3064u, 3928u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 3072u, 3936u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 3073u, 3940u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 3077u, 3960u, 15u, 3944u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 3096u, 3964u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 3108u, 3980u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 3109u, 3976u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 3113u, 3984u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 3114u, 3992u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 3122u, 4000u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 3123u, 4004u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 3127u, 4024u, 15u, 4008u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 3146u, 4028u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 3158u, 4044u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 3159u, 4040u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 3163u, 4048u, 1u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // tag
    { 3164u, 4056u, 8u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // value
    { 3172u, 4064u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // flag
    { 3173u, 4068u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // id
    { 3177u, 4088u, 15u, 4072u, kTableFixedNoGuard, kTableFixedText, 0, 1, 0, 0, 1 }, // label
    { 3196u, 4092u, 12u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // slots, whole
    { 3208u, 4108u, 1u, 0u, kTableFixedNoGuard, kTableFixedBool, 0, 0, 0, 0, 1 }, // counter present
    { 3209u, 4104u, 4u, 0u, kTableFixedNoGuard, kTableFixedCopy, 0, 0, 0, 0, 1 }, // counter
    { 3213u, 4128u, 12u, 4116u, kTableFixedNoGuard, kTableFixedText, 0, 3, 0, 0, 1 }, // blob
};
constexpr int32_t PaddedFrameFixedPlanCount = 516;
constexpr int32_t PaddedFrameFixedPlanGuarded = 516;
...
inline int64_t PaddedFrameFixedSave( const PaddedFrame * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    const int64_t need = PaddedFrameFixedMeasure( count );
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    memset( buffer, 0, (size_t) kTableFixedHeaderBytes ); // the seven reserved bytes, and the rest of the header
    buffer[0] = kTableFixedForm;
    TableFixedPut64( buffer + kTableFixedHashAt, PaddedFrameFixedHash );
    TableFixedPut32( buffer + kTableFixedHeaderBytes, (uint32_t) PaddedFrameFixedLayoutBytes );
    memcpy( buffer + kTableFixedHeaderBytes + 4, PaddedFrameFixedLayout, (size_t) PaddedFrameFixedLayoutBytes );
    uint8_t * at = buffer + kTableFixedHeaderBytes + 4 + PaddedFrameFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
...
template <typename... Args>
inline void PaddedRowSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedRow is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PaddedRowLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedRow is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PaddedRowSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedRow is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PaddedFrameLoadRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedFrame is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}

template <typename... Args>
inline void PaddedFrameMeasureRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedFrame is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PaddedFrameSaveRetain( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedFrame is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PaddedFrameLoadRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedFrame is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
...
template <typename... Args>
inline void PaddedFrameSaveRetainMessages( Args &&... )
{
    static_assert( sizeof...( Args ) == (size_t) -1,
        "PaddedFrame is a FIXED-class root (docs/SPEC-TABLES.md §6.1): it is a value with no region and no node directory, so retain-unknown has no anchor for a path's first step. LoadRetain, MeasureRetain and SaveRetain, and the message form's LoadRetainMessages and SaveRetainMessages beside them (§3.3), are refused by name on a fixed-class root (§6.6). Retention is a REGION round trip: load a variable-class root, or save without it." );
}
