...
{
    return 1 + 4 + PaddedRowFixedLayoutBytes + count * PaddedRowFixedRecordBytes;
}
...
inline int64_t PaddedRowFixedSave( const PaddedRow * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    TableFixedPut32( buffer + 1, (uint32_t) PaddedRowFixedLayoutBytes );
    memcpy( buffer + 5, PaddedRowFixedLayout, (size_t) PaddedRowFixedLayoutBytes );
    uint8_t * at = buffer + 5 + PaddedRowFixedLayoutBytes;
...
{
    return 1 + 4 + PaddedFrameFixedLayoutBytes + count * PaddedFrameFixedRecordBytes;
}
...
inline int64_t PaddedFrameFixedSave( const PaddedFrame * values, int64_t count, uint8_t * buffer, int64_t capacity )
{
    ...
    if ( count < 0 || buffer == NULL || capacity < need ) { return -1; }
    buffer[0] = kTableFixedForm;
    TableFixedPut32( buffer + 1, (uint32_t) PaddedFrameFixedLayoutBytes );
    memcpy( buffer + 5, PaddedFrameFixedLayout, (size_t) PaddedFrameFixedLayoutBytes );
    uint8_t * at = buffer + 5 + PaddedFrameFixedLayoutBytes;
    for ( int64_t k = 0; k < count; ++k )
    {
