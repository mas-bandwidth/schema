...
// package wide — THE UNIT REGISTRY (docs/SPEC-TABLES.md §8.3): one entry point
...
// THE TYPES NO TABLE REACHES (docs/SPEC-TABLES.md §8.2). A `type` a table
// reaches keeps its descriptor in the table header, where a game that loads
// tables links it. These are the rest, and they live here: a view describes,
// it does not add a wire, so nothing below gains a codec, a text form or a
// baseline row from having one.
void NarrowFifteenReset( NarrowFifteen & value );
const TableTypeInfo * NarrowFifteenTableType();
void WideFourReset( WideFour & value );
const TableTypeInfo * WideFourTableType();
void WideInteropReset( WideInterop & value );
const TableTypeInfo * WideInteropTableType();
void WideSevenReset( WideSeven & value );
