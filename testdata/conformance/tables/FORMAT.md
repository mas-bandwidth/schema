...
The forms this data describes are SPEC-TABLES.md's: §3 the table wire, §4 the
...
The five counters of SPEC-TABLES.md §4, `<malformed>` spelled `true` or `false`.
...
The SPEC-TABLES.md §16 text of the instance, and EXACTLY that: no trailing
...
The canonical node dump of a cooked file (SPEC-TABLES.md §7.5): the walk every
