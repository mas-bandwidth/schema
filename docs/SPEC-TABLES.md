...
- **`elem=` is the ELEMENT's SIZE** here, the storage one slot takes, where
  §18.1's baseline records the element's KIND under the same spelling. An
  `arm` line carries it the same way a `field` line does.
