...
message loads, and writes retained file roots (§6.6). Dart, Rust, Java and JavaScript still write
the earlier form in this tree; that code is dead and is being removed port by
port. Elixir writes no table wire: it carries the block and cook read halves
only. [ROADMAP.md](../ROADMAP.md) records coverage by construct and form.
