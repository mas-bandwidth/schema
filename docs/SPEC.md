...
`WireLaw` `2` applies construction defaults whenever a packet union payload
is selected, including repeated tags during decoding (§4.8). This changes
the resulting storage values while preserving the packet field encodings.
...
- **Zero initialization is the rule, in every generated language, with an
  ...
  Arrays, `wstring(N)`, composite and union fields take no field-level
  override. Composite fields and array elements follow their type's
  construction rule recursively; union fields start at `None` and wide-text
  buffers start empty.
  **A counted array's COUNT is the one stated exception to zero
  ...
  elements follow their element type's construction rule, including its
  declared defaults; the count determines which elements are used. The §5 zero form does not carry the birth
  ...
  **On the packet wire, defaults initialize storage without eliding fields.**
  ...
  including §5's explicit zero rule for untaken branches.
...
specified defaults** — untaken branches retain this explicit zero rule.
Ordinary construction and union selection apply §4.2's declared defaults.
*Fully initializes* concerns the received fields and used prefixes:
...
(the selected arm is freshly constructed, §4.8). A reused ordinary object
keeps prior tail data. A selected union payload instead starts with its
construction values, so unread capacity retains those values.
...
- **JavaScript:** one ES module per schema file, cross-file `import`s derived
  ...
  in declaration order. `InitX` restores a type's construction values in
  existing storage; `ZeroX` applies the §5 zero form. **Generated JS never imports the serialize runtime** —
...
**Selected-arm construction (§4.8).** `examples/ArmDefaults.schema` has a
seven-bit independent packet oracle, byte `0x51`: arm `first`, zero used
entries, marker `5`. Every language's packet suite checks the written byte
and bit count, then reads it twice. Targets with backing capacity poison
the selected payload before each read and require declared defaults in both
unused entries; managed targets also check that existing storage is retained.
JavaScript exercises both runtime and flat readers, and a second bulk-bearing
case exercises C#'s ordinary reader beside its batch reader. Elixir checks
fresh immutable construction. `make packet-arm-defaults-negative-controls`
restores zero-only selection independently in each changed decoder path and
requires the selected-payload default assertion to fail; a compile failure
...
| --- | --- | --- | --- |
| C | `-ffp-contract=off` on every compile of generated or conformance sources. clang's DEFAULT for C contracts, so an unflagged build is a WRONG-BYTES build, not a slow one. | `make/c.mk`, `make/checks/*.mk`, `bench/run.sh`, `bench/tables/*/leg` — every `$(CC)` line | `TestNoContractionFlagOnEveryCompile` sweeps `Makefile` and every makefile, shell script and bench leg under `make/` and `bench/` RECURSIVELY, and reds on any `$(CC)`/`$(CXX)`/`cc`/`c++`/`clang`/`gcc` invocation that names an output or a translation unit — whatever `-std=` it spells, or none — unless the flag is on the line or in a flags variable the line reads; the C leg asserts the discrimination property directly |
...
| Go | The Go spec permits fusing `x*y+z` unless an explicit conversion intervenes, and arm64 takes the permission. Every emitted product is wrapped: `float32(normalizedValue*steps) + 0.5`. | `internal/codegen/golang/functions.go`, `internal/codegen/golang/flat.go`, `internal/codegen/gotable/message.go` (the message codec quantizes at RUNTIME in Go) | `TestNoContractionInEmittedCode` locates the fold inside each NAMED emitter function (`emitWriteCompressedFold`, `flatCompressedPiece`, `tableMessageWriteScalar`, `tableMessageReadScalar`) and requires the `float32(` wrap on THAT expression. A file-wide token grep is vacuous here: these files carry unrelated `float32(` conversions, so it stays green after the wrap is deleted from the product |
| Rust | rustc NEVER contracts; a fused product requires an explicit `f32::mul_add`. Nothing to turn off; the rule is that no emitter may reach for it. | `internal/codegen/rust`, `internal/codegen/rusttable` | `TestNoFusedMultiplyAddIntrinsics` greps the emitters for `mul_add`, and labels a hit by the emitter DIRECTORY's language |
| Java | JLS 15.18.2 FORBIDS contraction, and since JDK 17 (JEP 306) strict semantics are the only semantics. Java is the one inlining leg that therefore needs NO wrap: it emits the bare `n * 200.0f + 0.5f` on purpose. A fused product would require `Math.fma`. | `--release 17` on the Java leg; `internal/codegen/java/functions.go` | `TestNoFusedMultiplyAddIntrinsics` greps for `Math.fma` — the absence IS the enforcement, so the grep is the whole gate. `Math.fma` is also a JavaScript spelling, so the gate names the language of the directory the hit is in, not of the needle |
...
| JavaScript | The language has no FMA and no `float` type. The discipline is the opposite problem — doubles must be narrowed — so EVERY intermediate is wrapped in `Math.fround`. | `internal/codegen/js/flat.go` | `TestNoContractionInEmittedCode` requires the product and the `+ 0.5` inside `Math.fround(` at the fold `emitWriteCompressedFloat` emits |
| Dart | The VM and AOT compilers do not fuse; `double` is the only float type, so the float32 discipline is an explicit `_fround` helper on every intermediate. | `internal/codegen/dart/functions.go` | `TestNoContractionInEmittedCode` requires the `_fround` wrap on the fold `emitWriteCompressedFloat` emits; the Dart leg runs the pinned probe compiled AOT on arm64 |
| Elixir | BEAM floats are doubles and the VM does not fuse. float32 rounding is emulated by the Veltkamp/Dekker `fr/1` helper, applied to every intermediate. | `internal/codegen/elixir/functions.go` | `TestNoContractionInEmittedCode` requires `fr(` on both statements of the fold `emitSupportHelpers` emits |
