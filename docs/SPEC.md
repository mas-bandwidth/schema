...
- **Zero initialization is the rule, in every generated language, with an
  ...
  **A counted array's COUNT is the one stated exception to zero
  ...
  as a scalar's is. C++ carries all three on both wires. C, Go, Rust, C#, Java and JavaScript carry them for
  packet-only fields; a default reachable from any table is refused by name
  in C, Go, Rust, C#, Java and JavaScript. Every other backend refuses a unit that declares one, naming the
  follow-on.
