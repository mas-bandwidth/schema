...
| `forged_ordinal_both_plans` (§5.8 row 12) | `VOLD_/VNEW_enum_append` as they stand — `Tier { Bronze, Silver, Gold }` → `+ Platinum` — plus TWO forged files: `hostile_enum_append.bin`, `old_enum_append.bin` with `r0.tier` set to `4` — an ordinal PAST the writer's three variants and a name the reader DOES have, **THE BAND CASE**: the raw sits BETWEEN the writer's count and the reader's larger extent, so a pass held to the READER's extent lands `Platinum`, a name the writer never had, and counts NOTHING (§5.9 #27's correction) | **the SAME bytes read TWICE**: once by the NEW build (the COMPILED plan, selected through the lineage) and once by the OLD build (the IDENTITY plan, its own hash) | both reads `n == 1`; both land `tier == None` and never `Platinum` — **the bound is the WRITER's three, carried by the plan** (the `ordinal` entry's remap table length word), which is the only number that can see the band; the IDENTITY plan is UNAFFECTED, its ordinals riding inside plain copy runs where the writer IS the reader and the two extents coincide; both leave `seq == 9` | **`clamped == 1` on BOTH plans, the same number on both — on the COMPILED plan the count is the plan-carried writer-bounds half's, read back out of the RECORD because storage holds the REMAPPED ordinal** — the BOUNDS pass's count, once per field (§5.4), and the equality of the two is the row's whole proof. `== 1` and not `>= 1`: a leg that counts in the `ordinal` op AS WELL as in the bounds pass lands `2` and is wrong (§5.4's "counts twice"). Every other counter `0` | NEW-READS-OLD (the compiled column) and a second column on the OLD build (the identity one). LOCK-* are `enum_append`'s; OLD-REFUSES-NEW `—` |
