...
| `float_nan_bounded` (§4.6, §5.4) | A NEW PAIR is not needed: `VOLD_/VNEW_range_widen`'s shape with a FLOAT — `fixed table NanBounded { lead uint32 = 1, v float32 = 0 | 0, 1, trail uint32 = 2 }` — plus two forged files, `hostile_float_nan_bounded.bin` with `r0.v`'s four bytes OVERWRITTEN with `0x7FC00000` (a QUIET NaN) and `hostile_float_snan_bounded.bin` with `0x7F800001` (a SIGNALLING one). A bounded float is the same field a COMPRESSED float rides as in this form, so the two are one row | each forged file read TWICE: by the identity plan (this build's own layout) and by a compiled plan (the same bytes through a lineage entry), the pair `forged_ordinal_both_plans` uses | both reads land `v` as the bits of `min` — `0x00000000` for a `min` of `+0.0` — and NEVER a NaN; `lead` and `trail` stand. **The quiet NaN and the signalling one land the SAME**: the pass compares, it does not inspect bits | **`clamped == 1` EXACTLY, on BOTH plans and for BOTH NaNs** — a NaN is out of range exactly as `-inf` is, and `-inf` in the same field is the control that must count the same `1`. `== 1` and not `>= 1`. Every other counter `0`, `malformed` false, `refused` false | NEW-READS-OLD (compiled) and a second column on the identity plan. LOCK-* are `—`: no definition changed |
| `float_negzero_bounded` (§4.6, §5.4) | `float_nan_bounded`'s table, one forged file `hostile_float_negzero_bounded.bin` with `r0.v`'s four bytes OVERWRITTEN with `0x80000000` — `-0.0` against a `min` of `+0.0` | the same two reads | both reads land `v` with the bits `0x80000000` EXACTLY — **`-0.0` as written, sign bit and all**, never normalised to `+0.0` and never replaced by `min`: `-0.0 >= +0.0` is TRUE, so the value is IN RANGE | **`clamped == 0`** — the negative control for the row above, and the reason the low test is `!(v >= min)` and not `v != v || v < min`. Every counter `0`, `malformed` false, `refused` false | as above |
