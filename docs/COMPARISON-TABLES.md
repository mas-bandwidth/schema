...
| Union | `union` with implicit `None`; arms by name hash, so add, remove, reorder freely (§2.6, §5); table arms are [#258](https://github.com/mas-bandwidth/schema/issues/258) | `union` of tables; structs and strings experimental; vectors of unions C++ only (FB-schema) | `oneof`; `Any` for open typing (PB-proto3) |
