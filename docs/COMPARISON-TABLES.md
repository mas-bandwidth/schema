...
| **A read report instead of pass or fail.** `unknown`, `kind_mismatch`, `widened`, `clamped`, `duplicate`, `malformed`; never fatal on data from another build; a damaged level stops only itself. | §4 | Tools surface it, games set policy, and a corrupt sub-table does not kill the file. |
...
| Change a type | `kind_mismatch`: skipped, counted, never misdecoded (§4) — except a kind that merely GREW, an integer to a wider one of the same signedness or `f32` to `f64`, which decodes exactly and counts `widened`; same-kind respellings are the silent class the baseline refuses (§4.1) | only at identical width, "with careful handling" (FB-evolution) | a fixed list of compatible pairs; anything else misdecodes silently (PB-proto3) |
...
| Read report | `unknown`, `kind_mismatch`, `widened`, `clamped`, `duplicate`, `malformed`; nothing fatal from another build (§4) | verifier pass or fail (FB-cpp) | success or failure, plus the unknown set (PB-message) |
