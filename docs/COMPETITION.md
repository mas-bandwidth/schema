...
- **Unknown fields through a rewrite.** Protocol Buffers keeps them without
  ...
  the never-clobber rule [s23]. The opt-in beside it is built in the C++
  reference and in no port: a caller hands `LoadRetain` a bounded side buffer
  it declares and owns, and `SaveRetain` writes the unknown FIELDS back into
  the body they came from, the `unknown` class alone, never a variant, an arm,
  a keyed slot or a node record, each gap counted `retain_lost` (SPEC-TABLES
  §6.6, [#525](https://github.com/mas-bandwidth/schema/issues/525)).
...
- s9. Defaults for `string(N)`, `bytes(N)` and `flags` fields are built ([#396](https://github.com/mas-bandwidth/schema/issues/396)): SPEC §4.2's `Default` production admits a quoted string and a brace list of flags variant names, a field at its declared default elides on the table wire and an absent field reads as it (SPEC-TABLES §4), and the baseline refuses a change to one (SPEC-TABLES §18.2). The C++ reference and the tool carry all three on both wires. The other eight backends carry them on the packet wire and refuse defaults in a table closure. 🔶 because port support is partial and a composite default is the adopt-later row of #396.
- s9b. An arm IS a field line, so an arm's type is any type a field's is — a scalar with its bounds, a compressed float, a string, a bounded array, an enum, a `flags` mask, a declared `type`, a `table` inside a table closure, a pointer, another union — and an arm may carry no payload at all, which rides under kind `32` on the table wire (SPEC §4.8, SPEC-TABLES §2.6, §3). Payload-free packet arms ride as the tag alone in all nine targets. What an arm may not take is a default, a `?`, a `json`, an `[E]T`, an `if` guard, a `map` or an unbounded `[]T`, each refused by name. 🔶 because general arms on the table wire are the reference's and the tool's: the eight ports answer ABSENT on the message-class cases ([#392](https://github.com/mas-bandwidth/schema/issues/392), SPEC-TABLES §15).
...
- s13. The FIELD-attribute vocabulary is closed and an unknown attribute is a compile error (SPEC §4.2). The TAG, at every line kind, is the open namespace beside it: "one user-chosen identifier, in its own namespace", any identifier legal there, parsed, carried through the IR and into the descriptors, with the unit registry's `ViewType` listing a declaration's tags (SPEC §4.2, SPEC-TABLES §8.3). 🔶 because a tag changes no generated wire code and is read out of the descriptors by the project's own tooling, where the three ✅ columns let an annotation reach a runtime API. It is more than Avro's 🔶 [58], which requires only that an implementation tolerate the attribute.
...
- s23. The DEFAULT is a drop, by decision, with the read report counting what a rewrite would lose under the never-clobber rule ([VERSIONING.md](VERSIONING.md)). Retain-unknown is the opt-in beside it, a REGION round trip whose buffer the caller sizes and owns, covering unknown FIELDS and no other class and no other counter, with `retained` and `retain_lost` on the same report struct (SPEC-TABLES §6.6). 🔶 because the C++ reference and the compiler's own engine carry it and no port does, and because it is held by those two engines' own gates and the wire fuzzer's retention leg rather than by the conformance manifest. Three things are owed before the cell is green: the conformance manifest's `retain` surface, the MESSAGE form's `LoadRetain`, and the eight ports ([#525](https://github.com/mas-bandwidth/schema/issues/525)).
...
- s34. By-value depth is fixed by the schema and the pointer graph is flat, so a long chain is not a recursion (SPEC-TABLES §3.1). The amplification bound on an untrusted read is stated and tested ([#466](https://github.com/mas-bandwidth/schema/issues/466), SPEC-TABLES §6.5): a node is numbered once however many pointers name it, and a hostile record is two wire bytes, so the region a read can be made to materialize is the schema's largest record per two wire bytes, times only the records the framing carries; `LoadMeasure` is exact and the caller refuses the answer it will not pay.
