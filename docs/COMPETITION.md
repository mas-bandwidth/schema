...
- s9. Defaults for `string(N)`, `bytes(N)` and `flags` fields are built ([#396](https://github.com/mas-bandwidth/schema/issues/396)): SPEC §4.2's `Default` production admits a quoted string and a brace list of flags variant names, a field at its declared default elides on the table wire and an absent field reads as it (SPEC-TABLES §4), and the baseline refuses a change to one (SPEC-TABLES §18.2). The C++ reference and the tool carry all three on both wires. C, Go, Rust and C# carry them on the packet wire and refuse defaults in a table closure; the other four backends refuse a unit that declares one, naming the follow-on. 🔶 because port support is partial and a composite default is the adopt-later row of #396.
