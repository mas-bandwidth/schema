...
line from the same expression (`MS` / `MSI` / `MSE` / `MSTR` / `MWCPY`), so the manifest cannot drift from the
corpus: a changed value changes both or neither. A record's values are prefixed `r<i>.`, nested fields are
