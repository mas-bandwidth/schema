...
prefill included. `malformed` is the residue and not a bucket a named rule falls into. **The one refusal
...
of its own LINEAGE and its own files, and a table with no form is a root of neither. **THE NAMING IS A WARNING
AND NOT A REFUSAL** — the table GENERATES, keeping form `1`, declared and derived alike, and the only record-size
bound that fails a compile is `--fixed-record-limit`, off by default (§6). **The LOCK is the one place this
number refuses anything**, and it refuses a CROSSING rather than a size: a table already shipping form `3` whose
bounds grow past the ceiling is a change of FORM and not a version, so `schema lock` names it
(`internal/lockfile.diffCeiling`); a table born past the ceiling crosses nothing and is refused by nobody.
