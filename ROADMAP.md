...
Rust, JavaScript and Elixir still generate fixed-class tables in the form that
preceded the id-table wire, which the current specification does not describe
and the C++ tool does not open; that code is dead and is being removed port by
port, and their cells above are green only where the current block and cook
readers, descriptors and build-version check earn them. Dart and Java are cut:
each emits no table wire at all today, only its block and cook readers and their
build-version check. #514 to #518 carry each of those columns forward to the
id-table wire when there is time and support for it.
