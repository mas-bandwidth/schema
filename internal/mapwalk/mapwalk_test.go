...
import (
	...
	"github.com/mas-bandwidth/schema/v2/internal/mapwalk"
	"github.com/mas-bandwidth/schema/v2/ir"
)

// TestMapWalkHalfRidesTheMapBearingUnits is the MAP-WALK GATE
// (docs/SPEC-TABLES.md §2.8, §16). Three claims, one per unit of the C++ table
// corpus, and the MAP-FREE SET IS DERIVED FROM THE SCHEMAS:
//
//  1. a MAP-BEARING unit's every generated .cpp carries the map half;
//  2. a MAP-FREE unit's carries none of it, which is §2.2's zero-cost
//     property holding for the text form;
//  3. the half is ONE half, the same bytes wherever it rides.
//
// `make tables-json-map-walk` points SCHEMA_MAP_WALK_DIR at the generated
...
func TestMapWalkHalfRidesTheMapBearingUnits(t *testing.T) {
	dir := os.Getenv("SCHEMA_MAP_WALK_DIR")
	// SCHEMA_MAP_WALK_UNITS narrows the scan to named generated directories,
	...
	for name := range strings.SplitSeq(os.Getenv("SCHEMA_MAP_WALK_UNITS"), ",") {
		if name != "" {
			only[name] = true
		}
	}
	...
	for _, unit := range units {
		...
		maps := mapwalk.Bearing(load(t, unit.Source))
		if len(maps) > 0 {
			bearingUnits++
		} else {
			freeUnits++
		...
		for _, source := range sources {
			...
			half, carried := mapwalk.Half(string(text))
			switch {
			case len(maps) == 0 && carried:
				freeFiles++
				t.Errorf("MAP-WALK GATE FAILED: the map half reached the map-free unit %s.\n"+
					"No table in %s's closure carries a map", source, unit.Source)
			case len(maps) > 0 && !carried:
				bearingFiles++
				t.Errorf("MAP-WALK GATE FAILED: no map half in %s.\n"+
					"%s declares %s", source, unit.Source, strings.Join(maps, ", "))
			case carried:
				bearingFiles++
				if firstHalf == "" {
					firstName, firstHalf = source, half
				} else if half != firstHalf {
					t.Errorf("MAP-WALK GATE FAILED: the map half in %s is not the map half in %s.\n%s",
	...
	if len(only) == 0 {
		if bearingUnits == 0 || freeUnits == 0 {
			...
			t.Fatalf("the derived map-free set is %d map-bearing and %d map-free units. "+
				"The corpus carries both kinds, and an instrument that reports one is not deriving anything",
				bearingUnits, freeUnits)
		}
		if dir != "" && firstHalf == "" {
			t.Fatal("MAP-WALK GATE FAILED: not one generated .cpp of the corpus carries a map half")
		}
	}
	...
	summary := "tables map-walk gate: one map half, byte-identical in " + strconv.Itoa(bearingFiles) +
		" .cpp files across " + strconv.Itoa(bearingUnits) + " map-bearing units, and in none of the " +
		strconv.Itoa(freeFiles) + " .cpp files of the " + strconv.Itoa(freeUnits) +
		" map-free ones, the set derived from the schemas\n"
	if path := os.Getenv("SCHEMA_MAP_WALK_SUMMARY"); path != "" {
