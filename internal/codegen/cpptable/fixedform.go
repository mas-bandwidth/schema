...
// Nothing rides under it in a record — it is a layout kind and not a wire kind.
// It is ir's, beside the rest of the kind vocabulary, because it is wire law
// and not this backend's.
...
// wire law and not this backend's arithmetic, and the compiler holds a fixed
// table's record to its two size bounds out of the same functions, so the
// reference's bytes and the compiler's verdict cannot drift apart.
const (
...
// fixedWalk builds the layout and, beside it, the DESTINATION row per entry:
// the storage facts a layout entry cannot carry, which is what a plan compiled
// from another writer's layout lands values through.
...
func (g *tableGen) fixedWalkRoot(st *ir.Struct) *fixedWalk {
	w := &fixedWalk{}
	w.push(fixedLayoutEntry{id: ir.TableWireId(st.WireName()), kind: ir.TableKindTable, size: fixedTypeBytes(st), children: len(st.Fields), note: st.Name}, dstRow("0", "", "", 0, 0))
	for _, f := range st.Fields {
...
func (g *tableGen) fixedWalkPayload(w *fixedWalk, owner string, f *ir.Field, id uint64, size int64) {
	...
	case f.Type.Kind == ir.TBytes:
		...
		w.push(fixedLayoutEntry{id: 0, kind: ir.TableKindU8, size: 1, children: 0, note: "u8"}, dstRow("0", "", "", 0, 0))
...
func (g *tableGen) fixedWalkElement(w *fixedWalk, f *ir.Field, id uint64, note, dst string) {
	...
	if f.Type.Kind == ir.TNamed {
		switch r := f.Type.Ref.(type) {
		case *ir.Struct:
			w.push(fixedLayoutEntry{id: id, kind: ir.TableKindTable, size: size, children: len(r.Fields), note: note}, dstRow(dst, "", "", 0, 0))
	...
	w.push(fixedLayoutEntry{id: id, kind: ir.TableWireScalarKind(f), size: size, children: 0, note: note}, dstRow(dst, "", "", 0, 0))
}

func (g *tableGen) fixedWalkEnum(w *fixedWalk, e *ir.Enum, id uint64, note, dst string) {
	w.push(fixedLayoutEntry{id: id, kind: ir.TableKindEnum, size: int64(e.StorageBits / 8), children: len(e.Variants), note: note}, dstRow(dst, "", "", 0, 0))
	for i := range e.Variants {
		w.push(fixedLayoutEntry{id: ir.TableWireId(e.VariantWireName(i)), kind: ir.TableKindNoPayload, size: 0, children: 0, note: e.Variants[i]}, dstRow("0", "", "", 0, 0))
	}
}
...
// format, so a layout format change is a new form byte (docs/SPEC-TABLES.md
// §3.4).
...
func (g *tableGen) fixedRoots(members []*ir.Struct) []*ir.Struct {
	...
	for _, st := range members {
		...
		// §3.4'S RECORD CEILING, WHICH IS THE WIRE'S. A reader holds an
		// untrusted peer's layout to 65536 bytes of record, so a writer past it
		// would produce bytes no conforming reader decodes. The table keeps
		// form 1, and the compiler names it (ir.TableFixedRecordBounds).
...
func (g *tableGen) emitFixedRoot(st *ir.Struct) {
	...
	g.pf("inline constexpr uint64_t %sFixedHash = 0x%016xull; // fnv1a64 over the layout's bytes\n\n", st.Name, hash)
	...
	g.pf("inline constexpr uint8_t %sFixedLayout[] = {\n", st.Name)
	g.emitByteArray(layout)
	g.pf("};\n")
	g.pf("inline constexpr int64_t %sFixedLayoutBytes = (int64_t) sizeof( %sFixedLayout );\n\n", st.Name, st.Name)
	...
	g.pf("    return 1 + 4 + %sFixedLayoutBytes + count * %sFixedRecordBytes;\n}\n\n", st.Name, st.Name)
	...
	g.pf("    TableFixedPut32( buffer + 1, (uint32_t) %sFixedLayoutBytes );\n", st.Name)
	g.pf("    memcpy( buffer + 5, %sFixedLayout, (size_t) %sFixedLayoutBytes );\n", st.Name, st.Name)
	g.pf("    uint8_t * at = buffer + 5 + %sFixedLayoutBytes;\n", st.Name)
	...
	g.pf("    const uint32_t layout_bytes = TableFixedGet32( data + 1 );\n")
	g.pf("    if ( (int64_t) layout_bytes + 5 > bytes ) { report->refused = true; report->reason = layout_malformed; return -1; }\n")
	g.pf("    const uint8_t * layout = data + 5;\n")
	g.pf("    const uint64_t hash = TableFixedHashOf( layout, layout_bytes );\n")
	g.pf("    const uint8_t * at = layout + layout_bytes;\n")
	g.pf("    const int64_t rest = bytes - 5 - (int64_t) layout_bytes;\n")
	...
	g.pf("        const int32_t made = TableFixedCompile( parsed, %sFixedLayout, (int32_t) %sFixedLayoutBytes, %sFixedDst, plan, plan_capacity, report );\n", st.Name, st.Name, st.Name)
