...
func (g *tableGen) emitBlockSurface(bl *ir.BlockLayout) {
	...
	g.pf("        uint64_t build_version = 0; // generated: the unit's build version (docs/SPEC-TABLES.md §20)\n")
	...
	g.pf("// BlockOpen checks once and points, and this is the WHOLE check (§19.2), in\n")
	g.pf("// §7's own order: the magic read bytewise, the BYTE ORDER the prologue\n")
	g.pf("// carries against this build's own, the BUILD VERSION against this build's\n")
	g.pf("// own, each array's pitch, its COUNT against the declared maximum, its\n")
	g.pf("// offset_of and its extent inside the block, the used extent against the\n")
	g.pf("// bytes the caller passed, and LAST the base's alignment, the one clause\n")
	g.pf("// that reads nothing out of the block.\n")
	...
	g.pf("// points at nothing, and NAMES the first failing clause in the caller's\n")
	g.pf("// TableRefuseReason, the same enum a cook's Open fills (§7): not_a_cook,\n")
	g.pf("// foreign_order, wrong_build_version, bad_layout for a pitch, a count, an\n")
	g.pf("// offset or an extent, truncated where the used extent the arrays derive\n")
	g.pf("// runs past the caller's bytes, and unaligned_base. The reason is written\n")
	g.pf("// on the refusal path only.\n")
	g.pf("//\n")
...
func (g *tableGen) emitBlockLayoutAsserts(bl *ir.BlockLayout) {
	...
	g.pf("static_assert( offsetof( %sBlock::Projection, magic ) == 0, \"the block prologue's magic sits at offset 0 (docs/SPEC-TABLES.md §19.1)\" );\n", name)
	g.pf("static_assert( offsetof( %sBlock::Projection, build_version ) == 8, \"the block prologue's build_version sits at offset 8 (docs/SPEC-TABLES.md §19.1, §20)\" );\n", name)
	g.pf("static_assert( offsetof( %sBlock::Projection, byte_order ) == 16, \"the block prologue's byte_order sits at offset 16 (docs/SPEC-TABLES.md §19.1)\" );\n", name)
	for _, fl := range bl.Projection.Fields {
		g.pf("static_assert( offsetof( %sBlock::Projection, %s ) == %d, \"%s's projection field %s moved: the C# side asserts %d (docs/SPEC-TABLES.md §19.3)\" );\n",
	...
	for _, a := range bl.Arrays {
		...
		field := ir.GoExportName(a.Field.Name)
		g.pf("static_assert( %sBlock::%sStride == (int64_t) sizeof( %s ), \"%s's pitch is its element's sizeof, always (docs/SPEC-TABLES.md §2.7)\" );\n",
...
func (g *tableGen) emitBlockRowAsserts(name string, seen map[string]bool) {
	...
	g.noteRef(name)
	g.pf("static_assert( sizeof( %s ) == %d, \"block row %s's sizeof moved: the C# side asserts %d for the same declaration (docs/SPEC-TABLES.md §19.3)\" );\n",
		name, ml.Size, name, ml.Size)
	g.pf("static_assert( alignof( %s ) == %d, \"block row %s's alignof moved (docs/SPEC-TABLES.md §19.3)\" );\n", name, ml.Align, name)
	for _, fl := range ml.Fields {
		g.pf("static_assert( offsetof( %s, %s ) == %d, \"block row %s's field %s moved: the C# side asserts %d (docs/SPEC-TABLES.md §19.3)\" );\n",
			name, fl.Field.Name, fl.Offset, name, fl.Field.Name, fl.Offset)
	}
	for _, fl := range ml.Fields {
...
func (g *tableGen) emitBlockFillPath(bl *ir.BlockLayout) {
	...
	for _, a := range bl.Arrays {
		...
		g.pf("// (docs/SPEC-TABLES.md §19.1). Iterating steps at the pitch the INSTANCE gives.\n")
...
func (g *tableGen) emitBlockOpenBody(bl *ir.BlockLayout) {
	...
	g.pf("    if ( bytes < %d ) { return TableCookRefuse( reason, truncated ) != NULL; }\n", bl.Projection.Size)
	g.pf("    // EVERY PROLOGUE WORD IS READ BYTEWISE, because the base's alignment is\n")
	g.pf("    // the LAST clause (§7, §19.2) and nothing before it may assume one\n")
	...
	g.pf("        // not a block at all: a cook's magic lands here too\n")
	g.pf("        return TableCookRefuse( reason, magic == table_block_byteswap64( TableBlockMagic ) ? foreign_order : not_a_cook ) != NULL;\n")
	g.pf("    }\n")
	g.pf("    // a byte-order word that contradicts its own magic describes no block in\n")
	g.pf("    // either order, so it shares the magic's own value (§7.1, §19.1)\n")
	g.pf("    if ( table_block_read64( raw + 16 ) != TableBlockByteOrder ) { return TableCookRefuse( reason, not_a_cook ) != NULL; }\n")
	g.pf("    if ( table_block_read64( raw + 8 ) != BuildVersion ) { return TableCookRefuse( reason, wrong_build_version ) != NULL; }\n")
	g.pf("    int64_t used = %d;\n", bl.Projection.Size)
	for _, a := range bl.Arrays {
		g.pf("    {\n")
		...
		g.pf("        const uint64_t offset_of = table_block_read64( triple );\n")
		g.pf("        const uint64_t count = table_block_read32( triple + 8 );\n")
		g.pf("        const uint64_t stride = table_block_read32( triple + 12 );\n")
		g.pf("        if ( stride != (uint64_t) sizeof( %s ) ) { return TableCookRefuse( reason, bad_layout ) != NULL; }\n", a.ElemName)
		...
		g.pf("        if ( count > (uint64_t) %sBlock::%sMax ) { return TableCookRefuse( reason, bad_layout ) != NULL; }\n", name, ir.GoExportName(a.Field.Name))
		g.pf("        if ( offset_of < %d || ( offset_of %% %d ) != 0 ) { return TableCookRefuse( reason, bad_layout ) != NULL; }\n", bl.Projection.Size, blockStartAlign(a))
		g.pf("        if ( offset_of > (uint64_t) bytes ) { return TableCookRefuse( reason, bad_layout ) != NULL; }\n")
		g.pf("        const uint64_t rows = count * stride; // both bounded above: this cannot carry\n")
		g.pf("        if ( rows > (uint64_t) bytes - offset_of ) { return TableCookRefuse( reason, bad_layout ) != NULL; }\n")
	...
	g.pf("    // An extent the caller's bytes do not cover is truncated (§19.2).\n")
	g.pf("    const int64_t padding = ( %d - ( used %% %d ) ) %% %d;\n", ir.BlockAlign, ir.BlockAlign, ir.BlockAlign)
	g.pf("    if ( padding > bytes - used ) { return TableCookRefuse( reason, truncated ) != NULL; }\n")
	g.pf("    used += padding;\n")
