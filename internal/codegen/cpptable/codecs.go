...
func (g *tableGen) emitTableRead(st *ir.Struct) {
	...
	g.pf("    // have met: nothing is decoded and one event is counted (§3). THE READ\n")
	g.pf("    // ITSELF ANSWERS THAT, and it is asked AFTER the walk instead of before\n")
	g.pf("    // it: a body that returned at its own zero reference left the cursor on\n")
	g.pf("    // the byte after that reference, and a root body's terminator is its\n")
	g.pf("    // LAST byte. Every field leaves the cursor exactly where the framing\n")
	g.pf("    // walk's skip would — a length-framed field ends at its own L, whatever\n")
	g.pf("    // it did or refused to do inside it — so the two walks find the same\n")
	g.pf("    // terminator in the same place, and one walk answers what two used to.\n")
	g.pf("    //\n")
	g.pf("    // NOTHING IS DECODED ON THE DAMAGED PATH, and that is the whole of it:\n")
	g.pf("    // the value goes back to its declared defaults and the report goes back\n")
	g.pf("    // to what the caller handed in, so a body that ends early counts no\n")
	g.pf("    // unknown, no kind mismatch and no clamp for the fields it happened to\n")
	g.pf("    // carry before the end — exactly as when the framing walk refused this\n")
	g.pf("    // file before the reader had seen one byte of it.\n")
	g.pf("    const TableReport before = *to;\n")
	...
	g.pf("        // THE READING WALK STOPS ON RULES THE FRAMING WALK HAS NO OPINION\n")
	g.pf("        // ABOUT — a reserved id in a file body is the one that matters (§3.1,\n")
	g.pf("        // §3.3) — so a body that stopped is still asked the framing question,\n")
	g.pf("        // and a body whose framing ends early is damage whatever else was\n")
	g.pf("        // wrong with it. This is the walk that used to run first, on the one\n")
	g.pf("        // path where it is not spent for nothing.\n")
	g.pf("        if ( TableBodyEndsEarly( buffer + 1, body_bytes, table ) )\n        {\n")
	g.pf("            %sReset( value );\n", st.Name)
	g.pf("            *to = before;\n")
	g.pf("            to->malformed = true;\n")
	g.pf("            return TableOpenDamaged;\n        }\n")
	g.pf("        return TableOpenBodyStopped;\n    }\n")
	g.pf("    if ( r.offset != body_bytes )\n    {\n")
	g.pf("        %sReset( value );\n", st.Name)
	g.pf("        *to = before;\n")
	g.pf("        to->malformed = true;\n")
	g.pf("        return TableOpenDamaged;\n    }\n")
