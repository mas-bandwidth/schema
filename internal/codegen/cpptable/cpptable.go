...
func Generate(u *ir.Unit) (map[string][]byte, error) {
	...
	for _, f := range u.Files {
		...
		//
		// Gated on a FIXED-SIZE member, not on any member: the variable class
		// has no text form yet (§16.1, schema#275), so a file whose tables are
		// all pointered has nothing to define and would emit a walker nothing
		// can call — dead lines that a glob-driven build compiles every time.
		// The file is not emitted at all rather than emitted empty, because
