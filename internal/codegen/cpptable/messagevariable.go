...
func (g *tableGen) emitVariableMessageSurface(st *ir.Struct) {
	...
	g.pf("// blob's by its length — and the root's own fields after it.\n")
	g.pf("inline bool %sMessageBodyStorage( TableBitReader & r, const TableVocabulary & vocabulary, int64_t & records, int64_t & data )\n{\n", n)
	g.pf("    records = 0;\n")
	g.pf("    data = 0;\n")
	...
	g.pf("        if ( !%sMessageBodyStorage( br.r, vocabulary, records, body_data ) ) { return -1; }\n", n)
	g.pf("        data += body_data;\n")
	...
	g.pf("    if ( region == NULL || ( ( (uintptr_t) region ) & ( kTableAlign - 1 ) ) != 0 ) { out->malformed = true; return false; }\n")
