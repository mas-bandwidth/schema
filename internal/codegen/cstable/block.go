...
// declares a table, plus <Package>Block.cs — the unit's one runtime home — when no
// file is named for the package. A file whose tables all lack a block form
// still gets one, saying which table and why.
func generateBlockFiles(u *ir.Unit, blocks *ir.BlockUnit) (map[string][]byte, error) {
	out := map[string][]byte{}
	if blocks == nil {
		return out, nil
	}
	// THE BLOCK HOME is <Package>Block.cs — one home per unit, named by the
	// package, independent of file order (runtimeHome, cstable.go). The
	// runtime and every blittable record land there, once per unit. It is not
	// the protocol id's home, which may declare no table at all and so gets no
	// Block source to hold anything.
	//
	...
	// every Block.cs it gets says which table has no block form and why, and
	// none of them references a runtime symbol.
	home := ""
	...
	if home != "" && !runtimeWritten {
		g := &blockGen{unit: u, blocks: blocks, home: true}
		g.emit()
		out[home+"Block.cs"] = g.assemble()
	}
	return out, nil
}

// anyBlockForm reports whether the unit has a block form at all — the
// condition under which its shared block runtime and blittable records exist
// and need a home.
...
func (g *blockGen) assemble() []byte {
	...
	if g.home {
		h.WriteString("//\n")
		h.WriteString("// " + runtimeHomeMarker + " — <Package>Block.cs, one home per unit, named by\n")
		h.WriteString("// the package and independent of file order (§19.2). The shared runtime and\n")
		h.WriteString("// every blittable record of the unit are here and nowhere else.\n")
	}
	h.WriteString("//\n")
