...
func tableMessageRegionWrite(w *TableMessageWriter,p unsafe.Pointer,t *TableTypeInfo,a *TableArena)bool {
 n,ok:=tableNumber(p,t,a);if !ok{return false};defer n.Release();w.Numbering=&n;w.IndexBits=tableMessageBits(uint64(len(n.entries)));if w.IndexBits<1{w.IndexBits=1}
