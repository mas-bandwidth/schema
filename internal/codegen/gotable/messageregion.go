...
func (g *tableGen) emitRegionMessage(st *ir.Struct) {
	...
	g.pf("func %sMeasureMessages(values []*%s,report *TableReport,arena ...*TableArena)int64 {if len(values)<1{return -1};if len(values)>TableMessageBatchMax {if report!=nil{tableMessageRefuse(report,\"batch_too_large\")};return -1};w:=TableMessageWriter{TableBitWriter:TableBitWriter{Measuring:true}};for _,value:=range values{if !tableMessageRegionWrite(&w,unsafe.Pointer(value),%sTableType(),tableOptionalArena(arena)){return -1}};w.Align();if w.Overflow{return -1};return 2+w.Bits/8}\n", n, typ, n)
	g.pf("func %sSaveMessages(values []*%s,buffer []byte,report *TableReport,arena ...*TableArena)int64 {if len(values)<1{return -1};if len(values)>TableMessageBatchMax {if report!=nil{tableMessageRefuse(report,\"batch_too_large\")};return -1};if len(buffer)<2{return -1};buffer[0]=2;buffer[1]=byte(len(values)-1);w:=TableMessageWriter{TableBitWriter:TableBitWriter{Buffer:buffer[2:]}};for _,value:=range values{if !tableMessageRegionWrite(&w,unsafe.Pointer(value),%sTableType(),tableOptionalArena(arena)){return -1}};w.Align();if w.Overflow{return -1};return 2+w.Bits/8}\n", n, typ, n)
...
func tableMessageRegionWrite(w *TableMessageWriter,p unsafe.Pointer,t *TableTypeInfo,a *TableArena)bool {
 n,ok:=tableNumber(p,t,a);if !ok{return false};w.Numbering=n;w.IndexBits=tableMessageBits(uint64(len(n.entries)));if w.IndexBits<1{w.IndexBits=1}
...
func tableMessageRegionInto(r *TableMessageReader,root *TableTypeInfo,blobs int,region []byte,used *int64)(out unsafe.Pointer,ok bool){
 ...
 for k:=int64(0);k<count;k++ {record,good:=tableMessageRecordScan(r,root,blobs);if !good{r.Report.Malformed=true;return nil,false};entry:=&directory[k+1];entry.TypeId=record.id;if record.size<=0 {entry.Offset=^uint64(0);unknown++;continue};if record.size>limit-*used{r.Report.Malformed=true;return nil,false};entry.Offset=uint64(*used);p:=unsafe.Add(base,*used);if t:=root.NodeType(record.id);t!=nil{t.Reset(p)}else{*(*uint64)(p)=uint64(record.length)};*used+=record.size}
