...
func TestTypedOpen(t *testing.T){
 ...
 header:=unsafe.Slice((*byte)(built.Base),built.Bytes);for i:=0;i<64;i++{for bit:=byte(1);bit!=0;bit<<=1{header[i]^=bit;err:=block.Open(built.Base,built.Bytes);var plain RootBlock;if RootBlockOpen(&plain,built.Base,built.Bytes)!=(err==nil){t.Fatal("block wrappers disagree")};header[i]^=bit}}
 if n:=testing.AllocsPerRun(50,func(){openErrorSink=block.Open(nil,0)});n!=0{t.Fatalf("refused block open allocated %v",n)}
 if n:=testing.AllocsPerRun(50,func(){openErrorSink=block.Open(built.Base,built.Bytes);if openErrorSink!=nil{panic("open")}});n!=0{t.Fatalf("successful block open allocated %v",n)}
 ...
 for i:=0;i<64;i++{for bit:=byte(1);bit!=0;bit<<=1{data[i]^=bit;err:=cook.Open(unsafe.Pointer(&data[0]),size);var plain RootCook;if RootOpen(&plain,unsafe.Pointer(&data[0]),size)!=(err==nil){t.Fatal("cook wrappers disagree")};data[i]^=bit}}
 if n:=testing.AllocsPerRun(50,func(){openErrorSink=cook.Open(nil,0)});n!=0{t.Fatalf("refused cook open allocated %v",n)}
 if n:=testing.AllocsPerRun(50,func(){openErrorSink=cook.Open(unsafe.Pointer(&data[0]),size);if openErrorSink!=nil{panic("open")}});n!=0{t.Fatalf("successful cook open allocated %v",n)}
