...
func (g *tableGen) emitListSurface(st *ir.Struct, f *ir.Field) {
	...
	field := "value." + member(f)
	g.pf("func %sAdd(worker *TableWorker,value *%s) *%s { p:=(*%s)(tableContainerAppend(worker,(*tableContainer)(unsafe.Pointer(&%s)),int64(unsafe.Sizeof(*new(%s)))));if p==nil{return nil};", name, g.storageName(st.Name), typ, typ, field, typ)
	if isStructRef(f.Type) && !f.Type.Pointer {
		g.pf("%sReset(p);", f.Type.Name)
	}
	g.pf("return p }\n")
	g.emitListExtras(st, f)

}
func (g *tableGen) emitListExtras(st *ir.Struct, f *ir.Field) {
	name := st.Name + ir.GoExportName(f.Name)
	typ := containerElementType(g.unit, f)
	field := "value." + member(f)
	g.pf("func %sErase(arena *TableArena,value *%s,element *%s) bool { return tableContainerErase(arena,(*tableContainer)(unsafe.Pointer(&%s)),unsafe.Pointer(element),int64(unsafe.Sizeof(*element))) }\n", name, g.storageName(st.Name), typ, field)
	g.pf("func %sEach(value *%s,visit func(*%s)bool,arena ...*TableArena) bool {cursor:=tableContainerCursorBegin((*tableContainer)(unsafe.Pointer(&%s)),int64(unsafe.Sizeof(*new(%s))),tableOptionalArena(arena));for i:=int32(0);i<%s.Count;i++ {p:=(*%s)(cursor.Next());if p==nil||!visit(p){return false}};return true }\n", name, g.storageName(st.Name), typ, field, typ, field, typ)
}
...
func (g *tableGen) emitListRead(f *ir.Field, ind string) {
	...
	g.pf("%sfor i:=uint64(0);i<count;i++ { p:=(*%s)(tableContainerFillAt(&sub.Nodes.Carve,slot,int32(i),int64(unsafe.Sizeof(*new(%s)))));if p==nil {r.Report.Malformed=true;break}\n", i, typ, typ)
	j := i + "\t"
	if kind == tkTable {
		g.pf("%selem,ok:=sub.Body();if !ok {r.Report.Malformed=true;break};%sLoadBody(&elem,p);sub.Take(elem)\n", j, f.Type.Name)
...
func (g *tableGen) emitListWrite(f *ir.Field, expr, writer, ind string, framed bool) {
	...
	for pass := 0; pass < 2; pass++ {
		...
		g.emitWireArrayElement(f, "(*p)", w, ind+"\t")
		g.pf("%s }\n", ind)
		if pass == 0 {
...
type tableContainerCursor struct { p unsafe.Pointer; chunk *tableContainerChunk; arena *TableArena; size int64; index int32 }
...
func (c *tableContainerCursor) Next() unsafe.Pointer {if c.arena==nil {if c.p==nil{return nil};p:=c.p;c.p=unsafe.Add(p,c.size);return p};for c.chunk!=nil {if c.index<c.chunk.Count {p:=unsafe.Add(unsafe.Pointer(c.chunk),32+int64(c.index)*c.size);dead:=c.chunk.Dead&(uint64(1)<<uint(c.index))!=0;c.index++;if dead{continue};return p};c.index=0;c.chunk=(*tableContainerChunk)(c.arena.At(c.chunk.Next))};return nil}
...
func tableContainerFill(c *TableExtentCarve,h *tableContainer,count uint64,size,align int64) bool {if c==nil{return false};if count>math.MaxInt32 {c.Refused=true;return false};*h=tableContainer{};if count==0{return true};if c.Worker!=nil {for i:=uint64(0);i<count;i++ {if tableContainerAppend(c.Worker,h,size)==nil{return false}};return true};p:=c.Alloc(int64(count)*size,align);if p==nil{return false};h.Ref=int64(uintptr(p)-uintptr(unsafe.Pointer(h)));h.Count=int32(count);return true}
func tableContainerFillAt(c *TableExtentCarve,h *tableContainer,index int32,size int64) unsafe.Pointer {var arena *TableArena;if c.Worker!=nil {arena=c.Worker.Arena};if arena!=nil {head:=(*tableContainerHead)(arena.At(h.Ref));if head==nil{return nil};cursor:=tableContainerCursor{arena:arena,size:size,chunk:(*tableContainerChunk)(arena.At(head.First))};return cursor.At(index)};cursor:=tableContainerCursorBegin(h,size,nil);return cursor.At(index)}
func tableExtentFieldEmpty(p,base unsafe.Pointer,f *TableFieldInfo,a *TableArena) bool {if f.List {return (*tableContainer)(p).Count==0};count:=int32(1);if f.IsArray{count=f.ArrayBound};for j:=int32(0);j<count;j++{at:=int64(0);if !tableExtentValue(unsafe.Add(p,uintptr(j)*uintptr(f.ElemSize)),nil,f,a,nil,&at)||at!=0{return false}};return true}
...
func tableExtentWalk(base,dest unsafe.Pointer,info *TableTypeInfo,a *TableArena,region unsafe.Pointer,at *int64) bool {
 ...
 if !tableJsonGuardHolds(base,info,f.Guard)||f.Optional&&!*(*bool)(unsafe.Add(base,f.PresentOffset)) {if !tableExtentFieldEmpty(p,base,f,a){return false};continue};if f.List {h:=(*tableContainer)(p);if h.Count<0{return false};if h.Count==0 {if q!=nil {*(*tableContainer)(q)=tableContainer{}};continue};*at=(*at+int64(f.ElemAlign)-1)& -int64(f.ElemAlign);start:=*at;*at+=int64(h.Count)*int64(f.ElemSize);if *at<start{return false};cursor:=tableContainerCursorBegin(h,int64(f.ElemSize),a);if q!=nil {out:=(*tableContainer)(q);out.Count=h.Count;out.Ref=int64(uintptr(unsafe.Add(region,start))-uintptr(q))};for j:=int32(0);j<h.Count;j++ {elem:=cursor.Next();if elem==nil{return false};var placed unsafe.Pointer;if q!=nil {placed=unsafe.Add(region,start+int64(j)*int64(f.ElemSize));copy(unsafe.Slice((*byte)(placed),f.ElemSize),unsafe.Slice((*byte)(elem),f.ElemSize))};if !tableExtentValue(elem,placed,f,a,region,at){return false}};continue}
...
func tableWireUnionExtent(r *TableReader,f *TableFieldInfo,at *int64) bool {ref,ok:=r.Leb();if !ok {r.Offset=int64(len(r.Buffer));return true};if ref==0{return true};id,ok:=r.Resolve(ref);if !ok||!r.Has(1){r.Offset=int64(len(r.Buffer));return true};r.Get8();body,ok:=r.Body();if !ok{r.Offset=int64(len(r.Buffer));return true};u:=f.Arms();for i:=1;i<len(u.Arms);i++ {if f.VariantId(uint64(i))!=id{continue};arm:=&u.Arms[i];if arm.Void||arm.Field.Pointer{return true};af:=&arm.Field;if af.Kind==13&&af.Table!=nil{return tableWireExtent(body,af.Table(),at)};if af.Kind==15 {return tableWireUnionExtent(&body,af,at)}};return true}
`

// Keep the extent machinery out of units which have no containers.
func containerRegionSource(s string) string {
	s = strings.Replace(s, "graph:=tableJsonGraph{alloc:worker.Alloc,", "graph:=tableJsonGraph{listAdd:func(p unsafe.Pointer,size int64)unsafe.Pointer{return tableContainerAppend(worker,(*tableContainer)(p),size)},alloc:worker.Alloc,", 1)
	...
	s = strings.Replace(s, "p:=unsafe.Add(base,f.Offset);count:=int32(1);", `p:=unsafe.Add(base,f.Offset);if f.List {h:=(*tableContainer)(p);if h.Count<0{return false};cursor:=tableContainerCursorBegin(h,int64(f.ElemSize),a);for j:=int32(0);j<h.Count;j++ {elem:=cursor.Next();if elem==nil||!tableRegionValueEdges(elem,f,a,visit){return false}};continue};count:=int32(1);`, 1)
...
func tableRegionPack(root unsafe.Pointer,info *TableTypeInfo,a *TableArena) ([]byte,bool) {
 measure,ok:=tableNumber(root,info,a);if !ok{return nil,false};total:=int64(0)
 for i:=range measure.entries {e:=&measure.entries[i];if e.info!=nil {extent:=tableExtentBytes(e.node,e.info,a);if extent<0{return nil,false};total+=tableRegionRound(tableRegionRound(int64(e.info.Size))+extent)}else {extra:=int64(0);if e.id==tableStringTypeId{extra=1};total+=tableRegionRound(8+int64(*(*uint64)(e.node))+extra)}}
 n,ok:=tableNumber(root,info,a);if !ok{return nil,false};size:=int64(0);for i:=range n.entries {e:=&n.entries[i];e.offset=size;if e.info!=nil {extent:=tableExtentBytes(e.node,e.info,a);if extent<0{return nil,false};size+=tableRegionRound(tableRegionRound(int64(e.info.Size))+extent)}else{extra:=int64(0);if e.id==tableStringTypeId{extra=1};size+=tableRegionRound(8+int64(*(*uint64)(e.node))+extra)}};if size!=total{return nil,false}
 region:=a.Allocator.alloc(size);if region==nil{return nil,false};clear(region);base:=unsafe.Pointer(&region[0])
 for i:=range n.entries {e:=&n.entries[i];p:=unsafe.Add(base,e.offset);if e.info==nil {length:=8+int(*(*uint64)(e.node));copy(unsafe.Slice((*byte)(p),length),unsafe.Slice((*byte)(e.node),length));continue};copy(unsafe.Slice((*byte)(p),e.info.Size),unsafe.Slice((*byte)(e.node),e.info.Size));at:=tableRegionRound(int64(e.info.Size));if !tableExtentWalk(e.node,p,e.info,a,p,&at){a.Allocator.free(region);return nil,false};if !tableRegionEdges(p,e.info,nil,func(slot *TableRef,f *TableFieldInfo) bool {if *slot==0{return true};target:=a.At(*slot);idx,ok:=n.seen[target];if !ok{return false};*slot=int64(uintptr(unsafe.Add(base,n.entries[idx].offset))-uintptr(unsafe.Pointer(slot)));return true}){a.Allocator.free(region);return nil,false}}
 return region,true
}
`
