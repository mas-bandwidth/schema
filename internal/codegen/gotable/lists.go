...
func containerRegionSource(s string) string {
	...
	s = strings.Replace(s, "p:=unsafe.Add(base,f.Offset);count:=int32(1);", `p:=unsafe.Add(base,f.Offset);if f.List||f.Map {h:=(*tableContainer)(p);if h.Count<0{return false};cursor:=tableMapCursor(h,f,a);defer cursor.Release();for j:=int32(0);j<h.Count;j++ {elem:=cursor.Next();if elem==nil||!tableRegionValueEdges(elem,f,a,visit){return false}};continue};count:=int32(1);`, 1)
