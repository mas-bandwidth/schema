...
func tableRegionPack(root unsafe.Pointer,info *TableTypeInfo,a *TableArena) ([]byte,bool) {
 measure,ok:=tableNumber(root,info,a);if !ok{return nil,false};defer measure.Release();total:=int64(0)
 for i:=range measure.entries {e:=&measure.entries[i];if e.info!=nil {extent:=tableExtentBytes(e.node,e.info,a);if extent<0{return nil,false};total+=tableRegionRound(tableRegionRound(int64(e.info.Size))+extent)}else {extra:=int64(0);if e.id==tableStringTypeId{extra=1};total+=tableRegionRound(8+int64(uint64(*(*uint32)(e.node)))+extra)}}
 n,ok:=tableNumber(root,info,a);if !ok{return nil,false};defer n.Release();size:=int64(0);for i:=range n.entries {e:=&n.entries[i];e.offset=size;if e.info!=nil {extent:=tableExtentBytes(e.node,e.info,a);if extent<0{return nil,false};size+=tableRegionRound(tableRegionRound(int64(e.info.Size))+extent)}else{extra:=int64(0);if e.id==tableStringTypeId{extra=1};size+=tableRegionRound(8+int64(uint64(*(*uint32)(e.node)))+extra)}};if size!=total{return nil,false}
 region:=a.Allocator.alloc(size);if region==nil{return nil,false};clear(region);base:=unsafe.Pointer(&region[0])
 for i:=range n.entries {e:=&n.entries[i];p:=unsafe.Add(base,e.offset);if e.info==nil {length:=8+int(uint64(*(*uint32)(e.node)));copy(unsafe.Slice((*byte)(p),length),unsafe.Slice((*byte)(e.node),length));continue};copy(unsafe.Slice((*byte)(p),e.info.Size),unsafe.Slice((*byte)(e.node),e.info.Size));at:=tableRegionRound(int64(e.info.Size));if !tableExtentWalk(e.node,p,e.info,a,p,&at){a.Allocator.free(region);return nil,false};if !tableRegionEdges(p,e.info,nil,func(slot *TableRef,f *TableFieldInfo) bool {if *slot==0{return true};target:=a.At(*slot);idx,ok:=n.find(target);if !ok{return false};*slot=int64(uintptr(unsafe.Add(base,n.entries[idx].offset))-uintptr(unsafe.Pointer(slot)));return true}){a.Allocator.free(region);return nil,false}}
 return region,true
}
`
