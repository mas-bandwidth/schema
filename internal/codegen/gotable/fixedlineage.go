...
//
// Wire is the eight bytes the header and every record carry — the lineage's
// key and THE ONLY FACT A FILE IS MATCHED ON. Layout is the bytes verbatim,
// what LOAD compares and what PLAN walks. Record is the body size, taken from
// the lock and never from the file. Retired and Reason are the operator's:
...
func FixedLineageOf(u *ir.Unit, name string) (FixedLineageEntry, bool) {
	for _, st := range ir.TableFixedRoots(u) {
		...
		g := &tableGen{unit: u, file: fileOfTable(u, name)}
...
func fileOfTable(u *ir.Unit, name string) *ir.File {
...
// when the lineage does not already end on it. The floor is 1 + the highest
