...
func TestMeasureRefusalReasons(t *testing.T) {
 ...
 if n:=testing.AllocsPerRun(20,func(){RootLoadMeasureReason(wire)});n!=0{t.Fatalf("file refusal allocated %v",n)}
 }}
 for _,tc:=range []struct{wire []byte;reason TableRefuseReason}{{nil,TableRefuseTruncated},{[]byte{1},TableRefuseTruncated},{[]byte{2},TableRefuseUnknownForm},{[]byte{255},TableRefuseUnknownForm}}{if n,err:=RootLoadMeasureReason(tc.wire);n!=-1||err!=tc.reason{t.Fatalf("file reason %d %v",n,err)}}
 ...
 if n:=testing.AllocsPerRun(20,func(){RootLoadMeasureReason(wire)});n!=0{t.Fatalf("file measure allocated %v",n)}
