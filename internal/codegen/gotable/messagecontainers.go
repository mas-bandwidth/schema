...
func (g *tableGen) emitMessageReadMap(f *ir.Field, expr, entry, ind string) {
	...
	keyReader := "*r"
	if g.retain {
		keyReader = "r.tableMessagePlainReader"
	}
	g.pf("%s{count,ok:=tableMessageCount(&r.Bits,%s.Shape);if !ok {r.Report.Malformed=true;return false};field:=%s;holder:=(*tableContainer)(unsafe.Pointer(&%s));%sif !tableContainerFill(&r.Nodes.Carve,holder,count,%d,%d){r.Report.Malformed=true;return false};holder.Count=0;var last tableMapKey;var prior *%s;mapWidened:=false;for i:=uint64(0);i<count;i++ {key,end,bad,over,wide,whole:=tableMessageMapReadKey(%s,&field.Table().Fields[0]);if !whole{r.Report.Malformed=true;return false};if wide&&!mapWidened{mapWidened=true;r.Report.Widened++};if bad{*holder=tableContainer{};r.Report.KindMismatch++;r.Bits.Offset=end;for j:=i+1;j<count;j++ {if !tableMessageSkipBody(&r.Bits,r.Vocabulary,r.IndexBits,0){r.Report.Malformed=true;return false}};break};if over {r.Report.Clamped++;r.Bits.Offset=end;continue};order:= -1;if prior!=nil{order=tableMapKeyOrder(last,key,&field.Table().Fields[0])};if order>0{r.Report.Malformed=true;return false};p:=prior;if order==0 {r.Report.Duplicate++}else{p=(*%s)(tableContainerFillAt(&r.Nodes.Carve,holder,holder.Count,%d));if p==nil {r.Report.Malformed=true;return false};holder.Count++};\n", ind, entry, g.mapDescriptor(f), expr, g.retainMessageDiscardCode(), layout.Size, layout.Align, typ, keyReader, typ, layout.Size)
	restore := ""
	if g.retain {
