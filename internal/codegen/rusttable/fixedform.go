...
func (g *gen) emitFixedRoot(st *ir.Struct) {
	...
	g.pf("    let mut found = usize::MAX;\n")
	g.pf("    let mut probe = 0usize;\n")
	g.pf("    while probe < %s_FIXED_LINEAGE.len() {\n", up)
	g.pf("        if %s_FIXED_LINEAGE[probe].hash == hash {\n", up)
	g.pf("            found = probe;\n            break;\n        }\n")
	g.pf("        probe += 1;\n    }\n")
	g.pf("    if found == usize::MAX {\n")
