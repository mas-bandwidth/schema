...
// runtimeModule assembles table_runtime.rs — the unit's ONE shared table
// runtime: the report, the writer and the reader, the enum-keyed array's
// storage, the reflection descriptors, and the text form's single generic
// walk over them (docs/SPEC-TABLES.md §16.1).
//
// It is emitted ONCE PER UNIT and its body is the same bytes in every unit,
// whatever that unit declares — the Rust twin of the generic-walk gate the
// C++ backend is held to. Nothing in it names a table, a package or a kind
// this unit happens to use.
func runtimeModule(u *ir.Unit, closure map[string]bool) []byte {
...
// substitution happens, which is what makes "the same bytes in every unit"
// true by construction.
const runtimeSource = `// ---- the read report (docs/SPEC-TABLES.md §4) ----
...
const TABLE_JSON_BASE64_DECODE: [i8; 256] = [
...
// unused keeps the fmt import honest while the module grows.
var _ = fmt.Sprintf
