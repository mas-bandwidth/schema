...
const wireRuntime = `#[derive(Clone, Copy, PartialEq, Eq, Debug, Default)]
...
pub struct TableWriter<'a> {
    nodes: Option<&'a TableNumbering<'a>>,
...
impl<'a> TableWriter<'a> {
    ...
    #[inline(always)]
    pub fn put128(&mut self, value: u128) {
        self.raw(&value.to_le_bytes());
    }
    #[inline(always)]
    pub fn putleb(&mut self, mut v: u64) {
        while v >= 128 {
            self.put8((v as u8 & 127) | 128);
            v >>= 7;
        }
        self.put8(v as u8);
    }
    ...
    pub fn framed(&mut self, body: impl Fn(&mut TableWriter) -> bool) -> bool {
        ...
        probe.nodes = self.nodes;
        if !body(&mut probe) || probe.overflow {
            return false;
        }
    ...
    pub fn body_size(&mut self, body: impl Fn(&mut TableWriter) -> bool) -> i64 {
        let mut probe = TableWriter::new(None, self.ids);
        probe.count = self.count;
        probe.nodes = self.nodes;
        if !body(&mut probe) || probe.overflow {
    ...
    pub fn finish(&mut self) -> i64 {
        for i in 0..self.count {
            self.put64(self.ids[i]);
        }
        self.put64(self.count as u64);
        if self.overflow {
            -1
        } else {
            i64::try_from(self.offset).unwrap_or(-1)
        }
    }
}
...
pub struct TableReader<'a> {
    builder: Option<&'a TableDecodeBuilder>,
    extent: Option<&'a TableExtent>,
    nodes: Option<TableNodeMap<'a>>,
    pub buffer: &'a [u8],
    pub offset: usize,
    ids: &'a [u8],
    nested: bool,
...
impl<'a> TableReader<'a> {
    ...
    pub fn open(bytes: &'a [u8]) -> Result<Self, TableOpenVerdict> {
        if bytes.is_empty() {
            return Err(TableOpenVerdict::Damaged);
        }
        if bytes[0] != 1 {
            return Err(TableOpenVerdict::Refused);
        }
        if bytes.len() < 9 {
            return Err(TableOpenVerdict::Damaged);
        }
        let count = u64::from_le_bytes(bytes[bytes.len() - 8..].try_into().unwrap());
        if count > ((bytes.len() - 9) / 8) as u64 {
            return Err(TableOpenVerdict::Damaged);
        }
        ...
        for i in 0..count as usize {
            for j in 0..i {
                if ids[i * 8..i * 8 + 8] == ids[j * 8..j * 8 + 8] {
                    return Err(TableOpenVerdict::Damaged);
                }
            }
        }
    ...
    pub fn has(&self, bytes: u64) -> bool {
        bytes <= self.buffer.len().saturating_sub(self.offset) as u64
    }
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn get128(&mut self) -> u128 {
        let lo = self.get64() as u128;
        lo | ((self.get64() as u128) << 64)
    }
    #[inline(always)]
    ...
    pub fn getid(&mut self) -> Option<Option<u64>> {
        ...
        if reference > (self.ids.len() / 8) as u64 {
            return None;
        }
        let at = (reference as usize - 1) * 8;
        Some(Some(u64::from_le_bytes(
            self.ids[at..at + 8].try_into().unwrap(),
        )))
    }
    #[inline(always)]
    ...
    pub fn sub(&self, bytes: usize) -> Self {
        Self {
            ...
            buffer: &self.buffer[self.offset..self.offset + bytes],
            offset: 0,
            ids: self.ids,
            nested: true,
        }
    ...
    pub fn utf16(&self, out: &mut [u16]) -> Option<usize> {
        ...
        if keep < n && keep > 0 && (0xd800..=0xdbff).contains(&unit(keep - 1)) {
            keep -= 1;
        }
        for (i, v) in out[..keep].iter_mut().enumerate() {
            *v = unit(i);
        }
        Some(keep)
    }
        ...
        Some(match kind {
            ...
            10 => {
                ...
                if bits & 0x7f800000 == 0x7f800000 {
                    ...
                    ((bits as u64 & 0x80000000) << 32)
                        | 0x7ff0000000000000
                        | ((bits as u64 & 0x7fffff) << 29)
                } else {
