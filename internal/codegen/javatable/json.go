...
func (g *tableGen) emitJsonSurface(st *ir.Struct) {
	...
	g.pf("public static boolean %s(%s value, byte[] text, TableReport report) {\n", method(st.Name, "FromJson"), ref)
	g.pf("    return %s(value, text, 0, text.length, report);\n}\n\n", method(st.Name, "FromJson"))
	g.pf("public static boolean %s(%s value, byte[] text, int offset, int length, TableReport report) {\n", method(st.Name, "FromJson"), ref)
	g.pf("    return TableJson.read(value, %s(), text, offset, length, report);\n}\n\n", method(st.Name, "TableType"))
	g.pf("public static long %s(%s value) {\n", method(st.Name, "ToJsonMeasure"), ref)
	g.pf("    return TableJson.write(value, %s(), null, 0, 0, true);\n}\n\n", method(st.Name, "TableType"))
	g.pf("public static long %s(%s value, byte[] buffer) {\n", method(st.Name, "ToJson"), ref)
	g.pf("    return %s(value, buffer, 0, buffer.length);\n}\n\n", method(st.Name, "ToJson"))
	g.pf("public static long %s(%s value, byte[] buffer, int offset, int length) {\n", method(st.Name, "ToJson"), ref)
	g.pf("    return TableJson.write(value, %s(), buffer, offset, length, false);\n}\n\n", method(st.Name, "TableType"))
}
...
public final class TableJson {
    ...
    // Indexed by an unsigned input byte; -1 preserves invalid characters.
