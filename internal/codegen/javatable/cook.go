...
func (g *cookGen) emitAt() {
	...
	g.f("    //\n")
	g.f("    // It takes the SLOT's offset and not its value, because a self-relative delta\n")
	g.f("    // means nothing without the position it is relative to, and it answers the\n")
	g.f("    // TARGET's offset — or -1, which is null AND a delta that leaves the region.\n")
	g.f("    // The bound is Java's own divergence and this file's header states why: an\n")
	g.f("    // unchecked deref here is an exception escaping a reader, not a pointer into\n")
	g.f("    // trusted bytes.\n")
	g.f("    public int at(int slot) {\n")
	g.f("        long delta = TableBytes.i64(data, slot);\n")
	g.f("        if (delta == 0) { return -1; }\n")
	g.f("        // written as bounds ON THE DELTA so no addition can wrap: the two limits\n")
	g.f("        // are small and the delta is whatever the file carried\n")
	g.f("        long low = (long) region - slot;\n")
	g.f("        long high = (long) region + regionLength - slot;\n")
