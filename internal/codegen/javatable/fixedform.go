...
func (g *fixedGen) emitRoot(st *ir.Struct, body int64) {
	...
	g.pf("    /** the form byte, the u32 layout length and the layout: what a FILE\n")
	g.pf("     *  carries once, in front of the records. */\n")
	g.pf("    public static final int headerBytes = 1 + 4 + %d;\n\n", len(layout))

	g.pf("    // THE FILE HEADER LIVES BEHIND THESE TWO FUNCTIONS AND NOWHERE ELSE, so\n")
	g.pf("    // the day §3.4 moves a byte of it there is one place in this port to move.\n")
	...
	g.pf("        TableFixed.put32(buffer, 1, layout.length);\n")
	g.pf("        System.arraycopy(layout, 0, buffer, 5, layout.length);\n")
	...
	g.pf("     *\n")
	g.pf("     *  Nothing here allocates: the plan, the ordinal remap scratch and the\n")
	g.pf("     *  record image are all the caller's. */\n")
