...
func (g *fixedGen) emitRoot(st *ir.Struct, body int64) {
	...
	g.pf("     *  once per call: the header below, the hole list the COMPILED path\n")
	g.pf("     *  builds (cover and the packed ranges — the identity path builds\n")
	g.pf("     *  none, having no holes to fill), and on that same path the two\n")
	...
	g.pf("        // THE PREFILL IS THE BYTES THE PLAN DOES NOT WRITE, and no more than\n")
	g.pf("        // those: the holes this plan leaves, packed once per call rather than\n")
	g.pf("        // a whole-image copy per record. The identity path never gets here —\n")
	g.pf("        // its one copy covers the body, so its hole list would be empty.\n")
	g.pf("        final byte[] cover = new byte[bodyBytes];\n")
	g.pf("        final int[] hole = new int[Math.max(2, bodyBytes * 2)];\n")
	g.pf("        final int holeN = TableFixed.holes(plan, made, cover, hole);\n")
	g.pf("        for (int k = 0; k < n; k++) {\n")
