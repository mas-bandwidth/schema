...
// call with no lineage, so a unit that was never locked promises nothing and
// refuses every hash but its own, by name.
...
// LOAD compares and what PLAN walks. Record is the record size, taken from the
...
// fixedLineage is the lineage one table's library lays down: the entries the
// build handed it, OLDEST FIRST, with the table's OWN entry appended last when
// the lineage does not already end on it (§5.9 #2), and the floor — 1 + the
// highest retired index, 0 when none is (§5.2).
func (g *fixedModule) fixedLineage(st *ir.Struct, own FixedLineageEntry) ([]FixedLineageEntry, int) {
