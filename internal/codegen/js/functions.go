...
func (g *gen) emitMessageStorage() {
	g.pf("// MessageStorage is the pre-allocated home a read message lands in — the JS\n")
	...
	g.pf("export class MessageStorage {\n  constructor() {\n")
...
// and the JS dispatch surface: an instanceof chain over the generated classes
// — the direct translation of C#'s type-pattern switch (SPEC §6.1 item 6).
// null is None, the stream terminator.
func (g *gen) emitMessageTagFunctions() {
	...
	g.pf("// The write folds the tag's bit count at generation time (byte-identical to\n")
	g.pf("// the ranged form); a tag outside the set is refused — bool alone.\n")
	g.pf("export function WriteMessageType(stream, value) {\n")
	g.pf("  if (!Number.isInteger(value) || value < 0 || value > %d) {\n    return false;\n  }\n", count)
	g.pf("  %s.value = value;\n", g.numScratch())
	g.pf("  return stream.serializeBits(%s, %d);\n}\n\n", g.numScratch(), tagBits)
	g.pf("// ReadMessageType reads the tag into a { value } holder — the runtime's own\n")
	g.pf("// ref convention (JS has no out parameters).\n")
	g.pf("export function ReadMessageType(stream, value) {\n")
	g.pf("  if (!stream.serializeInt(%s, 0, %d)) {\n    return false;\n  }\n", g.numScratch(), count)
	g.pf("  value.value = %s.value;\n  return true;\n}\n\n", g.numScratch())
	...
	g.pf("export const MessageMaxBits = %d;\n", tagBits+largest)
	g.pf("export const MessageMaxBytes = %d;\n\n", ir.MaxBytes(tagBits+largest))

	// dispatch validates BEFORE the tag rides the wire: a value from outside
	// the generated class set writes nothing (a tag with no payload would
	// desynchronize the stream), and the tag framing is the tag pair's — one
	// source
	g.pf("// WriteMessage: null is the None terminator; a value outside the generated\n")
	g.pf("// class set (undefined included) writes NOTHING — a tag with no payload\n")
	g.pf("// would desynchronize the stream — and the refusal is bool alone.\n")
	g.pf("export function WriteMessage(stream, message) {\n")
	g.pf("  if (message === null) {\n")
	g.pf("    return WriteMessageType(stream, MessageType.None); // the stream terminator (SPEC §4.8)\n  }\n")
	for _, m := range g.unit.Messages {
		g.addRef(m, m, "Write"+m)
		g.pf("  if (message instanceof %s) {\n", m)
		g.pf("    if (!WriteMessageType(stream, MessageType.%s)) {\n      return false;\n    }\n", m)
		g.pf("    return Write%s(stream, message);\n  }\n", m)
	}
	g.pf("  return false; // not a generated message class; nothing was written\n}\n\n")

	g.pf("// ReadMessage lands in the caller's MessageStorage and hands the message\n")
	g.pf("// back through the { value } holder: null under a None tag (the terminator\n")
	g.pf("// — a success), the storage's instance otherwise. The returned message\n")
	g.pf("// points INTO storage and stays valid until the next read against it.\n")
	g.pf("export function ReadMessage(stream, storage, message) {\n")
	g.pf("  message.value = null;\n")
	g.pf("  if (!ReadMessageType(stream, %s)) {\n    return false;\n  }\n", g.numScratch())
	g.pf("  // the scratch holder is consumed by the switch head before any nested\n")
	g.pf("  // read runs — the holder discipline\n")
	g.pf("  switch (%s.value) {\n", g.numScratch())
	g.pf("    case MessageType.None:\n")
	g.pf("      return true; // the stream terminator (SPEC §4.8) — message stays null\n")
	for _, m := range g.unit.Messages {
		g.addRef(m, "Zero"+m, "Read"+m)
		g.pf("    case MessageType.%s:\n", m)
		g.pf("      Zero%s(storage.%s); // reused storage starts from the zero form, as the union does\n", m, m)
		g.pf("      if (!Read%s(stream, storage.%s)) {\n        return false;\n      }\n", m, m)
		g.pf("      message.value = storage.%s;\n      return true;\n", m)
	}
	g.pf("    default:\n")
	g.pf("      // unreachable: ReadMessageType bounds the tag to [0, %d] and every\n", count)
	g.pf("      // value in range has an arm; the default keeps the verdict total\n")
	g.pf("      return false;\n")
	g.pf("  }\n}\n\n")
}
...
func (g *gen) emitObjectTagFunctions() {
	...
	g.pf("// The write folds the tag's bit count at generation time (byte-identical to\n")
	g.pf("// the ranged form); a tag outside the set is refused — bool alone.\n")
	g.pf("export function WriteObjectType(stream, value) {\n")
	g.pf("  if (!Number.isInteger(value) || value < 0 || value > %d) {\n    return false;\n  }\n", count)
	g.pf("  %s.value = value;\n", g.numScratch())
	g.pf("  return stream.serializeBits(%s, %d);\n}\n\n", g.numScratch(),
		ir.BitsRequired(big.NewInt(0), big.NewInt(count)))
	g.pf("// ReadObjectType reads the tag into a { value } holder — the runtime's own\n")
	g.pf("// ref convention (JS has no out parameters).\n")
	g.pf("export function ReadObjectType(stream, value) {\n")
	g.pf("  if (!stream.serializeInt(%s, 0, %d)) {\n    return false;\n  }\n", g.numScratch(), count)
	g.pf("  value.value = %s.value;\n  return true;\n}\n\n", g.numScratch())
}
