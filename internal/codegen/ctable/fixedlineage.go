...
// **nothing parses a stranger's layout, on any path**. So the backend needs one
...
// `lockfile.Floor(lock, T)` — and a build that has no lock for a unit hands
// nothing, which leaves a table with the single entry it can always compute:
// its own (§5.9 #1, #2).
//
...
//
// THE HASH IS THE SHARED FUNCTION'S (§5.9 #12): ir.TableFixedLayoutHash, whose
// signature has no digest argument for a leg to omit.
...
// when the lineage does not already end on it — so R.own_hash always resolves
// to an index and the IDENTITY plan is always reachable (§5.9 #2). The floor is
// 1 + the highest retired index, or 0 when none is (§5.2).
