...
func (g *tableGen) emitJsonDeclarations(st *ir.Struct) {
	if g.isVar(st.Name) {
		...
		// yet, exactly as the reference does not. The refusal is by ABSENCE
		// and it is loud: there is no function to call, so nothing silently
		// reads a pointered table as an empty one.
		...
		g.pf("   no %s and no %s exist to call. */\n\n", g.api(st.Name, "from_json"), g.api(st.Name, "to_json"))
		return
	}
