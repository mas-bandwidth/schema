...
func (g *fixedGen) emitLoad(st *ir.Struct, snake string) {
	...
	g.pf("  # ONE PREFILL-AND-PROJECT PER RECORD, and §5.3 STEP 10b's HASH PRE-PASS ahead\n")
	g.pf("  # of it: `_fixed_split` walks the WHOLE tail and holds every record's hash\n")
	g.pf("  # before this `with` opens, so no record lands until all of them are good and\n")
	g.pf("  # REFUSE stays total on a file of many records. Neither plan clamps a ranged\n")
	g.pf("  # integer: the generated projection holds the bound after the loop, the same\n")
	g.pf("  # pass for either plan.\n")
	...
	g.pf("        # NAME, never a guess and never damage. This split IS §5.3 step 10b's\n")
	g.pf("        # PRE-PASS: it runs over every record before the first one is landed.\n")
