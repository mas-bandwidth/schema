...
// `lockfile.Floor(lock, T)`, the CALLER's two calls (§5.9 #1) — and a build
// with no lock for a unit hands nothing, which leaves a table the single entry
// it can always compute: its own (§5.9 #2).
//
...
// (`ir.TableFixedLayoutHash`, the one function — §5.9 #12), and its record
// size. It is what a lock records at commit, and it is how a test — or a build
// reading a sibling generation — states an older entry without a lock.
...
// fixedLineage is the lineage one root's module lays down: the entries the
