...
// variableTableNames is the unit's tables whose WIRE surface needs the
// VARIABLE CLASS — an arena, a builder, a region and the node-table codec —
// sorted. A `table` with none of those constructs still rides the same bytes a
// `fixed table` does, so it keeps its codecs; only a closure that reaches a
// pointer, a byte buffer, a map or an unbounded array is refused by name.
func variableTableNames(u *ir.Unit) []string {
	...
	for name := range variable {
		st := u.Tables[name]
		if st != nil && tableNeedsVariableWire(st, map[string]bool{}) {
...
// tableNeedsVariableWire reports whether a table's by-value closure reaches a
// construct whose body is variable size on the id-table wire: a pointer, a
// `*bytes`/`*string` byte buffer, a map, or an unbounded array (docs/SPEC-TABLES.md
// §2.5, §2.8, §2.9). A table without one has a fixed-size body and rides the
// ordinary codec.
func tableNeedsVariableWire(st *ir.Struct, seen map[string]bool) bool {
	if st == nil || seen[st.Name] {
		return false
	}
	seen[st.Name] = true
	fields := append([]*ir.Field{}, st.Fields...)
	for len(fields) > 0 {
		f := fields[0]
		fields = fields[1:]
		if f.Type.Pointer || f.IsMap() || f.Array == ir.ArrayList {
			return true
		}
		...
		case *ir.Struct:
			if tableNeedsVariableWire(ref, seen) {
				return true
			}
		case *ir.Union:
			for _, v := range ref.Variants {
				if v.F != nil {
					fields = append(fields, v.F)
				}
			}
		}
	}
