...
func (g *fixedModule) emitRoot(st *ir.Struct) {
	...
	g.pf("  if (bytes === null || byteLength < 1) { report.malformed = true; return -1; }\n")
	...
	g.pf("  // AND IT IS READ BEFORE THE FILE'S LENGTH (§5.3): a committed form-1 file\n")
	g.pf("  // is TEN bytes and a form-2 batch THREE, so a reader that measured first\n")
	g.pf("  // would answer `malformed` for every real file of the two forms it names.\n")
	...
	g.pf("    return -1;\n  }\n")
	g.pf("  if (byteLength < TableFixedHeaderBytes + TableFixedLayoutHeaderBytes) { report.malformed = true; return -1; }\n")
	g.pf("  // THE SEVEN RESERVED BYTES ARE REFUSED, NOT IGNORED (§3.4, §5.3).\n")
	g.pf("  for (let i = 1; i < TableFixedHashAt; i++) { if (bytes[i] !== 0) { report.malformed = true; return -1; } }\n")
