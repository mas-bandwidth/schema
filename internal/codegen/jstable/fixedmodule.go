...
func (g *fixedModule) emitRoot(st *ir.Struct) {
	...
	g.pf("  bytes.fill(0, at, need); // the records' template zeros\n")
	...
	// ---- foreign read ----
	g.pf("// THE FOREIGN READ: a prefill and the compiled plan run for records from\n")
	g.pf("// another writer. Split out so the hot identity loop stays straight-line and\n")
	g.pf("// JIT-friendly without plan interpreter overhead.\n")
	g.pf("function _%sFixedLoadForeign(values, capacity, bytes, byteLength, plan, report, srcView, layoutAt, layoutBytes, hashLo, hashHi) {\n", st.Name)
	...
	g.pf("  }\n")
	g.pf("  const entries = plan.entries, entryCount = plan.count, remap = plan.remap;\n")
	g.pf("  const recordBytes = plan.recordBytes;\n")
	g.pf("  if (srcView.getUint32(TableFixedHashAt, true) !== hashLo ||\n")
	g.pf("      srcView.getUint32(TableFixedHashAt + 4, true) !== hashHi) {\n")
	...
	g.pf("    if (srcView.getUint32(at, true) !== hashLo ||\n")
	g.pf("        srcView.getUint32(at + 4, true) !== hashHi) {\n")
	...
	// ---- the read ----
	g.pf("// THE READ: straight-line direct DataView identity load when the layout's\n")
	g.pf("// hash matches this build's own (Glenn's ruling: learn from how the packet\n")
	g.pf("// wire is read/written), and a plan compiled once for foreign layouts.\n")
	...
	g.pf("      report.refused = TableFixedRefusal.LayoutMalformed; return -1;\n    }\n")
	g.pf("    const rest = byteLength - layoutAt - layoutBytes;\n")
	g.pf("    if (%sFixedRecordBytes <= 8 || rest %% %sFixedRecordBytes !== 0) { report.malformed = true; return -1; }\n", st.Name, st.Name)
	g.pf("    const n = (rest / %sFixedRecordBytes) | 0;\n", st.Name)
	g.pf("    if (n > capacity) { report.refused = TableFixedRefusal.BatchTooLarge; return -1; }\n")
	g.pf("    let at = layoutAt + layoutBytes;\n")
	g.pf("    for (let k = 0; k < n; k++) {\n")
	g.pf("      if (srcView.getUint32(at, true) !== hashLo ||\n")
	g.pf("          srcView.getUint32(at + 4, true) !== hashHi) {\n")
	g.pf("        report.refused = TableFixedRefusal.NoLayout; return -1;\n      }\n")
	g.pf("      %sFixedDecode(values[k], srcView, at + 8, report);\n", st.Name)
	g.pf("      at += %sFixedRecordBytes;\n", st.Name)
	g.pf("    }\n    return n;\n  }\n")
	g.pf("  return _%sFixedLoadForeign(values, capacity, bytes, byteLength, plan, report, srcView, layoutAt, layoutBytes, hashLo, hashHi);\n}\n\n", st.Name)
}
