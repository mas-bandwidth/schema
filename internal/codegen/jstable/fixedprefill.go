...
// by the compiler, laid down with one `set`. Every declared default is folded
// into it at generation time, so the read path neither walks nor branches to
// place one.
...
// §5's zero initialization holds, and the declared default laid in wherever one
// overrides it.
...
func fixedPrefillField(out []byte, at int64, f *ir.Field) {
	...
	switch {
	case f.KeyEnum != "":
		// a keyed array's slot this reader has but the writer never sent keeps
		// its declared default, exactly as an absent field does
		fixedPrefillElements(out, at, f, f.KeyEnumRef.Max)
	case f.Array == ir.ArrayFixed:
		fixedPrefillElements(out, at, f, f.ArrayBound)
	case f.Array == ir.ArrayCounted:
		// a counted array is born at its declared MINIMUM, which is where
		// internal/codegen/js's own constructor puts its count
...
// fixedVariantOrdinal is a variant's POSITION IN THE BLOCK, from 1.
...
// rides at every width including the 128-bit pair (the low half, then the high).
