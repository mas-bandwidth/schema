...
func (g *gen) emitHelpers(h *strings.Builder) {
	...
	if g.needF32Conv {
		h.WriteString("// The 32 bits of the IEEE-754 single representation of value. SPEC §4.3's\n")
		h.WriteString("// rule is that a wider cell CARRIES the pattern and never moves it through\n")
		h.WriteString("// a float conversion — so the hardware narrowing is used on the non-NaN\n")
		h.WriteString("// half only, where it IS that carry: every float32 value is a double\n")
		h.WriteString("// exactly (f32 ⊂ f64), so the double->float32 step reproduces the original\n")
		h.WriteString("// 32 bits — sign, exponent and all 23 mantissa bits — for every finite\n")
		h.WriteString("// value and for the infinities, negative zero included. A NaN narrows in\n")
		h.WriteString("// SOFTWARE, because the hardware step would set the quiet bit on a\n")
		h.WriteString("// signalling NaN and drop the payload past 23 bits: sign kept, top 23\n")
		h.WriteString("// mantissa bits kept, the quiet bit forced only for the all-low-payload\n")
		h.WriteString("// case — so every pattern the read half produces round trips.\n")
		h.WriteString("@pragma('vm:prefer-inline')\n")
