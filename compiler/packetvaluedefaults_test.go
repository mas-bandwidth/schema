...
`
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// NEITHER ELIXIR NOR RUST IS ON THIS LIST ANY MORE: both carry
			// the table half too (compiler/target_elixir.go,
			// compiler/target_rust.go), and their carrier test is
