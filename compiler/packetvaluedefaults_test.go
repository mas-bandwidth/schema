...
`
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, target := range []string{"c", "go", "cs", "java", "js", "dart", "elixir"} {
				...
				if strings.Contains(err.Error(), "Loose.label") || !strings.Contains(err.Error(), "generate with --lang cpp and --lang rust, or drop the default") {
