...
func TestPacketValueDefaultsBesideUnrelatedTable(t *testing.T) {
	...
	for _, target := range []string{"go", "rust", "cs", "java", "js", "dart", "elixir"} {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, target := range []string{"go", "rust", "cs", "java", "js", "dart", "elixir"} {
				...
				if strings.Contains(err.Error(), "Loose.label") || !strings.Contains(err.Error(), "generate with --lang c and --lang cpp, or drop the default") {
