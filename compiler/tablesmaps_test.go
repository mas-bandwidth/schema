...
func TestMapCarriers(t *testing.T) {
	if !slices.Equal(mapTargets, []string{"c", "cpp"}) {
		t.Fatalf("mapTargets = %v, want [c cpp]", mapTargets)
	}
}
...
func TestMapRefusalNamesTheCarrier(t *testing.T) {
	...
	for _, want := range []string{"a map is c and cpp only today", "Fleet.ships", "--lang cpp"} {
		if !strings.Contains(err.Error(), want) {
