...
// lineage as data today: `c`, `cs`, `go` and `rust` through `GenerateLineage`, and
// `cpp` through the lock the driver now opens for it. Every other built-in
// target's table backend has no second entry point yet (dart, elixir, java, js);
...
var fixedLineageShipTargets = []string{"c", "cpp", "cs", "go", "rust"}
