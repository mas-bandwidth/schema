...
// lineage as data today: `go`, `java` and `rust` through `GenerateLineage`, and
// `cpp` through the lock the driver now opens for it. Every other built-in
// target's table backend has no second entry point yet (c, cs, dart, elixir, js);
...
var fixedLineageShipTargets = []string{"cpp", "go", "java", "rust"}
