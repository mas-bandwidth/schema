...
func refuseWideText(u *ir.Unit, target string) error {
	...
	if len(tableFields) > 0 && target != "cpp" && target != "go" {
		return fmt.Errorf("unit puts a wstring(N) field in a table closure (%s): table wide text is C++ and Go only today, and the %s table codec is a named follow-on; generate with --lang cpp or --lang go (SPEC §4.12)", englishList(tableFields), target)
	}
