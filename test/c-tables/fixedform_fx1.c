...
void fixed_fx1_bounds( const uint8_t * data, int64_t bytes )
{
    ...
    table_fixed_run( fx_root_fixed_plan, fx_root_fixed_plan_count, fx_root_fixed_plan_guarded,
                     (const uint8_t *) fx_root_fixed_guards, body, (uint8_t *) &loose, &r );
       ...
       clamp, and it is the same pass for either plan. */
    {
        ...
        fx_root_reset( &held );
        memset( &r, 0, sizeof( r ) );
        table_fixed_run( compiled, made, guarded, (const uint8_t *) compiled, body, (uint8_t *) &held, &r );
...
void fixed_fx1_text_content( void )
{
    ...
    for ( k = 0; k < sizeof( cases ) / sizeof( cases[0] ); k++ )
    {
        ...
        if ( k == 0 )
        {
            ...
            body = file + kTableFixedHeaderBytes + 4 + (int64_t) sizeof( fx_root_fixed_layout ) + 8;
            table_fixed_run( fx_root_fixed_plan, fx_root_fixed_plan_count, fx_root_fixed_plan_guarded,
                             (const uint8_t *) fx_root_fixed_guards, body, (uint8_t *) &loose, &r2 );
...
void fixed_guard_width( void )
{
    ...
    pool[0].guard = 0; pool[0].arg_lo = 1; pool[0].argw = 2;
    ...
    memset( &r, 0, sizeof( r ) );
    table_fixed_run( plan, 2, 1, (const uint8_t *) pool, src, dst, &r );
