...
int main( void )
{
       ...
       written through both views must produce identical bytes. */
    {
        ...
        serialize_write_stream_init( &w, buffer, sizeof( buffer ) );
        check( write_body_deep( &w, &deep ), "write BodyData_Deep" );
        serialize_write_flush( &w );
        ...
        shallow.velocity = deep.velocity;
        {
            ...
            check( write_body_shallow( &ws, &shallow ), "write BodyData_Shallow" );
        ...
        check( read_body_deep( &r, &out ), "read BodyData_Deep" );
       ...
       wrong, which is exactly how it stayed wrong). */
    {
        ...
        serialize_write_stream_init( &w, buffer, sizeof( buffer ) );
        check( write_narrow_body_shallow( &w, &sh ), "write NarrowBodyData_Shallow" );
        serialize_write_flush( &w );
           ...
           67108863, above the range size 51200000 — reject, never clamp */
        {
            ...
            check( !read_narrow_body_shallow( &r, &h_out ),
        ...
        serialize_write_stream_init( &w, buffer, sizeof( buffer ) );
        check( write_narrow_body_deep( &w, &deep ), "write NarrowBodyData_Deep" );
        serialize_write_flush( &w );
...

        {
            ...
            serialize_read_stream_init( &r, buffer, serialize_write_bytes_processed( &w ) );
            check( read_narrow_body_deep( &r, &deep_out ), "read NarrowBodyData_Deep" );
