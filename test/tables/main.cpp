...
// ---- A BODY THAT ENDS EARLY, ANSWERED BY THE READ ITSELF (§3) -------------
//
// ANY BYTE BETWEEN THE ROOT'S TERMINATOR AND THE TABLE'S FIRST ENTRY IS
// MALFORMED, because no field claims it and the two ends of the file have met.
// The root load used to answer that with a FRAMING PRE-WALK before the reading
// walk, and now answers it with the reading walk's own cursor: a body that
// returned at its own zero reference must have left the cursor at the body's
// end. The verdict, the value and the report are the ones the pre-walk gave.
//
// TableBodyEndsEarly IS THE ORACLE HERE, and that is the point: it is the
// pre-walk itself, unchanged, still carried for the variable-class roots and
// for the announcement. Every wire below is put to both, and the read's
// verdict must be TableOpenDamaged for exactly the bodies the framing walk
// calls early — no wire may be damage the walk does not name, and none may
// escape damage the walk does.
//
// The battery: a body cut at every field boundary and mid-field, bodies that
// run long, bodies carrying unknown ids, kind mismatches and RESERVED ids
// before the early end — the last being the one shape where the two walks
// disagree about whether the body could be read at all, because a reserved id
// is invisible to framing and fatal to the read.
static int64_t cfg_projection( const tblv1::Cfg & value, uint8_t * out, int64_t capacity )
{
    const int64_t need = tblv1::CfgMeasure( value );
    if ( need < 0 || need > capacity ) { return -1; }
    return tblv1::CfgSave( value, out, need );
}

static void check_ends_early_rule( const char * name, const uint8_t * wire, int64_t bytes )
{
    tblv1::TableIdTable table;
    int64_t body_bytes = 0;
    const tblv1::TableOpenVerdict open = tblv1::TableOpen( wire, bytes, table, body_bytes );

    tblv1::Cfg value;
    tblv1::CfgReset( value );
    tblv1::TableReport report;
    const tblv1::TableOpenVerdict verdict = tblv1::CfgLoadVerdict( value, wire, bytes, &report );

    if ( open != tblv1::TableOpenOk )
    {
        // a table that cannot be read whole is the open's verdict and nothing
        // this rule has an opinion about
        if ( verdict != open ) { printf( "FAIL %s: open %d, load %d\n", name, (int) open, (int) verdict ); failures++; }
        return;
    }

    const bool early = tblv1::TableBodyEndsEarly( wire + 1, body_bytes, table );
    if ( ( verdict == tblv1::TableOpenDamaged ) != early )
    {
        printf( "FAIL %s: the framing walk says early=%d and the read says verdict=%d\n", name, early ? 1 : 0, (int) verdict );
        failures++;
        return;
    }
    if ( !early ) { return; }

    // NOTHING IS DECODED ON THAT PATH: the declared defaults stand and the
    // report carries the one flag and not one counter
    static uint8_t got[4096];
    static uint8_t want[4096];
    tblv1::Cfg fresh;
    tblv1::CfgReset( fresh );
    const int64_t got_bytes = cfg_projection( value, got, sizeof( got ) );
    const int64_t want_bytes = cfg_projection( fresh, want, sizeof( want ) );
    if ( got_bytes != want_bytes || got_bytes < 0 || memcmp( got, want, (size_t) got_bytes ) != 0 )
    {
        printf( "FAIL %s: a body that ends early decoded something\n", name );
        failures++;
    }
    if ( !report.malformed || report.refused || report.unknown != 0 || report.kind_mismatch != 0 ||
         report.widened != 0 || report.clamped != 0 || report.duplicate != 0 )
    {
        printf( "FAIL %s: a body that ends early counted an event\n", name );
        failures++;
    }
    // and Load says the same thing the verdict does
    tblv1::Cfg again;
    tblv1::CfgReset( again );
    tblv1::TableReport second;
    if ( tblv1::CfgLoad( again, wire, bytes, &second ) ) { printf( "FAIL %s: Load returned true on damage\n", name ); failures++; }
}

// a body of `fields` int32-shaped fields, ended, with `trailing` bytes no
// field claims after the terminator
static int64_t cfg_body_of( int fields, int trailing, uint8_t * out )
{
    static const char * names[6] = { "a", "b", "mode", "name", "inner", "items" };
    WireBuilder w;
    for ( int f = 0; f < fields; f++ )
    {
        w.field( names[f % 6], 4 ); // int32 at every position: a mismatch where the field is not int32
        w.u32( (uint32_t) ( f + 1 ) );
    }
    w.end();
    for ( int t = 0; t < trailing; t++ ) { w.u8( 0x22 ); }
    return w.finish( out );
}

static void test_body_ends_early_is_the_read_s_own_answer()
{
    static uint8_t wire[4096];
    static uint8_t forged[4096];

    // the writer's own wire, populated enough to have framing worth cutting
    tblv1::Cfg src;
    tblv1::CfgReset( src );
    src.a = 42;
    src.b = 3.25f;
    src.mode = tblv1::Mode::Alpha;
    memcpy( src.name, "hello", 5 );
    src.name_length = 5;
    src.inner.factor = 7.5f;
    src.items_count = 3;
    src.items[0] = 1; src.items[1] = 2; src.items[2] = 250;
    src.grade = tblv1::Grade::Gold;
    src.slots_count = 2;
    src.slots[0] = 11; src.slots[1] = 22;
    src.tier = 9;
    src.tier_present = true;
    const int64_t bytes = tblv1::CfgSave( src, wire, sizeof( wire ) );
    CHECK( bytes > 0 );
    check_ends_early_rule( "whole", wire, bytes );

    tblv1::TableIdTable table;
    int64_t body_bytes = 0;
    CHECK( tblv1::TableOpen( wire, bytes, table, body_bytes ) == tblv1::TableOpenOk );

    // A TERMINATOR PLANTED AT EVERY BYTE OF THE BODY: field boundaries, kind
    // bytes, lengths and payloads alike, so the cut lands mid-field as often
    // as it lands between two
    for ( int64_t k = 0; k < body_bytes; k++ )
    {
        memcpy( forged, wire, (size_t) bytes );
        forged[1 + k] = 0x00;
        char name[64];
        snprintf( name, sizeof( name ), "terminator_at_%d", (int) k );
        check_ends_early_rule( name, forged, bytes );
    }

    // AND EVERY SINGLE-BYTE MUTATION OF THE WHOLE FILE: lengths that run long,
    // references above the count, kinds outside the set, numbers that are not
    // canonical
    {
        const uint8_t values[6] = { 0x00, 0x01, 0x02, 0x7F, 0x80, 0xFF };
        for ( int64_t i = 0; i < bytes; i++ )
        {
            for ( int v = 0; v < 6; v++ )
            {
                ...
                forged[i] = values[v];
                char name[64];
                snprintf( name, sizeof( name ), "byte_%d_%02x", (int) i, values[v] );
                check_ends_early_rule( name, forged, bytes );
            }
        }
    }

    // EVERY FIELD BOUNDARY, BUILT FIELD BY FIELD, with and without the bytes
    // that make the end an early one
    for ( int fields = 0; fields <= 6; fields++ )
    {
        for ( int trailing = 0; trailing < 4; trailing++ )
        {
            const int64_t n = cfg_body_of( fields, trailing, forged );
            char name[64];
            snprintf( name, sizeof( name ), "fields_%d_trailing_%d", fields, trailing );
            check_ends_early_rule( name, forged, n );
        }
    }

    // THE PINNED ROWS. The rule above says the two walks agree; these say what
    // they agree ON, so a change to both at once still has to move a number
    // somebody wrote down.
    {
        // no trailing byte: the body ends where the file's table begins, and
        // the read is ordinary — one kind mismatch for `b` under kind 4
        const int64_t n = cfg_body_of( 2, 0, forged );
        tblv1::Cfg value;
        tblv1::CfgReset( value );
        tblv1::TableReport report;
        CHECK( tblv1::CfgLoadVerdict( value, forged, n, &report ) == tblv1::TableOpenOk );
        CHECK( value.a == 1 && !report.malformed );
        CHECK( report.kind_mismatch == 1 && report.unknown == 0 );

        // ONE BYTE MORE, and nothing is decoded at all: `a` reads its declared
        // default, the mismatch is not counted, and the one flag stands
        const int64_t m = cfg_body_of( 2, 1, forged );
        tblv1::Cfg after;
        tblv1::CfgReset( after );
        tblv1::TableReport damaged;
        CHECK( tblv1::CfgLoadVerdict( after, forged, m, &damaged ) == tblv1::TableOpenDamaged );
        CHECK( after.a == 5 ); // the declared default
        CHECK( damaged.malformed && damaged.kind_mismatch == 0 && damaged.unknown == 0 );
        CHECK( !damaged.refused && damaged.clamped == 0 && damaged.duplicate == 0 );
    }
    {
        // AN UNKNOWN ID BEFORE THE EARLY END is not counted either: the file
        // carries no readable body, so it carries no §4 event
        WireBuilder w;
        w.field( "a_field_this_build_cannot_name", 4 );
        w.u32( 7 );
        w.field( "a", 4 );
        w.u32( 11 );
        w.end();
        int64_t n = w.finish( forged );
        tblv1::Cfg value;
        tblv1::CfgReset( value );
        tblv1::TableReport report;
        CHECK( tblv1::CfgLoadVerdict( value, forged, n, &report ) == tblv1::TableOpenOk );
        CHECK( report.unknown == 1 && value.a == 11 && !report.malformed );

        w.u8( 0x77 ); // one byte no field claims, after the terminator
        n = w.finish( forged );
        tblv1::Cfg after;
        tblv1::CfgReset( after );
        tblv1::TableReport damaged;
        CHECK( tblv1::CfgLoadVerdict( after, forged, n, &damaged ) == tblv1::TableOpenDamaged );
        CHECK( damaged.malformed && damaged.unknown == 0 && after.a == 5 );
        check_ends_early_rule( "unknown_then_early_end", forged, n );
    }
    {
        // A RESERVED ID BEFORE THE EARLY END. The build version's id in a FILE
        // body stops the reading walk where the framing walk sees a field like
        // any other, so the two disagree about the body and agree about the
        // file: it ends early, and an early end is damage whatever else was
        // wrong with it (docs/SPEC-TABLES.md §3.1, §3.3).
        for ( int trailing = 0; trailing < 3; trailing++ )
        {
            WireBuilder w;
            w.field( "a", 4 );
            w.u32( 77 );
            w.field( 0xFFFFFFFFFFFFFFFEull, 9 ); // the build version's id, and eight bytes
            w.u64( 1 );
            w.end();
            for ( int t = 0; t < trailing; t++ ) { w.u8( 0x11 ); }
            const int64_t n = w.finish( forged );
            tblv1::Cfg value;
            tblv1::CfgReset( value );
            tblv1::TableReport report;
            const tblv1::TableOpenVerdict verdict = tblv1::CfgLoadVerdict( value, forged, n, &report );
            if ( trailing == 0 )
            {
                // the body ends where it should, so this is a body that STOPPED
                CHECK( verdict == tblv1::TableOpenBodyStopped );
                CHECK( report.malformed && value.a == 77 ); // what it decoded is kept
            }
            else
            {
                CHECK( verdict == tblv1::TableOpenDamaged );
                CHECK( report.malformed && value.a == 5 ); // and nothing is
            }
            char name[64];
            snprintf( name, sizeof( name ), "reserved_id_trailing_%d", trailing );
            check_ends_early_rule( name, forged, n );
        }
    }
    {
        // A BODY THAT RUNS LONG: a length that claims more than the body holds
        // is framing damage inside the field, never an early end
        for ( int over = 1; over <= 8; over++ )
        {
            WireBuilder w;
            w.field( "name", 12 );
            w.leb( (uint64_t) ( 4 + over ) );
            w.u32( 0x41414141 );
            w.end();
            const int64_t n = w.finish( forged );
            tblv1::Cfg value;
            tblv1::CfgReset( value );
            tblv1::TableReport report;
            CHECK( tblv1::CfgLoadVerdict( value, forged, n, &report ) == tblv1::TableOpenBodyStopped );
            CHECK( report.malformed );
            char name[64];
            snprintf( name, sizeof( name ), "runs_long_by_%d", over );
            check_ends_early_rule( name, forged, n );
        }
    }
    {
        // THE EMPTY BODY, THE BARE TERMINATOR, AND ONE BYTE PAST IT
        WireBuilder w;
        int64_t n = w.finish( forged );
        tblv1::Cfg value;
        tblv1::CfgReset( value );
        tblv1::TableReport report;
        CHECK( tblv1::CfgLoadVerdict( value, forged, n, &report ) == tblv1::TableOpenBodyStopped );
        check_ends_early_rule( "empty_body", forged, n );

        w.end();
        n = w.finish( forged );
        tblv1::CfgReset( value );
        tblv1::TableReport clean;
        CHECK( tblv1::CfgLoadVerdict( value, forged, n, &clean ) == tblv1::TableOpenOk );
        CHECK( !clean.malformed && value.a == 5 );
        check_ends_early_rule( "terminator_only", forged, n );

        w.u8( 0x99 );
        n = w.finish( forged );
        tblv1::CfgReset( value );
        tblv1::TableReport damaged;
        CHECK( tblv1::CfgLoadVerdict( value, forged, n, &damaged ) == tblv1::TableOpenDamaged );
        CHECK( damaged.malformed );
        check_ends_early_rule( "terminator_plus_junk", forged, n );
    }
}
...
int main()
{
    ...
    test_body_ends_early_is_the_read_s_own_answer();
