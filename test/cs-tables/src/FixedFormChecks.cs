...
static partial class Program
{
    ...
    // and FU2. UT holds the same-schema compiled-vs-identity half. This pair is
    // the trailing-field compile trigger: FU2 appends extra, so a read of FU1's
    // bytes is a compiled plan rather than the identity one — the path where an
    // overloaded plan lane dropped a union arm's text (reference-fix 12).
    //
    // ONE PATH: FuRootFixedLoad. Hash chooses the plan. No second reader.
    static void TestFixedFu1Fu2Case()
    {
        ...
        labelled.Note = 44;
        labelled.Pick.Type = FU1.PickType.Labelled; // the SECOND arm
        ...
        // THE IDENTITY READ, through FU1's own FuRootFixedLoad.
        ...
        // THE COMPILED READ, of the same bytes through FU2 — extra is the only
        // thing that changes the hash, and that is enough.
