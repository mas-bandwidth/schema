...
fn get_u32(buf: &[u8]) -> u32 {
    u32::from_le_bytes(buf[..4].try_into().expect("four bytes"))
}

fn entry_at_mut(f: &mut [u8], k: usize) -> &mut [u8] {
    let at = LAYOUT_AT + 4 + k * 17;
    &mut f[at..at + 17]
}
...
fn put_entry(layout: &mut Vec<u8>, id: u64, kind: u8, size: u32, children: u32) {
    ...
    layout.push(kind);
    layout.extend_from_slice(&size.to_le_bytes());
    layout.extend_from_slice(&children.to_le_bytes());
}

fn file_of(layout: &[u8]) -> Vec<u8> {
    let mut f = vec![0u8; 20];
    f[0] = 3;
    let hash = tblfx1::table_fixed_hash(layout);
    put_u64(&mut f[HASH_AT..HASH_AT + 8], hash);
    put_u32(&mut f[16..20], layout.len() as u32);
    f.extend_from_slice(layout);
    f
}

fn refuses(broken: &[u8], want: tblfx1::TableFixedReason, what: &str) {
    let mut v = [tblfx1::FxRootRow::default(); 8];
    let mut plan = [tblfx1::TableFixedEntry::default(); 1024];
    let mut remap = [0u16; 1024];
    let mut report = tblfx1::TableFixedReport::default();
    let n = tblfx1::fx_root_fixed_load(&mut v, broken, &mut plan, &mut remap, &mut report);
    check(n.is_none() && report.refused && report.reason == want, what);
    check(
        report.unknown == 0 && report.kind_mismatch == 0 && report.widened == 0 && report.clamped == 0 && !report.malformed,
...
fn the_negative_controls(dir: &str) {
    ...
    // 2c. THE SEVEN NAMED LAYOUT REFUSAL RULES (docs/SPEC-TABLES.md §3.4, §8368-8375).
    // Every break is tested against a file that loads cleanly when unbroken.
    {
        let good = slurp(dir, "fx2.bin");
        {
            let mut v = [tblfx1::FxRootRow::default(); 8];
            let mut plan = [tblfx1::TableFixedEntry::default(); 1024];
            let mut remap = [0u16; 1024];
            let mut r = tblfx1::TableFixedReport::default();
            check(
                tblfx1::fx_root_fixed_load(&mut v, &good, &mut plan, &mut remap, &mut r) == Some(1) && !r.refused,
        ...
        // 1. THE ENTRY COUNT FITS THE LAYOUT'S LENGTH EXACTLY
        {
            ...
            let count = get_u32(&f[LAYOUT_AT..LAYOUT_AT + 4]);
            put_u32(&mut f[LAYOUT_AT..LAYOUT_AT + 4], count + 1);
            refuses(&f, tblfx1::TableFixedReason::LayoutCountMismatch, "RULE: the entry count fits the layout length exactly");
        }
        {
            let mut f = good.clone();
            put_u32(&mut f[LAYOUT_AT..LAYOUT_AT + 4], 0);
            refuses(&f, tblfx1::TableFixedReason::LayoutCountMismatch, "RULE: an entry count of zero is not a layout");
        }

        // 2. EVERY KIND IS IN THE CLOSED SET
        {
            let mut f = good.clone();
            entry_at_mut(&mut f, 1)[8] = 200; // a kind no form byte defines
            refuses(&f, tblfx1::TableFixedReason::LayoutKindUnknown, "RULE: a kind outside the closed set is REFUSED, not skipped");
        }

        // 3. A KIND IS USED AS ITS DEFINITION ALLOWS — here, the ROOT is a table
        {
            let mut f = good.clone();
            entry_at_mut(&mut f, 0)[8] = 14; // an array as the root of a record
            refuses(&f, tblfx1::TableFixedReason::LayoutKindInvalid, "RULE: the root entry is a TABLE");
        }

        // 4. A CONSTANT SIZE MATCHES ITS KIND
        {
            let mut f = good.clone();
            put_u32(&mut entry_at_mut(&mut f, 1)[9..13], 5); // uint32 leaf in 5 bytes
            refuses(&f, tblfx1::TableFixedReason::LayoutSizeMismatch, "RULE: a constant size its kind does not admit");
        }
        {
            let mut f = good.clone();
            let body = get_u32(&entry_at_mut(&mut f, 0)[9..13]);
            put_u32(&mut entry_at_mut(&mut f, 0)[9..13], body + 4);
            refuses(&f, tblfx1::TableFixedReason::LayoutSizeMismatch, "RULE: a table's size is the sum of its fields'");
        }

        // 5. THE PRE-ORDER CHILD WALK CONSUMES EXACTLY THE ENTRIES
        {
            let mut f = good.clone();
            let kids = get_u32(&entry_at_mut(&mut f, 0)[13..17]);
            put_u32(&mut entry_at_mut(&mut f, 0)[13..17], kids + 1);
            refuses(&f, tblfx1::TableFixedReason::LayoutTreeUnclosed, "RULE: the tree runs out of layout");
        }
        {
            ...
            refuses(&f, tblfx1::TableFixedReason::LayoutTreeUnclosed, "RULE: the layout outlasts the tree");
        }

        // 6. THE TOTAL RECORD SIZE IS WITHIN 65536 AND DOES NOT OVERFLOW
        {
            let mut f = good.clone();
            put_u32(&mut entry_at_mut(&mut f, 0)[9..13], 65537);
            refuses(&f, tblfx1::TableFixedReason::LayoutRecordTooLarge, "RULE: a record size past 65536");
        }
        {
            let mut f = good.clone();
            put_u32(&mut entry_at_mut(&mut f, 1)[9..13], 0xFFFFFFFF);
            refuses(&f, tblfx1::TableFixedReason::LayoutRecordTooLarge, "RULE: a size that would overflow the sum");
        }
        ...
        // 7. NOTHING NESTED PAST THE READER'S WALK BOUND
        {
            let depth = 4096u32;
            let mut layout = Vec::new();
            layout.extend_from_slice(&(depth + 1).to_le_bytes());
            for i in 0..depth {
                put_entry(&mut layout, 1, if i == 0 { 13 } else { 35 }, depth - i, 1);
            }
            put_entry(&mut layout, 2, 1, 1, 0); // a bool at the bottom
            let f = file_of(&layout);
            refuses(&f, tblfx1::TableFixedReason::LayoutTooDeep, "RULE: a nesting depth past the walk's own bound");
        }

        // AND THE RESIDUE: bytes that are not a layout at all
        {
            let f = file_of(&[0u8; 2]);
            refuses(&f, tblfx1::TableFixedReason::LayoutMalformed, "RULE: fewer bytes than a header is layout_malformed");
        }
    }
