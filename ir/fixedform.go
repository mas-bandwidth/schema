...
func (w *fixedWalk) push(e FixedBlockEntry, dst FixedDstSpec) {
	e.Dst = dst
...
func FixedWalkRoot(st *Struct) []FixedBlockEntry {
	w := &fixedWalk{}
	w.push(FixedBlockEntry{ID: TableWireId(st.WireName()), Kind: TableKindTable, Size: FixedTypeBytes(st), Children: len(st.Fields), Note: st.Name}, FixedDstSpec{})
	for _, f := range st.Fields {
...
func fixedWalkPayload(w *fixedWalk, owner string, f *Field, id uint64, size int64) {
	...
	case f.KeyEnum != "":
		...
		w.push(FixedBlockEntry{ID: id, Kind: TableKindKeyed, Size: size, Children: 2, Note: f.Name},
	...
	case f.Array != ArrayNone:
		...
		w.push(FixedBlockEntry{ID: id, Kind: TableKindArray, Size: size, Children: 1, Note: f.Name}, spec)
		fixedWalkElement(w, f, 0, "element", nil)
	case f.Type.Kind == TBytes:
		...
		w.push(FixedBlockEntry{ID: id, Kind: TableKindArray, Size: size, Children: 1, Note: f.Name},
			FixedDstSpec{Dst: term(owner, f.Name+"_length"), Stride1: true, Aux: term(owner, f.Name), Counted: 1, Arg: 3})
		w.push(FixedBlockEntry{ID: 0, Kind: TableKindU8, Size: 1, Children: 0, Note: "u8"}, FixedDstSpec{})
	case f.Type.Kind == TString:
		w.push(FixedBlockEntry{ID: id, Kind: TableKindString, Size: size, Children: 0, Note: f.Name},
			FixedDstSpec{Dst: term(owner, f.Name+"_length"), Aux: term(owner, f.Name), Arg: 1})
	case f.Type.Kind == TWString:
		w.push(FixedBlockEntry{ID: id, Kind: TableKindWstring, Size: size, Children: 0, Note: f.Name},
...
func fixedWalkElement(w *fixedWalk, f *Field, id uint64, note string, dst []FixedTerm) {
	...
	if f.Type.Kind == TNamed {
		switch r := f.Type.Ref.(type) {
		case *Struct:
			w.push(FixedBlockEntry{ID: id, Kind: TableKindTable, Size: size, Children: len(r.Fields), Note: note}, FixedDstSpec{Dst: dst})
			for _, sub := range r.Fields {
				fixedWalkField(w, r.Name, sub)
			}
			return
		case *Union:
			w.push(FixedBlockEntry{ID: id, Kind: TableKindUnion, Size: size, Children: len(r.Variants), Note: note},
	...
	w.push(FixedBlockEntry{ID: id, Kind: TableWireScalarKind(f), Size: size, Children: 0, Note: note}, FixedDstSpec{Dst: dst})
}

func fixedWalkEnum(w *fixedWalk, e *Enum, id uint64, note string, dst []FixedTerm) {
	w.push(FixedBlockEntry{ID: id, Kind: TableKindEnum, Size: int64(e.StorageBits / 8), Children: len(e.Variants), Note: note}, FixedDstSpec{Dst: dst})
	for i := range e.Variants {
		w.push(FixedBlockEntry{ID: TableWireId(e.VariantWireName(i)), Kind: TableKindNoPayload, Size: 0, Children: 0, Note: e.Variants[i]}, FixedDstSpec{})
	}
}
