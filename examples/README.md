...
| `VoidArms.schema` | the payload-free union arm | SPEC §4.8's bare-name arm on the packet wire — two payload-free arms bracketing two typed ones, carried by a message behind a 3-bit lead so the tag is not byte aligned, in a union field, a second union field and a counted array. Its own file because `Wire.schema`'s `ProbeShape` pins an out-of-set tag value as a nine-leg read-refusal control, and an arm added there would make that value legal |
...
The independent packet wide-string unit is
[`../examples-wide/WideText.schema`](../examples-wide/WideText.schema).
All nine targets carry it. `make packet-wire-nine` stages it apart from the
table-wide baseline and runs the packet defaults, UTF-8 and UTF-16 checks
without changing this corpus's protocol identity.
