...
| `Types.schema` | every `type` | quantized-int types, prefix arrays `[..N]T`, bool-gated `if` with a `flags` field, and `RigidBody` — the serialize README's own example, schema-specified: the velocity group rides only when `!at_rest`, and §5's zero-on-untaken-branch rule is the hand-written `else if ( Stream::IsReading )` zeroing made contract |
| `Vector.schema` | the mapped continuous vector | the `cpp_native`/`cpp_include` mapping on `Vec3` — its own file so the mapping RIDES: the references live in `Types.schema`'s `RigidBody`, another file of the unit, because the mapped header derives from the basis in `Vector.h` and a mapped reference in that same header would be circular (SPEC §4.2) |
