...
program built from that schema, in four languages at once. Report these.
...
- **Any divergence between the four languages on the above** — if C++ refuses
...
- A native Go fuzz harness drives parse → check → generate across all four
  backends. Every crasher ever found is committed as a permanent regression
  input and re-run on every push.
- The cross-language corpus generates the same schemas in C++, C#, Go and Rust
