...
tables-c-ref-ordinal-negative-control:
	...
	@if SCHEMA_SLOW=1 go test -overlay build/c-ref-ordinal-nc/overlay.json -count=1 ./compiler \
...
tables-c-message-negative-control:
	...
	@if SCHEMA_SLOW=1 go test -count=1 -overlay=build/c-message-negative/overlay.json ./compiler -run '^TestCTableMessageSave$$' > build/c-message-negative/log 2>&1; then \
...
tables-c-retain-negative-control:
	...
	@if SCHEMA_SLOW=1 go test -count=1 -overlay=build/c-retain-negative/overlay.json ./compiler -run '^TestCTableRetainFile$$' > build/c-retain-negative/log 2>&1; then \
