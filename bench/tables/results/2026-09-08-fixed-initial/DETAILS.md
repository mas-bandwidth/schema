# Fixed Table — supporting details
...
These measurements guide each language's fastest correct implementation;
...
beside it. The results page uses one blended percentage per language, normalized to the fastest measured implementation. It does not compare machines or corpora.
...
The blended cost follows the packet README: the measured round-trip combines write and read, so averaging those two costs divides every language by the same factor of two. For the percentage, use fastest round-trip rate / language round-trip rate × 100. Do not add write to round-trip and count writing twice.
