...
**One line in `bench/tables/legs.txt` plus one command.** Nothing else moves:
no case in `run.sh`, no flag, no list anywhere else.
...
3. Add `<language> <command>` to `bench/tables/legs.txt`.
