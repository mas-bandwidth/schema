...
`java` is the first. It is table-only because its packet leg is not this
driver's to run, not because anybody chose to skip it: `bench/java/Main.java`
is the type board's runner, with neither `--gate` nor `--iterations` — the two
flags this driver passes on every invocation — and the paired set is the four
the board is defined over. Filling the driver's second wire with a fabricated
packet row would be inventing a measurement, so the driver accepts a table-only
row instead; Java's packet number is the type board's, from its own runner over
the same sixty-four records. `JAVA` and `JAVAC` name the JDK (the repository-
local pin under `dist/` when it is there, as in make/java.mk).
...
A request is one shape or the other and never a mixture: `-langs java` is asked
for alone, `-mode run` refuses it, and its fast summary prints the table wire's
own cost with **NO RATIO** written where the percentages would be. Because Java
has no form-1 table wire — the fixed form is its first — its leg measures the
FIXED form and names its rows `bench_fixed` — the same corpus, the same corpus
id and the same `table` family as every other table row (docs/SPEC-TABLES.md
§3.4).
